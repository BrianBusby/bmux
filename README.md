...
`ProvenanceEngineSQLite` remains an internal package target, not a public library product. Its repository actor currently bootstraps the first engine-owned `provenance_events` table plus current-state projections for sessions, repositories, worktrees, session relationships, external identities, work items, contributions, checkpoints, change sets, file changes, and validation runs. It satisfies the in-process `ProvenanceEngineClient` contract internally over the SQLite append, lifecycle recording, worktree list, session tree, file explanation, current context, and health paths.
