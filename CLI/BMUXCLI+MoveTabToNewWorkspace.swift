...
extension BMUXCLI {
    ...
    Usage: bmux move-tab-to-new-workspace [--tab <id|ref|index>] [--surface <id|ref|index>] [--workspace <id|ref|index>] [--window <id|ref|index>] [--title <text>] [--focus <true|false>]

    Move a tab into a newly created workspace in the same window.

    Flags:
      --tab <id|ref|index>         Target tab (accepts tab:<n> or surface:<n>; default: $BMUX_TAB_ID, then $BMUX_SURFACE_ID, then focused tab)
      --surface <id|ref|index>     Alias for --tab
      --workspace <id|ref|index>   Workspace context (default: current/$BMUX_WORKSPACE_ID)
    ...
    Example:
      bmux move-tab-to-new-workspace --tab tab:2
      bmux move-tab-to-new-workspace --surface surface:3 --title "build logs"
    """
}
