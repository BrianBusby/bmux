...
extension BMUXCLI {
    ...
    private func buildMemoryPayload(
        ...
        do {
            ...
            throw CLIError(message: String(localized: "cli.memory.error.diagnosticsUnsupported", defaultValue: "bmux memory requires a running bmux build that supports memory diagnostics"))
        }
    }
    ...
    private func renderMemoryText(
        ...
        lines.append("  \(appName.isEmpty ? "bmux" : appName) pid=\(appPID)")
