...
/// `isActionable` flag.
///
/// This is the single source of truth behind both the running `bmux` CLI
/// (`bmux hooks feed …`) and the `FeedEventClassificationTests` regression
/// suite — the file is compiled into the `bmux-cli` target and the
/// `bmuxTests` target so the pure decision can be unit-tested without
...
/// is the bug behind https://github.com/manaflow-ai/bmux/issues/4985.
struct FeedEventClassifier {
    ...
    private static let feedEventSemanticRegistry: [String: [String: FeedEventSemantic]] = [
        ...
        "codex": [
            ...
            // use Codex's auto-review path instead of blocking on bmux Feed.
