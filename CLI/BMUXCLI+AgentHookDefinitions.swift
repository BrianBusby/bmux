...
extension BMUXCLI {
    ...
    struct AgentHookDef {
        ...
        let sessionStoreSuffix: String // e.g. "cursor" -> ~/.bmuxterm/cursor-hook-sessions.json
        let disableEnvVar: String   // e.g. "BMUX_CURSOR_HOOKS_DISABLED"
        let hookMarker: String      // Marker in commands: "bmux hooks cursor"
        ...
        /// https://github.com/manaflow-ai/bmux/issues/5000.
        ///
        ...
        /// `agentEvent` that invokes `bmux hooks feed --source <name>`
        ...
        /// Kiro applies its hooks only when run as the `bmux` agent.
    ...
    static func hookCommandString(for def: AgentHookDef, event: AgentHookDef.HookEvent) -> String {
        let command = "bmux hooks \(def.name) \(event.bmuxSubcommand)"
        let inline: String
        if def.name == "codex", codexHookCanRunFireAndForget(event.bmuxSubcommand) {
            inline = codexFireAndForgetAgentHookShellCommand(command, for: def)
        } else {
            inline = agentHookShellCommand(command, for: def)
        }
        if def.name == "codex" {
            return codexPersistentHookScriptCommand(inline, eventTag: event.bmuxSubcommand)
        }
        return inline
    }

    /// Wraps a codex persistent hook command as a `#!/bin/sh` script file in the
    /// bmux-owned hooks dir and returns its path. A bare executable path runs
    ...
    static func feedHookCommandString(for def: AgentHookDef, agentEvent: String) -> String {
        ...
        case .kiroAgentJSON:
            inline = exitTwoPropagatingAgentHookShellCommand(
                "bmux hooks feed --source \(def.name) --event \(agentEvent)",
                for: def,
    ...
    static func codexOptimizerHookCommandString(for def: AgentHookDef) -> String {
        let inline = agentHookShellCommand(
            "bmux hooks codex optimize-pre-tool-use",
            for: def,
    ...
    private static let grokPinnedHookMarker = "bmux-grok-hook-v2"
    private static let antigravityPinnedHookMarker = "bmux-antigravity-hook-v2"
    ...
    private static func agentHookShellCommand(
        ...
        return "bmux_cli=\"${BMUX_BUNDLED_CLI_PATH:-}\"; if [ -z \"$bmux_cli\" ] || [ ! -x \"$bmux_cli\" ]; then bmux_cli=\"$(command -v bmux 2>/dev/null || true)\"; fi; if [ -n \"$BMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && [ -n \"$bmux_cli\" ]; then { if [ -n \"${BMUX_SOCKET_PATH:-}\" ]; then \"$bmux_cli\" --socket \"$BMUX_SOCKET_PATH\" \(routedArguments); else \"$bmux_cli\" \(routedArguments); fi; } || \(noOpSnippet); else \(noOpSnippet); fi"
    }
    ...
    private static func exitTwoPropagatingAgentHookShellCommand(
        ...
        let routedArguments = command.hasPrefix("bmux ") ? String(command.dropFirst("bmux ".count)) : command
        let noOpSnippet = shellNoOpSnippet(noOpCommand)
        return "bmux_cli=\"${BMUX_BUNDLED_CLI_PATH:-}\"; if [ -z \"$bmux_cli\" ] || [ ! -x \"$bmux_cli\" ]; then bmux_cli=\"$(command -v bmux 2>/dev/null || true)\"; fi; if [ -n \"$BMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && [ -n \"$bmux_cli\" ]; then if [ -n \"${BMUX_SOCKET_PATH:-}\" ]; then \"$bmux_cli\" --socket \"$BMUX_SOCKET_PATH\" \(routedArguments); else \"$bmux_cli\" \(routedArguments); fi; status=$?; if [ \"$status\" -eq 2 ]; then exit 2; fi; if [ \"$status\" -ne 0 ]; then \(noOpSnippet); fi; else \(noOpSnippet); fi"
    }
    ...
    private static func pinnedAgentHookShellCommand(
        ...
        let shellTraceExit = pinnedHookShellTraceCommand(
            ...
            routedArguments: routedArguments,
            socketPath: socketPath,
            statusExpression: "$bmux_hook_status"
        )
        ...
        if let cliPath = pinnedAgentHookCLIPath() {
            ...
            dispatch = "if [ -x \(quotedCLIPath) ]; then \(primaryInvocation); elif command -v bmux >/dev/null 2>&1; then \(fallbackInvocation); else \(noOpSnippet); fi"
        } else {
            dispatch = "command -v bmux >/dev/null 2>&1 && \(fallbackInvocation) || \(noOpSnippet)"
        }
        return ": \(pinnedHookMarker(for: def)); \(shellTraceStart); printenv \(def.disableEnvVar) | grep -qx 1 && { \(shellTraceDisabled); \(noOpCommand); } || { \(dispatch); bmux_hook_status=$?; \(shellTraceExit); exit $bmux_hook_status; }"
    }
    ...
    private static func pinnedAgentHookCLIPath(
        ...
        if let bundledPath = normalizedHookInstallValue(env["BMUX_BUNDLED_CLI_PATH"]) {
    ...
    private static func pinnedAgentHookSocketPath(
        ...
        if let socketPath = normalizedHookInstallValue(env["BMUX_SOCKET_PATH"]) {
            return NSString(string: socketPath).expandingTildeInPath
        }
        guard let tag = normalizedHookInstallValue(env["BMUX_TAG"]) else {
            return nil
        }
        let slug = tag
            .lowercased()
        ...
        return "/tmp/bmux-debug-\(slug).sock"
    }
    ...
    private static func pinnedHookShellTraceLogPath(socketPath: String?) -> String {
        guard let socketPath else {
            return "/tmp/bmux-debug.log"
        }
        let socketName = URL(fileURLWithPath: socketPath).lastPathComponent
        if socketName.hasPrefix("bmux-debug-"), socketName.hasSuffix(".sock") {
    ...
    static func isBmuxOwnedHookCommand(_ command: String, for def: AgentHookDef, includeLegacy: Bool = true) -> Bool {
        ...
        return includeLegacy && isLegacyBmuxOwnedHookCommand(command, for: def)
    }

    private static func isLegacyBmuxOwnedHookCommand(_ command: String, for def: AgentHookDef) -> Bool {
        // Codex also had older top-level codex-hook/feed-hook commands.
        // Other generic agents can have stale `bmux hooks ...` files from
        // earlier integration attempts, and setup should be able to prune them.
        return legacyBmuxCommandTokenLists(from: command, for: def).contains { tokens in
            isLegacyBmuxOwnedHookTokens(tokens, for: def)
        }
    }

    private static func isLegacyBmuxOwnedHookTokens(_ tokens: [String], for def: AgentHookDef) -> Bool {
        guard !tokens.isEmpty,
              URL(fileURLWithPath: tokens[0]).lastPathComponent == "bmux"
        else {
            return false
        }
    ...
    private static func legacyBmuxCommandTokenLists(from command: String, for def: AgentHookDef) -> [[String]] {
        if let bundledTokens = bundledCLIBmuxCommandTokenLists(from: command), !bundledTokens.isEmpty {
            return bundledTokens
        }

        let guardedPrefixes = [
            "[ -n \"$BMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && command -v bmux >/dev/null 2>&1 && ",
            "[ \"$\(def.disableEnvVar)\" != \"1\" ] && command -v bmux >/dev/null 2>&1 && ",
        ]
        let fallbackSuffix = " || echo '{}'"
        var body = command
        if def.name == "grok" {
            let grokPrefix = "printenv \(def.disableEnvVar) | grep -qx 1 && echo '{}' || { command -v bmux >/dev/null 2>&1 && "
            let grokSuffix = " || echo '{}'; }"
    ...
    private static func bundledCLIBmuxCommandTokenLists(from command: String) -> [[String]]? {
        guard command.contains("BMUX_BUNDLED_CLI_PATH"),
              command.contains("bmux_cli="),
              command.contains("command -v bmux") else {
            return nil
        }

        let invocationPrefixes = [
            "\"$bmux_cli\" --socket \"$BMUX_SOCKET_PATH\" ",
            "\"$bmux_cli\" ",
        ]
        ...
        for prefix in invocationPrefixes {
            ...
            while let prefixRange = command.range(of: prefix, range: searchStart..<command.endIndex) {
                ...
                if !args.isEmpty,
                   ...
                   !args.contains("`") {
                    tokenLists.append(
                        ["bmux"] + args.split(whereSeparator: { $0 == " " || $0 == "\t" }).map(String.init)
                    )
                }
    ...
    static func hookMarkers(for def: AgentHookDef) -> [String] {
        ...
        if def.name == "codex" {
            markers.append("bmux codex-hook")
        }
        return markers
    }
    ...
    static func feedHookMarkers(for def: AgentHookDef) -> [String] {
        var markers = ["bmux hooks feed --source"]
        if def.name == "codex" {
            markers.append("bmux feed-hook --source")
        }
        return markers
    }
}
