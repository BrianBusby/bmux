...
// bmux-pi-session-extension-marker v2
// Bridges Pi session lifecycle, tool telemetry, notifications, and resume bindings into bmux.
// Installed by `bmux hooks pi install` or `bmux hooks setup`.
...
function safeBmuxEnvKey(key: string): boolean {
  if (key.startsWith("BMUX_TEST_PI_")) return !secretLikeEnvKey(key);
  if (key.startsWith("BMUX_AGENT_LAUNCH_")) return !secretLikeEnvKey(key);
  if (key === "BMUX_AGENT_HOOK_STATE_DIR") return true;
  if (key === "BMUX_PI_BMUX_BIN" || key === "BMUX_PI_HOOKS_DISABLED") return true;
  if (key === "BMUX_SURFACE_ID" || key === "BMUX_WORKSPACE_ID" || key === "BMUX_WINDOW_ID") return true;
  if (key === "BMUX_PANE_ID" || key === "BMUX_TAB_ID" || key === "BMUX_PANEL_ID") return true;
  if (key === "BMUX_SOCKET" || key === "BMUX_SOCKET_PATH") return true;
  if (key === "BMUX_BUNDLE_ID" || key === "BMUX_BUNDLED_CLI_PATH") return true;
  if (key === "BMUX_CLI_SENTRY_DISABLED" || key === "BMUX_DEBUG_LOG") return true;
  return false;
}

function shouldPreserveEnvKey(key: string): boolean {
  if (safeBmuxEnvKey(key)) return true;
...
function hookEnvironment(cwd: string, includeSocketPassword = false): NodeJS.ProcessEnv {
  ...
  // Only bmux CLI children need the socket credential; keep it out of the generic allowlist.
  if (includeSocketPassword) {
    const socketPassword = process.env.BMUX_SOCKET_PASSWORD;
    if (socketPassword) env.BMUX_SOCKET_PASSWORD = socketPassword;
  }
  if (!env.BMUX_AGENT_LAUNCH_ARGV_B64) {
    const argv = normalizedLaunchArgv();
    env.BMUX_AGENT_LAUNCH_KIND = "pi";
    env.BMUX_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("pi");
...
function warn(ctx: ExtensionContext | null, message: string, details: Record<string, unknown> = {}): void {
  const payload = { source: "bmux-pi-extension", level: "warning", message, ...details };
  try {
    console.warn(JSON.stringify(payload));
  } catch (_) {
    console.warn(`[bmux-pi-extension] ${message}`);
  }
  const ui = (ctx as unknown as { ui?: { notify?: (message: string, type?: string) => void } } | null)?.ui;
  try {
    ui?.notify?.("bmux Pi integration warning - check the terminal for details", "warning");
  } catch (_) {}
}

function bmuxExecutable(): string {
  return process.env.BMUX_PI_BMUX_BIN || "bmux";
}

function runBmux(args: string[], cwd: string, input?: string): CommandResult {
  try {
    const result = spawnSync(bmuxExecutable(), args, {
      input,
      encoding: "utf8",
