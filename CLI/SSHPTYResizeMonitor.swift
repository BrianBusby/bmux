...
actor SSHPTYResizeMonitor {
    ...
    init(
        ...
        self.source = DispatchSource.makeSignalSource(
            signal: SIGWINCH,
            queue: DispatchQueue(label: "com.bmux.ssh-pty.resize.signal")
        )
        signal(SIGWINCH, SIG_IGN)
        source.setEventHandler { [eventContinuation] in
            let size = BMUXCLI.currentCLITerminalSize()
    ...
    private func drainPendingResizes() async {
        ...
        while true {
            ...
            if sent {
                lastSentSize = size
                let currentSize = BMUXCLI.currentCLITerminalSize()
    ...
    private static func sendResizeBlocking(
        ...
        do {
            ...
            try BMUXCLI.authenticateSocketClientIfNeeded(
                resizeClient,
                explicitPassword: explicitPassword,
                socketPath: socketPath,
