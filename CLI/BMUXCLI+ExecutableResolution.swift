...
extension BMUXCLI {
    ...
    func isBundledProviderExecutable(at path: String) -> Bool {
        let candidate = URL(fileURLWithPath: path, isDirectory: false)
            .standardizedFileURL
            .path
        if isBmuxAppBundleResourceBinChild(candidate) {
            return true
        }
    ...
    func isBmuxClaudeWrapper(at path: String) -> Bool {
    ...
    func isBmuxClaudeCommandShim(at path: String) -> Bool {
        let candidate = URL(fileURLWithPath: path, isDirectory: false)
            .standardizedFileURL
            .path
        let environment = ProcessInfo.processInfo.environment
        let shimPaths = [
            environment["BMUX_CLAUDE_WRAPPER_SHIM"],
        ]
    ...
    func resolveClaudeExecutable(configuredCandidates: [String?], searchPath: String?) -> String? {
        for raw in configuredCandidates {
            ...
            guard FileManager.default.fileExists(atPath: trimmed, isDirectory: &isDirectory),
                  ...
                  !isBmuxClaudeCommandShim(at: trimmed),
                  !isBmuxClaudeWrapper(at: trimmed) else { continue }
    ...
    func resolveClaudeExecutable(searchPath: String?) -> String? {
        resolveExecutableInSearchPath(
            ...
            skip: { self.isBmuxClaudeCommandShim(at: $0) || self.isBmuxClaudeWrapper(at: $0) }
        )
    }
    ...
    /// in `BMUX_CLAUDE_TEAMS_SANDBOXED` so teammate respawns (which run as a separate
    /// `bmux __tmux-compat` process and cannot see this argv) re-apply the same
    ...
    func claudeTeamsExtraEnvVars(commandArgs: [String]) -> [(key: String, value: String)] {
        ...
        if claudeTeamsHasDangerousSkipPermissions(commandArgs: commandArgs) {
            ...
            vars.append((key: "BMUX_CLAUDE_TEAMS_SANDBOXED", value: "1"))
        }
        return vars
    }
    ...
    /// The whole point of `bmux claude-teams` is "just start a team." Claude Code's
    ...
    /// *what*?" — so a plain `bmux claude-teams "make a demo team with 5 subagents"`
    ...
    /// restore command) so that stays canonical; restore re-invokes `bmux
    ...
    var claudeTeamsTeamSpawnGuidance: String {
        """
        You are Claude Code running inside bmux, started with `bmux claude-teams`. \
    ...
    private func providerExecutableSearchDirectories(searchPath: String?) -> [String] {
        ...
        return directories.compactMap { rawDirectory in
            ...
            let standardized = URL(fileURLWithPath: trimmed, isDirectory: true)
                .standardizedFileURL
                .path
            guard !isBmuxAppBundleResourceBinDirectory(standardized) else { return nil }
    ...
    private func isBmuxAppBundleResourceBinDirectory(_ path: String) -> Bool {
    ...
    private func isBmuxAppBundleResourceBinChild(_ path: String) -> Bool {
        bmuxAppBundleResourceBinComponentIndex(path).map { index in
            URL(fileURLWithPath: path, isDirectory: false).standardizedFileURL.pathComponents.count > index + 4
        } ?? false
    }

    private func bmuxAppBundleResourceBinComponentIndex(_ path: String) -> Int? {
