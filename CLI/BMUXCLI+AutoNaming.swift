...
// the bundled CLI target (which drives it from agent hooks) and the bmux-unit
...
/// debounce shape proven by community prior art (manaflow-ai/bmux#2043)
...
/// that would recurse into bmux hooks or the parent agent session while
...
struct AutoNamingEnvironmentPolicy: Sendable {
    /// Exact variables marking a live agent session or bmux terminal; never pass them to the summarizer.
    ...
    func summarizerEnvironment(from env: [String: String]) -> [String: String] {
        env.filter { key, _ in
            if key.hasPrefix("BMUX_") { return false }
