...
extension BMUXCLI {
    private static let ampExtensionMarker = "bmux-amp-session-extension-marker"
    private static let ampExtensionFilename = "bmux-session.ts"
    private static let ampExtensionSource = #"""
// bmux-amp-session-extension-marker v2
// Bridges Amp session lifecycle events into bmux's restorable session store
// AND reports live agent status (idle/thinking/tool calls/done/error) into
// the bmux tab status bar.
// Installed by `bmux hooks amp install` or `bmux hooks setup`.
// DO NOT EDIT MANUALLY. bmux upgrades this file in place.
...
function hookEnvironment(cwd: string): NodeJS.ProcessEnv {
  ...
  if (!env.BMUX_AGENT_LAUNCH_ARGV_B64) {
    const argv = normalizedLaunchArgv();
    env.BMUX_AGENT_LAUNCH_KIND = "amp";
    env.BMUX_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("amp");
    env.BMUX_AGENT_LAUNCH_ARGV_B64 = base64NulSeparated(argv);
    env.BMUX_AGENT_LAUNCH_CWD = cwd || process.cwd();
  }
  return env;
}
...
function sendHook(
  ...
  if (process.env.BMUX_AMP_HOOKS_DISABLED === "1") return;
  if (!process.env.BMUX_SURFACE_ID) return;
  if (!sessionId) return;
  ...
  const bmux = process.env.BMUX_AMP_BMUX_BIN || "bmux";
  try {
    const child = spawn(bmux, ["hooks", "amp", subcommand], {
...
// Fires `bmux set-status` / `bmux clear-status` / `bmux log` so the tab
...
// Short verbs shown in the bmux status bar for each Amp tool.
...
// SF Symbol names rendered inside the bmux status badge.
...
// Pin every bmux call to the workspace this plugin process was launched in.
// bmux sets BMUX_WORKSPACE_ID in every pane env, so this is stable across
// async callbacks. Without --workspace, bmux defaults to whichever pane is
...
function workspaceArgs(): string[] {
  const ws = process.env.BMUX_WORKSPACE_ID;
  return ws ? ["--workspace", ws] : [];
}

// Sanitized environment for fire-and-forget bmux status subprocesses.
// Strips Amp-provided secrets (`AMP_API_KEY`) so we never propagate them to
// every spawned `bmux set-status` / `bmux log` / `bmux clear-status` child.
...
function runBmux(args: string[]): void {
  if (process.env.BMUX_AMP_HOOKS_DISABLED === "1") return;
  if (!process.env.BMUX_SURFACE_ID) return;
  const bmux = process.env.BMUX_AMP_BMUX_BIN || "bmux";
  try {
    const child = spawn(bmux, args, {
...
function clearStatus(): void {
  runBmux(["clear-status", STATUS_KEY, ...workspaceArgs()]);
}
...
export default function (amp: PluginAPI) {
  ...
  // final status badge with "thinking". bmux runs one Amp session per terminal
...
"""#
    ...
    func installAmpExtensionHooks(_ def: AgentHookDef) throws {
        ...
        if !existing.isEmpty, !existing.contains(Self.ampExtensionMarker) {
            throw CLIError(message: "\(extensionURL.path) exists and is not a bmux plugin; leaving it alone")
        }
        if !skipConfirm {
    ...
    func uninstallAmpExtensionHooks(_ def: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print("No Amp bmux plugin found at \(extensionURL.path)")
            return
        }
        let existing = (try? String(contentsOf: extensionURL, encoding: .utf8)) ?? ""
        guard existing.contains(Self.ampExtensionMarker) else {
            print("Refusing to remove \(extensionURL.path): missing bmux marker")
            return
        }
        try fm.removeItem(at: extensionURL)
        print("Removed Amp bmux plugin from \(extensionURL.path)")
    }
}
