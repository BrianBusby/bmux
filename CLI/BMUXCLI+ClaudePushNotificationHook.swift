...
extension BMUXCLI {
    func runClaudePushNotificationHook(
        ...
        // tool delivers through a raw OSC desktop notification, and bmux
        ...
        // swallowed inside bmux. The tool's own Notification hook never
    ...
    /// Whether the PushNotification tool call should surface as a bmux
    ...
    /// the bmux store is the only Mac-visible surface left. Deliberately not
    /// keyed on `localSent`: bmux swallows the tool's raw OSC delivery either
