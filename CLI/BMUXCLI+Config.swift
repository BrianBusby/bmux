...
extension BMUXCLI {
    func runConfigCommand(
        ...
        case "get":
            guard args.count == 2, let key = canonicalFontSizeKey(args[1]) else {
                throw CLIError(message: "Usage: bmux config get <sidebar-font-size|surface-tab-bar-font-size>")
            }
            try runConfigGetFontSize(forKey: key, jsonOutput: wantsJSON)
        case "set":
            guard args.count == 3, let key = canonicalFontSizeKey(args[1]) else {
                throw CLIError(message: "Usage: bmux config set <sidebar-font-size|surface-tab-bar-font-size> <points>")
            }
        ...
        case BmuxGhosttyConfigSettingEditor.sidebarFontSizeKey, BmuxGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey:
            if args.count == 1 {
                ...
                throw CLIError(message: "Usage: bmux config \(subcommand) [points]")
            }
        case "path", "paths":
            guard args.count == 1 else {
                throw CLIError(message: "Usage: bmux config path")
            }
            printSettingsPaths(jsonOutput: wantsJSON)
        case "docs", "documentation":
            guard args.count == 1 else {
                throw CLIError(message: "Usage: bmux config docs")
            }
        ...
        case "doctor", "check", "validate":
            ...
            if report.errorCount > 0 {
                throw CLIError(message: "bmux config doctor found \(report.errorCount) error(s)")
            }
        case "reload":
            guard args.count == 1 else {
                throw CLIError(message: "Usage: bmux config reload")
            }
            guard let socketPath else {
                throw CLIError(message: "bmux config reload requires a socket-backed bmux command path")
            }
            ...
            print(response)
        default:
            throw CLIError(message: "Unknown config subcommand '\(subcommand)'. Run 'bmux config --help'.")
        }
    }
    ...
    func configCommandDoesNotNeedSocket(_ commandArgs: [String]) -> Bool {
        ...
        if subcommand == BmuxGhosttyConfigSettingEditor.sidebarFontSizeKey
            || subcommand == BmuxGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey {
    ...
    func configUsage() -> String {
        return """
        Usage: bmux config <doctor|check|validate|path|paths|docs|documentation|reload|get|set|sidebar-font-size|surface-tab-bar-font-size>

        Inspect bmux.json, print configuration references, update selected Ghostty config keys, or reload the running app.

        Subcommands:
          doctor|check|validate [--path <path>]   Validate JSONC syntax for bmux config files.
          path|paths                              Print bmux.json paths, docs URL, and schema URL.
          docs|documentation                      Print the same output as `bmux docs settings`.
          reload                                  Reload Ghostty config + bmux.json and refresh terminals (alias for `bmux reload-config`).
          get <key>                               Print sidebar-font-size or surface-tab-bar-font-size.
          set <key> <points>                      Set sidebar-font-size (10-20 pt) or surface-tab-bar-font-size (8-24 pt), then reload if bmux is running.
        ...
        Examples:
          bmux config doctor
          bmux config doctor --path .bmux/bmux.json
          bmux config set sidebar-font-size 14
          bmux config sidebar-font-size 12.5
          bmux config set surface-tab-bar-font-size 13
          bmux config surface-tab-bar-font-size 11
          bmux config reload
        """
    }
    ...
    func printSettingsPaths(jsonOutput: Bool) {
        let payload: [String: Any] = [
            ...
            "reload_command": "bmux reload-config",
            "reload_scope": "Reloads Ghostty config + bmux.json and refreshes terminals in place. No app restart needed.",
            "backup": "Back up any existing bmux.json file to a timestamped .bak copy before editing so the user can revert.",
        ]
        ...
        print()
        print("Reload after editing (covers BOTH bmux.json and Ghostty config; no app restart needed):")
        print("  bmux reload-config")
    }
    ...
    private func canonicalFontSizeKey(_ raw: String) -> String? {
        ...
        case BmuxGhosttyConfigSettingEditor.sidebarFontSizeKey:
            return BmuxGhosttyConfigSettingEditor.sidebarFontSizeKey
        case BmuxGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey:
            return BmuxGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey
        default:
            return nil
        }
    }
    ...
    private func fontSizeConfig(
        ...
        case BmuxGhosttyConfigSettingEditor.sidebarFontSizeKey:
            return (
                BmuxGhosttyConfigSettingEditor.defaultSidebarFontSize,
                BmuxGhosttyConfigSettingEditor().clampedSidebarFontSize,
                BmuxGhosttyConfigSettingEditor().formattedSidebarFontSize,
                { BmuxGhosttyConfigSettingEditor().parsedSidebarFontSize(in: $0) }
            )
        case BmuxGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey:
            return (
                BmuxGhosttyConfigSettingEditor.defaultSurfaceTabBarFontSize,
                BmuxGhosttyConfigSettingEditor().clampedSurfaceTabBarFontSize,
                BmuxGhosttyConfigSettingEditor().formattedSurfaceTabBarFontSize,
                { BmuxGhosttyConfigSettingEditor().parsedSurfaceTabBarFontSize(in: $0) }
            )
        default:
            return nil
        }
    ...
    private func runConfigGetFontSize(forKey key: String, jsonOutput: Bool) throws {
        ...
        let url = try bmuxGhosttyConfigURLForCLI()
    ...
    private func runConfigSetFontSize(
        ...
        case "failed":
            ...
            print("Run `bmux config reload` after bmux is running to apply it.")
        default:
            print("OK \(key) = \(formattedValue) (saved)")
            print("Run `bmux config reload` to apply it.")
        }
        print("path: \(Self.tildePath(url.path))")
    }

    private func bmuxGhosttyConfigURLForCLI() throws -> URL {
        let environment = ProcessInfo.processInfo.environment
        let fileManager = FileManager.default
        let appSupportDirectories = BmuxApplicationSupportDirectories(environment: environment, fileManager: fileManager)
            .userDirectories
        ...
        let bundleIdentifier = normalizedConfigValue(environment["BMUX_BUNDLE_ID"])
        ...
        for appSupportDirectory in appSupportDirectories {
            if let existing = BmuxGhosttyConfigPathResolver().loadConfigURLs(
        ...
        return BmuxGhosttyConfigPathResolver().activeOrEditableConfigURL(
            currentBundleIdentifier: bundleIdentifier,
    ...
    private struct ConfigDoctorReport {
        ...
        var payload: [String: Any] {
            [
                ...
                "reload_command": "bmux reload-config",
                "docs_url": BMUXCLI.settingsDocsURL,
                "schema_url": BMUXCLI.settingsSchemaURL,
            ]
        }
    }
        ...
        while index < arguments.count {
            ...
            if argument == "--path" {
                ...
                guard nextIndex < arguments.count else {
                    throw CLIError(message: "bmux config doctor --path requires a path")
                }
    ...
    private func configDoctorFinding(for target: ConfigDoctorTarget) -> ConfigDoctorFinding {
        ...
        guard fileManager.fileExists(atPath: target.path, isDirectory: &isDirectory) else {
            let message = target.missingIsError
                ? "file not found"
                : "not found; bmux will use defaults until this file exists"
    ...
    private func printConfigDoctorReport(_ report: ConfigDoctorReport) {
        ...
        print("Reload: bmux reload-config")
    }
