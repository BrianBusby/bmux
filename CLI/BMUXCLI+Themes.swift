...
extension BMUXCLI {
    static let bmuxThemeOverrideBundleIdentifier = BmuxGhosttyConfigPathResolver.releaseBundleIdentifier
    static let bmuxThemesBlockStart = "# bmux themes start"
    static let bmuxThemesBlockEnd = "# bmux themes end"
    static let bmuxThemesReloadNotificationName = "com.bmuxterm.themes.reload-config"
    ...
    private func runInteractiveThemes(
        ...
        environment["BMUX_THEME_PICKER_CONFIG"] = try bmuxThemeOverrideConfigURL(
            targetBundleIdentifier: targetBundleIdentifier
        ).path
        environment["BMUX_THEME_PICKER_BUNDLE_ID"] = targetBundleIdentifier
        environment["BMUX_THEME_PICKER_TARGET"] = defaultThemePickerTargetMode(current: selection).rawValue
        environment["BMUX_THEME_PICKER_COLOR_SCHEME"] = defaultAppearancePrefersDarkThemes() ? "dark" : "light"
        if let light = selection.light {
            environment["BMUX_THEME_PICKER_INITIAL_LIGHT"] = light
        }
        if let dark = selection.dark {
            environment["BMUX_THEME_PICKER_INITIAL_DARK"] = dark
        }
    ...
    func runThemes(
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'bmux themes --help'.")
            }
    ...
    private func printThemesList(
        ...
        let configPath = try bmuxThemeOverrideConfigURL(
            targetBundleIdentifier: targetBundleIdentifier
        ).path
