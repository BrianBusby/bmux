...
extension BMUXCLI {
    private static let piExtensionMarker = "bmux-pi-session-extension-marker"
    private static let piExtensionFilename = "bmux-session.ts"
    ...
    func installPiExtensionHooks(_ def: AgentHookDef) throws {
        ...
        if !existing.isEmpty, !existing.contains(Self.piExtensionMarker) {
            throw CLIError(message: String.localizedStringWithFormat(
                String(
                    localized: "cli.hooks.pi.error.notBmuxExtension",
    ...
    func uninstallPiExtensionHooks(_ def: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "No Pi bmux extension found at %@"
                ),
                extensionURL.path
            ))
            return
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed Pi bmux extension from %@"
            ),
            extensionURL.path
        ))
    }
