...
extension BMUXCLI {
    private static let ompExtensionMarker = "bmux-omp-session-extension-marker"
    private static let ompExtensionFilename = "bmux-omp-session.ts"
    private static let ompExtensionSource = #"""
// bmux-omp-session-extension-marker v1
// Bridges OMP session lifecycle events into bmux's restorable session store.
// Installed by `bmux hooks omp install` or `bmux hooks setup`.
...
function hookEnvironment(cwd: string): NodeJS.ProcessEnv {
  ...
  if (!env.BMUX_AGENT_LAUNCH_ARGV_B64) {
    ...
    env.BMUX_AGENT_LAUNCH_KIND = "omp";
    env.BMUX_AGENT_LAUNCH_EXECUTABLE = argv[0] || resolveExecutable("omp");
...
function hookInvocation(subcommand: string, ctx: ExtensionContext, extra: Record<string, unknown> = {}): HookInvocation | null {
  if (process.env.BMUX_OMP_HOOKS_DISABLED === "1") return null;
  if (!process.env.BMUX_SURFACE_ID) return null;
  ...
  const bmux = process.env.BMUX_OMP_BMUX_BIN || "bmux";
  return {
    bmux,
    cwd,
...
async function sendHook(subcommand: string, ctx: ExtensionContext, extra: Record<string, unknown> = {}): Promise<void> {
  ...
  await new Promise<void>((resolve) => {
    ...
    try {
      const child = spawn(invocation.bmux, ["hooks", "omp", subcommand], {
        env: invocation.env,
...
export default function bmuxOmpSessionExtension(api: ExtensionAPI) {
    ...
    func installOmpExtensionHooks(_ _: AgentHookDef) throws {
        ...
        if !existing.isEmpty, !existing.contains(Self.ompExtensionMarker) {
            throw CLIError(message: String.localizedStringWithFormat(
                String(
                    localized: "cli.hooks.omp.error.notBmuxExtension",
                    defaultValue: "%@ exists and is not a bmux extension; leaving it alone"
                ),
                extensionURL.path
            ))
        }
    ...
    func uninstallOmpExtensionHooks(_ _: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "No OMP bmux extension found at %@"
                ),
                extensionURL.path
            ))
            return
        ...
        guard existing.contains(Self.ompExtensionMarker) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "Refusing to remove %@: missing bmux marker"
                ),
                extensionURL.path
            ))
            return
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed OMP bmux extension from %@"
            ),
            extensionURL.path
        ))
    }
