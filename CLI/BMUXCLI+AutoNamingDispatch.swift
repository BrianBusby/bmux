...
extension BMUXCLI {
    ...
    func resolvedSummarizerAgent(
        ...
        // Pure decision (unit-tested in BmuxSettings); the CLI only supplies the
    ...
    func summarizerBinaryAvailable(agent: String, env: [String: String]) -> Bool {
        switch agent {
        case "claude":
            let customPath = env["BMUX_CUSTOM_CLAUDE_PATH"]?
                .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            if !customPath.isEmpty,
               FileManager.default.isExecutableFile(atPath: customPath),
               !isBmuxClaudeWrapper(at: customPath) {
                return true
            }
        ...
        default:
            guard let def = BMUXCLI.agentDef(named: agent) else { return false }
    ...
    func summarize(
        ...
        default:
            guard let def = BMUXCLI.agentDef(named: agent) else { return nil }
    ...
    private func summarizeWithCodex(
        ...
        summarizerEnv["BMUX_CODEX_HOOKS_DISABLED"] = "1"
        let outputFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-autoname-\(UUID().uuidString).txt")
        let workingDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-autoname-cwd-\(UUID().uuidString)", isDirectory: true)
