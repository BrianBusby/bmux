...
extension BMUXCLI {
    ...
    func themeDirectoryURLs() -> [URL] {
        ...
        for appSupportDirectory in BmuxApplicationSupportDirectories(environment: processEnv).userDirectories {
            appendIfExisting(
                appSupportDirectory
                    .appendingPathComponent(Self.bmuxThemeOverrideBundleIdentifier, isDirectory: true)
                    .appendingPathComponent("themes", isDirectory: true)
            )
        }
        appendIfExisting(
    ...
    func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'bmux themes' to list available themes.")
    }
    ...
    func themeConfigSearchURLs(targetBundleIdentifier: String) -> [URL] {
        ...
        for appSupportDirectory in BmuxApplicationSupportDirectories(
    ...
    func bmuxThemeOverrideConfigURL(targetBundleIdentifier: String) throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Failed to locate the Application Support directory")
        }
        return BmuxGhosttyConfigPathResolver().editableConfigURL(
            currentBundleIdentifier: targetBundleIdentifier,
    ...
    func writeManagedThemeOverride(
        ...
        let fileManager = FileManager.default
        let configURL = try bmuxThemeOverrideConfigURL(targetBundleIdentifier: targetBundleIdentifier)
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.bmuxThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.bmuxThemesBlockEnd)
        """
    ...
    func reloadThemesIfPossible(
        ...
        DistributedNotificationCenter.default().post(
            name: Notification.Name(Self.bmuxThemesReloadNotificationName),
            object: nil,
            userInfo: [
    ...
    func themeTargetBundleIdentifier(socketPath: String) -> String {
        bundleIdentifierForThemeReloadSocketPath(socketPath)
            ?? currentBmuxAppBundleIdentifier()
            ?? Self.bmuxThemeOverrideBundleIdentifier
    }
    ...
    private func bundleIdentifierForThemeReloadSocketPath(_ socketPath: String) -> String? {
        ...
        switch name {
        case "bmux.sock":
            return Self.bmuxThemeOverrideBundleIdentifier
        case "bmux-debug.sock":
            return "com.bmuxterm.app.debug"
        case "bmux-nightly.sock":
            return "com.bmuxterm.app.nightly"
        case "bmux-staging.sock":
            return "com.bmuxterm.app.staging"
        default:
            break
        }

        if name.range(of: #"^bmux-\d+\.sock$"#, options: .regularExpression) != nil {
            return Self.bmuxThemeOverrideBundleIdentifier
        }

        if let slug = themeReloadSocketSlug(name, prefix: "bmux-debug-", suffix: ".sock") {
            return "com.bmuxterm.app.debug.\(slug)"
        }
        if let slug = themeReloadSocketSlug(name, prefix: "bmux-nightly-", suffix: ".sock") {
            return "com.bmuxterm.app.nightly.\(slug)"
        }
        if let slug = themeReloadSocketSlug(name, prefix: "bmux-staging-", suffix: ".sock") {
            return "com.bmuxterm.app.staging.\(slug)"
        }
        return nil
    }
    ...
    func currentBmuxAppBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["BMUX_BUNDLE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
