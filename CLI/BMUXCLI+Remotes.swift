...
// `bmux remotes` (alias `remote`): manage the team's device-registry routes so
...
extension BMUXCLI {
    static let aiAccountsUsage = """
        Usage: bmux ai-accounts <list|upload|remove> [options]

        Upload local AI credentials to your team's subrouter tenant and manage
        the sanitized account records stored there.

          bmux ai-accounts list [--team <id>] [--json]
              List uploaded AI accounts for the selected or specified team.

          bmux ai-accounts upload <claude|codex|anthropic-key|openai-key> [--label <s>] [--key <s>] [--team <id>] [--validate] [--json]
              Upload credentials. Claude and Codex OAuth files are read by the
              bmux app. API-key providers read ANTHROPIC_API_KEY / OPENAI_API_KEY
              from your shell environment; --key overrides but exposes the
              secret in shell history and process listings.

          bmux ai-accounts remove <account-id> [--team <id>] [--json]
              Delete an uploaded AI account.

        Examples:
          bmux ai-accounts list
          bmux ai-accounts upload claude --label work
          ANTHROPIC_API_KEY=... bmux ai-accounts upload anthropic-key
          bmux ai-accounts remove acct_123
        """

    static let remotesUsage = """
        Usage: bmux remotes <list|add|remove> [options]

        Manage the remote Macs in your team's bmux device registry. Added remotes
        appear in your iOS app's device list and the phone can attach to them.

          bmux remotes list [--json]
              List your team's registered remotes (name, deviceId, routes, tag, last seen).

          bmux remotes add <name> --route <host:port> [--route <host:port> ...] [--tag <tag>] [--json]
          ...
          bmux remotes remove <name-or-deviceId> [--json]
              Remove a remote you registered from the device registry.

        Examples:
          bmux remotes add my-studio --route 100.64.1.2:51001
          bmux remotes add my-studio --route my-studio.tailnet.ts.net:51001 --tag stable
          bmux remotes list --json
          bmux remotes remove my-studio
        """
    ...
    func runRemotesCommand(commandArgs: [String], client: SocketClient, jsonOutput: Bool) throws {
        ...
        case "list", "ls":
            ...
            if remotes.isEmpty {
                print("No remotes. Add one: bmux remotes add <name> --route <host:port>")
                return
            }
            ...
            guard !routeValues.isEmpty else {
                throw CLIError(message: """
                    remotes add requires at least one --route host:port.
                      ...
                      bmux remotes add \(name) --route 100.64.1.2:51001
                    """)
            }
            ...
            guard let target = positionals.first, !target.isEmpty else {
                throw CLIError(message: """
                    remotes remove requires a name or deviceId.
                      ...
                      bmux remotes remove <name-or-deviceId>

                    List remotes: bmux remotes list
                    """)
            }
    ...
    func runAIAccountsCommand(commandArgs: [String], client: SocketClient, jsonOutput: Bool) throws {
        ...
        case "list", "ls":
            ...
            if accounts.isEmpty {
                print("No AI accounts. Upload one: bmux ai-accounts upload <claude|codex|anthropic-key|openai-key>")
                return
            }
            ...
            guard let accountID = remaining.first, !accountID.isEmpty, !Self.isAIAccountsFlagToken(accountID) else {
                throw CLIError(message: """
                    ai-accounts remove requires an account id.
                      ...
                      bmux ai-accounts remove <account-id>

                    List accounts: bmux ai-accounts list
                    """)
            }
    ...
    /// link BMUXMobileCore); the app and server remain the authoritative check.
