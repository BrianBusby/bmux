...
extension BMUXCLI {
    ...
    private func sigpipeProbeExecutablePath() throws -> String {
        let candidate: String? = {
            if let explicit = ProcessInfo.processInfo.environment["BMUX_CLI_PATH"],
               !explicit.isEmpty {
                return explicit
            }
        ...
        guard let path = candidate,
            ...
            throw CLIError(message: "SIGPIPE probe could not resolve bmux executable path")
        }
        return path
    }
    ...
    func runSIGPIPEProbe(commandArgs: [String]) throws {
        ...
        let inspectionURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sigpipe-\(UUID().uuidString).json")
