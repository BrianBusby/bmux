    ...
    func installKimiHooks(_ def: AgentHookDef) throws {
        ...
        let configDirectoryFileError = String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "bmux could not create the hooks directory: a file exists at %@; remove or rename the conflicting file and re-run `bmux hooks setup`"
            ),
            configDir
        )
    ...
    func uninstallKimiHooks(_ def: AgentHookDef) throws {
        ...
        guard oldString != newString else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "Removed 0 bmux hook(s) from %@"
                ),
                filePath
            ))
            return
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed Kimi Code bmux hooks from %@"
            ),
            filePath
        ))
    }
