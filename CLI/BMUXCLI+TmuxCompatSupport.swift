...
extension BMUXCLI {
    ...
    ///
    /// bmux hands a respawn/start command to the surface as the pane's process
    ...
    ///
    /// Teammate panes are respawned by bmux's surface layer, not by `bmux
    ...
    /// can appear in a cwd, quoted value, or other non-flag position): the `bmux
    /// claude-teams` launcher makes that decision once from its own argv and records
    /// it in `BMUX_CLAUDE_TEAMS_SANDBOXED` (see `claudeTeamsExtraEnvVars`). That
    ...
    func tmuxClaudeTeamsRespawnEnvironment() -> [(key: String, value: String)] {
        guard ProcessInfo.processInfo.environment["BMUX_CLAUDE_TEAMS_SANDBOXED"] == "1" else {
            return []
        }
    ...
    func tmuxStartupScript(commandTokens: [String], cwd: String?) -> String? {
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-tmux-command-\(UUID().uuidString.lowercased()).sh")
        var lines = [
            "#!/bin/sh",
