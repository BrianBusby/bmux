...
function sendHook(subcommand: string, ctx: ExtensionContext, extra: HookExtra = {}): boolean {
  if (process.env.BMUX_PI_HOOKS_DISABLED === "1") return true;
  if (!process.env.BMUX_SURFACE_ID) return true;
  ...
  const result = runBmux(["hooks", "pi", subcommand], cwd, JSON.stringify(payload));
  if (!result.ok) {
    warn(ctx, "bmux hook command failed", {
      subcommand,
...
function surfaceTargetArgs(): string[] | null {
  const surfaceId = firstString(process.env.BMUX_SURFACE_ID);
  if (!surfaceId) return null;
  const args: string[] = [];
  const workspaceId = firstString(process.env.BMUX_WORKSPACE_ID);
...
function ensureResumeBinding(ctx: ExtensionContext, sessionId: string, cwd: string): void {
  if (process.env.BMUX_PI_HOOKS_DISABLED === "1") return;
  ...
  const verified = parseJSONOutput(runBmux(["--json", "surface", "resume", "get", ...target], cwd));
...
function sendFeed(eventName: "PreToolUse" | "PostToolUse", ctx: ExtensionContext, event: unknown, extra: HookExtra = {}): void {
  ...
  try {
    const child = spawn(bmuxExecutable(), ["hooks", "feed", "--source", "pi", "--event", eventName], {
...
export default function bmuxPiSessionExtension(pi: ExtensionAPI) {
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    if (notificationRouted) stopPayload.bmux_notification_routed = true;
