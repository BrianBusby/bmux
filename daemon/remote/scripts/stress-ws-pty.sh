...
duration="${BMUX_PTY_STRESS_DURATION:-12h}"
fuzztime="${BMUX_PTY_FUZZTIME:-5m}"
log_every="${BMUX_PTY_STRESS_LOG_EVERY:-60}"
test_timeout="${BMUX_PTY_TEST_TIMEOUT:-2m}"
package="./cmd/bmuxd-remote"
...
stress_bin="$stress_tmp/bmuxd-remote.test"

cd "$REMOTE_DIR"
export GOTOOLCHAIN="${GOTOOLCHAIN:-go1.24.7+auto}"

if [[ ! "$log_every" =~ ^[1-9][0-9]*$ ]]; then
  echo "invalid BMUX_PTY_STRESS_LOG_EVERY: $log_every" >&2
  exit 2
fi
