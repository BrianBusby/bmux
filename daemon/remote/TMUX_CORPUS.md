...
This map records how each selected upstream tmux regression or fuzz target is represented in bmux. It is documentation only; CI confidence comes from executable Go tests, Go fuzz targets, and the macOS terminal-renderer tests in `.github/workflows/tmux-corpus.yml`. The daemon and tmux-compat rows marked `pr` run on pull requests touching `daemon/remote/**`; the terminal-renderer rows remain nightly because they need the macOS app and GhosttyKit.

| Upstream source | bmux layer | Status | CI lane | Port note |
| --- | --- | --- | --- | --- |
| `regress/am-terminal.sh` | terminal-renderer | adapted | nightly | Autowrap belongs to Ghostty/bmux rendering. |
| `regress/border-arrows.sh` | tmux-ui | not_applicable | none | bmux does not expose tmux border arrow indicators. |
...
| `regress/combine-test.sh` | terminal-renderer | adapted | nightly | Unicode combining width belongs to Ghostty/bmux rendering. |
| `regress/command-order.sh` | tmux-compat | ported | pr | Command splitting and sequential dispatch are covered. |
| `regress/conf-syntax.sh` | tmux-config | not_applicable | none | bmux does not parse tmux config files. |
| `regress/control-client-sanity.sh` | tmux-compat | adapted | pr | bmux exposes JSON-RPC and tmux-compat commands instead of tmux control mode. |
| `regress/control-client-size.sh` | remote-pty | ported | pr | WebSocket PTY resize control frames are covered. |
| `regress/copy-mode-test-emacs.sh` | tmux-copy-mode | not_applicable | none | bmux does not implement tmux copy-mode key tables. |
| `regress/copy-mode-test-vi.sh` | tmux-copy-mode | not_applicable | none | bmux does not implement tmux copy-mode key tables. |
...
| `regress/format-strings.sh` | tmux-compat | adapted | pr | bmux supports a deliberate tmux format subset for agent shims. |
...
| `regress/if-shell-TERM.sh` | tmux-shell | not_applicable | none | bmux does not implement tmux if-shell. |
| `regress/if-shell-error.sh` | tmux-shell | not_applicable | none | bmux does not implement tmux if-shell. |
| `regress/if-shell-nested.sh` | tmux-shell | not_applicable | none | bmux does not implement tmux if-shell. |
...
| `regress/new-session-base-index.sh` | tmux-indexing | not_applicable | none | bmux workspace numbering is not tmux base-index configurable. |
...
| `regress/osc-11colours.sh` | terminal-renderer | adapted | nightly | bmux should preserve Ghostty truecolor behavior instead of tmux default color assumptions. |
| `regress/run-shell-output.sh` | tmux-shell | not_applicable | none | bmux does not implement tmux run-shell. |
...
| `regress/style-trim.sh` | tmux-status-style | not_applicable | none | bmux does not implement tmux status line style trimming. |
| `regress/tty-keys.sh` | terminal-input | adapted | nightly | OS key forwarding is covered in macOS terminal tests. |
| `regress/utf8-test.sh` | terminal-renderer | adapted | nightly | UTF-8 rendering belongs to Ghostty/bmux; Go covers UTF-8 env and byte-safe command paths. |
...
| `fuzz/style-fuzzer.c` | terminal-renderer | adapted | nightly | Style and color parsing belongs to Ghostty/bmux rendering, with better truecolor expectations. |
