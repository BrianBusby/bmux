...
func startPersistentDaemonWithVerifierForTest(t *testing.T, verifier persistentDaemonTokenVerifier) (string, func()) {
	t.Helper()
	socketDir, err := os.MkdirTemp("/tmp", "bmuxd-remote-test-*")
	if err != nil {
...
func TestWrapperBinaryDispatchesIntoCLI(t *testing.T) {
	if os.Getenv("BMUXD_REMOTE_MAIN_HELPER") == "1" {
		separator := 0
	...
	wrapperPath := filepath.Join(t.TempDir(), "bmuxd-remote-current")
	...
	cmd.Env = append(os.Environ(), "BMUXD_REMOTE_MAIN_HELPER=1")
...
func TestPersistentDaemonPathsUseShortSocketPath(t *testing.T) {
	...
	t.Setenv("BMUX_REMOTE_DAEMON_ROOT", rootBase)
	t.Setenv("BMUX_REMOTE_DAEMON_SOCKET_DIR", "")
...
func TestPersistentDaemonSocketDirOverrideUsesPrivateChild(t *testing.T) {
	...
	t.Setenv("BMUX_REMOTE_DAEMON_ROOT", rootBase)
	t.Setenv("BMUX_REMOTE_DAEMON_SOCKET_DIR", socketParent)
...
func TestPersistentDaemonSocketDirFallsBackFromUnsafeSymlink(t *testing.T) {
	...
	unsafeChild := filepath.Join(socketParent, fmt.Sprintf("bmuxd-remote-%d", os.Getuid()))
...
func TestPersistentDaemonReadySignalAllowsImmediateDial(t *testing.T) {
	socketDir, err := os.MkdirTemp("/tmp", "bmuxd-remote-ready-*")
	if err != nil {
		t.Fatalf("create short socket dir: %v", err)
	}
...
func TestPersistentDaemonServerExitsAfterEmptySlotIdleTimeout(t *testing.T) {
	socketDir, err := os.MkdirTemp("/tmp", "bmuxd-remote-idle-*")
	if err != nil {
		t.Fatalf("create short socket dir: %v", err)
	}
	defer os.RemoveAll(socketDir)
