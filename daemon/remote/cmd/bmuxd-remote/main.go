...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "bmuxd-remote") || len(args) == 0 {
		return false
	}
	...
	case "serve":
		...
		if *ws {
			...
			if err := runWebSocketPTYServer(context.Background(), wsPTYServerConfig{
				...
				AdminTokenSHA256: firstNonEmpty(strings.TrimSpace(*adminTokenSHA256), strings.TrimSpace(os.Getenv("BMUXD_WS_ADMIN_TOKEN_SHA256"))),
				AdminEd25519PubKey: firstNonEmpty(
					strings.TrimSpace(*adminEd25519PublicKey),
					strings.TrimSpace(os.Getenv("BMUXD_WS_ADMIN_ED25519_PUBLIC_KEY")),
				),
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  bmuxd-remote version")
	_, _ = fmt.Fprintln(w, "  bmuxd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  bmuxd-remote serve --stdio --persistent --slot <slot>")
	_, _ = fmt.Fprintln(w, "  bmuxd-remote serve --ws --auth-lease-file <path> [--rpc-auth-lease-file <path>] [--listen 127.0.0.1:7777]")
	_, _ = fmt.Fprintln(w, "  bmuxd-remote cli <command> [args...]")
}
...
const (
	...
	persistentDaemonReadyFDEnv    = "BMUX_REMOTE_DAEMON_READY_FD"
...
func persistentDaemonPathsForSlot(rawSlot string) (persistentDaemonPaths, error) {
	...
	rootBase := strings.TrimSpace(os.Getenv("BMUX_REMOTE_DAEMON_ROOT"))
	if rootBase == "" {
		home, homeErr := os.UserHomeDir()
		if homeErr != nil || strings.TrimSpace(home) == "" {
			return persistentDaemonPaths{}, errors.New("cannot resolve remote home directory")
		}
		rootBase = filepath.Join(home, ".bmux", "daemon")
	}
...
func persistentDaemonSocketPath(root string, slot string) string {
	...
	if !overrideSet {
		socketBase = filepath.Join("/tmp", fmt.Sprintf("bmuxd-remote-%d", os.Getuid()))
	}
	digest := sha256.Sum256([]byte(root + "\x00" + slot))
	return filepath.Join(socketBase, "bmuxd-"+hex.EncodeToString(digest[:8])+".sock")
}

func persistentDaemonSocketBase() (string, bool) {
	socketBase := strings.TrimSpace(os.Getenv("BMUX_REMOTE_DAEMON_SOCKET_DIR"))
	if socketBase == "" {
		return "", false
	}
	return filepath.Join(socketBase, fmt.Sprintf("bmuxd-remote-%d", os.Getuid())), true
}
...
func createPersistentDaemonFallbackSocketDir(root string) (string, error) {
	for attempt := 0; attempt < 8; attempt++ {
		...
		socketDir := filepath.Join(
			os.TempDir(),
			fmt.Sprintf("bmuxd-remote-%d-%s", os.Getuid(), hex.EncodeToString(raw)),
		)
...
func signalPersistentDaemonReady() {
	...
	file := os.NewFile(uintptr(fd), "bmux-persistent-daemon-ready")
	if file == nil {
		return
	}
...
func (s *rpcServer) handleNotificationResponse(req rpcRequest, resp rpcResponse) error {
	...
	if s.frameWriter == nil {
		...
		_, _ = fmt.Fprintf(os.Stderr, "bmuxd-remote: %s notification failed without response writer: %s\n", req.Method, detail)
		return nil
	}
