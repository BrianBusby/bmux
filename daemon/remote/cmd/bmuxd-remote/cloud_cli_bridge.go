...
const defaultCloudCLIBridgeSocketPath = "/tmp/bmux-cloud-cli.sock"
...
func (b *cloudCLIBridge) start(ctx context.Context, socketPath string, stderr io.Writer) error {
	...
	b.listener = listener
	b.mu.Unlock()
	_, _ = fmt.Fprintf(stderr, "bmuxd-remote cloud CLI bridge listening on %s\n", socketPath)
	go func() {
		<-ctx.Done()
		_ = listener.Close()
	}()
...
func (b *cloudCLIBridge) forward(request []byte) ([]byte, error) {
	...
	if len(targets) == 0 {
		return nil, errors.New("no bmux app is attached to this cloud VM")
	}
	...
	if len(sentTargets) == 0 {
		...
		return nil, errors.New("no bmux app accepted cloud CLI request")
	}
	...
	select {
	case <-timeout:
		return nil, errors.New("timed out waiting for bmux app response")
	default:
	}
	for pending > 0 {
		select {
	...
	return nil, errors.New("bmux app rejected cloud CLI request")
}
...
func (s *rpcServer) handleCLIResponse(req rpcRequest) rpcResponse {
	...
	if responseOK {
		...
		if response.err == "" {
			response.err = "bmux app rejected cloud CLI request"
		}
	}
