...
func TestTmuxWaitForSignalPath(t *testing.T) {
	...
	if !strings.HasPrefix(path, "/tmp/bmux-wait-for-") {
...
func TestTmuxDisplayReporterFormatFields(t *testing.T) {
	origHome := os.Getenv("HOME")
	origWorkspace := os.Getenv("BMUX_WORKSPACE_ID")
	origSurface := os.Getenv("BMUX_SURFACE_ID")
	origPane := os.Getenv("TMUX_PANE")
	os.Setenv("HOME", t.TempDir())
	os.Setenv("BMUX_WORKSPACE_ID", "workspace:1")
	os.Setenv("BMUX_SURFACE_ID", "surface:1")
	leaderPaneToken := "%" + tmuxStableNumericId("33333333-3333-4333-8333-333333333333")
	os.Setenv("TMUX_PANE", leaderPaneToken)
	defer func() {
		os.Setenv("HOME", origHome)
		if origWorkspace != "" {
			os.Setenv("BMUX_WORKSPACE_ID", origWorkspace)
		} else {
			os.Unsetenv("BMUX_WORKSPACE_ID")
		}
		if origSurface != "" {
			os.Setenv("BMUX_SURFACE_ID", origSurface)
		} else {
			os.Unsetenv("BMUX_SURFACE_ID")
		}
		if origPane != "" {
	...
	if values["session_name"] != "bmux" {
		t.Fatalf("session_name = %q, want bmux", values["session_name"])
	}
...
func TestConfigureAgentEnvironment(t *testing.T) {
	...
	envKeys := []string{
		"BMUX_CLAUDE_TEAMS_BMUX_BIN", "PATH", "TMUX", "TMUX_PANE",
		"TERM", "BMUX_SOCKET_PATH", "TERM_PROGRAM",
		"BMUX_WORKSPACE_ID", "BMUX_SURFACE_ID",
	...
	// Verify socket path
	if os.Getenv("BMUX_SOCKET_PATH") != "127.0.0.1:54321" {
		t.Errorf("BMUX_SOCKET_PATH = %q", os.Getenv("BMUX_SOCKET_PATH"))
	}
	...
	if os.Getenv("BMUX_WORKSPACE_ID") != "ws-abc" {
		t.Errorf("BMUX_WORKSPACE_ID = %q", os.Getenv("BMUX_WORKSPACE_ID"))
	}
	if os.Getenv("BMUX_SURFACE_ID") != "surf-789" {
		t.Errorf("BMUX_SURFACE_ID = %q", os.Getenv("BMUX_SURFACE_ID"))
	}
	// Verify extra env
