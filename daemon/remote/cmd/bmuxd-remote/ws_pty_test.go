...
// for https://github.com/manaflow-ai/bmux/issues/5185, where the failure
...
func TestWebSocketPTYAdminLeaseInstallAcceptsEd25519Signature(t *testing.T) {
	...
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-Bmux-Admin-Signature-Ed25519", base64.StdEncoding.EncodeToString(ed25519.Sign(privateKey, body)))
	resp, err = http.DefaultClient.Do(req)
	if err != nil {
...
func TestWebSocketPTYRequiresSessionMatchAndConsumesLeaseOnce(t *testing.T) {
	...
	writeTestLease(t, leasePath, "bmux-secret", "sess-good", true, time.Now().Add(time.Minute))
	conn := dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "bmux-secret", "sess-other", 80, 24)
	_, _, err := conn.Read(ctx)
	if websocket.CloseStatus(err) != websocket.StatusPolicyViolation {
	...
	conn = dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "bmux-secret", "sess-good", 100, 30)
...
func TestWebSocketPTYRunsShellOverBinaryFrames(t *testing.T) {
	...
	output := waitForBinaryContains(t, ctx, conn, "BMUX_WS_OK", 15*time.Second)
...
func TestWebSocketPTYReconnectKeepsSessionProcess(t *testing.T) {
	...
	readReady(t, ctx, conn)
	if err := conn.Write(ctx, websocket.MessageBinary, []byte("BMUX_RECONNECT_MARKER=alive; export BMUX_RECONNECT_MARKER; printf 'first-ready\\n'\r")); err != nil {
...
func TestWebSocketPTYAnonymousAttachesAreIsolated(t *testing.T) {
	...
	readReady(t, ctx, a)
	if err := a.Write(ctx, websocket.MessageBinary, []byte("BMUX_ANON_MARK=one; export BMUX_ANON_MARK; printf 'A_READY\\n'\r")); err != nil {
...
func TestWebSocketPTYAnonymousSessionKeyCannotBeForged(t *testing.T) {
	...
	if err := anon.Write(ctx, websocket.MessageBinary, []byte("BMUX_FORGE_MARK=anon; export BMUX_FORGE_MARK; printf 'ANON_FORGE_READY\\n'\r")); err != nil {
...
func TestWebSocketPTYAttachmentWithoutSessionIDIsAnonymous(t *testing.T) {
	...
	readReady(t, ctx, a)
	if err := a.Write(ctx, websocket.MessageBinary, []byte("BMUX_NO_SESSION_MARK=one; export BMUX_NO_SESSION_MARK; printf 'NO_SESSION_A_READY\\n'\r")); err != nil {
...
func TestWebSocketPTYSeedsUTF8LocaleAndTerminalEnv(t *testing.T) {
	...
	command := "printf '%s\\n' \"$LANG|$LC_CTYPE|$LC_ALL|$TERM|$COLORTERM|$TERM_PROGRAM|$BMUX_REMOTE_TRANSPORT\"; locale charmap; exit\r"
