...
// runCLI is the entry point for the "cli" subcommand (or busybox "bmux" invocation).
func runCLI(args []string) int {
	socketPath := os.Getenv("BMUX_SOCKET_PATH")
	...
	for i := 0; i < len(args); i++ {
		...
		case "--socket":
			if i+1 >= len(args) {
				fmt.Fprintln(os.Stderr, "bmux: --socket requires a path")
				return 2
			}
			socketPath = args[i+1]
			i++
...
doneFlags:
	...
	if socketPath == "" {
		fmt.Fprintln(os.Stderr, "bmux: BMUX_SOCKET_PATH not set and --socket not provided")
		return 1
	}
	...
	if cmdName == "workspace" {
		...
		fmt.Fprintln(os.Stderr, "bmux workspace: only the \"group\" subcommand is supported here. Use list-workspaces, new-workspace, close-workspace, or select-workspace for workspace operations.")
		return 2
	}
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "bmux: unknown command %q\n", cmdName)
		return 2
	}
	...
	if !spec.noParams {
		...
		for _, key := range spec.flagKeys {
			...
			if val, ok := parsed.flags[key]; ok {
				...
				if _, isBool := boolFlagSet[key]; isBool {
					...
					default:
						fmt.Fprintf(os.Stderr, "bmux: --%s must be true or false\n", key)
						return 2
					}
				} else {
					params[paramKey] = val
		...
		if len(parsed.positional) > 0 {
			if spec.positionalKey != "" {
				...
				fmt.Fprintf(os.Stderr, "bmux: %s does not accept positional arguments\n", spec.name)
				return 2
			}
		}
...
// runNewWorkspaceRelay handles "bmux new-workspace" with full flag parity to the
...
func runNewWorkspaceRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bmux new-workspace: %v\n", err)
		return 2
	}
	if len(parsed.positional) > 0 {
		fmt.Fprintln(os.Stderr, "bmux: new-workspace does not accept positional arguments")
		return 2
	}
	...
	if val, ok := parsed.flags["focus"]; ok {
		...
		case "false", "0", "no":
			params["focus"] = false
		default:
			fmt.Fprintf(os.Stderr, "bmux: --focus must be true or false\n")
			return 2
		}
	}
	...
	if val, ok := parsed.flags["layout"]; ok {
		...
		if err := json.Unmarshal([]byte(val), &layout); err != nil {
			fmt.Fprintf(os.Stderr, "bmux new-workspace: --layout must be valid JSON: %v\n", err)
			return 2
		}
	...
	for _, kv := range parsed.repeated["env"] {
		...
		if !ok {
			fmt.Fprintf(os.Stderr, "bmux new-workspace: --env %q must be KEY=VALUE\n", kv)
			return 2
		}
		env[k] = v
	}
	if envFile, ok := parsed.flags["env-file"]; ok {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "bmux new-workspace: --env-file: %v\n", err)
			return 2
		}
		for _, line := range strings.Split(string(data), "\n") {
			...
			if !ok {
				fmt.Fprintf(os.Stderr, "bmux new-workspace: --env-file line %q must be KEY=VALUE\n", line)
				return 2
			}
			env[k] = v
		}
	...
	if cmd, ok := parsed.flags["command"]; ok {
		...
		if err := json.Unmarshal([]byte(resp), &result); err != nil {
			fmt.Fprintf(os.Stderr, "bmux new-workspace: --command skipped: could not parse create response: %v\n", err)
			return 1
		}
		surfaceID, _ := result["surface_id"].(string)
		if surfaceID == "" {
			fmt.Fprintf(os.Stderr, "bmux new-workspace: --command skipped: workspace.create response missing surface_id\n")
			return 1
		}
		...
		if _, err := socketRoundTripV2(socketPath, "surface.send_text", sendParams, refreshAddr); err != nil {
			fmt.Fprintf(os.Stderr, "bmux new-workspace: --command send failed: %v\n", err)
			return 1
		}
		...
		if _, err := socketRoundTripV2(socketPath, "surface.send_key", keyParams, refreshAddr); err != nil {
			fmt.Fprintf(os.Stderr, "bmux new-workspace: --command send-key failed: %v\n", err)
			return 1
		}
	}
...
func runRPC(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "bmux rpc: requires a method name")
		return 2
	}
	method := args[0]
	...
	if len(args) > 1 {
		if err := json.Unmarshal([]byte(args[1]), &params); err != nil {
			fmt.Fprintf(os.Stderr, "bmux rpc: invalid JSON params: %v\n", err)
			return 2
		}
	}
...
// runWorkspaceGroupRelay handles "bmux workspace group <sub>" (and the
// "workspace-group" alias) by mapping each subcommand to its
// workspace.group.* v2 method, mirroring the macOS bmux CLI flags.
func runWorkspaceGroupRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	...
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "bmux workspace group: requires a subcommand (%s)\n", subcommandHint)
		return 2
	}
	sub := args[0]
	flagKeys, ok := workspaceGroupFlagKeys[sub]
	if !ok {
		fmt.Fprintf(os.Stderr, "bmux workspace group: unknown subcommand %q (%s)\n", sub, subcommandHint)
		return 2
	}

	fail := func(format string, a ...any) int {
		fmt.Fprintf(os.Stderr, "bmux workspace group %s: %s\n", sub, fmt.Sprintf(format, a...))
		return 2
	}
...
// runBrowserRelay handles "bmux browser <subcommand>" by mapping to browser.* v2 methods.
func runBrowserRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "bmux browser: requires a subcommand (%s)\n", browserSubcommandHint())
		return 2
	}
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "bmux browser: unknown subcommand %q\n", sub)
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bmux browser: %v\n", err)
		return 2
	}
	for _, key := range spec.flagKeys {
		if val, ok := parsed.flags[key]; ok {
...
func applyWorkspaceEnvFallback(params map[string]any) {
	...
	if envWs := os.Getenv("BMUX_WORKSPACE_ID"); envWs != "" {
...
func applySurfaceEnvFallback(params map[string]any) {
	...
	if envSf := os.Getenv("BMUX_SURFACE_ID"); envSf != "" {
...
// readSocketAddrFile reads the socket address from ~/.bmux/socket_addr as a fallback
// when BMUX_SOCKET_PATH is not set. Written by the bmux app after the relay establishes.
func readSocketAddrFile() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	data, err := os.ReadFile(filepath.Join(home, ".bmux", "socket_addr"))
	if err != nil {
		return ""
	}
...
func readRelayAuthFile(socketPath string) *relayAuthState {
	if strings.Contains(socketPath, ":") && !strings.HasPrefix(socketPath, "/") {
		...
		data, err := os.ReadFile(filepath.Join(home, ".bmux", "relay", port+".auth"))
		if err != nil {
			return nil
		}
...
func currentRelayAuth(socketPath string) *relayAuthState {
	relayID := strings.TrimSpace(os.Getenv("BMUX_RELAY_ID"))
	relayToken := strings.TrimSpace(os.Getenv("BMUX_RELAY_TOKEN"))
...
// dialSocket connects to the bmux socket. If addr contains a colon and doesn't
...
func authenticateRelayConn(conn net.Conn, auth *relayAuthState) error {
	...
	if challenge.Protocol != "bmux-relay-auth" || challenge.Version != 1 || challenge.RelayID != auth.RelayID || challenge.Nonce == "" {
...
func cliUsage() {
	fmt.Fprintln(os.Stderr, "Usage: bmux [--socket <path>] [--json] <command> [args...]")
	...
	fmt.Fprintln(os.Stderr, "  browser <sub>             Browser commands through the local bmux browser relay")
	fmt.Fprintln(os.Stderr, "  claude-teams [args...]    Launch Claude Code in teammate mode")
	fmt.Fprintln(os.Stderr, "  omo [args...]             Launch OpenCode with bmux integration")
	fmt.Fprintln(os.Stderr, "  omx [args...]             Launch Oh My Codex with bmux integration")
	fmt.Fprintln(os.Stderr, "  omc [args...]             Launch Oh My Claude Code with bmux integration")
