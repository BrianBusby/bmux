...
const claudeNodeOptionsRestoreModuleScript = `const hadOriginalNodeOptions = process.env.BMUX_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
...
// runClaudeTeamsRelay implements `bmux claude-teams` on the remote side.
...
func runClaudeTeamsRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bmux claude-teams: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	configureAgentEnvironment(agentConfig{
		...
		focused:        focused,
		tmuxPathPrefix: "bmux-claude-teams",
		bmuxBinEnvVar:  "BMUX_CLAUDE_TEAMS_BMUX_BIN",
		termEnvVar:     "BMUX_CLAUDE_TEAMS_TERM",
	...
	if claudePath == "" {
		fmt.Fprintf(os.Stderr, "bmux claude-teams: claude not found in PATH\n")
		return 1
	}
	argv := append([]string{claudePath}, launchArgs...)
	execErr := syscall.Exec(claudePath, argv, os.Environ())
	fmt.Fprintf(os.Stderr, "bmux claude-teams: exec failed: %v\n", execErr)
	return 1
}

// runOMORelay implements `bmux omo` on the remote side.
func runOMORelay(socketPath string, args []string, refreshAddr func() string) int {
	rc := &rpcContext{socketPath: socketPath, refreshAddr: refreshAddr}

	shimDir, err := createOMOShimDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "bmux omo: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if opencodePath == "" {
		fmt.Fprintf(os.Stderr, "bmux omo: opencode not found in PATH\n"+
	...
	if err := omoEnsurePlugin(originalPath); err != nil {
		fmt.Fprintf(os.Stderr, "bmux omo: plugin setup: %v\n", err)
		return 1
	}
	...
	configureAgentEnvironment(agentConfig{
		...
		socketPath:     socketPath,
		focused:        focused,
		tmuxPathPrefix: "bmux-omo",
		bmuxBinEnvVar:  "BMUX_OMO_BMUX_BIN",
		termEnvVar:     "BMUX_OMO_TERM",
		extraEnv:       map[string]string{},
	})
	...
	fmt.Fprintf(os.Stderr, "bmux omo: exec failed: %v\n", execErr)
	return 1
}

// runOMXRelay implements `bmux omx` on the remote side.
func runOMXRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bmux omx: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omxPath == "" {
		fmt.Fprintf(os.Stderr, "bmux omx: omx not found in PATH\n"+
	...
	configureAgentEnvironment(agentConfig{
		...
		socketPath:     socketPath,
		focused:        focused,
		tmuxPathPrefix: "bmux-omx",
		bmuxBinEnvVar:  "BMUX_OMX_BMUX_BIN",
		termEnvVar:     "BMUX_OMX_TERM",
		extraEnv:       map[string]string{},
	})

	launchPath, launchArgv := resolveNodeScriptExec(omxPath, args, originalPath, shimDir)
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "bmux omx: exec failed: %v\n", execErr)
	return 1
}

// runOMCRelay implements `bmux omc` on the remote side.
func runOMCRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bmux omc: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omcPath == "" {
		fmt.Fprintf(os.Stderr, "bmux omc: omc not found in PATH\n"+
	...
	configureAgentEnvironment(agentConfig{
		...
		socketPath:     socketPath,
		focused:        focused,
		tmuxPathPrefix: "bmux-omc",
		bmuxBinEnvVar:  "BMUX_OMC_BMUX_BIN",
		termEnvVar:     "BMUX_OMC_TERM",
		extraEnv:       map[string]string{},
	})
	...
	if restoreModulePath, err := ensureClaudeNodeOptionsRestoreModule(); err == nil {
		...
		fmt.Fprintf(os.Stderr, "bmux omc: warning: failed to create NODE_OPTIONS restore module: %v\n", err)
	}

	launchPath, launchArgv := resolveNodeScriptExec(omcPath, args, originalPath, shimDir)
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "bmux omc: exec failed: %v\n", execErr)
	return 1
}
...
set -euo pipefail
exec "${BMUX_CLAUDE_TEAMS_BMUX_BIN:-bmux}" __tmux-compat "$@"
`
...
exec "${BMUX_OMO_BMUX_BIN:-bmux}" __tmux-compat "$@"
`
...
exec "${BMUX_OMX_BMUX_BIN:-bmux}" __tmux-compat "$@"
`
...
exec "${BMUX_OMC_BMUX_BIN:-bmux}" __tmux-compat "$@"
`

const omoNotifierShimScript = `#!/usr/bin/env bash
# Intercept terminal-notifier calls and route through bmux notify.
TITLE="" BODY=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
exec "${BMUX_OMO_BMUX_BIN:-bmux}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
`
...
func createTmuxShimDir(dirName string, tmuxScript string) (string, error) {
	...
	dir := filepath.Join(home, ".bmuxterm", dirName)
...
func ensureClaudeNodeOptionsRestoreModule() (string, error) {
	dir := filepath.Join(os.TempDir(), "bmux-claude-node-options")
	if err := os.MkdirAll(dir, 0755); err != nil {
		return "", err
	}
...
func configureClaudeNodeOptions(restoreModulePath string) {
	...
	if hadExisting {
		os.Setenv("BMUX_ORIGINAL_NODE_OPTIONS_PRESENT", "1")
		os.Setenv("BMUX_ORIGINAL_NODE_OPTIONS", existing)
	} else {
		os.Setenv("BMUX_ORIGINAL_NODE_OPTIONS_PRESENT", "0")
		os.Unsetenv("BMUX_ORIGINAL_NODE_OPTIONS")
	}
...
func configureAgentEnvironment(cfg agentConfig) {
	...
	os.Setenv(cfg.bmuxBinEnvVar, selfPath)
	...
	// Socket path
	os.Setenv("BMUX_SOCKET_PATH", cfg.socketPath)
	os.Unsetenv("BMUX_SOCKET")
	...
	if cfg.focused != nil {
		os.Setenv("BMUX_WORKSPACE_ID", cfg.focused.workspaceId)
		if cfg.focused.surfaceId != "" {
			os.Setenv("BMUX_SURFACE_ID", cfg.focused.surfaceId)
		}
	}
...
func omoShadowConfigDir() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".bmuxterm", "omo-config")
}
