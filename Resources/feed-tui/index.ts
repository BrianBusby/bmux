...
class FeedApp {
  ...
  private renderHeader(pendingCount: number) {
    ...
    return Box(
      ...
      Text({ content: "bmux feed", fg: theme.accentStrong }),
...
function writeReadyMarker(stage: string, details: Record<string, string> = {}): void {
  const markerPath = process.env.BMUX_FEED_TUI_READY_PATH?.trim();
  if (!markerPath) {
    return;
  }
  try {
    const payload = JSON.stringify({
      ...
      tui: process.env.BMUX_FEED_TUI_PATH ?? "opentui",
      ...details,
    });
...
async function main() {
  const socketPath = process.env.BMUX_SOCKET_PATH;
  if (!socketPath) {
    throw new Error("BMUX_SOCKET_PATH is required.");
  }
  ...
  const app = new FeedApp(renderer, new FeedSocketClient(socketPath, process.env.BMUX_SOCKET_PASSWORD));
...
void main().catch((error) => {
  console.error(`bmux feed tui failed: ${formatError(error)}`);
  process.exit(1);
});
