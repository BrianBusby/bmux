# bmux bash prompt bootstrap.
#
# macOS ships /bin/bash 3.2, where Ghostty's automatic bash integration is
# unsupported and HOME-based wrapper startup is not reliable. bmux instead
# exports this script as PROMPT_COMMAND so it runs once on the first interactive
# prompt: it sources bmux's bash integration and then hands control to
...
# after the first one (https://github.com/manaflow-ai/bmux/issues/5164).
#
...
# let bmux-bash-integration.bash's PROMPT_COMMAND merge prepend _bmux_prompt_command.
#
...
PROMPT_COMMAND="${PROMPT_COMMAND##*__bmux_bash_bootstrap_marker__}"
while [[ "$PROMPT_COMMAND" == [[:space:]\;]* ]]; do PROMPT_COMMAND="${PROMPT_COMMAND#?}"; done
if [[ "${BMUX_LOAD_GHOSTTY_BASH_INTEGRATION:-0}" == "1" && -n "${GHOSTTY_RESOURCES_DIR:-}" ]]; then
    _bmux_ghostty_bash="$GHOSTTY_RESOURCES_DIR/shell-integration/bash/ghostty.bash"
    [[ -r "$_bmux_ghostty_bash" ]] && source "$_bmux_ghostty_bash"
fi
if [[ "${BMUX_SHELL_INTEGRATION:-1}" != "0" && -n "${BMUX_SHELL_INTEGRATION_DIR:-}" ]]; then
    _bmux_bash_integration="$BMUX_SHELL_INTEGRATION_DIR/bmux-bash-integration.bash"
    [[ -r "$_bmux_bash_integration" ]] && source "$_bmux_bash_integration"
fi
unset _bmux_ghostty_bash _bmux_bash_integration
if declare -F _bmux_prompt_command >/dev/null 2>&1; then _bmux_prompt_command; fi
: __bmux_bash_bootstrap_marker__
