# bmux shell integration for zsh
...
typeset -g _BMUX_HAS_ZSOCKET=0
if zmodload zsh/net/unix 2>/dev/null; then
    _BMUX_HAS_ZSOCKET=1
fi

typeset -g _BMUX_HAS_ZSH_JOBSTATES=0
if zmodload zsh/parameter 2>/dev/null && (( ${+jobstates} )); then
    _BMUX_HAS_ZSH_JOBSTATES=1
fi

_bmux_zsh_job_table_saturated() {
    (( _BMUX_HAS_ZSH_JOBSTATES )) || return 1

    local limit="${BMUX_ZSH_JOB_TABLE_SOFT_LIMIT:-900}"
    case "$limit" in
        ''|*[!0-9]*) limit=900 ;;
    esac
    (( limit > 0 )) || limit=900
...
_bmux_restore_status() {
    builtin return "$1"
}

_bmux_send() {
    local payload="$1"
    if (( _BMUX_HAS_ZSOCKET )); then
        local fd
        zsocket "$BMUX_SOCKET_PATH" 2>/dev/null || return 1
        fd=$REPLY
    ...
    if command -v ncat >/dev/null 2>&1; then
        print -r -- "$payload" | ncat -w 1 -U "$BMUX_SOCKET_PATH" --send-only
    elif command -v socat >/dev/null 2>&1; then
        print -r -- "$payload" | socat -T 1 - "UNIX-CONNECT:$BMUX_SOCKET_PATH" >/dev/null 2>&1
    elif command -v nc >/dev/null 2>&1; then
        if print -r -- "$payload" | nc -N -U "$BMUX_SOCKET_PATH" >/dev/null 2>&1; then
            :
        else
            print -r -- "$payload" | nc -w 1 -U "$BMUX_SOCKET_PATH" >/dev/null 2>&1 || true
        fi
    fi
}
...
_bmux_send_bg() {
    if (( _BMUX_HAS_ZSOCKET )); then
        ...
        _bmux_zsh_job_table_saturated && return 0
        { _bmux_send "$1" } >/dev/null 2>&1 &!
    fi
}

_bmux_socket_is_unix() {
    [[ -n "$BMUX_SOCKET_PATH" && -S "$BMUX_SOCKET_PATH" ]]
}

_bmux_relay_cli_path() {
    if [[ -n "${BMUX_BUNDLED_CLI_PATH:-}" && -x "${BMUX_BUNDLED_CLI_PATH}" ]]; then
        print -r -- "${BMUX_BUNDLED_CLI_PATH}"
        return 0
    fi
    command -v bmux 2>/dev/null
}

_bmux_socket_uses_remote_relay() {
    [[ -n "$BMUX_SOCKET_PATH" ]] || return 1
    [[ "$BMUX_SOCKET_PATH" == /* ]] && return 1
    [[ "$BMUX_SOCKET_PATH" == *:* ]] || return 1
    [[ -n "$(_bmux_relay_cli_path)" ]]
}

_bmux_has_port_scan_transport() {
    _bmux_socket_is_unix && return 0
    _bmux_socket_uses_remote_relay
}
...
_bmux_relay_rpc_bg() {
    ...
    local params="$2"
    local relay_cli=""
    _bmux_zsh_job_table_saturated && return 1
    _bmux_socket_uses_remote_relay || return 1
    relay_cli="$(_bmux_relay_cli_path)" || return 1
...
_bmux_relay_rpc() {
    ...
    local response=""
    _bmux_socket_uses_remote_relay || return 1
    # Relay `bmux rpc` exits nonzero on server error. The real remote CLI prints
...
_bmux_relay_workspace_id() {
    if [[ -n "$BMUX_WORKSPACE_ID" ]]; then
        print -r -- "$BMUX_WORKSPACE_ID"
        return 0
    fi
    [[ -n "$BMUX_TAB_ID" ]] || return 1
    print -r -- "$BMUX_TAB_ID"
}
...
_bmux_report_tty_via_relay() {
    _bmux_socket_uses_remote_relay || return 1
    local workspace_id=""
    workspace_id="$(_bmux_relay_workspace_id)" || return 1
    [[ -n "$_BMUX_TTY_NAME" ]] || return 1

    local tty_name_json params
    tty_name_json="$(_bmux_json_escape "$_BMUX_TTY_NAME")"
    params="{\"workspace_id\":\"$workspace_id\",\"tty_name\":\"$tty_name_json\""
    if [[ -n "$BMUX_PANEL_ID" ]]; then
        params+=",\"surface_id\":\"$BMUX_PANEL_ID\""
    fi
    params+="}"
    _bmux_relay_rpc "surface.report_tty" "$params"
}

_bmux_report_pwd_via_relay() {
    local pwd="$1"
    _bmux_socket_uses_remote_relay || return 1
    [[ -n "$pwd" ]] || return 1
    local workspace_id=""
    ...
    pwd_json="$(_bmux_json_escape "$pwd")"
...
_bmux_restore_scrollback_once() {
    local path="${BMUX_RESTORE_SCROLLBACK_FILE:-}"
    [[ -n "$path" ]] || return 0
    unset BMUX_RESTORE_SCROLLBACK_FILE
...
typeset -g _BMUX_CLAUDE_WRAPPER=""
typeset -g _BMUX_GROK_WRAPPER=""
_bmux_path_prepend_unique_directory() {
    local directory="$1"
...
_bmux_install_cli_command_shim() {
    local command_name="$1"
    local wrapper_path="$2"
    local shim_root="${TMPDIR:-/tmp}/bmux-cli-shims/${BMUX_SURFACE_ID:-$$}"
    ...
    # Use zsh's explicit clobber redirection (>|) so bmux always refreshes its
    ...
    # startup (the writer runs again from the _bmux_fix_path precmd hook after
    # the shim already exists). See issue #6714.
    } >|"$shim_path" 2>/dev/null || return 0
    /bin/chmod 0700 "$shim_path" >/dev/null 2>&1 || return 0

    if [[ "$command_name" == "claude" ]]; then
        export BMUX_CLAUDE_WRAPPER_SHIM="$shim_path"
        export BMUX_CLAUDE_WRAPPER_SHIM_ROOT="$shim_root"
    fi

    PATH="$(_bmux_path_prepend_unique_directory "$shim_root" "${PATH-}")"
    hash -r >/dev/null 2>&1 || rehash >/dev/null 2>&1 || true
}
_bmux_claude_wrapper_command() {
    if [[ -x "${BMUX_CLAUDE_WRAPPER_SHIM:-}" ]]; then
        "$BMUX_CLAUDE_WRAPPER_SHIM" "$@"
    elif [[ -x "${_BMUX_CLAUDE_WRAPPER:-}" ]]; then
        "$_BMUX_CLAUDE_WRAPPER" "$@"
    else
        command claude "$@"
    fi
}
_bmux_install_cli_wrapper() {
    local command_name="$1"
    local wrapper_variable="$2"
    local wrapper_file="${3:-$command_name}"
    local integration_dir="${BMUX_SHELL_INTEGRATION_DIR:-}"
    ...
    if [[ "$command_name" == "claude" ]]; then
        eval "$command_name() { _bmux_claude_wrapper_command \"\$@\"; }"
    else
        eval "$command_name() { \"\${$wrapper_variable}\" \"\$@\"; }"
    fi
}
_bmux_install_cli_wrapper claude _BMUX_CLAUDE_WRAPPER bmux-claude-wrapper
_bmux_install_cli_wrapper grok _BMUX_GROK_WRAPPER

_bmux_normalize_claude_config_dir() {
...
typeset -g _BMUX_PWD_LAST_PWD=""
typeset -g _BMUX_GIT_LAST_PWD=""
typeset -g _BMUX_GIT_LAST_RUN=0
typeset -g _BMUX_GIT_JOB_PID=""
typeset -g _BMUX_GIT_JOB_STARTED_AT=0
typeset -g _BMUX_GIT_FORCE=0
typeset -g _BMUX_GIT_HEAD_LAST_PWD=""
typeset -g _BMUX_GIT_HEAD_PATH=""
typeset -g _BMUX_GIT_HEAD_SIGNATURE=""
typeset -g _BMUX_GIT_HEAD_WATCH_PID=""
typeset -g _BMUX_GIT_ACTIVE_PWD_FILE="${_BMUX_GIT_ACTIVE_PWD_FILE:-$(/usr/bin/mktemp "${TMPDIR:-/tmp}/bmux-git-active-pwd.XXXXXX" 2>/dev/null || true)}"
typeset -g _BMUX_PR_POLL_PID=""
typeset -g _BMUX_PR_POLL_PWD=""
typeset -g _BMUX_PR_LAST_BRANCH=""
typeset -g _BMUX_PR_NO_PR_BRANCH=""
...
typeset -g _BMUX_PORTS_LAST_RUN=0
typeset -g _BMUX_CMD_START=0
typeset -g _BMUX_SHELL_ACTIVITY_LAST=""
typeset -g _BMUX_TTY_NAME=""
typeset -g _BMUX_TTY_REPORTED=0
typeset -g _BMUX_GHOSTTY_SEMANTIC_PATCHED=0
typeset -g _BMUX_WINCH_GUARD_INSTALLED=0
typeset -g _BMUX_TMUX_PUSH_SIGNATURE=""
typeset -g _BMUX_TMUX_PULL_SIGNATURE=""
typeset -g _BMUX_DELAY_TERM_RESTORE_UNTIL_FIRST_PROMPT=${_BMUX_DELAY_TERM_RESTORE_UNTIL_FIRST_PROMPT:-0}
typeset -ga _BMUX_TMUX_SYNC_KEYS=(
    ...
    BMUX_PORT_END
    BMUX_PORT_RANGE
    BMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD
...
typeset -ga _BMUX_TMUX_SURFACE_SCOPED_KEYS=(
    BMUX_PANEL_ID
    BMUX_SURFACE_ID
)

_bmux_tmux_sync_key_is_managed() {
    local candidate="$1"
    local key
    for key in "${_BMUX_TMUX_SYNC_KEYS[@]}"; do
        [[ "$key" == "$candidate" ]] && return 0
    done
    return 1
}

_bmux_tmux_shell_env_signature() {
    local key value
    local -a parts
    for key in "${_BMUX_TMUX_SYNC_KEYS[@]}"; do
        value="${(P)key}"
...
_bmux_tmux_publish_bmux_environment() {
    [[ -z "$TMUX" ]] || return 0
    command -v tmux >/dev/null 2>&1 || return 0

    local signature
    signature="$(_bmux_tmux_shell_env_signature)"
    [[ -n "$signature" ]] || return 0
    [[ "$signature" == "$_BMUX_TMUX_PUSH_SIGNATURE" ]] && return 0
    ...
    for key in "${_BMUX_TMUX_SURFACE_SCOPED_KEYS[@]}"; do
        tmux set-environment -gu "$key" >/dev/null 2>&1 || return 0
    done

    _BMUX_TMUX_PUSH_SIGNATURE="$signature"
}

_bmux_tmux_refresh_bmux_environment() {
    [[ -n "$TMUX" ]] || return 0
    command -v tmux >/dev/null 2>&1 || return 0
    ...
    while IFS= read -r line; do
        [[ "$line" == BMUX_* ]] || continue
        key="${line%%=*}"
        _bmux_tmux_sync_key_is_managed "$key" || continue
        filtered+="${line}"$'\n'
    done <<< "$output"

    [[ -n "$filtered" ]] || return 0
    [[ "$filtered" == "$_BMUX_TMUX_PULL_SIGNATURE" ]] && return 0
    ...
    _BMUX_TMUX_PULL_SIGNATURE="$filtered"
    if (( did_change )); then
        _BMUX_TTY_REPORTED=0
        _BMUX_SHELL_ACTIVITY_LAST=""
        _BMUX_PWD_LAST_PWD=""
        _BMUX_GIT_LAST_PWD=""
...
_bmux_ensure_ghostty_preexec_strips_both_marks() {
    local fn_name="$1"
    ...
    if [[ "$updated" == *"$new_strip"* && "$updated" != *"$old_strip"* ]]; then
        ...
        _BMUX_GHOSTTY_SEMANTIC_PATCHED=1
        return 0
    fi
...
_bmux_patch_ghostty_semantic_redraw() {
    ...
    _bmux_ensure_ghostty_preexec_strips_both_marks _ghostty_deferred_init
    _bmux_ensure_ghostty_preexec_strips_both_marks _ghostty_preexec
}
_bmux_patch_ghostty_semantic_redraw

_bmux_prepend_job_table_guard_to_function() {
    local fn_name="$1"
    (( $+functions[$fn_name] )) || return 0
    local saved_var="__bmux_${fn_name}_saved_status"
    [[ "${functions[$fn_name]}" == *"$saved_var"* ]] && return 0

    functions[$fn_name]="builtin local ${saved_var}=\$?
_bmux_zsh_job_table_saturated && builtin return 0
_bmux_restore_status \"\$${saved_var}\"
${functions[$fn_name]}"
}

_bmux_insert_job_table_guard_after_declaration() {
...
_bmux_patch_ghostty_job_table_guard() {
    local guard_precmd=$'        builtin local __bmux__ghostty_precmd_saved_status=$?\n        _bmux_zsh_job_table_saturated && builtin return 0\n        _bmux_restore_status "$__bmux__ghostty_precmd_saved_status"'
    local guard_preexec=$'        builtin local __bmux__ghostty_preexec_saved_status=$?\n        _bmux_zsh_job_table_saturated && builtin return 0\n        _bmux_restore_status "$__bmux__ghostty_preexec_saved_status"'
    local guard_zle_init=$'          builtin local __bmux__ghostty_zle_line_init_saved_status=$?\n          _bmux_zsh_job_table_saturated && builtin return 0\n          _bmux_restore_status "$__bmux__ghostty_zle_line_init_saved_status"'
    local guard_zle_finish=$'          builtin local __bmux__ghostty_zle_line_finish_saved_status=$?\n          _bmux_zsh_job_table_saturated && builtin return 0\n          _bmux_restore_status "$__bmux__ghostty_zle_line_finish_saved_status"'
    local guard_zle_keymap=$'          builtin local __bmux__ghostty_zle_keymap_select_saved_status=$?\n          _bmux_zsh_job_table_saturated && builtin return 0\n          _bmux_restore_status "$__bmux__ghostty_zle_keymap_select_saved_status"'

    # Patch deferred definitions before Ghostty's first precmd installs and
    # invokes its live hook functions.
    if (( $+functions[_ghostty_deferred_init] )); then
        _bmux_insert_job_table_guard_after_declaration _ghostty_deferred_init _ghostty_precmd "$guard_precmd"
        _bmux_insert_job_table_guard_after_declaration _ghostty_deferred_init _ghostty_preexec "$guard_preexec"
        _bmux_insert_job_table_guard_after_declaration _ghostty_deferred_init _ghostty_zle_line_init "$guard_zle_init"
        _bmux_insert_job_table_guard_after_declaration _ghostty_deferred_init _ghostty_zle_line_finish "$guard_zle_finish"
        _bmux_insert_job_table_guard_after_declaration _ghostty_deferred_init _ghostty_zle_keymap_select "$guard_zle_keymap"
    fi

    _bmux_prepend_job_table_guard_to_function _ghostty_precmd
    _bmux_prepend_job_table_guard_to_function _ghostty_preexec
    _bmux_prepend_job_table_guard_to_function _ghostty_zle_line_init
    _bmux_prepend_job_table_guard_to_function _ghostty_zle_line_finish
    _bmux_prepend_job_table_guard_to_function _ghostty_zle_keymap_select
}
_bmux_patch_ghostty_job_table_guard

_bmux_prompt_wrap_guard() {
    local cmd_start="$1"
    local pwd="$2"
...
_bmux_install_winch_guard() {
    (( _BMUX_WINCH_GUARD_INSTALLED )) && return 0
...
_bmux_set_git_active_pwd() {
    local active_pwd="$1"
    [[ -n "$active_pwd" ]] || return 0
    [[ -n "${_BMUX_GIT_ACTIVE_PWD_FILE:-}" ]] || return 0
    print -r -- "$active_pwd" >| "$_BMUX_GIT_ACTIVE_PWD_FILE" 2>/dev/null || true
}

_bmux_git_report_path_is_active() {
    local repo_path="$1"
    [[ -n "$repo_path" ]] || return 1
    [[ -n "${_BMUX_GIT_ACTIVE_PWD_FILE:-}" ]] || return 0
    [[ -r "$_BMUX_GIT_ACTIVE_PWD_FILE" ]] || return 0

    local active_pwd=""
    IFS= read -r active_pwd < "$_BMUX_GIT_ACTIVE_PWD_FILE" || active_pwd=""
    ...
    repo_head="$(_bmux_git_resolve_head_path "$repo_path" 2>/dev/null || true)"
    active_head="$(_bmux_git_resolve_head_path "$active_pwd" 2>/dev/null || true)"
    [[ -n "$repo_head" && "$repo_head" == "$active_head" ]]
}

_bmux_report_tty_payload() {
    [[ -n "$BMUX_TAB_ID" ]] || return 0
    [[ -n "$_BMUX_TTY_NAME" ]] || return 0

    local payload="report_tty $_BMUX_TTY_NAME --tab=$BMUX_TAB_ID"
    if [[ -z "$TMUX" ]]; then
        [[ -n "$BMUX_PANEL_ID" ]] || return 0
        payload+=" --panel=$BMUX_PANEL_ID"
    fi
...
_bmux_report_tty_once() {
    ...
    (( _BMUX_TTY_REPORTED )) && return 0
    _bmux_has_port_scan_transport || return 0

    if _bmux_socket_is_unix; then
        local payload=""
        payload="$(_bmux_report_tty_payload)"
        [[ -n "$payload" ]] || return 0
        ...
        _bmux_report_tty_via_relay || return 0
        _BMUX_TTY_REPORTED=1
    fi
}

_bmux_report_shell_activity_state() {
    local state="$1"
    [[ -n "$state" ]] || return 0
    [[ -S "$BMUX_SOCKET_PATH" ]] || return 0
    [[ -n "$BMUX_TAB_ID" ]] || return 0
    [[ -n "$BMUX_PANEL_ID" ]] || return 0
    [[ "$_BMUX_SHELL_ACTIVITY_LAST" == "$state" ]] && return 0
    _BMUX_SHELL_ACTIVITY_LAST="$state"
    _bmux_send_bg "report_shell_state $state --tab=$BMUX_TAB_ID --panel=$BMUX_PANEL_ID"
}

_bmux_reset_terminal_keyboard_protocols() {
    [[ -t 1 || -n "${BMUX_TEST_FORCE_KEYBOARD_RESET:-}${BMUX_TEST_FORCE_KITTY_RESET:-}" ]] || return 0
...
_bmux_report_git_branch_for_path() {
    local repo_path="$1"
    [[ "${BMUX_NO_GIT_WATCH:-}" == "1" ]] && return 0
    [[ -n "$repo_path" ]] || return 0
    [[ -S "$BMUX_SOCKET_PATH" ]] || return 0
    [[ -n "$BMUX_TAB_ID" ]] || return 0
    [[ -n "$BMUX_PANEL_ID" ]] || return 0
    _bmux_git_report_path_is_active "$repo_path" || return 0

    local branch dirty_opt="--status=unknown"
    branch="$(_bmux_git_branch_for_path "$repo_path" 2>/dev/null || true)"
    _bmux_git_report_path_is_active "$repo_path" || return 0
    if [[ -n "$branch" ]]; then
        _bmux_send "report_git_branch $branch $dirty_opt --tab=$BMUX_TAB_ID --panel=$BMUX_PANEL_ID"
    else
        _bmux_send "clear_git_branch --tab=$BMUX_TAB_ID --panel=$BMUX_PANEL_ID"
    fi
}

_bmux_record_pr_command_hint() {
    local cmd="$1"
    ...
    while (( index <= ${#words} )); do
        ...
        case "$word" in
            ...
            -*)
                ...
                continue ;;
            *)
                _BMUX_LAST_PR_TARGET="$word"
                break ;;
        esac
    done
}

_bmux_emit_pr_command_hint() {
    [[ "${BMUX_NO_PR_WATCH:-}" == "1" ]] && return 0
    [[ -S "$BMUX_SOCKET_PATH" ]] || return 0
    [[ -n "$BMUX_TAB_ID" ]] || return 0
    [[ -n "$BMUX_PANEL_ID" ]] || return 0
    [[ -n "$_BMUX_LAST_PR_ACTION" ]] || return 0

    local payload="report_pr_action $_BMUX_LAST_PR_ACTION --tab=$BMUX_TAB_ID --panel=$BMUX_PANEL_ID"
    if [[ -n "$_BMUX_LAST_PR_TARGET" ]]; then
...
_bmux_clear_pr_for_panel() {
    ...
    [[ -n "$BMUX_TAB_ID" ]] || return 0
    [[ -n "$BMUX_PANEL_ID" ]] || return 0
    _bmux_send_bg "clear_pr --tab=$BMUX_TAB_ID --panel=$BMUX_PANEL_ID"
}

_bmux_pr_output_indicates_no_pull_request() {
    local output="${1:l}"
...
_bmux_git_config_resolve_include_path() {
...
_bmux_git_config_gitdir_pattern_matches() {
...
_bmux_git_config_include_condition_matches() {
    ...
    case "$lower" in
        gitdir/i:*)
            ...
            _bmux_git_config_gitdir_pattern_matches "$pattern" "$repo_path" "$git_dir" "$common_dir" 1 ;;
        gitdir:*)
            pattern="${condition#gitdir:}"
            _bmux_git_config_gitdir_pattern_matches "$pattern" "$repo_path" "$git_dir" "$common_dir" 0 ;;
        *)
            return 1 ;;
    esac
}

_bmux_git_origin_url_read_config_file() {
    ...
    case "$_bmux_git_origin_url_seen" in
        *$'\n'"$config_file"$'\n'*) return 0 ;;
    esac
    _bmux_git_origin_url_depth=$(( _bmux_git_origin_url_depth + 1 ))
    [[ "$_bmux_git_origin_url_depth" -le 32 ]] || return 0
    _bmux_git_origin_url_seen+="$config_file"$'\n'
    ...
    while IFS=$'\t' read -r kind entry_payload entry_value; do
        case "$kind" in
            remote)
                [[ -n "$entry_payload" ]] && _bmux_git_origin_url_result="$entry_payload" ;;
            include)
                include_path="$(_bmux_git_config_resolve_include_path "$entry_payload" "$config_dir")"
                [[ -r "$include_path" ]] && _bmux_git_origin_url_read_config_file "$repo_path" "$git_dir" "$common_dir" "$include_path" ;;
            includeIf)
                if _bmux_git_config_include_condition_matches "$entry_payload" "$repo_path" "$git_dir" "$common_dir"; then
                    include_path="$(_bmux_git_config_resolve_include_path "$entry_value" "$config_dir")"
                    [[ -r "$include_path" ]] && _bmux_git_origin_url_read_config_file "$repo_path" "$git_dir" "$common_dir" "$include_path"
                fi ;;
        esac
    done <<< "$output"
...
_bmux_git_origin_url_from_config_files() {
    local repo_path="$1" git_dir="$2" common_dir="$3"
    local _bmux_git_origin_url_seen=$'\n'
    local _bmux_git_origin_url_depth=0
    local _bmux_git_origin_url_result=""

    [[ -r "$common_dir/config" ]] && _bmux_git_origin_url_read_config_file "$repo_path" "$git_dir" "$common_dir" "$common_dir/config"
    [[ "$git_dir" != "$common_dir" && -r "$git_dir/config" ]] && _bmux_git_origin_url_read_config_file "$repo_path" "$git_dir" "$common_dir" "$git_dir/config"
    [[ -n "$_bmux_git_origin_url_result" ]] && printf '%s\n' "$_bmux_git_origin_url_result"
}

_bmux_github_repo_slug_for_path() {
    local repo_path="$1"
    local git_dir="" common_dir="" remote_url="" path_part=""
    [[ -n "$repo_path" ]] || return 0

    git_dir="$(_bmux_git_resolve_git_dir "$repo_path" 2>/dev/null || true)"
    [[ -n "$git_dir" ]] || return 0
    common_dir="$git_dir"
    ...
    remote_url="$(_bmux_git_origin_url_from_config_files "$repo_path" "$git_dir" "$common_dir")"
...
_bmux_pr_cache_prefix() {
    [[ -n "$BMUX_PANEL_ID" ]] || return 1
    print -r -- "/tmp/bmux-pr-cache-${BMUX_PANEL_ID}"
}

_bmux_pr_force_signal_path() {
    [[ -n "$BMUX_PANEL_ID" ]] || return 1
    print -r -- "/tmp/bmux-pr-force-${BMUX_PANEL_ID}"
}

_bmux_pr_debug_log() {
    (( _BMUX_PR_DEBUG )) || return 0

    local branch="$1"
    local event="$2"
    local now="${EPOCHSECONDS:-$SECONDS}"
    printf '%s\tbranch=%s\tevent=%s\n' "$now" "$branch" "$event" >> /tmp/bmux-pr-debug.log
}

_bmux_pr_cache_clear() {
    local prefix=""
    prefix="$(_bmux_pr_cache_prefix 2>/dev/null || true)"
    if [[ -n "$prefix" ]]; then
        /bin/rm -f -- \
            "${prefix}.branch" \
            "${prefix}.repo" \
    ...
    _BMUX_PR_LAST_BRANCH=""
    _BMUX_PR_NO_PR_BRANCH=""
}

_bmux_pr_request_probe() {
    local signal_path=""
    signal_path="$(_bmux_pr_force_signal_path 2>/dev/null || true)"
    [[ -n "$signal_path" ]] || return 0
    : >| "$signal_path"
}

_bmux_report_pr_for_path() {
    local repo_path="$1"
    local force_probe="${2:-0}"
    if [[ "${BMUX_NO_PR_WATCH:-}" == "1" ]]; then
        _bmux_pr_cache_clear
        _bmux_clear_pr_for_panel
        return 0
    fi
    ...
    if [[ -z "$branch" ]] || ! command -v gh >/dev/null 2>&1; then
        _bmux_pr_debug_log "$branch" "cache-miss:clear"
        _bmux_pr_cache_clear
        _bmux_clear_pr_for_panel
        return 0
    fi
    ...
    _BMUX_PR_LAST_BRANCH="$cache_branch"
    _BMUX_PR_NO_PR_BRANCH="$cache_no_pr_branch"
    if [[ "$cache_branch" == "$branch" && -n "$cache_result" ]]; then
        _bmux_pr_debug_log "$branch" "cache-refresh"
    else
        _bmux_pr_debug_log "$branch" "cache-miss"
    fi

    repo_slug="$(_bmux_github_repo_slug_for_path "$repo_path")"
    if [[ -n "$repo_slug" ]]; then
        gh_repo_args=(--repo "$repo_slug")
    fi

    err_file="$(/usr/bin/mktemp "${TMPDIR:-/tmp}/bmux-gh-pr-view.XXXXXX" 2>/dev/null || true)"
    [[ -n "$err_file" ]] || return 1
    gh_output="$(
    ...
    if (( gh_status != 0 )) || [[ -z "$gh_output" ]]; then
        if (( gh_status == 0 )) && [[ -z "$gh_output" ]]; then
            ...
            _BMUX_PR_LAST_BRANCH="$branch"
            _BMUX_PR_NO_PR_BRANCH="$branch"
            _bmux_clear_pr_for_panel
            return 0
        fi
        if _bmux_pr_output_indicates_no_pull_request "$gh_error"; then
            if [[ -n "$prefix" ]]; then
                print -r -- "$branch" >| "$branch_file"
                print -r -- "$repo_path" >| "$repo_file"
                print -r -- "$now" >| "$timestamp_file"
...
_bmux_kill_process_tree() {
    ...
    while IFS= read -r child_pid; do
        ...
        _bmux_kill_process_tree "$child_pid" "$signal"
    done < <(_bmux_child_pids "$pid")

    kill "-$signal" "$pid" >/dev/null 2>&1 || true
}

_bmux_run_pr_probe_with_timeout() {
    local repo_path="$1"
    local force_probe="${2:-0}"
    local probe_pid=""
    ...
    (
        _bmux_report_pr_for_path "$repo_path" "$force_probe"
    ) &
    probe_pid=$!
    ...
    while kill -0 "$probe_pid" >/dev/null 2>&1; do
        ...
        if (( _BMUX_ASYNC_JOB_TIMEOUT > 0 )) && (( now - started_at >= _BMUX_ASYNC_JOB_TIMEOUT )); then
            _bmux_kill_process_tree "$probe_pid" TERM
            sleep 0.2
            if kill -0 "$probe_pid" >/dev/null 2>&1; then
                _bmux_kill_process_tree "$probe_pid" KILL
                sleep 0.2
            fi
            if ! kill -0 "$probe_pid" >/dev/null 2>&1; then
...
_bmux_halt_pr_poll_loop() {
    ...
    [[ -z "$_BMUX_PR_POLL_PID" ]] || kill -KILL -- -"$_BMUX_PR_POLL_PID" 2>/dev/null || true
    local signal_path=""
    [[ -n "$BMUX_PANEL_ID" ]] && signal_path="/tmp/bmux-pr-force-${BMUX_PANEL_ID}"
...
_bmux_start_pr_poll_loop() {
    ...
    if [[ -n "$_BMUX_PR_POLL_PID" ]] && kill -0 "$_BMUX_PR_POLL_PID" 2>/dev/null; then
        _bmux_halt_pr_poll_loop
    else
        _BMUX_PR_POLL_PID=""
    fi
    _BMUX_PR_POLL_PWD="$watch_pwd"

    {
        ...
        while true; do
            ...
            _bmux_run_pr_probe_with_timeout "$watch_pwd" "$force_probe" || true
...
_bmux_stop_git_head_watch() {
    [[ -n "$_BMUX_GIT_HEAD_WATCH_PID" ]] || return 0
    kill "$_BMUX_GIT_HEAD_WATCH_PID" >/dev/null 2>&1 || true
    _BMUX_GIT_HEAD_WATCH_PID=""
}

_bmux_start_git_head_watch() {
    [[ "${BMUX_NO_GIT_WATCH:-}" == "1" ]] && return 0
    [[ -S "$BMUX_SOCKET_PATH" ]] || return 0
    [[ -n "$BMUX_TAB_ID" ]] || return 0
    [[ -n "$BMUX_PANEL_ID" ]] || return 0
    ...
    watch_head_path="$(_bmux_git_resolve_head_path "$watch_pwd" 2>/dev/null || true)"
    [[ -n "$watch_head_path" ]] || return 0

    local watch_head_signature
    watch_head_signature="$(_bmux_git_head_signature "$watch_head_path" 2>/dev/null || true)"

    _BMUX_GIT_HEAD_LAST_PWD="$watch_pwd"
    _BMUX_GIT_HEAD_PATH="$watch_head_path"
    _BMUX_GIT_HEAD_SIGNATURE="$watch_head_signature"
    ...
    local watch_shell_pid="$$"
    {
        ...
        while true; do
            ...
            local signature
            signature="$(_bmux_git_head_signature "$watch_head_path" 2>/dev/null || true)"
            if [[ -n "$signature" && "$signature" != "$last_signature" ]]; then
                last_signature="$signature"
                _bmux_pr_cache_clear
                _bmux_report_git_branch_for_path "$watch_pwd"
                _bmux_clear_pr_for_panel
            fi
        done
    } >/dev/null 2>&1 &!
...
_bmux_command_starts_nested_shell() {
    local cmd="$1"
    local -a words
    words=("${(z)cmd}")
...
_bmux_preexec() {
    ...
    _bmux_normalize_claude_config_dir
    if (( ! _BMUX_DELAY_TERM_RESTORE_UNTIL_FIRST_PROMPT )); then
        _bmux_restore_terminal_identity_after_startup
    fi
    _bmux_tmux_sync_bmux_environment

    if [[ -z "$_BMUX_TTY_NAME" ]]; then
        local t
        t="$(tty 2>/dev/null || true)"
        t="${t##*/}"
        [[ -n "$t" && "$t" != "not a tty" ]] && _BMUX_TTY_NAME="$t"
    fi

    _BMUX_CMD_START="$(_bmux_now)"
    _bmux_report_shell_activity_state running
    _bmux_record_pr_command_hint "$cmd"
    ...
    if _bmux_command_starts_nested_shell "$cmd"; then
        return 0
    fi
    _bmux_start_git_head_watch
}
...
_bmux_precmd() {
    ...
    _bmux_patch_ghostty_job_table_guard
    (( _BMUX_GHOSTTY_SEMANTIC_PATCHED )) || _bmux_patch_ghostty_semantic_redraw
    _bmux_stop_git_head_watch
    _bmux_zsh_job_table_saturated && return 0

    _bmux_normalize_claude_config_dir
    if (( _BMUX_DELAY_TERM_RESTORE_UNTIL_FIRST_PROMPT )); then
        _BMUX_DELAY_TERM_RESTORE_UNTIL_FIRST_PROMPT=0
    fi
    _bmux_tmux_sync_bmux_environment

    local bmux_has_unix_socket=0
    _bmux_socket_is_unix && bmux_has_unix_socket=1
    (( bmux_has_unix_socket )) || _bmux_has_port_scan_transport || return 0
    [[ -n "$BMUX_TAB_ID" ]] || return 0
    if [[ -n "$BMUX_PANEL_ID" ]]; then
        _bmux_reset_terminal_keyboard_protocols
    ...
    local now="$(_bmux_now)"
    local cmd_start="$_BMUX_CMD_START"
    _BMUX_CMD_START=0
    local pwd="$PWD"
    local cmd_dur=0
    ...
    if (( ! bmux_has_unix_socket )); then
        if [[ "$pwd" != "$_BMUX_PWD_LAST_PWD" ]]; then
            _bmux_report_pwd_via_relay "$pwd" && _BMUX_PWD_LAST_PWD="$pwd"
        fi
        if (( cmd_dur >= 2 || now - _BMUX_PORTS_LAST_RUN >= 10 )); then
    ...
    [[ -n "$BMUX_PANEL_ID" ]] || return 0
    _bmux_set_git_active_pwd "$pwd"

    _bmux_prompt_wrap_guard "$cmd_start" "$pwd"

    # Post-wake socket writes can occasionally leave a probe process wedged.
    # If one probe is stale, clear the guard so fresh async probes can resume.
    if [[ -n "$_BMUX_GIT_JOB_PID" ]]; then
        if ! kill -0 "$_BMUX_GIT_JOB_PID" 2>/dev/null; then
            _BMUX_GIT_JOB_PID=""
            _BMUX_GIT_JOB_STARTED_AT=0
        elif (( _BMUX_GIT_JOB_STARTED_AT > 0 )) && (( now - _BMUX_GIT_JOB_STARTED_AT >= _BMUX_ASYNC_JOB_TIMEOUT )); then
            _BMUX_GIT_JOB_PID=""
            _BMUX_GIT_JOB_STARTED_AT=0
            _BMUX_GIT_FORCE=1
        fi
    ...
    if [[ "$pwd" != "$_BMUX_PWD_LAST_PWD" ]]; then
        ...
        local qpwd="${pwd//\"/\\\"}"
        _bmux_send_bg "report_pwd \"${qpwd}\" --tab=$BMUX_TAB_ID --panel=$BMUX_PANEL_ID"
    fi

    # Git branch/dirty: update immediately on directory change, otherwise every ~3s.
    # While a foreground command is running, _bmux_start_git_head_watch probes HEAD
    ...
    if [[ "${BMUX_NO_GIT_WATCH:-}" == "1" ]]; then
        _bmux_stop_pr_poll_loop
        _bmux_stop_git_head_watch
        if [[ -n "$_BMUX_GIT_JOB_PID" ]] && kill -0 "$_BMUX_GIT_JOB_PID" 2>/dev/null; then
            kill "$_BMUX_GIT_JOB_PID" >/dev/null 2>&1 || true
        fi
        _BMUX_GIT_JOB_PID=""
        _BMUX_GIT_JOB_STARTED_AT=0
        _BMUX_GIT_FORCE=0
        ...
        if [[ "$pwd" != "$_BMUX_GIT_HEAD_LAST_PWD" ]]; then
            _BMUX_GIT_HEAD_LAST_PWD="$pwd"
            _BMUX_GIT_HEAD_PATH="$(_bmux_git_resolve_head_path "$pwd" 2>/dev/null || true)"
            _BMUX_GIT_HEAD_SIGNATURE=""
        fi
        if [[ -n "$_BMUX_GIT_HEAD_PATH" ]]; then
            local head_signature
            head_signature="$(_bmux_git_head_signature "$_BMUX_GIT_HEAD_PATH" 2>/dev/null || true)"
            if [[ -n "$head_signature" ]]; then
                if [[ -z "$_BMUX_GIT_HEAD_SIGNATURE" ]]; then
                    ...
                    _BMUX_GIT_HEAD_SIGNATURE="$head_signature"
                elif [[ "$head_signature" != "$_BMUX_GIT_HEAD_SIGNATURE" ]]; then
                    _BMUX_GIT_HEAD_SIGNATURE="$head_signature"
                    git_head_changed=1
    ...
    if [[ "$pwd" != "$_BMUX_GIT_LAST_PWD" ]]; then
        should_git=1
    elif (( _BMUX_GIT_FORCE )); then
        should_git=1
    elif (( now - _BMUX_GIT_LAST_RUN >= 3 )); then
        should_git=1
    fi

    if [[ "${BMUX_NO_GIT_WATCH:-}" != "1" ]] && (( should_git )); then
        ...
        if [[ -n "$_BMUX_GIT_JOB_PID" ]] && kill -0 "$_BMUX_GIT_JOB_PID" 2>/dev/null; then
            ...
            if [[ "$pwd" != "$_BMUX_GIT_LAST_PWD" ]] || (( _BMUX_GIT_FORCE )); then
                kill "$_BMUX_GIT_JOB_PID" >/dev/null 2>&1 || true
                _BMUX_GIT_JOB_PID=""
                _BMUX_GIT_JOB_STARTED_AT=0
            else
        ...
        if (( can_launch_git )); then
            _BMUX_GIT_FORCE=0
            _BMUX_GIT_LAST_PWD="$pwd"
            _BMUX_GIT_LAST_RUN=$now
            {
                _bmux_report_git_branch_for_path "$pwd"
            } >/dev/null 2>&1 &!
            _BMUX_GIT_JOB_PID=$!
            _BMUX_GIT_JOB_STARTED_AT=$now
        fi
    fi
    ...
    if [[ "${BMUX_NO_GIT_WATCH:-}" != "1" ]] && (( last_status == 0 )); then
        _bmux_emit_pr_command_hint
    else
        _BMUX_LAST_PR_ACTION=""
        _BMUX_LAST_PR_TARGET=""
...
# Contents/MacOS entry so the GUI bmux binary cannot shadow the CLI bmux.
...
# reinstall bmux-owned wrapper functions in case user startup replaced them.
_bmux_fix_path() {
    if [[ -n "${GHOSTTY_BIN_DIR:-}" ]]; then
        local gui_dir="${GHOSTTY_BIN_DIR%/}"
        local bin_dir="${gui_dir%/MacOS}/Resources/bin"
        if [[ -d "$bin_dir" ]]; then
            PATH="$(_bmux_path_prepend_unique_directory "$bin_dir" "${PATH-}" "$gui_dir")"
        fi
    fi
    _bmux_install_cli_wrapper claude _BMUX_CLAUDE_WRAPPER bmux-claude-wrapper
    _bmux_install_cli_wrapper grok _BMUX_GROK_WRAPPER
    add-zsh-hook -d precmd _bmux_fix_path
}
...
_bmux_restore_terminal_identity_after_startup() {
    if [[ -n "${BMUX_ZSH_RESTORE_TERM:-}" ]]; then
        builtin export TERM="$BMUX_ZSH_RESTORE_TERM"
        builtin unset BMUX_ZSH_RESTORE_TERM
    fi
    _BMUX_DELAY_TERM_RESTORE_UNTIL_FIRST_PROMPT=0
}
...
_bmux_zshexit() {
    _bmux_stop_git_head_watch
    _bmux_stop_pr_poll_loop
    [[ -n "${_BMUX_GIT_ACTIVE_PWD_FILE:-}" ]] && /bin/rm -f -- "$_BMUX_GIT_ACTIVE_PWD_FILE" >/dev/null 2>&1 || true
}

autoload -Uz add-zsh-hook
add-zsh-hook preexec _bmux_preexec
add-zsh-hook precmd _bmux_precmd
add-zsh-hook precmd _bmux_fix_path
add-zsh-hook chpwd _bmux_chpwd
add-zsh-hook zshexit _bmux_zshexit
