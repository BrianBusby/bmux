// bmux-feed-plugin-marker v1
// Bridges OpenCode's plugin event bus to the bmux socket's feed.* verbs.
// Installed by `bmux hooks setup` or `bmux hooks opencode install`.
...
const DEFAULT_SOCKET = `${os.homedir()}/.config/bmux/bmux.sock`;
const SOCKET_PATH = process.env.BMUX_SOCKET_PATH || DEFAULT_SOCKET;
const REPLY_TIMEOUT_MS = 120_000;
const MAX_PLAN_BYTES = 128 * 1024;

export const BMUXFeed = async (ctx) => {
  let client = null;
  let buffered = "";
  ...
  const readPlanFile = (planFilePath) => {
    ...
    try {
      ...
      return `${text}\n\n[bmux truncated plan file at ${bytes} bytes.]`;
    } finally {
      fs.closeSync(fd);
    }
  };
  ...
  const handleExitPlanDecision = async (sid, requestId, decision) => {
    ...
    if (feedback) {
      ...
      await sendPlanFeedback(
        sid,
        `User rejected the plan via bmux Feed and wants this change: ${feedback}\n\nUpdate the plan file, then call plan_exit again.`
      );
      return;
    }
    ...
    if (mode === "ultraplan") {
      ...
      await sendPlanFeedback(
        sid,
        "User chose Ultraplan via bmux Feed. Refine the plan more deeply, update the plan file, then call plan_exit again."
      );
      return;
    }
    ...
    if (!permissionsApplied) {
      ...
      await sendPlanFeedback(
        sid,
        "bmux could not apply the selected permission mode. Ask the user to approve the plan again before switching to build mode."
      );
      return;
    }
  ...
  const base = (sessionId, extra) => {
    ...
    const workspaceId =
      typeof process.env.BMUX_WORKSPACE_ID === "string" && process.env.BMUX_WORKSPACE_ID.trim()
        ? process.env.BMUX_WORKSPACE_ID.trim()
        : null;
    const event = {
          ...
          if (result?.status === "resolved" && result.decision?.kind === "permission") {
            ...
            try {
              await replyPermission({
                ...
                message: mode === "deny" ? "User denied permission via bmux Feed." : undefined,
              });
