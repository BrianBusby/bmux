...
# Defaults to 127.0.0.1:17321 to match the existing tools/bmux-tag-opener
...
IOS_TEAM = os.environ.get("BMUX_IOS_TEAM", "7WLXT3NR37")
...
TAG_MARKER_PATH = "/tmp/bmux-mobile-attach-qr-tags.json"
...
def _apply_tags(mac_tag: str, ios_tag: str) -> None:
    ...
    TAG = mac_tag
    IOS_TAG = ios_tag
    OUT_DIR = os.path.join(TMP_ROOT, f"bmux-mobile-attach-qr-{TAG}")
    IOS_TAG_SLUG = _ios_slug(IOS_TAG)
    IOS_BUNDLE_ID = f"dev.bmux.ios.{IOS_TAG_SLUG}"
...
def tagged_app_path() -> str:
    ...
    lockstep with the BMUX Tag Opener's `appURL(for:)` resolution so the
...
def tagged_app_executable() -> str:
    return os.path.join(tagged_app_path(), "Contents", "MacOS", "bmux DEV")


def app_info() -> dict:
    """Single source of truth for what 'Open bmux DEV <tag>' actually does.
    ...
    try:
        out = subprocess.run(
            ["pgrep", "-f", f"bmux DEV {TAG}.app/Contents/MacOS/bmux DEV"],
            check=False,
            timeout=2,
...
class Handler(http.server.BaseHTTPRequestHandler):
    def log_message(self, format: str, *args) -> None:
        # Stay quiet, the bmux helper pane is the visible signal.
        return
    ...
    def _open_tag(self) -> None:
        ...
        # macOS: launch the tagged `.app` via the BMUX Tag Opener at :17320 if
    ...
    def _serve_qr_page(self) -> None:
        ...
        with open(html_path, "rb") as fh:
            html = fh.read()
        # Inject a meta refresh + a small banner + an "Open bmux DEV <tag>"
        # button that hits the BMUX Tag Opener at 127.0.0.1:17320.
        marker = b"</head>"
        injection = (
        ...
        banner = (
            b'<button class="qr-open-tag" type="button" id="bmux-open-btn" disabled '
            b'title="resolving build..." '
            b'onclick="bmuxOpenTag(this)">resolving...</button>'
            + b'<div class="qr-fresh-banner">live, regenerates every 45s</div>'
            + b'<script>(function(){\n'
            + b'var bmuxBuilding=false;\n'
            ...
            + b'    var btn=document.getElementById("bmux-open-btn");\n'
            + b'    if(!btn||bmuxBuilding)return;\n'
            + b'    var built=fmtMtime(info.mtime);\n'
            + b'    var running=info.running_pid?(" \\u00b7 running pid "+info.running_pid):"";\n'
            + b'    if(info.exists){\n'
            + b'      btn.disabled=false;\n'
            + b'      btn.innerHTML="Open <code>bmux DEV "+info.tag+"</code> \\u00b7 built "+built+running;\n'
            + b'      btn.title=info.app_path;\n'
            + b'    }else{\n'
            + b'      btn.disabled=true;\n'
            + b'      btn.innerHTML="<code>bmux DEV "+info.tag+"</code> not built";\n'
            + b'      btn.title="missing on disk: "+info.app_path;\n'
            + b'    }\n'
            + b'  }).catch(function(){});}\n'
            + b'window.bmuxOpenTag=function(btn){\n'
            + b'  if(btn.disabled)return;\n'
            + b'  bmuxBuilding=true;btn.disabled=true;\n'
            ...
            + b'   .finally(function(){bmuxBuilding=false;btn.disabled=false;refresh();});};\n'
