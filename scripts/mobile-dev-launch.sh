#!/usr/bin/env bash
# Launch a tagged bmux iOS DEV build fully signed in (and optionally paired to a
...
#   BMUX_UITEST_STACK_EMAIL / BMUX_UITEST_STACK_PASSWORD  -> real Stack sign-in
#   BMUX_UITEST_MOCK_DATA=0                               -> real backend, not mock
#   BMUX_DOGFOOD_ATTACH_URL=<bmux-ios://attach...>        -> auto-pair after sign-in
...
# account (~/.secrets/bmuxterm-dev.env) wins by default; --agent forces the
# shared agent account (~/.secrets/bmux.env).
#
# Usage:
...
#
#   --attach   also pair to the running Mac. Uses BMUX_DOGFOOD_ATTACH_URL when it
...
AGENT=0
DETACH=0
ATTACH_TTL_SECONDS="${BMUX_ATTACH_TTL_SECONDS:-600}"
...
if [[ "$AGENT" -eq 1 ]]; then
  bmux_dev_secrets_load --agent || exit $?
else
  bmux_dev_secrets_load || exit $?
fi

# --- bundle id (matches ios/scripts/reload.sh sanitize_tag) ------------------
slug="$(bmux_attach__slug "$TAG")"
BUNDLE_ID="dev.bmux.ios.$slug"

# --- attach ticket ----------------------------------------------------------
# ATTACH_URL stays empty unless attach was explicitly requested, so a stale
# ambient BMUX_DOGFOOD_ATTACH_URL can NEVER auto-pair an unrequested launch
# (e.g. --no-attach from the reload scripts leaves ATTACH=0). The URL is injected
# as BMUX_DOGFOOD_ATTACH_URL, the NOT-mock-gated var the app reads with the real
# backend (BMUX_UITEST_MOCK_DATA=0).
ATTACH_URL=""
if [[ "$ATTACH" -eq 1 ]]; then
  ...
  if [[ "$ENSURE_MAC" -eq 0 ]]; then
    ATTACH_URL="${BMUX_DOGFOOD_ATTACH_URL:-}"
  fi
  if [[ -z "$ATTACH_URL" ]]; then
    if [[ "$ENSURE_MAC" -eq 1 ]]; then
      bmux_attach_ensure_mac "$TAG" "$REPO_ROOT" || true
    fi
    ...
    if bmux_attach_mac_socket_ready "$TAG"; then
      ATTACH_URL="$(bmux_attach_mint_url "$TAG" "$ATTACH_TTL_SECONDS" "$REPO_ROOT" || true)"
    fi
  fi
  if [[ -z "$ATTACH_URL" ]]; then
    if [[ "$ENSURE_MAC" -eq 1 ]]; then
...
echo "==> launching $BUNDLE_ID on $TARGET (signed in as $BMUX_UITEST_STACK_EMAIL${ATTACH_URL:+, auto-pairing})"
...
if [[ "$TARGET" == "simulator" ]]; then
  ...
  SIMCTL_CHILD_BMUX_UITEST_STACK_EMAIL="$BMUX_UITEST_STACK_EMAIL" \
  SIMCTL_CHILD_BMUX_UITEST_STACK_PASSWORD="$BMUX_UITEST_STACK_PASSWORD" \
  SIMCTL_CHILD_BMUX_UITEST_MOCK_DATA="0" \
  SIMCTL_CHILD_BMUX_DOGFOOD_ATTACH_URL="$ATTACH_URL" \
  ...
  DEVICECTL_CHILD_BMUX_UITEST_STACK_EMAIL="$BMUX_UITEST_STACK_EMAIL" \
  DEVICECTL_CHILD_BMUX_UITEST_STACK_PASSWORD="$BMUX_UITEST_STACK_PASSWORD" \
  DEVICECTL_CHILD_BMUX_UITEST_MOCK_DATA="0" \
  DEVICECTL_CHILD_BMUX_DOGFOOD_ATTACH_URL="$ATTACH_URL" \
