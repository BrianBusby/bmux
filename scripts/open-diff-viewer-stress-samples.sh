...
through the normal local git codepath:
  ...
  bmux diff --branch --base <base-ref>
...
Environment:
  BMUX_WORKSPACE_ID and BMUX_SURFACE_ID choose the target workspace/surface.
  BMUX_DIFF_STRESS_ROOT overrides the clone cache root. Each sample family gets
  its own parent directory so the normal bmux repo switcher does not treat
...
SAMPLE="bun-rust"
CLI="bmux"
ROOT="${BMUX_DIFF_STRESS_ROOT:-/tmp/bmux-diff-viewer-stress}"
...
sample_fetch_args() {
  case "$1" in
    bun-rust) echo "0d9b296af33f2b851fcbf4df3e9ec89751734ba4 pull/30412/head:refs/remotes/bmux-stress/bun-rust" ;;
    node-v8) echo "4d8834fbef690bf71dc9eb6bdd9edfb0783b3c5d pull/62526/head:refs/remotes/bmux-stress/node-v8" ;;
    node-v8-14-1) echo "0817b40c1b2938cff3c30f026d0ad4b255beb11d pull/59805/head:refs/remotes/bmux-stress/node-v8-14-1" ;;
...
sample_head_ref() {
  case "$1" in
    bun-rust) echo "refs/remotes/bmux-stress/bun-rust" ;;
    node-v8) echo "refs/remotes/bmux-stress/node-v8" ;;
    node-v8-14-1) echo "refs/remotes/bmux-stress/node-v8-14-1" ;;
...
sample_branch() {
  case "$1" in
    bun-rust) echo "bmux-stress-bun-rust" ;;
    node-v8) echo "bmux-stress-node-v8" ;;
    node-v8-14-1) echo "bmux-stress-node-v8-14-1" ;;
    linux-v6) echo "bmux-stress-linux-v6" ;;
    *) return 1 ;;
  esac
}
...
open_sample() {
  ...
  if [ -n "${BMUX_WORKSPACE_ID:-}" ]; then
    args+=(--workspace "$BMUX_WORKSPACE_ID")
  fi
  if [ -n "${BMUX_SURFACE_ID:-}" ]; then
    args+=(--surface "$BMUX_SURFACE_ID")
  fi
