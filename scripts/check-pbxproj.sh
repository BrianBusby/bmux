#!/usr/bin/env bash
# CI guard for bmux.xcodeproj/project.pbxproj.
# Fails when:
...
if [[ "$actual" != "$EXPECTED_OBJECT_VERSION" ]]; then
    echo "::error file=bmux.xcodeproj/project.pbxproj,line=6::objectVersion is $actual, expected $EXPECTED_OBJECT_VERSION for Xcode $XCODE_VERSION." >&2
