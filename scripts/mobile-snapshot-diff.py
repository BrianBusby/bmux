...
BMUX_TAG = os.environ.get("BMUX_TAG", "mobile")
DEBUG_CLI = os.path.join(REPO_ROOT, "scripts", "bmux-debug-cli.sh")


def run_cli(*args):
    env = os.environ.copy()
    env["BMUX_TAG"] = BMUX_TAG
...
def mac_visible_lines(workspace_id: str, terminal_id: str, lines: int = 45):
    """The Mac side via `bmux read-screen`, the canonical CLI used by
