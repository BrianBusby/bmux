...
// paired. Everything routes through `scripts/bmux-debug-cli.sh` (sibling
// `../bmux-debug-cli.sh`), which targets the TAGGED socket only.
//
// --dry-run prints the resolved `bmux` argument vectors for each profile
...
const DEBUG_CLI = path.join(SCRIPT_DIR, "..", "bmux-debug-cli.sh");
  ...
  if (opts.dryRun) {
    for (const { name, profile } of profiles) {
      ...
      for (const { argv } of resolveSteps(profile, context)) {
        process.stdout.write(`bmux ${argv.join(" ")}\n`);
      }
    }
    return;
  }
