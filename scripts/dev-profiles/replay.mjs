// Replay engine for bmux dev environment profiles (P3 of turnkey dev builds).
//
...
// TAGGED dev bmux instance. Every step is replayed through
// `scripts/bmux-debug-cli.sh`, which refuses without `BMUX_TAG` and targets
// `/tmp/bmux-debug-<slug>.sock` (never the user's stable app).
//
...
//   }
//
// - `args` is the exact `bmux` argument vector for that step (no leading `bmux`).
...
//   `${name}`. Captured values are bmux refs/ids, never secrets.
//
...
/**
 * Resolve a profile into the concrete `bmux` argument vectors that would run,
...
/**
 * Executes resolved profile steps against a tagged dev bmux socket.
 *
 * Each step shells out to `scripts/bmux-debug-cli.sh` with `BMUX_TAG` set, so
...
export class ProfileReplayer {
  /**
   * @param {object} opts
   * @param {string} opts.tag The `BMUX_TAG` value (tagged dev build).
   * @param {string} opts.cliPath Absolute path to `scripts/bmux-debug-cli.sh`.
  ...
  runStep(step) {
    ...
    this.log(`  bmux ${display}`);
    const res = spawnSync(this.cliPath, step.argv, {
      encoding: "utf8",
      env: { ...process.env, BMUX_TAG: this.tag, BMUX_QUIET: "1" },
    });
    if (res.error) {
    ...
    if (res.status !== 0) {
      ...
      throw new Error(`step failed (exit ${res.status}): bmux ${display}\n${detail}`);
    }
    ...
    if (step.capture) {
      ...
      try {
        ...
        throw new Error(
          `step declared captures but stdout was not JSON: bmux ${display}`,
        );
      }
      for (const [name, jsonPath] of Object.entries(step.capture)) {
        ...
        if (value === undefined || value === null) {
          throw new Error(
            `capture "${name}" path "${jsonPath}" not found in output of: bmux ${display}`,
          );
        }
