...
provisions a realistic test environment in a **tagged** dev bmux instance so a
...
`/tmp/bmux-debug-<slug>.sock` only, never the user's stable app. Profiles are
...
# Dry-run: print the resolved `bmux` arg vectors without touching a socket:
...
- **`steps`** (required): an ordered list. Each step is one `bmux` invocation.
- **`args`** (required): the exact `bmux` argument vector for the step, with the
  leading `bmux` omitted. These are passed verbatim to `bmux-debug-cli.sh`.
...
- **`capture`** (optional): maps a local variable name to a dotted JSON path
  ...
  values are bmux refs/ids, never secrets.

Only use `bmux` verbs that actually exist (see
`skills/bmux-workspace/references/commands.md` and the `bmux-groups` skill).
Prefer the canonical noun forms (`bmux workspace create`,
`bmux workspace group create`) which honor `--json`. Use `--focus false` on
creation steps so replay does not yank the dev window around.

The capture key name depends on the id format, because bmux renames id fields
...
- `replay.mjs` — the engine. `resolveSteps(profile, context)` is a pure,
  ...
  out to `bmux-debug-cli.sh` with `BMUX_TAG` set, so the tagged-socket safety
