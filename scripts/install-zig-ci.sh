...
ZIG_WORK_PARENT="${RUNNER_TEMP:-/tmp/bmux-zig-ci}"
...
publish_zig_for_later_steps() {
  ...
  if [ -n "${GITHUB_ENV:-}" ]; then
    echo "BMUX_ZIG=$zig_path" >> "$GITHUB_ENV"
  fi
}
...
ZIG_WORK_ROOT="$(mktemp -d "${ZIG_WORK_PARENT%/}/bmux-zig-install-${ZIG_REQUIRED}.XXXXXX")"
cleanup_work_root() {
  rm -rf "$ZIG_WORK_ROOT"
}
...
install_zig_without_sudo() {
  local install_parent="${RUNNER_TEMP:-/tmp/bmux-zig-ci}"
