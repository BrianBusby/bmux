...
# it never touches the user's stable bmux instance.
#
# Flow (--surface both):
...
#        defaults write com.bmuxterm.app.debug.<tag-id> mobile.iOSPairingHost.enabled -bool true
...
#      auto-signs-in from ~/.secrets/bmuxterm-dev.env (DebugDogfoodCredentialResolver).
...
#   5. Build + launch the iOS dev build, passing the URL as BMUX_DOGFOOD_ATTACH_URL
...
# must NOT export BMUX_UITEST_STACK_PASSWORD: reload.sh launches the long-lived
...
if ! ( source "$SCRIPT_DIR/lib/dev-secrets.sh"; bmux_dev_secrets_load "${DEV_SECRETS_ARGS[@]}" ); then
  exit 2
fi
...
# exact derivation so it stays in sync with reload.sh / bmux-debug-cli.sh.
dev_setup__sanitize_path() { bmux_attach__slug "$1"; }
...
enable_pairing_host() {
  ...
  bmux_attach_enable_pairing_host "$TAG"
}
...
build_and_launch_mac() {
  ...
  # auto-signs-in from ~/.secrets/bmuxterm-dev.env via DebugDogfoodCredentialResolver.
...
mint_attach_url() { bmux_attach_mint_url "$TAG" "$TTL_SECONDS" "$REPO_ROOT"; }

# --- iOS build + launch (auto-pair via BMUX_DOGFOOD_ATTACH_URL) -------------
build_and_launch_ios() {
  ...
  # pass via BMUX_DOGFOOD_ATTACH_URL (it ignores an ambient URL without --attach).
  ...
  # Pass the URL via env (BMUX_DOGFOOD_ATTACH_URL), never on the command line, so
  # it is not visible in `ps`. mobile-dev-launch.sh injects it into the app env.
  BMUX_DOGFOOD_ATTACH_URL="$attach_url" "$REPO_ROOT/scripts/mobile-dev-launch.sh" "${launch_args[@]}"
}

# --- apply environment profile(s) (P3) --------------------------------------
# Profiles provision a realistic test environment against the TAGGED Mac socket
# via scripts/bmux-debug-cli.sh. The replay engine spawns the debug CLI, which
# refuses without BMUX_TAG and never touches the stable app.
#
