...
IMMUTABLE_ASSET_PATTERNS = [
    re.compile(r"^bmux-nightly-macos-(?P<build>\d+)\.dmg$"),
    re.compile(r"^bmux-nightly-universal-macos-(?P<build>\d+)\.dmg$"),
    re.compile(r"^bmuxd-remote-(?:darwin-arm64|darwin-amd64|linux-arm64|linux-amd64)-(?P<build>\d+)$"),
    re.compile(r"^bmuxd-remote-checksums-(?P<build>\d+)\.txt$"),
    re.compile(r"^bmuxd-remote-manifest-(?P<build>\d+)\.json$"),
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--repo", required=True, help="owner/repo, for example manaflow-ai/bmux")
