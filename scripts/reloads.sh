...
APP_NAME="bmux STAGING"
BUNDLE_ID="com.bmuxterm.app.staging"
BASE_APP_NAME="bmux"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
...
write_last_socket_path() {
  ...
  local tmp_marker="/tmp/bmux-staging-last-socket-path"
  if [[ -n "${STAGING_SLUG:-}" ]]; then
    marker_name="staging-${STAGING_SLUG}-last-socket-path"
    tmp_marker="/tmp/bmux-staging-${STAGING_SLUG}-last-socket-path"
  fi
...
staging_slug_from_bundle_id() {
  local bundle_id="$1"
  local suffix=""
  if [[ "$bundle_id" == "com.bmuxterm.app.staging."* ]]; then
    suffix="${bundle_id#com.bmuxterm.app.staging.}"
  fi
...
Release build with isolated "bmux STAGING" identity. Runs side-by-side with
the production bmux app.
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$NAME_SET" -eq 0 ]]; then
    APP_NAME="bmux STAGING ${TAG}"
  fi
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.bmuxterm.app.staging.${TAG_ID}"
  fi
  if [[ "$DERIVED_SET" -eq 0 ]]; then
    DERIVED_DATA="/tmp/bmux-staging-${TAG_SLUG}"
  fi
fi
...
if [[ -f "$INFO_PLIST" ]]; then
  ...
  if [[ -n "$STAGING_SLUG" ]]; then
    BMUXD_SOCKET="${APP_SUPPORT_DIR}/bmuxd-${STAGING_SLUG}.sock"
    BMUX_SOCKET_PATH_VALUE="/tmp/bmux-staging-${STAGING_SLUG}.sock"
  else
    BMUXD_SOCKET="${APP_SUPPORT_DIR}/bmuxd-staging.sock"
    BMUX_SOCKET_PATH_VALUE="/tmp/bmux-staging.sock"
  fi
  write_last_socket_path "$BMUX_SOCKET_PATH_VALUE"
  /usr/libexec/PlistBuddy -c "Add :LSEnvironment dict" "$INFO_PLIST" 2>/dev/null || true
  /usr/libexec/PlistBuddy -c "Set :LSEnvironment:BMUX_BUNDLE_ID \"${BUNDLE_ID}\"" "$INFO_PLIST" 2>/dev/null \
    || /usr/libexec/PlistBuddy -c "Add :LSEnvironment:BMUX_BUNDLE_ID string \"${BUNDLE_ID}\"" "$INFO_PLIST"
  /usr/libexec/PlistBuddy -c "Set :LSEnvironment:BMUXD_UNIX_PATH \"${BMUXD_SOCKET}\"" "$INFO_PLIST" 2>/dev/null \
    || /usr/libexec/PlistBuddy -c "Add :LSEnvironment:BMUXD_UNIX_PATH string \"${BMUXD_SOCKET}\"" "$INFO_PLIST"
  /usr/libexec/PlistBuddy -c "Set :LSEnvironment:BMUX_SOCKET_PATH \"${BMUX_SOCKET_PATH_VALUE}\"" "$INFO_PLIST" 2>/dev/null \
    || /usr/libexec/PlistBuddy -c "Add :LSEnvironment:BMUX_SOCKET_PATH string \"${BMUX_SOCKET_PATH_VALUE}\"" "$INFO_PLIST"
...
"${OPEN_CLEAN_ENV[@]}" BMUX_BUNDLE_ID="$BUNDLE_ID" BMUX_SOCKET_PATH="$BMUX_SOCKET_PATH_VALUE" BMUXD_UNIX_PATH="$BMUXD_SOCKET" open -g "$APP_PATH"
