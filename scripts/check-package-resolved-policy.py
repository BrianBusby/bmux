#!/usr/bin/env python3
"""Verify bmux-owned SwiftPM lockfiles are not ignored."""
...
XCODE_PROJECT_FILE = "bmux.xcodeproj/project.pbxproj"
...
def package_roots_requiring_lockfiles(
    bmux_manifests: dict[str, Path] | None = None,
    graph: dict[str, tuple[bool, list[str]]] | None = None,
) -> set[str]:
    if bmux_manifests is None or graph is None:
        all_manifests = tracked_package_manifests(include_allowed_vendor=True)
        bmux_manifests = tracked_package_manifests(include_allowed_vendor=False)
        graph = package_graph(all_manifests)
    memo: dict[str, bool] = {}

    return {
        root for root in bmux_manifests
...
def main() -> int:
    ...
    graph = package_graph(all_manifests)
    roots = set(bmux_manifests)
    tracked_lockfiles = set(git_ls_files("*Package.resolved"))
    required_lockfile_roots = package_roots_requiring_lockfiles(bmux_manifests, graph)
    ...
    for gitignore in sorted(Path(".").rglob(".gitignore")):
        ...
        if is_allowed_vendor_path(rel):
            continue
        errors.append(
            f"{rel} ignores Package.resolved. bmux-owned SwiftPM lockfiles must be tracked."
        )
    ...
    for root, manifest in sorted(bmux_manifests.items()):
    ...
    for lockfile in tracked_lockfiles:
        ...
        if is_expected_lockfile_path(lockfile, roots):
            continue
        errors.append(f"Unexpected bmux Package.resolved location: {lockfile}")
