...
agent lives in another bmux instance), so the phone never sees it.

Data sources (read live over the tagged debug socket, no app rebuild):
  bmux rpc chat.sessions.dump   -> the registry
  bmux debug-terminals          -> current surfaces and their workspaces

Usage:  BMUX_TAG=<tag> scripts/bmux-chat-debug.py [--live] [--all]
"""
import json
import os
import re
...
CLI = os.path.join(os.path.dirname(__file__), "bmux-debug-cli.sh")


def cli(*args):
    if not os.environ.get("BMUX_TAG"):
        sys.exit("BMUX_TAG must be set, e.g. BMUX_TAG=asot scripts/bmux-chat-debug.py")
    out = subprocess.run([CLI, *args], capture_output=True, text=True, env=dict(os.environ))
    if out.returncode != 0:
        sys.exit("bmux-debug-cli.sh %s failed (rc=%d):\n%s"
...
def render(show_all):
    ...
    print("\033[1magent-chat host view\033[0m  tag=%s" % os.environ.get("BMUX_TAG", "?"))
