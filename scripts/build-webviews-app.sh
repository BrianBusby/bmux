...
write_agent_session_html() {
  ...
  {
    printf '<!doctype html>\n'
    printf '<html lang="en" data-bmux-webview-kind="agent-session" data-codex-window-type="electron" data-window-type="electron" data-codex-os="darwin">\n'
    printf '  <head>\n'
    printf '    <meta charset="UTF-8" />\n'
    printf '    <meta name="viewport" content="width=device-width, initial-scale=1.0" />\n'
    printf '    <title>bmux Agent Session</title>\n'
    printf '  </head>\n'
    printf '  <body data-bmux-webview-kind="agent-session" data-codex-window-type="electron">\n'
...
if [ "${1:-}" = "--check" ]; then
  ...
  (
    ...
    BMUX_WEBVIEWS_OUT_DIR="$tmp_dir" bun run build
