...
SOCKET_PATH="/tmp/bmux-debug.sock"
STABILITY_WAIT=15

echo "=== Smoke Test ==="

# --- Find the built app ---
APP=$(find ~/Library/Developer/Xcode/DerivedData -path "*/Build/Products/Debug/bmux DEV.app" -print -quit 2>/dev/null || true)
if [ -z "$APP" ]; then
  echo "ERROR: Built app not found in DerivedData"
  exit 1
fi
echo "App: $APP"
BINARY="$APP/Contents/MacOS/bmux DEV"
if [ ! -x "$BINARY" ]; then
...
echo "Launching app..."
BMUX_SOCKET_MODE=allowAll BMUX_UI_TEST_MODE=1 "$BINARY" > /tmp/bmux-smoke-stdout.log 2>&1 &
APP_PID=$!
...
if ! kill -0 "$APP_PID" 2>/dev/null; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/bmux-smoke-stdout.log 2>/dev/null | tail -50 || true
  echo "--- debug log ---"
  tail -50 /tmp/bmux-debug.log 2>/dev/null || true
  echo "--- crash reports ---"
  ls -lt ~/Library/Logs/DiagnosticReports/*bmux* 2>/dev/null | head -5 || echo "(none)"
  exit 1
fi
...
if [ "$SOCKET_READY" != "true" ]; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/bmux-smoke-stdout.log 2>/dev/null | tail -30 || true
  echo "--- debug log ---"
  tail -30 /tmp/bmux-debug.log 2>/dev/null || true
  ls -la /tmp/bmux-debug* 2>/dev/null || true
  pgrep -la "bmux" || echo "No bmux processes found"
  exit 1
fi
