...
LIB_NAME="libbmux_command_palette_nucleo_ffi.dylib"
BUILD_OUTPUT_DIR="${TARGET_BUILD_DIR:-${CRATE_DIR}/target}/bmux-nucleo-ffi"

if ! command -v cargo >/dev/null 2>&1; then
  case "${BMUX_NUCLEO_FFI_REQUIRE_CARGO:-${CI:-0}}" in
    1|true|TRUE|yes|YES)
...
requested_targets="${BMUX_NUCLEO_FFI_TARGETS:-}"
...
if [ -z "${requested_targets}" ]; then
  requested_archs="${BMUX_NUCLEO_FFI_ARCHS:-${ARCHS:-}}"
