#!/usr/bin/env python3
"""Check bmux-owned Swift warnings against a checked-in budget."""
...
OWNED_ROOTS = ("Sources", "CLI", "Packages", "bmuxTests", "bmuxUITests")
IGNORED_PATH_PARTS = (
    "/vendor/",
    "/ghostty/",
    "/homebrew-bmux/",
...
def write_budget(path: pathlib.Path, budget: WarningBudget) -> None:
    ...
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# bmux-owned Swift warning budget.\n")
...
def main(argv: list[str]) -> int:
    ...
    print_budget_summary("Actual bmux-owned Swift warnings", actual)
