...
# Interactive Kitty Graphics Protocol gallery for bmux/Ghostty verification.
# Downloads cat photos over HTTPS, converts them to PNG, then renders them.

cache_dir="${TMPDIR:-/tmp}/bmux-kitty-image-demo"
curl_user_agent="bmux-kitty-image-demo/1.0 (https://github.com/manaflow-ai/bmux)"
...
render_image() {
  ...
  clear_screen
  delete_images
  printf 'bmux Kitty graphics protocol gallery\n'
...
if ! supports_kitty_graphics; then
  ...
  printf 'Run inside bmux/Ghostty, WezTerm, or kitty.\n' >&2
  exit 1
fi
