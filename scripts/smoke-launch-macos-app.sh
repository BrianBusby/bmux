...
STARTUP_TIMEOUT_SECONDS="${BMUX_SMOKE_STARTUP_TIMEOUT_SECONDS:-10}"
STABLE_SECONDS="${BMUX_SMOKE_STABLE_SECONDS:-5}"
OPEN_LOG="$(mktemp -t bmux-smoke-open.XXXXXX)"
APP_PID=""
PREEXISTING_PIDS="$(pgrep -f "$EXECUTABLE_PATH" 2>/dev/null || true)"
DEBUG_LOGS="${BMUX_SMOKE_DEBUG_LOGS:-0}"
ALLOW_UNSUPPORTED_GUI="${BMUX_SMOKE_ALLOW_UNSUPPORTED_GUI:-0}"
DIRECT_EXEC="${BMUX_SMOKE_DIRECT_EXEC:-0}"
DISABLE_ICON_PERSISTENCE_KEY="bmuxDisableBundleIconPersistence"
...
dump_open_log() {
  ...
  if [[ "$DEBUG_LOGS" == "1" ]]; then
    ...
    echo "open launcher output captured (set BMUX_SMOKE_DEBUG_LOGS=1 to print)" >&2
  fi
}
...
dump_system_log() {
  if [[ "$DEBUG_LOGS" == "1" ]]; then
    ...
    echo "system log capture skipped (set BMUX_SMOKE_DEBUG_LOGS=1 to print)" >&2
  fi
}
...
if [[ "$DIRECT_EXEC" == "1" ]]; then
  BMUX_UI_TEST_MODE="${BMUX_UI_TEST_MODE:-1}" \
    "$EXECUTABLE_PATH" -ApplePersistenceIgnoreState YES --bmux-disable-bundle-icon-persistence >"$OPEN_LOG" 2>&1 &
else
  BMUX_UI_TEST_MODE="${BMUX_UI_TEST_MODE:-1}" \
    /usr/bin/open -n -g "$APP_PATH" --args -ApplePersistenceIgnoreState YES --bmux-disable-bundle-icon-persistence >"$OPEN_LOG" 2>&1 &
fi
OPEN_PID=$!
...
for _ in $(seq 1 "$STABLE_SECONDS"); do
  ...
  if ! kill -0 "$APP_PID" 2>/dev/null; then
    ...
    STARTUP_LOG="$HOME/Library/Logs/bmux/startup-${LOG_NAME}.log"
