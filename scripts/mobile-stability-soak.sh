...
Environment:
  ...
  SOAK_ROOT       Output directory. Defaults to /tmp/bmux-mobile-soak-<tag>-<profile>.
  BMUX_MOBILE_DEV_STACK_AUTH_TOKEN
...
dev_stack_auth_token="${BMUX_MOBILE_DEV_STACK_AUTH_TOKEN:-bmux-dev-mobile-stack-token}"
...
root="${SOAK_ROOT:-/tmp/bmux-mobile-soak-${tag}-${profile}}"
mkdir -p "$root"

session_prefix="bmux-${tag}-${profile}"
...
cleanup_tagged_soak_processes() {
  ...
  while IFS= read -r line; do
    ...
    command="${line#* }"
    [[ "$pid" =~ ^[0-9]+$ ]] || continue
    if [[ "$command" == *"mobile-stability-soak"* && "$command" == *"BMUX_TAG='$tag'"* ]]; then
  ...
  for sim_id in "$iphone_sim_id" "$ipad_sim_id"; do
    ...
    while IFS= read -r pid; do
      [[ "$pid" =~ ^[0-9]+$ ]] || continue
      kill "$pid" >/dev/null 2>&1 || true
    done < <(ps -axo pid=,command= | awk -v sim="$sim_id" 'index($0, "bmux.app/bmux") && index($0, "/Devices/" sim "/") { print $1 }')
...
app="$HOME/Library/Developer/Xcode/DerivedData/bmux-${tag}/Build/Products/Debug/bmux DEV ${tag}.app"
if [[ ! -x "$app/Contents/MacOS/bmux DEV" ]]; then
  cat >&2 <<EOF
...
for pid in $(pgrep -f "bmux-${tag}/Build/Products/Debug/bmux DEV ${tag}.app/Contents/MacOS/bmux DEV" || true); do
  kill "$pid" >/dev/null 2>&1 || true
done

rm -f "/tmp/bmux-debug-${tag}.sock" "/tmp/bmux-debug-${tag}.log"
rm -f \
...
BMUX_TAG="$tag" BMUX_REPO="$repo_root" SOAK_ROOT="$root" \
  screen -dmS "${session_prefix}-mac" "$helper_dir/launch-tagged-mac.sh"

for _ in $(seq 1 60); do
  if [[ -S "/tmp/bmux-debug-${tag}.sock" ]]; then
    break
  fi
  sleep 1
done
...
if [[ ! -S "/tmp/bmux-debug-${tag}.sock" ]]; then
  echo "tagged socket did not appear: /tmp/bmux-debug-${tag}.sock" >&2
  exit 1
fi

(
  cd "$repo_root"
  BMUX_TAG="$tag" scripts/bmux-debug-cli.sh ping >/dev/null
  if [[ -n "$dev_stack_auth_token" ]]; then
    BMUX_TAG="$tag" scripts/bmux-debug-cli.sh rpc mobile.dev_stack_auth.configure \
...
screen -dmS "${session_prefix}-iphone" bash -lc "
  export BMUX_TAG='$tag' BMUX_REPO='$repo_root' SIMULATOR_ID='$iphone_sim_id' CLIENT_ID='${profile}-iphone'
  ...
  export BMUX_MOBILE_DEV_STACK_AUTH_TOKEN='$dev_stack_auth_token'
  ...
  export MOBILE_FAILURE_LIMIT=1 SOAK_DIAGNOSTICS_DIR='$root/diagnostics' BMUX_DEBUG_LOG='/tmp/bmux-debug-${tag}.log'
...
screen -dmS "${session_prefix}-ipad" bash -lc "
  export BMUX_TAG='$tag' BMUX_REPO='$repo_root' SIMULATOR_ID='$ipad_sim_id' CLIENT_ID='${profile}-ipad'
  export SOAK_ROOT='$root' SOAK_PROFILE='$profile' SOAK_SECONDS='$seconds'
...
screen -dmS "${session_prefix}-macos" bash -lc "
  export BMUX_TAG='$tag' BMUX_REPO='$repo_root' SOAK_ROOT='$root' SOAK_SECONDS='$seconds'
  ...
  export BMUX_CLI_TIMEOUT_SECONDS='$socket_timeout'
  exec '$helper_dir/macos-soak.py' >'$root/macos.console.log' 2>&1
"

screen -dmS "${session_prefix}-resources" bash -lc "
  export BMUX_TAG='$tag' SOAK_SECONDS='$seconds' IPHONE_SIM_ID='$iphone_sim_id' IPAD_SIM_ID='$ipad_sim_id'
