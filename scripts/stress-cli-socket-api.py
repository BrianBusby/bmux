#!/usr/bin/env python3
"""Stress the bmux CLI and Unix socket API.
...
SKIPPED_SOCKET_METHODS = {
    ...
    "events.stream": "streaming protocol, covered by bmux events --limit",
...
class Diagnostics:
    ...
    def app_pids(self) -> list[int]:
        ...
        if self.tag:
            patterns.append(f"bmux DEV {self.tag}.app/Contents/MacOS/bmux DEV")
            patterns.append(f"bmux DEV {self.tag}")
        patterns.append("bmux DEV")
        seen: set[int] = set()
    ...
    def capture(self, reason: str, details: dict[str, Any] | None = None) -> pathlib.Path:
        ...
        if self.free_bytes(capture_dir) > MIN_LIGHT_DIAGNOSTIC_FREE_BYTES:
            self._capture_command(
                capture_dir,
                "log-show-bmux.txt",
                ["log", "show", "--last", "2m", "--style", "compact", "--predicate", 'process CONTAINS "bmux"'],
                timeout=20,
            )
        else:
            safe_write_text(capture_dir / "log-show-bmux.txt", "skipped: low free disk space\n")
    ...
    def _interesting_log_paths(self) -> list[pathlib.Path]:
        paths = []
        if self.tag:
            paths.append(pathlib.Path(f"/tmp/bmux-debug-{self.tag}.log"))
        paths.extend(pathlib.Path(path) for path in glob.glob("/tmp/bmux-debug*.log"))
        paths.extend(pathlib.Path(path) for path in glob.glob("/tmp/bmux-launch-*.out"))
    ...
    def _copy_recent_crash_reports(self, capture_dir: pathlib.Path) -> None:
        ...
        roots = [
            ...
            pathlib.Path.home() / ".local/state/bmux/crash",
        ]
        for root in roots:
            if not root.exists():
                continue
            for path in root.glob("*bmux*"):
                try:
...
class StressContext:
    def __init__(
        ...
        *,
        ...
        self.markdown_path.write_text("# bmux stress\n\nsocket and cli stress fixture\n", encoding="utf-8")
        self.text_path.write_text("bmux stress file\n", encoding="utf-8")
        html = "<!doctype html><title>bmux stress</title><main><input id=i value=ready><button id=b>go</button><select id=s><option>a</option><option>b</option></select><p>stress body</p></main>"
        self.browser_url = "data:text/html," + urllib.parse.quote(html, safe="")

    def base_env(self) -> dict[str, str]:
        env = dict(os.environ)
        env["BMUX_SOCKET_PATH"] = self.socket_path
        env["BMUX_CLI_SENTRY_DISABLED"] = "1"
        env["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        if self.tag:
            env["BMUX_TAG"] = self.tag
            env["BMUX_BUNDLE_ID"] = f"com.bmuxterm.app.debug.{self.tag.replace('-', '.')}"
        env.pop("BMUX_SOCKET", None)
        env.pop("BMUX_SOCKET_PASSWORD", None)
        env.pop("BMUX_WORKSPACE_ID", None)
        env.pop("BMUX_SURFACE_ID", None)
        env.pop("BMUX_TAB_ID", None)
        env.pop("BMUX_PANEL_ID", None)
        return env

    def no_socket_env(self) -> dict[str, str]:
        env = self.base_env()
        env.pop("BMUX_SOCKET_PATH", None)
        env["BMUX_BUNDLE_ID"] = f"com.bmuxterm.stress.{self.run_id.lower()}"
        return env
...
def browser_cli_cases() -> list[CliCase]:
    ...
    return [
        ...
        CliCase("browser-addscript", ctx_argv(lambda c: ["browser", "--surface", require(c.browser_surface_id, "browser surface"), "addscript", "window.__bmuxStress=1"]), expect_codes=any_code, covered_command="browser"),
...
def browser_socket_cases() -> list[SocketCase]:
    ...
    return [
        ...
        SocketCase("browser.find.title", "browser.find.title", lambda c: {**p_browser(c), "text": "bmux stress"}, expect_ok=None),
...
class StressRunner:
    ...
    def run(self) -> int:
        ...
        heartbeat = threading.Thread(target=self.heartbeat_loop, name="bmux-stress-heartbeat", daemon=True)
        heartbeat.start()
...
def resolve_cli_path(raw: str | None, tag: str | None) -> str:
    ...
    env_cli = os.environ.get("BMUXTERM_CLI") or os.environ.get("BMUX_BUNDLED_CLI_PATH")
    if env_cli:
        candidates.append(os.path.expanduser(env_cli))
    if tag:
        candidates.append(os.path.expanduser(f"~/Library/Developer/Xcode/DerivedData/bmux-{tag}/Build/Products/Debug/bmux DEV {tag}.app/Contents/Resources/bin/bmux"))
        candidates.append(os.path.expanduser(f"~/Library/Developer/Xcode/DerivedData/bmux-{tag}/Build/Products/Debug/bmux"))
    last_cli = pathlib.Path("/tmp/bmux-last-cli-path")
    if last_cli.exists():
        try:
            candidates.append(last_cli.read_text(encoding="utf-8").strip())
        except OSError:
            pass
    candidates.extend(glob.glob(os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/bmux"), recursive=True))
    for candidate in candidates:
        if candidate and os.path.isfile(candidate) and os.access(candidate, os.X_OK):
            return candidate
    raise SystemExit("error: could not find bmux CLI. Pass --cli or set BMUXTERM_CLI.")


def resolve_socket_path(raw: str | None, tag: str | None) -> str:
    if raw:
        return os.path.expanduser(raw)
    env_socket = os.environ.get("BMUX_SOCKET_PATH")
    if env_socket:
        return env_socket
    if tag:
        return f"/tmp/bmux-debug-{tag}.sock"
    last_socket = pathlib.Path("/tmp/bmux-last-socket-path")
    if last_socket.exists():
        try:
            ...
            pass
    raise SystemExit("error: socket path required. Pass --socket or --tag, or set BMUX_SOCKET_PATH.")


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("--cli", help="Path to bmux CLI binary. Defaults to BMUXTERM_CLI, tagged DerivedData, then recent DerivedData.")
    parser.add_argument("--socket", help="Unix socket path. Defaults to BMUX_SOCKET_PATH or /tmp/bmux-debug-<tag>.sock.")
    ...
    parser.add_argument("--artifacts", help="Artifact directory. Default: /tmp/bmux-cli-socket-stress-<timestamp>.")
    ...
    artifacts_dir = pathlib.Path(args.artifacts or f"/tmp/bmux-cli-socket-stress-{now_slug()}").expanduser()
