...
# ~/.secrets/bmuxterm-dev.env. Thereafter every DEBUG build they make with
...
#
# This script:
#   1. If ~/.secrets/bmuxterm-dev.env already has a complete dogfood pair, prints
...
DEV_ENV_FILE="$SECRETS_DIR/bmuxterm-dev.env"
...
# the app actually does. (DEBUG => BMUXAuthEnvironment.development.)
...
# bmux_dev_secrets_load resolves a COMPLETE pair from the same precedence chain
...
# process environment.
existing_email="$(
  bmux_dev_secrets_load >/dev/null 2>&1 && printf '%s' "${BMUX_UITEST_STACK_EMAIL:-}" || true
)"
...
echo "==> bmux team dogfood setup (one-time, per developer)"
...
umask 077
{
  echo "# bmux per-developer dogfood credentials. Written by scripts/setup-team-dev.sh."
  echo "# DEBUG-only, per-user; never commit. See scripts/bmuxterm-dev.env.example."
  printf 'BMUX_DOGFOOD_STACK_EMAIL=%s\n' "$email"
  printf 'BMUX_DOGFOOD_STACK_PASSWORD=%s\n' "$password"
} > "$DEV_ENV_FILE"
umask "$umask_old"
