...
# reload-extension.sh — build a BMUX sample sidebar extension scoped to a dev build tag.
#
# A tagged bmux dev app (built by reload.sh --tag <t>) declares a host-scoped
# extension point <host-bundle-id>.bmux.sidebar (baked at build time via the
# BMUX_SIDEBAR_EXTENSION_POINT_ID build setting; see BMUXSidebarExtensionPoint).
...
#
# This script passes the tagged point id (BMUX_SIDEBAR_EXTENSION_POINT_ID) and a per-tag
# bundle-id suffix (BMUX_BUNDLE_ID_SUFFIX=.<TAG_ID>) to xcodebuild as build settings, so
...
SIDEBAR_POINT_NAME="bmux.sidebar"
...
Usage: scripts/reload-extension.sh --tag <tag> [--host-bundle-id <id>] [--example sample|tabs|both] [--no-launch]
  ...
  --host-bundle-id   Host app bundle id when reload.sh used --bundle-id.
                     Defaults to com.bmuxterm.app.debug.<sanitized-tag>.
  --bundle-id        Alias for --host-bundle-id.
  --example <which>  sample (BMUX ExtKit Sample Sidebar), tabs (TabsVisibleSidebar),
                     or both (default).
...
HOST_BUNDLE_ID="${HOST_BUNDLE_ID_OVERRIDE:-com.bmuxterm.app.debug.${TAG_ID}}"
...
example_specs() {
  case "$1" in
    sample)
      echo "Examples/SampleSidebarExtensionApp/SampleSidebarExtensionApp.xcodeproj|BMUXExtKitSampleSidebarApp|BMUX ExtKit Sample Sidebar|co.manaflow.BMUXExtKitSampleSidebarApp|Contents/Extensions/BMUX ExtKit Sample Sidebar Extension.appex|co.manaflow.BMUXExtKitSampleSidebarApp.Extension" ;;
    tabs)
...
build_install_example() {
  ...
  local derived="/tmp/bmux-ext-${which}-${TAG_ID}"
  echo "==> building ${app_name} for tag ${TAG} (point ${TAGGED_POINT_ID})"
  rm -rf "$derived"

  # Bake the tagged point id at build time: BMUX_SIDEBAR_EXTENSION_POINT_ID feeds the
  ...
  #
  # Scope the bundle ids per tag via BMUX_BUNDLE_ID_SUFFIX (a default-empty build
  # setting the example projects insert into PRODUCT_BUNDLE_IDENTIFIER: app
  # <appBase>$(BMUX_BUNDLE_ID_SUFFIX), appex <appBase>$(BMUX_BUNDLE_ID_SUFFIX).Extension).
  ...
  #
  # Scope the visible name per tag via BMUX_DISPLAY_NAME_SUFFIX (default empty), appended
  ...
  # space so it reads "BMUX ExtKit Sample Sidebar <tag>".
  #
  ...
  xcodebuild -project "$REPO_ROOT/$project" -scheme "$scheme" -configuration Debug \
    ...
    BMUX_SIDEBAR_EXTENSION_POINT_ID="$TAGGED_POINT_ID" \
    BMUX_BUNDLE_ID_SUFFIX=".${TAG_ID}" \
    BMUX_DISPLAY_NAME_SUFFIX=" ${TAG}" \
  ...
  # (App Sandbox + the co.manaflow.bmux.sidebar app group) bound in. Those entitlements
  ...
  # (BMUX_BUNDLE_ID_SUFFIX); resealing the Info.plist is unnecessary.
  local appex
