#!/usr/bin/env python3
"""Require standalone bmux windows to own the standard close shortcut."""
...
OWNER_LIST_PATH = pathlib.Path("Sources/bmuxApp.swift")
OWNER_LIST_NAME = "bmuxAuxiliaryWindowIdentifiers"
...
IDENTIFIER_ASSIGNMENT_RE = re.compile(
    r"""\b[A-Za-z_][A-Za-z0-9_]*\.identifier\s*=\s*NSUserInterfaceItemIdentifier\("(?P<identifier>bmux\.[^"]+)"\)"""
)
# `window.identifier = NSUserInterfaceItemIdentifier(Self.windowIdentifier)` —
# the identifier comes from a named constant instead of an inline literal. The
# constant is resolved against `let <name> = "bmux...."` declarations in the
...
# the literal-only regex never saw `bmux.mobilePairingWindow`, so the lint
...
STRING_CONSTANT_DECL_RE = re.compile(
    r"""\blet\s+(?P<name>[A-Za-z_][A-Za-z0-9_]*)\s*(?::\s*String\s*)?=\s*"(?P<identifier>bmux\.[^"]+)\""""
)
STRING_LITERAL_RE = re.compile(r'"(?P<identifier>bmux\.[^"]+)"')
...
def main(argv: list[str]) -> int:
    ...
    if missing:
        ...
        print("")
        print(
            "These bmux window identifiers are assigned to NSWindow/NSPanel "
    ...
    print(f"Checked {len(assignments)} bmux window identifier(s).")
    return 0
