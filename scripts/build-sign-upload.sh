...
# Requires: source ~/.secrets/bmuxterm.env && export SPARKLE_PRIVATE_KEY
...
GHOSTTYKIT_CRASH_REPORT_SUBDIR="bmux/crash"
...
/usr/libexec/PlistBuddy -c "Add :SUFeedURL string https://github.com/manaflow-ai/bmux/releases/latest/download/appcast.xml" "$APP_PLIST"
echo "Sparkle keys injected"

# bmux is a non-sandboxed app. Sparkle's sandbox-only XPC services make the
# installer handoff wait for an agent connection that never arrives.
./scripts/remove-sparkle-sandbox-xpc-services.sh "$APP_PATH"

# --- Codesign ---
echo "Codesigning..."
./scripts/sign-bmux-bundle.sh "$APP_PATH" "$ENTITLEMENTS" "$SIGN_HASH"
echo "Codesign verified"

# --- Notarize app ---
echo "Notarizing app..."
ditto -c -k --sequesterRsrc --keepParent "$APP_PATH" bmux-notary.zip
xcrun notarytool submit bmux-notary.zip \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple "$APP_PATH"
xcrun stapler validate "$APP_PATH"
rm -f bmux-notary.zip
echo "App notarized"

# --- Create and notarize DMG ---
echo "Creating DMG..."
rm -f bmux-macos.dmg
create-dmg --codesign "$SIGN_HASH" bmux-macos.dmg "$APP_PATH"
echo "Notarizing DMG..."
xcrun notarytool submit bmux-macos.dmg \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple bmux-macos.dmg
xcrun stapler validate bmux-macos.dmg
echo "DMG notarized"

# --- Generate Sparkle appcast ---
echo "Generating appcast..."
./scripts/sparkle_generate_appcast.sh bmux-macos.dmg "$TAG" appcast.xml
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  for asset in bmux-macos.dmg appcast.xml; do
  ...
  if [[ "$ALLOW_OVERWRITE" == "true" ]]; then
    ...
    gh release upload "$TAG" bmux-macos.dmg appcast.xml --clobber
  else
    echo "Uploading to existing release $TAG..."
    gh release upload "$TAG" bmux-macos.dmg appcast.xml
  fi
else
  echo "Creating release $TAG and uploading..."
  gh release create "$TAG" bmux-macos.dmg appcast.xml --title "$TAG" --notes "See CHANGELOG.md for details"
fi
...
if [[ "$TAG" != *"-nightly"* ]]; then
  VERSION="${TAG#v}"
  DMG_SHA256=$(shasum -a 256 bmux-macos.dmg | cut -d' ' -f1)
  echo "Updating homebrew cask to $VERSION (SHA: $DMG_SHA256)..."
  CASK_FILE="homebrew-bmux/Casks/bmux.rb"
  if [ -f "$CASK_FILE" ]; then
...
cask "bmux" do
  ...
  url "https://github.com/manaflow-ai/bmux/releases/download/v#{version}/bmux-macos.dmg"
  name "bmux"
  desc "Lightweight native macOS terminal with vertical tabs for AI coding agents"
  homepage "https://github.com/manaflow-ai/bmux"
  ...
  app "bmux.app"
  binary "#{appdir}/bmux.app/Contents/Resources/bin/bmux"

  zap trash: [
    "~/Library/Application Support/bmux",
    "~/Library/Caches/bmux",
    "~/Library/Preferences/ai.manaflow.bmuxterm.plist",
  ]
end
CASKEOF
    cd homebrew-bmux
    git add Casks/bmux.rb
    if git diff --staged --quiet; then
      echo "Homebrew cask already up to date"
    else
      git commit -m "Update bmux to ${VERSION}"
      git push
      echo "Homebrew cask updated"
    fi
    cd ..
  else
    echo "WARNING: homebrew-bmux submodule not found, skipping cask update"
  fi
fi

# --- Cleanup ---
rm -rf build/ bmux-macos.dmg appcast.xml
echo ""
echo "=== Release $TAG complete ==="
say "bmux release complete"
