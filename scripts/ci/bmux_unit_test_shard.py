#!/usr/bin/env python3
"""Generate deterministic -only-testing arguments for bmuxTests shards."""
...
FOCUSED_GATE_SELECTORS = {
    "bmuxTests/BrowserSystemProxyMirrorTests",
    "bmuxTests/GhosttyOptionAsAltModsTests",
}
...
def discover_selectors(root: Path) -> list[TestSelector]:
    test_root = root / "bmuxTests"
    if not test_root.is_dir():
        raise SystemExit(f"bmuxTests directory not found under {root}")
    ...
    for path in sorted(test_root.glob("**/*.swift")):
        ...
        for position, (line_number, kind, name) in enumerate(top_level_declarations):
            ...
            suite_identifier = f"bmuxTests/{name}"
    ...
    for extension_name in sorted(set(extension_methods) - declared_suite_names):
        ...
        print(
            f"Extension declares tests for unknown suite bmuxTests/{extension_name}: {locations}",
            file=sys.stderr,
        )
        raise SystemExit(1)

    for declaration in declarations:
        suite_identifier = f"bmuxTests/{declaration.name}"
    ...
    if not selectors:
        raise SystemExit("No bmuxTests suites found")
    ...
    if duplicates:
        print("Duplicate bmuxTests selector identifiers:", file=sys.stderr)
...
def main() -> int:
    ...
    if args.validate:
        ...
        print(
            f"Discovered {len(selectors)} bmuxTests selectors "
