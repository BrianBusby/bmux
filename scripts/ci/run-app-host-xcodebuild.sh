...
log_stem="${log_dir%/}/bmux-app-host-xcodebuild-${BMUX_TAG:-untagged}"
max_attempts="${BMUX_APP_HOST_XCODEBUILD_ATTEMPTS:-3}"
export BMUX_XCODEBUILD_NONINTERACTIVE_IDLE_TIMEOUT_SECONDS="${BMUX_XCODEBUILD_NONINTERACTIVE_IDLE_TIMEOUT_SECONDS:-${BMUX_XCODEBUILD_NONINTERACTIVE_TIMEOUT_SECONDS:-300}}"
echo "App-host xcodebuild idle timeout: ${BMUX_XCODEBUILD_NONINTERACTIVE_IDLE_TIMEOUT_SECONDS}s, attempts: ${max_attempts}"
...
if [ -z "${BMUX_APP_HOST_TEST_LOCK_ACTIVE:-}" ]; then
  lock_file="${BMUX_APP_HOST_TEST_LOCK_FILE:-${TMPDIR:-/tmp}/bmux-app-host-test.lock}"
  lock_wait_seconds="${BMUX_APP_HOST_TEST_LOCK_WAIT_SECONDS:-3600}"
  export BMUX_APP_HOST_TEST_LOCK_ACTIVE=1
...
kill_stale_app_host() {
  # Kill app-host executables (matched by their .../Build/Products/.../bmux DEV
  ...
  [ -n "$ci_app_host_root" ] && \
    pkill -f "${ci_app_host_root%/}/.*Build/Products/.*bmux DEV" 2>/dev/null || true
}
...
while [ "$attempt" -le "$max_attempts" ]; do
  ...
  : >"$log_path"
  # Self-hosted macOS runners reuse the GUI session. A stale "bmux DEV" app-host
  ...
  kill_stale_app_host
  set +e
  BMUX_XCODEBUILD_NONINTERACTIVE_LOG_PATH="$log_path" \
    scripts/ci/xcodebuild_noninteractive.py xcodebuild "$@"
  status=$?
  set -e

  if grep -Fq 'path = "/tmp/bmux-debug.sock"' "$log_path"; then
    echo "FAIL: app-host used default debug socket instead of an XCTest-scoped socket" >&2
    exit 1
  fi

  if grep -Fq 'SocketControlServer: Listening on /tmp/bmux-debug.sock' "$log_path"; then
  ...
  if [ "$status" -ne 0 ]; then
    ...
    if [ "$status" -eq 124 ]; then
      retry_reason="${BMUX_XCODEBUILD_NONINTERACTIVE_IDLE_TIMEOUT_SECONDS}s idle timeout"
