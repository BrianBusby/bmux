...
LOCK_DIR="${BMUX_VDISPLAY_LOCK_DIR:-/tmp/bmux-ci-virtual-display.lock}"
LOCK_TIMEOUT_SECONDS="${BMUX_VDISPLAY_LOCK_TIMEOUT_SECONDS:-600}"
LOCK_STALE_SECONDS="${BMUX_VDISPLAY_LOCK_STALE_SECONDS:-1800}"
LOCK_OWNERLESS_STALE_SECONDS="${BMUX_VDISPLAY_LOCK_OWNERLESS_STALE_SECONDS:-$LOCK_TIMEOUT_SECONDS}"
LOCK_POLL_SECONDS="${BMUX_VDISPLAY_LOCK_POLL_SECONDS:-2}"
...
macOS jobs. acquire prints BMUX_VDISPLAY_LOCK_DIR and
BMUX_VDISPLAY_LOCK_TOKEN shell assignments on stdout.
EOF
}
...
acquire() {
  ...
  while true; do
    if mkdir "$LOCK_DIR" 2>/dev/null; then
      ...
      printf 'BMUX_VDISPLAY_LOCK_DIR=%q\n' "$LOCK_DIR"
      printf 'BMUX_VDISPLAY_LOCK_TOKEN=%q\n' "$token"
      exit 0
    fi
...
require_token_match() {
  validate_lock_dir
  local token="${BMUX_VDISPLAY_LOCK_TOKEN:-}"
  if [ -z "$token" ]; then
    echo "BMUX_VDISPLAY_LOCK_TOKEN is required for $COMMAND" >&2
    exit 1
  fi
  if [ ! -d "$LOCK_DIR" ]; then
