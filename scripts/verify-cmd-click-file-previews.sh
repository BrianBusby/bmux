...
fixture_dir="${1:-${TMPDIR:-/tmp}/bmux-file-preview-fixtures}"
...
def write_pdf(path: Path) -> None:
    ...
    stream = (
        ...
        b"/F1 24 Tf\n"
        b"72 720 Td\n"
        b"(bmux file preview PDF) Tj\n"
        b"/F1 12 Tf\n"
        b"0 -36 Td\n"
        b"(Cmd-click this path in bmux. It should open in a split.) Tj\n"
        b"ET\n"
    )
    objects.append(
...
write_text(
    ...
    "# bmux file preview fixture\n\nCmd-click this Markdown path. With Markdown routing on, it opens in the rendered viewer.\n",
)
write_text(
    "Plain Text Fixture.txt",
    "This is a plain text fixture for bmux cmd-click file preview routing.\n",
)
write_text(
    "Code Fixture.swift",
    'import Foundation\n\nprint("bmux file preview")\n',
)
write_text(
    "Data Fixture.json",
...
copy_fixture "$repo_root/web/public/blog/cmd-shift-u.mp4" "BMUX Sample Video.mp4"
...
printf 'bmux cmd-click file preview fixtures\n'
printf 'Directory: %s\n\n' "$fixture_dir"
printf 'Run this inside the tagged bmux build, then Cmd-click each path below.\n'
printf 'Expected: supported files open in a bmux split. After Cmd Shift P -> Disable Cmd-click File Previews, the same paths should fall back to the external opener.\n\n'
...
for file in "Generated PNG Fixture.png" "Generated PDF Fixture.pdf" "BMUX Sample Video.mp4"; do
  path="$fixture_dir/$file"
  if [[ -f "$path" ]]; then
    python3 - "$path" <<'PY'
from pathlib import Path
