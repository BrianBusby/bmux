...
#   ~/Library/Developer/Xcode/DerivedData/bmux-<tag>/      (multi-GB)
#   /tmp/bmux-<tag>/                                       (build scratch)
#   /tmp/bmux-debug-<tag>.sock                             (control socket)
#   /tmp/bmux-debug-<tag>.log                              (debug log)
#   /tmp/bmux-reload-<tag>.log                             (build log)
#   ~/Library/Application Support/bmux/bmuxd-dev-<tag>.sock (bmuxd socket)
#
# This script removes those artifacts for tags that are safe to clean.
# Safety rules (always on):
#   - Skip any tag whose `bmux DEV <tag>` app is currently running.
#   - Skip the tag pointed at by /tmp/bmux-last-cli-path (most recent reload).
...
LAST_CLI_PATH_FILE="/tmp/bmux-last-cli-path"
...
# Tags come from DerivedData dirs named bmux-<tag>. Authoritative because
# reload.sh always creates one there.
discover_tags() {
    [[ -d "$DERIVED_DATA_ROOT" ]] || return 0
    local d name
    for d in "$DERIVED_DATA_ROOT"/bmux-*/; do
...
artifact_paths_for_tag() {
    local tag="$1"
    printf '%s\n' \
        "$DERIVED_DATA_ROOT/bmux-${tag}" \
        "/tmp/bmux-${tag}" \
        "/tmp/bmux-${tag}.tar" \
        "/tmp/bmux-debug-${tag}.sock" \
        "/tmp/bmux-debug-${tag}.log" \
        "/tmp/bmux-reload-${tag}.log" \
        "$APP_SUPPORT_DIR/bmuxd-dev-${tag}.sock"
}
...
# `/bmux-<tag>/` anywhere in the path so we cover paths under DerivedData,
# /tmp, or other locations reload.sh may emit.
active_tag=""
if [[ -r "$LAST_CLI_PATH_FILE" ]]; then
    last_path="$(cat "$LAST_CLI_PATH_FILE" 2>/dev/null || true)"
    if [[ "$last_path" =~ /bmux-([A-Za-z0-9._-]+)/ ]]; then
        active_tag="${BASH_REMATCH[1]}"
    fi
fi

# Running bmux DEV processes by tag (the app name embeds the tag).
running_tags=()
while IFS= read -r line; do
    # Match "bmux DEV <tag>" (with or without .app suffix).
    if [[ "$line" =~ bmux\ DEV\ ([A-Za-z0-9._-]+) ]]; then
        running_tags+=("${BASH_REMATCH[1]}")
    fi
done < <(pgrep -fl "bmux DEV " 2>/dev/null || true)
...
while IFS= read -r tag; do
    ...
    if (( older_than_days > 0 )); then
        age="$(derived_data_mtime_days "$DERIVED_DATA_ROOT/bmux-${tag}")"
