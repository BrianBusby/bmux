...
TMP_ROOT="${TMPDIR:-/tmp}"
OUT_DIR="${OUT_DIR:-${TMP_ROOT%/}/bmux-mobile-attach-qr-$TAG}"
umask 077
mkdir -p "$OUT_DIR"
chmod 700 "$OUT_DIR"
...
BMUX_TAG="$TAG" "$REPO_ROOT/scripts/bmux-debug-cli.sh" rpc mobile.attach_ticket.create "$PARAMS" > "$RAW_JSON_TMP"
...
<title>bmux mobile pairing</title>
<style>
...
<main>
  ...
  <section>
    <h1>Scan to pair bmux</h1>
    <p>Open bmux on iPhone, tap <strong>Scan QR Code</strong>, and scan this code. This ticket is route-filtered for the address below.</p>
    <table>
