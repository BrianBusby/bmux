...
Usage:
  BMUX_TAG=swmob ./scripts/mobile-stability-soak/latency-probe.py \
...
def bmux_debug_rpc(tag: str, method: str, params: dict) -> dict:
    env = {**os.environ, "BMUX_TAG": tag}
    out = subprocess.check_output(
        [str(REPO / "scripts/bmux-debug-cli.sh"), "rpc", method, json.dumps(params)],
        env=env,
        text=True,
    )
...
def create_attach_ticket(tag: str, prefer_route: str, workspace_id: str | None) -> dict:
    ...
    payload = bmux_debug_rpc(tag, "mobile.attach_ticket.create", params)
...
def pick_terminal(ticket: dict, *, create_if_missing: bool) -> str:
    ...
    if not ws_list:
        raise RuntimeError("tagged host has no workspaces; create one in bmux first")
    for ws in ws_list:
...
def main() -> int:
    ...
    ap.add_argument("--tag", default=os.environ.get("BMUX_TAG", "swmob"))
    args = ap.parse_args()

    if not args.tag:
        print("error: pass --tag or set BMUX_TAG", file=sys.stderr)
        return 2
