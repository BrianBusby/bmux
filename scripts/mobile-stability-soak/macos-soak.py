...
CLI_TIMEOUT_SECONDS = float(os.environ.get("BMUX_CLI_TIMEOUT_SECONDS", "20"))
...
def cli(*args: str, timeout: float = CLI_TIMEOUT_SECONDS) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["BMUX_TAG"] = TAG
    cmd = [str(REPO / "scripts/bmux-debug-cli.sh"), *args]
    try:
