...
tag = os.environ.get("BMUX_TAG", "swmob")
repo = Path(os.environ.get("BMUX_REPO", Path(__file__).resolve().parents[2]))
...
soak_root = Path(os.environ.get("SOAK_ROOT", f"/tmp/bmux-mobile-soak-{tag}"))
...
dev_origin = os.environ.get("BMUX_DEV_ORIGIN", "").strip().rstrip("/")
dev_stack_auth_token = os.environ.get("BMUX_MOBILE_DEV_STACK_AUTH_TOKEN", "").strip()
...
bmux_log_path = Path(os.environ.get("BMUX_DEBUG_LOG", f"/tmp/bmux-debug-{tag}.log"))
bmux_log_tail_lines = int(os.environ.get("BMUX_DEBUG_LOG_TAIL_LINES", "500"))
...
def bmux_rpc(method, params):
    return json.loads(run(["scripts/bmux-debug-cli.sh", "rpc", method, json.dumps(params)]))
...
# channel-specific in CmxPairingURLScheme) registers `bmux-ios-dev` in its
# CFBundleURLSchemes rather than the release `bmux-ios`. `simctl openurl` routes
...
ATTACH_URL_SCHEME = os.environ.get("MOBILE_ATTACH_URL_SCHEME", "bmux-ios-dev").strip()
...
def launch_app_with_attach_ticket(ticket):
    ...
    launch_output = run(
        ...
        cwd=Path("/"),
        extra_env={
            "SIMCTL_CHILD_BMUX_UITEST_MOCK_DATA": "1",
            "SIMCTL_CHILD_BMUX_UITEST_ATTACH_URL": ticket["attach_url"],
            "SIMCTL_CHILD_BMUX_MOBILE_DEV_STACK_AUTH_TOKEN": dev_stack_auth_token,
            "SIMCTL_CHILD_BMUX_MOBILE_SOAK_OPEN_SELECTED_WORKSPACE": "1",
...
def write_failure_diagnostics(ticket, terminal_id, iteration, error):
    ...
    try:
        host_status = framed_rpc(ticket, "mobile.host.status", {}) if external_ticket_path else bmux_rpc("mobile.host.status", {})
    ...
    if bmux_log_path.exists():
        try:
            lines = bmux_log_path.read_text(errors="replace").splitlines()
            (diagnostic_root / "bmux-debug-tail.log").write_text("\n".join(lines[-bmux_log_tail_lines:]) + "\n")
        except Exception as log_error:
            manifest["bmux_log_error"] = str(log_error)
