...
def find_pid(label: str, processes: list[tuple[int, str]]) -> int | None:
    if label == "mac":
        needle = f"bmux-{TAG}/Build/Products/Debug/bmux DEV {TAG}.app/Contents/MacOS/bmux DEV"
        ...
        candidates: list[int] = []
        for pid, command in processes:
            if f"/Devices/{IPHONE_SIM_ID}/" in command and "bmux.app/bmux" in command:
                candidates.append(pid)
        return max(candidates) if candidates else None
    elif label == "ipad":
        candidates: list[int] = []
        for pid, command in processes:
            if f"/Devices/{IPAD_SIM_ID}/" in command and "bmux.app/bmux" in command:
                candidates.append(pid)
        return max(candidates) if candidates else None
    return None
