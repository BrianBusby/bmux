...
function samplePayloadWithCanonicalURL() {
  return {
    ...samplePayload(),
    attach_url: "bmux-ios://attach?v=2&r=100.1.2.3:8080",
  };
}
...
test("builds a dev-scheme attach URL with the version and base64url ticket", () => {
  ...
  assert.match(attachURL, /^bmux-ios-dev:\/\/attach\?v=1&payload=/);
...
test("emits the release scheme when explicitly requested", () => {
  ...
  assert.match(attachURL, /^bmux-ios:\/\/attach\?v=1&payload=/);
  // The dev default and the release override are the two channel schemes.
  assert.equal(DEV_URL_SCHEME, "bmux-ios-dev");
  assert.equal(RELEASE_URL_SCHEME, "bmux-ios");
});
...
test("prefers the canonical attach_url returned by the Mac RPC", () => {
  ...
  assert.equal(attachURL, "bmux-ios://attach?v=2&r=100.1.2.3:8080");
