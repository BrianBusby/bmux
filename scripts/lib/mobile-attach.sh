...
# reload.sh / bmux-debug-cli.sh exactly).
#
...
# class is deliberate (matches reload.sh + bmux-debug-cli.sh socket/DerivedData
# naming); a locale-sensitive class would keep non-ASCII letters the slug drops.
bmux_attach__slug_raw() {
...
bmux_attach__slug() {
  local cleaned
  cleaned="$(bmux_attach__slug_raw "$1")"
...
bmux_attach_tag_has_alnum() {
  [[ -n "$(bmux_attach__slug_raw "$1")" ]]
}

# bundle id segment: lowercase, non-alnum -> '.', trimmed/collapsed.
bmux_attach__bundle_seg() {
  local cleaned
...
bmux_attach_mac_bundle_id() {
  printf 'com.bmuxterm.app.debug.%s' "$(bmux_attach__bundle_seg "$1")"
}

# The tagged Mac app's debug socket path.
bmux_attach_socket_path() {
  printf '/tmp/bmux-debug-%s.sock' "$(bmux_attach__slug "$1")"
}
...
# sanitized slug, matching reload.sh (`APP_NAME="bmux DEV ${TAG_SLUG}"`); the raw
# tag would miss for any tag whose slug differs (e.g. "Fix Foo" -> "fix-foo").
bmux_attach_mac_app_path() {
  local slug
  slug="$(bmux_attach__slug "$1")"
  printf '%s/Library/Developer/Xcode/DerivedData/bmux-%s/Build/Products/Debug/bmux DEV %s.app' \
    "$HOME" "$slug" "$slug"
}
...
bmux_attach_enable_pairing_host() {
  local tag="$1" bundle_id
  bundle_id="$(bmux_attach_mac_bundle_id "$tag")"
...
bmux_attach_mac_socket_ready() {
  local sock
  sock="$(bmux_attach_socket_path "$1")"
  [[ -S "$sock" ]]
}
...
#                     BMUX_ATTACH_ALLOW_RELAUNCH=1 to opt into auto-relaunching
...
bmux_attach_ensure_mac() {
  local tag="$1" repo_root="${2:-}" sock app slug _i
  sock="$(bmux_attach_socket_path "$tag")"
  app="$(bmux_attach_mac_app_path "$tag")"
  slug="$(bmux_attach__slug "$tag")"
  bmux_attach_enable_pairing_host "$tag" || true

  if [[ -S "$sock" ]]; then
    # Quick probe (2 attempts ~1s): if pairing already mints, done.
    if [[ -n "$repo_root" ]] && [[ -n "$(bmux_attach_mint_url "$tag" 60 "$repo_root" 2)" ]]; then
      return 0
    fi
    ...
    if [[ "${BMUX_ATTACH_ALLOW_RELAUNCH:-0}" != "1" ]]; then
      echo "warning: tagged Mac app for '$tag' is running but its iOS pairing listener is not bound (it was likely launched before pairing was enabled, or the macOS Local Network prompt is pending). Relaunch it to enable auto-pair, or re-run with BMUX_ATTACH_ALLOW_RELAUNCH=1; signing in only for now." >&2
      return 1
    fi
    if [[ ! -d "$app" ]]; then
    ...
    echo "==> relaunching tagged Mac app to bind the pairing listener ($tag) [BMUX_ATTACH_ALLOW_RELAUNCH=1]" >&2
    # Scoped to this tag's executable only (never the stable app or other tags).
    pkill -f "bmux DEV ${slug}.app/Contents/MacOS/bmux DEV" 2>/dev/null || true
  ...
  # The tagged app derives its socket from its baked BMUXDevTag, so a plain launch
  # binds /tmp/bmux-debug-<slug>.sock without extra env.
...
bmux_attach_mint_url() {
  local tag="$1" ttl="$2" repo_root="$3" max="${4:-20}" sock slug payload url _i
  sock="$(bmux_attach_socket_path "$tag")"
  # bmux-debug-cli.sh rejects BMUX_TAG outside [A-Za-z0-9._-] and re-sanitizes it
  ...
  for _i in $(seq 1 "$max"); do
    ...
    payload="$(BMUX_TAG="$slug" "$repo_root/scripts/bmux-debug-cli.sh" rpc mobile.attach_ticket.create \
