// Pure encoder for the bmux iOS attach deep link.
//
...
// `Packages/Shared/BMUXMobileCore`: development builds register and emit
// `bmux-ios-dev`, Release (TestFlight beta + App Store) emit `bmux-ios`. Both
...
export const DEV_URL_SCHEME = "bmux-ios-dev";

/** The pairing/attach URL scheme Release (beta + prod) builds emit. */
export const RELEASE_URL_SCHEME = "bmux-ios";
...
export function buildAttachURL(payload, filter = {}) {
  ...
  if (
    ...
    payload.attach_url.startsWith("bmux-ios://attach?") &&
