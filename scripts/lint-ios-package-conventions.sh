...
# line: the mobile packages, the bmuxFeature package, and the iOS app shell.
#
...
SCOPES=()
for d in Packages/Shared/BMUXMobileCore Packages/iOS/BmuxMobile* Packages/Shared/BmuxAgentChat Packages/iOS/BmuxAgentChatUI Packages/Shared/BmuxSyncStore ios/bmuxPackage/Sources ios/bmux; do
  [ -d "$d" ] && SCOPES+=("$d")
done
...
scan global WARN '\b(UserDefaults\.standard|FileManager\.default|Bundle\.main)\b' 1 Packages/Shared/BMUXMobileCore Packages/iOS/BmuxMobile* 2>/dev/null || true
...
# only shrink.
NS_TYPE_ROOTS=()
for d in Packages/*/*/Sources ios/bmuxPackage/Sources ios/bmux; do
  [ -d "$d" ] && NS_TYPE_ROOTS+=("$d")
done
