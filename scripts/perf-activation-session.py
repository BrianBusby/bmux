...
class BmuxPerfRunner:
    def __init__(self, args: argparse.Namespace):
        self.args = args
        self.tag = args.tag
        self.tag_slug = sanitize_path(args.tag)
        self.tag_id = sanitize_bundle(args.tag)
        self.socket_path = pathlib.Path(f"/tmp/bmux-debug-{self.tag_slug}.sock")
        self.bmuxd_socket_path = pathlib.Path(
            os.path.expanduser(f"~/Library/Application Support/bmux/bmuxd-dev-{self.tag_slug}.sock")
        )
        self.debug_log_path = pathlib.Path(f"/tmp/bmux-debug-{self.tag_slug}.log")
        self.stdout_path = pathlib.Path(f"/tmp/bmux-perf-{self.tag_slug}-stdout.log")
        self.app_path = pathlib.Path(args.app_path).expanduser() if args.app_path else self.default_app_path()
        self.binary_path = self.app_path / "Contents/MacOS/bmux DEV"
        self.cli_path = self.app_path / "Contents/Resources/bin/bmux"
    ...
    def make_fixture_root(self, fixture_root_arg: str) -> pathlib.Path:
        if fixture_root_arg:
            ...
            return pathlib.Path(tempfile.mkdtemp(prefix=f"bmux-perf-{self.tag_slug}-", dir=str(fixture_parent)))
        return pathlib.Path(tempfile.mkdtemp(prefix=f"bmux-perf-{self.tag_slug}-"))

    def default_app_path(self) -> pathlib.Path:
        return pathlib.Path.home() / (
            f"Library/Developer/Xcode/DerivedData/bmux-{self.tag_slug}/"
            f"Build/Products/Debug/bmux DEV {self.tag_slug}.app"
        )
    ...
    def check_paths(self) -> None:
        ...
        if not self.cli_path.exists():
            raise PerfFailure(f"bmux CLI not found: {self.cli_path}")
    ...
    def clean_persisted_state(self) -> None:
        app_support = pathlib.Path.home() / "Library/Application Support/bmux"
        bundle_id = f"com.bmuxterm.app.debug.{self.tag_id}"
        for suffix in ("", "-previous"):
            (app_support / f"session-{bundle_id}{suffix}.json").unlink(missing_ok=True)
        self.socket_path.unlink(missing_ok=True)
        self.bmuxd_socket_path.unlink(missing_ok=True)
    ...
    def app_env(self) -> dict[str, str]:
        ...
        env.update(
            {
                "BMUX_SOCKET": str(self.socket_path),
                "BMUX_SOCKET_MODE": "automation",
                "BMUX_SOCKET_PATH": str(self.socket_path),
                "BMUXD_UNIX_PATH": str(self.bmuxd_socket_path),
                "BMUX_DEBUG_LOG": str(self.debug_log_path),
                "BMUX_TAG": self.tag,
                "BMUX_BUNDLE_ID": f"com.bmuxterm.app.debug.{self.tag_id}",
            }
        )
        return env
    ...
    def cli_env(self) -> dict[str, str]:
        env = os.environ.copy()
        env["BMUX_SOCKET"] = str(self.socket_path)
        env["BMUX_SOCKET_PATH"] = str(self.socket_path)
        env["BMUX_TAG"] = self.tag
        env["BMUX_BUNDLE_ID"] = f"com.bmuxterm.app.debug.{self.tag_id}"
        env["BMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = str(max(15, int(self.args.snapshot_timeout)))
        return env
        ...
        if proc and proc.poll() is None:
            ...
            try:
                ...
                try:
                    ...
                    pass
        subprocess.run(
            ["pkill", "-f", re.escape(f"bmux DEV {self.tag_slug}.app/Contents/MacOS/bmux DEV")],
    ...
    def make_repo(self, index: int) -> pathlib.Path:
        ...
        subprocess.run(
            ["git", "-c", "user.name=bmux", "-c", "user.email=bmux@example.invalid", "commit", "-m", "seed"],
            cwd=repo,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            check=True,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run bmux activation/session snapshot performance benchmark.")
