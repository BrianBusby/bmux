...
extension AppDelegate {
    ...
    @discardableResult
    func handleBmuxNavigationURLRequest(_ request: BmuxNavigationURLRequest) -> Bool {
        let lookup = bmuxNavigationWorkspaceLookup()
        let resolver = BmuxNavigationTargetResolver(workspaces: lookup.descriptors)
        guard let resolution = resolver.resolve(request.target) else {
            ...
            case .workspace(let workspaceId):
                bmuxDebugLog("navigationURL.notFound workspace=\(workspaceId.uuidString.prefix(8))")
    ...
    private func bmuxNavigationWorkspaceLookup() -> (
        descriptors: [BmuxNavigationTargetResolver.WorkspaceDescriptor],
        contextByWorkspaceId: [UUID: MainWindowContext]
    ) {
        var descriptors: [BmuxNavigationTargetResolver.WorkspaceDescriptor] = []
        ...
        for context in mainWindowContexts.values.sorted(by: { $0.windowId.uuidString < $1.windowId.uuidString }) {
            for workspace in context.tabManager.tabs {
                descriptors.append(workspace.bmuxNavigationDescriptor)
    ...
    func bmuxNavigationWorkspaceDescriptors() -> [BmuxNavigationTargetResolver.WorkspaceDescriptor] {
        bmuxNavigationWorkspaceLookup().descriptors
    }
