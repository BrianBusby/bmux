...
nonisolated struct BmuxTopProcessScopeCacheKey: Hashable {
    let pid: Int
    let startSeconds: Int
    let startMicroseconds: Int
}

private nonisolated struct BmuxTopProcessScopeCacheValue {
    // nil means "this process was probed and has no bmux scope". A negative entry
    // is honored as a hit only until `negativeExpiresAtNanos`, so a non-bmux
    ...
    // ignored when `scope != nil`): a bmux scope comes from inherited environment
    // or a stable argv and does not disappear for the process lifetime.
    let scope: BmuxTopProcessScope?
    let negativeExpiresAtNanos: UInt64
}

// How long a "no bmux scope" result stays cached before the process is probed
...
// `bmux hooks … monitor` later, must be re-probed eventually or it would never
...
// collapsing the steady-state re-probe storm for non-bmux processes.
// Internal (not private) so tests can read the TTL via @testable import.
nonisolated let bmuxTopNegativeScopeTTLNanoseconds: UInt64 = 60 * 1_000_000_000

// Result of probing a single process for its bmux scope. `resolved` means the
...
nonisolated enum BmuxTopProcessScopeProbeResult: Equatable {
    case resolved(BmuxTopProcessScope?)
    case unavailable
}

// BmuxTopProcessSnapshot.capture is intentionally synchronous because it backs
...
private nonisolated let bmuxTopScopeCache = OSAllocatedUnfairLock(
    initialState: [BmuxTopProcessScopeCacheKey: BmuxTopProcessScopeCacheValue]()
)

nonisolated extension BmuxTopProcessSnapshot {
    static func scopeCacheKey(from kinfo: kinfo_proc) -> BmuxTopProcessScopeCacheKey {
        let startTime = kinfo.kp_proc.p_un.__p_starttime
        return BmuxTopProcessScopeCacheKey(
    ...
    static func scopeCacheKey(from bsdInfo: proc_bsdinfo) -> BmuxTopProcessScopeCacheKey {
        BmuxTopProcessScopeCacheKey(
    ...
    static func cachedBMUXScope(
        for pid: Int,
        cacheKey: BmuxTopProcessScopeCacheKey,
        nowNanoseconds: UInt64,
        probe: (Int, BmuxTopProcessScopeCacheKey) -> BmuxTopProcessScopeProbeResult = BmuxTopProcessSnapshot.bmuxScopeProbe
    ) -> BmuxTopProcessScope? {
        if let cached = bmuxTopScopeCache.withLock({ cache in cache[cacheKey] }) {
            if let scope = cached.scope {
                // Positive results never expire: a discovered bmux scope is stable.
                return scope
            }
            ...
            // execed into a bmux-scoped command since it was last sampled.
        }
        ...
        case .resolved(let scope):
            ...
            // return it.
            return bmuxTopScopeCache.withLock { cache -> BmuxTopProcessScope? in
                if scope == nil, let existing = cache[cacheKey], let existingScope = existing.scope {
                    return existingScope
                }
                cache[cacheKey] = BmuxTopProcessScopeCacheValue(
                    scope: scope,
                    negativeExpiresAtNanos: nowNanoseconds &+ bmuxTopNegativeScopeTTLNanoseconds
                )
                return scope
            }
        case .unavailable:
    ...
    static func pruneBMUXScopeCache(activeKeys: Set<BmuxTopProcessScopeCacheKey>) {
        bmuxTopScopeCache.withLock { cache in
            cache = cache.filter { activeKeys.contains($0.key) }
        }
    }

    // Probes a single process for its bmux scope via sysctl.
    //
    ...
    // "no readable bmux scope" — those processes are not bmux-scoped and stay in
    ...
    static func bmuxScopeProbe(
        for pid: Int,
        expectedCacheKey: BmuxTopProcessScopeCacheKey
    ) -> BmuxTopProcessScopeProbeResult {
        ...
        return .resolved(bmuxScope(fromKernProcArgs: Array(buffer.prefix(Int(size)))))
    }

    // True when `pid` is still the same process (same start time) as the key.
    private static func processMatchesKey(
        _ pid: Int,
        _ expectedCacheKey: BmuxTopProcessScopeCacheKey
    ) -> Bool {
        guard let process = kinfoProc(for: pid) else { return false }
        return scopeCacheKey(from: process) == expectedCacheKey
    }

    static func bmuxScope(fromKernProcArgs bytes: [UInt8]) -> BmuxTopProcessScope? {
        guard let process = processArgumentsAndEnvironment(fromKernProcArgs: bytes) else {
            return nil
        }
        return bmuxScope(arguments: process.arguments, environment: process.environment)
    }

    static func bmuxScope(arguments: [String], environment: [String: String]) -> BmuxTopProcessScope? {
        if let environmentScope = bmuxScopeFromEnvironment(environment) {
            return environmentScope
        }
        if let hookScope = bmuxHookMonitorScope(arguments: arguments) {
            return hookScope
        }
        return nil
    }

    private static func bmuxScopeFromEnvironment(_ environment: [String: String]) -> BmuxTopProcessScope? {
        let workspaceID = uuidValue(in: environment, keys: ["BMUX_WORKSPACE_ID", "BMUX_TAB_ID"])
        let surfaceID = uuidValue(in: environment, keys: ["BMUX_SURFACE_ID", "BMUX_PANEL_ID"])
        guard workspaceID != nil || surfaceID != nil else { return nil }
        return BmuxTopProcessScope(
            workspaceID: workspaceID,
            surfaceID: surfaceID,
            attributionReason: "bmux-environment"
        )
    }

    private static func bmuxHookMonitorScope(arguments: [String]) -> BmuxTopProcessScope? {
        ...
        return BmuxTopProcessScope(
            workspaceID: workspaceID,
            surfaceID: surfaceID,
            attributionReason: "bmux-hook-arguments"
        )
    }
    ...
    private static func containsSubcommandPath(_ path: [String], in arguments: [String]) -> Bool {
        ...
        let executableName = URL(fileURLWithPath: arguments[0])
            .lastPathComponent
            .lowercased()
        guard executableName == "bmux" else { return false }
...
nonisolated extension BmuxTopProcessArguments {
    func matchesBMUXScope(workspaceId: UUID, surfaceId: UUID) -> Bool {
        guard let scope = BmuxTopProcessSnapshot.bmuxScope(arguments: arguments, environment: environment) else {
            return false
        }
