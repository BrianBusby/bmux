...
private nonisolated let bmuxTopPIDPathBufferSize = 4096

nonisolated extension BmuxTopProcessSnapshot {
    static func allProcesses(includeProcessDetails: Bool, includeBMUXScope: Bool) -> [BmuxTopProcessInfo] {
        let sampledProcesses = allBSDProcesses()
        guard !sampledProcesses.isEmpty else { return [] }

        var scopeKeyByPID: [Int: BmuxTopProcessScopeCacheKey] = [:]
        ...
        var parentScopeKeys: [BmuxTopProcessScopeCacheKey: BmuxTopProcessScopeCacheKey] = [:]
        ...
        var currentCPUSamples: [BmuxTopProcessScopeCacheKey: BmuxTopProcessCPUSample] = [:]
        var processRecords: [(info: BmuxTopProcessInfo, cpuSampleKey: BmuxTopProcessScopeCacheKey?)] = []
        ...
        if includeBMUXScope {
            pruneBMUXScopeCache(activeKeys: activeScopeKeys)
        }
    ...
    private static func processInfo(
        ...
        includeBMUXScope: Bool,
        sampledAtNanoseconds: UInt64,
        currentCPUSamples: inout [BmuxTopProcessScopeCacheKey: BmuxTopProcessCPUSample]
    ) -> (info: BmuxTopProcessInfo, cpuSampleKey: BmuxTopProcessScopeCacheKey?)? {
        ...
        let bmuxScope = includeBMUXScope
            ? cachedBMUXScope(for: pid, cacheKey: cacheKey, nowNanoseconds: sampledAtNanoseconds)
            : nil
        ...
        let cpuSampleKey: BmuxTopProcessScopeCacheKey?
        if let taskInfo {
        ...
        return (BmuxTopProcessInfo(
            ...
            path: path,
            ttyDevice: ttyDevice,
            bmuxWorkspaceID: bmuxScope?.workspaceID,
            bmuxSurfaceID: bmuxScope?.surfaceID,
            bmuxAttributionReason: bmuxScope?.attributionReason,
    ...
    private static func processPath(pid: Int) -> String? {
        var buffer = [CChar](repeating: 0, count: bmuxTopPIDPathBufferSize)
