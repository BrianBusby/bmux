...
struct BmuxEventSubscriptionSnapshot {
    let subscription: BmuxEventSubscription
    let replay: [[String: Any]]
    let ack: [String: Any]
}

// Sendable safety: every mutable field is protected by `lock`; `semaphore` only wakes `next(timeout:)`.
final class BmuxEventSubscription: @unchecked Sendable {
    let id: UUID
...
// Sendable safety: event state is protected by `lock`; disk appends are delegated to `BmuxEventLogWriter`.
final class BmuxEventBus: @unchecked Sendable {
    static let shared = BmuxEventBus(eventLogURL: defaultEventLogURL())
    static let protocolName = "bmux-events"
    ...
    static let defaultMaxPendingEventLogLines = BmuxEventLogWriter.defaultMaxPendingLines
    ...
    private let eventLogWriter: BmuxEventLogWriter?
    private let bootId = UUID().uuidString
    private var nextSequence: Int64 = 1
    private var retained: [[String: Any]] = []
    private var subscriptions: [UUID: BmuxEventSubscription] = [:]

    init(
        retainedEventLimit: Int = BmuxEventBus.defaultRetainedEventLimit,
        eventLogURL: URL? = nil,
        maxEventLogBytes: UInt64 = BmuxEventBus.defaultMaxEventLogBytes,
        maxEventLineBytes: Int = BmuxEventBus.defaultMaxEventLineBytes,
        maxPendingEventLogLines: Int = BmuxEventBus.defaultMaxPendingEventLogLines,
        maxPendingEventsPerSubscription: Int = BmuxEventBus.defaultMaxPendingEventsPerSubscription
    ) {
    ...
    func subscribe(
        ...
        let gapReason: String? = afterSequence.flatMap { after in
            ...
            if after > latestSequence {
                return "requested sequence is newer than this bmux process; bmux probably restarted"
            }
            return nil
        }
        ...
        return BmuxEventSubscriptionSnapshot(subscription: subscription, replay: replay, ack: ack)
    }

    func unsubscribe(_ subscription: BmuxEventSubscription) {
        lock.lock()
    ...
    private func removeSubscriptionIfStillActive(_ subscription: BmuxEventSubscription) {
        lock.lock()
    ...
    func heartbeat(subscription: BmuxEventSubscription) -> [String: Any] {
        [
            "type": "heartbeat",
            "protocol": Self.protocolName,
            "version": Self.protocolVersion,
    ...
    private static func sanitizedJSONValue(_ value: Any, depth: Int) -> Any {
        ...
        case let value as [String: Any]:
            ...
            if value.count > maxSanitizedObjectEntries {
                result["__bmux_truncated_entries"] = value.count - maxSanitizedObjectEntries
            }
            return result
        case let value as [Any]:
            ...
            if value.count > maxSanitizedArrayItems {
                result.append(["__bmux_truncated_items": value.count - maxSanitizedArrayItems])
            }
            return result
        default:
