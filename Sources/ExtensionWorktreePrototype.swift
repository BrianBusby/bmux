...
struct BmuxExtensionWorktreeCreationResult: Sendable {
...
struct BmuxExtensionWorktreeWorkspaceSpawnArgs: Sendable, Equatable {
    let title: String
...
extension BmuxExtensionWorktreeCreationResult {
    ...
    func workspaceSpawnArgs() -> BmuxExtensionWorktreeWorkspaceSpawnArgs {
        ...
        BmuxExtensionWorktreeWorkspaceSpawnArgs(
...
final class BmuxExtensionProcessTermination: @unchecked Sendable {
...
enum BmuxExtensionWorktreePrototype {
    static func createWorktree(projectRootPath: String) async throws -> BmuxExtensionWorktreeCreationResult {
        try await Task.detached(priority: .userInitiated) {
            ...
            try await ensureBmuxWorktreeDirectoryIsLocallyIgnored(projectRoot: projectRoot)

            let branchName = "bmux-sidebar-\(Int(Date().timeIntervalSince1970))-\(UUID().uuidString.prefix(8).lowercased())"
            ...
            let samplePath = shellEscaped(worktree.appendingPathComponent("bmux-sample-dev", isDirectory: true).path)
            return BmuxExtensionWorktreeCreationResult(
    ...
    private static func ensureGitRepository(at projectRoot: URL) async throws {
        ...
        throw NSError(
            domain: "BmuxExtensionWorktreePrototype",
            code: 1,
            userInfo: [NSLocalizedDescriptionKey: "Project root is not a git repository."]
        )
    }

    private static func ensureBmuxWorktreeDirectoryIsLocallyIgnored(projectRoot: URL) async throws {
        ...
        let alreadyIgnored = existing
            ...
            .contains { $0 == ".bmux" || $0 == ".bmux/" }
        guard !alreadyIgnored else { return }

        let separator = existing.isEmpty || existing.hasSuffix("\n") ? "" : "\n"
        let next = existing + separator + "# bmux extension worktrees\n.bmux/\n"
    ...
    private static func writeSampleDevServerFiles(in worktree: URL, projectName: String) throws {
        let sample = worktree.appendingPathComponent("bmux-sample-dev", isDirectory: true)
        ...
        <!doctype html>
        <html>
          <head><meta charset="utf-8"><title>bmux worktree</title></head>
          <body style="font: 15px -apple-system; padding: 32px;">
            <h1>\(escapedProject) worktree</h1>
            <p>This page is served from a git worktree created by BmuxExtensionKit.</p>
          </body>
        </html>
        """
    ...
    private static func runCapturingOutput(_ executable: String, _ arguments: [String]) async throws -> Data {
        ...
        let termination = BmuxExtensionProcessTermination()
        process.terminationHandler = { process in
            termination.complete(process.terminationStatus)
        }
        try process.run()
        let outputCollector = BmuxExtensionPipeOutputCollector(fileHandle: pipe.fileHandleForReading)
        ...
        guard terminationStatus == 0 else {
            ...
            throw NSError(
                ...
                userInfo: [
                    ...
                    "BmuxExtensionWorktreePrototypeDetails": details
                ]
            )
        }
        return outputData
...
final class BmuxExtensionPipeOutputCollector: @unchecked Sendable {
