...
enum KeyboardShortcutSettings {
    static let didChangeNotification = Notification.Name("bmux.keyboardShortcutSettingsDidChange")
    static let actionUserInfoKey = "action"
    static let settingsFileDisplayPath = "~/.config/bmux/bmux.json"
    ...
    enum Action: String, CaseIterable, Identifiable {
        ...
        var label: String {
            ...
            case .quit: return String(localized: "menu.quitBmux", defaultValue: "Quit bmux")
        ...
        var defaultShortcut: StoredShortcut {
            ...
            case .newBrowserWorkspace:
                ...
                // (Cmd+Shift+N) without colliding with any bmux default or an
            ...
            case .clearScreenKeepScrollback:
                ...
                // both Ghostty defaults and bmux, and sits next to the full-clear
            ...
            case .toggleBrowserFocusMode:
                ...
                // way while focus mode is off and bmux owns the shortcut, and it
                // avoids the Ctrl+Cmd+Return global hotkey some screen recorders use.
                // Exit stays double-Escape; rebind in Settings or bmux.json.
            ...
            case .openDiffViewer:
                ...
                // Adding Shift yields a chord that reaches bmux while keeping the "D for
        ...
        func normalizedSettingsFileShortcut(_ shortcut: StoredShortcut) -> StoredShortcut? {
            // bmux.json can load while the global settings store is still initializing.
...
final class SystemWideHotkeyController {
    ...
    func start() {
        ...
        // The live Settings UI uses the BmuxSettingsUI package recorder, which
    ...
    private func refreshRegistration() {
        ...
        // or the BmuxSettingsUI package recorder) so a system-wide hotkey being
...
#if DEBUG
        if status != noErr {
            bmuxDebugLog("globalHotkey.handlerInstall failed status=\(status)")
        }
#endif
    }
    ...
    private func handleHotKeyEvent(_ event: EventRef?) -> OSStatus {
        ...
        bmuxDebugLog("globalHotkey.fire action=\(action.rawValue) shortcut=\(shortcut) active=\(NSApp.isActive ? 1 : 0)")
#endif
...
enum KeyboardShortcutRecorderActivity {
    static let didChangeNotification = Notification.Name("bmux.keyboardShortcutRecorderActivityDidChange")
    static let stopAllNotification = Notification.Name("bmux.keyboardShortcutRecorderActivityStopAll")
