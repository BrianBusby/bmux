...
/// live output callback. bmux owns the whole protocol here — so it never depends
...
final class RemoteTmuxControlConnection {
    ...
    private static let cwdSubscriptionPrefix = "bmux_cwd_"
    ...
    private static let reflowSubscriptionPrefix = "bmux_reflow_"
    ...
    private func spawnProcess(createIfMissing: Bool) throws {
        ...
        let stdinWriter = RemoteTmuxControlPipeWriter(
            ...
            label: "com.bmux.remote-tmux.stdin.\(UUID().uuidString)",
    ...
    /// bmux grid. Without this a freshly attached session stays at ssh's default
    ...
    /// usually does NOT: the window still has the size bmux itself left behind, so
    ...
    private func scheduleAttachRedrawKickIfNeeded() {
        ...
        guard windowAlreadyAtTarget else {
            #if DEBUG
            bmuxDebugLog("remote.size.kick skip=windowSizeDiffers target=\(size.columns)x\(size.rows)")
            #endif
            return
        }
        #if DEBUG
        bmuxDebugLog("remote.size.kick shrink to \(size.columns)x\(size.rows - 1)")
        #endif
        ...
        attachRedrawKickTask = Task { @MainActor [weak self] in
            ...
            #if DEBUG
            bmuxDebugLog("remote.size.kick restore to \(restore.columns)x\(restore.rows)")
            #endif
    ...
    /// (it set before bmux attached): scroll region, DEC private modes, the mouse
    ...
    /// bmux copy (exactly as a local terminal behaves with a mouse-mode app).
    func capturePane(paneId: Int) {
        ...
        // pane was already on the alt screen before bmux attached, so its 1049h is
    ...
    /// again on every change as `%subscription-changed bmux_cwd_<paneId> … : <path>`.
    ...
    private func handle(_ message: RemoteTmuxControlMessage) {
        ...
        case let .subscriptionChanged(name, value):
            // bmux subscribes each pane's working directory as "bmux_cwd_<paneId>".
