...
extension TerminalController {
    ...
    /// `remote.tmux.window` — open a dedicated bmux window mirroring every tmux
    /// session on a host (the `bmux ssh-tmux` CLI entry point).
    ///
    ...
    /// authentication. bmux's control client uses plain pipes and cannot prompt,
    ...
    ///
    /// Diagnostics surface for verifying the ghostty → bmux event pipe end to end.
