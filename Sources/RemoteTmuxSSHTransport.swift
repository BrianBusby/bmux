...
actor RemoteTmuxSSHTransport {
    ...
    /// (bmux treats "no server running" / "no sessions" as zero sessions, not
    ...
    private func serverSupportsRefreshClientSubscriptions() async throws -> Bool {
        let result = try await runTmux(["refresh-client", "-B", "bmux_probe::#{version}"])
    ...
    /// runs independently of bmux, so the master is torn down even as the app exits
    ...
    /// the user an interactive `ssh` (run in their terminal by `bmux ssh-tmux`) that
    ...
    private static func runProcess(
        ...
        // the fast auth-failure exits the `bmux ssh-tmux` flow classifies.
        let exitCode: Int32
        do {
