...
extension TerminalController {
    ...
    func v2WorkspaceCreate(
        ...
        // Decode optional layout param (same JSON schema as bmux.json layout field).
        // Validate before creating the workspace so malformed layouts fail fast.
        var layoutNode: BmuxLayoutNode?
        if let rawLayout = params["layout"] {
            ...
            do {
                layoutNode = try JSONDecoder().decode(BmuxLayoutNode.self, from: layoutData)
            } catch {
