...
// surface registry (BmuxTerminalEngine) calls it through the seam instead of
...
extension AppDelegate {
...
#if DEBUG
        if after != before {
            bmuxDebugLog("recoverableRoute.prune reason=\(reason) removed=\(before - after) remaining=\(after)")
        }
#endif
    }
...
#if DEBUG
            bmuxDebugLog("recoverableRoute.forget windowId=\(String(windowId.uuidString.prefix(8)))")
#endif
        }
    }
...
#if DEBUG
        bmuxDebugLog("recoverableRoute.remember windowId=\(String(windowId.uuidString.prefix(8)))")
#endif
    }
    ...
    /// in a context. See https://github.com/manaflow-ai/bmux/issues/5794.
