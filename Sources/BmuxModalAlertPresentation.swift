...
/// How ``runBmuxModalAlert(_:presentingWindow:willPresent:)`` ended up
/// presenting an alert.
///
...
enum BmuxModalAlertPresentation {
...
/// Returns whether `window` is one of bmux's main windows.
///
/// Main windows carry the identifier `bmux.main` or a `bmux.main.<id>`
...
@MainActor
func isBmuxMainWindow(_ window: NSWindow) -> Bool {
    guard let raw = window.identifier?.rawValue else { return false }
    return raw == "bmux.main" || raw.hasPrefix("bmux.main.")
}

/// Returns the visible main bmux window best suited to host a modal sheet.
///
...
/// when no main bmux window is currently on screen, in which case callers
/// should fall back to an app-modal presentation.
///
/// - Parameter preferredWindow: A window to consider ahead of the
///   key/main/any search, used when it is visible and a bmux main window
///   (e.g. a `TabManager`'s own owning window).
@MainActor
func bmuxMainWindowForModalPresentation(preferring preferredWindow: NSWindow? = nil) -> NSWindow? {
    if let preferredWindow, preferredWindow.isVisible, isBmuxMainWindow(preferredWindow) {
        return preferredWindow
    }
    if let keyWindow = NSApp.keyWindow, keyWindow.isVisible, isBmuxMainWindow(keyWindow) {
        return keyWindow
    }
    if let mainWindow = NSApp.mainWindow, mainWindow.isVisible, isBmuxMainWindow(mainWindow) {
        return mainWindow
    }
    return NSApp.windows.first { $0.isVisible && isBmuxMainWindow($0) }
}
...
/// alert as a sheet attached to the main bmux window when one is available,
/// falling back to an app-modal `runModal()` only when there is no eligible
/// host window.
///
/// - Parameters:
///   - alert: The configured alert to present.
///   - presentingWindow: An explicit host window. When `nil`, the main bmux
///     window is resolved via ``bmuxMainWindowForModalPresentation(preferring:)``.
...
@MainActor
func runBmuxModalAlert(
    _ alert: NSAlert,
    presentingWindow: NSWindow? = nil,
    willPresent: ((BmuxModalAlertPresentation) -> Void)? = nil
) -> NSApplication.ModalResponse {
    if NSApp.activationPolicy() == .regular {
        NSApp.activate(ignoringOtherApps: true)
    }

    let hostWindow = presentingWindow ?? bmuxMainWindowForModalPresentation()
