...
extension TerminalController {
    ...
    nonisolated func v2AnnotateTopSurface(
        ...
        if let surfaceID = v2TopUUID(surface["id"]) {
            let bmuxPIDs = processSnapshot.pids(forBMUXSurfaceID: surfaceID)
            surface["bmux_process_pids"] = bmuxPIDs.sorted()
            rootPIDs.formUnion(bmuxPIDs)
            surfacePIDs.formUnion(processSnapshot.expandedPIDs(rootPIDs: bmuxPIDs))
        } else {
            surface["bmux_process_pids"] = []
        }
    ...
    private nonisolated func v2TopMemoryAttributionByPID(in windows: [[String: Any]]) -> [Int: BmuxTopProcessAttribution] {
        var result: [Int: BmuxTopProcessAttribution] = [:]
    ...
    private nonisolated func assignTopMemoryAttribution(
        _ attribution: BmuxTopProcessAttribution,
        from node: [String: Any],
        to result: inout [Int: BmuxTopProcessAttribution],
    ...
    private nonisolated func v2TopMemoryAttributionCommonOwner(
        _ lhs: BmuxTopProcessAttribution,
        _ rhs: BmuxTopProcessAttribution
    ...
    private nonisolated func v2TopMemoryAttributionSpecificity(_ attribution: BmuxTopProcessAttribution) -> Int {
