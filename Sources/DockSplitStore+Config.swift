...
extension DockSplitStore {
    ...
    ///
    /// - `.workspace`: only the project `.bmux/dock.json` (searched upward from
    ...
    /// - `.global`: only `~/.config/bmux/dock.json` with a home base directory.
    ///
    ...
    nonisolated static func writeTemplate(to url: URL) throws {
        ...
        // Intentionally empty: bmux ships no opinionated default controls (no
    ...
    nonisolated static func trustDescriptor(for resolution: DockConfigResolution) -> BmuxActionTrustDescriptor {
        let encoder = JSONEncoder()
        ...
        return BmuxActionTrustDescriptor(
            actionID: "bmux.dock",
            kind: "dockControls",
    ...
    nonisolated private static func projectBaseDirectory(for configURL: URL) -> String {
        let bmuxDirectory = configURL.deletingLastPathComponent()
        return bmuxDirectory.deletingLastPathComponent().path
    }

    nonisolated private static func globalConfigURL() -> URL {
        if ProcessInfo.processInfo.environment["BMUX_UI_TEST_MODE"] == "1",
           let testPath = ProcessInfo.processInfo.environment["BMUX_UI_TEST_DOCK_CONFIG_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !testPath.isEmpty {
            return URL(fileURLWithPath: testPath)
        }
        return FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/bmux/dock.json", isDirectory: false)
    }
