...
nonisolated func bmuxGhosttyModsFromFlags(modifierFlagsRawValue rawValue: UInt) -> ghostty_input_mods_e {
    ...
    // as the left key (https://github.com/manaflow-ai/bmux/issues/5993).
...
/// must keep the side bits (`bmuxGhosttyModsFromFlags`); mouse paths send
/// the normalized binding bits libghostty stores.
nonisolated func bmuxGhosttyMouseModsFromFlags(modifierFlagsRawValue rawValue: UInt) -> ghostty_input_mods_e {
    let flags = NSEvent.ModifierFlags(rawValue: rawValue)
    var mods = GHOSTTY_MODS_NONE.rawValue
    if flags.contains(.shift) { mods |= GHOSTTY_MODS_SHIFT.rawValue }
    if flags.contains(.control) { mods |= GHOSTTY_MODS_CTRL.rawValue }
...
nonisolated func bmuxTranslationModifierFlags(
