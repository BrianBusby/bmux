...
/// Pastebot, …) is mid-read of bmux's promised clipboard data, that round-trip
/// can wedge for ~30s on the main thread until the OS force-kills the app
/// (https://github.com/manaflow-ai/bmux/issues/6758). The same structural gap —
...
/// wedged on exactly such a lock. bmux's critical session/state save runs
...
final class TerminationWatchdog: Sendable {
    ...
    static let threadScheduler: DeadlineScheduler = { deadline, fire in
        ...
        thread.name = "com.bmuxterm.termination-watchdog"
    ...
    nonisolated(unsafe) private var latch = BMUXTerminationWatchdogLatchMake()
    ...
    func arm(deadline: TimeInterval = TerminationWatchdog.defaultDeadline) {
        guard BMUXTerminationWatchdogLatchClaim(&latch) else {
            return
        }
