...
struct RegisteredSessionAgent: Hashable, Sendable {
    ...
    init(registration: BmuxVaultAgentRegistration) {
...
enum SessionAgent: Identifiable, Codable, Sendable, Hashable {
    ...
    init?(rawValue: String) {
        ...
        default:
            guard BmuxVaultAgentRegistration.isValidID(value) else { return nil }
        ...
        if let container = try? decoder.container(keyedBy: CodingKeys.self),
           container.contains(.id) {
            ...
            if let builtIn = SessionAgent(rawValue: id),
               (!BmuxVaultAgentRegistration.isValidID(id) || SessionAgent.builtInCases.contains(builtIn)),
               !hasRegisteredMetadata {
                self = builtIn
                return
            }
            guard BmuxVaultAgentRegistration.isValidID(id) else {
...
enum AgentSpecifics: Hashable {
    ...
    case registered(BmuxVaultAgentRegistration)
}
...
struct SessionEntry: Identifiable, Hashable {
    ...
    private var resumeCommandWithoutWorkingDirectory: String? {
        ...
        case let .claude(model, permissionMode, configDirectoryForResume):
            ...
            // re-injects bmux hooks even when the command runs in a shell where the
            ...
            let environment = configDirectoryForResume.map {
                ["CLAUDE_CONFIG_DIR": $0, "BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV": "1", "BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS": "CLAUDE_CONFIG_DIR"]
            } ?? [:]
        ...
        case let .codex(model, approval, sandbox, effort):
            ...
            // auto-resumed codex session re-injects bmux hooks even when the
            ...
            // bypassing bmux-codex-wrapper, so no SessionStart fires and the iOS
            ...
            // `resumeCommandWithCwd`. https://github.com/manaflow-ai/bmux/issues/5639
    ...
    ///
    /// bmux captures Codex's *internal* sandbox-policy `type`, which is a
    ...
    /// (see https://github.com/manaflow-ai/bmux/issues/5262).
    ...
    /// the per-session policy bmux captured, always yielding a valid invocation.
    ///
