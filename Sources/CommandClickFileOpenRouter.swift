...
enum CommandClickFileOpenRouter {
    nonisolated static func shouldRouteInBmux(path: String) -> Bool {
    ...
    /// Schedule a file open in bmux, deferred to the next runloop tick.
    ///
    ...
    @MainActor
    static func deferredOpenFileInBmux(
        workspace: Workspace,
        ...
        DispatchQueue.main.async {
            ...
            guard shouldRouteInBmux(path: filePath) else {
                fallback?()
                return
            }
            if openInBmux(
