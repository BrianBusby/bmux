...
final class GlobalSearchPanelCaptureManager {
...
#if DEBUG
                    bmuxDebugLog("globalSearch.markdown.upsert failed panel=\(context.panelID.uuidString.prefix(5)) error=\(error.localizedDescription)")
#endif
                }
            }
...
#if DEBUG
                        bmuxDebugLog("globalSearch.markdown.capture failed panel=\(panelID.uuidString.prefix(5)) error=\(error.localizedDescription)")
#endif
                    }
                }
...
#if DEBUG
            bmuxDebugLog("globalSearch.markdown.purge failed panel=\(panelID.uuidString.prefix(5)) error=\(error.localizedDescription)")
#endif
        }
    }
    ...
    private func indexBrowserPanel(_ panel: BrowserPanel) async {
        ...
        do {
            ...
            guard !Task.isCancelled else { return }
#if DEBUG
            bmuxDebugLog("globalSearch.browser.upsert failed panel=\(panel.id.uuidString.prefix(5)) error=\(error.localizedDescription)")
#endif
        }
    }
