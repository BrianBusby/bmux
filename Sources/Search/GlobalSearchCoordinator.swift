...
#if DEBUG
                bmuxDebugLog("globalSearch.index.clear failed error=\(error.localizedDescription)")
#endif
            }
...
#if DEBUG
            bmuxDebugLog("globalSearch.search failed error=\(error.localizedDescription)")
#endif
            return []
        }
    }
...
#if DEBUG
                bmuxDebugLog("globalSearch.title.upsert failed panel=\(context.panelID.uuidString.prefix(5)) error=\(error.localizedDescription)")
#endif
            }
    ...
    func purgePanel(id panelID: UUID) {
        ...
        let task = Task { @MainActor [weak self] in
            ...
            do {
                ...
                guard !Task.isCancelled else { return }
#if DEBUG
                bmuxDebugLog("globalSearch.panel.purge failed panel=\(panelID.uuidString.prefix(5)) error=\(error.localizedDescription)")
#endif
            }
        }
...
#if DEBUG
            bmuxDebugLog("globalSearch.index.open failed error=\(error.localizedDescription)")
#endif
            return nil
        }
    }
