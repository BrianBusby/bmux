...
private let reactGrabMessageHandlerName = "bmuxReactGrab"
...
class ReactGrabMessageHandler: NSObject, WKScriptMessageHandler {
    ...
    func userContentController(
        ...
        case .stateChange(let isActive):
            bmuxDebugLog("reactGrab.messageHandler type=stateChange isActive=\(isActive)")
        case .copySuccess(let content, _):
            bmuxDebugLog("reactGrab.messageHandler type=copySuccess len=\(content.count)")
        }
        #endif
        Task { @MainActor in
            #if DEBUG
            switch bridgeMessage {
            case .stateChange(let isActive):
                bmuxDebugLog("reactGrab.messageHandler.mainActor type=stateChange isActive=\(isActive)")
            case .copySuccess(let content, _):
                bmuxDebugLog("reactGrab.messageHandler.mainActor type=copySuccess len=\(content.count)")
            }
            #endif
...
extension BrowserPanel {
    ...
    func injectReactGrab() async {
        #if DEBUG
        bmuxDebugLog("reactGrab.inject.start")
        #endif
        guard let scriptSource = await ReactGrabScriptLoader.fetch() else {
            #if DEBUG
            bmuxDebugLog("reactGrab.inject.fetchFailed")
            #endif
            return
        }
        #if DEBUG
        bmuxDebugLog("reactGrab.inject.fetched len=\(scriptSource.count)")
        #endif
        ...
        (function() {
            ...
            var installBridge = function(api) {
                if (!api || window.__BMUX_REACT_GRAB_BRIDGE_INSTALLED__) return;
                window.__BMUX_REACT_GRAB_BRIDGE_INSTALLED__ = true;
        ...
        """
        #if DEBUG
        bmuxDebugLog("reactGrab.inject.evalJS len=\(combined.count)")
        #endif
        webView.evaluateJavaScript(combined) { [weak self] _, error in
            #if DEBUG
            bmuxDebugLog("reactGrab.inject.evalJS.done error=\(error?.localizedDescription ?? "none")")
            #endif
            if let error {
        ...
        #if DEBUG
        bmuxDebugLog("reactGrab.inject.end")
        #endif
    }

    func toggleReactGrab() {
        #if DEBUG
        bmuxDebugLog("reactGrab.toggle.start")
        #endif
        let script = "window.__REACT_GRAB__?.toggle()"
        webView.evaluateJavaScript(script, completionHandler: nil)
        #if DEBUG
        bmuxDebugLog("reactGrab.toggle.end")
        #endif
    }
...
#if DEBUG
            bmuxDebugLog("reactGrab.bridgeSessionRefresh.error error=\(error.localizedDescription)")
#endif
            return false
        }
    }
