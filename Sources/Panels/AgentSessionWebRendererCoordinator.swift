...
final class AgentSessionWebRendererCoordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandlerWithReply {
    ...
    private func handle(_ request: AgentSessionBridgeRequest) async throws -> Any {
        ...
        case "provider.start":
            ...
            initialProviderID = provider
            onProviderIDChanged?(provider)
            let configuredExecutablePaths = AgentExecutableResolver.bmuxConfiguredExecutablePaths()
    ...
    private func sendEvent(_ event: [String: Any]) {
        ...
        webView.evaluateJavaScript("window.bmuxAgentBridge?.receive(\(json));") { _, error in
#if DEBUG
            if let error {
                bmuxDebugLog("agentSession.bridge.event.failed error=\(error.localizedDescription)")
            }
#else
            _ = error
#endif
