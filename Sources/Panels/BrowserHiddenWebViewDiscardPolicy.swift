...
nonisolated enum BrowserHiddenWebViewDiscardPolicy {
    ...
    static func isEnabled(defaults: UserDefaults) -> Bool {
        let value = ProcessInfo.processInfo.environment["BMUX_BROWSER_HIDDEN_WEBVIEW_DISCARD_ENABLED"]?
    ...
    static func hiddenDelay(defaults: UserDefaults) -> TimeInterval {
        let rawValue = ProcessInfo.processInfo.environment["BMUX_BROWSER_HIDDEN_WEBVIEW_DISCARD_DELAY_SECONDS"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
