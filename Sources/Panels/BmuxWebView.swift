...
extension WKWebView {
    nonisolated private static var bmuxSetPageMutedSelector: Selector {
        NSSelectorFromString("_setPageMuted:")
    }

    nonisolated private static var bmuxMediaMutedStateAudio: Int {
        1 << 0
    }

    @discardableResult
    func bmuxSetPageAudioMuted(_ muted: Bool) -> Bool {
        let selector = Self.bmuxSetPageMutedSelector
        ...
        function(self, selector, muted ? Self.bmuxMediaMutedStateAudio : 0)
        return true
    }

    var bmuxIsElementFullscreenActiveOrTransitioning: Bool {
    ...
    func bmuxIsManagedByExternalFullscreenWindow(relativeTo expectedWindow: NSWindow?) -> Bool {
        guard bmuxIsElementFullscreenActiveOrTransitioning else { return false }
...
/// Find-family shortcuts. The configured Find shortcut stays app-owned so bmux can
/// choose browser find or right-sidebar file search from the current focus owner.
final class BmuxWebView: WKWebView {
    ...
    private static let pasteAsPlainTextFocusMessageHandlerName = "bmuxPasteAsPlainTextFocus"
    private static let browserFocusModeContextMenuItemIdentifier =
        NSUserInterfaceItemIdentifier("bmux.browserFocusMode.toggle")
    ...
    const __bmuxPasteAsPlainTextHelpers = (() => {
      const existing = window.__bmuxPasteAsPlainTextHelpers;
      ...
      window.__bmuxPasteAsPlainTextHelpers = helpers;
      return helpers;
    })();
    """
    ...
    (() => {
      try {
        if (window.__bmuxPasteAsPlainTextFocusTrackerInstalled) return true;
        window.__bmuxPasteAsPlainTextFocusTrackerInstalled = true;
        ...
        const publish = (canPaste) => {
          ...
          window.__bmuxPasteAsPlainTextTargetAvailable = canPaste;
          try {
            handler?.postMessage({ canPaste });
          } catch (_) {}
        };

        window.__bmuxCanPasteAsPlainTextIntoCurrentFocus = () => {
          return __bmuxPasteAsPlainTextHelpers.canPasteAsPlainTextInto(document.activeElement);
        };

        const publishForElement = (el) => {
          publish(__bmuxPasteAsPlainTextHelpers.canPasteAsPlainTextInto(el));
        };
        ...
        document.addEventListener("mousedown", (ev) => {
          ...
          if (!__bmuxPasteAsPlainTextHelpers.canPasteAsPlainTextInto(target)) {
            publish(false);
          }
        }, true);
    ...
    private final class PasteAsPlainTextFocusMessageHandler: NSObject, WKScriptMessageHandler {
        func userContentController(
            ...
            guard let webView = message.webView as? BmuxWebView else {
                return
            }
    ...
    static func hasRecentMiddleClickIntent(for webView: WKWebView) -> Bool {
        guard let webView = webView as? BmuxWebView else { return false }
    ...
    private static func recordMiddleClickIntent(for webView: BmuxWebView) {
    ...
    private static var bmuxDownloadDelegateKey: UInt8 = 0
    ...
    var contextMenuLinkURLProvider: ((BmuxWebView, NSPoint, @escaping (URL?) -> Void) -> Void)?
    ...
    var bmuxDownloadDelegate: WKDownloadDelegate? {
        get {
            objc_getAssociatedObject(self, &Self.bmuxDownloadDelegateKey) as? WKDownloadDelegate
        }
        set {
            objc_setAssociatedObject(
                self,
                &Self.bmuxDownloadDelegateKey,
                newValue,
    ...
    private func pageCanAcceptPlainTextPaste() -> Bool {
        ...
        (() => {
            try {
                const fn = window.__bmuxCanPasteAsPlainTextIntoCurrentFocus;
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        if let decision = AppDelegate.shared?.handleBrowserFocusModeKeyEvent(
            ...
            case .forwardToWebView:
                ...
                // While focus mode is active, the page gets the shortcut once and bmux/main-menu
        ...
        let inspectorOwnsUndoRedo = event.bmuxIsUndoRedoCommandEquivalent && bmuxIsLikelyWebInspectorResponder(window?.firstResponder)
        if !inspectorOwnsUndoRedo && (event.bmuxIsUndoRedoCommandEquivalent || !shouldRouteCommandEquivalentDirectlyToMainMenu(event)) {
            return finish(super.performKeyEquivalent(with: event))
        }
    ...
    override func keyDown(with event: NSEvent) {
        ...
        // If this path reaches keyDown, forward it to WebKit instead of bmux.
    ...
    private func handleMouseNavigationButton(_ event: NSEvent) -> Bool {
        ...
        case 3:
#if DEBUG
            bmuxDebugLog("browser.mouse.navigation web=\(ObjectIdentifier(self)) kind=back canGoBack=\(canGoBack ? 1 : 0)")
#endif
        ...
        case 4:
#if DEBUG
            bmuxDebugLog("browser.mouse.navigation web=\(ObjectIdentifier(self)) kind=forward canGoForward=\(canGoForward ? 1 : 0)")
#endif
    ...
    /// and its lifecycle live in `BmuxWebView+ContextMenuLinkCapture.swift`;
...
#if DEBUG
        bmuxDebugLog(Self.redactedContextDownloadDebugMessage(message()))
#endif
    }
    ...
    private static let blockedDragTypes: Set<NSPasteboard.PasteboardType> = [
        ...
        NSPasteboard.PasteboardType("com.bmux.sidebar-tab-reorder"),
    ]
