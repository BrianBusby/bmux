...
struct MarkdownWebRenderer: NSViewRepresentable {
    static let localImageURLScheme = "bmux-local-image"
    static let remoteImageURLScheme = "bmux-remote-image"
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        // Bridge: JS posts to `bmuxLib` to request lazy-loaded libraries
        // (mermaid / vega-lite). Swift fetches the bundled source from the
        // app bundle and injects it via evaluateJavaScript.
        config.userContentController.add(WeakMarkdownScriptMessageHandler(context.coordinator), name: "bmuxLib")
    ...
    static func dismantleNSView(_ nsView: WKWebView, coordinator: Coordinator) {
        ...
        nsView.configuration.userContentController.removeScriptMessageHandler(forName: "bmuxLib")
    ...
    /// panel from the system appearance and follow the bmux color scheme.
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler, WKURLSchemeHandler {
        ...
        private func applyFontSize(forceShellSync: Bool = false) {
            ...
            if shouldSyncShell { webView.evaluateJavaScript("window.__bmuxSetMarkdownZoom && window.__bmuxSetMarkdownZoom(\(Double(zoom)));", completionHandler: nil) }
        }
        ...
        func close() {
            if let webView {
                webView.stopLoading()
                webView.configuration.userContentController.removeScriptMessageHandler(forName: "bmuxLib")
        ...
        func renderedHTML(markdown: String? = nil) async -> String? {
            ...
            return await evaluateString("window.__bmuxRenderedHTML && window.__bmuxRenderedHTML()")
        }

        func renderedText() async -> String? {
            guard isLoaded else { return nil }
            return await evaluateString("window.__bmuxRenderedText && window.__bmuxRenderedText()")
        }
        ...
        private func applyTheme(_ theme: MarkdownWebTheme) {
            ...
            (function(vars) {
              ...
              if (window.__bmuxApplyTheme) { window.__bmuxApplyTheme(); }
            })(\(json));
            """
            webView.evaluateJavaScript(js, completionHandler: nil)
        }
        ...
        private static func renderMarkdownScript(_ markdown: String) -> String? {
            ...
            return """
            (function(md) {
              if (window.__bmuxRenderMarkdown) {
                window.__bmuxRenderMarkdown(md);
                return;
              }
        ...
        private func resolveMarkdownFile(_ rawPath: String, requestId: String) {
            ...
            guard let data = try? JSONSerialization.data(withJSONObject: payload),
                  let json = String(data: data, encoding: .utf8) else { return }
            webView.evaluateJavaScript("window.__bmuxMarkdownFileResolved && window.__bmuxMarkdownFileResolved(\(json));", completionHandler: nil)
        }
        ...
        private func handleLibRequest(_ lib: String) {
            ...
            let libLiteral = (try? JSONSerialization.data(withJSONObject: [lib]))
                .flatMap { String(data: $0, encoding: .utf8) } ?? "[\"\"]"
            let suffix = "\nwindow.__bmuxLibLoaded && window.__bmuxLibLoaded(\(libLiteral)[0]);"
        ...
        func webView(
            ...
            // route through the bmux tab/browser machinery.
        ...
        /// Route a clicked link to a brand-new bmux browser tab in the same
        ...
        private func handleExternalLink(_ url: URL) {
            ...
            // open as markdown tabs in bmux, not in the browser.
            ...
            _ = location.workspace.createBrowserSurfaceForAction(
                inPane: paneId,
                url: url,
                focus: true
            )
