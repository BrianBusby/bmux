...
enum BrowserWebAuthnBridgeContract {
    static let handlerName = "bmuxWebAuthn"
    static let contentWorld = WKContentWorld.world(name: "bmux.webauthn")

    private static let requestEventName = "__bmuxWebAuthnRequest"
    private static let acknowledgeEventName = "__bmuxWebAuthnRequestAcknowledged"
    private static let responseEventName = "__bmuxWebAuthnResponse"
    ...
    static let relayScriptSource: String = {
        ...
        return #"""
        (() => {
          if (window.__bmuxWebAuthnNativeRelayInstalled) {
            return true;
          }
          window.__bmuxWebAuthnNativeRelayInstalled = true;
    ...
    static let scriptSource: String = {
        ...
        (() => {
          ...
          if (window.__bmuxWebAuthnBridgeInstalled) {
            return true;
          }
          window.__bmuxWebAuthnBridgeInstalled = true;
          ...
          const nextNativeRequestID = () => {
            nextRequestID += 1;
            return `bmux-webauthn-${Date.now()}-${nextRequestID}`;
          };
    ...
    func userContentController(
        ...
        Task { @MainActor in
            do {
                ...
                #if DEBUG
                bmuxDebugLog("webauthn.dispatch kind=\(envelope.kind.rawValue) frame=\(message.frameInfo.isMainFrame ? "main" : "sub") url=\(message.frameInfo.securityOrigin.host)")
                #endif
                ...
                case .capabilities:
                    ...
                    #if DEBUG
                    bmuxDebugLog("webauthn.capabilities reply=\(capReply)")
                    #endif
                ...
                case .createCredential:
                    ...
                    #if DEBUG
                    bmuxDebugLog("webauthn.createCredential reply.ok=\(reply["ok"] ?? "nil") hasCredential=\(reply["credential"] != nil) fallback=\(reply["useWebKitFallback"] ?? "nil")")
                    #endif
                ...
                case .getCredential:
                    ...
                    #if DEBUG
                    bmuxDebugLog("webauthn.getCredential reply.ok=\(reply["ok"] ?? "nil") hasCredential=\(reply["credential"] != nil) fallback=\(reply["useWebKitFallback"] ?? "nil")")
                    #endif
                    replyHandler(reply, nil)
                }
            } catch let error as BrowserWebAuthnBridgeError {
                #if DEBUG
                bmuxDebugLog("webauthn.error bridge: \(error.replyObject())")
                #endif
                replyHandler(error.replyObject(), nil)
            } catch {
                #if DEBUG
                bmuxDebugLog("webauthn.error unknown: \(error.localizedDescription)")
                #endif
...
extension BrowserWebAuthnCoordinator: ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    func authorizationController(
        ...
        #if DEBUG
        bmuxDebugLog("webauthn.asAuth.didComplete credentialType=\(type(of: authorization.credential))")
        #endif
        do {
            finishAuthorization(
                with: .success(
            ...
            #if DEBUG
            bmuxDebugLog("webauthn.asAuth.didComplete replyMarshalError=\(error)")
            #endif
    ...
    func authorizationController(
        ...
        bmuxDebugLog("webauthn.asAuth.didFail domain=\(nsError.domain) code=\(nsError.code)")
        #endif
    ...
    func presentationAnchor(for controller: ASAuthorizationController) -> ASPresentationAnchor {
        ...
        #if DEBUG
        bmuxDebugLog("webauthn.asAuth.presentationAnchor hasTitle=\(anchor.title.isEmpty ? 0 : 1) isVisible=\(anchor.isVisible) isKey=\(anchor.isKeyWindow)")
        #endif
        return anchor
    }
}
...
private extension BrowserWebAuthnCoordinator {
    ...
    func handleCreateCredential(
        ...
        guard let plan = try buildCreationPlan(request, clientDataContext: clientDataContext) else {
            #if DEBUG
            bmuxDebugLog("webauthn.handleCreate no plan — returning fallback")
            #endif
        ...
        guard !requests.isEmpty else {
            #if DEBUG
            bmuxDebugLog("webauthn.handleCreate authorizationRequests empty — returning fallback")
            #endif
            return fallbackReply()
        }
    ...
    func handleGetCredential(
        ...
        guard let plan = try buildAssertionPlan(request, clientDataContext: clientDataContext) else {
            #if DEBUG
            bmuxDebugLog("webauthn.handleGet no plan — returning fallback")
            #endif
            return fallbackReply()
        }
        ...
        guard !requests.isEmpty else {
            #if DEBUG
            bmuxDebugLog("webauthn.handleGet authorizationRequests empty — returning fallback")
            #endif
            return fallbackReply()
        }
        let presentationWindow = try interactivePresentationWindow(for: message)
    ...
    func authorizationRequests(
        ...
        #if DEBUG
        bmuxDebugLog("webauthn.authRequests hasPlatform=\(plan.hasPlatformRequests) hasSecurityKey=\(plan.securityKeyRequests.count > 0) order=\(plan.order)")
        #endif

        if includePlatformRequests {
            let currentState = BrowserPasskeyAuthorizationGate.shared.currentAuthorizationState()
            #if DEBUG
            bmuxDebugLog("webauthn.authRequests passkeyAuthState=\(currentState.rawValue) callerMayPrompt=\(callerMayPromptForPlatformAuthorization(message))")
            #endif
            if currentState == .notDetermined && !callerMayPromptForPlatformAuthorization(message) {
                #if DEBUG
                bmuxDebugLog("webauthn.authRequests skipping platform: cross-origin subframe can't prompt")
                #endif
                includePlatformRequests = false
            } else {
                let authorizationState = await BrowserPasskeyAuthorizationGate.shared.authorizeIfNeeded()
                #if DEBUG
                bmuxDebugLog("webauthn.authRequests authorizeIfNeeded result=\(authorizationState.rawValue)")
                #endif
        ...
        #if DEBUG
        bmuxDebugLog("webauthn.authRequests finalCount=\(requests.count) includePlatform=\(includePlatformRequests)")
        #endif
        guard !requests.isEmpty else {
            #if DEBUG
            bmuxDebugLog("webauthn.authRequests FAIL: no requests available, throwing notAllowed")
            #endif
        ...
        if plan.needsBluetoothPreparation(includePlatformRequests: includePlatformRequests) {
            #if DEBUG
            bmuxDebugLog("webauthn.authRequests preparing bluetooth")
            #endif
            let btState = await BrowserBluetoothAuthorizationGate.shared.prepareIfNeeded()
            #if DEBUG
            bmuxDebugLog("webauthn.authRequests bluetooth result=\(btState)")
            #endif
        }
    ...
    func performAuthorization(
        ...
        for (i, req) in requests.enumerated() {
            bmuxDebugLog("webauthn.performAuth request[\(i)]=\(type(of: req))")
        }
        #endif
        guard !requests.isEmpty else {
            throw BrowserWebAuthnBridgeError.notSupported("Native passkey support is unavailable.")
        ...
        guard let window = browserInteractiveModalHostWindow(window) else {
            #if DEBUG
            bmuxDebugLog("webauthn.performAuth FAIL: no interactive window")
            #endif
            throw BrowserWebAuthnBridgeError.notAllowed("Passkey access is not available.")
        }
        guard activeAuthorizationContinuation == nil else {
            #if DEBUG
            bmuxDebugLog("webauthn.performAuth FAIL: ceremony already in progress")
            #endif
            throw BrowserWebAuthnBridgeError.notAllowed("The passkey request failed.")
        }

        #if DEBUG
        bmuxDebugLog("webauthn.performAuth launching ASAuthorizationController")
        #endif
    ...
    func buildCreationPlan(
        ...
        guard !platformRequests.isEmpty || !securityKeyRequests.isEmpty else {
            #if DEBUG
            bmuxDebugLog("webauthn.buildCreationPlan no requests built — returning nil")
            #endif
            return nil
        }

        #if DEBUG
        bmuxDebugLog("webauthn.buildCreationPlan rp=\(relyingPartyIdentifier) platform=\(platformRequests.count) securityKey=\(securityKeyRequests.count) attachment=\(attachment ?? "(nil)")")
        #endif
        return .init(
    ...
    func buildAssertionPlan(
        ...
        guard !platformRequests.isEmpty || !securityKeyRequests.isEmpty else {
            #if DEBUG
            bmuxDebugLog("webauthn.buildAssertionPlan no requests built — returning nil")
            #endif
            return nil
        }
        ...
        #if DEBUG
        bmuxDebugLog("webauthn.buildAssertionPlan rp=\(relyingPartyIdentifier) platform=\(platformRequests.count) securityKey=\(securityKeyRequests.count) allowCredentials=\(allowCredentials.count) mediation=\(request.mediation ?? "(nil)") hybridTransport=\(transportSummary.shouldShowHybridTransport)")
        #endif
        return .init(
            platformRequests: platformRequests,
            securityKeyRequests: securityKeyRequests,
