...
final class DiffCommentsBridge: NSObject, WKScriptMessageHandlerWithReply {
    ...
    static func isTrustedDiffViewerFrame(_ frameInfo: WKFrameInfo) -> Bool {
        ...
        return BmuxDiffViewerURLSchemeHandler.shared.hasActiveSession(token: token)
    }
    ...
    /// in-page router rewrites `#bmux-diff-viewer` to `#/bmux-diff-viewer`
    ...
    static func diffViewerToken(from url: URL?) -> String? {
        if let components = BmuxDiffViewerURLSchemeHandler.diffViewerComponents(from: url) {
        ...
        guard parts.count >= 2,
              BmuxDiffViewerURLSchemeHandler.isValidToken(parts[0]),
              BmuxDiffViewerURLSchemeHandler.isValidRequestPath("/" + parts.dropFirst().joined(separator: "/")) else {
            return nil
        }
        return parts[0]
    }
