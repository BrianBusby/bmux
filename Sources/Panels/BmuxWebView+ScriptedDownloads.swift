...
extension BmuxWebView {
    private static let scriptedDownloadMessageHandlerName = "bmuxScriptedDownload"
    ...
    (() => {
      try {
        if (window.__bmuxScriptedDownloadInstalled) return true;
        window.__bmuxScriptedDownloadInstalled = true;
    ...
    private func startScriptedWebKitDownload(
        ...
        guard let downloadDelegate = bmuxDownloadDelegate else {
#if DEBUG
...
private final class ScriptedDownloadMessageHandler: NSObject, WKScriptMessageHandler {
    func userContentController(
        ...
        guard let webView = message.webView as? BmuxWebView,
