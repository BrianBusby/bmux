...
enum BrowserScreenshotWebViewSnapshotter {
    static func captureFullPage(
        ...
        do {
            ...
            #if DEBUG
            bmuxDebugLog("browser.screenshot.fullPage.singleSnapshot.failed error=\(error.localizedDescription)")
            #endif
        }
    ...
    static func withOffscreenRenderHost<T>(
        ...
        window.identifier = NSUserInterfaceItemIdentifier("bmux.browserVisualAutomationRender")
    ...
    static func prepareForVisualCapture(_ webView: WKWebView, expectedURL: URL?) async throws {
        ...
        do {
            ...
            #if DEBUG
            bmuxDebugLog("browser.screenshot.prepare.failed error=\(error.localizedDescription)")
            #endif
        }
