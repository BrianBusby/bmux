...
enum MarkdownFontFamily {
    /// UserDefaults / bmux.json key (`markdown.fontFamily`).
    ...
    /// collapse to spaces so a malformed bmux.json value cannot produce invalid
    /// multiline CSS.
    ...
    /// UserDefaults / bmux.json and falling back to the System default.
