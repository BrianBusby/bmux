...
@_spi(BmuxHostTransport) import BmuxSidebar
@_spi(BmuxHostTransport) import BmuxExtensionKit
import AppKit
import ExtensionFoundation
import SwiftUI

private struct BMUXSidebarExtensionGrant: Codable, Equatable {
    var manifestID: String
    var manifestDisplayName: String
    var apiVersion: BmuxExtensionAPIVersion
    var readScopes: Set<BmuxExtensionScope>
    var actionScopes: Set<BmuxExtensionActionScope>
}

private struct BMUXSidebarExtensionEffectiveGrant: Equatable {
    var manifest: BmuxExtensionManifest
    var readScopes: Set<BmuxExtensionScope>
    var actionScopes: Set<BmuxExtensionActionScope>
    ...
    var hasSensitiveAccess: Bool {
        readScopes.contains { !BMUXSidebarExtensionGrantStore.defaultReadScopes.contains($0) } ||
            actionScopes.contains { !BMUXSidebarExtensionGrantStore.defaultActionScopes.contains($0) }
    }
}

private struct BMUXSidebarExtensionGrantStore {
    static let defaultReadScopes: Set<BmuxExtensionScope> = []
    static let defaultActionScopes: Set<BmuxExtensionActionScope> = []

    private static let defaultsKey = "bmuxExtensionSidebar.grants.v1"
    ...
    func effectiveGrant(
    ...
    ) -> BMUXSidebarExtensionEffectiveGrant {
        ...
        guard let grant = storedGrants()[bundleIdentifier],
            ...
            return BMUXSidebarExtensionEffectiveGrant(
                manifest: manifest,
    ...
    func grantRequestedAccess(bundleIdentifier: String, manifest: BmuxExtensionManifest) {
        updateGrant(
    ...
    func revokeSensitiveAccess(bundleIdentifier: String, manifest: BmuxExtensionManifest) {
        updateGrant(
            bundleIdentifier: bundleIdentifier,
            manifest: manifest,
    ...
    private func updateGrant(
        bundleIdentifier: String,
        manifest: BmuxExtensionManifest,
        readScopes: Set<BmuxExtensionScope>,
        actionScopes: Set<BmuxExtensionActionScope>
    ) {
        var grants = storedGrants()
        grants[bundleIdentifier] = BMUXSidebarExtensionGrant(
    ...
    private func storedGrants() -> [String: BMUXSidebarExtensionGrant] {
        guard let data = defaults.data(forKey: Self.defaultsKey) else { return [:] }
        return (try? JSONDecoder().decode([String: BMUXSidebarExtensionGrant].self, from: data)) ?? [:]
    }

    private func save(_ grants: [String: BMUXSidebarExtensionGrant]) {
...
private struct BMUXSidebarExtensionLimitedChoiceStore {
    private static let defaultsKey = "bmuxExtensionSidebar.limitedChoices.v1"
...
struct BMUXInstalledExtensionSidebarHostView: View {
    private static let selectedExtensionBundleIDDefaultsKey = "bmuxExtensionSidebar.selectedExtensionBundleId"
    private static let selectedExtensionNameDefaultsKey = "bmuxExtensionSidebar.selectedExtensionName"

    var snapshotProvider: @MainActor () -> BmuxSidebarSnapshot
    var snapshotUpdateToken: UInt64 = 0
    var actionHandler: @MainActor (BmuxSidebarAction) -> BmuxSidebarActionResult
    ...
    @State private var xpcHost = BMUXSidebarExtensionHostXPC()
    @State private var effectiveGrant: BMUXSidebarExtensionEffectiveGrant?
    ...
    @State private var keptLimitedManifestKeys = BMUXSidebarExtensionLimitedChoiceStore().choices()
        ...
        Group {
            if let identity {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    .accessibilityIdentifier("BMUXExtensionSidebarHostView")
                ...
                .padding(24)
                .accessibilityIdentifier("BMUXExtensionSidebarEmptyState")
            } else {
                VStack(spacing: 16) {
                    Image(systemName: "puzzlepiece.extension")
                        .bmuxFont(size: 26, weight: .regular)
                        .foregroundStyle(.secondary)
    ...
    private func observeExtensionAvailability() async {
        ...
        do {
            try await observeIdentitySequence(
                extensionPointIdentifier: BmuxSidebarExtensionPoint.identifier()
            )
        } catch {
            identity = nil
            xpcHost.invalidate()
            ...
            errorText = String(
                ...
                defaultValue: "BMUX could not load sidebar extensions."
            )
        }
    }
    ...
    private var emptyStateDetail: String {
        ...
        return String(
            ...
            defaultValue: "Install and enable a BMUX sidebar extension to show it here."
        )
    }
    ...
    private var extensionAvailabilityDetail: String {
        if unapprovedExtensionCount > 0 {
            return String(
                ...
                defaultValue: "An installed sidebar extension needs approval before BMUX can use it."
            )
        }
        return String(
    ...
    private func extensionDetailsPopover(activeIdentity: AppExtensionIdentity?) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 8) {
                Image(systemName: "puzzlepiece.extension")
                    .bmuxFont(size: 18, weight: .medium)
    ...
    private func blockedExtensionView(reason: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Image(systemName: "exclamationmark.triangle")
                .bmuxFont(size: 20, weight: .regular)
                .foregroundStyle(.secondary)
        ...
        .accessibilityIdentifier("BMUXExtensionSidebarBlockedState")
    }
    ...
    private func blockedDetailText(reason: String) -> String {
        switch reason {
        case "connectionInterrupted":
            return String(localized: "sidebar.extensions.blocked.detail.connectionInterrupted", defaultValue: "BMUX lost the extension connection. No workspace data or actions are being shared.")
        case "manifestTimedOut":
            return String(localized: "sidebar.extensions.blocked.detail.manifestTimedOut", defaultValue: "BMUX did not receive this extension's configuration in time. No workspace data or actions are being shared.")
        case "missingManifest":
            return String(localized: "sidebar.extensions.blocked.detail.missingManifest", defaultValue: "BMUX did not receive a sidebar extension configuration, so no workspace data or actions were shared.")
        case "invalidManifest":
            return String(localized: "sidebar.extensions.blocked.detail.invalidManifest", defaultValue: "BMUX rejected this extension's configuration. No workspace data or actions were shared.")
        default:
            return String(localized: "sidebar.extensions.blocked.detail.failedManifest", defaultValue: "BMUX could not load this extension's configuration. No workspace data or actions were shared.")
        }
    }
    ...
    private func permissionSection(effectiveGrant: BMUXSidebarExtensionEffectiveGrant) -> some View {
        VStack(alignment: .leading, spacing: 8) {
    ...
    private func extensionAccessBanner(
        ...
        effectiveGrant: BMUXSidebarExtensionEffectiveGrant
    ) -> some View {
        VStack(alignment: .leading, spacing: 8) {
    ...
    private func accessReviewSheet(
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text(String(localized: "sidebar.extensions.access.review.detail", defaultValue: "BMUX will only share the following data and actions if you allow this request."))
                .bmuxFont(size: 12)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private func grantRequestedAccess(
        ...
        BMUXSidebarExtensionLimitedChoiceStore().remove(key)
    ...
    private func keepLimitedAccess(
        ...
        BMUXSidebarExtensionLimitedChoiceStore().insert(key)
    ...
    private func permissionDescription(scope: BmuxExtensionScope) -> String {
        switch scope {
        case .workspaceList:
    ...
    private func permissionDescription(actionScope: BmuxExtensionActionScope) -> String {
        switch actionScope {
...
private extension BmuxExtensionScope {
...
private extension BmuxExtensionActionScope {
    var displayName: String {
        switch self {
        case .createWorkspace:
...
@MainActor
private final class BMUXSidebarExtensionHostXPC {
    private static let untrustedScopes: Set<BmuxExtensionScope> = []
    private static let untrustedActionScopes: Set<BmuxExtensionActionScope> = []
    private static let manifestRequestTimeoutNanoseconds: UInt64 = 5_000_000_000

    private var connection: NSXPCConnection?
    private var extensionProxy: BMUXSidebarExtensionXPC?
    private var exportedObject: BMUXSidebarHostXPCObject?
    private var snapshotProvider: (() -> BmuxSidebarSnapshot)?
    private var actionHandler: ((BmuxSidebarAction) -> BmuxSidebarActionResult)?
    private var allowedScopes = untrustedScopes
    private var allowedActionScopes = untrustedActionScopes
    private var connectionGeneration: UInt64 = 0
    private var bundleIdentifier: String?
    private var currentManifest: BmuxExtensionManifest?
    private var onGrantChanged: ((BMUXSidebarExtensionEffectiveGrant?) -> Void)?
    ...
    private let grantStore = BMUXSidebarExtensionGrantStore()

    var currentEffectiveGrant: BMUXSidebarExtensionEffectiveGrant? {
    ...
    func update(
        snapshotProvider: @escaping @MainActor () -> BmuxSidebarSnapshot,
        actionHandler: @escaping @MainActor (BmuxSidebarAction) -> BmuxSidebarActionResult
    ) {
    ...
    func attach(
        ...
        snapshotProvider: @escaping @MainActor () -> BmuxSidebarSnapshot,
        actionHandler: @escaping @MainActor (BmuxSidebarAction) -> BmuxSidebarActionResult,
        onGrantChanged: @escaping @MainActor (BMUXSidebarExtensionEffectiveGrant?) -> Void,
        onManifestBlocked: @escaping @MainActor (String?) -> Void
    ) {
        invalidate()
        connectionGeneration += 1
        let generation = connectionGeneration
        let exportedObject = BMUXSidebarHostXPCObject(
        ...
        connection.exportedInterface = NSXPCInterface(with: BMUXSidebarHostXPC.self)
        connection.exportedObject = exportedObject
        connection.remoteObjectInterface = NSXPCInterface(with: BMUXSidebarExtensionXPC.self)
        ...
        self.extensionProxy = connection.remoteObjectProxy as? BMUXSidebarExtensionXPC
        connection.resume()
    ...
    func sendSnapshotDidChange() {
        ...
        do {
            extensionProxy.sidebarSnapshotDidChange(try BmuxSidebarXPCCodec.encodeSnapshot(filteredSnapshot(from: snapshotProvider)))
        } catch {
#if DEBUG
            bmuxDebugLog("extension.sidebar.xpc.snapshot.encode.failed error=\(error.localizedDescription)")
#endif
        }
    }
    ...
    private func requestManifestThenSendInitialSnapshot(generation: UInt64) {
        ...
        requestExtensionManifest { [weak self] payload, error in
            Task { @MainActor [generation] in
                ...
                if let payload {
                    do {
                        let manifest = try BmuxSidebarXPCCodec.decodeManifest(payload)
                        try validateSidebarManifest(manifest)
                        self.applyManifest(manifest)
                    } catch {
                        self.blockUntrustedExtension(reason: "invalidManifest")
#if DEBUG
                        bmuxDebugLog("extension.sidebar.manifest.invalid error=\(error.localizedDescription)")
#endif
                    }
                } else {
                    ...
                    if let error {
#if DEBUG
                        bmuxDebugLog("extension.sidebar.manifest.failed error=\(error)")
#endif
                    }
                }
    ...
    private func applyManifest(_ manifest: BmuxExtensionManifest) {
        cancelManifestRequestTimeout()
    ...
    private func filteredSnapshot(from snapshotProvider: () -> BmuxSidebarSnapshot) -> BmuxSidebarSnapshot {
    ...
    private func scopedActionHandler(
        _ actionHandler: @escaping @MainActor (BmuxSidebarAction) -> BmuxSidebarActionResult
    ) -> (@MainActor (BmuxSidebarAction) -> BmuxSidebarActionResult) {
        { [weak self] action in
            guard let self,
                  self.currentManifest != nil,
...
#if DEBUG
        bmuxDebugLog("extension.sidebar.manifest.blocked reason=\(reason)")
#endif
    }

    private static func untrustedSnapshot(from snapshot: BmuxSidebarSnapshot) -> BmuxSidebarSnapshot {
        BmuxSidebarSnapshot(
...
private final class BMUXSidebarHostXPCObject: NSObject, BMUXSidebarHostXPC {
    @MainActor var snapshotProvider: () -> BmuxSidebarSnapshot
    @MainActor var actionHandler: (BmuxSidebarAction) -> BmuxSidebarActionResult
    ...
    func requestSidebarSnapshot(reply: @escaping (NSData?, NSString?) -> Void) {
        Task { @MainActor in
            ...
            do {
                reply(try BmuxSidebarXPCCodec.encodeSnapshot(snapshotProvider()), nil)
            } catch {
    ...
    func performSidebarAction(_ payload: NSData, reply: @escaping (NSData?, NSString?) -> Void) {
        Task { @MainActor in
            ...
            do {
                let action = try BmuxSidebarXPCCodec.decodeAction(payload)
                let result = actionHandler(action)
                reply(try BmuxSidebarXPCCodec.encodeActionResult(result), nil)
                if result.accepted {
                    onAcceptedAction()
                }
            } catch {
