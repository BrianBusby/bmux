...
private enum BmuxThemeNotifications {
    static let reloadConfig = Notification.Name("com.bmuxterm.themes.reload-config")
}
...
/// (cloudVM in particular launches `bmux vm base open` and returns immediately).
...
#if DEBUG
enum BmuxTypingTiming {
    static let isEnabled: Bool = {
        let environment = ProcessInfo.processInfo.environment
        if environment["BMUX_TYPING_TIMING_LOGS"] == "1" || environment["BMUX_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
        let defaults = UserDefaults.standard
        return defaults.bool(forKey: "bmuxTypingTimingLogs") || defaults.bool(forKey: "bmuxKeyLatencyProbe")
    }()
    static let isVerboseProbeEnabled: Bool = {
        let environment = ProcessInfo.processInfo.environment
        if environment["BMUX_KEY_LATENCY_PROBE"] == "1" {
            return true
        }
        return UserDefaults.standard.bool(forKey: "bmuxKeyLatencyProbe")
    }()
    ...
    static func logEventDelay(path: String, event: NSEvent) {
        ...
        bmuxDebugLog("typing.delay path=\(path) delayMs=\(format(delayMs)) \(eventFields(event))")
    }

    @inline(__always)
    static func logDuration(path: String, startedAt: TimeInterval?, event: NSEvent? = nil, extra: String? = nil) {
        BmuxMainThreadTurnProfiler.endMeasure(path, startedAt: startedAt)
...
final class BmuxMainRunLoopStallMonitor {
    static let shared = BmuxMainRunLoopStallMonitor()
    ...
    func installIfNeeded() {
        guard BmuxTypingTiming.isEnabled else { return }
        ...
        observer = CFRunLoopObserverCreate(
            ...
            { _, activity, info in
                ...
                let monitor = Unmanaged<BmuxMainRunLoopStallMonitor>.fromOpaque(info).takeUnretainedValue()
...
final class BmuxMainThreadTurnProfiler {
    static let shared = BmuxMainThreadTurnProfiler()
    ...
    static func endMeasure(_ bucket: String, startedAt: TimeInterval?) {
        guard let startedAt, BmuxTypingTiming.isEnabled, Thread.isMainThread else { return }
        let elapsedMs = max(0, (ProcessInfo.processInfo.systemUptime - startedAt) * 1000.0)
    ...
    func installIfNeeded() {
        ...
        observer = CFRunLoopObserverCreate(
            ...
            { _, activity, info in
                guard let info else { return }
                let profiler = Unmanaged<BmuxMainThreadTurnProfiler>.fromOpaque(info).takeUnretainedValue()
...
@MainActor
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation, NSMenuDelegate, BmuxConfigStoreReloadEnvironment {
    nonisolated(unsafe) static var shared: AppDelegate?
    /// Stateless control-socket syscall layer (BmuxControlSocket); composition-root owned.
    nonisolated let socketTransport = SocketTransport()
    /// Owns the About Titlebar Debug subsystem (BmuxAppKitSupportUI); composition-root
    ...
    private static let reloadConfigurationMenuItemIdentifier = NSUserInterfaceItemIdentifier("com.bmux.reloadConfiguration")
    ...
    static func detectRunningUnderXCTest(_ env: [String: String]) -> Bool {
        ...
        if env.keys.contains(where: { $0.hasPrefix("BMUX_UI_TEST_") }) { return true }
        return false
    }
    ...
    final class MainWindowContext {
        ...
        var bmuxConfigStore: BmuxConfigStore?
        ...
        init(
            ...
            self.bmuxConfigStore = bmuxConfigStore
            self.window = window
    ...
    private final class MainWindowController: NSWindowController, NSWindowDelegate {
        ...
        func windowWillUseStandardFrame(_ window: NSWindow, defaultFrame newFrame: NSRect) -> NSRect {
            guard window is BmuxMainWindow else { return newFrame }
            return BmuxMainWindow.standardFrame(forDefaultFrame: newFrame)
        }
    }
    ...
    /// Lifted to ``BmuxWindowing/SessionDisplayGeometry``; aliased so existing
    /// `AppDelegate.SessionDisplayGeometry` references stay source-identical.
    typealias SessionDisplayGeometry = BmuxWindowing.SessionDisplayGeometry
    ...
    private nonisolated static let persistedWindowGeometryDefaultsKey = "bmux.session.lastWindowGeometry.v2"
#if DEBUG
    ...
    private nonisolated static let legacyPersistedWindowGeometryDefaultsKeys = [
        "bmux.session.lastWindowGeometry.v1"
    ]
    ...
    private lazy var configStoreReloadCoordinator: BmuxConfigStoreReloadCoordinator = {
        BmuxConfigStoreReloadCoordinator(environment: self) { source, storeCount in
#if DEBUG
            bmuxDebugLog("bmuxConfig.reload source=\(source) stores=\(storeCount)")
#endif
        }
    }()
...
#if DEBUG
        debugLog = { line in bmuxDebugLog(line) }
#else
        debugLog = nil
#endif
    ...
    private static let didInstallWindowKeyEquivalentSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.bmux_performKeyEquivalent(with:))
    ...
    private static let didInstallWindowFirstResponderSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.bmux_makeFirstResponder(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallWindowSendEventSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSWindow.bmux_sendEvent(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallApplicationSendEventSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSApplication.bmux_applicationSendEvent(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallApplicationSendActionSwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSApplication.bmux_sendAction(_:to:from:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    private static let didInstallApplicationAccessibilitySwizzle: Void = {
        ...
        let swizzledSelector = #selector(NSApplication.bmux_accessibilityAttributeValue(_:))
        guard let originalMethod = class_getInstanceMethod(targetClass, originalSelector),
              let swizzledMethod = class_getInstanceMethod(targetClass, swizzledSelector) else {
            return
        }
    ...
    /// Live `bmux diff` viewer subprocesses, keyed by pid, retained until they exit.
    ...
    /// instead of spawning the bundled `bmux diff` CLI, so shortcut-dispatch tests can
    ...
    private func childExitKeyboardProbePath() -> String? {
        ...
        guard env["BMUX_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1",
              let path = env["BMUX_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
    ...
    var pendingStartupNavigationURLRequests: [BmuxNavigationURLRequest] = []
    private var sessionAutosaveTimer: DispatchSourceTimer?
    private var sessionAutosaveTickInFlight = false
    private var sessionAutosaveDeferredRetryPending = false
    private var processDetectedSessionSaveGeneration: UInt64 = 0
    private let sessionPersistenceQueue = DispatchQueue(
        label: "com.bmuxterm.app.sessionPersistence",
        qos: .utility
    )
    /// Session snapshot persistence (BmuxSession); composition-root owned.
    ...
    /// Accessibility window-hierarchy cache (BmuxWindowing); composition-root
    ...
    /// First-responder bypass guard (BmuxBrowserPanel); composition-root owned.
    ...
    private nonisolated static let launchServicesRegistrationQueue = DispatchQueue(
        label: "com.bmuxterm.app.launchServicesRegistration",
        qos: .utility
    )
    ...
    static let screenChangeReconcileNotification = Notification.Name("com.bmuxterm.app.screenChangeReconcile")
    static let displayReconfigurationNotification = Notification.Name("com.bmuxterm.app.displayReconfiguration")
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        ...
        #endif
        if handleBmuxExternalURLs(from: urls) {
            #if DEBUG
        ...
        // (built-in bmux schemes) so dropped callbacks are still detected.
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DistributedNotificationCenter.default().addObserver(
            ...
            name: BmuxThemeNotifications.reloadConfig,
            object: nil,
        ...
        BmuxMainRunLoopStallMonitor.shared.installIfNeeded()
        BmuxMainThreadTurnProfiler.shared.installIfNeeded()
        ...
        if telemetryEnabled {
            ...
            SentrySDK.start { options in
                ...
                // trace sample rate). bmux does not consume these performance
                // traces today.
        ...
        if !isRunningUnderXCTest {
            BmuxFeatureFlags.shared.start()
        }

        let forceDuplicateLaunchObserver = env["BMUX_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] == "1"
        ...
        if env["BMUX_UI_TEST_MODE"] == "1" {
            let trigger = env["BMUX_UI_TEST_TRIGGER_UPDATE_CHECK"] ?? "<nil>"
            let feed = env["BMUX_UI_TEST_FEED_URL"] ?? "<nil>"
            updateLog.append("ui test env: trigger=\(trigger) feed=\(feed)")
        }
        if env["BMUX_UI_TEST_TRIGGER_UPDATE_CHECK"] == "1" {
        ...
        if isRunningUnderXCTest {
            if let rawVariant = env["BMUX_UI_TEST_BROWSER_IMPORT_HINT_VARIANT"] {
                UserDefaults.standard.set(
                    BrowserImportHintSettings.variant(for: rawVariant).rawValue,
                    forKey: BrowserImportHintSettings.variantKey
                )
            }
            if let rawShow = env["BMUX_UI_TEST_BROWSER_IMPORT_HINT_SHOW"] {
                UserDefaults.standard.set(
                    rawShow == "1",
                    forKey: BrowserImportHintSettings.showOnBlankTabsKey
                )
            }
            if let rawDismissed = env["BMUX_UI_TEST_BROWSER_IMPORT_HINT_DISMISSED"] {
                UserDefaults.standard.set(
                    rawDismissed == "1",
            ...
            if env["BMUX_UI_TEST_BROWSER_IMPORT_HINT_OPEN_BLANK_BROWSER"] == "1" {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) { [weak self] in
                    guard let self else { return }
                    _ = self.openBrowserAndFocusAddressBar(insertAtEnd: true)
                }
            }
            if env["BMUX_UI_TEST_BROWSER_IMPORT_HINT_OPEN_SETTINGS"] == "1" {
            ...
            if env["BMUX_UI_TEST_BROWSER_IMPORT_AUTO_OPEN"] == "1" {
    ...
    private func writeUITestDiagnosticsIfNeeded(stage: String) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["BMUX_UI_TEST_DIAGNOSTICS_PATH"], !path.isEmpty else { return }
        ...
        let screenIDs = windows.map { $0.screen?.bmuxDisplayID.map(String.init) ?? "" }.joined(separator: ",")
        let targetDisplayID = env["BMUX_UI_TEST_TARGET_DISPLAY_ID"] ?? ""
        ...
        if let rawDisplayID = UInt32(targetDisplayID) {
            let screenPresent = NSScreen.screens.contains(where: { $0.bmuxDisplayID == rawDisplayID })
            let movedWindow = windows.contains(where: { $0.screen?.bmuxDisplayID == rawDisplayID })
    ...
    private func appendUITestSocketDiagnosticsIfNeeded(
        ...
        guard env["BMUX_UI_TEST_SOCKET_SANITY"] == "1" else { return }

        guard let config = socketListenerConfigurationIfEnabled() else {
            payload["socketExpectedPath"] = env["BMUX_SOCKET_PATH"] ?? ""
    ...
    private func appendUITestPortalDiagnosticsIfNeeded(
        ...
        guard env["BMUX_UI_TEST_PORTAL_STATS"] == "1" else { return }
    ...
    private func appendUITestRenderDiagnosticsIfNeeded(
        ...
        guard env["BMUX_UI_TEST_DISPLAY_RENDER_STATS"] == "1" else { return }
    ...
    private func moveUITestWindowToTargetDisplayIfNeeded(attempt: Int = 0) {
        let env = ProcessInfo.processInfo.environment
        guard let rawDisplayID = env["BMUX_UI_TEST_TARGET_DISPLAY_ID"],
              let targetDisplayID = UInt32(rawDisplayID) else {
            return
        }

        guard let screen = NSScreen.screens.first(where: { $0.bmuxDisplayID == targetDisplayID }) else {
            if attempt < 20 {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [weak self] in
        ...
        window.orderFrontRegardless()
        if window.screen?.bmuxDisplayID != targetDisplayID, attempt < 20 {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { [weak self] in
                self?.moveUITestWindowToTargetDisplayIfNeeded(attempt: attempt + 1)
            }
            return
    ...
    func configure(
        ...
        let env = ProcessInfo.processInfo.environment
        if isRunningUnderXCTest(env) || env["BMUX_UI_TEST_MODE"] == "1" {
            scheduleUITestSocketSanityCheckIfNeeded()
        }
        // Best-effort one-time migration: a value previously stored in the
        // legacy ~/.config/bmux/dev-window-display file moves into the shared
        // bmux.json (app.devWindowDisplay) so an existing dev-display default
        ...
        ghosttyCrashBreadcrumbTask = Task { [weak self, weak notificationStore] in
            ...
            notificationStore.addNotification(
                ...
                title: String(
                    ...
                    defaultValue: "bmux crashed during your last session"
                ),
                subtitle: String(
    ...
    private func setupTerminalCmdClickUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_SETUP"] == "1" else {
            bmuxDebugLog("cmdclick.ui.setup skip reason=env_missing tag=\(env["BMUX_TAG"] ?? "nil")")
            return
        }
        guard let manifestPath = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !manifestPath.isEmpty else {
            bmuxDebugLog("cmdclick.ui.setup skip reason=missing_manifest_path")
            return
        }
        didSetupTerminalCmdClickUITest = true
        guard let fixtureDirectory = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_FIXTURE_DIR"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !fixtureDirectory.isEmpty else {
            bmuxDebugLog("cmdclick.ui.setup error reason=missing_fixture_dir manifest=\(manifestPath)")
            writeTerminalCmdClickUITestData(at: manifestPath, updates: [
                "setupError": "Missing fixture directory"
            ])
            return
        }
        let commandPath = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_COMMAND_PATH"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let screenshotDirectory = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_SCREENSHOT_DIR"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let displayMode = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_MODE"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let lineFormat = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_LINE_FORMAT"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let linePrefix = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_LINE_PREFIX"] ?? ""
        let displaySuffix = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_SUFFIX"] ?? ""
        let displayAsAbsolutePath = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_AS_ABSOLUTE_PATH"] == "1"
        if let rawOpenSupportedFiles = env["BMUX_UI_TEST_OPEN_SUPPORTED_FILES_IN_BMUX"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
        ...
        if let rawOpenMarkdown = env["BMUX_UI_TEST_OPEN_MARKDOWN_IN_BMUX_VIEWER"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !rawOpenMarkdown.isEmpty {
            FileRouteSettingsStore(defaults: .standard).setMarkdownRouteEnabled(rawOpenMarkdown == "1")
        }
        let extraFileNamesJSON = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_EXTRA_FILE_NAMES_JSON"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)

        let fileName = env["BMUX_UI_TEST_TERMINAL_CMD_CLICK_FILE_NAME"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        func captureWindowSnapshotIfRequested(label: String, window: NSWindow) -> String? {
            ...
            do {
                ...
                bmuxDebugLog("cmdclick.ui.snapshot failed label=\(label) error=\(error.localizedDescription)")
                return nil
            }
        }
        ...
        func executePendingCommandIfNeeded(
            ...
            case "stationary_cmd_click_token":
                ...
                let capturePath = ProcessInfo.processInfo.environment["BMUX_UI_TEST_CAPTURE_OPEN_URL_PATH"]
        ...
        bmuxDebugLog("cmdclick.ui.setup poller_started manifest=\(manifestPath)")
        poller.resume()
    }
    ...
    private func writeTerminalCmdClickUITestData(at path: String, updates: [String: Any]) {
        ...
        guard let data = try? JSONSerialization.data(withJSONObject: payload, options: [.sortedKeys]) else {
            bmuxDebugLog("cmdclick.ui.write skip reason=json path=\(path)")
            return
        }
        do {
            try FileManager.default.createDirectory(
            ...
            bmuxDebugLog("cmdclick.ui.write error path=\(path) error=\(error.localizedDescription)")
        }
    }
    ...
    private func setupFeedSidebarUITestIfNeeded() {
        ...
        guard let path = env["BMUX_UI_TEST_FEED_SIDEBAR_RESULT_PATH"], !path.isEmpty else { return }
    ...
    private func startFeedSidebarUITestPushIfNeeded(resultPath: String) {
        ...
        guard let requestId = env["BMUX_UI_TEST_FEED_SIDEBAR_REQUEST_ID"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    private func loadStartupSessionSnapshotPruningCrashDiagnostics() -> AppSessionSnapshot? {
        ...
        case .loaded(let snapshot):
            if let prunedSnapshot = SessionPersistencePolicy
                .pruningBmuxCrashDiagnosticWindows(from: snapshot)
                .snapshot {
    ...
    private func loadManualRestoreSessionSnapshotPruningCrashDiagnostics() -> AppSessionSnapshot? {
        sessionSnapshotStore.loadReopenSessionSnapshot(fileURL: nil).flatMap {
            SessionPersistencePolicy.pruningBmuxCrashDiagnosticWindows(from: $0).snapshot
        }
    }
    ...
    func restorePreviousSessionSnapshot(
        ...
        guard let snapshot = SessionPersistencePolicy.pruningBmuxCrashDiagnosticWindows(from: snapshot).snapshot else {
            return false
        }
...
#if DEBUG
            bmuxDebugLog("session.save.skipped reason=session_restore_in_progress includeScrollback=0")
#endif
            return false
        }
...
#if DEBUG
        let timingStart = BmuxTypingTiming.start()
        defer {
    ...
    private func buildSessionSnapshotResult(
        ...
        for context in contexts {
            ...
            let pruned = SessionPersistencePolicy.pruningBmuxCrashDiagnosticWindows(
    ...
    // (bmuxTests/AppDelegateMainWindowTestingSupport.swift, via @testable
    ...
    func registerMainWindow(
        ...
        bmuxConfigStore: BmuxConfigStore? = nil
    ) {
        ...
        if let existing = mainWindowContexts[key] {
            ...
            if let bmuxConfigStore {
                existing.bmuxConfigStore = bmuxConfigStore
            }
    ...
    /// Lifted to ``BmuxWindowing/MainWindowSummary``; aliased so existing
    /// `AppDelegate.MainWindowSummary` references stay source-identical.
    typealias MainWindowSummary = BmuxWindowing.MainWindowSummary
...
#if DEBUG
            bmuxDebugLog("surface.move.fail panel=\(panelId.uuidString.prefix(5)) reason=sourcePanelNotFound elapsedMs=\(elapsedMs(since: moveStart))")
#endif
            return false
        }
        guard let sourceWorkspace = source.tabManager.tabs.first(where: { $0.id == source.workspaceId }) else {
#if DEBUG
            bmuxDebugLog("surface.move.fail panel=\(panelId.uuidString.prefix(5)) reason=sourceWorkspaceMissing elapsedMs=\(elapsedMs(since: moveStart))")
#endif
            return false
        }
        guard let destinationManager = tabManagerFor(tabId: targetWorkspaceId) else {
#if DEBUG
            bmuxDebugLog("surface.move.fail panel=\(panelId.uuidString.prefix(5)) reason=destinationManagerMissing elapsedMs=\(elapsedMs(since: moveStart))")
#endif
            return false
        }
        guard let destinationWorkspace = destinationManager.tabs.first(where: { $0.id == targetWorkspaceId }) else {
#if DEBUG
            bmuxDebugLog("surface.move.fail panel=\(panelId.uuidString.prefix(5)) reason=destinationWorkspaceMissing elapsedMs=\(elapsedMs(since: moveStart))")
#endif
            return false
        }
#if DEBUG
    ...
    private func pruneExpiredCommandPalettePendingOpenStates(
        ...
        for outcome in pruned {
            ...
            case .missingTimestamp(let windowId):
                bmuxDebugLog("shortcut.palette.pendingPrune windowId=\(windowId.uuidString.prefix(8)) reason=missingTimestamp")
...
#if DEBUG
        bmuxDebugLog("shortcut.escape suppressionClear target={nil} clearedAll=1 keyUpConsumed=\(didConsume ? 1 : 0)")
#endif
        return didConsume
    }
    ...
    private func keyRoutingOwnerView(for responder: NSResponder?) -> NSView? {
        ...
        if let editor = responder as? NSTextView,
           editor.isFieldEditor {
            return bmuxFieldEditorOwnerView(editor) ?? editor
        }
    ...
    private func responderHasViableKeyRoutingOwner(
        ...
        if let ghosttyView = bmuxOwningGhosttyView(for: responder) {
...
#if DEBUG
        bmuxDebugLog("reload.config.surfaceRefresh source=\(source) count=\(refreshedCount)")
#endif
    }
    ...
    func focusMainWindow(windowId: UUID) -> Bool {
        ...
        if didFocus {
            publishBmuxWindowLifecycle(name: "window.focused", windowId: windowId, origin: "focus_request")
        }
        return didFocus
    }
    ...
    private func mainWindowId(for window: NSWindow) -> UUID? {
        ...
        guard let rawIdentifier = window.identifier?.rawValue,
              rawIdentifier.hasPrefix("bmux.main.") else { return nil }
        let idPart = String(rawIdentifier.dropFirst("bmux.main.".count))
    ...
    /// bundled `bmux diff` CLI. This is the single shared diff-open path: both the
    ...
    func launchDiffViewerProcess(
        ...
        environment["BMUX_WORKSPACE_ID"] = workspaceId.uuidString
        if let surfaceId {
            environment["BMUX_SURFACE_ID"] = surfaceId.uuidString
        }
        ...
        process.terminationHandler = { terminatedProcess in
            ...
            Task { @MainActor in
                ...
                bmuxDebugLog("openDiffViewer exited status=\(terminationStatus) outputBytes=\(output.utf8.count)")
#endif
                NSSound.beep()
            }
        }
...
#if DEBUG
            bmuxDebugLog("openDiffViewer pid=\(process.processIdentifier)")
#endif
            return true
        } catch {
            outputCollector.cancel()
#if DEBUG
            bmuxDebugLog("openDiffViewer failed errorType=\(type(of: error))")
#endif
            return false
        }
    }
    ...
    private func auxiliaryWindowForFocusedCloseShortcut(event: NSEvent) -> NSWindow? {
        ...
        .compactMap { $0 }
        .first { bmuxWindowShouldOwnCloseShortcut($0) }
    }
    ...
    func bootstrapInitialMainWindowIfNeeded(
        ...
        if ProcessInfo.processInfo.environment["BMUX_UI_TEST_SHOW_SETTINGS"] == "1" {
...
#if DEBUG
            bmuxDebugLog("newBrowserWorkspace.blocked_browser_disabled source=\(debugSource)")
#endif
            NSSound.beep()
            return false
        }
    ...
    func performProUpgradeWorkspaceAction(
        ...
        guard BrowserAvailabilitySettings.isEnabled() else {
#if DEBUG
            bmuxDebugLog("proUpgradeWorkspace.blocked_browser_disabled source=\(debugSource)")
#endif
            return nil
        }
    ...
    func performCloudVMAction(
        ...
        let didStart = CloudVMActionLauncher.shared.start(
            ...
            environmentOverrides: [
                "BMUX_CLOUD_ATTACH_RETRY_LIMIT": "12",
                "BMUX_CLOUD_ATTACH_RETRY_DELAY_SECONDS": "2",
            ],
    ...
    private func existingCloudVMWorkspace(in tabManager: TabManager) -> Workspace? {
        tabManager.tabs.first { workspace in
            ...
            return remote.persistentDaemonSlot == "bmux-default-freestyle-sshd-v1" &&
    ...
    private func executeConfiguredNewWorkspaceActionIfAvailable(
        ...
        guard let bmuxConfigStore = context.bmuxConfigStore,
              let action = bmuxConfigStore.resolvedNewWorkspaceAction() else {
            return false
        }
        guard let window = resolvedWindow(for: context) else {
            discardOrphanedMainWindowContext(context)
    ...
    private func workspaceGroupNewWorkspaceTarget(in context: MainWindowContext) -> WorkspaceGroupNewWorkspaceTarget? {
        ...
        let configured = context.bmuxConfigStore?.resolveWorkspaceGroupConfig(forCwd: anchorCwd)?.newWorkspacePlacement
    ...
    func showRepoAgentLauncherMenu(
        ...
        let menu = NSMenu()
        for launcher in bmuxConfigStore.repoAgentLauncherItems {
            let item = NSMenuItem(
    ...
    private func promptForRepoAgentLauncher(
        ...
        while true {
            ...
            guard runBmuxModalAlert(alert, presentingWindow: presentingWindow) == .alertFirstButtonReturn else {
                return nil
            }
            currentPath = pathField.stringValue.trimmingCharacters(in: .whitespacesAndNewlines)
            selectedAgent = (agentPopup.selectedItem?.representedObject as? BmuxConfigAgentKind) ?? selectedAgent
        ...
        var errorDescription: String? {
            ...
            case .unreadableConfig:
                return String(
                    ...
                    defaultValue: "The bmux config file could not be read."
                )
            case .noSupportedAgent:
                return String(
            ...
            case .invalidRoot:
                return String(
                    ...
                    defaultValue: "The bmux config file must contain a JSON object."
                )
            case .invalidActions:
                return String(
                    localized: "dialog.repoAgentLauncher.saveFailed.invalidActions",
                    defaultValue: "The bmux config actions section is not an object."
                )
            case .invalidCommands:
                return String(
                    localized: "dialog.repoAgentLauncher.saveFailed.invalidCommands",
                    defaultValue: "The bmux config commands section is not an array."
                )
            case .encodeFailed:
                return String(
            ...
            case .writeFailed:
                return String(
                    ...
                    defaultValue: "The bmux config file could not be updated."
                )
            }
        }
    }
    ...
    private func repoAgentLauncherAgentTitle(_ agent: BmuxConfigAgentKind) -> String {
        switch agent {
        case .codex:
    ...
    private func externalOpenDirectories(from urls: [URL]) -> [String] {
        ...
        FinderServicePathResolver.orderedUniqueDirectories(
            ...
            !SessionPersistencePolicy.isBmuxCrashStoragePath($0)
        }
    }
    ...
    private func externalOpenFileURLs(from urls: [URL]) -> [URL] {
        ...
        for url in urls where url.isFileURL && !externalOpenURLIsDirectory(url) {
            ...
            guard !SessionPersistencePolicy.isBmuxCrashStorageURL(standardized) else { continue }
...
#if DEBUG
        bmuxDebugLog("file.externalOpen source=\(debugSource) path=\(filePath)")
#endif
    ...
    func preferredMainWindowContextForShortcutRouting(event: NSEvent) -> MainWindowContext? {
        ...
        if shortcutEventHasAddressableWindow(event) {
            if let eventWindow = resolvedShortcutEventWindow(event),
               bmuxWindowShouldOwnCloseShortcut(eventWindow) {
                // Auxiliary bmux windows do not own a terminal tab manager. Let them fall back
    ...
    func createMainWindow(
        ...
        let shouldStartWithHiddenSidebarForTerminalViewportUITest =
            ProcessInfo.processInfo.environment["BMUX_UI_TEST_TERMINAL_VIEWPORT_HIDE_SIDEBAR"] == "1"
#else
...
#if DEBUG
        if ProcessInfo.processInfo.environment["BMUX_UI_TEST_BONSPLIT_SHOW_RIGHT_SIDEBAR"] == "1" {
        ...
        let root = ContentView(updateViewModel: updateViewModel, windowId: windowId)
            ...
            .environmentObject(bmuxConfigStore)
            ...
            .bmuxFontMagnificationEnvironment()
        ...
        if restoredFrame == nil, let existingFrame {
            ...
            initialRect = BmuxMainWindow.defaultContentRect(styleMask: styleMask)
        }
        ...
        let minimumWindowSize = BmuxMainWindow.minimumContentSize
        ...
        // bmux persists and restores main windows itself. Disable AppKit window
        // restoration so the OS cannot resurrect stale duplicate main windows.
        window.isRestorable = false
        configureBmuxMainWindowDragBehavior(window)
        ...
        publishBmuxWindowLifecycle(name: "window.created", windowId: windowId, origin: "create")
...
#if DEBUG
        // Honor the shared dev-only default display (set via `bmux window
    ...
    func sendWelcomeCommandWhenReady(to workspace: Workspace, markShownOnSend: Bool = false) {
        sendTextWhenReady("bmux welcome\n", to: workspace, beforeSend: {
            if markShownOnSend {
    ...
    func isBmuxCLIInstalledInPATH() -> Bool {
        BmuxCLIPathInstaller().isInstalled()
    }

    @objc func installBmuxCLIInPath(_ sender: Any?) {
        ...
        do {
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.installed", defaultValue: "bmux CLI Installed"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.installFailed", defaultValue: "Couldn't Install bmux CLI"),
    ...
    @objc func uninstallBmuxCLIInPath(_ sender: Any?) {
        let installer = BmuxCLIPathInstaller()
        do {
            ...
            let prefix = outcome.removedExistingEntry
                ...
                : String(localized: "cli.uninstall.notFound", defaultValue: "No bmux CLI symlink was found at \(outcome.destinationURL.path).")
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.uninstalled", defaultValue: "bmux CLI Uninstalled"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.uninstallFailed", defaultValue: "Couldn't Uninstall bmux CLI"),
                informativeText: error.localizedDescription,
                style: .warning
            )
        }
...
#if DEBUG
        bmuxDebugLog("settings.open.present path=swiftuiWindow")
#endif
        showFallbackSettingsWindow(navigationTarget)
        activateApplication()
#if DEBUG
        bmuxDebugLog("settings.open.present activate=1")
#endif
    }
...
#if DEBUG
        bmuxDebugLog("settings.open.request source=\(debugSource)")
#endif
    ...
    private func waitForDebugStressMountedWorkspaces(_ workspaces: [Workspace]) async -> Int {
        ...
        bmuxDebugLog("stress.setup.mount mounted=\(mountedWorkspaceCount)/\(workspaces.count)")
    ...
    private func setupJumpUnreadUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["BMUX_UI_TEST_JUMP_UNREAD_SETUP"] == "1" else { return }
        guard let notificationStore else { return }
    ...
    func armJumpUnreadFocusRecord(tabId: UUID, surfaceId: UUID) {
        let env = ProcessInfo.processInfo.environment
        guard let path = env["BMUX_UI_TEST_JUMP_UNREAD_PATH"], !path.isEmpty else { return }
    ...
    private func setupGotoSplitUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        if env["BMUX_UI_TEST_GOTO_SPLIT_RECORD_ONLY"] == "1" {
            installGotoSplitUITestFocusObserversIfNeeded()
            startGotoSplitRecordOnlyRecorder()
            return
        }
        guard env["BMUX_UI_TEST_GOTO_SPLIT_SETUP"] == "1" else { return }
        guard tabManager != nil else { return }

        let useGhosttyConfig = env["BMUX_UI_TEST_GOTO_SPLIT_USE_GHOSTTY_CONFIG"] == "1"
        ...
        func runSetupWhenWindowReady() {
            ...
            let requestedBrowserURL = env["BMUX_UI_TEST_GOTO_SPLIT_BROWSER_URL"]?
                .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func setupBonsplitTabDragUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["BMUX_UI_TEST_BONSPLIT_TAB_DRAG_SETUP"] == "1" else { return }
        guard tabManager != nil else { return }
        let startWithHiddenSidebar = env["BMUX_UI_TEST_BONSPLIT_START_WITH_HIDDEN_SIDEBAR"] == "1"
        let showRightSidebar = env["BMUX_UI_TEST_BONSPLIT_SHOW_RIGHT_SIDEBAR"] == "1"
        ...
        func mainWindowContextForUITest() -> (window: NSWindow, context: MainWindowContext)? {
            for window in NSApp.windows {
                ...
                guard raw == "bmux.main" || raw.hasPrefix("bmux.main.") else { continue }
        ...
        func runSetupWhenWindowReady() {
            ...
            if let screenFrame {
                let targetSize: NSSize
                if let rawSize = env["BMUX_UI_TEST_BONSPLIT_WINDOW_SIZE"] {
                    let parts = rawSize
            ...
            if let rawActionButtonCount = env["BMUX_UI_TEST_BONSPLIT_ACTION_BUTTON_COUNT"],
               let requestedActionButtonCount = Int(rawActionButtonCount),
               requestedActionButtonCount > 0 {
                guard let bmuxConfigStore = context.bmuxConfigStore else {
                    self.writeBonsplitTabDragUITestData(["setupError": "Missing bmux config store"])
                    return
                }
                ...
                let buttons = (1...actionButtonCount).map { index in
                    ...
                    return BmuxSurfaceTabBarButton.actionReference(
                        "bmux-ui-test-action-\(index)",
                        title: actionTitle,
    ...
    private func bonsplitTabDragUITestDataPath() -> String? {
        let env = ProcessInfo.processInfo.environment
        guard env["BMUX_UI_TEST_BONSPLIT_TAB_DRAG_SETUP"] == "1",
              let path = env["BMUX_UI_TEST_BONSPLIT_TAB_DRAG_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
    ...
    private func isGotoSplitUITestRecordingEnabled() -> Bool {
        let env = ProcessInfo.processInfo.environment
        return env["BMUX_UI_TEST_GOTO_SPLIT_SETUP"] == "1" || env["BMUX_UI_TEST_GOTO_SPLIT_RECORD_ONLY"] == "1"
    }

    private func gotoSplitUITestDataPath() -> String? {
        guard isGotoSplitUITestRecordingEnabled() else { return nil }
        let env = ProcessInfo.processInfo.environment
        guard let path = env["BMUX_UI_TEST_GOTO_SPLIT_PATH"], !path.isEmpty else { return nil }
        return path
    }
    ...
    private func gotoSplitFindStateSnapshot(for workspace: Workspace) -> [String: String] {
        ...
        updates["firstResponderTerminalPanelId"] =
            bmuxOwningGhosttyView(for: currentResponder)?.terminalSurface?.id.uuidString ?? ""

        updates.merge(bmuxFindResponderSnapshot()) { _, new in new }
        return updates
    }
    ...
    private func focusWebViewForGotoSplitUITest(tab: Workspace, browserPanelId: UUID) {
        ...
        func recordFocusedState() {
            ...
            if ProcessInfo.processInfo.environment["BMUX_UI_TEST_GOTO_SPLIT_INPUT_SETUP"] == "1" {
        ...
        (() => {
          const snapshot = () => {
            ...
            return {
              ...
              trackerInstalled: window.__bmuxAddressBarFocusTrackerInstalled === true,
              trackedStateId:
                window.__bmuxAddressBarFocusState &&
                typeof window.__bmuxAddressBarFocusState.id === "string"
                  ? window.__bmuxAddressBarFocusState.id
                  : "",
          ...
          const seed = () => {
            ...
            let container = document.getElementById("bmux-ui-test-focus-container");
            if (!container || !container.tagName || container.tagName.toLowerCase() !== "div") {
              container = document.createElement("div");
              container.id = "bmux-ui-test-focus-container";
            ...
            const input = ensureInput("bmux-ui-test-focus-input", "bmux-ui-focus-primary");
            const secondaryInput = ensureInput("bmux-ui-test-focus-input-secondary", "bmux-ui-focus-secondary");
            ...
            let trackedFocusId = input.getAttribute("data-bmux-addressbar-focus-id");
            if (!trackedFocusId) {
              trackedFocusId = "bmux-ui-test-focus-input-tracked";
              input.setAttribute("data-bmux-addressbar-focus-id", trackedFocusId);
            }
            ...
            if (
              !window.__bmuxAddressBarFocusState ||
              typeof window.__bmuxAddressBarFocusState.id !== "string" ||
              window.__bmuxAddressBarFocusState.id !== trackedFocusId
            ) {
              window.__bmuxAddressBarFocusState = { id: trackedFocusId, selectionStart, selectionEnd };
            }
          ...
          const ready = () =>
            window.__bmuxAddressBarFocusTrackerInstalled === true &&
            ...
            try {
              ...
              if (!active) {
                return {
                  ...
                  editable: "false",
                  trackedFocusStateId: "",
                  focusTrackerInstalled: window.__bmuxAddressBarFocusTrackerInstalled === true ? "true" : "false"
                };
              }
              ...
              return {
                ...
                focusTrackerInstalled:
                  window.__bmuxAddressBarFocusTrackerInstalled === true ? "true" : "false"
              };
            } catch (_) {
              return {
                id: "",
    ...
    private func setupMultiWindowNotificationsUITestIfNeeded() {
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["BMUX_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] == "1" else { return }
        guard let path = env["BMUX_UI_TEST_MULTI_WINDOW_NOTIF_PATH"], !path.isEmpty else { return }
    ...
    private func prepareMultiWindowNotificationSourceTerminalIfNeeded(
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["BMUX_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] == "1" else { return }
    ...
    private func runMultiWindowWindowRouteCLIIfNeeded(
        ...
        let env = ProcessInfo.processInfo.environment
        guard env["BMUX_UI_TEST_WINDOW_ROUTE_CLI"] == "1" else { return }
        let currentStatus = loadMultiWindowNotificationTestData(at: path)["windowRouteStatus"] ?? ""
        guard currentStatus.isEmpty else { return }

        let title = env["BMUX_UI_TEST_WINDOW_ROUTE_CLI_TITLE"] ?? "window-route-\(UUID().uuidString.prefix(8))"
        writeMultiWindowNotificationTestData([
        ...
        guard let cliURL = Bundle.main.resourceURL?.appendingPathComponent("bin/bmux"),
              FileManager.default.isExecutableFile(atPath: cliURL.path) else {
            writeMultiWindowNotificationTestData([
                "windowRouteStatus": "0",
        ...
        let processEnv = env.merging([
            "BMUXTERM_CLI_RESPONSE_TIMEOUT_SEC": "6",
        ]) { _, new in new }
    ...
    private func publishMultiWindowNotificationSocketStateIfNeeded(
        ...
        guard let config = socketListenerConfigurationIfEnabled() else {
            writeMultiWindowNotificationTestData([
                "socketExpectedPath": env["BMUX_SOCKET_PATH"] ?? "",
                "socketMode": "off",
                "socketReady": "0",
                "socketPingResponse": "",
                "socketIsRunning": "0",
    ...
    // Satisfies BmuxAppKitSupportUI's WindowDecorating seam (see extension below).
    ...
    /// `BmuxNotifications`, reached through the `FocusedNotificationResolving`
    ...
    static func setWindowFirstResponderGuardTesting(currentEvent: NSEvent?, hitView: NSView?) {
        bmuxFirstResponderGuardCurrentEventOverride = currentEvent
        bmuxFirstResponderGuardHitViewOverride = hitView
    }

    static func clearWindowFirstResponderGuardTesting() {
        bmuxFirstResponderGuardCurrentEventOverride = nil
        bmuxFirstResponderGuardHitViewOverride = nil
    }
#endif
    ...
    private func installShortcutMonitor() {
        ...
        shortcutMonitor = NSEvent.addLocalMonitorForEvents(
            ...
            if event.type == .keyDown {
                ...
                BmuxTypingTiming.logEventDelay(path: "appMonitor", event: event)
                let shortcutMonitorTraceEnabled =
                    ProcessInfo.processInfo.environment["BMUX_SHORTCUT_MONITOR_TRACE"] == "1"
                    || UserDefaults.standard.bool(forKey: "bmuxShortcutMonitorTrace")
                ...
                let shortcutTimingStart = BmuxTypingTiming.start()
#endif
                let shortcutStart = ProcessInfo.processInfo.systemUptime
                let handledByShortcut = bmuxCloseFocusedTerminalFindForEscape(event: event, appDelegate: self)
...
#if DEBUG
                    bmuxDebugLog("  → consumed by handleCustomShortcut")
#endif
    ...
    private func currentConfiguredShortcutChordActions() -> [KeyboardShortcutSettings.Action] {
        KeyboardShortcutSettings.Action.allCases.filter { action in
            ...
            // bmux.json entry somehow stores one as a chord, arming the prefix
...
#if DEBUG
        bmuxDebugLog("reload.config.request source=\(source) soft=\(soft)")
#endif
    ...
    var reloadableConfigStores: [any BmuxConfigStoreReloading] {
        mainWindowContexts.values.compactMap { $0.bmuxConfigStore }
    }
    ...
    private func handleCustomShortcut(event: NSEvent) -> Bool {
        ...
        // the `BmuxSettingsUI` package recorder, which publishes its own armed
...
#if DEBUG
            bmuxDebugLog("browser.focusMode.shortcutMonitor.bypass \(debugShortcutRouteSnapshot(event: event))")
#endif
            return false
        }
...
#if DEBUG
                bmuxDebugLog("shortcut.escape paletteDismiss consumed=1 target={\(debugWindowToken(paletteWindow))}")
#endif
                return true
            }
        ...
        if browserAddressBarFocusedPanelId != nil,
           bmuxOwningGhosttyView(for: shortcutRoutingKeyWindow?.firstResponder) != nil {
#if DEBUG
        ...
        if !normalizedFlags.contains(.command),
           let ghosttyView = bmuxOwningGhosttyView(for: shortcutRoutingKeyWindow?.firstResponder),
...
#if DEBUG
            bmuxDebugLog("handleCustomShortcut: unresolved event window context; bypassing app shortcut handling")
#endif
            return false
        }
        ...
        if bmuxCloseFocusedTerminalFindForEscape(event: event, appDelegate: self) { return true }
        if matchConfiguredShortcut(event: event, action: .find) {
            let shortcutWindow = resolvedShortcutEventWindow(event)
            bmuxRememberFindSelectionBeforePanelFocusMove(tabManager: tabManager, window: shortcutWindow ?? shortcutRoutingKeyWindow); return performFindShortcutInActiveMainWindow(preferredWindow: shortcutWindow)
        }
        ...
        if isControlD {
            ...
            let frType = shortcutRoutingKeyWindow?.firstResponder.map { String(describing: type(of: $0)) } ?? "nil"
            bmuxDebugLog("shortcut.ctrlD stage=preReconcile selected=\(selected) focused=\(focused) fr=\(frType)")
#endif
            ...
            bmuxDebugLog("shortcut.ctrlD stage=postReconcile fr=\(frAfterType)")
        ...
        let configuredBmuxShortcutContext = preferredMainWindowContextForShortcutRouting(event: event)
        let configuredBmuxShortcutActions = configuredBmuxShortcutActions(for: configuredBmuxShortcutContext)

        if activeConfiguredShortcutChordPrefixForCurrentEvent == nil,
           armConfiguredShortcutChordIfNeeded(
               event: event,
               actions: [],
               shortcuts: configuredBmuxShortcutActions.compactMap(\.shortcut)
           ) {
            return true
        }
        ...
        if handleConfiguredBmuxShortcut(
            event: event,
            actions: configuredBmuxShortcutActions,
            context: configuredBmuxShortcutContext
        ) {
            return true
        }
...
#if DEBUG
            bmuxDebugLog("shortcut.action name=newWorkspace \(debugShortcutRouteSnapshot(event: event))")
#endif
...
#if DEBUG
            bmuxDebugLog("shortcut.action name=newBrowserWorkspace \(debugShortcutRouteSnapshot(event: event))")
#endif
        ...
        if matchConfiguredShortcut(event: event, action: .closeOtherTabsInPane) {
            if let targetWindow = event.window ?? shortcutRoutingActiveWindow,
               targetWindow.identifier?.rawValue == "bmux.settings" {
...
#if DEBUG
                let route = targetWindow.identifier?.rawValue == "bmux.browser-popup" ? "browserPopup" : "auxWindow"
                bmuxDebugLog("shortcut.closeTab route=\(route)")
#endif
                targetWindow.performClose(nil)
                return true
            } else {
...
#if DEBUG
                    bmuxDebugLog("shortcut.closeTab route=noManager")
#endif
                    return false
                }
            }
        ...
        if matchConfiguredDirectionalShortcut(
            ...
            bmuxRememberFindSelectionBeforePanelFocusMove(tabManager: routedTabs, window: shortcutRoutingKeyWindow)
...
#if DEBUG
            bmuxDebugLog("shortcut.action name=splitRight \(debugShortcutRouteSnapshot(event: event))")
#endif
...
#if DEBUG
            bmuxDebugLog("shortcut.action name=splitDown \(debugShortcutRouteSnapshot(event: event))")
#endif
...
#if DEBUG
            bmuxDebugLog("shortcut.action name=splitBrowserRight \(debugShortcutRouteSnapshot(event: event))")
#endif
...
#if DEBUG
            bmuxDebugLog("shortcut.action name=splitBrowserDown \(debugShortcutRouteSnapshot(event: event))")
#endif
    ...
    func openSidebarExtensionBrowser(from anchorView: NSView?, title: String) -> UUID? {
        ...
        guard BmuxExtensionSidebarSelection.isEnabled else { return nil }
    ...
    func clearBrowserAddressBarFocus(panelId: UUID, reason: String) {
        ...
        stopBrowserOmnibarSelectionRepeat()
#if DEBUG
        bmuxDebugLog("addressBar CLEAR panelId=\(panelId.uuidString.prefix(8)) reason=\(reason)")
#endif
    }
    ...
    func performSplitShortcut(direction: SplitDirection, preferredWindow: NSWindow? = nil) -> Bool {
        ...
        if let browser = tabManager?.focusedBrowserPanel {
            ...
            bmuxDebugLog("split.shortcut dir=\(directionLabel) pre panel=\(browser.id.uuidString.prefix(5)) \(browser.debugDeveloperToolsStateSummary()) webWin=\(webWindow) webSuper=\(webSuperview) \(splitContext)")
        } else {
            bmuxDebugLog("split.shortcut dir=\(directionLabel) pre panel=nil \(splitContext)")
        }
        #endif
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) { [weak self] in
            ...
            if let browser = self?.tabManager?.focusedBrowserPanel {
                let webWindow = browser.webView.window?.windowNumber ?? -1
                let webSuperview = browser.webView.superview.map { String(describing: Unmanaged.passUnretained($0).toOpaque()) } ?? "nil"
                bmuxDebugLog("split.shortcut dir=\(directionLabel) post panel=\(browser.id.uuidString.prefix(5)) \(browser.debugDeveloperToolsStateSummary()) webWin=\(webWindow) webSuper=\(webSuperview) \(splitContext)")
            } else {
                bmuxDebugLog("split.shortcut dir=\(directionLabel) post panel=nil \(splitContext)")
            }
        }
    ...
    func handleBrowserPopupCloseShortcutKeyEquivalent(event: NSEvent, popupWindow: NSWindow) -> Bool {
        ...
        if matchConfiguredShortcut(event: event, action: .closeTab) {
#if DEBUG
            bmuxDebugLog("popup.panel.closeShortcut close")
#endif
...
#if DEBUG
            bmuxDebugLog("popup.panel.closeShortcut armChord")
#endif
            return true
        }
        return false
    ...
    func configuredBmuxShortcutActions(
        for context: MainWindowContext?
    ) -> [BmuxResolvedConfigAction] {
        context?.bmuxConfigStore?.shortcutActions() ?? []
    }

    private func handleConfiguredBmuxShortcut(
        event: NSEvent,
        actions: [BmuxResolvedConfigAction],
        ...
        for action in actions {
            ...
            return executeConfiguredBmuxActionShortcut(
                action,
                event: event,
                context: context
            )
    ...
    private func executeConfiguredBmuxActionShortcut(
    ...
    func runWorkspaceGroupConfiguredAction(
        ...
        let groupPlacement: WorkspaceGroupNewPlacement = {
            ...
            let configured = context.bmuxConfigStore?.resolveWorkspaceGroupConfig(forCwd: cwd)?.newWorkspacePlacement
            return configured
                ?? UserDefaultsSettingsClient(defaults: .standard).value(for: SettingCatalog().workspaceGroups.newWorkspacePlacement)
        }()
        ...
        let onExecuted: () -> Void = { [weak tabManager, groupId, beforeIds, previousSelectedId, anchorId, groupPlacement, action] in
            ...
            // cloudVM launches a `bmux vm base open` process and returns before the
        ...
        let didRun = executeConfiguredBmuxAction(
            action,
            context: context,
        ...
        // executeConfiguredBmuxAction returns false when the action couldn't
    ...
    func executeConfiguredBmuxAction(
        ...
        case .builtIn(let builtIn):
            ...
            case .cloudVM:
                let didStart = performCloudVMAction(
                    tabManager: context.tabManager,
                    preferredWindow: resolvedWindow(for: context) ?? preferredWindow,
                    debugSource: "configured.bmux.cloudvm",
        ...
        case .command, .agent, .workspaceCommand, .workspace:
            ...
            return BmuxConfigExecutor.execute(
                ...
                commandSourcePaths: bmuxConfigStore.commandSourcePaths,
                tabManager: context.tabManager,
                baseCwd: baseCwd,
                globalConfigPath: bmuxConfigStore.globalConfigPath,
    ...
    func shouldSuppressStaleBmuxMenuShortcut(event: NSEvent) -> Bool {
    ...
    private func installMainWindowKeyObserver() {
        ...
        windowKeyObservers.append(center.addObserver(forName: NSWindow.didBecomeKeyNotification, object: nil, queue: .main) { [weak self] note in
            MainActor.assumeIsolated {
                self?.handleBmuxWindowBecameKey(note)
            }
        })
        windowKeyObservers.append(center.addObserver(forName: NSWindow.didResignKeyNotification, object: nil, queue: .main) { [weak self] note in
            MainActor.assumeIsolated {
                self?.handleBmuxWindowResignedKey(note)
            }
        })
    }
    ...
    func browserFindBarIsVisible(for webView: BmuxWebView) -> Bool {
        browserPanelOwning(webView)?.searchState != nil
    }

    func isBrowserFocusModeActive(for webView: BmuxWebView) -> Bool {
    ...
    private func browserFocusModePanelForShortcutEvent(_ event: NSEvent) -> BrowserPanel? {
        ...
        // page can't run the double-Escape exit anyway and bmux shortcuts must work).
    ...
    func browserFocusModeContextMenuState(for webView: BmuxWebView) -> (isActive: Bool, canToggle: Bool) {
    ...
    @discardableResult
    func toggleBrowserFocusModeFromContextMenu(for webView: BmuxWebView) -> Bool {
    ...
    private func shouldLetFocusedBrowserOwnFindShortcut(_ event: NSEvent) -> Bool {
        let shortcutWindow = resolvedShortcutEventWindow(event) ?? shortcutRoutingActiveWindow
        let shortcutResponder = shortcutWindow?.firstResponder
        let owningWebView = tabManager?.focusedBrowserPanel?.webView as? BmuxWebView
    ...
    private func browserPanelOwning(_ webView: BmuxWebView) -> BrowserPanel? {
    ...
    private func browserPanelOwning(_ webView: BmuxWebView, in manager: TabManager) -> BrowserPanel? {
        for workspace in manager.tabs {
    ...
    private func unregisterMainWindow(_ window: NSWindow) {
        ...
        publishBmuxWindowLifecycle(name: "window.closed", windowId: removed.windowId, origin: "appkit_close")
...
#if DEBUG
private var bmuxFirstResponderGuardCurrentEventOverride: NSEvent?
private var bmuxFirstResponderGuardHitViewOverride: NSView?
#endif
private var bmuxFirstResponderGuardCurrentEventContext: NSEvent?
private var bmuxFirstResponderGuardHitViewContext: NSView?
private var bmuxFirstResponderGuardContextWindowNumber: Int?
private var bmuxFieldEditorOwningWebViewAssociationKey: UInt8 = 0

private final class BmuxFieldEditorOwningWebViewBox: NSObject {
    weak var webView: BmuxWebView?

    init(webView: BmuxWebView?) {
        self.webView = webView
    }
}

private extension NSApplication {
    @objc func bmux_accessibilityAttributeValue(_ attribute: NSAccessibility.Attribute) -> Any? {
        ...
        return bmux_accessibilityAttributeValue(attribute)
    }

    @objc func bmux_applicationSendEvent(_ event: NSEvent) {
#if DEBUG
        let typingTimingStart = event.type == .keyDown ? BmuxTypingTiming.start() : nil
        let phaseTotalStart = event.type == .keyDown ? ProcessInfo.processInfo.systemUptime : 0
        if event.type == .keyDown {
            BmuxTypingTiming.logEventDelay(path: "app.sendEvent", event: event)
        }
        defer {
            if event.type == .keyDown {
                let totalMs = (ProcessInfo.processInfo.systemUptime - phaseTotalStart) * 1000.0
        ...
        if AppDelegate.shared?.shouldSuppressStaleBmuxMenuShortcut(event: event) == true {
            if AppDelegate.shared?.handleFocusedFileExplorerOpenSelectionShortcut(
                event,
                preferredWindow: event.window ?? keyWindow ?? mainWindow
            ) == true {
#if DEBUG
                bmuxDebugLog("app.sendEvent routed file explorer shortcut before stale bmux menu shortcut")
#endif
                return
            }
            if AppDelegate.shared?.handleConfiguredShortcutKeyEquivalent(event) == true {
#if DEBUG
                bmuxDebugLog("app.sendEvent routed configured shortcut before stale bmux menu shortcut")
#endif
                return
            }
...
#if DEBUG
                bmuxDebugLog("app.sendEvent suppressed stale bmux menu shortcut and forwarded to terminal")
#endif
            } else {
#if DEBUG
                bmuxDebugLog("app.sendEvent suppressed stale bmux menu shortcut")
#endif
            }
            return
        }
        bmux_applicationSendEvent(event)
    }

    @objc func bmux_sendAction(_ action: Selector, to target: Any?, from sender: Any?) -> Bool {
        ...
        return bmux_sendAction(action, to: target, from: sender)
    }
}
...
private extension AppDelegate {
    ...
    @objc func handleThemesReloadNotification(_ notification: Notification) {
        ...
        let source = GhosttySurfaceConfigurationRefresh.bmuxThemeReloadSource(
            phase: notification.userInfo?["phase"] as? String
        )
        DispatchQueue.main.async {
            self.reloadGhosttyConfigurationForBmuxThemeSource(source)
        }
    }

    func reloadGhosttyConfigurationForBmuxThemeSource(_ source: String) {
        if GhosttySurfaceConfigurationRefresh.shouldDebounceBmuxThemeReload(source: source) {
            bmuxThemePreviewReloadGeneration += 1
            let generation = bmuxThemePreviewReloadGeneration
            bmuxThemePreviewReloadWorkItem?.cancel()

            let workItem = DispatchWorkItem { [weak self] in
                guard let self,
                      self.bmuxThemePreviewReloadGeneration == generation else { return }
                self.bmuxThemePreviewReloadWorkItem = nil
                self.reloadConfiguration(source: source)
            }
            bmuxThemePreviewReloadWorkItem = workItem
            DispatchQueue.main.asyncAfter(
                deadline: .now() + .milliseconds(
                    GhosttySurfaceConfigurationRefresh.bmuxThemePreviewReloadDebounceMilliseconds
                ),
                execute: workItem
            )
            return
        ...
        bmuxThemePreviewReloadGeneration += 1
        bmuxThemePreviewReloadWorkItem?.cancel()
        bmuxThemePreviewReloadWorkItem = nil
...
private extension NSWindow {
    static func bmuxCommandPaletteOwnsFieldEditor(_ textView: NSTextView?, in window: NSWindow) -> Bool {
        guard let textView,
              textView.isFieldEditor,
        ...
        if let ownerView = bmuxFieldEditorOwnerView(textView) {
            guard let container = bmuxCommandPaletteOverlayAncestor(of: ownerView) else {
                return false
            }
            return bmuxCommandPaletteOverlayIsPresented(container)
        }

        guard let container = bmuxCommandPaletteOverlayContainer(in: window) else {
            return false
        }
    ...
    private static func bmuxCommandPaletteOverlayAncestor(of view: NSView) -> NSView? {
        var current: NSView? = view
        while let candidate = current {
            if candidate.identifier == commandPaletteOverlayContainerIdentifier {
                return candidate
    ...
    private static func bmuxCommandPaletteOverlayIsPresented(_ container: NSView) -> Bool {
        !container.isHidden && container.alphaValue > 0.001
    }

    private static func bmuxCommandPaletteOverlayContainer(in window: NSWindow) -> NSView? {
    ...
    @objc func bmux_makeFirstResponder(_ responder: NSResponder?) -> Bool {
        ...
        let currentEvent = Self.bmuxCurrentEvent(for: self)
        let responderWebView = responder.flatMap {
            Self.bmuxOwningWebView(for: $0, in: self, event: currentEvent)
        }
        ...
        if let request = AppDelegate.shared?.terminalKeyboardFocusRequest(for: responder),
           Self.bmuxShouldAllowPointerInitiatedTerminalFocus(
               window: self,
               request: request,
               event: currentEvent
           ) {
        ...
        if let responder,
           ...
           !webView.allowsFirstResponderAcquisitionEffective {
            let pointerInitiatedFocus = Self.bmuxShouldAllowPointerInitiatedWebViewFocus(
                window: self,
                webView: webView,
                event: currentEvent
            )
        ...
        if pointerInitiatedWebFocus, let webView = responderWebView {
            // `NSWindow.makeFirstResponder` may run before `BmuxWebView.mouseDown(with:)`.
            // Preserve pointer intent during this synchronous responder change.
            result = webView.withPointerFocusAllowance {
                bmux_makeFirstResponder(responder)
            }
        } else {
            result = bmux_makeFirstResponder(responder)
        }
        if result {
            ...
            if let fieldEditor = responder as? NSTextView, fieldEditor.isFieldEditor {
                Self.bmuxTrackFieldEditor(fieldEditor, owningWebView: responderWebView)
    ...
    @objc func bmux_sendEvent(_ event: NSEvent) {
#if DEBUG
        let typingTimingStart = event.type == .keyDown ? BmuxTypingTiming.start() : nil
        let phaseTotalStart = event.type == .keyDown ? ProcessInfo.processInfo.systemUptime : 0
        ...
        let typingTimingExtra: String? = {
            ...
            let responderWebView = self.firstResponder.flatMap {
                Self.bmuxOwningWebView(for: $0, in: self, event: event)
            }
        ...
        if event.type == .keyDown {
            BmuxTypingTiming.logEventDelay(path: "window.sendEvent", event: event)
        }
#endif
        // recordTypingActivity must run in all builds so runSessionAutosaveTick
        // can honor the typing quiet period in release.
        if event.type == .keyDown, let app = AppDelegate.shared, bmuxCloseFocusedTerminalFindForEscape(event: event, appDelegate: app) { return }
...
#endif
        let previousContextEvent = bmuxFirstResponderGuardCurrentEventContext
        let previousContextHitView = bmuxFirstResponderGuardHitViewContext
        let previousContextWindowNumber = bmuxFirstResponderGuardContextWindowNumber
        bmuxFirstResponderGuardCurrentEventContext = event
        bmuxFirstResponderGuardHitViewContext = Self.bmuxHitViewForFirstResponderGuard(in: self, event: event)
        bmuxFirstResponderGuardContextWindowNumber = self.windowNumber
#if DEBUG
        if event.type == .keyDown {
...
#endif
        defer {
            bmuxFirstResponderGuardCurrentEventContext = previousContextEvent
            bmuxFirstResponderGuardHitViewContext = previousContextHitView
            bmuxFirstResponderGuardContextWindowNumber = previousContextWindowNumber
        }
        ...
        let hitView = WindowInputRoutingContext(event: event).allowsPortalPointerHitTesting
            ? Self.bmuxHitViewForEventDispatch(in: self, event: event)
            : nil
#endif
        defer {
            ...
            if shouldFinishSuppression {
                bmuxDebugLog("window.sendEvent.\(reasonDescription) finish nowMovable=\(isMovable)")
            } else {
                bmuxDebugLog("window.sendEvent.\(reasonDescription) keepSuppressed nowMovable=\(isMovable)")
            }
            #endif
        }
        ...
        bmuxDebugLog("window.sendEvent.\(reasonDescription) suppress=1 hit=\(hitDesc) movable=\(isMovable) depth=\(depth)")
        #endif
    ...
    @objc func bmux_performKeyEquivalent(with event: NSEvent) -> Bool {
#if DEBUG
        ...
        bmuxDebugLog("performKeyEquiv: \(Self.keyDescription(event)) fr=\(frType)")
#endif
        ...
        let firstResponderGhosttyView = bmuxOwningGhosttyView(for: self.firstResponder)
        ...
        let firstResponderIsCommandPaletteFieldEditor = Self.bmuxCommandPaletteOwnsFieldEditor(
        ...
        let browserWebKitKeyDownReentry = firstResponderWebView != nil && bmuxBrowserWebKitKeyDownDispatchIsActive()
        if AppDelegate.shared?.shouldBypassPrintableOptionTextForShortcutRouting(event: event) == true {
            ...
            if let textInputTarget, textInputTarget !== self {
                if bmuxForceDispatchKeyDownOnce(event, to: textInputTarget, reason: "printable Option text") {
                    return true
                }
        ...
        if bmuxRouteUndoRedoCommandEquivalentAwayFromAppKit(event, terminalView: firstResponderGhosttyView, webView: firstResponderWebView, browserWebKitKeyDownReentry: browserWebKitKeyDownReentry) { return true }
...
#if DEBUG
                bmuxDebugLog("  → consumed by file explorer shortcut before stale bmux menu shortcut")
#endif
                return true
            }
            if AppDelegate.shared?.handleConfiguredShortcutKeyEquivalent(event) == true {
#if DEBUG
                bmuxDebugLog("  → consumed by configured shortcut before stale bmux menu shortcut")
#endif
                return true
            }
            if let firstResponderGhosttyView,
               bmuxForceDispatchKeyDownOnce(
                   event,
                   to: firstResponderGhosttyView,
                   reason: "stale bmux menu shortcut terminal bypass"
               ) {
#if DEBUG
                bmuxDebugLog("  → terminal received command equivalent bypassing stale bmux menu shortcut")
#endif
                return true
            }
#if DEBUG
            bmuxDebugLog("  → suppressed stale bmux menu shortcut")
#endif
            return false
        }
        if let ghosttyView = firstResponderGhosttyView {
            ...
            if !flags.contains(.command) {
                ...
                if shouldDispatchTerminalArrowViaFirstResponderKeyDown(
                    ...
                    if bmuxForceDispatchKeyDownOnce(event, to: ghosttyView, reason: "terminal arrow") {
                        return true
                    }
                    return false
                }
...
#if DEBUG
                bmuxDebugLog("  → ghostty direct: \(result)")
#endif
                return result
            }
            ...
            if shouldRouteTerminalFontZoomShortcutToGhostty(
                ...
                if bmuxForceDispatchKeyDownOnce(event, to: ghosttyView, reason: "terminal font zoom") {
#if DEBUG
                    bmuxDebugLog("zoom.shortcut stage=window.ghosttyKeyDownDirect event=\(Self.keyDescription(event)) handled=1")
#endif
                    return true
                }
                return false
        ...
        if shouldDispatchCommandPaletteHorizontalArrowViaFirstResponderKeyDown(
            ...
            guard let target = self.firstResponder,
                  bmuxForceDispatchKeyDownOnce(event, to: target, reason: "command palette arrow")
            else {
                return false
            }
            return true
        ...
        if shouldDispatchBrowserOmnibarArrowViaFirstResponderKeyDown(
            ...
            if bmuxForceDispatchKeyDownOnce(
                event,
                to: target,
            ...
            return bmux_performKeyEquivalent(with: event)
        }
        ...
        if shouldDispatchTextBoxInputArrowViaFirstResponderKeyDown(
            ...
            guard let target = self.firstResponder,
                  bmuxForceDispatchKeyDownOnce(event, to: target, reason: "text-box input arrow")
            else {
                return false
            }
            return true
        ...
        if shouldDispatchTextBoxInputControlNavViaFirstResponderKeyDown(
            ...
            guard let target = self.firstResponder,
                  bmuxForceDispatchKeyDownOnce(event, to: target, reason: "text-box input control nav")
            else {
                return false
            }
            return true
        ...
        // arrow navigation works as in any text editor (manaflow-ai/bmux#5227).
        if shouldDispatchEditableTextViewArrowViaFirstResponderKeyDown(
            ...
            guard let target = self.firstResponder,
                  bmuxForceDispatchKeyDownOnce(event, to: target, reason: "editable text view arrow")
            else {
                return false
            }
            return true
        ...
        if shouldDispatchBrowserReturnViaFirstResponderKeyDown(
            ...
            if browserWebKitKeyDownReentry { return false }
            guard let target = self.firstResponder else { return false }
            if bmuxForceDispatchKeyDownOnce(event, to: target, reason: "browser Return/Enter") {
                return true
            }
        ...
        if shouldDispatchBrowserArrowViaFirstResponderKeyDown(
...
#if DEBUG
                        bmuxDebugLog("  → browser arrow omnibar restore rejected")
#endif
                        return false
                    }
...
#if DEBUG
                    bmuxDebugLog("  → browser arrow omnibar restore did not become first responder")
#endif
                    return false
                }
                if bmuxForceDispatchKeyDownOnce(
            ...
            guard let target = self.firstResponder else { return false }
            if bmuxForceDispatchKeyDownOnce(event, to: target, reason: "browser arrow") {
                return true
            }
            return bmux_performKeyEquivalent(with: event)
        }
...
#if DEBUG
            bmuxDebugLog("  → browser focus mode routed before bmux/menu fallback handled=\(handled ? 1 : 0)")
#endif
            return handled
        }
        ...
        if let firstResponderWebView,
           shouldRouteBrowserDocumentEditingCommandEquivalentThroughWebContentFirst(
            ...
            // `BmuxWebView.performKeyEquivalent` also runs the main-menu fallback
...
#if DEBUG
            if result {
                bmuxDebugLog("  → browser find command resolved before window menu path")
            } else {
                bmuxDebugLog("  → browser find command preflight left unclaimed; suppressing replay")
            }
#endif
...
#if DEBUG
            bmuxDebugLog("  → consumed by handleBrowserSurfaceKeyEquivalent")
#endif
            return true
        }
...
#if DEBUG
                bmuxDebugLog("  → mainMenu miss; ghostty command path: \(consumedByGhostty)")
#endif
...
#if DEBUG
                bmuxDebugLog("  → consumed by mainMenu (bypassed SwiftUI)")
#endif
                return true
            }
        }

        let result = bmux_performKeyEquivalent(with: event)
#if DEBUG
        if result { bmuxDebugLog("  → consumed by original performKeyEquivalent") }
#endif
        return result
    }

    private static func bmuxOwningWebView(for responder: NSResponder) -> BmuxWebView? {
        if let webView = responder as? BmuxWebView {
            return webView
        }

        if let view = responder as? NSView,
           let webView = bmuxOwningWebView(for: view) {
            return webView
        }
        ...
        while let next = current {
            if let webView = next as? BmuxWebView {
                return webView
            }
    ...
    private static func bmuxOwningWebView(
        ...
        if let webView = bmuxOwningWebView(for: responder) {
            return webView
        }
        ...
        if let event,
           let hitWebView = bmuxPointerHitWebView(in: window, event: event) {
            bmuxTrackFieldEditor(textView, owningWebView: hitWebView)
            return hitWebView
        }

        return bmuxTrackedOwningWebView(for: textView)
    }

    private static func bmuxOwningWebView(for view: NSView) -> BmuxWebView? {
        if let webView = view as? BmuxWebView {
            return webView
        }

        var current: NSView? = view.superview
        while let candidate = current {
            if let webView = candidate as? BmuxWebView {
                return webView
            }
            if String(describing: type(of: candidate)).contains("WindowBrowserSlotView"),
               let portalWebView = bmuxUniqueBrowserWebView(in: candidate) {
                ...
                if bmuxAllowsPortalSlotTextEntryFocus(view) {
                    return nil
                }
                return portalWebView
            }
    ...
    private static func bmuxAllowsPortalSlotTextEntryFocus(_ view: NSView) -> Bool {
        var current: NSView? = view
        while let candidate = current {
    ...
    private static func bmuxUniqueBrowserWebView(in root: NSView) -> BmuxWebView? {
        var stack: [NSView] = [root]
        var found: BmuxWebView?
        while let current = stack.popLast() {
            if let webView = current as? BmuxWebView {
                if found == nil {
                    found = webView
    ...
    private static func bmuxCurrentEvent(for window: NSWindow) -> NSEvent? {
#if DEBUG
        if let override = bmuxFirstResponderGuardCurrentEventOverride {
            return override
        }
#endif
        if bmuxFirstResponderGuardContextWindowNumber == window.windowNumber {
            return bmuxFirstResponderGuardCurrentEventContext
        }
        return NSApp.currentEvent
    }

    private static func bmuxHitViewInThemeFrame(in window: NSWindow, event: NSEvent) -> NSView? {
    ...
    private static func bmuxHitViewInContentView(in window: NSWindow, event: NSEvent) -> NSView? {
    ...
    private static func bmuxTopHitViewForEvent(in window: NSWindow, event: NSEvent) -> NSView? {
        if let hitInThemeFrame = bmuxHitViewInThemeFrame(in: window, event: event) {
            return hitInThemeFrame
        }
        return bmuxHitViewInContentView(in: window, event: event)
    }

    private static func bmuxHitViewForEventDispatch(in window: NSWindow, event: NSEvent) -> NSView? {
        ...
        return bmuxTopHitViewForEvent(in: window, event: event)
    }

    private static func bmuxHitViewForFirstResponderGuard(in window: NSWindow, event: NSEvent) -> NSView? {
        guard WindowInputRoutingContext(event: event).allowsFirstResponderHitTesting else { return nil }
        return bmuxHitViewForEventDispatch(in: window, event: event)
    }

    private static func bmuxHitViewForCurrentEvent(in window: NSWindow, event: NSEvent) -> NSView? {
#if DEBUG
        if let override = bmuxFirstResponderGuardHitViewOverride {
            return override
        }
#endif
        if bmuxFirstResponderGuardContextWindowNumber == window.windowNumber,
           let contextHitView = bmuxFirstResponderGuardHitViewContext {
            return contextHitView
        }
        return bmuxTopHitViewForEvent(in: window, event: event)
    }

    private static func bmuxTrackFieldEditor(_ fieldEditor: NSTextView, owningWebView webView: BmuxWebView?) {
        if let webView {
            objc_setAssociatedObject(
                fieldEditor,
                &bmuxFieldEditorOwningWebViewAssociationKey,
                BmuxFieldEditorOwningWebViewBox(webView: webView),
    ...
    private static func bmuxTrackedOwningWebView(for fieldEditor: NSTextView) -> BmuxWebView? {
        guard let box = objc_getAssociatedObject(
            fieldEditor,
            &bmuxFieldEditorOwningWebViewAssociationKey
        ) as? BmuxFieldEditorOwningWebViewBox else {
            return nil
        }
        guard let webView = box.webView else {
            bmuxTrackFieldEditor(fieldEditor, owningWebView: nil)
            return nil
        }
        return webView
    }

    private static func bmuxEventAllowsFirstResponderHitTesting(_ event: NSEvent) -> Bool {
        WindowInputRoutingContext(event: event).allowsFirstResponderHitTesting
    }

    private static func bmuxPointerEventTargetsWindow(_ event: NSEvent, _ window: NSWindow) -> Bool {
        if event.windowNumber != 0, event.windowNumber != window.windowNumber {
            return false
        }
        if let eventWindow = event.window, eventWindow !== window {
    ...
    private static func bmuxPointerHitWebView(in window: NSWindow, event: NSEvent) -> BmuxWebView? {
        guard bmuxEventAllowsFirstResponderHitTesting(event) else { return nil }
        guard bmuxPointerEventTargetsWindow(event, window) else { return nil }
        ...
        guard let hitView = bmuxHitViewForCurrentEvent(in: window, event: event) else {
            return nil
        }
        return bmuxOwningWebView(for: hitView)
    }

    private static func bmuxPointerHitGhosttyView(in window: NSWindow, event: NSEvent) -> GhosttyNSView? {
        guard bmuxEventAllowsFirstResponderHitTesting(event) else { return nil }
        guard bmuxPointerEventTargetsWindow(event, window) else { return nil }
        guard let hitView = bmuxHitViewForCurrentEvent(in: window, event: event) else {
            return nil
        }
        return bmuxOwningGhosttyView(for: hitView)
    }

    private static func bmuxShouldAllowPointerInitiatedTerminalFocus(
        window: NSWindow,
        request: AppDelegate.TerminalKeyboardFocusRequest,
        event: NSEvent?
    ) -> Bool {
        guard let event,
              let hitGhosttyView = bmuxPointerHitGhosttyView(in: window, event: event) else {
            return false
        }
        return hitGhosttyView === request.ghosttyView
    }

    private static func bmuxShouldAllowPointerInitiatedWebViewFocus(
        window: NSWindow,
        webView: BmuxWebView,
        event: NSEvent?
    ) -> Bool {
        guard let event,
              let hitWebView = bmuxPointerHitWebView(in: window, event: event) else {
            return false
        }
...
extension AppDelegate {
    ...
    /// the `bmux window display --list` CLI so callers can discover screen names.
    /// Lifted to ``BmuxWindowing/DisplayInfo``; aliased so existing
    /// `AppDelegate.DisplayInfo` references stay source-identical.
    typealias DisplayInfo = BmuxWindowing.DisplayInfo

    /// All currently-connected displays, in `NSScreen.screens` order.
    func availableDisplays() -> [DisplayInfo] {
        let mainID = NSScreen.main?.bmuxDisplayID
        return NSScreen.screens.enumerated().map { index, screen in
            let displayID = screen.bmuxDisplayID
            return DisplayInfo(
...
// MARK: - BmuxAppKitSupportUI seam conformance
