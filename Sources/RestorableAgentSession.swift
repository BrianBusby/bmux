...
nonisolated enum TerminalStartupWorkingDirectoryPrefix {
    static func optionalChangeDirectoryPrefix(for workingDirectory: String?) -> String? {
        ...
        // (bmux spawns via `/usr/bin/login → $SHELL`), which may be fish — fish has no
...
enum AgentResumeCommandBuilder {
    ...
    static func resumeShellCommand(
        ...
        workingDirectory: String?,
        registrationOverride: BmuxVaultAgentRegistration? = nil,
    ...
    private static func shellCommand(
        ...
        launchCommand: AgentLaunchCommandSnapshot?,
        workingDirectory: String?,
        customRegistration: BmuxVaultAgentRegistration?,
        ...
        // executed command routes through bmux's `claude`/`codex` wrapper
    ...
    private static func launchEnvironmentParts(
        ...
        if !preservedClaudeAuthSelectionEnvironmentKeys.isEmpty {
            environmentParts.append("BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1")
            environmentParts.append(
                "BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=\(preservedClaudeAuthSelectionEnvironmentKeys.joined(separator: ","))"
            )
        }
    ...
    private static func resumeArguments(
        ...
        if case .custom = kind {
            ...
            if customRegistration.id == BmuxVaultAgentRegistration.builtInAntigravity.id {
...
struct SessionRestorableAgentSnapshot: Codable, Sendable {
    ...
    var registration: BmuxVaultAgentRegistration? = nil
...
private enum AgentResumeScriptStore {
    private static let directoryName = "bmux-agent-resume"
...
struct RestorableAgentSessionIndex: Sendable {
    ...
    static func load(
        ...
        let registry = BmuxVaultAgentRegistry.load(homeDirectory: homeDirectory, fileManager: fileManager)
        return load(
    ...
    static func load(
        ...
        let hookKinds: [(kind: RestorableAgentKind, registration: BmuxVaultAgentRegistration?)] =
    ...
    /// carries claude's `BMUX_AGENT_LAUNCH_*`) or the hook dispatch shell's own
    ...
    /// The directory bmux must `cd` into to resume or fork this session.
    ///
    ...
    private static func liveScopedProcessID(
        ...
        guard pid > 0,
              ...
              process.matchesBMUXScope(workspaceId: workspaceId, surfaceId: panelId) else {
            return nil
        }
...
private extension BmuxTopProcessArguments {
