...
private var bmuxWindowTerminalPortalKey: UInt8 = 0
private var bmuxWindowTerminalPortalCloseObserverKey: UInt8 = 0
...
#if BMUX_ISSUE_483_PORTAL_RECOVERY
...
#if DEBUG
        bmuxDebugLog("portal.hideEntry hosted=\(portalDebugToken(entry.hostedView)) reason=workspaceUnmount")
#endif
    }
    ...
    private func synchronizeHostedView(withId hostedId: ObjectIdentifier, syncLayout: Bool = true) {
        ...
        guard let anchorView = entry.anchorView, let window else {
            ...
            if !hostedView.isHidden {
                bmuxDebugLog("portal.hidden hosted=\(portalDebugToken(hostedView)) value=1 reason=missingAnchorOrWindow")
            }
#endif
            hostedView.isHidden = true
            if entry.visibleInUI {
...
enum TerminalWindowPortalRegistry {
    ...
    private static func installWindowCloseObserverIfNeeded(for window: NSWindow) {
        guard objc_getAssociatedObject(window, &bmuxWindowTerminalPortalCloseObserverKey) == nil else { return }
        ...
        objc_setAssociatedObject(
            window,
            &bmuxWindowTerminalPortalCloseObserverKey,
            observer,
    ...
    private static func removePortal(windowId: ObjectIdentifier, window: NSWindow?) {
        ...
        guard let window else { return }
        if let observer = objc_getAssociatedObject(window, &bmuxWindowTerminalPortalCloseObserverKey) {
            NotificationCenter.default.removeObserver(observer)
        }
        objc_setAssociatedObject(window, &bmuxWindowTerminalPortalCloseObserverKey, nil, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        objc_setAssociatedObject(window, &bmuxWindowTerminalPortalKey, nil, .OBJC_ASSOCIATION_RETAIN)
    }
    ...
    private static func portal(for window: NSWindow, syncLayout: Bool = true) -> WindowTerminalPortal {
        if let existing = objc_getAssociatedObject(window, &bmuxWindowTerminalPortalKey) as? WindowTerminalPortal {
        ...
        objc_setAssociatedObject(window, &bmuxWindowTerminalPortalKey, portal, .OBJC_ASSOCIATION_RETAIN)
