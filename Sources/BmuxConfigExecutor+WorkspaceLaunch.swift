...
extension BmuxConfigExecutor {
    ...
    static func workspaceShellDisclosure(_ command: BmuxCommandDefinition) -> String {
    ...
    private static var cwdDisclosureFormat: String {
        String(localized: "dialog.bmuxConfig.disclosure.cwd", defaultValue: "cwd: %@")
    }

    private static var cwdCommandDisclosureFormat: String {
        String(localized: "dialog.bmuxConfig.disclosure.cwdCommand", defaultValue: "cwd %1$@: %2$@")
    }
    ...
    private static func firstTerminalSurfaceCwd(_ node: BmuxLayoutNode) -> String? {
        firstTerminalSurface(node)?.cwd
    }

    private static func firstTerminalSurface(_ node: BmuxLayoutNode) -> BmuxSurfaceDefinition? {
        switch node {
        case .pane(let pane):
    ...
    private static func collectSurfaceDisclosures(_ node: BmuxLayoutNode, into lines: inout [String]) {
        switch node {
        case .pane(let pane):
    ...
    static func executeWorkspaceCommand(
        command: BmuxCommandDefinition,
        workspace wsDef: BmuxWorkspaceDefinition,
        ...
        if let existing = tabManager.tabs.first(where: { $0.customTitle == workspaceName }) {
            ...
            case .confirm:
                ...
                alert.messageText = String(
                    localized: "dialog.bmuxConfig.confirmRestart.title",
                    defaultValue: "Workspace Already Exists"
                )
                alert.informativeText = String(
                    localized: "dialog.bmuxConfig.confirmRestart.message",
                    defaultValue: "A workspace with this name already exists. Close it and create a new one?"
                )
                alert.alertStyle = .warning
                alert.addButton(withTitle: String(
                    localized: "dialog.bmuxConfig.confirmRestart.recreate",
                    defaultValue: "Recreate"
                ))
                alert.addButton(withTitle: String(
                    localized: "dialog.bmuxConfig.confirmRestart.cancel",
                    defaultValue: "Cancel"
                ))
        ...
        let resolvedCwd = BmuxConfigStore.resolveCwd(wsDef.cwd, relativeTo: baseCwd)
