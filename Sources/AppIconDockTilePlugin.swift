...
private let bmuxAppIconDidChangeNotification = Notification.Name("com.bmuxterm.appIconDidChange")
private let bmuxAppIconModeKey = "appIconMode"
...
final class BmuxDockTilePlugin: NSObject, NSDockTilePlugIn {
    ...
    private let pluginBundle = Bundle(for: BmuxDockTilePlugin.self)
    ...
    private func setDockTileOnMain(_ dockTile: NSDockTile?) {
        ...
        iconChangeObserver = DistributedNotificationCenter.default().addObserver(
            forName: bmuxAppIconDidChangeNotification,
            object: nil,
            queue: .main
        ) { [weak self] _ in
    ...
    private func updateDockTile(_ dockTile: NSDockTile) {
        ...
        let mode = DockTileAppIconMode(defaultsValue: appDefaults?.string(forKey: bmuxAppIconModeKey))
...
private extension NSDockTile {
    func showDefaultAppIcon() {
        BmuxDockTilePlugin.assertMainQueue()
