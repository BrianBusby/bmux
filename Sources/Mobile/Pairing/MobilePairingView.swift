...
///
/// Walks the user through the two requirements (signed in to bmux, Tailscale
...
struct MobilePairingView: View {
    ...
    /// Where a Mac user goes to get bmux for iPhone while the beta is invite-only.
    private static let iphoneAppURL = URL(string: "https://github.com/manaflow-ai/bmux#founders-edition")!
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(String(localized: "mobile.pairing.window.heading", defaultValue: "Pair your iPhone"))
                .bmuxFont(.title2, weight: .semibold)
            Text(String(localized: "mobile.pairing.window.subheading", defaultValue: "Scan this code with the bmux app on your iPhone to sync your terminal workspaces."))
                .bmuxFont(.callout)
    ...
    private var signInRow: some View {
        requirementRow(
            title: String(localized: "mobile.pairing.req.signIn.title", defaultValue: "Signed in to bmux"),
    ...
    private func requirementRow<Trailing: View>(
        ...
        HStack(alignment: .firstTextBaseline, spacing: 10) {
            VStack(alignment: .leading, spacing: 2) {
                Text(title).bmuxFont(.callout, weight: .medium)
                Text(subtitle)
                    .bmuxFont(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private var signedOut: some View {
        VStack(spacing: 12) {
            ...
            Text(String(localized: "mobile.pairing.signIn.prompt", defaultValue: "Sign in with your bmux account to pair your iPhone."))
                .multilineTextAlignment(.center)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private var steps: some View {
        VStack(alignment: .leading, spacing: 10) {
            step(1, String(localized: "mobile.pairing.step.install", defaultValue: "Install bmux on your iPhone and open it."))
            HStack(spacing: 4) {
                Spacer(minLength: 30)
                Text(String(localized: "mobile.pairing.getApp.prompt", defaultValue: "Don't have it yet?"))
                    ...
                    .foregroundStyle(.secondary)
                Link(
                    String(localized: "mobile.pairing.getApp.link", defaultValue: "Get bmux for iPhone"),
