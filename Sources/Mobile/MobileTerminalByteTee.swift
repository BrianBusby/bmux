...
/// Captures raw PTY-output bytes from every bmux terminal surface and
...
/// callback (bmux fork addition). The callback fires on the IO read
...
final class MobileTerminalByteTee {
    ...
    private let publishQueue = DispatchQueue(
        label: "dev.bmux.mobile.byte-tee.publish",
        qos: .userInitiated
    )
...
/// pointer.
public let bmuxMobileTerminalByteTeeCallback: @convention(c) (
