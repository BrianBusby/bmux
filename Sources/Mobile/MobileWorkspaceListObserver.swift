...
private let mobileWorkspaceObserverLog = Logger(subsystem: "dev.bmux", category: "mobile-workspace-observer")
...
final class MobileWorkspaceListObserver {
    ...
    init(tabManager: TabManager, notificationStore: TerminalNotificationStore? = nil) {
        ...
        #if DEBUG
        bmuxDebugLog("mobile.observer init tabs=\(tabManager.tabs.count)")
        #endif
    ...
    private func attach(to tabManager: TabManager) {
        ...
        tabsCancellable = tabManager.tabsPublisher
            ...
            .sink { [weak self] tabs in
                ...
                #if DEBUG
                bmuxDebugLog("mobile.observer tabs sink fired count=\(tabs.count)")
                #endif
    ...
    private func emitIfNeeded(force: Bool) {
        ...
        if !force, hash == lastSummaryHash {
            #if DEBUG
            bmuxDebugLog("mobile.observer skip: hash unchanged=\(hash) tabs=\(tabManager.tabs.count)")
            #endif
            return
        }
        ...
        #if DEBUG
        bmuxDebugLog("mobile.observer EMIT workspace.updated hash=\(hash) tabs=\(tabManager.tabs.count) force=\(force)")
        #endif
