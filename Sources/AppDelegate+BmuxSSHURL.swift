    ...
    var errorDescription: String? {
        ...
        case .launchServicesRegistrationFailed:
            return String(
                ...
                defaultValue: "bmux could not register as the default terminal app."
            )
        }
    }
}
...
enum DefaultTerminalRegistration {
    ...
    static func currentStatus(
        ...
        let matchedURLSchemes = urlSchemes.filter { scheme in
            guard let url = URL(string: "\(scheme)://bmux-default-terminal-check") else {
                return false
            }
...
enum DefaultTerminalUserAction {
...
#if DEBUG
        bmuxDebugLog("defaultTerminal.setAsDefault source=\(debugSource)")
#endif
        Task {
            do {
...
#if DEBUG
                bmuxDebugLog("defaultTerminal.setAsDefault.failed source=\(debugSource) error=\(error)")
#endif
    ...
    private static func presentSetAsDefaultError(_ error: Error) {
        ...
        alert.window.identifier = NSUserInterfaceItemIdentifier("bmux.defaultTerminalRegistrationError")
        alert.runModal()
    }
}
...
struct TerminalDefaultFileOpenRequest: Equatable {
    ...
    init?(fileURL: URL, contentType: UTType? = nil, isExecutable: Bool? = nil) {
        ...
        guard !SessionPersistencePolicy.isBmuxCrashStorageURL(standardizedURL) else { return nil }
        guard !SessionPersistencePolicy.isBmuxCrashStorageURL(directoryCheckURL) else { return nil }
...
@MainActor
final class BmuxSSHURLProcessLauncher {
    static let shared = BmuxSSHURLProcessLauncher()
    ...
    @discardableResult
    func start(request: BmuxSSHURLRequest, preferredWindow: NSWindow?) -> Bool {
        ...
        process.terminationHandler = { [weak preferredWindow] terminatedProcess in
            ...
            Task { @MainActor in
                ...
                let format = String(
                    ...
                    defaultValue: "bmux ssh exited with status %d."
                )
        ...
        do {
...
#if DEBUG
            bmuxDebugLog("sshURL.launchCLI pid=\(process.processIdentifier) socket=\(socketPath) targetLength=\(request.destination.count)")
#endif
            return true
        } catch {
            ...
            presentLaunchFailure(
                summary: String(
                    ...
                    defaultValue: "bmux ssh could not be launched."
                ),
...
@MainActor
private final class BmuxSSHURLConfirmationGate: NSObject {
...
extension AppDelegate {
    ...
    @discardableResult
    func handleBmuxExternalURLs(from urls: [URL]) -> Bool {
        let intentCounts = bmuxExternalURLIntentCounts(in: urls)
        ...
        guard intentCounts.total == 1 else {
            if intentCounts.ssh > 1 && intentCounts.navigation == 0 && intentCounts.text == 0 {
                showBmuxSSHURLParseError(.multipleLinks)
            } else {
                showBmuxTextURLParseError(.multipleLinks)
            }
            return true
        }

        if handleBmuxSSHURLs(from: urls) {
            return true
        }
        if handleBmuxNavigationURLs(from: urls) {
            return true
        }
        if handleBmuxTextURLs(from: urls) {
            return true
        }
        return false
    }

    private struct BmuxExternalURLIntentCounts {
        var ssh = 0
        var navigation = 0
        var text = 0
    ...
    private func bmuxExternalURLIntentCounts(in urls: [URL]) -> BmuxExternalURLIntentCounts {
        urls.reduce(BmuxExternalURLIntentCounts()) { counts, url in
            var nextCounts = counts
            switch BmuxSSHURLRequest.parse(url) {
            case .success(.some), .failure:
                nextCounts.ssh += 1
            case .success(nil):
                break
            }
            switch BmuxNavigationURLRequest.parse(url) {
            case .success(.some), .failure:
                nextCounts.navigation += 1
            case .success(nil):
                break
            }
            switch BmuxTextURLRequest.parse(url) {
            case .success(.some), .failure:
                nextCounts.text += 1
            case .success(nil):
                break
    ...
    @discardableResult
    func handleBmuxNavigationURLs(from urls: [URL]) -> Bool {
        var navigationRequests: [BmuxNavigationURLRequest] = []
        var parseErrors: [(url: URL, error: BmuxNavigationURLParseError)] = []
...
#if DEBUG
            bmuxDebugLog("navigationURL.ignored reason=multipleLinks count=\(urls.count) intents=\(navigationIntentCount)")
#endif
            return true
        }
...
#if DEBUG
            bmuxDebugLog("navigationURL.blocked reason=\(parseError.error) url=\(parseError.url.absoluteString.prefix(160))")
#endif
            return true
        }

        if let request = navigationRequests.first {
            _ = handleBmuxNavigationURLRequest(request)
        }
        return true
    }

    @discardableResult
    func handleBmuxSSHURLs(from urls: [URL]) -> Bool {
        var sshURLRequests: [BmuxSSHURLRequest] = []
        var sshURLParseErrors: [BmuxSSHURLParseError] = []
        for url in urls {
            switch BmuxSSHURLRequest.parse(url) {
            case .success(.some(let request)):
        ...
        if sshURLIntentCount > 1 {
            ...
            for error in sshURLParseErrors {
                showBmuxSSHURLParseError(error)
            }
            if let request = sshURLRequests.first {
                handleBmuxSSHURLRequest(request)
            }
        }
        return true
    }
    ...
    @discardableResult
    func handleBmuxTextURLs(from urls: [URL]) -> Bool {
        var textURLRequests: [BmuxTextURLRequest] = []
        var textURLParseErrors: [BmuxTextURLParseError] = []
        for url in urls {
            switch BmuxTextURLRequest.parse(url) {
            case .success(.some(let request)):
        ...
        if textURLIntentCount > 1 {
            ...
            for error in textURLParseErrors {
                showBmuxTextURLParseError(error)
            }
            if let request = textURLRequests.first {
                handleBmuxTextURLRequest(request)
            }
        }
        return true
    }

    private func handleBmuxSSHURLRequest(_ request: BmuxSSHURLRequest) {
#if DEBUG
        let target = request.originalURL.host ?? request.originalURL.path
        bmuxDebugLog("sshURL.prompt target=\(target) destinationLength=\(request.destination.count) hasPort=\(request.port != nil)")
#endif

        deferInitialMainWindowBootstrapForExternalConfirmation()
        guard confirmBmuxSSHURLRequest(request) else {
            resumeInitialMainWindowBootstrapAfterExternalConfirmation(debugSource: "sshURL.cancelled")
#if DEBUG
            bmuxDebugLog("sshURL.cancelled")
#endif
            return
        }
        ...
        _ = BmuxSSHURLProcessLauncher.shared.start(
            request: request,
            preferredWindow: NSApp.keyWindow ?? NSApp.mainWindow
        )
    }

    private func handleBmuxTextURLRequest(_ request: BmuxTextURLRequest) {
#if DEBUG
        let target = request.originalURL.host ?? request.originalURL.path
        bmuxDebugLog("textURL.prompt target=\(target) kind=\(request.kind.rawValue) textLength=\(request.text.count)")
#endif

        deferInitialMainWindowBootstrapForExternalConfirmation()
        guard confirmBmuxTextURLRequest(request) else {
            resumeInitialMainWindowBootstrapAfterExternalConfirmation(debugSource: "textURL.cancelled")
#if DEBUG
            bmuxDebugLog("textURL.cancelled")
#endif
            return
        }
        ...
        let didPaste = pasteTextInPreferredMainWindowFromExternalLink(
            ...
            onSendFailure: { [weak self] in
                self?.showBmuxTextURLPasteFailure(request)
            }
        )
        if !didPaste {
            showBmuxTextURLPasteFailure(request)
        }
    }

    private func confirmBmuxSSHURLRequest(_ request: BmuxSSHURLRequest) -> Bool {
        let alert = NSAlert()
        alert.alertStyle = .warning
        alert.messageText = String(
            localized: "dialog.sshURL.title",
            defaultValue: "Open SSH Workspace in bmux?"
        )
        alert.informativeText = String(
            format: String(
                localized: "dialog.sshURL.message",
                defaultValue: "An external link wants to open \"%@\" in bmux. Do you want to open this SSH workspace?\n\nIf you did not initiate this request, it may represent an attempted attack on your system. Only continue if you explicitly started this action."
            ),
        ...
        let gate = BmuxSSHURLConfirmationGate()
        if alert.buttons.count > 1 {
            gate.connectButton = alert.buttons[1]
        }
        alert.accessoryView = bmuxSSHURLAccessoryView(request: request, gate: gate)
    ...
    private func confirmBmuxTextURLRequest(_ request: BmuxTextURLRequest) -> Bool {
        let alert = NSAlert()
        alert.alertStyle = .warning
        ...
        let messageFormat = request.kind == .prompt
            ? String(
                ...
                defaultValue: "A %@:// link is asking bmux to paste a prompt into the current workspace. bmux cannot verify which website or app opened this link.\n\nbmux will paste the text into the terminal and will not press Return. Only continue if you trust this prompt."
            )
            : String(
                localized: "dialog.textURL.rules.message",
                defaultValue: "A %@:// link is asking bmux to paste rules into the current workspace. bmux cannot verify which website or app opened this link.\n\nbmux will paste the rules into the terminal and will not write files or press Return. Only continue if you trust these rules."
            )
        alert.informativeText = String(
        ...
        alert.accessoryView = bmuxTextURLAccessoryView(request: request)
        return alert.runModal() == .alertSecondButtonReturn
    }

    private func bmuxSSHURLAccessoryView(
        request: BmuxSSHURLRequest,
        gate: BmuxSSHURLConfirmationGate
    ) -> NSView {
        ...
        let socketPath = BmuxSSHURLProcessLauncher.shared.resolvedSocketPath()
        let commandScrollView = bmuxSSHURLTextPreview(request.cliPreview(socketPath: socketPath), height: 80)
        ...
        let checkbox = NSButton(
            checkboxWithTitle: String(
                ...
                defaultValue: "I trust this SSH target and want bmux to connect."
            ),
            target: gate,
            action: #selector(BmuxSSHURLConfirmationGate.checkboxChanged(_:))
        )
    ...
    private func bmuxTextURLAccessoryView(request: BmuxTextURLRequest) -> NSView {
        let stack = NSStackView()
        stack.orientation = .vertical
        stack.alignment = .leading
        stack.spacing = 8
        ...
        let preview = bmuxSSHURLTextPreview(request.pasteText, height: 180)
    ...
    private func bmuxSSHURLTextPreview(_ text: String, height: CGFloat) -> NSScrollView {
    ...
    private func showBmuxSSHURLParseError(_ error: BmuxSSHURLParseError) {
        let alert = NSAlert()
        alert.alertStyle = .critical
        alert.messageText = String(
            localized: "dialog.sshURL.blocked.title",
            defaultValue: "bmux SSH Link Blocked"
        )
        alert.informativeText = bmuxSSHURLParseErrorMessage(error)
        alert.addButton(withTitle: String(localized: "dialog.sshURL.blocked.ok", defaultValue: "OK"))
        alert.runModal()
    }

    private func showBmuxTextURLPasteFailure(_ request: BmuxTextURLRequest) {
        let alert = NSAlert()
        alert.alertStyle = .warning
        alert.messageText = request.kind == .prompt
        ...
        alert.informativeText = String(
            ...
            defaultValue: "bmux could not send the link text to a terminal."
        )
        alert.addButton(withTitle: String(localized: "common.ok", defaultValue: "OK"))
        alert.runModal()
    }

    private func showBmuxTextURLParseError(_ error: BmuxTextURLParseError) {
        let alert = NSAlert()
        alert.alertStyle = .critical
        alert.messageText = String(
            localized: "dialog.textURL.blocked.title",
            defaultValue: "bmux Link Blocked"
        )
        alert.informativeText = bmuxTextURLParseErrorMessage(error)
        alert.addButton(withTitle: String(localized: "dialog.textURL.blocked.ok", defaultValue: "OK"))
        alert.runModal()
    }

    private func bmuxSSHURLParseErrorMessage(_ error: BmuxSSHURLParseError) -> String {
        switch error {
        ...
        case .destinationContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The SSH host or user contains unsupported or hidden characters, so bmux refused to use it."
            )
        ...
        case .titleContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The workspace title contains hidden control or formatting characters, so bmux refused to use it."
            )
        case .invalidPort:
            return String(
    ...
    private func bmuxTextURLParseErrorMessage(_ error: BmuxTextURLParseError) -> String {
        switch error {
        case .missingText:
            return String(
        ...
        case .textContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The link text contains unsupported or hidden characters, so bmux refused to use it."
            )
        ...
        case .nameContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The link name contains hidden control or formatting characters, so bmux refused to use it."
            )
        case .titleTooLong(let maxLength):
            return String(
        ...
        case .titleContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The link title contains hidden control or formatting characters, so bmux refused to use it."
            )
        case .invalidBooleanParameter(let parameter):
            return String(
        ...
        case .multipleLinks:
            return String(
                ...
                defaultValue: "Only one bmux external link can be opened at a time."
            )
        }
    }
}
