...
nonisolated let bmuxTopMemoryDiagnosticDefaultGroupLimit = 12

nonisolated struct BmuxTopProcessAttribution: Hashable, Sendable {
...
nonisolated extension BmuxTopProcessSnapshot {
    func memoryDiagnosticPayload(
        ...
        topGroupLimit: Int = bmuxTopMemoryDiagnosticDefaultGroupLimit,
        attributionByPID: [Int: BmuxTopProcessAttribution] = [:]
    ) -> [String: Any] {
        ...
        return [
            ...
            "app": [
                ...
                "memory_source": appProcess?.memorySource.rawValue ?? BmuxTopProcessMemorySource.unavailable.rawValue,
                "resident_memory_source": appProcess?.residentMemorySource.rawValue ?? BmuxTopProcessMemorySource.unavailable.rawValue
            ] as [String: Any],
            "children": [
                "root_pid": appPID,
    ...
    private struct MemoryDiagnosticGroupAccumulator {
        ...
        var processIDs: [Int] = []
        var attributions: [BmuxTopProcessAttribution: MemoryDiagnosticAttributionAccumulator] = [:]

        mutating func append(
            process: BmuxTopProcessInfo,
            attribution: BmuxTopProcessAttribution?
        ) {
            rssBytes = BmuxTopProcessSnapshot.clampedAdd(rssBytes, process.residentBytes)
    ...
    private struct MemoryDiagnosticAttributionAccumulator {
        let attribution: BmuxTopProcessAttribution
        var rssBytes: Int64 = 0
        var processIDs: [Int] = []
        ...
        mutating func append(process: BmuxTopProcessInfo) {
            rssBytes = BmuxTopProcessSnapshot.clampedAdd(rssBytes, process.residentBytes)
            processIDs.append(process.pid)
        }
    ...
    private func memoryDiagnosticGroups(
        ...
        for pid in pids.sorted() {
            ...
            groups[key]?.append(
                process: process,
                attribution: nearestBMUXAttribution(
                    for: pid,
                    attributionByPID: attributionByPID
                )
            )
    ...
    private func nearestBMUXAttribution(
        for pid: Int,
        attributionByPID: [Int: BmuxTopProcessAttribution]
    ) -> BmuxTopProcessAttribution? {
        var visited: Set<Int> = []
        var currentPID = pid
        while currentPID > 0, visited.insert(currentPID).inserted {
            ...
            if process.bmuxWorkspaceID != nil || process.bmuxSurfaceID != nil {
                return BmuxTopProcessAttribution(
                    workspaceID: process.bmuxWorkspaceID,
                    workspaceRef: nil,
                    paneID: nil,
                    paneRef: nil,
                    surfaceID: process.bmuxSurfaceID,
                    surfaceRef: nil,
                    surfaceType: nil,
                    reason: process.bmuxAttributionReason ?? "bmux-process-scope"
                )
            }
