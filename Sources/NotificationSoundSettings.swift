...
enum NotificationSoundSettings {
    ...
    private static let stagedCustomSoundBaseName = "bmux-custom-notification-sound"
    private static let customSoundPreparationQueue = DispatchQueue(
        label: "com.bmuxterm.notification-sound-preparation",
        qos: .utility
    )
    private static let systemSoundBaseName = "bmux-system-notification-sound"
    ...
    private static let dndAssertionQueue = DispatchQueue(
        label: "com.bmuxterm.notification-dnd-assertion",
        qos: .utility
    )
    ...
    ///
    /// DEBUG builds honor `BMUX_DEBUG_DND_ASSERTIONS_PATH` so a tagged dev app
    /// can be driven end-to-end against fixture files instead of the real
    /// (TCC-protected) store.
    static let defaultAssertionsFileURL: URL = {
#if DEBUG
        if let override = ProcessInfo.processInfo.environment["BMUX_DEBUG_DND_ASSERTIONS_PATH"],
           !override.isEmpty {
    ...
    private static let customCommandQueue = DispatchQueue(
        label: "com.bmuxterm.notification-custom-command",
        qos: .utility
    )
    ...
    static func runCustomCommand(title: String, subtitle: String, body: String, defaults: UserDefaults = .standard) {
        ...
        customCommandQueue.async {
            ...
            env["BMUX_NOTIFICATION_TITLE"] = title
            env["BMUX_NOTIFICATION_SUBTITLE"] = subtitle
            env["BMUX_NOTIFICATION_BODY"] = body
