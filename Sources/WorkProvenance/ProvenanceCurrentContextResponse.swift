...
struct ProvenanceCurrentContextResponse: Codable, Equatable, Sendable {
    ...
    let dirtyFiles: [ProvenanceCurrentContextFileChange]

    /// Bounded unattributed file changes in query order.
    let unattributedChanges: [ProvenanceCurrentContextFileChange]

    /// Bounded recent checkpoints in query order.
    let recentCheckpoints: [ProvenanceCurrentContextCheckpoint]

    /// Bounded validation runs in query order.
    let validationRuns: [ProvenanceCurrentContextValidationRun]

    /// Bounded potential contribution conflicts in query order.
    let conflicts: [ProvenanceCurrentContextConflict]
