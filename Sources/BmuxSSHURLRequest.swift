...
enum BmuxSSHURLParseError: Error, Equatable {
...
struct BmuxSSHURLRequest: Equatable {
    static let maxDestinationLength = 256
    static let maxTitleLength = 160
    static let supportedSchemes: Set<String> = ["bmux", "bmux-nightly", "bmux-dev"]
    ...
    ) -> Result<BmuxSSHURLRequest?, BmuxSSHURLParseError> {
    ...
    private static func parseStandardSSHURL(_ url: URL) -> Result<BmuxSSHURLRequest?, BmuxSSHURLParseError> {
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false) else {
            return .failure(.missingDestination)
        }
    ...
    private static func standardSSHURLPort(in components: URLComponents) -> Result<Int?, BmuxSSHURLParseError> {
    ...
    private static func structuredSSHOptions(from queryItems: [URLQueryItem]) -> Result<[String], BmuxSSHURLParseError> {
        var options: [String] = []
    ...
    private static func boundedInteger(_ value: String, parameter: String, range: ClosedRange<Int>) -> Result<Int, BmuxSSHURLParseError> {
        guard !containsUnsafeHiddenCharacter(value),
    ...
    private static func normalizedBooleanValue(named name: String, in queryItems: [URLQueryItem]) -> Result<Bool, BmuxSSHURLParseError> {
...
enum BmuxNavigationURLParseError: Error, Equatable {
    case unsupportedURLShape
    case invalidIdentifier(String)
}

struct BmuxNavigationURLRequest: Equatable {
    ...
    static func parse(
        _ url: URL,
        supportedSchemes: Set<String> = activeSupportedSchemes
    ) -> Result<BmuxNavigationURLRequest?, BmuxNavigationURLParseError> {
        guard isSupportedScheme(url.scheme, supportedSchemes: supportedSchemes) else {
            return .success(nil)
        }
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false) else {
        ...
        if route.count == 2 {
            return .success(BmuxNavigationURLRequest(originalURL: url, target: .workspace(workspaceId)))
        }
...
enum BmuxTextURLParseError: Error, Equatable {
    case missingText
...
struct BmuxTextURLRequest: Equatable {
    ...
    static let maxTitleLength = 160
    static let supportedSchemes: Set<String> = BmuxSSHURLRequest.supportedSchemes
    static var activeSupportedSchemes: Set<String> {
        BmuxSSHURLRequest.activeSupportedSchemes
    }
    ...
    static func parse(
        _ url: URL,
        supportedSchemes: Set<String> = activeSupportedSchemes
    ) -> Result<BmuxTextURLRequest?, BmuxTextURLParseError> {
        guard isSupportedScheme(url.scheme, supportedSchemes: supportedSchemes) else {
            return .success(nil)
        }
        ...
        return .success(BmuxTextURLRequest(
            originalURL: url,
            kind: kind,
            text: text,
            name: name,
    ...
    private static func normalizedBooleanValue(named name: String, in queryItems: [ParsedQueryItem]) -> Result<Bool, BmuxTextURLParseError> {
        guard let item = queryItems.first(where: { $0.name.lowercased() == name }) else {
            return .success(false)
        }
        guard let rawValue = item.value else {
