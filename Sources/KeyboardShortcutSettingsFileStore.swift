...
nonisolated private let bmuxSettingsFileStoreLogger = Logger(subsystem: "com.bmuxterm.app", category: "SettingsStore")
...
final class BmuxSettingsFileStore {
    static let shared = BmuxSettingsFileStore()

    static let currentSchemaVersion = 1
    static let schemaURLString = "https://raw.githubusercontent.com/manaflow-ai/bmux/main/web/data/bmux.schema.json"
    private static let legacySchemaURLString = "https://raw.githubusercontent.com/manaflow-ai/bmux/main/web/data/bmux-settings.schema.json"
    private static let releaseBundleIdentifier = "com.bmuxterm.app"
    private static let backupsDefaultsKey = "bmux.settingsFile.backups.v1"
    private static let importedManagedDefaultsDefaultsKey = "bmux.settingsFile.importedManagedDefaults.v1"
    ...
    static var defaultFallbackPath: String? {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        return (home as NSString).appendingPathComponent(".config/bmux/settings.json")
    }
    ...
    init(
        primaryPath: String = BmuxSettingsFileStore.defaultPrimaryPath,
        fallbackPath: String? = BmuxSettingsFileStore.defaultFallbackPath,
        additionalFallbackPaths: [String] = [BmuxSettingsFileStore.defaultApplicationSupportFallbackPath].compactMap { $0 },
        ...
        // The app init path loads bmux.json before applying language/appearance
    ...
    /// bmux.json, or `nil` when the action has no configured override (so the
    ...
    private func bootstrapPrimaryTemplateIfNeeded() {
        ...
        do {
            ...
            bmuxSettingsFileStoreLogger.warning("failed to bootstrap \(self.primaryPath, privacy: .private(mask: .hash)): \(String(describing: error), privacy: .private(mask: .hash))")
        }
    }
    ...
    private func loadSettings(at path: String) -> LoadResult {
        ...
        do {
            ...
            bmuxSettingsFileStoreLogger.warning("parse error at \(path, privacy: .private(mask: .hash)): \(String(describing: error), privacy: .private(mask: .hash))")
            return .invalid
        }
    }
    ...
    private func parseSettingsFile(root: [String: Any], sourcePath: String) -> ResolvedSettingsSnapshot {
        ...
        if schemaVersion > Self.currentSchemaVersion {
            bmuxSettingsFileStoreLogger.warning("\(sourcePath, privacy: .private(mask: .hash)) uses future schemaVersion \(schemaVersion, privacy: .private(mask: .hash)); parsing known fields only")
        }
        ...
        if section.keys.contains("colors") {
            ...
            for (rawName, rawValue) in rawColors {
                ...
                guard !name.isEmpty else {
                    bmuxSettingsFileStoreLogger.warning("ignoring empty workspace color name in \(sourcePath, privacy: .private(mask: .hash))")
                    continue
                }
                guard let hex = jsonString(rawValue),
                      let normalizedHex = WorkspaceTabColorSettings.normalizedHex(hex) else {
                    bmuxSettingsFileStoreLogger.warning("ignoring invalid workspace color '\(name, privacy: .private(mask: .hash))' in \(sourcePath, privacy: .private(mask: .hash))")
                    continue
                }
        ...
        if let rawOverrides = section["paletteOverrides"] as? [String: Any] {
            ...
            for (name, rawValue) in rawOverrides {
                guard validNames.contains(name) else {
                    bmuxSettingsFileStoreLogger.warning("ignoring unknown workspace color '\(name, privacy: .private(mask: .hash))' in \(sourcePath, privacy: .private(mask: .hash))")
                    continue
                }
                guard let hex = jsonString(rawValue),
                      let normalizedHex = WorkspaceTabColorSettings.normalizedHex(hex) else {
                    bmuxSettingsFileStoreLogger.warning("ignoring invalid workspace color override '\(name, privacy: .private(mask: .hash))' in \(sourcePath, privacy: .private(mask: .hash))")
                    continue
                }
    ...
    private func parseAutomationSection(
        ...
        if let raw = jsonString(section["socketControlMode"]) {
            let knownModes = Set([
                "off", "bmuxonly", "automation", "password", "allowall", "openaccess", "fullopenaccess",
                "notifications", "full",
            ])
    ...
    private func parseShortcutsSection(
        ...
        for (rawAction, rawBinding) in bindings {
            guard let action = KeyboardShortcutSettings.Action(rawValue: rawAction) else {
                bmuxSettingsFileStoreLogger.warning("ignoring unknown shortcut action '\(rawAction, privacy: .private(mask: .hash))' in \(sourcePath, privacy: .private(mask: .hash))")
                continue
            }
            guard let shortcut = parseShortcutBindingValue(rawBinding, action: action) else {
                bmuxSettingsFileStoreLogger.warning("ignoring invalid shortcut binding for '\(rawAction, privacy: .private(mask: .hash))' in \(sourcePath, privacy: .private(mask: .hash))")
                continue
            }
    ...
    private func parseShortcutWhenClauses(
        ...
        for (rawAction, rawClause) in whenSection {
            guard let action = KeyboardShortcutSettings.Action(rawValue: rawAction) else {
                bmuxSettingsFileStoreLogger.warning("ignoring shortcuts.when for unknown action '\(rawAction, privacy: .private(mask: .hash))' in \(sourcePath, privacy: .private(mask: .hash))")
                continue
            }
            guard let expression = jsonString(rawClause),
                  let clause = ShortcutWhenClause.parse(expression) else {
                bmuxSettingsFileStoreLogger.warning("ignoring invalid shortcuts.when clause for '\(rawAction, privacy: .private(mask: .hash))' in \(sourcePath, privacy: .private(mask: .hash))")
                continue
            }
    ...
    private func parseShortcutBindingValue(
        ...
        let shortcut: StoredShortcut? = {
            ...
            // Object form written by the BmuxSettings package recorder (the
    ...
    /// Decodes the nested-object binding the BmuxSettings package writes
    ...
    private func restoreUserDefaultsBackup(
        ...
        if didMutateStoredValue {
            return managedDefaultSideEffects(
                for: defaultsKey,
                source: "bmuxConfig.restoreUserDefault",
    ...
    private func applyManagedUserDefaultsValue(
        ...
        if didMutateStoredValue {
            return managedDefaultSideEffects(
                for: defaultsKey,
                source: "bmuxConfig.applyManagedDefault",
                synchronizeAppearanceTerminalTheme: synchronizeManagedAppearanceTerminalTheme
            )
        }
        return ManagedDefaultBatchSideEffects()
    ...
    private func shouldApplyManagedUserDefaultsValue(
        ...
        // Precedence: user explicit choice (UserDefaults) > bmux.json imported default > built-in default.
    ...
    private func logInvalid(_ path: String, sourcePath: String) {
        bmuxSettingsFileStoreLogger.warning("ignoring invalid setting '\(path, privacy: .private(mask: .hash))' in \(sourcePath, privacy: .private(mask: .hash))")
    }
...
typealias KeyboardShortcutSettingsFileStore = BmuxSettingsFileStore
...
private struct ManagedCustomSettings: Equatable {
    ...
    var managedIdentifiers: Set<String> {
        ...
        if socketPassword != nil {
            identifiers.insert(BmuxSettingsFileStore.socketPasswordBackupIdentifier)
        }
        return identifiers
    }
