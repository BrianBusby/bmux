...
nonisolated struct BmuxTopResourceSummary: Sendable {
...
nonisolated enum BmuxTopProcessMemorySource: String, Sendable {
...
nonisolated struct BmuxTopProcessInfo: Sendable {
    let pid: Int
    let parentPID: Int
    let name: String
    let path: String?
    let ttyDevice: Int64?
    let bmuxWorkspaceID: UUID?
    let bmuxSurfaceID: UUID?
    let bmuxAttributionReason: String?
    let processGroupID: Int?
    let terminalProcessGroupID: Int?
    var cpuPercent: Double
    let memoryBytes: Int64
    let memorySource: BmuxTopProcessMemorySource
    let residentBytes: Int64
    let residentMemorySource: BmuxTopProcessMemorySource
    ...
    init(
        ...
        path: String?,
        ttyDevice: Int64?,
        bmuxWorkspaceID: UUID?,
        bmuxSurfaceID: UUID?,
        bmuxAttributionReason: String?,
        processGroupID: Int?,
        terminalProcessGroupID: Int?,
        cpuPercent: Double,
        memoryBytes: Int64? = nil,
        memorySource: BmuxTopProcessMemorySource? = nil,
        residentBytes: Int64,
        residentMemorySource: BmuxTopProcessMemorySource = .residentSize,
        virtualBytes: Int64,
        threadCount: Int
    ) {
        self.pid = pid
        ...
        self.bmuxWorkspaceID = bmuxWorkspaceID
        self.bmuxSurfaceID = bmuxSurfaceID
        self.bmuxAttributionReason = bmuxAttributionReason
...
nonisolated struct BmuxTopProcessScope: Sendable, Equatable {
...
nonisolated final class BmuxTopProcessSnapshot: @unchecked Sendable {
    let sampledAt: Date
    private let includesProcessDetails: Bool
    private let includesBMUXScope: Bool
    let processesByPID: [Int: BmuxTopProcessInfo]
    private let childrenByParentPID: [Int: [Int]]
    private let pidsByTTYDevice: [Int64: [Int]]
    private let pidsByBMUXSurfaceID: [UUID: [Int]]
    private let residentMemorySources: [BmuxTopProcessMemorySource]
    ...
    static func capture(
        ...
        BmuxTopProcessSnapshot(
            ...
            includesBMUXScope: includeBMUXScope
        )
    }

    init(
        processes: [BmuxTopProcessInfo],
        sampledAt: Date,
        includesProcessDetails: Bool,
        includesBMUXScope: Bool = true
    ) {
        self.sampledAt = sampledAt
        self.includesProcessDetails = includesProcessDetails
        self.includesBMUXScope = includesBMUXScope
        var processMap: [Int: BmuxTopProcessInfo] = [:]
        ...
        for process in processMap.values {
            ...
            if let bmuxSurfaceID = process.bmuxSurfaceID {
                bmuxSurfaceMap[bmuxSurfaceID, default: []].append(process.pid)
            }
        }
        self.childrenByParentPID = children.mapValues { $0.sorted() }
        self.pidsByTTYDevice = ttyMap.mapValues { $0.sorted() }
        self.pidsByBMUXSurfaceID = bmuxSurfaceMap.mapValues { $0.sorted() }
    }
    ...
    func samplePayload() -> [String: Any] {
        ...
        return [
            ...
            "memory_source": BmuxTopProcessMemorySource.physicalFootprint.rawValue,
            "memory_fallback_source": BmuxTopProcessMemorySource.residentSize.rawValue,
            ...
            "resident_memory_fallback_source": BmuxTopProcessMemorySource.rusageResidentSize.rawValue,
            "process_details": includesProcessDetails,
            "bmux_scope": includesBMUXScope
        ]
    }

    var hasBMUXScope: Bool {
        includesBMUXScope
    }

    private static func sortedMemorySources(
        in sources: [BmuxTopProcessMemorySource]
    ) -> [BmuxTopProcessMemorySource] {
        [
            .physicalFootprint,
            .residentSize,
            .rusageResidentSize,
    ...
    private static func summaryMemorySource(
        _ sources: [BmuxTopProcessMemorySource]
    ) -> BmuxTopProcessMemorySource {
    ...
    func pids(forBMUXSurfaceID surfaceID: UUID) -> Set<Int> {
        Set(pidsByBMUXSurfaceID[surfaceID] ?? [])
    }

    func bmuxScopedProcesses() -> [BmuxTopProcessInfo] {
        processesByPID.values
            .filter { $0.bmuxWorkspaceID != nil && $0.bmuxSurfaceID != nil }
            .sorted { $0.pid < $1.pid }
    }

    func process(pid: Int) -> BmuxTopProcessInfo? {
        processesByPID[pid]
    }
    ...
    func summary(for pids: Set<Int>, rootPIDs: Set<Int> = []) -> BmuxTopResourceSummary {
        let sortedPIDs = pids.filter { $0 > 0 }.sorted()
        var summary = BmuxTopResourceSummary()
    ...
    func programSummaryPayload(for pids: Set<Int>) -> [[String: Any]] {
        var aggregates: [String: BmuxProgramProcessAggregate] = [:]
        ...
        for pid in pids.sorted() {
            ...
            if aggregates[key] == nil {
                aggregates[key] = BmuxProgramProcessAggregate(id: key, title: title)
            }
    ...
    func codingAgentSummaryPayload(for pids: Set<Int>) -> [[String: Any]] {
        var aggregates: [String: BmuxCodingAgentProcessAggregate] = [:]

        for pid in pids.sorted() {
            guard let process = processesByPID[pid] else { continue }
            let processArguments = Self.processArgumentsIfNeeded(for: process)
            guard let definition = BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
            ...
            if aggregates[definition.id] == nil {
                aggregates[definition.id] = BmuxCodingAgentProcessAggregate(definition: definition)
            }
            aggregates[definition.id]?.append(process)
        }

        return BmuxTaskManagerCodingAgentDefinition.builtIns.compactMap { definition in
            guard let aggregate = aggregates[definition.id] else { return nil }
            return aggregate.payload()
        }
    }

    private static func processArgumentsIfNeeded(for process: BmuxTopProcessInfo) -> BmuxTopProcessArguments? {
        guard BmuxTaskManagerCodingAgentDefinition.shouldReadArguments(
            processName: process.name,
            processPath: process.path
        ) else { return nil }
    ...
    private struct BmuxProgramProcessAggregate {
        let id: String
        let title: String
        var cpuPercent: Double = 0
        var memoryBytes: Int64 = 0
        ...
        mutating func append(_ process: BmuxTopProcessInfo) {
            guard seenProcessIds.insert(process.pid).inserted else { return }
            cpuPercent += process.cpuPercent
            memoryBytes = BmuxTopProcessSnapshot.clampedAdd(memoryBytes, process.memoryBytes)
            residentBytes = BmuxTopProcessSnapshot.clampedAdd(residentBytes, process.residentBytes)
        ...
        func payload() -> [String: Any] {
            ...
            return [
                "id": id,
                "name": title,
                "resources": BmuxTopResourceSummary(
    ...
    private struct BmuxCodingAgentProcessAggregate {
        let definition: BmuxTaskManagerCodingAgentDefinition
        var cpuPercent: Double = 0
        var memoryBytes: Int64 = 0
        var residentBytes: Int64 = 0
        var processIds: [Int] = []
    ...
    private func processTreeNode(
        ...
        if let bmuxWorkspaceID = process.bmuxWorkspaceID {
            payload["bmux_workspace_id"] = bmuxWorkspaceID.uuidString
        } else {
            payload["bmux_workspace_id"] = NSNull()
        }
        if let bmuxSurfaceID = process.bmuxSurfaceID {
            payload["bmux_surface_id"] = bmuxSurfaceID.uuidString
        } else {
            payload["bmux_surface_id"] = NSNull()
        }
    ...
    private func attributionReason(
        for process: BmuxTopProcessInfo,
        allowedPIDs: Set<Int>,
        rootPIDs: Set<Int>
    ) -> String {
        if let reason = process.bmuxAttributionReason {
            return reason
        }
    ...
    private func isWebKitWebContentProcess(_ process: BmuxTopProcessInfo) -> Bool {
