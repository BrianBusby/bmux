...
/// Shared, cross-tag default for which display new bmux DEV windows open on.
///
/// The value is a display's `localizedName` (e.g. `"LG HDR 4K"`), persisted in
/// the shared `bmux.json` under `app.devWindowDisplay` through ``BmuxSettings``.
///
/// It lives in `bmux.json` (a fixed path shared across bundle ids) rather than
...
///
/// This is a thin wrapper over ``BmuxSettings/JSONConfigStore``: the
...
enum DevWindowDisplayDefault {
    ...
    /// `bmux.json`. Read for migration and as a first-launch fallback, then
    /// ignored.
    static var legacyFileURL: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/bmux/dev-window-display")
    }
    ...
    /// parent), matching `bmux window default-display --clear`.
    ...
    /// One-time best-effort migration of the pre-`bmux.json` file value into
    ...
    /// never steals focus. DEBUG-only: production bmux is never auto-moved.
    @MainActor
    static func applyToNewWindow(_ window: NSWindow) {
        ...
        // async one-time migration has committed to bmux.json.
