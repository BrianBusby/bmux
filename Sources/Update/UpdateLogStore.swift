...
// other stored properties are immutable. Conforms to BmuxUpdater's `UpdateLogging` seam so the
// updater package can log through this app-owned file logger.
final class UpdateLogStore: UpdateLogging, @unchecked Sendable {
    private let queue = DispatchQueue(label: "bmux.update.log")
    ...
    init() {
        ...
        logURL = logsDir.appendingPathComponent("Logs/bmux-update.log")
        ensureLogFile()
    }
...
final class FocusLogStore: @unchecked Sendable {
    private let queue = DispatchQueue(label: "bmux.focus.log")
    private var entries: [String] = []
    ...
    init() {
        ...
        let logsDir = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first
            ?? FileManager.default.temporaryDirectory
        logURL = logsDir.appendingPathComponent("Logs/bmux-focus.log")
        ensureLogFile()
    }
