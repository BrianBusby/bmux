...
nonisolated struct BmuxTopProcessCPUSample: Sendable {
    let totalTimeTicks: UInt64
    let sampledAtNanoseconds: UInt64
}

private nonisolated struct BmuxTopProcessCPUTrackerState: Sendable {
    var entries: [BmuxTopProcessScopeCacheKey: BmuxTopProcessCPUTrackerEntry] = [:]
    var latestPrunedAtNanoseconds: UInt64 = 0
}

private nonisolated struct BmuxTopProcessCPUTrackerEntry: Sendable {
    let sample: BmuxTopProcessCPUSample
    let cpuPercent: Double
    let parentKey: BmuxTopProcessScopeCacheKey?
}

private nonisolated final class BmuxTopProcessCPUTracker: @unchecked Sendable {
    private static let minimumSampleWindowNanoseconds: UInt64 = 1_000_000_000

    private let state = OSAllocatedUnfairLock(initialState: BmuxTopProcessCPUTrackerState())
    ...
    func cpuPercentages(
        for currentSamples: [BmuxTopProcessScopeCacheKey: BmuxTopProcessCPUSample],
        activeKeys: Set<BmuxTopProcessScopeCacheKey>,
        parentKeysByKey: [BmuxTopProcessScopeCacheKey: BmuxTopProcessScopeCacheKey],
        sampledAtNanoseconds: UInt64
    ) -> [BmuxTopProcessScopeCacheKey: Double] {
        state.withLock { state in
            var percentages: [BmuxTopProcessScopeCacheKey: Double] = [:]
            percentages.reserveCapacity(currentSamples.count)
            var heldKeys: Set<BmuxTopProcessScopeCacheKey> = []
            ...
            for (key, sample) in currentSamples {
                ...
                if let existing {
                    ...
                    guard elapsedNanoseconds >= Self.minimumSampleWindowNanoseconds else {
                        ...
                        if existing.parentKey != parentKey {
                            state.entries[key] = BmuxTopProcessCPUTrackerEntry(
                ...
                let cpuPercent = BmuxTopProcessSnapshot.cpuPercent(
                    current: sample,
...
private nonisolated let bmuxTopProcessCPUTracker = BmuxTopProcessCPUTracker()
private nonisolated let bmuxTopAbsoluteTimeNanosecondsRatio: Double? = {
...
nonisolated extension BmuxTopProcessSnapshot {
    ...
    static func cpuPercentages(
        for samples: [BmuxTopProcessScopeCacheKey: BmuxTopProcessCPUSample],
        activeKeys: Set<BmuxTopProcessScopeCacheKey>,
        parentKeysByKey: [BmuxTopProcessScopeCacheKey: BmuxTopProcessScopeCacheKey] = [:],
        sampledAtNanoseconds: UInt64
    ) -> [BmuxTopProcessScopeCacheKey: Double] {
        bmuxTopProcessCPUTracker.cpuPercentages(
            for: samples,
    ...
    static func cpuPercent(
        current: BmuxTopProcessCPUSample,
        previous: BmuxTopProcessCPUSample?
    ) -> Double {
        guard let previous,
    ...
    private static func absoluteTimeNanoseconds(_ ticks: UInt64) -> Double? {
        guard let ratio = bmuxTopAbsoluteTimeNanosecondsRatio else { return nil }
