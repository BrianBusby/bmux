...
/// Identifies a remote host whose tmux server bmux mirrors over SSH.
///
/// A host is addressed by its SSH `destination` — either a `~/.ssh/config`
/// alias (e.g. `claude-box`) or an explicit `user@host`. bmux multiplexes
...
struct RemoteTmuxHost: Sendable, Equatable, Identifiable {
    ...
    ///
    /// Namespaced under `~/.bmux/ssh/`. The filename combines the lossy
    ...
    ///
    /// Intended to be run by the `bmux ssh-tmux` CLI **inside the user's terminal**
    ...
    static func tmuxRemoteCommand(arguments: [String]) -> String {
        (["/bin/sh", "-c", tmuxResolverShellScript, "bmux-remote-tmux"] + arguments)
            .map(shellSingleQuoted)
            .joined(separator: " ")
    }

    /// Stable stderr marker the resolver emits with exit 127 when no tmux binary is usable.
    static let tmuxNotFoundSentinel = "bmux-remote-tmux: tmux not found"
    ...
    private static let tmuxResolverShellScript =
        "bmux_tmux=\"\"; " +
        "if command -v tmux >/dev/null 2>&1; then bmux_tmux=\"$(command -v tmux)\"; else " +
        "for bmux_dir in \"$HOME/.local/bin\" \"$HOME/bin\" /opt/homebrew/bin /usr/local/bin /opt/local/bin /usr/pkg/bin /snap/bin /usr/bin /bin; do " +
        "if [ -x \"$bmux_dir/tmux\" ]; then bmux_tmux=\"$bmux_dir/tmux\"; break; fi; done; " +
        "if [ -z \"$bmux_tmux\" ] && [ -x /usr/libexec/path_helper ]; then eval \"$(/usr/libexec/path_helper -s 2>/dev/null)\"; " +
        "if command -v tmux >/dev/null 2>&1; then bmux_tmux=\"$(command -v tmux)\"; fi; fi; fi; " +
        "if [ -n \"$bmux_tmux\" ]; then exec \"$bmux_tmux\" \"$@\"; fi; " +
