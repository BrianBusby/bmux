...
extension TerminalController {
    ...
    nonisolated func handleEventsStreamRequest(_ line: String, socket: Int32) {
        ...
        let afterSequence = BmuxEventBus.int64(params["after_seq"] ?? params["after"])
        ...
        defer { BmuxEventBus.shared.unsubscribe(snapshot.subscription) }
        ...
        while true {
            if let event = snapshot.subscription.next(timeout: BmuxEventBus.defaultHeartbeatIntervalSeconds) {
                guard writeEventsStreamLine(event, socket: socket) else { return }
                ...
                if let reason = snapshot.subscription.closeReason {
                    _ = writeEventsStreamLine([
                        ...
                        "error": [
                            "code": "slow_consumer",
                            "message": reason,
                            "latest_seq": NSNumber(value: BmuxEventBus.shared.latestSequence)
                        ]
                    ], socket: socket)
                }
                return
            } else if includeHeartbeats {
                let heartbeat = BmuxEventBus.shared.heartbeat(subscription: snapshot.subscription)
    ...
    private nonisolated func writeEventsStreamLine(_ object: [String: Any], socket: Int32) -> Bool {
        autoreleasepool {
            guard let line = BmuxEventBus.encodeLine(object) else { return false }
