...
nonisolated private let bmuxEventLogLogger = Logger(subsystem: "com.bmuxterm.app", category: "event-log")

// Sendable safety: pending state is protected by `lock`; file IO runs on `queue`.
final class BmuxEventLogWriter: @unchecked Sendable {
    static let defaultMaxPendingLines = 1_024

    private static let queue = DispatchQueue(label: "com.bmuxterm.event-log", qos: .utility)
    ...
    private func flushPendingLines() {
        while true {
            ...
            if pendingLines.isEmpty {
                ...
                lock.unlock()
                if droppedCount > 0 {
                    bmuxEventLogLogger.warning("Dropped \(droppedCount, privacy: .public) bmux event log line(s) under disk backpressure")
                }
                return
            }
            lines = pendingLines
    ...
    private func append(_ lines: [String]) {
        ...
        do {
            ...
            bmuxEventLogLogger.error("Failed to append bmux event log: \(String(describing: error), privacy: .private)")
        }
    }
