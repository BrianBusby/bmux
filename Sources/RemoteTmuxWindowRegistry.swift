...
/// "one bmux window per remote endpoint" mirror mode (Option 1): the host↔window
...
final class RemoteTmuxWindowRegistry {
    /// ``RemoteTmuxHost/connectionHash`` → the dedicated bmux window mirroring that
    /// endpoint (Option 1).
    private var windowIdByHost: [String: UUID] = [:]
    /// Reverse map: bmux window id → the full host it mirrors (for window-close
