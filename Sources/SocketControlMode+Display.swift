...
// `SocketControlMode` lives in BmuxSettings (a value type with no UI concerns).
...
extension SocketControlMode {
    ...
    static var uiCases: [SocketControlMode] { [.off, .bmuxOnly, .automation, .password, .allowAll] }
    ...
    var displayName: String {
        ...
        case .bmuxOnly:
            return String(localized: "socketControl.bmuxOnly.name", defaultValue: "bmux processes only")
        case .automation:
    ...
    var description: String {
        ...
        case .bmuxOnly:
            return String(localized: "socketControl.bmuxOnly.description", defaultValue: "Only processes started inside bmux terminals can send commands.")
        case .automation:
