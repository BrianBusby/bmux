...
private extension NSView {
    func bmuxAncestor<T: NSView>(of type: T.Type) -> T? {
        ...
        GeometryReader { geo in
            HStack(spacing: 4) {
                SearchTextFieldRepresentable(
                    ...
                    onEscape: {
                        #if DEBUG
                        bmuxDebugLog("find.nativeField.escape surface=\(surfaceId.uuidString.prefix(5)) needleEmpty=\(searchState.needle.isEmpty)")
                        #endif
                        onClose()
                    },
                    onReturn: { isShift in
                ...
                Button(action: {
                    #if DEBUG
                    bmuxDebugLog("findbar.next surface=\(surfaceId.uuidString.prefix(5))")
                    #endif
                ...
                Button(action: {
                    #if DEBUG
                    bmuxDebugLog("findbar.prev surface=\(surfaceId.uuidString.prefix(5))")
                    #endif
                ...
                Button(action: {
                    #if DEBUG
                    bmuxDebugLog("findbar.close surface=\(surfaceId.uuidString.prefix(5))")
                    #endif
                    onClose()
                }) {
            ...
            .onAppear {
                #if DEBUG
                bmuxDebugLog("find.overlay.appear tab=\(tabId.uuidString.prefix(5)) surface=\(surfaceId.uuidString.prefix(5))")
                #endif
...
private struct SearchTextFieldRepresentable: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTextFieldDelegate {
        ...
        func focusField(_ field: SearchNativeTextField, in window: NSWindow, selectAll: Bool) {
            let alreadyFocused = bmuxTextFieldIsFirstResponder(field, in: window)
            guard alreadyFocused || window.makeFirstResponder(field) else { return }
            let rememberedRange = field.bmuxLastSelectedRange ?? bmuxStoredFindSelection(for: self.parent.selectionOwner) ?? self.lastSelectedRange
            if let selection = bmuxApplyFindFocusSelection(field: field, selectAll: selectAll, alreadyFocused: alreadyFocused, rememberedRange: rememberedRange) { self.lastSelectedRange = selection; return }
            DispatchQueue.main.async { [weak field, weak self] in
                guard let field, let self,
                      let selection = bmuxApplyFindFocusSelection(field: field, selectAll: selectAll, alreadyFocused: alreadyFocused, rememberedRange: rememberedRange) else { return }
        ...
        func controlTextDidBeginEditing(_ obj: Notification) {
            #if DEBUG
            bmuxDebugLog("find.nativeField.beginEditing surface=\(parent.surfaceId.uuidString.prefix(5))")
            #endif
        ...
        func controlTextDidEndEditing(_ obj: Notification) {
            #if DEBUG
            bmuxDebugLog("find.nativeField.endEditing surface=\(parent.surfaceId.uuidString.prefix(5))")
            #endif
        ...
        func control(_ control: NSControl, textView: NSTextView, doCommandBy commandSelector: Selector) -> Bool {
            ...
            case #selector(NSResponder.insertNewline(_:)):
                ...
                return true
            default:
                if bmuxFindCommandMayChangeSelection(commandSelector) {
        ...
        func handleEscape(from textView: NSTextView, control: NSControl? = nil) -> Bool {
            ...
            if textView.hasMarkedText() { return false }
            rememberSelection(from: textView)
            (control ?? parentField)?.bmuxAncestor(of: GhosttySurfaceScrollView.self)?.beginFindEscapeSuppression()
            parent.onEscape()
            return true
        }
        ...
        private func rememberSelection(from textView: NSTextView) {
            let selection = bmuxClampedFindSelection(textView.selectedRange(), in: textView.string)
            lastSelectedRange = selection
            parentField?.bmuxLastSelectedRange = selection
            bmuxStoreFindSelection(selection, for: parent.selectionOwner)
        }
    }
    ...
    func makeNSView(context: Context) -> SearchNativeTextField {
        ...
        field.bmuxSelectionOwner = selectionOwner
        field.bmuxOnEscape = { [weak coordinator = context.coordinator] textView in coordinator?.handleEscape(from: textView) ?? false }
    ...
    func updateNSView(_ nsView: SearchNativeTextField, context: Context) {
        ...
        nsView.bmuxSelectionOwner = selectionOwner
        nsView.bmuxOnEscape = { [weak coordinator = context.coordinator] textView in coordinator?.handleEscape(from: textView) ?? false }
        ...
        if let editor = nsView.currentEditor() as? NSTextView {
            if editor.string != text, !editor.hasMarkedText() {
                let selectedRange = nsView.bmuxRememberSelection(editor.selectedRange(), in: text)
                context.coordinator.isProgrammaticMutation = true
                editor.string = text
                nsView.stringValue = text
                editor.setSelectedRange(selectedRange)
                context.coordinator.lastSelectedRange = selectedRange
                bmuxStoreFindSelection(selectedRange, for: selectionOwner)
        ...
        if let window = nsView.window {
            let isFirstResponder = bmuxTextFieldIsFirstResponder(nsView, in: window)
            ...
            if isFocused,
               ...
               context.coordinator.pendingFocusRequest != true {
                ...
                DispatchQueue.main.async { [weak nsView, weak coordinator = context.coordinator] in
                    ...
                    let alreadyFocused = bmuxTextFieldIsFirstResponder(nsView, in: window)
                    guard !alreadyFocused else { return }
    ...
    static func dismantleNSView(_ nsView: SearchNativeTextField, coordinator: Coordinator) {
        ...
        nsView.delegate = nil
        nsView.bmuxSelectionOwner = nil
        nsView.bmuxOnEscape = nil
