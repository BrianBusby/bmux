...
final class TerminalViewportUITestRecorder {
    ...
    static func isEnabled(environment: [String: String]) -> Bool {
        guard environment["BMUX_UI_TEST_TERMINAL_VIEWPORT_SETUP"] == "1",
              environment["BMUX_UI_TEST_TERMINAL_VIEWPORT_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false else {
            return false
        }
        return true
    }
    ...
    init(
        ...
        initialWindowSize = Self.parseWindowSize(environment["BMUX_UI_TEST_TERMINAL_VIEWPORT_WINDOW_SIZE"])
        initialWindowSizeText = Self.requestedWindowSizeText(environment["BMUX_UI_TEST_TERMINAL_VIEWPORT_WINDOW_SIZE"])
        resizeWindowSize = Self.parseWindowSize(environment["BMUX_UI_TEST_TERMINAL_VIEWPORT_RESIZE_WINDOW_SIZE"])
        resizeWindowSizeText = Self.requestedWindowSizeText(environment["BMUX_UI_TEST_TERMINAL_VIEWPORT_RESIZE_WINDOW_SIZE"])
        hideSidebar = environment["BMUX_UI_TEST_TERMINAL_VIEWPORT_HIDE_SIDEBAR"] == "1"
        hideRightSidebar = environment["BMUX_UI_TEST_TERMINAL_VIEWPORT_HIDE_RIGHT_SIDEBAR"] == "1"
