...
struct BmuxActionTrustDescriptor: Codable, Sendable {
    var schemaVersion: Int = 1
    var actionID: String
    var kind: String
    var command: String?
    var target: String?
    var workspaceCommand: BmuxCommandDefinition?
...
final class BmuxActionTrust {
    static let shared = BmuxActionTrust()
    static let didChangeNotification = Notification.Name("bmux.actionTrustDidChange")
    ...
    private init() {
        let appSupport = FileManager.default.urls(
            ...
            in: .userDomainMask
        ).first!.appendingPathComponent("bmux")
    ...
    func isTrusted(_ descriptor: BmuxActionTrustDescriptor) -> Bool {
        trustedFingerprints.contains(descriptor.fingerprint)
    }

    func trust(_ descriptor: BmuxActionTrustDescriptor) {
