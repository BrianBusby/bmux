...
extension TerminalController: ControlWorkspaceGroupContext {
    ...
    func controlCreateWorkspaceInGroup(
        ...
        // per-cwd `newWorkspacePlacement` from bmux.json, then the global default.
        ...
        let configStore = AppDelegate.shared?.mainWindowContexts.values.first(where: { $0.tabManager === tabManager })?.bmuxConfigStore
