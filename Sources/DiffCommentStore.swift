...
/// `Application Support/bmux/diff-comments/`. Comments outlive individual
/// `bmux diff` invocations, so a regenerated diff for the same repo shows the
/// same comments.
@MainActor
...
#if DEBUG
            bmuxDebugLog("diffComments.store.saveFailed error=\(error.localizedDescription)")
#endif
        }
    }
