...
final class DraggableFolderNSView: NSView, NSDraggingSource {
    ...
    init(directory: String) {
        ...
        identifier = NSUserInterfaceItemIdentifier("bmux.folderDragIcon")
        setupImageView()
    }
    ...
    func draggingSession(_ session: NSDraggingSession, endedAt screenPoint: NSPoint, operation: NSDragOperation) {
        ...
        bmuxDebugLog("folder.dragEnd dirBytes=\(directory.utf8.count) operation=\(operation.rawValue) screen=\(formatPoint(screenPoint)) nowMovable=\(nowMovable) windowOrigin=\(windowOrigin)")
        #endif
    }
    ...
    override func hitTest(_ point: NSPoint) -> NSView? {
        ...
        let nowMovable = window.map { String($0.isMovable) } ?? "nil"
        bmuxDebugLog("folder.hitTest point=\(formatPoint(point)) hit=\(hitDesc) imageViewHit=\(imageHit) returning=DraggableFolderNSView nowMovable=\(nowMovable)")
        #endif
        return self
    }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        let nowMovable = window.map { String($0.isMovable) } ?? "nil"
        let windowOrigin = window.map { formatPoint($0.frame.origin) } ?? "nil"
        bmuxDebugLog("folder.mouseDown dirBytes=\(directory.utf8.count) point=\(formatPoint(localPoint)) firstResponder=\(responderDesc) nowMovable=\(nowMovable) windowOrigin=\(windowOrigin)")
        #endif
    ...
    private func beginFolderDrag(with event: NSEvent) {
        ...
        bmuxDebugLog("folder.dragStart dirBytes=\(directory.utf8.count) pasteboardItems=\(itemCount)")
        #endif
    }
