...
struct BmuxWorkspaceDefinition: Codable, Sendable, Hashable {
    var name: String?
    var cwd: String?
    var color: String?
    /// User-defined environment variables inherited by every shell spawned in the
    /// workspace (issue #5995). Managed `BMUX_*` variables always win.
    ...
    init(from decoder: Decoder) throws {
        ...
        layout = try container.decodeIfPresent(BmuxLayoutNode.self, forKey: .layout)

        if let rawColor = try container.decodeIfPresent(String.self, forKey: .color) {
            let defaults = decoder.userInfo[.bmuxWorkspaceColorDefaults] as? UserDefaults ?? .standard
