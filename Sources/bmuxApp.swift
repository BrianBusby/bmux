...
struct bmuxApp: App {
    ...
    init() {
        ...
        // apps" prompt; the password now lives in the non-protected bmux state
        // directory (https://github.com/manaflow-ai/bmux/issues/5146). The app
        ...
        // Secrets live in their own 0600 files under the bmux state directory,
        ...
        // (`BmuxSettingsFileStore`, loaded later during app launch) reads the
        ...
        // If invoked with CLI-style arguments (e.g. `bmux hooks setup`), exec the
        // bundled CLI at Contents/Resources/bin/bmux. The GUI binary and the CLI
        // share the name `bmux`, so if the GUI's Contents/MacOS leaks onto $PATH
        // (which happens for any shell descended from this process), bare `bmux`
        // resolves here instead of the CLI. See
        // https://github.com/manaflow-ai/bmux/issues/4678.
        // bmux ships a universal binary so it still supports Intel Macs, but a
        ...
        // launches. See https://github.com/manaflow-ai/bmux/issues/753.
        ...
        self.settingsRuntime = appRuntimeComposition.makeSettingsRuntime(
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged bmux DEV; start with ./scripts/reload.sh --tag <name> (or set BMUX_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
    ...
    static func resolvedGhosttyResourcesDirectory(
        ...
        // Tagged bmux builds may inherit GHOSTTY_RESOURCES_DIR from another running
        // bmux instance. Prefer this app's bundled resources when they are present.
    ...
    var body: some Scene {
        WindowGroup {
            MainWindowBootstrapView()
                ...
                .bmuxAppearanceColorScheme(appearanceMode)
                .onAppear {
                    ...
                    if ProcessInfo.processInfo.environment["BMUX_UI_TEST_MODE"] == "1" {
                        AppDelegate.shared?.updateLog.append("ui test: bmuxApp onAppear")
                    }
#endif
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                ...
                Button(String(localized: "menu.app.openBmuxSettingsFile", defaultValue: "Open bmux.json")) {
                ...
                Button(String(localized: "menu.app.makeDefaultTerminal", defaultValue: "Make bmux the Default Terminal")) {
                    DefaultTerminalUserAction.setAsDefault(debugSource: "menu.makeDefaultTerminal")
                }
            }

            CommandGroup(replacing: .appInfo) {
                Button(String(localized: "menu.app.about", defaultValue: "About bmux")) {
                    showAboutPanel()
                }
            ...
            CommandGroup(replacing: .appTermination) {
                splitCommandButton(title: String(localized: "menu.quitBmux", defaultValue: "Quit bmux"), shortcut: menuShortcut(for: .quit)) {
                    NSApp.terminate(nil)
                }
            }
...
#if DEBUG
                        bmuxDebugLog("find.menu Cmd+F fired")
#endif
    ...
    private func applyAppearance() {
        let mode = AppearanceSettings.applyStoredMode(
            ...
            source: "bmuxApp.appearanceModeChanged"
        )
    ...
    private static func applyAppearance(_ mode: AppearanceMode, duringLaunch: Bool = false) {
        AppearanceSettings.applyLiveMode(
            mode,
            source: duringLaunch ? "bmuxApp.launch" : "bmuxApp.applyAppearance",
    ...
    private func closePanelOrWindow() {
        ...
        if let window, bmuxWindowShouldOwnCloseShortcut(window) { window.performClose(nil); return }
...
private let bmuxAuxiliaryWindowIdentifiers: Set<String> = [
    ...
    "bmux.licenses",
    "bmux.browser-popup",
    "bmux.browserProfilePopoverDebug",
    "bmux.configEditor",
    "bmux.defaultTerminalRegistrationError",
    "bmux.feedButtonStyleDebug",
    "bmux.feedPreview",
    "bmux.feedTextEditorDebug",
    "bmux.fileExplorerStyleDebug",
    "bmux.folderDragIcon",
    "bmux.pdfPreviewChromeDebug",
    "bmux.proBadgeDebug",
    "bmux.recentlyClosedHistory",
    "bmux.splitButtonLayoutDebug",
    "bmux.tabBarBackdropLab",
    "bmux.taskManager",
    "bmux.aboutTitlebarDebug",
    "bmux.debugWindowControls",
    "bmux.browserImportHintDebug",
    "bmux.extensionSidebarInspector",
    "bmux.sidebarDebug",
    "bmux.menubarDebug",
    "bmux.backgroundDebug",
    "bmux.startupAppearanceDebug",
    "bmux.bonsplitTabBarDebug",
    "bmux.titlebarLayoutDebug",
    "bmux.devWindowDisplay",
    "bmux.mobilePairingWindow",
]

/// Returns whether the given window should handle the standard close shortcut
/// as a standalone auxiliary window instead of routing it through workspace or
/// panel-close behavior.
func bmuxWindowShouldOwnCloseShortcut(_ window: NSWindow?) -> Bool {
    guard let identifier = window?.identifier?.rawValue else { return false }
    return bmuxAuxiliaryWindowIdentifiers.contains(identifier)
}
...
private final class DebugWindowControlsWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("bmux.debugWindowControls")
        window.center()
...
private final class BrowserImportHintDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("bmux.browserImportHintDebug")
        window.center()
...
private final class BrowserProfilePopoverDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("bmux.browserProfilePopoverDebug")
        window.center()
...
private final class AboutWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("bmux.about")
        window.center()
...
private final class AcknowledgmentsWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("bmux.licenses")
        window.center()
...
private struct AcknowledgmentsView: View {
    ...
    var body: some View {
        ScrollView {
            Text(content)
                .bmuxFont(.body, design: .monospaced)
...
private final class FileExplorerStyleDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("bmux.fileExplorerStyleDebug")
        window.center()
...
private final class SidebarDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("bmux.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["BMUXCommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["BMUX_COMMIT"] ?? ""
        ...
        VStack(alignment: .center) {
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text(String(localized: "about.appName", defaultValue: "bmux"))
                        .bmuxFont(.title)
                        .bold()
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/manaflow-ai/bmux/commit/\(hash)")
                    }
...
private final class MenuBarExtraDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("bmux.menubarDebug")
        window.center()
...
private final class SplitButtonLayoutDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("bmux.splitButtonLayoutDebug")
        window.center()
...
private final class TabBarBackdropLabWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("bmux.tabBarBackdropLab")
        window.center()
    ...
    private func labSlider(
        ...
        VStack(alignment: .leading, spacing: 4) {
            Text("\(title) \(displayValue)")
                .bmuxFont(.caption, monospacedDigit: true)
                .lineLimit(1)
            Slider(value: value, in: range)
                .frame(width: width)
        }
...
private struct TabBarBackdropLabSample: View {
    ...
    private static func makeController(for variant: TabBarBackdropLabVariant) -> BonsplitController {
        ...
        let titles = [
            ...
            String(localized: "debug.tabBarBackdropLab.tab.terminalTransparency", defaultValue: "bmux terminal transparency"),
...
private struct TabBarBackdropLabTitlebar: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(title)
                .bmuxFont(.caption2, weight: .medium)
                .lineLimit(1)
            Spacer(minLength: 0)
        }
...
private struct TabBarBackdropLabTerminalPane: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("\(String(localized: "debug.tabBarBackdropLab.terminal.prompt", defaultValue: "lawrence in ~/bmux")) \(title)")
...
private final class BackgroundDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("bmux.backgroundDebug")
        window.center()
...
private final class StartupAppearanceDebugWindowController: ReleasingWindowController {
    ...
    override func makeWindow() -> NSWindow {
        ...
        window.titlebarAppearsTransparent = false
        window.isMovableByWindowBackground = true
        window.identifier = NSUserInterfaceItemIdentifier("bmux.startupAppearanceDebug")
        window.center()
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ScrollView {
                            Text(selectedConfigText)
                                .bmuxFont(.caption, design: .monospaced)
                                .textSelection(.enabled)
                                .frame(maxWidth: .infinity, alignment: .topLeading)
                                .padding(8)
                        }
...
enum AppIconSettings {
    ...
    private static let dockTileIconDidChangeNotification = Notification.Name("com.bmuxterm.appIconDidChange")
...
nonisolated enum BuildFlavor: String, Sendable {
    ...
    static func detect(bundleNames: [String], bundleIdentifier: String?) -> BuildFlavor {
        ...
        if normalizedBundleIdentifier == "com.bmuxterm.app.nightly"
            || normalizedBundleIdentifier?.hasPrefix("com.bmuxterm.app.nightly.") == true {
            return .nightly
        }
...
enum TelemetrySettings {
    ...
    // live in `BmuxSettings` (`AppCatalogSection().sendAnonymousTelemetry`) as the
...
@MainActor
func openBmuxSettingsFileInEditor() {
