...
/// Persists saved workspace layout results into the global bmux.json,
/// preserving JSONC comments and formatting via `JSONCObjectEditor`.
enum BmuxConfigActionSaver {
    ...
    enum SaveError: LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            ...
            case .unreadableConfig(let path):
                let format = String(
                    localized: "error.bmuxConfigActionSaver.unreadableConfig",
                    defaultValue: "Couldn't read %@."
                )
                return String(format: format, path)
            case .malformedConfig(let path):
                let format = String(
                    localized: "error.bmuxConfigActionSaver.malformedConfig",
    ...
    static func saveWorkspaceAction(
        ...
        let actionDefinition = BmuxConfigActionDefinition(
    ...
    private static func writeOwnerOnlyConfig(
        ...
        let tempURL = directoryURL.appendingPathComponent(".bmux.json.tmp-\(UUID().uuidString)")
    ...
    static func encodeActionValueJSON(_ definition: BmuxConfigActionDefinition) throws -> String {
...
struct NewWorkspaceDefaultLayoutMenuModel: Equatable {
    ...
    static func build(
        loadedActions: [BmuxResolvedConfigAction],
