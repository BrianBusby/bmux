...
private var bmuxWindowBrowserPortalKey: UInt8 = 0
private var bmuxWindowBrowserPortalCloseObserverKey: UInt8 = 0
private var bmuxBrowserSearchOverlayPanelIdAssociationKey: UInt8 = 0
private var bmuxWindowInteractiveSplitDividerDragKey: UInt8 = 0
...
private extension NSWindow {
    var browserPortalHasInteractiveSplitDividerDrag: Bool {
        get {
            let isActive =
                (objc_getAssociatedObject(self, &bmuxWindowInteractiveSplitDividerDragKey) as? NSNumber)?
                    .boolValue ?? false
            guard isActive else { return false }
            guard (NSEvent.pressedMouseButtons & 1) != 0 else {
                objc_setAssociatedObject(
                    self,
                    &bmuxWindowInteractiveSplitDividerDragKey,
...
final class WindowBrowserSlotView: NSView {
    ...
    func setSearchOverlay(_ configuration: BrowserPortalSearchOverlayConfiguration?) {
        guard let configuration else {
            ...
            if let overlay = searchOverlayHostingView {
                objc_setAssociatedObject(
                    overlay,
                    &bmuxBrowserSearchOverlayPanelIdAssociationKey,
                    nil,
                    .OBJC_ASSOCIATION_RETAIN_NONATOMIC
                )
            }
    ...
    func searchOverlayPanelId(for responder: NSResponder) -> UUID? {
        ...
        let panelId = objc_getAssociatedObject(overlay, &bmuxBrowserSearchOverlayPanelIdAssociationKey) as? UUID
    ...
    func yieldSearchOverlayFocusIfOwned(by panelId: UUID, in window: NSWindow) -> Bool {
        ...
        _ = bmuxRememberFindSelection(in: searchOverlayHostingView); return window.makeFirstResponder(nil)
    }
...
final class WindowBrowserPortal: NSObject {
...
#if DEBUG
    // Test seam for https://github.com/manaflow-ai/bmux/issues/5733. Installs a
    ...
    private func relatedWebKitTransferSubviews(
        ...
        for view in sourceSuperview.subviews {
            ...
            if bmuxIsWebInspectorClassName(className) || Self.containsInspectorView(in: view) {
                continue
            }
    ...
    func bind(webView: WKWebView, to anchorView: NSView, visibleInUI: Bool, zPriority: Int = 0) {
        ...
        let shouldPreserveExternalFullscreenHost =
            webView.bmuxIsManagedByExternalFullscreenWindow(relativeTo: window)
...
#if DEBUG
        bmuxDebugLog("browser.portal.sync.defer.schedule entries=\(entriesByWebViewId.count)")
#endif
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.hasDeferredFullSyncScheduled = false
#if DEBUG
            bmuxDebugLog("browser.portal.sync.defer.tick entries=\(self.entriesByWebViewId.count)")
#endif
...
enum BrowserWindowPortalRegistry {
    ...
    private static func installWindowCloseObserverIfNeeded(for window: NSWindow) {
        guard objc_getAssociatedObject(window, &bmuxWindowBrowserPortalCloseObserverKey) == nil else { return }
        ...
        objc_setAssociatedObject(
            window,
            &bmuxWindowBrowserPortalCloseObserverKey,
            observer,
            .OBJC_ASSOCIATION_RETAIN_NONATOMIC
        )
    }
    ...
    private static func removePortal(windowId: ObjectIdentifier, window: NSWindow?) {
        ...
        guard let window else { return }
        if let observer = objc_getAssociatedObject(window, &bmuxWindowBrowserPortalCloseObserverKey) {
            NotificationCenter.default.removeObserver(observer)
        }
        objc_setAssociatedObject(window, &bmuxWindowBrowserPortalCloseObserverKey, nil, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        objc_setAssociatedObject(window, &bmuxWindowBrowserPortalKey, nil, .OBJC_ASSOCIATION_RETAIN)
    }
    ...
    private static func portal(for window: NSWindow) -> WindowBrowserPortal {
        if let existing = objc_getAssociatedObject(window, &bmuxWindowBrowserPortalKey) as? WindowBrowserPortal {
        ...
        objc_setAssociatedObject(window, &bmuxWindowBrowserPortalKey, portal, .OBJC_ASSOCIATION_RETAIN)
