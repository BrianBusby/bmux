...
extension TerminalController {
    nonisolated func socketWorkerCloudVMResponse(
        ...
        case "vm.create":
            ...
            guard let idempotencyKey, !idempotencyKey.isEmpty else {
                return v2Error(
                    id: id,
                    code: "invalid_params",
                    message: "vm.create requires `idempotency_key`. Use `bmux vm new` instead of calling the socket method directly."
                )
            }
            return v2VmCall(id: id) {
        ...
        case "vm.status":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.status requires `id`. Run `bmux vm ls` to find one.")
            }
            return v2VmCall(id: id) {
        ...
        case "vm.snapshot":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.snapshot requires `id`. Run `bmux vm ls` to find one.")
            }
            let name = Self.socketWorkerString(params["name"])
            return v2VmCall(id: id) {
        ...
        case "vm.fork":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.fork requires `id`. Run `bmux vm ls` to find one.")
            }
            guard let idempotencyKey = Self.socketWorkerString(params["idempotency_key"]), !idempotencyKey.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.fork requires `idempotency_key`. Use `bmux vm fork` instead of calling the socket method directly.")
            }
            let name = Self.socketWorkerString(params["name"])
            return v2VmCall(id: id) {
        ...
        case "vm.restore":
            guard let snapshotId = Self.socketWorkerString(params["snapshot_id"]) ?? Self.socketWorkerString(params["snapshotId"]),
                ...
                return v2Error(id: id, code: "invalid_params", message: "vm.restore requires `snapshot_id`. Run `bmux vm snapshot <id>` first.")
            }
            guard let idempotencyKey = Self.socketWorkerString(params["idempotency_key"]), !idempotencyKey.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.restore requires `idempotency_key`. Use `bmux vm restore` instead of calling the socket method directly.")
            }
            let provider = Self.socketWorkerString(params["provider"])
            return v2VmCall(id: id) {
        ...
        case "vm.destroy":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.destroy requires `id`. Run `bmux vm ls` to find one, then `bmux vm rm <id>`.")
            }
            return v2VmCall(id: id) {
        ...
        case "vm.exec":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.exec requires `id`. Run `bmux vm ls` to find one.")
            }
            guard let command = Self.socketWorkerString(params["command"]), !command.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.exec requires `command`. From the CLI, use `bmux vm exec <id> -- <command>`.")
            }
        ...
        case "vm.ssh_info":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.ssh_info requires `id`. Run `bmux vm ls` to find one.")
            }
            return v2VmCall(id: id) {
        ...
        case "vm.attach_info":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.attach_info requires `id`. Run `bmux vm ls` to find one, then `bmux vm ssh <id>`.")
            }
        ...
        case "vm.sessions":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.sessions requires `id`. Run `bmux vm ls` to find one.")
            }
            return v2VmCall(id: id) {
        ...
        case "vm.session_attach_info":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "vm.session_attach_info requires `id`. Run `bmux vm ls` to find one.")
            }
    ...
    /// Handles the `remotes.*` socket methods backing `bmux remotes`. Each maps
    ...
    nonisolated func socketWorkerRemotesResponse(
        ...
        case "remotes.add":
            guard let name = Self.socketWorkerString(params["name"]), !name.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "remotes.add requires `name`. Use `bmux remotes add <name> --route host:port`.")
            }
        ...
        case "remotes.remove":
            guard let target = Self.socketWorkerString(params["target"]), !target.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "remotes.remove requires `target` (a remote name or deviceId). Run `bmux remotes list`.")
            }
            return v2VmCall(id: id) {
    ...
    /// Handles `aiAccounts.*` socket methods backing `bmux ai-accounts`.
    ...
    nonisolated func socketWorkerAIAccountsResponse(
        ...
        case "aiAccounts.remove":
            guard let accountID = Self.socketWorkerString(params["id"]), !accountID.isEmpty else {
                return v2Error(id: id, code: "invalid_params", message: "aiAccounts.remove requires `id`. Run `bmux ai-accounts list`.")
            }
            let teamID = Self.socketWorkerString(params["teamId"]) ?? Self.socketWorkerString(params["team_id"])
            return v2VmCall(id: id) {
