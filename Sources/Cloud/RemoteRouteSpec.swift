...
// Value types for `bmux remotes`: a parsed host:port attach spec and the
...
/// row per device for `bmux remotes list`.
struct RemoteSummary {
    ...
    let lastSeen: String?
    /// True for remotes added via `bmux remotes add` (device `labels.manual`),
    /// false for a Mac's own self-registration. `bmux remotes` only lists and
