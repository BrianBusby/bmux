    ...
    var description: String {
        ...
        case let .missingCodexTokens(openAIAPIKeyPresent):
            if openAIAPIKeyPresent {
                return "Codex auth contains an OpenAI API key but no OAuth tokens. Use `bmux ai-accounts upload openai-key` instead."
            }
        ...
        case let .missingAPIKey(provider):
            ...
            return "\(provider.displayName) upload requires `--key <value>` or \(envName) in the bmux app environment."
