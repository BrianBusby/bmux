...
/// window, so a single bmux tab can render the tmux window's full multi-pane
/// split layout side by side — with the native bmux pane chrome (each pane is a
...
final class RemoteTmuxWindowMirror {
    ...
    /// Tells tmux to size this session's windows to the rendered bmux area, so
    ...
    /// the bmux area doesn't change when tmux reflows).
