...
private struct FeedListView: View {
    ...
    private func filtered(_ items: [WorkstreamItem]) -> [WorkstreamItem] {
        ...
        case .activity:
            ...
            // terminal or the bmux notification system. Stop events
...
struct FeedItemRow: View, Equatable {
    ...
    private func chip(text: String, fg: Color, bg: Color, mono: Bool = false) -> some View {
        Text(text)
            .bmuxFont(size: 10, weight: .medium, monospacedDigit: mono)
            .foregroundColor(fg)
            ...
            if let primary = preview.primary {
                HStack(alignment: .firstTextBaseline, spacing: 6) {
                    if let sigil = preview.sigil {
                        Text(sigil)
                            .bmuxFont(size: 11, weight: .medium, design: .monospaced)
                            .foregroundColor(.orange)
                    }
                    Text(primary)
        ...
        HStack(spacing: iconSpacing) {
            if let leadingIcon {
                Image(systemName: leadingIcon)
                    .bmuxFont(size: iconSize, weight: .semibold)
            }
            Text(label)
                .bmuxFont(size: labelSize, weight: .semibold)
            if let trailingIcon {
...
private struct FeedMarkdownInlineText: View {
    ...
    var body: some View {
        ...
        Text(parsed)
            .bmuxFont(size: fontSize, weight: weight ?? .regular)
...
private struct PlanBodyView: View {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ForEach(Array(items.enumerated()), id: \.offset) { _, item in
                            HStack(alignment: .top, spacing: 5) {
                                Text("\(item.index).")
                                    .bmuxFont(size: 11, weight: .medium, monospacedDigit: true)
                                    .foregroundColor(.secondary)
    ...
    private func markdownText(
        ...
        if rendersMarkdown {
            ...
            Text(text)
                .bmuxFont(size: 11, weight: weight ?? .regular)
                .foregroundColor(color)
                .fixedSize(horizontal: false, vertical: true)
        }
    }
...
private struct QuestionActionArea: View {
    ...
    private static let customAnswerSelectionId = "__bmux_custom_answer__"
    ...
    private func longFormOptionCard(
        ...
        return Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                Text("\(index)")
                    .bmuxFont(size: 11, weight: .bold, monospacedDigit: true)
    ...
    private func questionBlock(index: Int, question: WorkstreamQuestionPrompt) -> some View {
        VStack(alignment: .leading, spacing: 5) {
            HStack(alignment: .top, spacing: 5) {
                Text("\(index).")
                    .bmuxFont(size: 11, weight: .semibold, monospacedDigit: true)
