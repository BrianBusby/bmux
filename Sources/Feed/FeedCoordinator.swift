...
final class FeedCoordinator: @unchecked Sendable {
    ...
    static let storeInstalledNotification = Notification.Name("bmux.feed.storeInstalled")
    ...
    private let pidWatcherQueue = DispatchQueue(
        label: "bmux.feed.pidWatcher", qos: .utility
    )
...
extension FeedCoordinator {
    ...
    /// attention path the `bmux hooks <agent> notification` hook uses. Doing
    ...
    /// terminal's BMUX_WORKSPACE_ID, a raw UUID) so a stale hook-session map
...
extension FeedCoordinator {
    ...
    /// `~/.bmuxterm/<agent>-hook-sessions.json` (written by
    /// `bmux <agent>-hook session-start`). Returns `true` if a match
...
/// Reads the per-agent hook session stores (`~/.bmuxterm/<agent>-hook-sessions.json`)
/// to map a feed `workstream_id` back to a bmux `(workspaceId, surfaceId)` pair.
/// The schema is the same one written by `bmux <agent>-hook session-start`.
enum FeedJumpResolver {
    ...
    /// through the existing bmux notification pathway so we don't need
...
extension Notification.Name {
    static let feedRequestFocus = Notification.Name("bmux.feedRequestFocus")
    static let feedRequestSendText = Notification.Name("bmux.feedRequestSendText")
}
...
private extension FeedCoordinator {
    ...
    private static func permissionNotificationCategoryId(for event: WorkstreamEvent) -> String {
        ...
        return suffix.isEmpty ? "BMUXFeedPermissionDeny" : "BMUXFeedPermission\(suffix)"
    }
...
private func makeFeedNotificationPolicyContext(
    ...
    let context = workspaceID.flatMap { appDelegate?.contextContainingTabId($0) }
        ?? appDelegate?.mainWindowContexts.values.first(where: { $0.bmuxConfigStore != nil })
    ...
    return FeedNotificationPolicyContext(
        ...
        hooks: context?.bmuxConfigStore?.notificationHooks(startingFrom: workspace?.isRemoteWorkspace == true ? nil : (normalizedFeedNotificationCWD(event.cwd) ?? workspace?.surfaceTabBarDirectory)) ?? [],
        globalConfigPath: context?.bmuxConfigStore?.globalConfigPath
    )
}
