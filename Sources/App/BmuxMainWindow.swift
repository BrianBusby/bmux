...
final class MainWindowHostingView<Content: View>: NSHostingView<Content> {
    ...
    override var fittingSize: NSSize { BmuxMainWindow.minimumContentSize }
    override var intrinsicContentSize: NSSize { BmuxMainWindow.minimumContentSize }
...
@MainActor
func configureBmuxMainWindowDragBehavior(_ window: NSWindow) {
    window.isMovableByWindowBackground = false
    window.isMovable = false
}

@MainActor
final class BmuxMainWindow: NSWindow {
    ...
    /// bmux creates its main window programmatically (never from a nib), so it
    ...
    /// `.fullScreenNone`) so a bmux main window can always enter a native
    ...
    /// bmux owns main-window placement: it persists and restores window frames
    ...
    /// arrangements. Because bmux never re-asserts the saved frame after wake,
    ...
    /// (``configureBmuxMainWindowDragBehavior`` sets `isMovable = false`) and can
    ...
    /// clamp (`AppDelegate`).
    ///
    /// The window is non-movable (``configureBmuxMainWindowDragBehavior`` sets
