...
struct BmuxCLIPathInstaller {
    ...
    enum InstallerError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .bundledCLIMissing(let expectedPath):
                return "Bundled bmux CLI was not found at \(expectedPath)."
            ...
            case .installVerificationFailed(let path):
                return "Installed symlink at \(path) did not point to the bundled bmux CLI."
    ...
    init(
        ...
        destinationURL: URL = URL(fileURLWithPath: "/usr/local/bin/bmux"),
        bundledCLIURLProvider: @escaping () -> URL? = {
            BmuxCLIPathInstaller.defaultBundledCLIURL()
        },
        expectedBundledCLIPath: String = BmuxCLIPathInstaller.defaultBundledCLIExpectedPath(),
    ...
    private static func defaultBundledCLIURL(bundle: Bundle = .main) -> URL? {
        bundle.resourceURL?.appendingPathComponent("bin/bmux", isDirectory: false)
    }
