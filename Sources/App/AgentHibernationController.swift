...
final class AgentHibernationController {
    ...
    private let timerQueue = DispatchQueue(label: "com.bmux.agent-hibernation", qos: .utility)
    ...
    private func terminateScopedProcessesForHibernation(record: AgentHibernationRecord) {
        ...
        for rawPID in record.processIDs.sorted(by: >) {
            ...
            guard let process = BmuxTopProcessSnapshot.processArgumentsAndEnvironment(for: rawPID),
                  process.matchesBMUXScope(workspaceId: record.key.workspaceId, surfaceId: record.key.panelId) else {
                continue
            }
