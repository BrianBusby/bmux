...
@MainActor
func bmuxCloseFocusedTerminalFindForEscape(event: NSEvent, appDelegate: AppDelegate) -> Bool {
    guard bmuxFindEventIsPlainEscape(event) else { return false }
...
#if DEBUG
    bmuxDebugLog("find.escape.close terminal panel=\(panel.id.uuidString.prefix(5))")
#endif
