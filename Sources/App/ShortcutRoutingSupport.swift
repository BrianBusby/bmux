...
func shouldDispatchBrowserArrowViaFirstResponderKeyDown(
    ...
    // Keep modified arrow routing narrow to avoid stealing bmux shortcuts such
...
/// combos. Cmd+Option+Arrow is excluded so it still reaches bmux's pane-focus
...
/// editable `NSTextView`s bmux hosts, the file-preview editor today, any
...
// BmuxTerminal lift; both callers (TabManager, overlay tests) are @MainActor.
@MainActor
@discardableResult
...
private enum BrowserFindCommandEquivalent: CaseIterable {
    ...
    var keepsBmuxBrowserFindBarOwnershipWhenVisible: Bool {
        switch self {
...
func bmuxIsWebInspectorClassName(_ className: String) -> Bool {
    className.contains("WKInspector") || className.contains("WebInspector")
}

func bmuxIsWebInspectorObject(_ object: NSObject) -> Bool {
    bmuxIsWebInspectorClassName(String(describing: type(of: object))) ||
        bmuxIsWebInspectorClassName(NSStringFromClass(type(of: object)))
}
...
func bmuxIsLikelyWebInspectorResponder(_ responder: NSResponder?) -> Bool {
    guard let responder else { return false }
...
/// before bmux's menu fallback. Rich web apps often implement copy/cut/select-all
...
func shouldRouteBrowserDocumentEditingCommandEquivalentThroughWebContentFirst(
    ...
    if bmuxIsLikelyWebInspectorResponder(responder) {
        return false
    }
...
/// For browser content, let the page try browser-local Find-family commands before bmux's menu fallback.
/// Cmd+F is excluded because bmux chooses terminal, browser, or right-sidebar
/// find from the current focus owner.
func shouldRouteBrowserFindCommandEquivalentThroughWebContentFirst(
    _ event: NSEvent,
    responder: NSResponder? = nil,
    owningWebView: BmuxWebView? = nil
) -> Bool {
    ...
    if shortcut.keepsBmuxBrowserFindBarOwnershipWhenVisible,
       let owningWebView {
...
func bmuxOwningGhosttyView(for responder: NSResponder?) -> GhosttyNSView? {
    ...
    if let view = responder as? NSView,
       let ghosttyView = bmuxOwningGhosttyView(for: view) {
        return ghosttyView
    }
    ...
    if let textView = responder as? NSTextView {
        if textView.isFieldEditor,
           ...
           let ghosttyView = bmuxOwningGhosttyView(for: ownerView) {
            return ghosttyView
        }
    }
...
func bmuxFieldEditorOwnerView(_ editor: NSTextView) -> NSView? {
    guard editor.isFieldEditor else { return nil }
    if let owner = bmuxTrackedFindFieldEditorOwner(editor) { return owner }
...
private func bmuxOwningGhosttyView(for view: NSView) -> GhosttyNSView? {
