...
final class MenuBarProfilingProgressWindowController: NSWindowController {
    ...
    private init() {
        ...
        window.title = String(localized: "statusMenu.profiling.title", defaultValue: "Profiling bmux")
    ...
    func startProfiling(
        ...
        do {
            let outputLog = try makeTemporaryLogFile(prefix: "bmux-profile-output")
            let errorLog = try makeTemporaryLogFile(prefix: "bmux-profile-error")
    ...
    private func buildInterface() {
        ...
        titleLabel.stringValue = String(localized: "statusMenu.profiling.reviewTitle", defaultValue: "Send a bmux profile")
        ...
        permissionLabel.stringValue = String(
            ...
            defaultValue: "macOS may ask for administrator permission because Instruments samples the running bmux process."
        )
    ...
    private func parseOutputURL(from text: String) {
        for line in text.components(separatedBy: .newlines) {
            if let range = line.range(of: "bmux profiling capture written to ") {
