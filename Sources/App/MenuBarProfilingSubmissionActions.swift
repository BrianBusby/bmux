...
extension MenuBarProfilingProgressWindowController {
    ...
    private func makePrivateSubmitInputs(email: String, note: String) throws -> (replyToFile: URL, noteFile: URL) {
        var createdURLs: [URL] = []
        do {
            let replyToFile = try writePrivateSubmitInput(prefix: "bmux-profile-reply-to", text: email)
            createdURLs.append(replyToFile)
            let noteFile = try writePrivateSubmitInput(prefix: "bmux-profile-note", text: note)
    ...
    private func runSubmitProcess(_ process: Process) {
        do {
            let outputLog = try makeTemporaryLogFile(prefix: "bmux-profile-submit-output")
            let errorLog = try makeTemporaryLogFile(prefix: "bmux-profile-submit-error")
