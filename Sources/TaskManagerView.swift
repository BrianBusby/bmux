...
struct BmuxTaskManagerView: View {
    @Bindable var model: BmuxTaskManagerModel
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            BmuxTaskManagerListView(
                ...
                actions: BmuxTaskManagerRowActions.bound(to: model)
            )
        }
    ...
    private var toolbar: some View {
        HStack(spacing: 12) {
            Text(String(localized: "taskManager.title", defaultValue: "Task Manager"))
                .bmuxFont(.title3, weight: .semibold)
    ...
    private var summary: some View {
        HStack(spacing: 24) {
            metric(
                ...
                value: BmuxTaskManagerFormat.cpu(model.snapshot.total.cpuPercent)
            )
            metric(
                title: String(localized: "taskManager.summary.memory", defaultValue: "Memory"),
                value: BmuxTaskManagerFormat.bytes(model.snapshot.total.memoryBytes)
            )
            if let memoryDiagnostic = model.snapshot.memoryDiagnostic {
                metric(
                    ...
                    value: BmuxTaskManagerFormat.bytes(memoryDiagnostic.appFootprintBytes)
                )
                metric(
                    title: String(localized: "taskManager.summary.childRSS", defaultValue: "Child RSS"),
                    value: BmuxTaskManagerFormat.bytes(memoryDiagnostic.childRSSBytes)
                )
            }
            metric(
    ...
    private func metric(title: String, value: String) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            Text(value)
                .bmuxFont(.body, weight: .semibold, design: .monospaced)
                .monospacedDigit()
        }
    }
    ...
    private func sortHeader(
        title: String,
        column: BmuxTaskManagerSortOrder.Column,
        width: CGFloat? = nil,
    ...
    private func sortIndicator(for column: BmuxTaskManagerSortOrder.Column) -> some View {
...
/// `@Observable` `BmuxTaskManagerModel`. Matches the
...
struct BmuxTaskManagerRowActions {
    let viewWorkspace: @MainActor (BmuxTaskManagerRow) -> Void
    let viewTerminal: @MainActor (BmuxTaskManagerRow) -> Void
    let killProcess: @MainActor (BmuxTaskManagerRow) -> Void
    let activate: @MainActor (BmuxTaskManagerRow) -> Void

    @MainActor
    static func bound(to model: BmuxTaskManagerModel) -> BmuxTaskManagerRowActions {
        BmuxTaskManagerRowActions(
...
/// `BmuxTaskManagerModel`. Combined with the `Equatable` conformance on
/// `BmuxTaskManagerRowView`, this stops the 3 s refresh timer from
/// invalidating every row on every tick.
struct BmuxTaskManagerListView: View {
...
private struct BmuxTaskManagerSectionHeaderView: View, Equatable {
    let title: String

    static func == (lhs: BmuxTaskManagerSectionHeaderView, rhs: BmuxTaskManagerSectionHeaderView) -> Bool {
        lhs.title == rhs.title
    }
...
private struct BmuxTaskManagerLoadingView: View {
    var body: some View {
        VStack(spacing: 10) {
            ProgressView()
                .controlSize(.regular)
...
private struct BmuxTaskManagerMessageView: View {
    let title: String
    let detail: String
...
struct BmuxTaskManagerRowView: View, Equatable {
    ...
    static func == (lhs: BmuxTaskManagerRowView, rhs: BmuxTaskManagerRowView) -> Bool {
    ...
    private var rowContent: some View {
        HStack(spacing: 8) {
            ...
            Text(BmuxTaskManagerFormat.cpu(row.resources.cpuPercent))
                .frame(width: 82, alignment: .trailing)
            Text(BmuxTaskManagerFormat.bytes(row.resources.memoryBytes))
                .frame(width: 96, alignment: .trailing)
            Text("\(row.resources.processCount)")
                .frame(width: 70, alignment: .trailing)
        }
        .bmuxFont(size: 12.5, design: .default)
        .monospacedDigit()
        .padding(.horizontal, 16)
        .padding(.vertical, 3)
