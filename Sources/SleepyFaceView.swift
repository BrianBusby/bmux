...
struct SleepyFaceView: View {
    ...
    private func draw(in ctx: inout GraphicsContext, size: CGSize, time t: Double, config: SleepyModeConfig, agents: SleepyAgentCounts, status: SleepyStatusSample, reactions: SleepyReactions) {
        ...
        if config.mascot == .logoFace {
            ...
            drawBmuxLogo(in: &ctx, center: center, mascotRows: rows.count, pixel: pixel, time: t, palette: palette)
        }
    ...
    /// logoFace: bmux chevron `>` as the left eye, a `-` dash as the (winking)
    ...
    private func drawLogoFace(in ctx: inout GraphicsContext, center: CGPoint, pixel: CGFloat, breath: Double, time t: Double, palette: [Character: Color], ink: Color) {
        ...
        let chevW = SleepyArt.bmuxLogo.first?.count ?? 11
        let chevH = SleepyArt.bmuxLogo.count
        let gap = 3 * eyePixel

        // Left eye: bmux chevron.
        let leftOrigin = CGPoint(
            x: (center.x - gap - CGFloat(chevW) * eyePixel).rounded(),
            y: (center.y - CGFloat(chevH) / 2 * eyePixel).rounded()
        )
        drawSprite(in: &ctx, rows: SleepyArt.bmuxLogo, palette: palette, origin: leftOrigin, pixel: eyePixel)
    ...
    // MARK: - bmux logo, moon, stars, z's

    private func drawBmuxLogo(in ctx: inout GraphicsContext, center: CGPoint, mascotRows: Int, pixel: CGFloat, time t: Double, palette: [Character: Color]) {
        let cols = SleepyArt.bmuxLogo.first?.count ?? 9
        ...
        drawSprite(in: &ctx, rows: SleepyArt.bmuxLogo, palette: palette, origin: origin, pixel: logoPixel, alpha: pulse)
    }
