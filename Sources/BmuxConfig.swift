...
extension CodingUserInfoKey {
    static let bmuxWorkspaceColorDefaults = CodingUserInfoKey(rawValue: "bmuxWorkspaceColorDefaults")!
}

struct BmuxConfigFile: Codable, Sendable {
    var actions: [String: BmuxConfigActionDefinition]
    var ui: BmuxConfigUIDefinition?
    var notifications: BmuxNotificationConfigDefinition?
    var agentChat: BmuxAgentChatConfigDefinition?
    var newWorkspaceCommand: String?
    var surfaceTabBarButtons: [BmuxSurfaceTabBarButton]?
    var commands: [BmuxCommandDefinition]
    var vault: BmuxVaultConfigDefinition?
    var workspaceGroups: BmuxConfigWorkspaceGroupsDefinition?
    ...
    init(
        actions: [String: BmuxConfigActionDefinition] = [:],
        ui: BmuxConfigUIDefinition? = nil,
        notifications: BmuxNotificationConfigDefinition? = nil,
        agentChat: BmuxAgentChatConfigDefinition? = nil,
        newWorkspaceCommand: String? = nil,
        surfaceTabBarButtons: [BmuxSurfaceTabBarButton]? = nil,
        commands: [BmuxCommandDefinition] = [],
        vault: BmuxVaultConfigDefinition? = nil,
        workspaceGroups: BmuxConfigWorkspaceGroupsDefinition? = nil
    ) {
    ...
    init(from decoder: Decoder) throws {
        ...
        let decodedActions = try container.decodeIfPresent(
            [String: BmuxConfigActionDefinition].self,
            forKey: .actions
        ) ?? [:]
        ...
        ui = try container.decodeIfPresent(BmuxConfigUIDefinition.self, forKey: .ui)
        notifications = try container.decodeIfPresent(BmuxNotificationConfigDefinition.self, forKey: .notifications)
        agentChat = try container.decodeIfPresent(BmuxAgentChatConfigDefinition.self, forKey: .agentChat)
        ...
        commands = try container.decodeIfPresent([BmuxCommandDefinition].self, forKey: .commands) ?? []
        vault = try container.decodeIfPresent(BmuxVaultConfigDefinition.self, forKey: .vault)
        workspaceGroups = try container.decodeIfPresent(
            BmuxConfigWorkspaceGroupsDefinition.self,
            forKey: .workspaceGroups
        )
    }

    private static func normalizedActions(
        _ decodedActions: [String: BmuxConfigActionDefinition],
        codingPath: [CodingKey]
    ) throws -> [String: BmuxConfigActionDefinition] {
        var actions: [String: BmuxConfigActionDefinition] = [:]
        ...
        for (rawID, action) in decodedActions {
            ...
            let canonicalID = BmuxSurfaceTabBarBuiltInAction(configID: id)?.configID ?? id
    ...
    private static func validatedSurfaceTabBarButtons(
        ...
        codingPath: [CodingKey]
    ) throws -> [BmuxSurfaceTabBarButton] {
        var seen = Set<String>()
...
struct BmuxConfigWorkspaceGroupsDefinition: Codable, Sendable, Equatable {
    var byCwd: [String: BmuxConfigWorkspaceGroupEntry]?
...
struct BmuxConfigWorkspaceGroupEntry: Codable, Sendable, Equatable {
    var color: String?
    var icon: String?
    var contextMenu: [BmuxConfigContextMenuItem]?
...
struct BmuxResolvedWorkspaceGroupConfig: Sendable, Equatable {
    let originalKey: String
    let normalizedKey: String
    let isGlob: Bool
    let color: String?
    let iconSymbol: String?
    let contextMenuItems: [BmuxResolvedConfigContextMenuItem]
...
enum BmuxNotificationHooksMode: String, Codable, Sendable, Hashable {
    case append
    case replace
}

struct BmuxNotificationConfigDefinition: Codable, Sendable, Hashable {
    var hooks: [BmuxNotificationHookDefinition]?
    var hooksMode: BmuxNotificationHooksMode?
...
struct BmuxNotificationHookDefinition: Codable, Sendable, Hashable {
...
struct BmuxResolvedNotificationHook: Sendable, Hashable {
    let id: String
    let command: String
    let timeoutSeconds: TimeInterval
    let sourcePath: String?
    let cwd: String
    let trustDescriptor: BmuxActionTrustDescriptor?
    ...
    init(
        ...
        sourcePath: String?,
        cwd: String,
        trustDescriptor: BmuxActionTrustDescriptor? = nil
    ) {
        self.id = id
        self.command = command
        self.timeoutSeconds = timeoutSeconds
    ...
    static func == (lhs: BmuxResolvedNotificationHook, rhs: BmuxResolvedNotificationHook) -> Bool {
        lhs.id == rhs.id &&
...
enum BmuxConfigTerminalCommandTarget: String, Codable, Sendable, Hashable {
    case currentTerminal
    case newTabInCurrentPane

    static let defaultForActions: BmuxConfigTerminalCommandTarget = .newTabInCurrentPane
}
...
enum BmuxConfigAgentKind: Sendable, Hashable {
    case codex
    case claudeCode
    case opencode
    case custom(String)
    ...
    var defaultIcon: BmuxButtonIcon {
        switch self {
        case .codex:
...
extension BmuxConfigAgentKind: Codable {
    init(from decoder: Decoder) throws {
...
enum BmuxButtonIcon: Codable, Sendable, Hashable {
    case symbol(String)
    ...
    func bonsplitIcon(
        ...
        case .imagePath(let path):
            guard let preparedImage = Self.preparedImageAsset(
                ...
                NSLog("[BmuxConfig] icon image path is not allowed: %@", path)
    ...
    func resolvingRelativeImagePath(relativeToConfig configSourcePath: String?) -> BmuxButtonIcon {
    ...
    private static func preparedImageAsset(
        ...
        guard let data = FileManager.default.contents(atPath: resolvedPath) else {
            NSLog("[BmuxConfig] icon image does not exist: %@", resolvedPath)
            return nil
        }
        guard data.count <= maxImageBytes else {
            NSLog("[BmuxConfig] icon image is too large: %@", resolvedPath)
            return nil
        }
        if looksLikeSVGPath(resolvedPath), !isSafeSVG(data: data) {
            NSLog("[BmuxConfig] icon SVG contains unsupported content: %@", resolvedPath)
            return nil
        }
    ...
    static func projectRoot(forConfigPath configPath: String) -> String {
        ...
        if (configDir as NSString).lastPathComponent == ".bmux" {
...
enum BmuxSurfaceTabBarButtonAction: Sendable, Hashable {
    case builtIn(BmuxSurfaceTabBarBuiltInAction)
    case command(String)
    case agent(BmuxConfigAgentKind, args: String?)
    case workspaceCommand(String)
    case workspace(BmuxWorkspaceDefinition, restart: BmuxRestartBehavior?)
    ...
    var defaultButtonIcon: BmuxButtonIcon {
        switch self {
        case .builtIn(let action):
    ...
    var inlineWorkspace: (definition: BmuxWorkspaceDefinition, restart: BmuxRestartBehavior?)? {
...
struct BmuxSurfaceTabBarButton: Codable, Sendable, Hashable, Identifiable {
    var id: String
    var title: String?
    var icon: BmuxButtonIcon?
    var tooltip: String?
    var action: BmuxSurfaceTabBarButtonAction
    var confirm: Bool?
    var terminalCommandTarget: BmuxConfigTerminalCommandTarget?
    ...
    static let newTerminal = actionReference(BmuxSurfaceTabBarBuiltInAction.newTerminal.configID)
    ...
    static let defaults: [BmuxSurfaceTabBarButton] = [
        .newTerminal,
        .newBrowser,
        .splitRight,
        .splitDown
    ...
    static func builtIn(
        _ action: BmuxSurfaceTabBarBuiltInAction,
        id: String? = nil,
        title: String? = nil,
        icon: BmuxButtonIcon? = nil,
        tooltip: String? = nil
    ) -> BmuxSurfaceTabBarButton {
        BmuxSurfaceTabBarButton(
    ...
    var resolvedTerminalCommandTarget: BmuxConfigTerminalCommandTarget {
        terminalCommandTarget ?? BmuxConfigTerminalCommandTarget.defaultForActions
    }
    ...
    var inlineWorkspaceSyntheticCommand: BmuxCommandDefinition? {
    ...
    init(
        ...
        icon: BmuxButtonIcon? = nil,
        tooltip: String? = nil,
        action: BmuxSurfaceTabBarButtonAction,
        confirm: Bool? = nil,
        terminalCommandTarget: BmuxConfigTerminalCommandTarget? = nil,
    ...
    init(from decoder: Decoder) throws {
        if let legacy = try? decoder.singleValueContainer().decode(String.self) {
            ...
            self = BmuxSurfaceTabBarButton(
                id: BmuxSurfaceTabBarBuiltInAction(configID: trimmed)?.configID ?? trimmed,
                action: .actionReference(BmuxSurfaceTabBarBuiltInAction(configID: trimmed)?.configID ?? trimmed)
            )
            return
        }
        ...
        let explicitIcon = try container.decodeIfPresent(BmuxButtonIcon.self, forKey: .icon)
        ...
        let rawAgent = try container.decodeIfPresent(BmuxConfigAgentKind.self, forKey: .agent)
        ...
        terminalCommandTarget = try container.decodeIfPresent(BmuxConfigTerminalCommandTarget.self, forKey: .target)
        ...
        if let rawType {
            ...
            case "workspace":
                ...
                let definition = try container.decode(BmuxWorkspaceDefinition.self, forKey: .workspace)
                let restart = try container.decodeIfPresent(BmuxRestartBehavior.self, forKey: .restart)
            ...
            guard let builtIn = BmuxSurfaceTabBarBuiltInAction(configID: rawBuiltin) else {
                throw DecodingError.dataCorruptedError(
                    forKey: .builtin,
                    in: container,
            ...
            action = .actionReference(rawAction)
                  ...
                  let builtIn = BmuxSurfaceTabBarBuiltInAction(configID: explicitId) {
            action = .builtIn(builtIn)
        } else {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
    ...
    func resolved(
        actions: [String: BmuxResolvedConfigAction],
        codingPath: [CodingKey]
    ) throws -> BmuxSurfaceTabBarButton {
        ...
        let resolvedIdentifier = BmuxSurfaceTabBarBuiltInAction(configID: identifier)?.configID ?? identifier
        if let definition = actions[resolvedIdentifier] {
            return BmuxSurfaceTabBarButton(
                id: id,
        ...
        if let builtIn = BmuxSurfaceTabBarBuiltInAction(configID: identifier) {
            return BmuxSurfaceTabBarButton(
                id: id,
                title: title,
                icon: icon,
...
struct BmuxResolvedConfigAction: Identifiable, Sendable, Hashable {
    var id: String
    var title: String
    var subtitle: String?
    var keywords: [String]
    ...
    func applying(
        _ definition: BmuxConfigActionDefinition,
        sourcePath: String?
    ...
    static func builtIn(_ builtIn: BmuxSurfaceTabBarBuiltInAction) -> BmuxResolvedConfigAction {
        let title: String
        let keywords: [String]
        switch builtIn {
        case .newWorkspace:
        ...
        return BmuxResolvedConfigAction(
            id: builtIn.configID,
            title: title,
            subtitle: String(localized: "command.bmuxConfig.builtInSubtitle", defaultValue: "bmux"),
            keywords: keywords,
            palette: true,
            shortcut: nil,
    ...
    private static func defaultTitle(for id: String, action: BmuxSurfaceTabBarButtonAction) -> String {
        switch action {
        case .agent(let agent, _):
            switch agent {
            case .codex:
                return String(localized: "command.bmuxConfig.defaultCodexTitle", defaultValue: "Codex")
            case .claudeCode:
                return String(localized: "command.bmuxConfig.defaultClaudeCodeTitle", defaultValue: "Claude Code")
            case .opencode:
                return String(localized: "command.bmuxConfig.defaultOpenCodeTitle", defaultValue: "OpenCode")
            case .custom(let name):
                return name
            }
        case .command:
...
struct BmuxCommandDefinition: Codable, Sendable, Identifiable {
    var name: String
    var description: String?
    var keywords: [String]?
    var restart: BmuxRestartBehavior?
    var workspace: BmuxWorkspaceDefinition?
    var command: String?
    var confirm: Bool?

    var id: String {
        "bmux.config.command." + (name.addingPercentEncoding(withAllowedCharacters: .alphanumerics) ?? name)
    }
    ...
    init(
        ...
        keywords: [String]? = nil,
        restart: BmuxRestartBehavior? = nil,
        workspace: BmuxWorkspaceDefinition? = nil,
        command: String? = nil,
        confirm: Bool? = nil
    ) {
        self.name = name
    ...
    init(from decoder: Decoder) throws {
        ...
        restart = try container.decodeIfPresent(BmuxRestartBehavior.self, forKey: .restart)
        workspace = try container.decodeIfPresent(BmuxWorkspaceDefinition.self, forKey: .workspace)
...
indirect enum BmuxLayoutNode: Codable, Sendable, Hashable {
    case pane(BmuxPaneDefinition)
    case split(BmuxSplitDefinition)
    ...
    init(from decoder: Decoder) throws {
        ...
        if hasPane && hasDirection {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
                    debugDescription: "BmuxLayoutNode must not contain both 'pane' and 'direction' keys"
                )
            )
        }

        if hasPane {
            let pane = try container.decode(BmuxPaneDefinition.self, forKey: .pane)
            self = .pane(pane)
        } else if hasDirection {
            let splitDef = try BmuxSplitDefinition(from: decoder)
            self = .split(splitDef)
        } else {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
                    debugDescription: "BmuxLayoutNode must contain either a 'pane' key or a 'direction' key"
                )
            )
        }
    }
...
struct BmuxSplitDefinition: Codable, Sendable, Hashable {
    var direction: BmuxSplitDirection
    var split: Double?
    var children: [BmuxLayoutNode]

    init(direction: BmuxSplitDirection, split: Double? = nil, children: [BmuxLayoutNode]) {
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        direction = try container.decode(BmuxSplitDirection.self, forKey: .direction)
        split = try container.decodeIfPresent(Double.self, forKey: .split)
        children = try container.decode([BmuxLayoutNode].self, forKey: .children)
        if children.count != 2 {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
...
enum BmuxSplitDirection: String, Codable, Sendable {
    case horizontal
    case vertical
}

struct BmuxPaneDefinition: Codable, Sendable, Hashable {
    var surfaces: [BmuxSurfaceDefinition]

    init(surfaces: [BmuxSurfaceDefinition]) {
        self.surfaces = surfaces
    }

    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        surfaces = try container.decode([BmuxSurfaceDefinition].self, forKey: .surfaces)
        if surfaces.isEmpty {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
...
struct BmuxSurfaceDefinition: Codable, Sendable, Hashable {
    var type: BmuxSurfaceType
    var name: String?
    var command: String?
    var cwd: String?
...
enum BmuxSurfaceType: String, Codable, Sendable {
    case terminal
    case browser
    case project
}
...
struct BmuxResolvedRepoAgentLauncher: Identifiable, Sendable, Hashable {
    let id: String
    let title: String
    let icon: BmuxButtonIcon?
    let iconSourcePath: String?
    let tooltip: String?
    let action: BmuxResolvedConfigAction
}

struct BmuxConfigIssue: Identifiable, Equatable, Sendable {
...
@MainActor
final class BmuxConfigStore: ObservableObject {
    private static let defaultNewWorkspaceContextMenu: [BmuxConfigContextMenuItem] = [
        .action(BmuxConfigContextMenuActionItem(action: BmuxSurfaceTabBarBuiltInAction.newWorkspace.configID)),
    ]

    @Published private(set) var loadedCommands: [BmuxCommandDefinition] = []
    @Published private(set) var loadedActions: [BmuxResolvedConfigAction] = []
    @Published private(set) var newWorkspaceCommandName: String?
    @Published private(set) var newWorkspaceActionID: String?
    @Published private(set) var newWorkspaceContextMenuItems: [BmuxResolvedConfigContextMenuItem] = []
    @Published private(set) var newWorkspaceContextMenuIsConfigured = false
    @Published private(set) var repoAgentLauncherItems: [BmuxResolvedRepoAgentLauncher] = []
    @Published private(set) var newWorkspaceMenuSectionOrder: BmuxNewWorkspaceMenuSectionOrder = .default
    @Published private(set) var agentChat: BmuxAgentChatConfiguration = .default
    ...
    /// configured.
    @Published private(set) var workspaceGroupConfigs: [BmuxResolvedWorkspaceGroupConfig] = []
    @Published private(set) var surfaceTabBarButtons: [BmuxSurfaceTabBarButton] = BmuxSurfaceTabBarButton.defaults
    @Published private(set) var notificationHooks: [BmuxResolvedNotificationHook] = []
    @Published private(set) var configurationIssues: [BmuxConfigIssue] = []
    ...
    private(set) var actionLookup: [String: BmuxResolvedConfigAction] = [:]
    ...
    nonisolated private static func defaultGlobalConfigPath() -> String {
        ...
        return (home as NSString).appendingPathComponent(".config/bmux/bmux.json")
    }

    private struct ActionEntry {
        let definition: BmuxConfigActionDefinition
        let sourcePath: String?
    }
    ...
    private struct ResolvedSurfaceTabBarButtons {
        let buttons: [BmuxSurfaceTabBarButton]
        let terminalCommandSourcePaths: [String: String]
    }

    private struct ResolvedContextMenuItems {
        let items: [BmuxResolvedConfigContextMenuItem]
        let issues: [BmuxConfigIssue]
    }

    private struct NewWorkspaceCommandResolution {
        let command: BmuxResolvedCommand?
        let issue: BmuxConfigIssue?
    }
    ...
    private var surfaceTabBarWorkspaceCommands: [String: BmuxResolvedCommand] = [:]
    private var resolvedNewWorkspaceCommandCache: BmuxResolvedCommand?
    private var resolvedNewWorkspaceActionCache: BmuxResolvedConfigAction?
    ...
    // The global config and hook files use BmuxFileWatch.FileWatcher.
    ...
    private let watchQueue = DispatchQueue(label: "com.bmux.config-file-watch")
    ...
    private static func searchDirectoryForLocalConfigPath(_ path: String) -> String {
        ...
        if (configDirectory as NSString).lastPathComponent == ".bmux" {
    ...
    init(
        globalConfigPath: String = BmuxConfigStore.defaultGlobalConfigPath(),
        ...
        NotificationCenter.default.publisher(for: BmuxActionTrust.didChangeNotification)
    ...
    func notificationHooks(startingFrom directory: String?) -> [BmuxResolvedNotificationHook] {
        let globalConfig = parseConfig(at: globalConfigPath).config
        let localConfigs: [(path: String, config: BmuxConfigFile)]
        if let directory, !directory.isEmpty {
            localConfigs = findBmuxConfigHierarchy(startingFrom: directory).compactMap { path in
    ...
    private func resolvedLocalConfigPath(startingFrom directory: String) -> String {
        findBmuxConfig(startingFrom: directory)
            ?? defaultLocalConfigPath(startingFrom: directory)
    }

    private func defaultLocalConfigPath(startingFrom directory: String) -> String {
        (((directory as NSString).appendingPathComponent(".bmux") as NSString)
            .appendingPathComponent("bmux.json"))
    }

    private func findBmuxConfig(startingFrom directory: String) -> String? {
        var current = directory
        let fs = FileManager.default
        while true {
            let candidates = [
                ((current as NSString).appendingPathComponent(".bmux") as NSString)
                    .appendingPathComponent("bmux.json"),
    ...
    private func findBmuxConfigHierarchy(startingFrom directory: String) -> [String] {
        var current = directory
        let fs = FileManager.default
        var paths: [String] = []
        while true {
    ...
    func loadAll() {
        ...
        var configuredNewWorkspaceContextMenu: [BmuxConfigContextMenuItem]?
        var configuredNewWorkspaceContextMenuSourcePath: String?
        var configuredNewWorkspaceMenuSectionOrder: BmuxNewWorkspaceMenuSectionOrder?
        var configuredSurfaceTabBarButtons: [BmuxSurfaceTabBarButton]?
        ...
        let configuredButtons = configuredSurfaceTabBarButtons ?? BmuxSurfaceTabBarButton.defaults
        let defaultResolvedButtons = (try? BmuxSurfaceTabBarButton.defaults.map {
        ...
        agentChat = BmuxAgentChatConfiguration.resolved(
    ...
    private func resolvedLocalNotificationHookPaths(fallbackLocalPath: String?) -> [String] {
        if let searchDirectory = localConfigSearchDirectory {
            var paths = findBmuxConfigHierarchy(startingFrom: searchDirectory)
    ...
    private func resolveNotificationHooks(
        globalConfig: BmuxConfigFile?,
        localConfigs: [(path: String, config: BmuxConfigFile)]
    ) -> [BmuxResolvedNotificationHook] {
        var hooks: [BmuxResolvedNotificationHook] = []
    ...
    private func resolvedNotificationHooks(
        _ definitions: [BmuxNotificationHookDefinition],
        sourcePath: String
    ) -> [BmuxResolvedNotificationHook] {
        let cwd = BmuxButtonIcon.projectRoot(forConfigPath: sourcePath)
        ...
        return definitions.compactMap { definition in
            ...
            if isGlobalHook {
                ...
                trustDescriptor = BmuxActionTrustDescriptor(
            ...
            return BmuxResolvedNotificationHook(
                id: definition.id,
                command: definition.command,
    ...
    private func actionEntries(
        from actions: [String: BmuxConfigActionDefinition],
        sourcePath: String?
    ) -> [String: ActionEntry] {
    ...
    private func resolvedActionRegistry(
        ...
        var registry = Dictionary(
            uniqueKeysWithValues: BmuxSurfaceTabBarBuiltInAction.allCases.map {
                ($0.configID, BmuxResolvedConfigAction.builtIn($0))
            }
        )

        func apply(_ entries: [String: ActionEntry]) {
            for (id, entry) in entries {
                let registryID = BmuxSurfaceTabBarBuiltInAction(configID: id)?.configID ?? id
                if let existing = registry[registryID] {
                ...
                } else if let resolved = BmuxResolvedConfigAction.fromDefinition(
                    id: id,
                    ...
                    NSLog("[BmuxConfig] action '%@' ignored because it does not define a runnable action", id)
                }
            }
        }
        ...
        for command in commands where registry[command.id] == nil {
            ...
            registry[command.id] = BmuxResolvedConfigAction(
                id: command.id,
                title: String(
                    localized: "command.bmuxConfig.customTitle",
                    defaultValue: "Custom: \(sanitizeConfigText(command.name))"
                ),
                subtitle: command.description.map { sanitizeConfigText($0) }
                    ?? String(localized: "command.bmuxConfig.subtitle", defaultValue: "bmux.json"),
    ...
    private func resolvedSurfaceTabBarButtons(
        ...
        var resolvedButtons: [BmuxSurfaceTabBarButton] = []
        ...
        for button in buttons {
            do {
                ...
                NSLog("[BmuxConfig] %@ ignored: %@", settingName, String(describing: error))
                return nil
            }
        }
    ...
    private func resolvedSurfaceTabBarButton(
        _ button: BmuxSurfaceTabBarButton,
        actions: [String: BmuxResolvedConfigAction]
        ...
        if let entry = actions[resolvedIdentifier] {
            let resolvedButton = BmuxSurfaceTabBarButton(
                id: button.id,
        ...
        if let builtIn = BmuxSurfaceTabBarBuiltInAction(configID: identifier) {
            return ResolvedSurfaceTabBarButtonEntry(
                button: BmuxSurfaceTabBarButton(
                    id: button.id,
                    title: button.title,
                    icon: button.icon,
    ...
    private func resolvedSurfaceTabBarWorkspaceCommands(
    ...
    ) -> (buttons: [BmuxSurfaceTabBarButton], workspaceCommands: [String: BmuxResolvedCommand]) {
        var visibleButtons: [BmuxSurfaceTabBarButton] = []
        var workspaceCommands: [String: BmuxResolvedCommand] = [:]
        ...
        for button in buttons {
            ...
            guard let command = resolvedWorkspaceCommand(
                ...
                NSLog(
                    "[BmuxConfig] surfaceTabBarButtons action '%@' hidden because workspace command '%@' is unavailable",
                    button.id,
                    commandName
                )
                continue
    ...
    func resolvedNewWorkspaceCommand() -> BmuxResolvedCommand? {
        resolvedNewWorkspaceCommandCache
    }

    func resolvedNewWorkspaceAction() -> BmuxResolvedConfigAction? {
        resolvedNewWorkspaceActionCache
    }

    func resolvedAction(id: String) -> BmuxResolvedConfigAction? {
        actionLookup[canonicalActionID(id)]
    }

    func paletteCustomActions() -> [BmuxResolvedConfigAction] {
        let builtInIDs = Set(BmuxSurfaceTabBarBuiltInAction.allCases.map(\.configID))
        return loadedActions.filter { action in
            action.palette && !builtInIDs.contains(action.id)
        }
    }

    func shortcutActions() -> [BmuxResolvedConfigAction] {
        let builtInIDs = Set(BmuxSurfaceTabBarBuiltInAction.allCases.map(\.configID))
        return loadedActions.filter { action in
    ...
    private func resolvedConfiguredNewWorkspaceAction(
        ...
        if let actionID {
            ...
            guard let action = actions[resolvedActionID] else {
                ...
                NSLog("[BmuxConfig] %@", issue.logMessage)
        ...
        return NewWorkspaceActionResolution(
            action: BmuxResolvedConfigAction(
    ...
    func resolveWorkspaceGroupConfig(forCwd cwd: String?) -> BmuxResolvedWorkspaceGroupConfig? {
        guard let cwd, !cwd.isEmpty, !workspaceGroupConfigs.isEmpty else { return nil }
        let normalizedCwd = Self.normalizeAbsolutePath(cwd)
        var best: (BmuxResolvedWorkspaceGroupConfig, Int)?
    ...
    private static func cwdEntryMatches(
        _ entry: BmuxResolvedWorkspaceGroupConfig,
        cwd: String
    ) -> Bool {
    ...
    private func resolveWorkspaceGroupConfigsFromLayers(
        ...
        commands: [BmuxCommandDefinition],
        sourcePaths: [String: String],
        issues: inout [BmuxConfigIssue]
    ) -> [BmuxResolvedWorkspaceGroupConfig] {
        var resolved: [String: BmuxResolvedWorkspaceGroupConfig] = [:]
    ...
    private func resolveWorkspaceGroupConfigEntry(
        key: String,
        entry: BmuxConfigWorkspaceGroupEntry,
        sourcePath: String,
        actions: [String: BmuxResolvedConfigAction],
        commands: [BmuxCommandDefinition],
        sourcePaths: [String: String],
        ...
        return BmuxResolvedWorkspaceGroupConfig(
            originalKey: trimmed,
    ...
    private func resolvedConfigContextMenuItems(
        _ configuredItems: [BmuxConfigContextMenuItem]?,
        actions: [String: BmuxResolvedConfigAction],
        commands: [BmuxCommandDefinition],
        sourcePaths: [String: String],
        settingName: String,
        ...
        var resolvedItems: [BmuxResolvedConfigContextMenuItem] = []
        var issues: [BmuxConfigIssue] = []
    ...
    private func resolvedRepoAgentLaunchers(
        ...
        commands: [BmuxCommandDefinition],
        sourcePaths: [String: String]
    ) -> [BmuxResolvedRepoAgentLauncher] {
        actions.compactMap { action -> BmuxResolvedRepoAgentLauncher? in
            ...
            return BmuxResolvedRepoAgentLauncher(
                id: action.id,
    ...
    private func canonicalActionID(_ id: String) -> String {
        BmuxSurfaceTabBarBuiltInAction(configID: id)?.configID ?? id
    }
    ...
    private func appendingAutoNewWorkspaceMenuActions(
        ...
        var autoActions: [BmuxResolvedConfigAction] = []
    ...
    private func resolvedConfiguredNewWorkspaceCommand(
        ...
        return NewWorkspaceCommandResolution(
            command: BmuxResolvedCommand(command: command, sourcePath: sourcePaths[command.id]),
            issue: nil
        )
    }

    private func newWorkspaceResolutionIssue(
        kind: BmuxConfigIssue.Kind,
        settingName: String,
        commandName: String?,
        sourcePath: String?
    ) -> NewWorkspaceCommandResolution {
    ...
    private func resolvedWorkspaceCommand(
        ...
        guard let command = commands.first(where: { $0.name == commandName }) else {
            NSLog("[BmuxConfig] %@ '%@' does not match any loaded command", settingName, commandName)
            return nil
        }
        guard command.workspace != nil else {
            NSLog("[BmuxConfig] %@ '%@' must reference a workspace command", settingName, commandName)
            return nil
        }
        return BmuxResolvedCommand(command: command, sourcePath: sourcePaths[command.id])
    }
    ...
    private func parseConfig(at path: String) -> ParsedConfigResult {
        ...
        guard let data = fileManager.contents(atPath: path),
              !data.isEmpty else {
            let issue = schemaIssue(path: path, message: "bmux.json is empty")
        ...
        do {
            ...
            NSLog("[BmuxConfig] JSONC preprocessing error at %@: %@", path, String(describing: error))
            return ParsedConfigResult(config: nil, issue: issue)
        }

        do {
            let config = try JSONDecoder().decode(BmuxConfigFile.self, from: sanitized)
            parsedConfigCache[path] = ParsedConfigCacheEntry(
                fileSize: fileSize,
                modificationDate: modificationDate,
                workspaceColorPaletteFingerprint: paletteFingerprint,
            ...
            NSLog("[BmuxConfig] parse error at %@: %@", path, String(describing: error))
            return ParsedConfigResult(config: nil, issue: issue)
        }
    }

    private func schemaIssue(path: String, message: String) -> BmuxConfigIssue {
        BmuxConfigIssue(
            kind: .schemaError,
    ...
    /// Watches the global config via ``BmuxFileWatch/FileWatcher``, which handles
