...
/// Serial lane for in-process `bmux(...)` sidebar actions. Worker-lane methods
...
private let bmuxSidebarWorkerQueue = DispatchQueue(label: "com.bmux.sidebar-action-worker")

// The custom-sidebar rendering, interpreter, JSON DSL, resizable split, and
// the file-watching model now live in the `BmuxSwiftRender` (logic) and
// `BmuxSwiftRenderUI` (SwiftUI) packages. The app target keeps only the
// bmux-coupled action dispatch, the one piece that must reach
...
/// live bmux command dispatcher.
///
/// `bmux(...)` commands run in-process through
/// `TerminalController.handleSocketLine(_:)` (the same worker-aware surface the
/// socket CLI uses); `log` is a debug-only no-op for now.
@MainActor
func makeBmuxSidebarActionDispatch() -> SidebarActionDispatch {
    SidebarActionDispatch { action in
        ...
        bmuxSidebarWorkerQueue.async {
            for command in commands {
                switch command {
                case let .bmux(method, params):
                ...
                case let .openURL(urlString):
                    ...
                    if let url = URL(string: urlString) {
                        DispatchQueue.main.sync { _ = BrowserExternalLinkOpener().openWebLink(url) }
                    }
                case .log:
                    break
                }
