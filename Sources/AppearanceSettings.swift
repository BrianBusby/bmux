...
enum AppearanceSettings {
    ...
    /// resolution. Lifted to ``TerminalSystemAppearance`` in BmuxTerminalCore so
    ...
    // Ghostty split-theme resolution follows bmux's persisted appearance mode.
    // AppKit view/window appearances can lag during live mode changes.
    // The resolution itself now lives in BmuxTerminalCore
...
final class AppearanceSettingsUserDefaultsObserver {
    ...
    init(
        ...
        source: String = "bmuxApp.appearanceDefaultsChanged"
    ) {
...
extension View {
    func bmuxAppearanceColorScheme(_ rawValue: String?) -> some View {
