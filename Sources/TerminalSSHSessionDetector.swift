...
struct DetectedSSHSession: Equatable {
        ...
        do {
            for localURL in fileURLs {
                ...
                guard normalizedLocalURL.isFileURL else {
                    throw NSError(domain: "bmux.detected-ssh.drop", code: 1, userInfo: [
                ...
                guard result.status == 0 else {
                    throw NSError(domain: "bmux.detected-ssh.drop", code: 2, userInfo: [
                        NSLocalizedDescriptionKey: String(
    ...
    private static func runProcess(
        ...
        if exitSignal.wait(timeout: .now() + timeout) == .timedOut {
            ...
            terminateProcessAndWait()
            throw NSError(domain: "bmux.detected-ssh.drop", code: 3, userInfo: [
                NSLocalizedDescriptionKey: String(
