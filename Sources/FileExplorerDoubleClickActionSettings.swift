...
/// opening the built-in bmux file preview, so existing users see no change.
enum FileExplorerDoubleClickAction: String, CaseIterable, Sendable {
    /// Open the built-in bmux file preview (historical default).
    case preview
...
enum FileExplorerDoubleClickActionSettings {
    ...
    static let didChangeNotification = Notification.Name("bmux.fileExplorerDoubleClickActionDidChange")
