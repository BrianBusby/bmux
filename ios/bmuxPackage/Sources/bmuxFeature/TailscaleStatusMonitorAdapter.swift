import BmuxMobileShellUI
import BmuxMobileTransport

/// Adapts the transport layer's ``BmuxMobileTransport/TailscaleStatusMonitor``
/// onto the shell UI's read-only ``BmuxMobileShellUI/TailscaleStatusObserving``
/// port, so `BmuxMobileShellUI` stays decoupled from the concrete system
