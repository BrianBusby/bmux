...
private let mobileRootSceneLog = Logger(subsystem: "dev.bmux.ios", category: "mobile-root-scene")

/// Top-level mobile scene root.
///
/// Renders the live bmux mobile UI: a ``BMUXMobileAppView`` backed by a fresh
/// ``BMUXMobileShellStore`` and the injected ``AuthCoordinator``. In DEBUG
/// builds, setting the environment variable `BMUX_ZOOM_STRESS=1` instead mounts
/// the terminal zoom-stress repro harness (`MobileZoomStressView`).
///
/// The composition root (`bmuxApp`) builds the ``BMUXMobileRuntime`` and the
...
public struct BMUXMobileRootScene: View {
    private let runtime: BMUXMobileRuntime
    ...
    private var content: some View {
        ...
        } else if ProcessInfo.processInfo.environment["BMUX_ZOOM_STRESS"] == "1" {
            MobileZoomStressView()
        } else if ProcessInfo.processInfo.environment["BMUX_BOTTOM_SCROLL_STRESS"] == "1" {
            MobileBottomScrollStressView()
        } else {
            BMUXMobileAppView(store: makeStore(), onboardingStore: onboardingStore)
        }
        #else
        BMUXMobileAppView(store: makeStore(), onboardingStore: onboardingStore)
        #endif
        #else
        BMUXMobileAppView(store: makeStore())
        #endif
    }

    @MainActor
    private func makeStore() -> BMUXMobileShellStore {
        let coordinator = auth.coordinator
