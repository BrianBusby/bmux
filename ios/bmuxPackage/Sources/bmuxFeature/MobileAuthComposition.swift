...
/// ``BmuxAuthRuntime/AuthConfig`` from the environment + an injected
/// `LocalConfig.plist` override table, builds the ``BmuxAuthRuntime/AuthCoordinator``
/// (with a `StackAuthClient`, persistence caches over an injected `UserDefaults`,
/// and an ``BmuxAuthRuntime/AuthPresentationContextProvider``), and the
/// ``BmuxAuthRuntime/PushRegistrationService``. Replaces `AuthManager.shared`,
...
public struct MobileAuthComposition {
    ...
    /// (https://github.com/manaflow-ai/bmux/issues/7145). Exposed so the
    /// identity provider can label the channel its user ids belong to.
    public let authEnvironment: BMUXAuthEnvironment
    ...
    public init(
        ...
        let teamSelection = BMUXAuthTeamSelectionStore(
            keyValueStore: defaults,
        ...
        let launch = AuthLaunchOptions(
            clearAuthRequested: environment["BMUX_UITEST_CLEAR_AUTH"] == "1",
    ...
    /// the channel into the build via the `BMUX_IOS_AUTH_ENV` build setting —
    /// the same mechanism as `BMUXPresenceBaseURL`. Keep in sync with
    /// `ios/Config/Info.plist` and `ios/Config/Shared.xcconfig`.
    nonisolated static let authEnvironmentInfoPlistKey = "BMUXAuthEnvironment"
    ...
    /// `$(BMUX_IOS_AUTH_ENV)` expansion in a normal build contributes nothing.
    ...
    nonisolated static func includesDevAuth(
        ...
        resolvedEnvironment: BMUXAuthEnvironment
    ) -> Bool {
