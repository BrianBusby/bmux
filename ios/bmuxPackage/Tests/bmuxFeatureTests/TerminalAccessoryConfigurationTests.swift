import BmuxMobileTerminal
import BmuxMobileTerminalKit
import Foundation
import Testing
...
struct TerminalAccessoryConfigurationTests {
    ...
    private func freshDefaults() -> UserDefaults {
        let name = "bmux.toolbar.test.\(UUID().uuidString)"
    ...
    func migratesV2ConfigForceEnablingModifiers() throws {
        ...
        defaults.set(
            ...
            forKey: "bmux.terminal.toolbar.order.v2"
        )
        defaults.set(
            [id(.escape).storageKey, id(.tab).storageKey],
            forKey: "bmux.terminal.toolbar.enabled.v2"
        )
    ...
    func migratesV2ConfigPreservingHiddenShortcut() {
        ...
        defaults.set([id(.tab).storageKey], forKey: "bmux.terminal.toolbar.enabled.v2")
    ...
    func migrationPersistsUnderV3Keys() {
        let defaults = freshDefaults()
        defaults.set([id(.tab).storageKey], forKey: "bmux.terminal.toolbar.order.v2")
        defaults.set([id(.tab).storageKey], forKey: "bmux.terminal.toolbar.enabled.v2")
        ...
        let v3Order = defaults.array(forKey: "bmux.terminal.toolbar.order.v3") as? [String]
        #expect(v3Order != nil)
    ...
    func migratesV2ConfigWithCustomAction() throws {
        ...
        defaults.set(customData, forKey: "bmux.terminal.toolbar.custom.v2")
        defaults.set(
    ...
    func migratesExistingV3LayoutForceEnablingShift() throws {
        ...
        defaults.set(preShift.map { id($0).storageKey }, forKey: "bmux.terminal.toolbar.order.v3")
        defaults.set(preShift.map { id($0).storageKey }, forKey: "bmux.terminal.toolbar.enabled.v3")
    ...
    func v3LayoutWithHiddenShiftIsNotRefolded() {
        ...
        defaults.set(order.map { id($0).storageKey }, forKey: "bmux.terminal.toolbar.order.v3")
        defaults.set(enabled.map { id($0).storageKey }, forKey: "bmux.terminal.toolbar.enabled.v3")
    ...
    func migratesExistingV3LayoutForceEnablingReturn() throws {
        ...
        defaults.set(preReturn.map { id($0).storageKey }, forKey: "bmux.terminal.toolbar.order.v3")
        defaults.set(preReturn.map { id($0).storageKey }, forKey: "bmux.terminal.toolbar.enabled.v3")
    ...
    func migratesExistingV3LayoutFoldingShiftAndReturn() throws {
        ...
        defaults.set(pre.map { id($0).storageKey }, forKey: "bmux.terminal.toolbar.order.v3")
        defaults.set(pre.map { id($0).storageKey }, forKey: "bmux.terminal.toolbar.enabled.v3")
