...
import BmuxMobilePairedMac
import BmuxMobileRPC
@testable import BmuxMobileShell
@testable import BmuxMobileShellUI
import BmuxMobileShellModel
import BmuxMobileTransport
...
import UIKit
#endif
@testable import bmuxFeature

/// Test collector that mounts a surface's ``BMUXMobileShellStore`` output stream
...
final class TerminalOutputCollector {
    ...
    func mount(store: BMUXMobileShellStore, surfaceID: String) {
...
@Test func mobileDevStackAuthTokenProviderUsesExplicitEnvironmentOnly() {
    ...
    #expect(MobileShellDevStackAuthTokenProvider.token(environment: [
        MobileShellDevStackAuthTokenProvider.environmentKey: " bmux-dev-token "
    ]) == "bmux-dev-token")
}
#endif

// Auth error mapping + cached-session recovery are now owned and tested by
// BmuxAuthRuntime (AuthErrorMapperTests). The display-safe error and
// cached-session-validation assertions moved there with the AuthCoordinator
// lift; see Packages/Shared/BmuxAuthRuntime/Tests.

@Test func mobileRuntimeDefaultsToThirtySecondRPCTimeout() {
    let runtime = BMUXMobileRuntime(
...
@Test func mobileRuntimeMapsTimedOutStackTokenToRequestTimeout() {
    guard case .requestTimedOut = BMUXMobileRuntime.connectionError(forStackAuthError: AuthError.timedOut) else {
...
@Test func activeMacReconnectRouteSkipsUnsupportedLoopbackRoute() throws {
    ...
    let route = BMUXMobileShellStore.firstReconnectHostPortRoute(
        [loopback, tailscale],
...
@Test func signInMovesToPairingUntilCodeConnects() {
    ...
    #expect(store.connectedHostName == "bmux-macbook")
    #expect(store.macConnectionStatus == .connected)
}

@MainActor
@Test func pairingURLUsesBMUXMobileCorePayloadWithoutConcreteTransport() async throws {
...
@Test func macConnectionStatusMarksUnavailableWhenEventStreamCloses() async throws {
    ...
    let store = BMUXMobileShellStore.preview(runtime: runtime)
...
@Test func connectPreviewHostIgnoresPairingURLsForTrackedAsyncPath() async {
    ...
    store.signIn()
    store.pairingCode = "bmux-ios://attach?v=1&payload=invalid"
    store.connectPreviewHost()
    await Task.yield()
...
@Test func expiredPairingURLPayloadIsRejectedBeforePreviewConnection() async throws {
    ...
    """
    let url = try #require(URL(string: "bmux-ios://pair?v=1&payload=\(base64URLEncode(Data(json.utf8)))"))
    let store = BMUXMobileShellStore.preview()
...
@Test func versionWarningDoesNotClearExistingConnectionBeforeApproval() async throws {
    ...
    let store = BMUXMobileShellStore(
        runtime: runtime,
        ...
        feedbackStampProvider: {
            MobileFeedbackStamp(
                ...
                appVersion: "0.65.0",
                appBuild: "10",
                bundleIdentifier: "dev.bmux.ios.test",
                osVersion: "iOS test",
                deviceModel: "test"
            )
        }
    ...
    let warningResult = await store.connectPairingURLResult(
        "bmux-ios://attach?v=2&pc=2&av=0.65.0&ab=9&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func qrPairingURLStillConnectsTenMinutesAfterMint() async throws {
    ...
    let url = "bmux-ios://attach?v=\(ticket.version)&payload=\(base64URLEncode(payload))"
    let responses = ScriptedTransportResponses([
...
@Test func minimalPairingCodeConnectsAndAdoptsHostReportedIdentity() async throws {
    ...
    store.signIn()
    await store.connectPairingURL("bmux-ios://attach?v=2&pc=1&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)")
...
@Test func minimalPairingCodeRequiresMatchingEmailBeforeDialing() async throws {
    ...
    let result = await store.connectPairingURLResult(
        "bmux-ios://attach?v=2&ub=mac-user&pc=1&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func minimalPairingCodeCompatibilityMismatchWarnsAndContinuesAfterAcceptance() async throws {
    ...
    store.signIn()
    let result = await store.connectPairingURLResult(
        "bmux-ios://attach?v=2&ub=phone-user&pc=2&av=0.65.0&ab=9&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func minimalPairingCodeMissingCompatibilityWarnsBeforeDialing() async throws {
    ...
    store.signIn()
    let result = await store.connectPairingURLResult(
        "bmux-ios://attach?v=2&ub=phone-user&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func minimalPairingCodeAppVersionMismatchDoesNotWarnWhenCompatibilityMatches() async throws {
    ...
    store.signIn()
    let result = await store.connectPairingURLResult(
        "bmux-ios://attach?v=2&ub=phone-user&pc=1&av=0.65.0&ab=95&r=100.71.210.41:\(CmxMobileDefaults.defaultHostPort)"
    )
...
@Test func scannedLoopbackPairingCodeIsRejectedWithGuidance() async throws {
    ...
    store.signIn()
    let result = await store.connectPairingURLResult("bmux-ios://attach?v=2&r=127.0.0.1:\(CmxMobileDefaults.defaultHostPort)")
...
private func testRuntime(
    ...
    rpcRequestTimeoutNanoseconds: UInt64 = BMUXMobileRuntime.defaultRPCRequestTimeoutNanoseconds,
    pairingRequestTimeoutNanoseconds: UInt64 = BMUXMobileRuntime.defaultPairingRequestTimeoutNanoseconds,
...
private func attachURL(for ticket: CmxAttachTicket) throws -> URL {
    ...
    return try #require(URL(string: "bmux-ios://attach?v=\(ticket.version)&payload=\(payload)"))
}
...
@MainActor private func deeplinkTestStore() -> BMUXMobileShellStore {
