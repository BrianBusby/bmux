...
/// Regression coverage for https://github.com/manaflow-ai/bmux/issues/7145:
...
@Suite struct MobileAuthEnvironmentOverrideTests {
    /// The production Stack project id (`BmuxAuthRuntime.AuthConfig`).
    private static let productionProjectID = "9790718f-14cd-4f7e-824d-eaf527a82b82"
    /// The development Stack project id (`BmuxAuthRuntime.AuthConfig`).
    ...
    private func fixtureBundle(localConfig: [String: String]) throws -> Bundle {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-auth-env-fixture-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeComposition(bundle: Bundle) throws -> MobileAuthComposition {
        let defaults = try #require(UserDefaults(suiteName: "bmux-auth-env-tests-\(UUID().uuidString)"))
    ...
    @Test func localConfigProductionOverrideFlipsDevBuildToProductionAuth() throws {
        ...
        #expect(composition.config.apiBaseURL == "https://bmux.com")
        #expect(composition.config.magicLinkCallbackURL == "https://bmux.com/auth/callback")
    }
    ...
    @Test func bakedInfoPlistValueFillsInWhenLocalConfigHasNoEntry() {
        ...
        // rides in the Info.plist BMUXAuthEnvironment value.
    ...
    @Test func blankBakedValueContributesNothing() {
        // A normal (non --prod-auth) build expands $(BMUX_IOS_AUTH_ENV) to ""
    ...
    private func freshDefaults() throws -> UserDefaults {
        try #require(UserDefaults(suiteName: "bmux-auth-env-switch-\(UUID().uuidString)"))
    }
    ...
    @Test func explicitPresenceOverrideStillBeatsChannelDefault() throws {
        ...
        #expect(PresenceClient.resolvedServiceBaseURL(
            environment: [PresenceClient.serviceURLEnvKey: "https://bmux-presence-dev-alice.acct.workers.dev"],
            defaults: try freshDefaults(),
            infoPlistValue: nil,
            isDebugBuild: true,
            isDevelopmentAuthChannel: false
        ) == "https://bmux-presence-dev-alice.acct.workers.dev")
    }
}
