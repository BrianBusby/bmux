#!/usr/bin/env bash
# Turnkey cloud TestFlight lane for the bmux iOS beta.
#
...
# (dev.bmux.app.beta): no signing material ever lands on the shared Macs (they
...
# upload.
#
# The cloud build is provided by the bmuxterm-hq script scripts/reload-cloud-ios.sh
...
# (bmux-aws-m4pro routinely sits ~99.7% APFS-full) is skipped instead of dying
# mid-archive; it also prunes its remote work dir after the download. A standalone
# bmux clone with no hq checkout transparently falls back to a LOCAL Release
...
# https://github.com/manaflow-ai/bmux/pull/5647 with the same re-sign/gates plus
...
# testers (the "bmux beta" group) get every build instantly with no review.
set -euo pipefail
...
BETA_BUNDLE_ID="${IOS_BETA_BUNDLE_ID:-dev.bmux.app.beta}"
...
Build an UNSIGNED Release archive for the bmux iOS beta on a leased fleet Mac,
download it, then export/re-sign/verify/upload via upload-testflight.sh.
  ...
  --external         Make the build eligible for EXTERNAL TestFlight testers
                     ...
                     Default is internal-only (instant for the "bmux beta" group).
...
[[ -d "$IOS_DIR/bmux.xcworkspace" ]] || die "run from a bmux checkout containing ios/bmux.xcworkspace"

# --- locate the hq cloud build script (mirrors ios/scripts/reload-cloud.sh) ---
# scripts/reload-cloud-ios.sh lives in the bmuxterm-hq checkout, two levels up
# from the worktree's git common dir (.../bmuxterm-hq/repo/.git -> bmuxterm-hq).
# A standalone bmux clone has no such file; we fall back to a local build.
find_hq_cloud_ios() {
...
build_archive_cloud() {
  ...
  # Run from the repo root so the hq script's "run from a bmux checkout" check and
...
build_archive_local() {
  ...
  ARCHIVE_PATH="$out/bmux-ios-beta.xcarchive"
  ...
  ( cd "$REPO_ROOT" && xcodebuild archive \
      -workspace ios/bmux.xcworkspace \
      -scheme bmux-ios \
...
if [[ "$FORCE_LOCAL" == "1" ]]; then
  ...
  if hq_cloud="$(find_hq_cloud_ios)"; then
    ...
    err "no bmuxterm-hq cloud build script found; building locally"
    build_archive_local
  fi
fi
