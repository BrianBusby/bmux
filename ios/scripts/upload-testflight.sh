...
Archives bmux iOS, exports an App Store Connect IPA, and uploads it to
TestFlight. The default lane is beta:

  bundle id: dev.bmux.app.beta
  profile:   bmux Beta Distribution
...
the https://github.com/manaflow-ai/bmux/pull/5496 regression that killed
...
exported app does. So the re-sign merges Config/bmux-release.entitlements into
...
Options:
  ...
  --signing <mode>          Export signing mode: manual (default) or automatic.
                            ...
                            the "bmux Beta Distribution" provisioning profile from
  ...
  --external                Make the build eligible for EXTERNAL TestFlight
                            ...
                            groups (e.g. "bmux beta") instantly but can never
...
# review). Set to 1 by --external or BMUX_TESTFLIGHT_EXTERNAL=1 to drop the
...
# MARKETING_VERSION.
EXTERNAL_TESTING=0
if [[ "${BMUX_TESTFLIGHT_EXTERNAL:-}" == "1" ]]; then
  EXTERNAL_TESTING=1
fi
...
if [[ "${BMUX_TESTFLIGHT_ASSIGN_EXTERNAL_GROUP:-1}" == "0" ]]; then
...
# timestamp. Set to 1 by --skip-notes or BMUX_TESTFLIGHT_SKIP_NOTES=1.
SKIP_NOTES=0
if [[ "${BMUX_TESTFLIGHT_SKIP_NOTES:-}" == "1" ]]; then
  SKIP_NOTES=1
fi
# --notes-from-range <base>: auto-generate the "What to Test" notes from the
...
case "$LANE" in
  beta)
    PRODUCT_BUNDLE_IDENTIFIER="dev.bmux.app.beta"
    PROVISIONING_PROFILE_NAME="${IOS_BETA_PROVISIONING_PROFILE_NAME:-bmux Beta Distribution}"
    ;;
  *)
...
if [[ -n "${BMUX_BUILD_NUMBER_OUT_FILE:-}" ]]; then
  printf '%s\n' "$SHIPPED_BUILD_NUMBER" > "$BMUX_BUILD_NUMBER_OUT_FILE"
fi

OUT_DIR="${BMUX_IOS_UPLOAD_DIR:-/tmp/bmux-ios-testflight-$BUILD_NUMBER}"
...
if [[ -z "$ARCHIVE_PATH" ]]; then
  ARCHIVE_PATH="$OUT_DIR/bmux.xcarchive"
  if [[ "$SIGNING" == "automatic" ]]; then
    ...
    xcodebuild archive \
      ...
      CODE_SIGN_ENTITLEMENTS="Config/bmux-release.entitlements" \
...
# regressed in
# https://github.com/manaflow-ai/bmux/pull/5496 (June 2026): the signed beta IPA
...
# (Config/bmux-release.entitlements) is necessary but NOT sufficient on its own:
...
# the capability enabled, but the installed "bmux Beta Distribution" profile
...
# Inc."), which is present for local/fleet-archive beta cuts. The bmux iOS app is
...
if [[ "$SIGNING" == "manual" ]]; then
  ...
  # CODE_SIGN_ENTITLEMENTS = Config/bmux-release.entitlements, so default to that
  # path rather than parsing xcodebuild -showBuildSettings (slower, more brittle).
  RELEASE_ENTITLEMENTS="${IOS_RELEASE_ENTITLEMENTS:-$IOS_DIR/Config/bmux-release.entitlements}"
...
PY
  ...
  RESIGNED_IPA="$EXPORT_PATH/bmux-resigned.ipa"
  rm -f "$RESIGNED_IPA"
  ...
  # push-capable "bmux Beta Distribution" profile produced only the 4-key baseline
...
if [[ "$EXPORT_ONLY" -ne 1 && "$EXTERNAL_TESTING" -eq 1 && "$ASSIGN_EXTERNAL_GROUP" -eq 1 ]]; then
  ...
  ASC_API_KEY_ID="$ASC_API_KEY_ID" ASC_API_ISSUER_ID="$ASC_API_ISSUER_ID" \
    ASC_API_KEY_PATH="${ASC_API_KEY_PATH:-}" ASC_API_KEY_P8_BASE64="${ASC_API_KEY_P8_BASE64:-}" \
    BMUX_TESTFLIGHT_EXTERNAL_GROUP_ID="${BMUX_TESTFLIGHT_EXTERNAL_GROUP_ID:-}" \
    BMUX_TESTFLIGHT_EXTERNAL_GROUP_NAME="${BMUX_TESTFLIGHT_EXTERNAL_GROUP_NAME:-}" \
