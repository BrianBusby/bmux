...
/// injected here by `bmuxApp`.
final class BmuxAppDelegate: NSObject, @preconcurrency UIApplicationDelegate, UNUserNotificationCenterDelegate {
    /// The app-root push coordinator, injected by `bmuxApp` at launch.
    @MainActor var pushCoordinator: MobilePushCoordinator?
    /// The app-root analytics emitter, injected by `bmuxApp` at launch.
    ...
    func application(
        ...
        // `analytics` is assigned in `bmuxApp.init()` which runs before
    ...
    func userNotificationCenter(
        ...
        let ids = Self.bmuxIDs(from: notification.request.content.userInfo)
    ...
    func userNotificationCenter(
        ...
        // A swipe/clear of a bmux banner delivers the custom dismiss action
        // (enabled via the `bmux.terminal` category's `.customDismissAction`).
        // Forward it to the Mac so the desktop banner + store entry clear too.
        let ids = Self.bmuxIDs(from: request.content.userInfo)
    ...
    private nonisolated static func dismissedIDs(from userInfo: [AnyHashable: Any]) -> [String] {
        guard let bmux = userInfo["bmux"] as? [String: Any],
              let ids = bmux["dismissedIds"] as? [String] else {
            return []
        }
        return ids
    ...
    private nonisolated static func bmuxIDs(
        from userInfo: [AnyHashable: Any]
    ) -> (workspaceId: String?, surfaceId: String?, macDeviceId: String?) {
        guard let bmux = userInfo["bmux"] as? [String: Any] else { return (nil, nil, nil) }
        return (
            bmux["workspaceId"] as? String,
            bmux["surfaceId"] as? String,
            bmux["macDeviceId"] as? String
        )
    }
    ...
    ///
    /// The `bmux.notificationId` payload key is authoritative: the Mac stamps the
    ...
    private nonisolated static func notificationID(from request: UNNotificationRequest) -> String? {
        guard let bmux = request.content.userInfo["bmux"] as? [String: Any],
              let id = (bmux["notificationId"] as? String)?.trimmingCharacters(in: .whitespaces),
              !id.isEmpty else {
            return nil
        }
        return id
