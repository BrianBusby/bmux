...
public final class E2e {
    public static void main(String[] args) throws Exception {
        String socket = System.getenv("BMUX_MUX_SOCKET");
        if (socket == null || socket.isBlank()) {
            throw new IllegalStateException("BMUX_MUX_SOCKET is required");
        }
        String marker = "BMUX_JAVA_E2E_" + ProcessHandle.current().pid() + "_" + System.nanoTime();
        String later = marker + "_ATTACH";
        try (BmuxClient client = BmuxClient.builder().socketPath(socket).build()) {
            IdentifyResult identify = client.identify();
            check("bmux-mux".equals(identify.app()), "unexpected app " + identify.app());
            ...
            try (BmuxClient.BmuxStream events = client.subscribe()) {
                ...
                try {
                    ...
                    gotDuplicate = true;
                } catch (BmuxTimeoutException expectedTimeout) {
                    // no event is the expected result
                }
                check(!gotDuplicate, "same-size resize emitted surface-resized");
            }
            try (BmuxClient.BmuxStream attach = client.attachSurface(created.surface())) {
                BmuxEvent first = attach.next(Duration.ofSeconds(1));
            ...
            } catch (BmuxCommandException err) {
                check(!err.serverMessage().isBlank(), "server error string was not preserved");
            }
        }
    }

    private static void waitForMarker(BmuxClient client, long surface, String marker) throws Exception {
    ...
    private static SurfaceResizedEvent nextResized(BmuxClient.BmuxStream stream, long surface, Duration timeout) throws Exception {
        long deadline = System.nanoTime() + timeout.toNanos();
        while (System.nanoTime() < deadline) {
            BmuxEvent event = stream.next(Duration.ofNanos(deadline - System.nanoTime()));
            if (event instanceof SurfaceResizedEvent resized && resized.surface() == surface) {
                return resized;
            }
        }
        throw new BmuxTimeoutException("surface-resized not observed");
    }

    private static void nextAttachOutput(BmuxClient.BmuxStream stream, Duration timeout) throws Exception {
        long deadline = System.nanoTime() + timeout.toNanos();
        while (System.nanoTime() < deadline) {
            BmuxEvent event = stream.next(Duration.ofNanos(deadline - System.nanoTime()));
        ...
        throw new BmuxTimeoutException("attach output not observed");
    }
