...
class BmuxConnectionError(BmuxError):
    pass


class ProtocolError(BmuxError):
    pass


class TimeoutError(BmuxError):
    pass
...
class _JsonLineConnection:
    def __init__(self, path: str, timeout: float):
        ...
        try:
            ...
            self.sock.close()
            raise BmuxConnectionError(f"cannot connect to session socket {path}: {exc}") from exc
    ...
    def send(self, value: Dict[str, Any]) -> None:
        ...
        with self._lock:
            try:
                ...
                raise BmuxConnectionError(f"socket write failed: {exc}") from exc
    ...
    def recv(self) -> Dict[str, Any]:
        try:
            ...
            raise BmuxConnectionError(f"socket read failed: {exc}") from exc
        if line == "":
            raise BmuxConnectionError("session socket closed")
        try:
...
class _Stream:
    def __init__(self, client: "BmuxClient", request: Dict[str, Any]):
        self._client = client
...
def default_socket_path(session: str) -> str:
    ...
    return os.path.join(base, f"bmux-mux-{os.getuid()}", f"{session}.sock")
