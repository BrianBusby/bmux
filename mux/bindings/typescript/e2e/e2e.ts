import { BmuxClient, BmuxCommandError, BmuxTimeoutError, Tree } from "../src/index.js";

async function main(): Promise<void> {
  const socketPath = process.env.BMUX_MUX_SOCKET;
  if (!socketPath) throw new Error("BMUX_MUX_SOCKET is required");

  const marker = `BMUX_TS_E2E_${process.pid}_${Date.now()}`;
  const later = `${marker}_ATTACH`;
  const client = new BmuxClient({ socketPath, timeoutMs: 5000 });
  try {
    ...
    const duplicate = await nextSurfaceResized(events, created.surface, 500).catch((err) => {
      if (err instanceof BmuxTimeoutError) return null;
      throw err;
    });
    ...
    try {
      ...
      assert(err instanceof BmuxCommandError, `closed surface error was not command error: ${err}`);
...
async function waitForMarker(client: BmuxClient, surface: number, marker: string): Promise<void> {
...
async function nextSurfaceResized(events: Awaited<ReturnType<BmuxClient["subscribe"]>>, surface: number, timeoutMs: number) {
  const deadline = Date.now() + timeoutMs;
  for (;;) {
    const remaining = deadline - Date.now();
    if (remaining <= 0) throw new BmuxTimeoutError("surface-resized not observed");
    const event = await events.next(remaining);
    if (event.event === "surface-resized" && event.surface === surface) return event;
  }
}

async function nextAttachOutput(attach: Awaited<ReturnType<BmuxClient["attachSurface"]>>, timeoutMs: number) {
  const deadline = Date.now() + timeoutMs;
  for (;;) {
    const remaining = deadline - Date.now();
    if (remaining <= 0) throw new BmuxTimeoutError("attach output not observed");
