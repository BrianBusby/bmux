...
struct TabsVisibleSidebarView: View {
    ...
    private func workspaceList(_ snapshot: BmuxSidebarSnapshot) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            if snapshot.workspaces.isEmpty {
                Text(String(localized: "tabsVisible.noWorkspaces", defaultValue: "No workspaces shared by bmux"))
                    .font(.system(size: 12))
    ...
    private func workspaceDisclosure(
        _ workspace: BmuxSidebarWorkspace,
    ...
    private func surfacesList(_ workspace: BmuxSidebarWorkspace) -> some View {
    ...
    private var waitingState: some View {
        VStack(alignment: .leading, spacing: 8) {
            ProgressView()
                .controlSize(.small)
            Text(String(localized: "tabsVisible.waiting", defaultValue: "Waiting for bmux"))
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
        }
    }
    ...
    private func iconName(for kind: BmuxSidebarSurfaceKind) -> String {
        switch kind {
        case .terminal:
            return "terminal"
        case .browser:
