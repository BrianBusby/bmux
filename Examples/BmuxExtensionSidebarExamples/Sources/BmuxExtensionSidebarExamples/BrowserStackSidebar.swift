...
public struct BrowserStackSidebar: BmuxMutableSidebarProvider {
    public static let stateDidLoadNotification = Notification.Name("BmuxBrowserStackSidebarStateDidLoad")

    public let descriptor = BmuxSidebarProviderDescriptor(
        id: "com.example.bmux.sidebar.browser-stack",
    ...
    public func render(snapshot: BmuxSidebarProviderSnapshot) -> BmuxSidebarProviderRenderModel {
    ...
    public func handle(
        ...
        guard case .moveWorkspace(let move) = mutation else {
            return BmuxSidebarProviderCommandResult(ok: false)
        }
        stateCache.moveWorkspace(move, snapshot: snapshot)
        return BmuxSidebarProviderCommandResult(ok: true)
    }

    private func recentActivityText(_ workspace: BmuxSidebarProviderWorkspace) -> BmuxSidebarProviderText? {
        workspace.latestSubmittedAt.map { .relativeDate($0, style: .compact) }
    }

    private func browserIcon(_ workspace: BmuxSidebarProviderWorkspace) -> BmuxSidebarProviderIcon? {
        ...
        if title.contains("google") {
            return BmuxSidebarProviderIcon(
                text: "G",
...
private final class BrowserStackSidebarStateCache: @unchecked Sendable {
    ...
    private let persistenceQueue = DispatchQueue(label: "bmux.browser-stack-sidebar.persistence")
    ...
    func state(for snapshot: BmuxSidebarProviderSnapshot) -> BrowserStackSidebarState {
    ...
    func moveWorkspace(
        _ move: BmuxSidebarProviderWorkspaceMove,
        snapshot: BmuxSidebarProviderSnapshot
    ) {
        let scopeKey = Self.scopeKey(for: snapshot)
    ...
    private func startLoadIfNeeded(scopeKey: String, snapshot: BmuxSidebarProviderSnapshot) {
    ...
    private static func scopeKey(for snapshot: BmuxSidebarProviderSnapshot) -> String {
...
public struct BrowserStackSidebarStore: Sendable {
    ...
    public func load(scopeKey: String, snapshot: BmuxSidebarProviderSnapshot) throws -> BrowserStackSidebarState {
        let url = scopedStateURL(scopeKey: scopeKey)
        if url != stateURL, FileManager.default.fileExists(atPath: url.path) {
            return try load(from: url)
        }
    ...
    private func loadBestScopedFallback(
        matching snapshot: BmuxSidebarProviderSnapshot,
    ...
    public func reconciledState(for snapshot: BmuxSidebarProviderSnapshot) throws -> BrowserStackSidebarState {
...
public struct BrowserStackSidebarState: Codable, Equatable, Sendable {
    ...
    public static func initial(snapshot: BmuxSidebarProviderSnapshot) -> BrowserStackSidebarState {
    ...
    public func reconciled(with snapshot: BmuxSidebarProviderSnapshot) -> BrowserStackSidebarState {
        let liveIds = Set(snapshot.workspaceIds)
        var seen = Set<UUID>()
    ...
    public mutating func moveWorkspace(_ move: BmuxSidebarProviderWorkspaceMove) {
