...
public struct ProjectWorktreeSidebar: BmuxSidebarProvider {
    public let descriptor = BmuxSidebarProviderDescriptor(
        id: "com.example.bmux.sidebar.project-worktrees",
    ...
    public func render(snapshot: BmuxSidebarProviderSnapshot) -> BmuxSidebarProviderRenderModel {
        var sections: [BmuxSidebarProviderSection] = []
        ...
        var grouped: [String: [BmuxSidebarProviderWorkspace]] = [:]
    ...
    private func branchSubtitle(_ workspace: BmuxSidebarProviderWorkspace) -> BmuxSidebarProviderText? {
        trimmed(workspace.branchSummary).map(BmuxSidebarProviderText.plain)
    }
}
