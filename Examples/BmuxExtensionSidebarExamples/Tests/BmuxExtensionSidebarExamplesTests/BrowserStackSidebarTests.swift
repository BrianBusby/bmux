...
@testable import BmuxExtensionSidebarExamples
import XCTest
...
final class BrowserStackSidebarTests: XCTestCase {
    ...
    func testReconcilePreservesUserStateWhileApplyingSnapshotMembership() throws {
        ...
        let reconciled = try store.reconciledState(for: BmuxSidebarProviderSnapshot(
            sequence: 2,
    ...
    func testWindowScopedStateFallsBackToMatchingRelaunchScope() throws {
        ...
        preferredState.moveWorkspace(BmuxSidebarProviderWorkspaceMove(
            workspaceId: movedWorkspace.id,
            sourceSectionId: "loose",
            targetSectionId: "group:reading-list",
            targetIndex: 0
        ...
        let relaunchedSnapshot = BmuxSidebarProviderSnapshot(
            sequence: 2,
    ...
    func testBrowserStackRenderModelPreservesEmptyRequiredSections() {
        let snapshot = BmuxSidebarProviderSnapshot(sequence: 1, selectedWorkspaceId: nil, workspaces: [])
        let sections = [
    ...
    func testAsyncStateLoadNotifiesHostAndUpdatesRenderModel() throws {
        ...
        let snapshot = BmuxSidebarProviderSnapshot(
            sequence: 1,
            selectedWorkspaceId: nil,
    ...
    private func temporaryStateURL() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-browser-stack-\(UUID().uuidString)", isDirectory: true)
    ...
    private func snapshot(titles: [String], windowId: UUID? = nil) -> BmuxSidebarProviderSnapshot {
    ...
    private func workspace(title: String) -> BmuxSidebarProviderWorkspace {
