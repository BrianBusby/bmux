...
/// Regression coverage for https://github.com/manaflow-ai/bmux/issues/5173.
///
/// The host renders sidebar views through an `any BmuxSidebarProvider`
/// existential (`BmuxExtensionSidebarSelection.provider(for:)?.render(snapshot:)`).
...
final class SidebarProviderExistentialDispatchTests: XCTestCase {
    ...
    func testSuperCompactRendersIdenticallyThroughExistential() {
        ...
        let existential: any BmuxSidebarProvider = concrete
    ...
    private static func snapshot(workspaceCount: Int) -> BmuxSidebarProviderSnapshot {
