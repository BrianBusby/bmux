# BMUX Sample Sidebar Extension

This is a standalone macOS app that embeds a BMUX sidebar ExtensionKit app extension. It is the reference project for third-party sidebar authors.
...
5. Keep the extension point identifier as `com.bmuxterm.app.bmux.sidebar`.
6. Build and launch the containing app once.
7. In BMUX, click the puzzle button next to the sidebar help button, open Sidebar Extensions, and enable the sample.
8. In the same puzzle menu, choose the extension sidebar provider.
9. In the extension sidebar header, choose `BMUX ExtKit Sample Sidebar` if more than one sidebar extension is enabled.

The sample targets macOS 14+, matching BMUX.

## What It Shows

The extension renders real workspace data supplied by BMUX:
...
`BmuxSidebarExtension`. App-specific state lives in `SidebarConnectionModel`. BMUX
...
@main
@MainActor
final class SampleSidebarExtension: BmuxSidebarExtension {
...
`BmuxSidebarExtension` owns the ExtensionKit scene and XPC connection, so extension
authors do not define `configuration`, bind an extension point in Swift, or touch
`NSXPCConnection`.

`BmuxSidebarContext` exposes one typed host channel through `context.host`.

The manifest is the permission request BMUX shows to users. Request only the scopes
your sidebar actually needs.

## Troubleshooting

If the extension does not appear in BMUX, launch the containing app once, then reopen BMUX's Sidebar Extensions browser.

If it appears but cannot be enabled, check signing on both the containing app and the embedded appex.

If it loads but row clicks do not select workspaces, open the BMUX extension details popover and grant the requested action.
