...
struct SidebarInsightModel {
    ...
    init(snapshot: BmuxSidebarSnapshot) {
...
struct WorkspaceInsight: Identifiable {
    ...
    init(workspace: BmuxSidebarWorkspace, selectedWorkspaceID: UUID?) {
        id = workspace.id
...
struct SurfaceInsight: Identifiable {
    var id: UUID
    var title: String
    var kind: BmuxSidebarSurfaceKind
    var isFocused: Bool
    var unreadCount: Int

    init(surface: BmuxSidebarSurface) {
        id = surface.id
        title = surface.title
        kind = surface.kind
