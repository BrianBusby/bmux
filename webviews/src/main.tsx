...
function resolveWebviewKind(): WebviewKind {
  if (
    document.documentElement.dataset.bmuxWebviewKind === "agent-session" ||
    document.body.dataset.bmuxWebviewKind === "agent-session" ||
    document.getElementById("bmux-agent-session-config")
  ) {
...
if (!rootElement) {
  throw new Error("Missing bmux webview root");
}
