...
function ToolActivityTurn({ copy, entry }: { copy?: AgentSessionCopy; entry: TranscriptEntry }) {
  ...
  const showRawOutputLabel = copy?.showRawOutput ?? "Show raw output";
  const showOptimizedOutputLabel = copy?.showOptimizedOutput ?? "Show optimized output";
  const rawOutputUnavailableLabel = copy?.rawOutputUnavailable ?? "Raw output unavailable";
  ...
  const rawOutputRef = entry.outputMetadata?.rawOutputRef;
  const [rawOutput, setRawOutput] = useState<string | null>(null);
  const [isRawOutputVisible, setIsRawOutputVisible] = useState(false);
  const [isRawOutputLoading, setIsRawOutputLoading] = useState(false);
  const [rawOutputError, setRawOutputError] = useState<string | null>(null);
  ...
  const loadRawOutput = useCallback(async () => {
    ...
    setIsRawOutputLoading(true);
    setRawOutputError(null);
    try {
      const response = await callNative<{ rawOutput: string }>("agentSession.rawOutput", { rawOutputRef });
      setRawOutput(response.rawOutput);
      return response.rawOutput;
    } catch {
      setRawOutputError(rawOutputUnavailableLabel);
      return null;
    } finally {
      setIsRawOutputLoading(false);
    }
  }, [rawOutput, rawOutputRef, rawOutputUnavailableLabel]);
  const toggleRawOutput = useCallback(async (event: React.MouseEvent<HTMLButtonElement>) => {
    event.stopPropagation();
    if (isRawOutputVisible) {
      setIsRawOutputVisible(false);
      return;
    }
    const nextRawOutput = await loadRawOutput();
    if (nextRawOutput !== null) {
      setIsRawOutputVisible(true);
    }
  }, [isRawOutputVisible, loadRawOutput]);
  const shellCopyText = shellCopyTextForEntry(entry);
  const shellCopyTextProvider = useCallback(async () => {
    const output = await loadRawOutput();
    if (output === null) {
      return shellCopyText;
    }
    return shellCopyTextForEntry({ ...entry, output });
  }, [entry, loadRawOutput, shellCopyText]);
  const outputCopyTextProvider = useCallback(async () => {
    const output = await loadRawOutput();
    return output ?? entry.output ?? "";
  }, [entry.output, loadRawOutput]);
  const displayedOutput = isRawOutputVisible && rawOutput !== null ? rawOutput : entry.output;
  const rawToggleLabel = isRawOutputVisible ? showOptimizedOutputLabel : showRawOutputLabel;
            ...
            h(
              ...
              h(
                ...
                h(
                  ...
                  h(ShellHeaderCopyButton, {
                    ...
                    getText: rawOutputRef ? shellCopyTextProvider : undefined,
                    text: shellCopyText,
                  }),
                  rawOutputRef
                    ? h(
                        "button",
                        {
                          type: "button",
                          className:
                            `codex-shell-raw-output ${CODEX_BUTTON_BASE} ${CODEX_BUTTON_GHOST} ${CODEX_BUTTON_ICON} rounded-full electron:rounded-md hover:bg-transparent hover:text-token-button-foreground${isRawOutputVisible ? " text-token-foreground" : ""}`,
                          "aria-label": rawToggleLabel,
                          disabled: isRawOutputLoading,
                          title: rawOutputError ?? rawToggleLabel,
                          onClick: toggleRawOutput,
                        },
                        rawOutputIcon("icon-xxs"),
                      )
                    : null,
                  h(
                    "button",
              ...
              h(
                ...
                h("div", {
                  ...
                  ref: outputRef,
                  dangerouslySetInnerHTML: { __html: renderPlainTextHTML(displayedOutput ?? "") },
                }),
                outputFadeEdges.top
                  ? h("div", {
                      "aria-hidden": true,
              ...
              rawOutputRef
                ? h(CopyOutputButton, {
                    getOutput: outputCopyTextProvider,
...
function ShellHeaderCopyButton({
  ...
  const copyContents = useCallback(
    async (event: React.MouseEvent<HTMLButtonElement>) => {
      event.stopPropagation();
      const copyText = getText === undefined ? text : await getText();
...
function CopyOutputButton({
  ...
  const copyOutput = useCallback(
    async (event: React.MouseEvent<HTMLButtonElement>) => {
      event.stopPropagation();
      const copyText = getOutput === undefined ? output : await getOutput();
      const writePromise = navigator.clipboard?.writeText(copyText);
      void writePromise?.catch(() => undefined);
      setIsCopied(true);
      if (resetTimerRef.current !== null) {
...
function rawOutputIcon(className = "icon-sm") {
  return h(
    "svg",
    { className, width: "20", height: "20", viewBox: "0 0 20 20", fill: "none", "aria-hidden": true },
    h("path", {
      d: "M4.5 4.75C4.5 3.7835 5.2835 3 6.25 3H13.75C14.7165 3 15.5 3.7835 15.5 4.75V15.25C15.5 16.2165 14.7165 17 13.75 17H6.25C5.2835 17 4.5 16.2165 4.5 15.25V4.75Z",
      stroke: "currentColor",
      strokeWidth: "1.3",
    }),
    h("path", {
