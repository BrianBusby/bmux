...
const persistedLayoutKey = "bmux.diffViewer.layout";
  ...
  return (
    <div id="app">
      <Toolbar
        ...
        state={state}
      />
      <section id="content" style={{ "--bmux-diff-files-width": `${state.filesWidth}px` } as React.CSSProperties}>
        <FilesSidebar
...
function useDiffComments({
  ...
  const saveDraft = (item: DiffItem, message: string) => {
    ...
    save
      ...
      .catch((error) => console.warn("bmux diff comment save failed", error));
  };
  ...
  const editMessage = (
    ...
    save
      ...
      .catch((error) => console.warn("bmux diff comment edit failed", error));
  };

  const remove = (comment: DiffCommentRecord) => {
    if (bridgeAvailable && repoRoot != null) {
      bridgeDeleteComment(repoRoot, comment.id)
        .catch((error) => console.warn("bmux diff comment delete failed", error));
    }
...
// Reads the FROZEN CONTRACT `branchPicker` object. In dev, a `?bmuxBranchPickerMock=1`
...
function devBranchPickerMockEnabled(): boolean {
  try {
    return new URLSearchParams(window.location.search).get("bmuxBranchPickerMock") === "1";
  } catch {
    return false;
  }
}
    ...
    <div className="diff-loading-placeholder" aria-hidden="true">
      {fileSkeletonWidths.map((width, index) => (
        <div key={`${width}-${index}`} className="grid h-6 grid-cols-[16px_minmax(0,1fr)_44px] items-center gap-2 rounded-[5px] px-[7px]">
          <span className="size-4 rounded-[5px] border border-[color-mix(in_lab,var(--bmux-diff-fg)_18%,transparent)]" />
          <span className="h-[11px] rounded bg-[var(--bmux-diff-muted-bg)]" style={{ width }} />
          <span className="h-[11px] justify-self-end rounded bg-[var(--bmux-diff-muted-bg)] opacity-70" style={{ width: index % 2 === 0 ? "34px" : "24px" }} />
        </div>
      ))}
    </div>
  );
...
function LoadingDiffSkeleton() {
  return (
    <div className="diff-loading-placeholder mx-3.5 mt-3.5 border-t border-[var(--bmux-diff-border)] pt-3" aria-hidden="true">
      <div className="mb-3 grid h-9 grid-cols-[72px_minmax(0,1fr)_96px] items-center gap-3 rounded-md bg-[color-mix(in_lab,var(--bmux-diff-fg)_5%,transparent)] px-3">
        <span className="h-3 rounded bg-[var(--bmux-diff-muted-bg)]" />
        <span className="h-3 w-2/5 rounded bg-[var(--bmux-diff-muted-bg)]" />
        <span className="h-3 rounded bg-[var(--bmux-diff-muted-bg)] opacity-70" />
      </div>
      <div className="space-y-[13px] px-3 py-1">
        {diffSkeletonWidths.map((width, index) => (
          <div key={`${width}-${index}`} className="grid grid-cols-[42px_minmax(0,1fr)] items-center gap-4">
            <span className="h-px bg-[color-mix(in_lab,var(--bmux-diff-fg)_10%,transparent)]" />
            <span className="h-3 rounded bg-[var(--bmux-diff-muted-bg)]" style={{ width }} />
          </div>
        ))}
      </div>
    </div>
...
function useWorkerRenderOptionsSync(
  ...
  useEffect(() => {
    ...
    workerPool.setRenderOptions(highlighterOptions)
      ...
      .catch((error: unknown) => console.warn("bmux diff worker render options update failed", error));
    return () => {
      active = false;
    };
    ...
    streamPatch({
      ...
      onComplete: (metrics) => {
        ...
        preloadHighlighter({ themes, langs: langs.length > 0 ? langs : ["text"] })
          .catch((error) => console.warn("bmux diff highlighter preload failed", error));
      },
      ...
      console.error("bmux diff viewer render failed", error);
...
function usePendingReplacement(payload: any, label: DiffViewerLabelResolver, dispatch: React.Dispatch<AppAction>) {
  ...
  useEffect(() => {
    ...
    if (payload.pendingReplacement === true) {
      ...
      fetch("/__bmux_diff_viewer_wait" + location.pathname, { cache: "no-store" })
        .then(async (response) => {
          if (!response.ok) {
            throw new Error("replacement failed");
          }
          const text = await response.text();
          if (!text.includes("data-bmux-diff-pending=\"true\"")) {
            window.location.reload();
          }
        })
        .catch((error) => {
          document.documentElement.dataset.bmuxDiffWait = "failed";
          dispatch({ type: "set-status", status: createDiffViewerStatus(label("renderFailed"), { error: true, loading: false, statusOnly: true }) });
          console.warn("bmux diff viewer deferred load failed", error);
        });
      return;
    }
