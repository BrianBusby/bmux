...
export function resolveDiffNavigationURL(rawURL: string): string {
  ...
  try {
    ...
    if (
      window.location.protocol === "bmux-diff-viewer:" &&
      (target.protocol === "http:" || target.protocol === "https:")
    ) {
      const rest = target.pathname.split("/").filter(Boolean).slice(1).join("/");
      return `bmux-diff-viewer://${window.location.host}/${rest}`;
    }
    return target.href;
  } catch {
    return rawURL;
...
// `http://`, `bmux-diff-viewer://`, `data:`). A root-relative path (`/foo?x`)
...
function safeGitApplyDelimiter(patch: string): string {
  ...
  let delimiter = "BMUX_DIFF_PATCH";
  let index = 0;
  while (lines.has(delimiter)) {
    index += 1;
    delimiter = `BMUX_DIFF_PATCH_${index}`;
  }
  return delimiter;
}
