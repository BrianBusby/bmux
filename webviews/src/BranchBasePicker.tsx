...
// scheme (`bmux-diff-viewer://<token>/...`) and the HTTP port changes, so the
...
// `/__bmux_diff_viewer_*` specially regardless of the token path segment, and
  ...
  const openPopover = () => {
    ...
    if (groups == null && loadState !== "loading") {
      ...
      fetchRefs(toCurrentOriginRelative(picker.refsURL))
        ...
        .catch((error) => {
          console.warn("bmux diff branch picker refs load failed", error);
          setLoadState("error");
        });
    }
  };
