...
describe("appearanceBackgroundColor", () => {
  ...
  test("applies readable semantic diff colors from the terminal palette", () => {
    ...
    expect(style.getPropertyValue("--bmux-diff-deletion-fg-light")).toBe("#cc0000");
    expect(style.getPropertyValue("--bmux-diff-addition-fg-light")).toBe("#007a00");
    expect(style.getPropertyValue("--bmux-diff-deletion-fg-dark")).toBe("#ff8a80");
    expect(style.getPropertyValue("--bmux-diff-addition-fg-dark")).toBe("#a6e22e");
  });
  ...
  test("keeps the page surface transparent for opaque themes", () => {
    ...
    const style = dom.window.document.documentElement.style;
    expect(style.getPropertyValue("--bmux-diff-surface-fill-light")).toBe("transparent");
    expect(style.getPropertyValue("--bmux-diff-surface-fill-dark")).toBe("transparent");
  });
