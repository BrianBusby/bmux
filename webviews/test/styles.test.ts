...
test("toolbar and files pane keep structural surfaces transparent", () => {
  expect(styles).toContain("--bmux-diff-toolbar-bg: transparent");
  expect(styles).toContain("--bmux-diff-sidebar-bg: transparent");
  expect(styles).toMatch(/#toolbar\s*\{[^}]*border-bottom: 1px solid var\(--bmux-diff-border\)[^}]*background: var\(--bmux-diff-toolbar-bg\)/s);
  expect(styles).toMatch(/#toolbar\s*\{[^}]*padding: 3px 4px 3px 8px;/s);
  expect(styles).toMatch(/#files-sidebar\s*\{[^}]*background: var\(--bmux-diff-sidebar-bg\)/s);
  expect(styles).toMatch(/#files-header\s*\{[^}]*border-bottom: 1px solid var\(--bmux-diff-border\)[^}]*background: var\(--bmux-diff-sidebar-bg\)/s);
  expect(styles).toMatch(/#file-list\s*\{[^}]*background: var\(--bmux-diff-sidebar-bg\)/s);
  ...
  expect(rendererHostBlock).not.toContain("background: var(--bmux-diff-bg)");
  expect(styles).toContain("--trees-bg-override: var(--bmux-diff-sidebar-bg)");
  ...
  expect(styles).not.toContain("box-shadow: 0 -1px 0 var(--bmux-diff-border), 0 1px 0 var(--bmux-diff-border)");
});
