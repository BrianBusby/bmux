...
test("toCurrentOriginRelative strips an http origin so a restored page re-resolves it", () => {
  ...
  expect(
    toCurrentOriginRelative("http://127.0.0.1:51234/__bmux_diff_viewer_refs?repo=%2Ftmp%2Fr&token=abc"),
  ).toBe("/__bmux_diff_viewer_refs?repo=%2Ftmp%2Fr&token=abc");
  expect(
    toCurrentOriginRelative("bmux-diff-viewer://tok/__bmux_diff_viewer_branch?group=g&token=abc&base={ref}"),
  ).toBe("/__bmux_diff_viewer_branch?group=g&token=abc&base={ref}");
});
...
test("toCurrentOriginRelative leaves a data: URL and an already-relative path untouched", () => {
  ...
  expect(toCurrentOriginRelative("/__bmux_diff_viewer_refs?token=abc")).toBe("/__bmux_diff_viewer_refs?token=abc");
});
...
test("selecting a ref navigates to a root-relative regenerate URL", async () => {
  ...
  const picker: BranchPickerPayload = {
    ...
    regenerateURLTemplate: "http://127.0.0.1:51234/__bmux_diff_viewer_branch?group=g&repo=%2Ftmp%2Fmock&token=abc&base={ref}",
  };
  ...
  expect(navigated[0]).toBe("/__bmux_diff_viewer_branch?group=g&repo=%2Ftmp%2Fmock&token=abc&base=develop");
});
