...
test("layout toggle persists user choice while explicit payload layout wins", async () => {
  ...
  await waitFor(() => dom?.window.localStorage.getItem("bmux.diffViewer.layout") === "split");
...
function contentFilesWidth(): string | undefined {
  return dom?.window.document.getElementById("content")?.style.getPropertyValue("--bmux-diff-files-width");
}
