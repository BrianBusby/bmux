...
test("code view CSS keeps Pierre structural surfaces transparent", () => {
  ...
  expect(css).toContain("--diffs-bg-buffer-override: color-mix(in srgb, var(--bmux-diff-fg) 12%, transparent)");
  ...
  expect(css).toContain("--bmux-diff-surface-bg: transparent");
  expect(css).toContain("--bmux-diff-header-bg: color-mix(in srgb, var(--bmux-diff-bg) 42%, transparent)");
  expect(css).not.toContain("color-mix(in srgb, var(--bmux-diff-bg) 94%, #3e3d32)");
  ...
  expect(css).toContain("background-color: var(--bmux-diff-header-bg) !important");
  ...
  expect(css).not.toContain("border-block: 1px solid var(--bmux-diff-border)");
...
test("file tree sticky overlays use a non-transparent surface", () => {
  ...
  expect(css).toContain("background-color: var(--bmux-diff-sidebar-bg)");
  expect(css).toContain("--bmux-diff-tree-sticky-bg: var(--bmux-diff-bg)");
  expect(css).toContain("[data-file-tree-sticky-overlay-content]");
  expect(css).toContain("background-color: var(--bmux-diff-tree-sticky-bg) !important");
