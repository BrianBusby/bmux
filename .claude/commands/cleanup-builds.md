...
2. **Read the preview to the user.** Confirm the active tag and any tag they care about appears under `skipping:` (running, or most recent reload via `/tmp/bmux-last-cli-path`).
...
- Safety rules always on: skip running `bmux DEV <tag>` apps, skip the tag in `/tmp/bmux-last-cli-path` (most recent reload).
