...
bmux markdown open plan.md

# Absolute path
bmux markdown open /path/to/PLAN.md

# Target a specific workspace
bmux markdown open design.md --workspace workspace:2
```
...
When creating a plan or task list, write it to a `.md` file and open it in bmux:
...
```bash
# Open in the caller's workspace (default -- uses BMUX_WORKSPACE_ID)
bmux markdown open plan.md

# Open in a specific workspace
bmux markdown open plan.md --workspace workspace:2

# Open splitting from a specific surface
bmux markdown open plan.md --surface surface:5

# Open in a specific window
bmux markdown open plan.md --window window:1
```
