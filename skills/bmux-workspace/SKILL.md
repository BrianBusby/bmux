---
name: bmux-workspace
description: "Work inside the current bmux workspace and terminal. Use for bmux workspace, current workspace, caller surface, panes, surfaces, socket targeting, and non-interfering bmux automation."
---

# bmux Workspace

Use this skill when a task should be scoped to the bmux workspace that invoked the agent. A workspace is the sidebar tab-like unit in bmux. It contains split panes, and each pane contains one or more surfaces. A surface is the terminal or browser session the user interacts with.
...
Do not assume the visually focused bmux workspace is the right target. An agent can be running in one workspace while the user is looking at another. Prefer the caller environment first:
...
Use `BMUX_WORKSPACE_ID` as the default workspace anchor and `BMUX_SURFACE_ID` as the default caller terminal/surface anchor. If those are missing, use `bmux identify --json` and be explicit that you are using the currently focused bmux context.
...
bmux new-pane --workspace "${BMUX_WORKSPACE_ID}" --type browser --direction right --url "http://127.0.0.1:8765"
bmux new-pane --workspace "${BMUX_WORKSPACE_ID}" --type terminal --direction down
```
...
Pass `--focus false` whenever the verb supports it. `move-surface --focus false` preserves the user's current attention. Other commands may grow the same flag over time (https://github.com/manaflow-ai/bmux/issues/1418, https://github.com/manaflow-ai/bmux/issues/2820).
...
```bash
bmux identify --json
bmux list-panes --workspace "${BMUX_WORKSPACE_ID:-}" --json
bmux list-pane-surfaces --workspace "${BMUX_WORKSPACE_ID:-}" --json
```
...
- If the caller workspace already has a non-caller helper pane, add a new surface to that pane instead of creating another pane:
  ```bash
  bmux new-surface --workspace "${BMUX_WORKSPACE_ID:-}" --pane pane:<helper> --type terminal --focus false
  ```
- If there is no helper pane, create exactly one right-side pane:
  ```bash
  bmux new-pane --workspace "${BMUX_WORKSPACE_ID:-}" --type terminal --direction right --focus false
  ```
...
bmux list-panes --workspace "${BMUX_WORKSPACE_ID:-}" --json
bmux list-pane-surfaces --workspace "${BMUX_WORKSPACE_ID:-}" --json
bmux list-panels --workspace "${BMUX_WORKSPACE_ID:-}" --json
```

Use `--id-format both` when logs or handoffs need stable UUIDs plus human refs:

```bash
bmux --json --id-format both identify
```
...
bmux new-workspace --name "debug auth" --cwd "$PWD"

# rename / close (only when explicitly requested)
bmux rename-workspace --workspace "${BMUX_WORKSPACE_ID:-}" -- "build fix"
...
bmux new-surface --workspace "${BMUX_WORKSPACE_ID:-}" --type terminal
...
```bash
bmux move-surface --surface "${BMUX_SURFACE_ID}" --before surface:3
bmux move-surface --surface "${BMUX_SURFACE_ID}" --after surface:3
bmux move-surface --surface "${BMUX_SURFACE_ID}" --index 0
```

Move a surface to another existing pane. Pass `--focus false` to keep the user's current attention put:

```bash
bmux move-surface --surface surface:240 --pane pane:172 --focus false
```

Split a surface off into a new pane:

```bash
bmux drag-surface-to-split --surface surface:240 down
```

Known papercut: `drag-surface-to-split` currently routes through V1 and resolves the workspace via UI focus, so it can fail with `ERROR: Surface not found` when the caller's workspace is not the visually focused one. Tracked at https://github.com/manaflow-ai/bmux/issues/1901, related to https://github.com/manaflow-ai/bmux/issues/3189. Until that lands, prefer building the layout additively (see Non-Disruptive Automation above) over create-then-split.
...
```bash
bmux set-status build "running" --workspace "${BMUX_WORKSPACE_ID:-}" --color "#ff9500"
bmux set-progress 0.4 --label "Building" --workspace "${BMUX_WORKSPACE_ID:-}"
bmux log --workspace "${BMUX_WORKSPACE_ID:-}" --level info -- "Started build"
...
Never build or launch untagged `bmux DEV`. If tests or tools need a socket, use the tag-specific socket:
...
Use the socket path provided by bmux before falling back to defaults:

```bash
SOCK="${BMUX_SOCKET_PATH:-/tmp/bmux.sock}"
```

Socket access can be off, restricted to bmux-spawned processes, or allow all local processes. If a command cannot connect, inspect capabilities before changing settings:
...
- [../bmux-browser/SKILL.md](../bmux-browser/SKILL.md) covers browser surfaces with the same current-workspace rule.

## Rules

- Work in the current caller workspace by default.
- Use `BMUX_WORKSPACE_ID`, `BMUX_SURFACE_ID`, and `BMUX_SOCKET_PATH` before focused-window fallbacks.
