...
bmux uses isolated browser contexts per surface. Treat each browser surface as its own session.
...
# session B
bmux browser open https://example.com --json
# -> surface:8

bmux browser surface:7 get url
bmux browser surface:8 get url
```
...
```bash
bmux browser surface:7 state save /tmp/auth-state.json
```

### Load State

```bash
bmux browser surface:8 state load /tmp/auth-state.json
...
bmux browser open https://app.example.com/login --json
# login on surface:7 ...
bmux browser surface:7 state save /tmp/auth.json

bmux browser open https://app.example.com --json
# assume surface:8
bmux browser surface:8 state load /tmp/auth.json
bmux browser surface:8 goto https://app.example.com/dashboard
```
...
```bash
bmux browser open https://site-a.example --json
bmux browser open https://site-b.example --json
bmux browser open https://site-c.example --json

bmux browser surface:11 get text body > /tmp/a.txt
bmux browser surface:12 get text body > /tmp/b.txt
bmux browser surface:13 get text body > /tmp/c.txt
```
...
bmux close-surface --surface surface:8
