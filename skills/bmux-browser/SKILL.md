---
name: bmux-browser
description: End-user browser automation with bmux. Use when you need to open sites, interact with pages, wait for state changes, and extract data from bmux browser surfaces.
---

# Browser Automation with bmux

Use this skill for browser tasks inside bmux webviews.
...
```bash
bmux --json browser open https://example.com
# use returned surface ref, for example: surface:7

bmux browser surface:7 get url
bmux browser surface:7 wait --load-state complete --timeout-ms 15000
bmux browser surface:7 snapshot --interactive
bmux browser surface:7 fill e1 "hello"
bmux --json browser surface:7 click e2 --snapshot-after
bmux browser surface:7 snapshot --interactive
```
...
bmux browser open https://example.com --workspace workspace:2 --window window:1 --json
```
...
bmux supports wait patterns similar to agent-browser:

```bash
bmux browser <surface> wait --selector "#ready" --timeout-ms 10000
bmux browser <surface> wait --text "Success" --timeout-ms 10000
...
```bash
bmux --json browser open https://example.com/signup
bmux browser surface:7 get url
bmux browser surface:7 wait --load-state complete --timeout-ms 15000
bmux browser surface:7 snapshot --interactive
bmux browser surface:7 fill e1 "Jane Doe"
bmux browser surface:7 fill e2 "jane@example.com"
bmux --json browser surface:7 click e3 --snapshot-after
bmux browser surface:7 wait --url-contains "/welcome" --timeout-ms 15000
bmux browser surface:7 snapshot --interactive
```
...
```bash
bmux browser surface:7 fill e11 "" --snapshot-after --json
bmux browser surface:7 get value e11 --json
```
...
bmux browser surface:7 wait --load-state complete --timeout-ms 15000
bmux browser surface:7 snapshot --interactive
bmux --json browser surface:7 click e5 --snapshot-after
bmux browser surface:7 snapshot --interactive
```
...
```bash
bmux browser surface:7 get url
bmux browser surface:7 get text body
bmux browser surface:7 get html body
```
