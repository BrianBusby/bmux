---
name: bmux-backend
description: "Backend TypeScript and Cloud VM development rules for bmux. Use when editing web/app/api, web/services, backend scripts, Cloud VM lifecycle, provider integrations, Postgres, Stack Auth pricing gates, migrations, or provider image build scripts."
---
...
- Runtime env names are `BMUX_DB_DRIVER=aws-rds-iam`, `AWS_ROLE_ARN`, `AWS_REGION`, `PGHOST`, `PGPORT`, `PGUSER`, and `PGDATABASE`.
...
- Local development keeps using the `BMUX_PORT`-derived Docker Postgres path from `bun dev`.
...
Cloud VM build, test, and local dev scripts use provider secrets from `~/.secrets/bmux.env`.
...
```bash
set -a
source ~/.secrets/bmux.env
set +a
```

`~/.secrets/bmuxterm-dev.env` is for local Stack/web env and does not contain the provider build keys. `bun dev` sources `~/.secrets/bmux.env` first when present, then `~/.secrets/bmuxterm-dev.env` so bmuxterm-specific Stack settings override broader bmux secrets. The web dev loader still accepts the legacy `~/.secret/bmuxterm.env` and `~/.secrets/bmuxterm.env` paths while machines migrate.
