...
A `.swift` file added to `bmuxTests/` without matching project entries can be silently ignored by Xcode. Both targeted `xcodebuild test -only-testing:bmuxTests/<TestClass>` and bot reviews can pass with "Executed 0 tests".
