---
name: bmux-customization
description: "Customize bmux for an end user. Use when changing bmux.json actions, custom commands, workspace layouts, plus-button behavior, surface tab bar buttons, Command Palette entries, Dock controls, sidebar and app settings, shortcuts, notifications, browser routing, examples-library presets, or Ghostty-backed terminal preferences."
---

# bmux Customization

Use this skill for user-facing bmux customization. Keep the user's config intact, prefer schema-backed edits, and validate before reporting completion.

## What Can Be Customized

- Custom actions: define reusable `actions` in `bmux.json`. Actions can appear in Cmd+Shift+P, surface tab bars, shortcuts, and the plus-button right-click menu.
...
- Surface tab bar buttons: set `ui.surfaceTabBar.buttons` to replace the default tab bar buttons. Include built-in IDs such as `bmux.newTerminal`, `bmux.newBrowser`, `bmux.splitRight`, and `bmux.splitDown` only when they should stay visible.
...
- Dock controls: create `.bmux/dock.json` or `~/.config/bmux/dock.json` for right-sidebar terminal controls such as logs, test watchers, git TUIs, dev servers, queues, or `bmux feed tui --opentui`.
- Sidebar and app behavior: use `bmux-settings` for supported settings such as appearance, sidebar display, notification behavior, browser routing, automation, shortcuts, and new-workspace placement.
- Workspace metadata: use the bmux CLI or `bmux-workspace` for workspace names, descriptions, colors, read state, and sidebar metadata updates.
- Feed and notifications: use `bmux hooks setup` for Feed event sources, notification settings for delivery behavior, and notification hooks in `bmux.json` for filtering or post-processing banners.
- Team presets and examples: use project-local `.bmux/bmux.json` and `.bmux/dock.json` to share worktree, SSH, review, dev, CI, and docs workspace patterns with a repo.
...
- bmux app preferences: use `bmux-settings` for global `~/.config/bmux/bmux.json` settings such as appearance, sidebar, notifications, browser behavior, automation, and shortcuts.
- Custom actions, workspace layouts, tab bar buttons, plus-button behavior, and Command Palette entries: edit `~/.config/bmux/bmux.json` globally or `.bmux/bmux.json` in the project. Project-local actions and commands override global entries with the same ID or name.
- Dock controls: edit `.bmux/dock.json` in the project or `~/.config/bmux/dock.json` globally. Run `bmux docs dock` when available.
...
- Project-specific behavior: prefer `.bmux/bmux.json` in the project so actions, commands, UI action wiring, and notification hooks travel with the repo. Do not put global app preferences there.

If a request can be handled by Ghostty config, say that and use Ghostty config instead of inventing bmux UI settings.
...
1. Inspect existing config before editing.
   ...
   ```bash
   test -f ~/.config/bmux/bmux.json && sed -n '1,220p' ~/.config/bmux/bmux.json
   test -f .bmux/bmux.json && sed -n '1,220p' .bmux/bmux.json
   ```
...
3. Before editing, back up the target file when it already exists:
   ...
   test -f ~/.config/bmux/bmux.json && cp -p ~/.config/bmux/bmux.json ~/.config/bmux/bmux.json."$stamp".bak
   test -f .bmux/bmux.json && cp -p .bmux/bmux.json .bmux/bmux.json."$stamp".bak
   ```

   Use the applicable path only. Do not create a backup for a missing file.
4. For app settings and bmux-owned shortcuts, use the settings helper from the installed skill or checkout:

   ```bash
   ~/.agents/skills/bmux-settings/scripts/bmux-settings list-supported
   ~/.agents/skills/bmux-settings/scripts/bmux-settings set browser.openTerminalLinksInBmuxBrowser true
...
- App settings: run `bmux-settings validate`.
- JSONC shape: keep valid JSONC and avoid duplicate keys.
- Dock JSON: parse `.bmux/dock.json` or `~/.config/bmux/dock.json` with a JSON parser before reporting completion.
- Runtime reload: run `bmux reload-config` when the CLI is available.
...
- Do not add a bmux setting for behavior Ghostty already owns.
