...
target `bmux.json`. Preserve unrelated sections, then run `bmux reload-config`
when available.

Prefer project-local `.bmux/bmux.json` for team workflows and global
`~/.config/bmux/bmux.json` for personal app preferences.
...
```json
{
  "controls": [
    ...
    { "id": "feed", "title": "Feed", "command": "bmux feed tui --opentui", "height": 260 }
  ]
}
```
...
Use this when the user's normal environment is remote, or when local bmux should
                    ...
                    "surfaces": [
                      {
                        "type": "terminal",
                        "name": "Markdown",
                        "command": "bmux markdown open docs/README.md --direction right --focus false; exec ${SHELL:-/bin/zsh} -l"
                      }
                    ]
                  }
                }
