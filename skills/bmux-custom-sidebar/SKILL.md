---
name: bmux-custom-sidebar
description: "Build a custom bmux sidebar from a plain-language request. Use when the user asks for a custom sidebar, a sidebar that shows their workspaces/tabs/PRs/clock, a vibe-coded sidebar, or anything involving files in ~/.config/bmux/sidebars/. Covers authoring the interpreted SwiftUI-style file, enabling the beta flag, selecting it, and iterating with hot reload."
---

# bmux Custom Sidebar

bmux renders custom sidebars from a small SwiftUI-style file at runtime: no Xcode, no build step, no signing. The file hot-reloads on save, binds to live bmux state (workspaces, tabs, git, PRs, clock), and can run real bmux commands on tap.
...
```bash
bmux docs sidebars
curl -fsSL https://raw.githubusercontent.com/manaflow-ai/bmux/main/docs/custom-sidebars.md
```
...
2. **Write a named file.** The name becomes the menu label; use short kebab-case:
   ```
   ~/.config/bmux/sidebars/<name>.swift
   ```
...
3. **Validate and select it:**
   ```bash
   bmux sidebar validate <name>   # parse/interpret check with real data shapes
   bmux sidebar select <name>     # switch the sidebar to it
   ```
   The user can also pick it manually: right-click the sidebar toggle button.
4. **Iterate.** Saving the file hot-reloads the sidebar in place (`bmux sidebar reload` forces it). Look at the result, fix what looks off, and verify rows show real data and taps do the right thing before declaring it done.

## Authoring rules

- **Default to live data.** Bind to the `workspaces` context instead of hard-coding text so the sidebar stays correct on its own.
- **Make it interactive by default.** Rows that represent something openable should run the matching `bmux(...)` action on tap. A list that just displays text is rarely what they wanted.
...
```bash
cat > ~/.config/bmux/sidebars/mine.swift <<'SWIFT'
VStack(alignment: .leading, spacing: 8) {
    ...
    Reorderable(workspaces, move: "workspace.reorder") { w in
        Button(action: { bmux("workspace.select", workspace_id: w.id) }) {
            HStack {
...
SWIFT
bmux sidebar validate mine && bmux sidebar select mine
```
...
A button or `.onTapGesture` body calls `bmux("<method>", param: value)`, dispatched through the same surface as the `bmux` CLI. Common methods: `workspace.select` (`workspace_id`), `surface.focus` (`surface_id`), `workspace.reorder` (`workspace_id` + `index`). `openURL("https://...")` opens links. Discover the full command surface with `bmux docs api`.
...
Not yet supported (write the natural Swift anyway; it degrades gracefully): `@State` and input controls (`TextField`, `Toggle`, `Slider`, `Picker`), custom `struct`/`View` definitions, navigation (`sheet`/`popover`), `AsyncImage`. Two-way editing does not work yet; taps that run `bmux(...)` do.

## Troubleshooting

- Sidebar missing from the right-click picker: the beta flag is off, or the file is not directly under `~/.config/bmux/sidebars/`.
- Blank or partial render: run `bmux sidebar validate <name>`; errors show inline in the sidebar with the failing location. A broken save keeps the last working render on screen, so re-save after fixing.
- Rows not tappable: wrap the row in `Button(action: { bmux(...) }) { ... }` or add `.onTapGesture { bmux(...) }`.
