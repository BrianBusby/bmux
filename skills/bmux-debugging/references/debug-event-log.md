...
- tagged Debug app: `/tmp/bmux-debug-<tag>.log`

`reload.sh` writes the current path to `/tmp/bmux-last-debug-log-path`, so the most robust tail command is:
...
The package implementation lives in `Packages/macOS/BMUXDebugLog/Sources/BMUXDebugLog/DebugEventLog.swift`, and the app shim lives in `Sources/App/DebugLogging.swift`.

Call sites use:

```swift
#if DEBUG
bmuxDebugLog("focus.panel ...")
#endif
```
...
The debug logger has a 500-entry ring buffer. `BMUXDebugLog.DebugEventLog.shared.dump()` writes the full buffer to file. Use this when the interesting event occurred before you started tailing.
