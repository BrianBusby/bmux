...
This reference expands the high-risk bmux runtime rules.
...
Do not add an app-level display link or manual `ghostty_surface_draw` loop. bmux relies on Ghostty wakeups and renderer scheduling. A second draw loop can make typing lag worse and hide the real invalidation source.
...
When a user reports a repro on an older macOS, test on that macOS before declaring the repro disproven. AWS M4 Pro builders such as `bmux-aws-mac`, `bmux-aws-m4pro`, and `aws-m4pro-1..6` are pre-provisioned on macOS 15.7.4 and are the preferred empirical repro path.
