---
name: bmux-debugging
description: "Debug logging, Debug menu, runtime pitfalls, typing-latency-sensitive paths, SwiftUI list snapshot boundaries, OS-version repros, and local visual iteration for bmux. Use when adding debug probes, diagnosing UI/runtime issues, touching terminal rendering, tab/sidebar list views, drag/drop UTTypes, or using the Debug menu."
---
...
```bash
tail -f "$(cat /tmp/bmux-last-debug-log-path 2>/dev/null || echo /tmp/bmux-debug.log)"
```

- Untagged Debug app: `/tmp/bmux-debug.log`
- Tagged Debug app (`./scripts/reload.sh --tag <tag>`): `/tmp/bmux-debug-<tag>.log`
- `reload.sh` writes the current path to `/tmp/bmux-last-debug-log-path`
- `reload.sh` writes the selected dev CLI path to `/tmp/bmux-last-cli-path`
- `reload.sh` updates `/tmp/bmux-cli` and `$HOME/.local/bin/bmux-dev` to that CLI
- Implementation: `Packages/macOS/BMUXDebugLog/Sources/BMUXDebugLog/DebugEventLog.swift`
- App shim: `Sources/App/DebugLogging.swift`
- Free function `bmuxDebugLog("message")` logs with timestamp and appends to file in real time from bmux code
- The package implementation and app shim are `#if DEBUG`; all call sites must be wrapped in `#if DEBUG` / `#endif`
- 500-entry ring buffer; `BMUXDebugLog.DebugEventLog.shared.dump()` writes full buffer to file
...
- To add a debug toggle or visual option: create an `NSWindowController` subclass with a `shared` singleton, add it to the "Debug Windows" menu in `Sources/bmuxApp.swift`, and add a SwiftUI view with `@AppStorage` bindings for live changes.
