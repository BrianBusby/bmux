...
This reference expands bmux package extraction and refactor architecture rules.
...
5. Executable: `bmuxApp` and `AppDelegate` as the composition root.
...
`@main` `bmuxApp` and `AppDelegate` stay in the executable target. Do not move extensions of executable-owned types down into lower packages. A lower package cannot extend a higher-owned type without creating the wrong dependency direction.
...
`bmux.xcodeproj` lists package dependencies explicitly. Adding `Packages/BmuxFoo` means mirroring existing package entries:
...
App-target packages link into both `bmux` and `bmux-unit`, so tests can import and inject them. A package linked by the app but not `bmux-unit` can make the app build pass while the test target fails.
