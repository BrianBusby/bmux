...
def main() -> int:
    token = "BMUX_PALETTE_FOCUS_PROBE_9412"
    restore_token = "BMUX_PALETTE_RESTORE_PROBE_7731"
    ...
    with bmux(SOCKET_PATH) as client:
        ...
        if token in post_text and token not in pre_text:
            raise bmuxError("typed probe text leaked into terminal while palette is open")
