#!/usr/bin/env python3
"""
Stability regression test: browser panels should not crash bmux when:
...
BROWSER_TEST_URL = "data:text/html;charset=utf-8," + urllib.parse.quote(
    "<!doctype html><html><head><title>bmux stability</title></head>"
    "<body><h1>bmux browser stability</h1></body></html>"
)
...
def ensure_webview_focused(client: bmux, panel_id: str, timeout_s: float = 2.0) -> None:
    """
...
def test_open_browser_then_new_surface_loop(client: bmux) -> tuple[bool, str]:
...
def test_focus_panes_with_loaded_browser(client: bmux) -> tuple[bool, str]:
    ws_id = client.new_workspace()
    client.select_workspace(ws_id)
    time.sleep(0.5)
...
def run_tests() -> int:
    print("=" * 60)
    print("bmux Browser Panel Stability Test")
    print("=" * 60)
    print()
    ...
    try:
        with bmux(socket_path=probe.socket_path) as client:
