...
def _wait_for_file_text(path: Path, needle: str, timeout_s: float = 8.0) -> str:
    ...
    while time.time() < deadline:
        ...
        if needle in last_text:
            return last_text
        time.sleep(0.1)
    raise bmuxError(f"Timed out waiting for {needle!r} in background workspace file: {last_text!r}")
...
def main() -> int:
    with bmux(SOCKET_PATH) as c:
        ...
        marker_path = Path(tempfile.gettempdir()) / f"bmux-bg-start-{int(time.time() * 1000)}.txt"
        layout_marker_path = Path(tempfile.gettempdir()) / f"bmux-bg-layout-start-{int(time.time() * 1000)}.txt"
        try:
            token = f"BMUX_BG_START_{int(time.time() * 1000)}"
            initial_command = (
                "python3 -c " +
                shlex.quote(
            ...
            layout_token = f"BMUX_BG_LAYOUT_START_{int(time.time() * 1000)}"
            layout_command = (
                "python3 -c " +
                shlex.quote(
