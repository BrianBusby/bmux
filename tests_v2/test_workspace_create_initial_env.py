    ...
    with bmux(SOCKET_PATH) as c:
        ...
        try:
            ...
            payload = c._call(
                "workspace.create",
                {
                    "initial_env": {"BMUX_INITIAL_ENV_TOKEN": token},
                },
            ) or {}
            ...
            c.send("printf 'BMUX_ENV_CHECK=%s\\n' \"$BMUX_INITIAL_ENV_TOKEN\"\\n")
            text = _wait_for_text(c, created_workspace, f"BMUX_ENV_CHECK={token}")
            _must(
                f"BMUX_ENV_CHECK={token}" in text,
