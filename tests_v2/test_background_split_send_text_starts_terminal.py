...
def _wait_for_file_text(path: Path, needle: str, timeout_s: float = 8.0) -> str:
    ...
    while time.time() < deadline:
        ...
        if needle in last_text:
            return last_text
        time.sleep(0.1)
    raise bmuxError(f"Timed out waiting for {needle!r} from background split terminal: {last_text!r}")
...
def _first_terminal_surface_id(payload: dict) -> str:
    ...
    for row in surfaces:
        if row.get("type") == "terminal":
            ...
            if surface_id:
                return surface_id
    raise bmuxError(f"surface.list returned no terminal surface: {payload}")
...
def main() -> int:
    with bmux(SOCKET_PATH) as c:
        ...
        created_workspace = ""
        marker_path = Path(tempfile.gettempdir()) / f"bmux-bg-split-send-{time.time_ns()}.txt"
        try:
            ...
            token = f"BMUX_BG_SPLIT_SEND_{time.time_ns()}"
            command = (
                "python3 -c "
                + shlex.quote(
