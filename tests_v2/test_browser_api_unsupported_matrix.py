...
def _expect_not_supported(c: bmux, method: str, params: dict) -> None:
    try:
        ...
        if "not_supported" in text:
            return
        raise bmuxError(f"Expected not_supported for {method}, got: {text}")
    raise bmuxError(f"Expected not_supported for {method}, but call succeeded")
