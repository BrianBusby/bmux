...
SOCKET_PATH = os.environ.get("BMUX_SOCKET", "/tmp/bmux-debug.sock")
WORKSPACE_COUNT = 10
...
def _run(cmd: list[str]) -> str:
    try:
        ...
        raise bmuxError(f"Command timed out after 10s ({' '.join(cmd)}){detail}") from exc
...
def _socket_owner_pid(socket_path: str) -> int:
    ...
    for raw in output.splitlines():
        ...
        if pid != os.getpid():
            return pid
    raise bmuxError(f"Could not resolve bmux PID from socket owner list: {output!r}")
...
def _thread_count(pid: int) -> int:
    ...
    if len(lines) < 2:
        raise bmuxError(f"Unexpected ps -M output for PID {pid}: {output!r}")
    return len(lines) - 1
...
def _physical_footprint_bytes(pid: int) -> int:
    ...
    if not match:
        raise bmuxError(f"Could not parse physical footprint from vmmap output: {output[:400]!r}")
...
def _debug_terminals(client: bmux) -> list[dict]:
...
def _ready_background_terminals_for_workspaces(client: bmux, workspace_ids: list[str]) -> list[dict]:
