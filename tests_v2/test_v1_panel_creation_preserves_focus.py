...
def _send_v1(command: str, *, expect_ok: bool = True) -> str:
    ...
    if expect_ok and not payload.startswith("OK"):
        raise bmuxError(f"{command!r} failed: {payload!r}")
    return payload


def _focused_surface_id(client: bmux, workspace_id: str) -> str:
    surfaces = client.list_surfaces(workspace=workspace_id)
    for _, surface_id, focused in surfaces:
        if focused:
            return surface_id
    raise bmuxError(f"no focused surface in workspace {workspace_id}: {surfaces}")


def _surface_ids(client: bmux, workspace_id: str) -> set[str]:
    return {surface_id for _, surface_id, _ in client.list_surfaces(workspace=workspace_id)}


def _wait_for_surface(client: bmux, workspace_id: str, surface_id: str, *, timeout_s: float = 5.0) -> None:
    ...
    while time.time() < deadline:
        ...
        if surface_id in seen:
            return
        time.sleep(0.02)
    raise bmuxError(f"surface {surface_id!r} never appeared in workspace {workspace_id}: {seen}")


def _wait_for_current_workspace(client: bmux, workspace_id: str, *, timeout_s: float = 5.0) -> None:
    ...
    while time.time() < deadline:
        ...
        if current == workspace_id:
            return
        time.sleep(0.02)
    raise bmuxError(f"workspace selection never settled on {workspace_id!r}: current={current!r}")
...
def _sidebar_state(workspace_id: str) -> str:
    ...
    if payload.startswith("ERROR"):
        raise bmuxError(f"sidebar_state failed: {payload!r}")
    return payload
...
def main() -> int:
    ...
    with bmux(SOCKET_PATH) as client:
        try:
            ...
            target_directory = f"/tmp/bmux-v1-report-pwd-{int(time.time() * 1000)}"
            _send_v1(
