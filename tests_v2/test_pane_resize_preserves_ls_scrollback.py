...
DEFAULT_SOCKET_PATHS = ["/tmp/bmux-debug.sock", "/tmp/bmux.sock"]
...
def _extract_segment_lines(
    ...
    *,
    ...
    if not saw_start:
        raise bmuxError(f"start marker not found in scrollback: {start_marker}")
    if require_end and not saw_end:
        raise bmuxError(f"end marker not found in scrollback: {end_marker}")
    return out


def _run_once(socket_path: str) -> int:
    workspace_id = ""
    fixture_dir = Path(tempfile.mkdtemp(prefix="bmux-ls-resize-regression-"))
    try:
        with bmux(socket_path) as client:
            ...
            start_marker = f"BMUX_LS_SCROLLBACK_START_{secrets.token_hex(4)}"
            end_marker = f"BMUX_LS_SCROLLBACK_END_{secrets.token_hex(4)}"
...
def main() -> int:
    ...
    if last_error is not None:
        raise last_error
    raise bmuxError("No socket candidates configured")
