...
def _find_pane_for_surface(c: bmux, surface_id: str) -> str:
    for _pidx, pane_id, _count, _focused in c.list_panes():
        ids = _pane_surface_ids(c, pane_id)
        if surface_id in ids:
            return pane_id
    raise bmuxError(f"Surface not found in any pane: {surface_id}")
...
def _wait_pane_for_surface(c: bmux, surface_id: str, timeout: float = 6.0) -> str:
    ...
    with bmux(SOCKET_PATH) as c:
        ...
        if not moved:
            if s_move in after_src:
                raise bmuxError(f"Expected moved surface to leave source pane (src={src_pane}, ids={after_src})")
            if s_move not in after_dst:
                raise bmuxError(f"Expected moved surface in destination pane (dst={dst_pane}, ids={after_dst})")
        ...
        if len(after_dst) < 2:
            ...
            if not _wait(lambda: extra in _pane_surface_ids(c, dst_pane)):
                raise bmuxError("Failed to create extra destination surface for reorder test")
            after_dst = _pane_surface_ids(c, dst_pane)
        ...
        if s_move not in reordered:
            raise bmuxError(f"Expected moved surface to remain in destination pane after reorder (ids={reordered})")
        if reordered[0] != s_move:
            raise bmuxError(f"Expected moved surface at front after reorder (ids={reordered})")
        ...
        if not ordered_ws:
            raise bmuxError("workspace.list returned empty after reorder")
        if ordered_ws[0] != ws2:
            raise bmuxError(f"Expected ws2 first after reorder (ordered={ordered_ws}, ws2={ws2})")
