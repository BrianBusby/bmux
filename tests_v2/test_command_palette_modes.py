...
def _palette_input_selection(client: bmux, window_id: str) -> dict:
...
def main() -> int:
    with bmux(SOCKET_PATH) as client:
        ...
        if not switched_rows:
            raise bmuxError("switcher returned no rows for workspace query")
        top_id = str((switched_rows[0] or {}).get("command_id") or "")
        if not top_id.startswith("switcher."):
            raise bmuxError(f"expected switcher row on top for cmd+p query, got: {switched_rows[0]}")
        ...
        if not command_rows:
            raise bmuxError("commands mode returned no rows")
        top_command_id = str((command_rows[0] or {}).get("command_id") or "")
        if not top_command_id.startswith("palette."):
            raise bmuxError(f"expected command row in commands mode, got: {command_rows[0]}")
