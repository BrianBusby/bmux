...
def _wait_for(pred, msg: str, timeout_s: float = 5.0, step_s: float = 0.05):
    ...
    while True:
        ...
        if time.time() >= deadline:
            raise bmuxError(f"Timed out waiting for condition: {msg}")
        time.sleep(step_s)


def _panes(client: bmux, workspace_id: str) -> list:
    payload = client._call("pane.list", {"workspace_id": workspace_id}) or {}
    return payload.get("panes") or []


def _surface_ids_by_pane(client: bmux, workspace_id: str) -> dict:
    return {
...
def _focused_pane_id(client: bmux, workspace_id: str) -> str:
    for row in _panes(client, workspace_id):
