...
def _wait_for_materialized_paths(client: bmux, surface_id: str, expected_count: int, timeout: float = 10.0) -> list[str]:
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.2)
    raise bmuxError(f"Timed out waiting for {expected_count} materialized image paths: {last[-1000:]!r}")


def _wait_for_terminal_text(client: bmux, surface_id: str, predicate, timeout: float = 10.0) -> str:
    deadline = time.time() + timeout
    last = ""
    while time.time() < deadline:
        last = client.read_terminal_text(surface_id)
        if predicate(last):
            return last
        time.sleep(0.2)
    raise bmuxError(f"Timed out waiting for terminal predicate: {last[-1000:]!r}")


def _run_drop_case(client: bmux, expected_paths: list[str], payload: str) -> tuple[str, str]:
...
def _run_bracketed_paste_case(client: bmux, expected_paths: list[str], payload: str, temp_dir: Path) -> tuple[str, bytes, list[float]]:
...
def main() -> int:
    temp_dir = Path(tempfile.mkdtemp(prefix="bmux-local-multi-image-drop-"))
