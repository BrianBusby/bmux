#!/usr/bin/env python3
"""
Visual Screenshot Tests for bmux
...
def take_screenshot(client: bmux, label: str) -> Optional[Screenshot]:
...
def capture_state(client: bmux) -> str:
    try:
...
def stamp_terminals(client: bmux, label: str) -> None:
    ...
    for h in terminal_surfaces:
        ...
        try:
            ...
            client.send_surface(idx, f"echo BMUX_VIS {safe} surf={idx}\n")
        except Exception:
            pass


def capture(client: bmux, label: str):
...
def reset_workspace(client: bmux) -> bmux:
...
def surface_count(client: bmux) -> int:
    return len(client.list_surfaces())

def pane_count(client: bmux) -> int:
...
def wait_surface_count(client: bmux, expected: int, timeout: float = 3.0) -> bool:
    start = time.time()
...
def wait_url_contains(client: bmux, panel_id: str, needle: str, timeout: float = 8.0) -> bool:
...
def cleanup_workspaces(client: bmux):
...
def _verify_surface_responsive(client: bmux, surface_idx: int, marker: Path,
                                retries: int = 3) -> bool:
...
def verify_views_in_window(client: bmux, label: str = "", timeout: float = 5.0) -> Optional[str]:
...
def verify_all_responsive(client: bmux, label: str = "") -> Optional[str]:
    ...
    for idx, h in enumerate(terminal_surfaces):
        surface_idx = h["index"]
        marker = Path(tempfile.gettempdir()) / f"bmux_vis_{os.getpid()}_{idx}"
        try:
...
def test_a1_initial_state(client: bmux) -> StateChange:
...
def test_a2_split_right(client: bmux) -> StateChange:
...
def test_a3_split_down(client: bmux) -> StateChange:
...
def _close_and_verify(client: bmux, change: StateChange, close_idx: Union[int, str],
...
def test_b4_close_right(client: bmux) -> StateChange:
...
def test_b5_close_left(client: bmux) -> StateChange:
...
def test_b6_close_bottom(client: bmux) -> StateChange:
...
def test_b7_close_top(client: bmux) -> StateChange:
...
def test_c8_3way_close_middle(client: bmux) -> StateChange:
...
def test_c9_grid_close_topleft(client: bmux) -> StateChange:
...
def test_c10_grid_close_bottomright(client: bmux) -> StateChange:
...
def test_d11_nested_close_bottomright(client: bmux) -> StateChange:
...
def test_d12_nested_close_top(client: bmux) -> StateChange:
...
def test_d13_4pane_close_second(client: bmux) -> StateChange:
...
def test_e14_browser_close_terminal(client: bmux) -> StateChange:
...
def test_e15_browser_close_browser(client: bmux) -> StateChange:
...
def test_f16_nested_tabs_close_first(client: bmux) -> StateChange:
...
def test_g17_rapid_down_close_top(client: bmux) -> StateChange:
...
def test_g18_rapid_right_close_left(client: bmux) -> StateChange:
...
def test_g19_alternating_close_reverse(client: bmux) -> StateChange:
...
def test_h20_workspace_switch_back(client: bmux) -> StateChange:
...
def _create_browser_surface(client: bmux, url: Optional[str] = None) -> str:
    return client.new_surface(panel_type="browser", url=url)


def test_i21_browser_drag_split_right_wait_load(client: bmux) -> StateChange:
...
def test_i22_browser_drag_split_right_immediate(client: bmux) -> StateChange:
...
def test_i23_browser_drag_split_right_webview_focused(client: bmux) -> StateChange:
...
def test_i24_browser_drag_split_right_focus_bounce(client: bmux) -> StateChange:
...
def test_i25_browser_drag_split_right_then_switch_panes(client: bmux) -> StateChange:
...
def test_i26_browser_drag_split_right_initial_url(client: bmux) -> StateChange:
...
def test_i27_browser_drag_split_right_after_reload(client: bmux) -> StateChange:
...
def test_i28_browser_drag_split_right_double_drag(client: bmux) -> StateChange:
...
<html>
<head>
    <title>bmux Visual Test Report</title>
    <style>
        body {
...
def run_visual_tests():
    ...
    print("=" * 60)
    print(f"bmux Visual Screenshot Tests ({len(test_fns)} scenarios)")
    print("=" * 60)
    print()
