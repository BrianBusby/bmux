...
def _rename_surface(client: bmux, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: bmux, workspace_id: str) -> str:
...
def main() -> int:
    with bmux(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise bmuxError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise bmuxError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise bmuxError("switcher returned no rows for renamed surface query")
