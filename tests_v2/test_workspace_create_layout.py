...
def _wait_for_text(c: bmux, workspace_id: str, needle: str, timeout_s: float = 8.0) -> str:
    ...
    while time.time() < deadline:
        ...
        if needle in last_text:
            return last_text
        time.sleep(0.1)
    raise bmuxError(f"Timed out waiting for {needle!r} in panel text: {last_text!r}")


def _pane_count(c: bmux, workspace_id: str) -> int:
    payload = c._call("pane.list", {"workspace_id": workspace_id}) or {}
    return len(payload.get("panes") or [])


def _surface_list(c: bmux, workspace_id: str) -> list:
    payload = c._call("surface.list", {"workspace_id": workspace_id}) or {}
    return list(payload.get("surfaces") or [])


def _close_workspace_quietly(c: bmux, workspace_id: str) -> None:
    try:
...
def _create_and_get_id(c: bmux, params: dict) -> str:
...
def test_horizontal_split(c: bmux) -> None:
...
def test_nested_splits(c: bmux) -> None:
        ...
        ws = _create_and_get_id(c, {
            ...
            "layout": {
                "pane": {
                    "surfaces": [{
                        "type": "terminal",
                        "env": {"BMUX_LAYOUT_TEST_TOKEN": token},
                    }],
                },
            },
        })
...
def test_surface_commands(c: bmux) -> None:
...
def test_multi_surface_pane(c: bmux) -> None:
...
def test_malformed_layout_rejected(c: bmux) -> None:
    ...
    try:
        ...
        raise bmuxError("Expected error for malformed layout, but call succeeded")
...
def test_empty_surfaces_rejected(c: bmux) -> None:
    ...
    try:
        ...
        raise bmuxError("Expected error for empty surfaces, but call succeeded")
    except bmuxError as e:
        if "invalid_params" in str(e) or "Invalid layout" in str(e):
            pass  # expected
        else:
...
def test_layout_overrides_initial_command_and_env(c: bmux) -> None:
    ...
    try:
        ...
        c.send_surface(
            surface_id,
            "printf 'LAYOUT_ENV=%s INITIAL_ENV=%s\\n' \"$BMUX_LAYOUT_TEST_TOKEN\" \"$SHOULD_NOT_EXIST\"\\n",
        )
