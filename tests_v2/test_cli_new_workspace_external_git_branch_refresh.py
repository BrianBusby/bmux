...
def _resolve_socket_path() -> str:
    ...
    if not re.fullmatch(r"/tmp/bmux-debug-[^/]+\.sock", socket_path):
        raise bmuxError(f"BMUX_SOCKET_PATH must be a tagged debug socket, got: {socket_path!r}")
    return socket_path
...
def _find_cli_binary() -> str:
    ...
    candidates = glob.glob(
        os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/bmux"),
        recursive=True,
    )
...
def _create_git_repo(root: Path) -> Path:
    ...
    subprocess.run(
        ["git", "config", "user.name", "bmux-test"],
        cwd=repo,
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ...
    subprocess.run(
        ["git", "config", "user.email", "bmux-test@example.com"],
        cwd=repo,
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
...
def main() -> int:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="bmux_issue_915_external_git_"))
