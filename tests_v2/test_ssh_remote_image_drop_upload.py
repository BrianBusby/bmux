#!/usr/bin/env python3
"""Regression: image drops into bmux ssh terminals upload to the remote host."""
...
DEFAULT_CMD_TIMEOUT = float(os.environ.get("BMUX_TEST_CMD_TIMEOUT", "60"))
SLOW_CMD_TIMEOUT = float(os.environ.get("BMUX_TEST_SLOW_CMD_TIMEOUT", "300"))
...
def _run(
    ...
    *,
    ...
    try:
        ...
        raise bmuxError(f"Command timed out after {timeout}s ({' '.join(cmd)})") from exc
...
def _wait_for_ssh(host: str, host_port: int, key_path: Path, timeout: float = 20.0) -> None:
    ...
    while time.time() < deadline:
        ...
        if probe.returncode == 0 and "ready" in probe.stdout:
            return
        time.sleep(0.4)
    raise bmuxError("Timed out waiting for SSH server in docker fixture")


def _wait_remote_ready(client: bmux, workspace_id: str, timeout: float = 45.0) -> None:
    deadline = time.time() + timeout
    last_status = {}
    while time.time() < deadline:
...
def _run_cli_json(
    ...
    *,
    ...
    try:
        ...
        raise bmuxError(f"bmux {' '.join(args)} timed out after {timeout}s") from exc
    if proc.returncode != 0:
        raise bmuxError(f"bmux {' '.join(args)} failed: {(proc.stdout + proc.stderr).strip()}")
    try:
...
def _resolve_workspace_id(client: bmux, payload: dict) -> str:
...
def _wait_for_remote_drop_paths(client: bmux, surface_id: str, expected_count: int, timeout: float = 20.0) -> list[str]:
    pattern = re.compile(r"/tmp/bmux-drop-[0-9a-f-]+\.png")
    deadline = time.time() + timeout
    last = ""
    while time.time() < deadline:
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="bmux-ssh-image-drop-"))
    image_tag = f"bmux-ssh-image-drop:{secrets.token_hex(4)}"
    container_name = f"bmux-ssh-image-drop-{secrets.token_hex(4)}"
    workspace_id = ""
