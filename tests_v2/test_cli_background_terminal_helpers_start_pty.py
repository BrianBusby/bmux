...
def _extract_ref(output: str, kind: str) -> str:
    ...
    if not match:
        raise bmuxError(f"Could not find {kind} ref in CLI output: {output!r}")
...
def _wait_for_read_screen(cli: str, workspace_ref: str, surface_ref: str, token: str) -> str:
    ...
    while time.time() < deadline:
        ...
        if code == 0 and token in output:
            return output
        time.sleep(0.1)
    raise bmuxError(f"read-screen never observed {token!r} for {surface_ref}: {last_output!r}")


def _exercise_helper(cli: str, workspace_ref: str, surface_ref: str, label: str) -> None:
    token = f"BMUX_HELPER_PTY_{label}_{int(time.time() * 1000)}"
    _run_cli(
        cli,
        [
            "send",
...
def _find_unhosted_background_workspace(c: bmux, cli: str, baseline_ws: str) -> Tuple[str, List[str]]:
    ...
    try:
        ...
        raise bmuxSkip("could not create an unhosted background workspace for helper terminal regression")
    except Exception:
