...
def _wait_for_terminal_focus(c: bmux, panel_id: str, timeout_s: float = 8.0) -> None:
    start = time.time()
...
def _wait_for_text(c: bmux, panel_id: str, needle: str, timeout_s: float = 2.5) -> None:
    start = time.time()
    last = ""
    while time.time() - start < timeout_s:
    ...
    raise bmuxError(f"Timed out waiting for token in terminal text: {needle}\nLast tail:\n{tail}")


def _type_and_wait_visible(c: bmux, panel_id: str, cmd: str) -> bool:
...
def _wait_for_tmp_write(c: bmux, panel_id: str, tmp: str, token: str) -> None:
    ...
    raise bmuxError(f"Timed out waiting for tmp file write: {tmp}")
...
def main() -> int:
    with bmux(SOCKET_PATH) as c:
        ...
        panes = c.list_panes()
        if len(panes) < 2:
            raise bmuxError(f"expected multiple panes, got: {panes}")
        ...
        for i in range(6):
            ...
            token = f"BMUX_NEW_TAB_OK_{i}_{uuid.uuid4().hex[:10]}"
            tmp = f"/tmp/bmux_new_tab_{token}.txt"
