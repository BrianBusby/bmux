...
from bmux import bmuxError


SOCKET_PATH = os.environ.get("BMUX_SOCKET_PATH", "").strip()
if not SOCKET_PATH:
    raise bmuxError("BMUX_SOCKET_PATH is required (expected /tmp/bmux-debug-<tag>.sock)")
LAST_SOCKET_HINT_PATH = Path("/tmp/bmux-last-socket-path")
...
def main() -> int:
    ...
    _must("bmux" in version_out, f"--version output should mention bmux: {version_out!r}")

    legacy_socket_key = "BMUX_" + "SOCKET"
    conflict_env = dict(os.environ)
    conflict_env["BMUX_SOCKET_PATH"] = SOCKET_PATH
    conflict_env[legacy_socket_key] = "/tmp/bmux-conflicting-legacy.sock"
    ...
    _must("bmux" in conflict_version_out, f"--version with socket env conflict should mention bmux: {conflict_version_out!r}")
    ...
    _must("usage: bmux help" in conflict_help_command_help_out, f"help --help should show help command usage: {conflict_help_command_help_out!r}")
    ...
    _must("usage: bmux ping" in conflict_subcommand_help_out, f"subcommand --help should show command usage: {conflict_subcommand_help_out!r}")
    ...
    _must("BMUX_SOCKET_PATH" in conflict_out and "differ" in conflict_out, f"conflict error should name canonical socket env: {conflict_out!r}")

    # Debug builds should auto-resolve the active debug socket via /tmp/bmux-last-socket-path
    # when BMUX_SOCKET_PATH is not set.
    ...
    try:
        ...
        auto_env.pop("BMUX_SOCKET_PATH", None)
    ...
    ping_proc = _run([cli, "--socket", SOCKET_PATH, "--password", "ignored-in-bmuxonly", "ping"])
