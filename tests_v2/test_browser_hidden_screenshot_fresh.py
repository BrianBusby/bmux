...
def _focused_workspace_id(c: bmux) -> str:
...
def _decode_png_rgb(png: bytes) -> tuple[int, int, Iterable[tuple[int, int, int]]]:
    if not png.startswith(b"\x89PNG\r\n\x1a\n"):
        raise bmuxError("Screenshot payload is not a PNG")
    ...
    for _y in range(height):
        ...
        for i, val in enumerate(scanline):
            ...
            if filter_type == 0:
                ...
                raise bmuxError(f"Unsupported PNG filter type: {filter_type}")
...
def _screenshot_color_counts(c: bmux, surface_id: str) -> tuple[int, int, int, int, int]:
...
<html>
  <head>
    <title>bmux-hidden-screenshot-fresh</title>
    <style>
      html, body, #app {
        width: 100%;
        height: 100%;
...
(() => {
  ...
  window.__bmuxHiddenScreenshotState = 'green';
  return window.__bmuxHiddenScreenshotState;
})()
""".strip()
        ...
        _wait_until(
            lambda: str(
                _value(c._call("browser.eval", {"surface_id": surface_id, "script": "window.__bmuxHiddenScreenshotState || ''"}))
                or ""
            )
            == "green",
            timeout_s=3.0,
