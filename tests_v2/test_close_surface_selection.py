...
def _wait_focused_index(client: bmux, index: int, timeout: float = 4.0) -> bool:
    start = time.time()
...
def _ensure_surfaces(client: bmux, count: int) -> None:
    surfaces = client.list_surfaces()
...
def test_close_middle_keeps_index(client: bmux) -> TestResult:
