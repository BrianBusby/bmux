...
def _wait_for_proxy_conflict_status(client: bmux, workspace_id: str, expected_local_proxy_port: int, timeout: float = 30.0) -> dict:
    deadline = time.time() + timeout
    last_status = {}
    while time.time() < deadline:
    ...
    raise bmuxError(f"Remote did not reach structured proxy_unavailable status for bind conflict: {last_status}")
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="bmux-ssh-proxy-conflict-"))
    image_tag = f"bmux-ssh-test:{secrets.token_hex(4)}"
    container_name = f"bmux-ssh-proxy-conflict-{secrets.token_hex(4)}"
    workspace_id = ""
