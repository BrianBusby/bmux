...
def _wait_focused(client: bmux, expected_pane_id: str, timeout_s: float = 6.0) -> Optional[str]:
...
def _wait_url_loaded(client: bmux, browser_id: str, timeout_s: float = 10.0) -> str:
...
def test_goto_split_from_loaded_browser(client: bmux) -> tuple[bool, str]:
    """
...
def test_goto_split_roundtrip_loaded_browser(client: bmux) -> tuple[bool, str]:
    """
...
def run_tests() -> int:
    print("=" * 60)
    print("bmux Browser goto_split Regression Test")
    print("=" * 60)
    print()
    ...
    try:
        with bmux(socket_path=socket_path) as client:
