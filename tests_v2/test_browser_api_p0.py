...
<!doctype html>
<html>
  <head><title>bmux-browser-p0</title></head>
  <body>
    <input id='name' value=''>
...
""".strip()
        ...
        _must("bmux" in str(out.get("value", "")), f"Expected #out text to include 'bmux': {out}")
        ...
        _must(str(eval_res.get("value", "")) == "bmux", f"Expected eval value 'bmux': {eval_res}")

        snap = c._call("browser.snapshot", {"surface_id": target}) or {}
        snapshot_text = str(snap.get("snapshot") or "")
        _must("bmux-browser-p0" in snapshot_text, f"Expected snapshot text to include page title: {snap}")
    ...
    print("PASS: browser.* P0 methods work on bmux webview with ref handles")
    return 0
