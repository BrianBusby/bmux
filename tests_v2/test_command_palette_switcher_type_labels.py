...
def _open_switcher(client: bmux, window_id: str) -> None:
...
def main() -> int:
    with bmux(SOCKET_PATH) as client:
        ...
        if not rows:
            raise bmuxError("switcher returned no rows for token query")
        ...
        if not workspace_rows:
            raise bmuxError(f"expected workspace rows for switcher query: rows={rows}")
        if not surface_rows:
            raise bmuxError(f"expected surface rows for switcher query: rows={rows}")

        bad_workspace = [row for row in workspace_rows if str((row or {}).get("trailing_label") or "") != "Workspace"]
        if bad_workspace:
            raise bmuxError(f"workspace rows missing 'Workspace' trailing label: {bad_workspace}")

        bad_surface = [row for row in surface_rows if str((row or {}).get("trailing_label") or "") != "Surface"]
        if bad_surface:
            raise bmuxError(f"surface rows missing 'Surface' trailing label: {bad_surface}")
