...
def focused_pane_id(client: bmux) -> Optional[str]:
...
def wait_url_contains(client: bmux, panel_id: str, needle: str, timeout_s: float = 10.0) -> None:
    start = time.time()
...
def wait_focused_pane(client: bmux, expected_pane_id: str, timeout_s: float = 5.0) -> Optional[str]:
...
def test_cmd_ctrl_h_goto_split_left_from_webview(client: bmux) -> tuple[bool, str]:
    """
...
def test_cmd_opt_left_arrow_goto_split_left_from_webview(client: bmux) -> tuple[bool, str]:
    """
...
def main() -> int:
    print("bmux Browser Custom Keybind Tests")
    print("=" * 50)
    client = bmux()
    client.connect()
