#!/usr/bin/env python3
"""Integration: bmux ssh PTY sessions survive local surface detach and reattach."""
...
def _wait_remote_ready(client: bmux, workspace_id: str, timeout_s: float = 45.0) -> None:
    deadline = time.time() + timeout_s
    last_status = {}
    while time.time() < deadline:
...
def _workspace_row(client: bmux, workspace_id: str) -> dict:
    ...
    for row in rows:
        if str(row.get("id") or "") == workspace_id:
            return row
    raise bmuxError(f"workspace.list missing {workspace_id}: {rows}")


def _run_surface_probe(client: bmux, surface_id: str, command: str, token_prefix: str, timeout_s: float = 18.0) -> str:
    token = f"__BMUX_{token_prefix}_{secrets.token_hex(6)}__"
    client.send_surface(
        surface_id,
        (
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.15)
    raise bmuxError(f"Timed out waiting for probe {token!r}: {last[-1200:]!r}")


def _open_ssh_workspace(client: bmux, cli: str) -> tuple[str, str, str, str]:
    ...
    _must(session_id.startswith("ssh-"), f"bmux ssh did not create a persistent PTY session: {payload}")
    _must(persistent_slot.startswith("ssh-"), f"bmux ssh did not create a persistent daemon slot: {payload}")
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set BMUX_SSH_TEST_HOST to run ssh detachable PTY regression")
        return 0
    ...
    try:
        with bmux(SOCKET_PATH) as client:
            ...
            first_probe = _run_surface_probe(
                client,
                surface_id,
                f"export BMUX_DETACH_MARK={marker}; printf 'pid=%s marker=%s socket=%s' \"$$\" \"$BMUX_DETACH_MARK\" \"$BMUX_SOCKET_PATH\"",
                "SSH_DETACH_FIRST",
            )
            ...
            second_probe = _run_surface_probe(
                client,
                reattached_surface,
                "printf 'pid=%s marker=%s socket=%s' \"$$\" \"$BMUX_DETACH_MARK\" \"$BMUX_SOCKET_PATH\"",
                "SSH_DETACH_SECOND",
            )
    ...
    print("PASS: bmux ssh PTY survives local detach and reattaches to the same remote shell")
    return 0
