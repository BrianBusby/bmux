...
def _remote_session_count(client: bmux, workspace_id: str) -> int:
    row = _workspace_row(client, workspace_id)
    remote = row.get("remote") or {}
    return int(remote.get("active_terminal_sessions") or 0)


def _run_surface_probe(client: bmux, surface_id: str, command: str, token_prefix: str, timeout_s: float = 12.0) -> str:
    token = f"__BMUX_{token_prefix}_{int(time.time() * 1000)}__"
    client.send_surface(
        surface_id,
        (
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set BMUX_SSH_TEST_HOST to run ssh last-surface remote state regression")
        return 0
        ...
        with bmux(SOCKET_PATH) as client:
            ...
            for idx, surface_id in enumerate(local_surface_ids):
                socket_output = _run_surface_probe(
                    client,
                    surface_id,
                    r'''printf '%s' "${BMUX_SOCKET_PATH:-}"''',
                    f"SSH_LAST_SURFACE_SOCKET_{idx}",
                ).strip()
                _must(
                    not socket_output.startswith("127.0.0.1:"),
                    f"surface {surface_id} should be local after clearing remote state, got BMUX_SOCKET_PATH={socket_output!r}",
                )
    finally:
        if workspace_id:
            try:
