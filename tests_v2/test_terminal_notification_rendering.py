...
def _focused_surface_id(c: bmux) -> str:
    ...
    if not surfaces:
        raise bmuxError("Expected at least one terminal surface")
    return next((sid for _idx, sid, focused in surfaces if focused), surfaces[0][1])


def _surface_health_row(c: bmux, surface_id: str) -> Optional[dict]:
...
def _assert_surface_visible(c: bmux, surface_id: str, context: str) -> None:
    row = _surface_health_row(c, surface_id)
    if row is None:
        raise bmuxError(f"{context}: surface missing from health output: {surface_id}")
...
def _send_osc777_notification(c: bmux, surface_id: str, title: str, body: str) -> None:
...
def _wait_for_notification(c: bmux, title: str, surface_id: str) -> None:
    surface_id = surface_id.lower()
...
def _wait_for_terminal_text(c: bmux, surface_id: str, text: str) -> None:
    _wait_for(
...
def _assert_renders_after_notification(c: bmux, surface_id: str, marker: str) -> None:
...
def main() -> int:
    token = f"BMUX_OSC777_{int(time.time() * 1000)}"
