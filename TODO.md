...
- [x] `bmux ssh` creates remote workspace metadata and does not require `--name`
- [x] Remote daemon bootstrap/upload/start path with `bmuxd-remote serve --stdio`
...
- [ ] Extend `bmuxd-remote` RPC beyond `hello/ping` with proxy stream methods (`proxy.open|close`)
...
- [x] Add browser automation API inspired by `vercel-labs/agent-browser`, but backed by bmux's WKWebView (wait, click, type, eval, screenshot, etc.).
- [x] Finalize browser parity contract and command mapping decisions in `docs/agent-browser-port-spec.md`.
- [x] Add `bmux browser` command surface that mirrors agent-browser semantics and targets explicit `surface_id` handles.
...
- [x] Fix `bmux browser open|open-split|new` URL parsing so routing flags (`--workspace`, `--window`) are removed before URL construction.
...
- [x] Expand end-user skill docs with deep-linkable bmux-browser references/templates plus a new core `skills/bmux/` topology skill.
...
- [ ] Add "Install Claude Code integration" menu item in menubar
  ...
  - Implement as part of `bmux` CLI, menubar just triggers the CLI command
...
- [ ] Cmd+click should open links in bmux (browser panel) instead of external browser
...
- [x] Extend `bmux browser ...` CLI grammar for the new families (including aliases).
...
- [x] Re-run full `run-tests-v1.sh` and `run-tests-v2.sh` on `bmux-vm`.
