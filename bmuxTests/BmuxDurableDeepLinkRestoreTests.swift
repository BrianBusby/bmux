...
/// Regression coverage for https://github.com/manaflow-ai/bmux/issues/5486:
/// a copied `bmux://` deep link must keep resolving to the same logical
...
struct BmuxDurableDeepLinkRestoreTests {
    private let scheme = "bmux"

    private func parsedTarget(_ link: String) throws -> BmuxNavigationURLRequest.Target {
        let url = try #require(URL(string: link))
        switch BmuxNavigationURLRequest.parse(url, supportedSchemes: [scheme]) {
    ...
    @Test func workspaceLinkResolvesAfterRestoreWithRemintedIds() throws {
        ...
        let link = BmuxNavigationURLRequest.workspaceLink(
        ...
        let resolver = BmuxNavigationTargetResolver(
            workspaces: restored.tabs.map(\.bmuxNavigationDescriptor)
        )
    ...
    @Test func surfaceLinkResolvesToSameLogicalTabAfterRestore() throws {
        ...
        let link = BmuxNavigationURLRequest.surfaceLink(
            workspaceId: workspace.stableId,
    ...
    @Test func terminalRespawnPreservesStableSurfaceIdForLinks() throws {
        ...
        let resolver = BmuxNavigationTargetResolver(
            workspaces: manager.tabs.map(\.bmuxNavigationDescriptor)
        )
        let resolution = try resolver.resolve(parsedTarget(link))
    ...
    @Test func terminalContextMenuSurfaceLinkUsesMappedPanelStableId() throws {
        ...
        #expect(
            link == BmuxNavigationURLRequest.surfaceLink(
                workspaceId: workspace.stableId,
    ...
    @Test func duplicateReopenWithLiveIdentitiesMintsFreshOnes() throws {
        ...
        let resolver = BmuxNavigationTargetResolver(
            workspaces: (manager.tabs + duplicate.tabs).map(\.bmuxNavigationDescriptor)
        )
        let link = BmuxNavigationURLRequest.surfaceLink(
            workspaceId: workspace.stableId,
