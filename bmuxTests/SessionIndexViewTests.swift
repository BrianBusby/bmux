...
final class SessionIndexViewTests: XCTestCase {
    func testUserTranscriptRowsInvertTheActiveAppearance() {
        XCTAssertEqual(
            nsColorHex(SessionTranscriptRole.user.backgroundColor(colorScheme: .dark)),
            "#FFFFFFFF"
        )
        XCTAssertEqual(
            nsColorHex(SessionTranscriptRole.user.foregroundColor(colorScheme: .dark)),
            "#000000FF"
        )
        XCTAssertEqual(
            nsColorHex(SessionTranscriptRole.user.backgroundColor(colorScheme: .light)),
            "#000000FF"
        )
        XCTAssertEqual(
            nsColorHex(SessionTranscriptRole.user.foregroundColor(colorScheme: .light)),
            "#FFFFFFFF"
        )
    }
    ...
    func testClaudeResumeCommandPinsSnapshotConfigDirectory() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-session-index-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertEqual(
            entry.resumeCommand,
            posixShWrappedForTest("env CLAUDE_CONFIG_DIR='\(configDir.path)' BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1 BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR \"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" --resume claude-session-123")
        )
    }
    ...
    // Regression for https://github.com/manaflow-ai/bmux/issues/5262.
    // bmux captures Codex's *internal* sandbox-policy `type`, which is a superset of
    ...
    func testCodexResumeCommandReproducesBypassFlagForDisabledSandbox() {
        ...
        // The codex resume now routes the codex executable through the bmux codex
    ...
    func testCodexSQLSearchMatchesRolloutTranscriptContent() async throws {
        ...
        {"timestamp":"2026-05-01T09:01:00.000Z","type":"response_item","payload":{"type":"function_call","name":"exec_command","arguments":"{\\"cmd\\":\\"tail -n 80 /tmp/bmux-debug-task-activation-performance-harness.log\\"}","call_id":"call_1"}}
        """
        ...
        let outcome = await SessionIndexStore.loadCodexEntriesForTesting(
            ...
            needle: "/tmp/bmux-debug-task-activation-performance-harness.log",
            offset: 0,
            limit: 10,
    ...
    // Regression for https://github.com/manaflow-ai/bmux/issues/6302.
    ...
    private func nsColorHex(_ color: Color) -> String? {
        NSColor(color).usingColorSpace(.sRGB)?.hexString(includeAlpha: true)
    }
    ...
    /// (fish/csh/tcsh). https://github.com/manaflow-ai/bmux/issues/5639
