...
struct TerminalClearScreenKeepScrollbackTests {
    ...
    func remappedDefaultShortcutDoesNotTriggerStaleMenuSuppression() throws {
        ...
        withTemporaryShortcut(action: .clearScreenKeepScrollback, shortcut: .unbound) {
            #expect(!appDelegate.shouldSuppressStaleBmuxMenuShortcut(event: event))
        }
    }

    @Test
    func clearScreenKeepScrollbackDeliversFormFeedToForegroundProgram() throws {
        let readyMarker = "BMUX_CLEAR_READY_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let captureMarker = "BMUX_CLEAR_HEX_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-clear-keep-scrollback-\(UUID().uuidString).py")
        let script = """
        import os
        import select
        import sys
