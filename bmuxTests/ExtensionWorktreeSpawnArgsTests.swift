...
/// Regression coverage for https://github.com/manaflow-ai/bmux/issues/5032.
///
...
struct ExtensionWorktreeSpawnArgsTests {
    private func makeResult(setupCommand: String) -> BmuxExtensionWorktreeCreationResult {
        BmuxExtensionWorktreeCreationResult(
            worktreePath: "/tmp/project/.bmux/worktrees/bmux-sidebar-123",
            workspaceTitle: "bmux-sidebar-123",
    ...
    func setupCommandRunsAsShellInput() {
        ...
        // interactive shell, matching the `bmux new-workspace --cwd` contract.
    ...
    func worktreePathIsWorkingDirectory() {
        ...
        #expect(args.workingDirectory == "/tmp/project/.bmux/worktrees/bmux-sidebar-123")
        #expect(args.inheritWorkingDirectory == false)
        #expect(args.title == "bmux-sidebar-123")
    }
