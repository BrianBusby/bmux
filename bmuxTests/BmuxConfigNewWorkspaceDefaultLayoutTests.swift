...
/// saved default through `BmuxConfigStore`, and the plus-menu
/// default-layout submenu model.
struct BmuxConfigNewWorkspaceDefaultLayoutTests {
    private func temporaryRoot(_ label: String) throws -> URL {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-workspace-action-\(label)-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    private func decodeJSONC(_ source: String) throws -> BmuxConfigFile {
    ...
    @Test func setNewWorkspaceDefaultActionPreservesCommentsAndResolves() throws {
        ...
        try BmuxConfigActionSaver.setNewWorkspaceDefaultAction(
            id: "review-setup",
        ...
        let store = BmuxConfigStore(globalConfigPath: configPath)
        store.loadAll()
    ...
    @Test func setNewWorkspaceDefaultActionReplacesExistingValue() throws {
        ...
        try BmuxConfigActionSaver.setNewWorkspaceDefaultAction(id: "second", globalConfigPath: configPath)
        try BmuxConfigActionSaver.setNewWorkspaceDefaultAction(id: "second", globalConfigPath: configPath)
    ...
    @Test func setNewWorkspaceDefaultActionCreatesMissingNewWorkspaceObject() throws {
        ...
        try BmuxConfigActionSaver.setNewWorkspaceDefaultAction(id: "layout", globalConfigPath: configPath)
    ...
    @Test func unsetNewWorkspaceDefaultActionRemovesKeyAndIsIdempotent() throws {
        ...
        try """
        {
          ...
          "ui": {
            "newWorkspace": {
              "contextMenu": ["bmux.newTerminal"], // keep menu
              "action": "layout"
            }
          }
        }
        ...
        try BmuxConfigActionSaver.setNewWorkspaceDefaultAction(id: nil, globalConfigPath: configPath)
        let saved = try String(contentsOfFile: configPath, encoding: .utf8)
    ...
    @Test func setNewWorkspaceDefaultActionFailsClosedForMalformedConfig() throws {
        ...
        #expect(throws: (any Error).self) {
            try BmuxConfigActionSaver.setNewWorkspaceDefaultAction(id: "layout", globalConfigPath: brokenPath)
        }
        ...
        #expect(throws: (any Error).self) {
            try BmuxConfigActionSaver.setNewWorkspaceDefaultAction(id: "layout", globalConfigPath: nonObjectPath)
        }
    ...
    @Test func saveWorkspaceActionThenSetAsDefaultResolvesNewID() throws {
        ...
        let result = try BmuxConfigActionSaver.saveWorkspaceAction(
            title: "Review Setup",
            definition: BmuxWorkspaceDefinition(name: "Review"),
            globalConfigPath: configPath
        )
        try BmuxConfigActionSaver.setNewWorkspaceDefaultAction(
            id: result.actionID,
    ...
    @Test func newWorkspaceDefaultLayoutMenuModelBuildsSortedState() throws {
        let zebra = try #require(BmuxResolvedConfigAction.fromDefinition(
            id: "zebra",
            definition: BmuxConfigActionDefinition(
                action: .workspace(BmuxWorkspaceDefinition(name: "Zebra"), restart: nil),
                title: "Zebra"
            ),
            sourcePath: nil
        ))
        let alphaTwo = try #require(BmuxResolvedConfigAction.fromDefinition(
            id: "alpha-2",
            definition: BmuxConfigActionDefinition(
                action: .workspace(BmuxWorkspaceDefinition(name: "Alpha"), restart: nil),
                title: "Alpha"
            ),
            sourcePath: nil
        ))
        let alphaOne = try #require(BmuxResolvedConfigAction.fromDefinition(
            id: "alpha-1",
            definition: BmuxConfigActionDefinition(
                action: .workspace(BmuxWorkspaceDefinition(name: "Alpha"), restart: nil),
                title: "Alpha"
    ...
    @Test func newWorkspaceDefaultLayoutMenuModelListsOnlyWorkspaceLayouts() throws {
        let layout = try #require(BmuxResolvedConfigAction.fromDefinition(
            id: "layout",
            definition: BmuxConfigActionDefinition(
                action: .workspace(BmuxWorkspaceDefinition(name: "Layout"), restart: nil),
                title: "Layout"
            ),
            sourcePath: nil
        ))
        let workspaceCommand = try #require(BmuxResolvedConfigAction.fromDefinition(
            id: "ws-dev",
            definition: BmuxConfigActionDefinition(
        ...
        let builtIn = BmuxResolvedConfigAction.builtIn(.splitRight)
    ...
    @Test func deleteActionClearsMatchingNewWorkspaceDefault() throws {
        ...
        try BmuxConfigActionSaver.deleteAction(id: "review-setup", globalConfigPath: configPath)
