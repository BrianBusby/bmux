...
final class PiVaultAgentPersistenceTests: XCTestCase {
    ...
    func testBuiltInPiRegistrationUsesBrandedIconAsset() {
        let agent = RegisteredSessionAgent(registration: BmuxVaultAgentRegistration.builtInPi)
    ...
    func testBuiltInAntigravityRegistrationUsesBrandedIconAsset() {
        let agent = RegisteredSessionAgent(registration: BmuxVaultAgentRegistration.builtInAntigravity)
        ...
        XCTAssertEqual(BmuxVaultAgentRegistration.builtInAntigravity.detect.processNames, ["agy", "antigravity"])
    }

    func testBuiltInAntigravityRegistrationLoadsHistoryDisplayAndWorkspace() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-antigravity-vault-history-\(UUID().uuidString)", isDirectory: true)
        ...
        var registration = BmuxVaultAgentRegistration.builtInAntigravity
    ...
    func testBuiltInAntigravityRegistrationIndexesEachHistoryConversation() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-antigravity-vault-conversations-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testRegisteredAgentJSONLWorkspaceKeyIsSharedCWDMetadata() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-registered-workspace-cwd-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        ...
        let registration = BmuxVaultAgentRegistration(
            id: "acme-agent",
            name: "Acme Agent",
            detect: BmuxVaultAgentDetectRule(processName: "acme-agent"),
    ...
    func testRegisteredAgentJSONLDisplayFieldIsNotSharedTitleMetadata() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-registered-display-title-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testRegisteredAgentJSONLSessionIDDoesNotUseAntigravityConversationID() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-registered-session-id-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testBuiltInGrokRegistrationUsesNativeSessionDirectory() {
        let registration = BmuxVaultAgentRegistration.builtInGrok
    ...
    func testRegisteredAgentJSONLNativeSessionIDOverridesPathFallback() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-registered-native-id-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testRegisteredAgentCWDFilterUsesJSONLMetadataNotFallback() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-registered-cwd-filter-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testRegisteredAgentMetadataKeepsScanningForBranchWhenFallbackCWDSet() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-pi-vault-branch-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiJSONLTypedContentBlocksUseFirstUserTextAsTitle() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-pi-vault-title-blocks-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiJSONLTopLevelAssistantTypedContentDoesNotBecomeTitle() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-pi-vault-top-level-role-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiJSONLMessagesArrayUsesNilRoleTextAsTitle() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-pi-vault-messages-nil-role-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiJSONLTypedContentBlocksRequireTextType() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-pi-vault-typed-content-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testGrokVaultLoadsNativeChatHistoryFromEncodedDirectory() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-vault-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        ...
        var registration = BmuxVaultAgentRegistration.builtInGrok
    ...
    func testGrokVaultTitlePrefersUserQueryOverInjectedMetadata() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-vault-metadata-title-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testGrokVaultFindsBranchAfterStableMetadata() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-vault-late-branch-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testGrokVaultLoadsHookObservedShellGrokHome() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-vault-observed-home-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }

        let homeDirectory = tempDir.appendingPathComponent("home", isDirectory: true)
        let hookStore = homeDirectory
            .appendingPathComponent(".bmuxterm", isDirectory: true)
    ...
    func testGrokVaultLoadsHookObservedShellGrokHomeFromCustomStateDir() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-vault-custom-state-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        ...
        let entries = await SessionIndexStore.loadGrokEntries(
            ...
            offset: 0,
            limit: 10,
            environment: ["BMUX_AGENT_HOOK_STATE_DIR": hookStateDir.path],
            homeDirectory: homeDirectory.path
        )
    ...
    func testRegisteredGrokSessionDirectoryUsesNativeDirectoryLayout() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-registered-grok-vault-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        ...
        let registration = BmuxVaultAgentRegistration(
            id: "custom-grok",
            name: "Custom Grok",
            detect: BmuxVaultAgentDetectRule(processName: "custom-grok"),
    ...
    func testGrokVaultCWDFilterUsesEncodedProjectDirectory() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-vault-filter-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testGrokAgentSearchScopeUsesCurrentDirectoryCWDFilter() async throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-agent-scope-filter-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testPiVaultAgentSnapshotRoundTripBuildsTargetedSessionCommand() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-pi-vault-agent-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
            ...
            SessionPanelSnapshot(
                ...
                terminal: SessionTerminalPanelSnapshot(
                    ...
                    agent: SessionRestorableAgentSnapshot(
                        ...
                        registration: BmuxVaultAgentRegistration.builtInPi
                    ),
