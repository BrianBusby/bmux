...
struct CrashDiagnosticSessionPolicyTests {
    @Test
    func terminalDefaultFileOpenIgnoresGhosttyCrashReportsInBmuxCrashDirectory() {
        let crashReport = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/bmux/crash/bmux.ghosttycrash", isDirectory: false)
    ...
    @Test
    func terminalDefaultFileOpenIgnoresSymlinkedGhosttyCrashReportsInBmuxCrashDirectory() throws {
        let crashReport = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/bmux/crash/bmux.ghosttycrash", isDirectory: false)
        let symlink = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-symlinked-crash-\(UUID().uuidString).ghosttycrash", isDirectory: false)
    ...
    func appDelegateSessionSnapshotDropsCrashDiagnosticWindow() throws {
        ...
        let projectDirectory = "/tmp/bmux-project"
        ...
        let crashDirectory = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/state/bmux/crash", isDirectory: true)
            .path
    ...
    func sessionSnapshotDropsEmptyCrashDiagnosticWorkspace() {
        ...
        let pruned = SessionPersistencePolicy.pruningBmuxCrashDiagnosticWindows(from: snapshot)
    ...
    func sessionSnapshotPruningDoesNotResolveSymlinkedCrashDirectories() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-crash-storage-symlink-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(!SessionPersistencePolicy.isBmuxCrashStoragePath(
            symlink.path,
        ...
        #expect(!SessionPersistencePolicy.isBmuxCrashDiagnosticWindow(
            window,
            homeDirectory: homeDirectory,
            environment: [:]
        ))
