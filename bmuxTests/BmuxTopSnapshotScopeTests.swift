...
final class BmuxTopSnapshotScopeTests: XCTestCase {
    ...
    func testWindowRollupMatchesPSForApplicationProcessTree() throws {
        ...
        let snapshot = BmuxTopProcessSnapshot.capture(includeProcessDetails: false)
    ...
    func testApplicationProcessTreeIsExposedAtWindowLevel() throws {
        ...
        let snapshot = BmuxTopProcessSnapshot.capture(includeProcessDetails: true)
        var windows: [[String: Any]] = [[
            "kind": "window",
            "id": UUID().uuidString,
            "index": 0,
    ...
    func testSamplePayloadDescribesPhysicalFootprintFallbackSource() {
        let sample = BmuxTopProcessSnapshot.capture(includeProcessDetails: false).samplePayload()

        XCTAssertEqual(
            sample["memory_source"] as? String,
            BmuxTopProcessMemorySource.physicalFootprint.rawValue
        )
        XCTAssertEqual(
            sample["memory_fallback_source"] as? String,
            BmuxTopProcessMemorySource.residentSize.rawValue
        )
        XCTAssertEqual(
            sample["resident_memory_fallback_source"] as? String,
            BmuxTopProcessMemorySource.rusageResidentSize.rawValue
        )
        XCTAssertEqual(sample["bmux_scope"] as? Bool, true)

        let unscopedSnapshot = BmuxTopProcessSnapshot(
            processes: [],
            sampledAt: Date(timeIntervalSince1970: 0),
            includesProcessDetails: false,
            includesBMUXScope: false
        )
        XCTAssertEqual(unscopedSnapshot.samplePayload()["bmux_scope"] as? Bool, false)

        let fallbackSnapshot = BmuxTopProcessSnapshot(
            processes: [
                BmuxTopProcessInfo(
                    pid: 1,
                    parentPID: 0,
    ...
    func testKernProcArgsWorkspaceID() {
        ...
        let scope = BmuxTopProcessSnapshot.bmuxScope(fromKernProcArgs: bytes)
    ...
    func testKernProcArgsSurfaceID() {
        ...
        let bytes = kernProcArgs(environment: [
            "BMUX_SURFACE_ID=\(surfaceID.uuidString)"
        ])
    ...
    func testCodexMonitorArgumentsSupportJoinedUUIDOptions() throws {
        ...
        let scope = try XCTUnwrap(BmuxTopProcessSnapshot.bmuxScope(
            arguments: [
        ...
        XCTAssertEqual(scope.workspaceID, workspaceID)
        XCTAssertEqual(scope.surfaceID, surfaceID)
        XCTAssertEqual(scope.attributionReason, "bmux-hook-arguments")
    }

    func testCodexMonitorArgumentsIgnorePathValuedSubcommandLookalikes() {
        let workspaceID = UUID(uuidString: "55555555-5555-5555-5555-555555555555")!

        let scope = BmuxTopProcessSnapshot.bmuxScope(
            arguments: [
                "/Applications/bmux.app/Contents/Resources/bin/bmux",
                "other",
                "/tmp/hooks",
    ...
    func testCodexMonitorArgumentsRequireBmuxExecutable() {
        let workspaceID = UUID(uuidString: "55555555-5555-5555-5555-555555555555")!
    ...
    func testLaunchdParentedCodexMonitorArgumentsAttachToOwningSurface() throws {
        ...
        let scope = try XCTUnwrap(BmuxTopProcessSnapshot.bmuxScope(fromKernProcArgs: bytes))
        XCTAssertEqual(scope.workspaceID, workspaceID)
        XCTAssertEqual(scope.surfaceID, surfaceID)
        ...
        let snapshot = BmuxTopProcessSnapshot(
            processes: [
                BmuxTopProcessInfo(
                    ...
                    parentPID: 1,
                    name: "bmux",
                    path: "/Applications/bmux.app/Contents/Resources/bin/bmux",
                    ttyDevice: nil,
                    bmuxWorkspaceID: scope.workspaceID,
                    bmuxSurfaceID: scope.surfaceID,
                    bmuxAttributionReason: scope.attributionReason,
                    processGroupID: nil,
                    terminalProcessGroupID: nil,
                    cpuPercent: 0,
        ...
        XCTAssertEqual(monitorProcess["attribution_reason"] as? String, "bmux-hook-arguments")
    ...
    private struct SpawnedProcessTree {
        ...
        static func start() throws -> SpawnedProcessTree {
            let directory = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("bmux-top-\(UUID().uuidString)", isDirectory: true)
