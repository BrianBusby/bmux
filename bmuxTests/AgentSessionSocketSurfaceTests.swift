...
struct AgentSessionSocketSurfaceTests {
    ...
    func testWorkspaceSessionSnapshotPersistsAgentSessionWorkingDirectory() throws {
        ...
        let panel = try #require(
            workspace.newAgentSessionSurface(
                ...
                providerID: .codex,
                rendererKind: .react,
                workingDirectory: "/tmp/bmux-agent-session-cwd",
                focus: true
            )
        )
        ...
        expectEqual(panelSnapshot.directory, "/tmp/bmux-agent-session-cwd")
        expectEqual(panelSnapshot.agentSession?.workingDirectory, "/tmp/bmux-agent-session-cwd")
    }
    ...
    func testAgentSessionProcessStorePersistsRawActivityOutput() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-session-raw-output-\(UUID().uuidString)", isDirectory: true)
        defer {
