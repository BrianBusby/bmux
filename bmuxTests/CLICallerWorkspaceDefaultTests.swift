...
/// workspace (the `BMUX_WORKSPACE_ID` the app injects into every terminal surface)
...
/// `--workspace "${BMUX_WORKSPACE_ID:-}"` expands to an empty value whenever a
...
struct CLICallerWorkspaceDefaultTests {
    ...
    /// dangerous case where `--workspace "${BMUX_WORKSPACE_ID:-}"` expands to an empty
    ...
    /// visibly retarget there. Pass `callerWorkspaceId: nil` to omit `BMUX_WORKSPACE_ID`.
    ...
    private func cliEnvironment(socketPath: String, callerWorkspaceId: String?) -> [String: String] {
        ...
        environment.removeValue(forKey: "BMUX_WINDOW_ID")
