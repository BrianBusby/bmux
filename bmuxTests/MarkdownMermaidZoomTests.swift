...
final class MarkdownMermaidZoomTests {
    ...
    func renderedMermaidDiagramScalesWithViewerZoom() async throws {
        let markdownURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-markdown-mermaid-zoom-\(UUID().uuidString).md")
        let frame = NSRect(x: 0, y: 0, width: 720, height: 480)
        let configuration = WKWebViewConfiguration()
        let mermaidHandler = MarkdownMermaidStubHandler()
        configuration.userContentController.add(mermaidHandler, name: "bmuxLib")
        ...
        defer {
            ...
            configuration.userContentController.removeScriptMessageHandler(forName: "bmuxLib")
            window.close()
        }
        ...
        _ = try await webView.evaluateJavaScript("window.__bmuxSetMarkdownZoom(2);")
    ...
    private func mermaidSnapshot(in webView: WKWebView) async throws -> [String: Double]? {
        let result = try await webView.evaluateJavaScript(
            """
            (function() {
              var svg = document.querySelector('.bmux-mermaid svg');
              if (!svg) { return null; }
              var rect = svg.getBoundingClientRect();
              var container = svg.closest('.bmux-mermaid');
              ...
              var zoom = Number(svg.getAttribute('data-bmux-mermaid-zoom') || '1');
              return {
                width: rect.width || 0,
    ...
    private func exportedMermaidSnapshot(in webView: WKWebView) async throws -> [String: String] {
        let result = try await webView.evaluateJavaScript(
            ...
            (function() {
              ...
              wrapper.innerHTML = window.__bmuxRenderedHTML();
              var svg = wrapper.querySelector('.bmux-mermaid svg');
              if (!svg) { return {}; }
              return {
                width: svg.style.width || '',
                height: svg.style.height || '',
                maxWidth: svg.style.maxWidth || '',
                hasBmuxData: Array.prototype.some.call(svg.attributes || [], function(attr) {
                  return String(attr.name || '').indexOf('data-bmux-') === 0;
                }) ? '1' : '0'
              };
            })();
            """
...
private final class MarkdownMermaidStubHandler: NSObject, WKScriptMessageHandler {
    ...
    func userContentController(
        ...
        webView?.evaluateJavaScript(
            ...
            if (window.__bmuxLibLoaded) { window.__bmuxLibLoaded('mermaid'); }
            """,
