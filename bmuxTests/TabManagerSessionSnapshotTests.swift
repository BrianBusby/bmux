...
final class TabManagerSessionSnapshotTests: XCTestCase {
    private var originalAppDelegateForTest: AppDelegate?
    private var settingsSuiteNames: [String] = []
    private final class ManualFocusHistoryClock {
        var now: Date
        init(_ now: Date) {
            self.now = now
        }
    ...
    private func makeTabManager(
        ...
        newWorkspacePlacement: WorkspacePlacement = .afterCurrent
    ) -> TabManager {
        let suiteName = "bmux.tests.TabManagerSessionSnapshotTests.\(UUID().uuidString)"
        settingsSuiteNames.append(suiteName)
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            ...
            return TabManager(
                ...
                focusHistoryNavigation: focusHistoryNavigation ?? FocusHistoryModel()
            )
        }
        defaults.removePersistentDomain(forName: suiteName)
        let settings = UserDefaultsSettingsClient(defaults: defaults)
        settings.set(newWorkspacePlacement, for: SettingCatalog().app.newWorkspacePlacement)
        return TabManager(
            initialWorkspaceTitle: initialWorkspaceTitle,
            initialWorkingDirectory: initialWorkingDirectory,
            initialTerminalInput: initialTerminalInput,
    ...
    override func setUp() {
        super.setUp()
        originalAppDelegateForTest = AppDelegate.shared
    ...
    override func tearDown() {
        ...
        AppDelegate.shared = originalAppDelegateForTest
        originalAppDelegateForTest = nil
        super.tearDown()
    }
    private func reserveRemoteRestoreSocket() -> String {
        TerminalController.shared.stop()
        let requestedPath = "/tmp/bmux-restore-\(UUID().uuidString).sock"
    ...
    func testFocusHistoryRevisionInvalidatesWhenClosedPanelChangesAvailability() throws {
        ...
        let closedPanelId = try XCTUnwrap(workspace.focusedPanelId)
        let fallbackPanelId = try XCTUnwrap(workspace.newTerminalSurface(inPane: pane, focus: true)?.id)
        manager.focusWorkspaceSurfaceForAction(workspaceId: workspace.id, surfaceId: closedPanelId)
        manager.focusWorkspaceSurfaceForAction(workspaceId: workspace.id, surfaceId: fallbackPanelId)
        XCTAssertTrue(manager.canNavigateBack)
        var notificationCount = 0
        let observer = NotificationCenter.default.addObserver(
            forName: .tabManagerFocusHistoryRevisionDidChange,
    ...
    func testFocusHistoryRevisionInvalidatesWhenClosedPaneChangesAvailability() throws {
        ...
        manager.focusWorkspaceSurfaceForAction(workspaceId: workspace.id, surfaceId: leftPanelId)
        manager.focusWorkspaceSurfaceForAction(workspaceId: workspace.id, surfaceId: rightPanel.id)
        XCTAssertTrue(manager.canNavigateBack)
        var notificationCount = 0
        let observer = NotificationCenter.default.addObserver(
            forName: .tabManagerFocusHistoryRevisionDidChange,
    ...
    func testFocusHistoryMenuSnapshotCarriesFocusedTimestamp() throws {
        let clock = ManualFocusHistoryClock(Date(timeIntervalSince1970: 100))
        let manager = makeTabManager(focusHistoryNavigation: FocusHistoryModel(now: { clock.now }))
        let firstWorkspaceFocusedAt = clock.now
        ...
        XCTAssertEqual(item.focusedAt, firstWorkspaceFocusedAt)
    }
    ...
    func testClosedItemHistoryPersistsRecordsWithoutSharedCapacityLimit() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-closed-history-\(UUID().uuidString)", isDirectory: true)
    ...
    func testClosedItemHistoryAsyncLoadMergesEarlyMutation() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-closed-history-merge-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testClosedItemHistoryFlushPendingSavesPersistsLatestRecords() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-closed-history-flush-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testClosedItemHistoryAsyncLoadReplaysQueuedPanelWorkspaceRemap() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-closed-history-remap-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testSessionSnapshotIncludesRemoteWorkspacesForRestore() throws {
        ...
        XCTAssertEqual(remoteSnapshot.remote?.destination, "bmux-macmini")
    }
    ...
    func testSessionSnapshotSkipsTemporaryDiffViewerBrowserPanels() throws {
        ...
        let url = try XCTUnwrap(URL(string: "\(BmuxDiffViewerURLSchemeHandler.scheme)://token/index.html"))
        _ = try XCTUnwrap(
    ...
    func testSessionSnapshotRestoresSSHWorkspaceDescriptor() throws {
        ...
        let originalAgentSocketPath = "/tmp/bmux-original-restore-agent.sock"
        let restoredAgentSocketPath = "/tmp/bmux-current-restore-agent-\(UUID().uuidString).sock"
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/bmux-restore-test.sock",
        ...
        let snapshotURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ssh-session-restore-\(UUID().uuidString).json")
        ...
        let store = SessionSnapshotRepository<AppSessionSnapshot>(
            ...
            bundleIdentifier: "com.bmuxterm.tests"
        )
    ...
    func testSessionSnapshotRestoreOmitsSSHAgentEnvironmentWhenSocketUnavailable() throws {
        ...
        let originalAgentSocketPath = "/tmp/bmux-original-missing-agent.sock"
        let previousAgentSocketPath = getenv("SSH_AUTH_SOCK").map { String(cString: $0) }
        defer {
            if let previousAgentSocketPath {
                setenv("SSH_AUTH_SOCK", previousAgentSocketPath, 1)
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/bmux-missing-agent-test.sock",
            terminalStartupCommand: "ssh dev@example.com",
            agentSocketPath: originalAgentSocketPath
        )
        remoteWorkspace.configureRemoteConnection(configuration, autoConnect: false)
    ...
    func testSessionSnapshotRestoresPersistentSSHPTYSessionAfterRelaunch() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/bmux-persist-test.sock",
        ...
        let snapshotURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ssh-pty-session-restore-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: snapshotURL) }
        let snapshot = AppSessionSnapshot(
            version: SessionSnapshotSchema.currentVersion,
            createdAt: Date().timeIntervalSince1970,
        ...
        XCTAssertTrue(expectedScrollback.contains("bmux perf synthetic scrollback"), expectedScrollback)
        ...
        XCTAssertTrue(
            restoredWorkspace.remoteConfiguration?.sshOptions.contains("ControlPath=/tmp/bmux-ssh-\(getuid())-64003-%C") == true
        )
        ...
        XCTAssertNil(restoredWorkspace.terminalPanel(for: restoredPanelId)?.requestedWorkingDirectory)
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("export BMUX_SOCKET_PATH=127.0.0.1:64003"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("export PATH=\"$HOME/.bmux/bin:$PATH\""),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(restoredDefaultRemoteCommand.contains("BMUX_SHELL_INTEGRATION_DIR"), restoredDefaultRemoteCommand)
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("bmux_workspace_id='__BMUX_WORKSPACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("'__BMUX_''WORKSPACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            restoredDefaultRemoteCommand.contains("[ -n '__BMUX_WORKSPACE_ID__' ]"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("bmux_surface_id='__BMUX_SURFACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            restoredDefaultRemoteCommand.contains("'__BMUX_''SURFACE_ID__'"),
            restoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            restoredDefaultRemoteCommand.contains("[ -n '__BMUX_SURFACE_ID__' ]"),
            restoredDefaultRemoteCommand
        )
        let substitutedRestoredDefaultRemoteCommand = restoredDefaultRemoteCommand
            .replacingOccurrences(of: "__BMUX_WORKSPACE_ID__", with: restoredWorkspace.id.uuidString)
            .replacingOccurrences(of: "__BMUX_SURFACE_ID__", with: restoredPanelId.uuidString)
        XCTAssertTrue(
            substitutedRestoredDefaultRemoteCommand.contains(
                "bmux_workspace_id='\(restoredWorkspace.id.uuidString)'"
            ),
            substitutedRestoredDefaultRemoteCommand
        )
        XCTAssertTrue(
            substitutedRestoredDefaultRemoteCommand.contains("bmux_surface_id='\(restoredPanelId.uuidString)'"),
            substitutedRestoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            substitutedRestoredDefaultRemoteCommand.contains("BMUX_WORKSPACE_ID=__BMUX_WORKSPACE_ID__"),
            substitutedRestoredDefaultRemoteCommand
        )
        XCTAssertFalse(
            substitutedRestoredDefaultRemoteCommand.contains("BMUX_SURFACE_ID=__BMUX_SURFACE_ID__"),
            substitutedRestoredDefaultRemoteCommand
        )
        let restoredInitialCommand = try XCTUnwrap(
            restoredWorkspace.terminalPanel(for: restoredPanelId)?.surface.debugInitialCommand()
        ...
        XCTAssertTrue(restoredInitialCommand.contains("BMUX_SURFACE_ID"), restoredInitialCommand)
    ...
    func testSessionSnapshotRestoresSplitPersistentSSHPTYWithoutDefaultAttachScaffold() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "c", count: 64),
            localSocketPath: "/tmp/bmux-persist-split.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYRestoreRewritesStaleRemoteRelayContextIDs() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/bmux-relay-alias.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
        ...
        let refreshedRelayConfiguration = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/bmux-ssh-\(getuid())-64006-%C",
                "StrictHostKeyChecking=accept-new",
            ],
            localProxyPort: nil,
            relayPort: 64006,
            ...
            relayToken: String(repeating: "c", count: 64),
            localSocketPath: "/tmp/bmux-relay-alias-refreshed.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
    ...
    func testPersistentSSHPTYRestoreRewritesMovedSourceWorkspaceContextID() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "d", count: 64),
            localSocketPath: "/tmp/bmux-relay-moved-alias.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYReattachRewritesStaleRemoteRelayContextIDs() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "b", count: 64),
            localSocketPath: "/tmp/bmux-relay-reattach-alias.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYRestoreFallsBackToSnapshotPanelDefaultSessionIDWhenActiveMarkerExists() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "f", count: 64),
            localSocketPath: "/tmp/bmux-legacy-persist.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: persistentDaemonSlot
        )
    ...
    func testPersistentSSHPTYRestoreDoesNotReattachEndedSnapshotPanel() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/bmux-ended-persist.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
    ...
    func testPersistentSSHPTYRestorePreservesLocalTerminalWorkingDirectory() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/bmux-local-terminal.sock",
            terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
            preserveAfterTerminalExit: true,
        ...
        remoteWorkspace.configureRemoteConnection(configuration, autoConnect: false)
        let paneId = try XCTUnwrap(remoteWorkspace.bonsplitController.allPaneIds.first)
        let localDirectory = "/tmp/bmux-local-terminal"
    ...
    func testSessionSnapshotFallsBackWhenPersistentSSHPTYRestoreHasNoSocketPath() throws {
        ...
        remoteWorkspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "f", count: 64),
                localSocketPath: "/tmp/bmux-no-socket.sock",
                terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
                preserveAfterTerminalExit: true,
    ...
    func testSessionSnapshotFallsBackFromSkipBootstrapPersistentSSHPTYWithoutDaemonBridge() throws {
        ...
        let snapshotURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ssh-pty-durable-restore-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: snapshotURL) }
        let snapshot = AppSessionSnapshot(
            version: SessionSnapshotSchema.currentVersion,
            createdAt: Date().timeIntervalSince1970,
    ...
    func testSessionSnapshotRestoresDefaultFreestyleSSHDAsSelfHealingAttach() throws {
        let snapshot = SessionRemoteWorkspaceSnapshot(
            transport: .ssh,
            destination: "nncop8f8h6w9blhns6sy+bmux@vm-ssh.freestyle.sh",
            port: 22,
            identityFile: nil,
            sshOptions: [
        ...
        XCTAssertEqual(configuration.persistentDaemonSlot, "bmux-default-freestyle-sshd-v1")
        ...
        XCTAssertTrue(terminalStartupCommand.contains("BMUX_SSH_RECONNECT_LIMIT"), terminalStartupCommand)
        XCTAssertTrue(terminalStartupCommand.contains("BMUX_CLOUD_RECONNECT_ATTEMPT"), terminalStartupCommand)
        XCTAssertFalse(terminalStartupCommand.contains("Cloud VM reconnecting"), terminalStartupCommand)
        XCTAssertFalse(terminalStartupCommand.contains("bmux_freestyle_notify_reconnect"), terminalStartupCommand)
        XCTAssertFalse(terminalStartupCommand.contains("[bmux] ssh exited with status"), terminalStartupCommand)
    }
    ...
    func testSessionRestoreDropsStalePTYSessionForDefaultFreestyleSSHD() throws {
        ...
        XCTAssertTrue(restoredInitialCommand.contains("BMUX_CLOUD_RECONNECT_ATTEMPT"), restoredInitialCommand)
        XCTAssertFalse(restoredInitialCommand.contains("[bmux] ssh exited with status"), restoredInitialCommand)
        let restoredSessionID = restoredWorkspace.sessionSnapshot(includeScrollback: false)
            .panels.first { $0.id == restoredPanelId }?.terminal?.remotePTYSessionID
        XCTAssertNotEqual(restoredSessionID, "ssh-stale-session")
        XCTAssertEqual(
            restoredSessionID,
    ...
    func testLegacyDefaultFreestyleSSHDRestoreRepairsRawSSHSnapshotBeforeSpawning() throws {
        let legacyRemote = SessionRemoteWorkspaceSnapshot(
            transport: .ssh,
            destination: "71smiccrg35sw9pydt8k+bmux@vm-ssh.freestyle.sh",
            port: 22,
            identityFile: nil,
            sshOptions: [],
            preserveAfterTerminalExit: nil,
    ...
    func testSessionRemoteWorkspaceSnapshotRequiresPersistentDaemonSlotForPTYRestore() throws {
        ...
        let configuration = try XCTUnwrap(snapshot.workspaceConfiguration(localSocketPath: "/tmp/bmux-restore.sock"))
    ...
    func testSessionRemoteWorkspaceSnapshotStripsTransientControlOptionsWhenPreservedRestoreFallsBack() throws {
        let snapshot = SessionRemoteWorkspaceSnapshot(
            ...
            sshOptions: [
                ...
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/bmux-ssh-501-64003-%C",
            ],
            preserveAfterTerminalExit: true,
            skipDaemonBootstrap: nil,
            relayPort: 64003,
    ...
    /// Regression for https://github.com/manaflow-ai/bmux/issues/5931 — a restored
    ...
    func testRestoredTerminalPaneHeaderTitleSyncsToBonsplitTab() throws {
        ...
        let restoredTitle = "~/projects/bmux"
