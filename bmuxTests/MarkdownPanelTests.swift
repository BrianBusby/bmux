...
final class MarkdownPanelTests: XCTestCase {
    ...
    func testMarkdownFontSizeSettingsResolvedDefaultHonorsDefaults() throws {
        let suiteName = "bmux.markdownFontSizeTests.\(UUID().uuidString)"
    ...
    func testMarkdownFontFamilyNormalizesDefaultsAndEscapesCSSValue() throws {
        let suiteName = "bmux.markdownFontFamilyTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
    ...
    func testMarkdownMaxWidthSettingsClampAndResolvedDefault() throws {
        ...
        let suiteName = "bmux.markdownMaxWidthTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
    ...
    func testMarkdownPanelZoomStepsClampAndReset() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-markdown-zoom-\(UUID().uuidString)", isDirectory: true)
    ...
    func testMarkdownPanelTypographyResetsToConfiguredDefaults() throws {
        let fileManager = FileManager.default
        let directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-markdown-typography-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        let fileURL = directoryURL.appendingPathComponent("README.md")
        try "# hello".write(to: fileURL, atomically: true, encoding: .utf8)
        defer { try? fileManager.removeItem(at: directoryURL) }
    ...
    func testFileOpenRoutesMarkdownFilesToPreviewMarkdownPanel() throws {
        let fileManager = FileManager.default
        let directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-markdown-file-open-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer {
    ...
    func testExternalFileOpenRoutesMarkdownFilesToPreviewMarkdownPanel() throws {
        let fileManager = FileManager.default
        let directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-markdown-external-open-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer {
            try? fileManager.removeItem(at: directoryURL)
        }
    ...
    func testOpenMarkdownPanelReloadsWhenFileChangesOnDisk() async throws {
        let fileManager = FileManager.default
        let directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-markdown-panel-tests-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: directoryURL) }
    ...
    func testMarkdownRenderHandlesLocalImageSources() async throws {
        ...
        let rootURL = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-markdown-image-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertTrue((localImage["currentSrc"] as? String ?? "").hasPrefix("bmux-local-image://"))
        ...
        XCTAssertTrue((traversalImage["currentSrc"] as? String ?? "").hasPrefix("bmux-local-image://"))
    ...
    func testMarkdownRenderDeniesLocalImageWhenMarkdownPathIsMissing() async throws {
        let fileManager = FileManager.default
        let rootURL = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-markdown-missing-path-\(UUID().uuidString)", isDirectory: true)
    ...
    func testMarkdownRenderLoadsSafeDataImage() async throws {
        let markdownURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-markdown-data-image-\(UUID().uuidString).md")
    ...
    func testMarkdownRenderBlocksRemoteImagesUntilUserAction() async throws {
        let markdownURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-markdown-remote-image-\(UUID().uuidString).md")
        ...
        let expectedConsentMessage = String(
            ...
            defaultValue: "bmux will not contact this image URL until you load this image."
        )
        ...
        try await renderMarkdown(
            ...
            <img alt="Spoofed internal" data-bmux-remote-src="https%3A%2F%2Fspoof.example%2Fpixel.png">
            """,
            in: webView
        )
        ...
        let copiedHTTPImageURL = try await webView.evaluateJavaScript(
            ...
            (function() {
              ...
              var id = img && img.getAttribute('data-bmux-remote-placeholder-id');
              var placeholder = id && document.querySelector('[data-bmux-remote-placeholder-for="' + id + '"]');
        ...
        XCTAssertTrue((loadingLinkedImage["src"] as? String ?? "").hasPrefix("bmux-remote-image://"))
        XCTAssertEqual(loadingLinkedImage["hidden"] as? Bool, true)
        XCTAssertTrue((loadingDuplicateImage["src"] as? String ?? "").hasPrefix("bmux-remote-image://"))
        ...
        _ = try await webView.evaluateJavaScript(
            """
            (function() {
              Array.prototype.slice.call(document.querySelectorAll('img[src^="bmux-remote-image://"]')).forEach(function(img) {
        ...
        XCTAssertTrue((linkedImage["src"] as? String ?? "").hasPrefix("bmux-remote-image://"))
        XCTAssertEqual(linkedImage["hidden"] as? Bool, false)
        XCTAssertTrue((duplicateImage["src"] as? String ?? "").hasPrefix("bmux-remote-image://"))
        ...
        XCTAssertTrue((autoLoadingImage["src"] as? String ?? "").hasPrefix("bmux-remote-image://"))
    ...
    private func renderMarkdown(_ markdown: String, in webView: WKWebView) async throws {
        ...
        _ = try await webView.evaluateJavaScript("window.__bmuxRenderMarkdown(\(literal)[0]);")
    }
                ...
                (function() {
                  return Array.prototype.slice.call(document.querySelectorAll('img')).map(function(img) {
                    return {
                      ...
                      hidden: !!img.hidden,
                      remoteSrc: img.getAttribute('data-bmux-remote-src') || ''
                    };
                  });
                })();
                """
        ...
        let result = try await webView.evaluateJavaScript(
            ...
            (function() {
              return {
                ...
                placeholders: Array.prototype.slice.call(document.querySelectorAll('.bmux-remote-image-placeholder')).map(function(el) {
                  return el.textContent || '';
                }),
                remoteImageURLs: Array.prototype.slice.call(document.querySelectorAll('.bmux-remote-image-url')).map(function(el) {
                  return el.textContent || '';
                }),
                buttons: Array.prototype.slice.call(document.querySelectorAll('.bmux-remote-image-placeholder button')).map(function(el) {
                  return el.textContent || '';
                }),
                buttonStates: Array.prototype.slice.call(document.querySelectorAll('.bmux-remote-image-placeholder button')).map(function(el) {
                  return {
                    text: el.textContent || '',
                ...
                codeFiles: Array.prototype.slice.call(document.querySelectorAll('code[data-bmux-file]')).map(function(el) {
                  return decodeURIComponent(el.getAttribute('data-bmux-file') || '');
                }),
                ...
                renderedText: window.__bmuxRenderedText ? window.__bmuxRenderedText() : ''
              };
            })();
            """
        )
