...
private var bmuxUnitTestWKWebViewDragLifecycleOverrideInstalled = false
private var bmuxUnitTestWKWebViewDragLifecycleEvents: [String]?

extension WKWebView {
    @objc func bmuxUnitTest_draggingEntered(_ sender: any NSDraggingInfo) -> NSDragOperation {
        if bmuxUnitTestWKWebViewDragLifecycleEvents != nil {
            bmuxUnitTestWKWebViewDragLifecycleEvents?.append("entered")
            return .copy
        }
        return bmuxUnitTest_draggingEntered(sender)
    }

    @objc func bmuxUnitTest_draggingUpdated(_ sender: any NSDraggingInfo) -> NSDragOperation {
        if bmuxUnitTestWKWebViewDragLifecycleEvents != nil {
            bmuxUnitTestWKWebViewDragLifecycleEvents?.append("updated")
            return .copy
        }
        return bmuxUnitTest_draggingUpdated(sender)
    }

    @objc func bmuxUnitTest_prepareForDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        if bmuxUnitTestWKWebViewDragLifecycleEvents != nil {
            bmuxUnitTestWKWebViewDragLifecycleEvents?.append("prepare")
            return true
        }
        return bmuxUnitTest_prepareForDragOperation(sender)
    }

    @objc func bmuxUnitTest_performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        if bmuxUnitTestWKWebViewDragLifecycleEvents != nil {
            bmuxUnitTestWKWebViewDragLifecycleEvents?.append("perform")
            return true
        }
        return bmuxUnitTest_performDragOperation(sender)
    }

    @objc func bmuxUnitTest_concludeDragOperation(_ sender: (any NSDraggingInfo)?) {
        if bmuxUnitTestWKWebViewDragLifecycleEvents != nil {
            bmuxUnitTestWKWebViewDragLifecycleEvents?.append("conclude")
            return
        }
        bmuxUnitTest_concludeDragOperation(sender)
    }
}

private func installBmuxUnitTestWKWebViewDragLifecycleOverride() {
    guard !bmuxUnitTestWKWebViewDragLifecycleOverrideInstalled else { return }
    ...
    swizzle(
        ...
        #selector(WKWebView.bmuxUnitTest_draggingEntered(_:))
    )
    swizzle(
        #selector(NSView.draggingUpdated(_:)),
        #selector(WKWebView.bmuxUnitTest_draggingUpdated(_:))
    )
    swizzle(
        #selector(NSView.prepareForDragOperation(_:)),
        #selector(WKWebView.bmuxUnitTest_prepareForDragOperation(_:))
    )
    swizzle(
        #selector(NSView.performDragOperation(_:)),
        #selector(WKWebView.bmuxUnitTest_performDragOperation(_:))
    )
    swizzle(
        #selector(NSView.concludeDragOperation(_:)),
        #selector(WKWebView.bmuxUnitTest_concludeDragOperation(_:))
    )

    bmuxUnitTestWKWebViewDragLifecycleOverrideInstalled = true
}
...
@MainActor
final class BmuxWebViewDragRoutingTests: XCTestCase {
    override func setUp() {
        super.setUp()
        installBmuxUnitTestWKWebViewDragLifecycleOverride()
        bmuxUnitTestWKWebViewDragLifecycleEvents = nil
    }
    ...
    func testRejectsInternalPaneDragEvenWhenFilePromiseTypesArePresent() {
        XCTAssertTrue(
            BmuxWebView.shouldRejectInternalPaneDrag([
    ...
    func testAllowsRegularExternalFileImageAndURLDrops() {
        ...
        for pasteboardTypes in externalPayloads {
            XCTAssertFalse(
                BmuxWebView.shouldRejectInternalPaneDrag(pasteboardTypes),
    ...
    func testWebsiteDragPayloadReachesWebKitDragLifecycle() {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("bmux.web-drag.\(UUID().uuidString)"))
        ...
        bmuxUnitTestWKWebViewDragLifecycleEvents = []
    ...
    func testInternalPaneDragDoesNotReachWebKitDragLifecycle() {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("bmux.internal-drag.\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(bmuxUnitTestWKWebViewDragLifecycleEvents, [])
    }
}
...
final class BrowserScreenshotPipelineTests: XCTestCase {
    ...
    func testFullPageSnapshotWritesPNGAndTIFFToPasteboard() async throws {
        let pasteboard = NSPasteboard(name: .init("bmux-browser-screenshot-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testScreenshotPasteboardWriterClearsExistingContentsBeforeWriting() async throws {
        let pasteboard = NSPasteboard(name: .init("bmux-browser-screenshot-existing-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testSectionSnapshotCropsSelectionBeforeWritingPasteboard() async throws {
        let pasteboard = NSPasteboard(name: .init("bmux-browser-screenshot-section-\(UUID().uuidString)"))
        pasteboard.clearContents()
