...
struct GhosttyDECCKMArrowKeyTests {
    ...
    func windowKeyEquivalentTerminalArrowsEncodeDECCKMBytes() throws {
        ...
        let captureReadyMarker = "BMUX_DECCKM_READY_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let captureMarker = "BMUX_DECCKM_HEX_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-decckm-arrow-capture-\(UUID().uuidString).py")
        let script = """
        import os
        import select
        import sys
