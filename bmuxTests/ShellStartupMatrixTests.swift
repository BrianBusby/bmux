...
struct ShellStartupMatrixTests {
    ...
    func zshStartupPreservesUserZdotdirAndLoadsGhosttyIntegration() throws {
        let bundled = try makeBundledIntegrationDir(files: [".zshenv": "# bmux zsh bootstrap stub\n"])
        ...
        expectEqual(environment["BMUX_ZSH_ZDOTDIR"], "/Users/example/.zsh")
        expectEqual(environment["BMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"], "1")
        expectTrue(protectedKeys.isSuperset(of: ["ZDOTDIR", "BMUX_ZSH_ZDOTDIR", "BMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"]))
    }
    ...
    func zshStartupDoesNotPreserveGhosttyInjectedZdotdir() throws {
        ...
        expectEqual(environment["BMUX_ZSH_ZDOTDIR"], nil)
        expectFalse(protectedKeys.contains("BMUX_ZSH_ZDOTDIR"))
    }

    @Test
    func bashStartupUsesBundledPromptCommandAndHonorsGhosttyMode() {
        let integrationDir = "/Applications/bmux.app/Contents/Resources/shell-integration"
        ...
        _ = TerminalSurface.applyManagedShellSpecificStartupEnvironment(
            ...
            protectedKeys: &protectedKeys,
            readFile: { path in
                expectEqual(path, "\(integrationDir)/bmux-bash-bootstrap.bash")
                return """
                # comment
        ...
        expectEqual(environment["BMUX_LOAD_GHOSTTY_BASH_INTEGRATION"], nil)
        expectEqual(environment["PROMPT_COMMAND"], "export BMUX_TEST=1\nbmux-test-command")
        expectTrue(protectedKeys.contains("PROMPT_COMMAND"))
        expectFalse(protectedKeys.contains("BMUX_LOAD_GHOSTTY_BASH_INTEGRATION"))
    }

    @Test
    func fishStartupReturnsManagedCommandAndPreservesUserConfigHome() throws {
        let bundled = try makeBundledIntegrationDir(files: ["fish/config.fish": "# bmux fish bootstrap stub\n"])
        defer { try? FileManager.default.removeItem(at: bundled.root) }
        let integrationDir = bundled.integrationDir
        ...
        expectEqual(environment["BMUX_FISH_INTEGRATION_FILE"], "\(integrationDir)/fish/config.fish")
    ...
    @Test(arguments: [
        ...
        RemoteShellCase(name: "fish", expectedArgs: "-il --init-command source \"$BMUX_FISH_INTEGRATION_FILE\""),
    ...
    func generatedSshBootstrapHandlesShellMatrix(shellCase: RemoteShellCase) throws {
        ...
        expectTrue(result.capture.contains("PATH=\(result.home.path)/.bmux/bin:"), result.capture)
        expectTrue(result.capture.contains("BMUX_SOCKET_PATH=127.0.0.1:64123"), result.capture)
        ...
        case "zsh":
            expectTrue(result.capture.contains("ZDOTDIR=\(result.home.path)/.bmux/relay/64123.shell"), result.capture)
            expectTrue(result.capture.contains("BMUX_REAL_ZDOTDIR=\(result.home.path)/user-zdotdir"), result.capture)
        ...
        case "fish":
            expectTrue(result.capture.contains("BMUX_FISH_INTEGRATION_FILE=\(result.home.path)/.bmux/relay/64123.shell/fish/config.fish"), result.capture)
            expectTrue(result.capture.contains("BMUX_FISH_USER_CONFIG_ALREADY_LOADED=1"), result.capture)
            expectTrue(result.capture.contains("FISH_HAS_MARKER=yes"), result.capture)
        default:
            expectTrue(result.capture.contains("BMUX_FISH_INTEGRATION_FILE=\n"), result.capture)
            expectTrue(result.capture.contains("BMUX_REAL_ZDOTDIR=\n"), result.capture)
        }
    }
    ...
    func generatedSshBootstrapDoesNotBlockOnRelayCliWarmup() throws {
        ...
        expectTrue(
            ...
            "bmux ssh bootstrap waited for relay CLI warmup: \(formatSeconds(result.process.duration))"
        )
    }

    /// Regression for #6352: running Claude Code (or any full-screen TUI) inside
    /// a `bmux ssh` remote workspace garbled the output because the remote
    ...
    func remoteTerminalSetupInstallsGhosttyTerminfoBeforeChoosingTerm() throws {
        ...
        guard fileManager.isExecutableFile(atPath: "/usr/bin/tic"),
            ...
            // cannot be exercised here. bmux CI runners ship both binaries.
            return
        }

        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-terminfo-\(UUID().uuidString)")
        ...
        let terminfoSource = """
        xterm-ghostty|bmux ghostty regression terminfo,
        ...
        let script = lines.joined(separator: "\n")
            + "\nprintf 'BMUX_TERMINFO_TEST_TERM=%s\\n' \"$TERM\"\n"
        ...
        expectTrue(
            result.stdout.contains("BMUX_TERMINFO_TEST_TERM=xterm-ghostty"),
    ...
    private func runGeneratedBootstrap(
        shellName: String,
        fakeBmuxDelay: TimeInterval? = nil,
        workspaceID: String? = nil,
        surfaceID: String? = nil,
        bootstrapTTY: String? = nil
    ) throws -> GeneratedBootstrapResult {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("bmux-shell-matrix-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        ...
        if let fakeBmuxDelay {
            try writeExecutableBmuxFile(at: bin.appendingPathComponent("bmux"), delay: fakeBmuxDelay)
        }
        var script = RemoteInteractiveShellBootstrapBuilder.script(
            ...
            bundledZshIntegration: "bmux_zsh_marker=1",
            bundledBashIntegration: "bmux_bash_marker=1",
            bundledFishIntegration: "set -gx BMUX_FISH_MARKER 1"
        )
        if let workspaceID {
            script = script.replacingOccurrences(of: "__BMUX_WORKSPACE_ID__", with: workspaceID)
        }
        if let surfaceID {
            script = script.replacingOccurrences(of: "__BMUX_SURFACE_ID__", with: surfaceID)
        }
        var arguments = [
            "HOME=\(home.path)",
            ...
            "BMUX_CAPTURE_PATH=\(capturePath.path)",
        ]
        if let bootstrapTTY {
            arguments.append("BMUX_BOOTSTRAP_TTY=\(bootstrapTTY)")
        }
        arguments += [
            "/bin/sh",
            "-c",
    ...
    private func writeExecutableShellFile(at url: URL, capturePath: URL) throws {
        ...
        #!/bin/sh
        {
          printf 'ARGS=%s\\n' "$*"
          printf 'PATH=%s\\n' "$PATH"
          printf 'BMUX_SOCKET_PATH=%s\\n' "$BMUX_SOCKET_PATH"
          printf 'GHOSTTY_SHELL_FEATURES=%s\\n' "$GHOSTTY_SHELL_FEATURES"
          printf 'BMUX_SHELL_INTEGRATION_DIR=%s\\n' "$BMUX_SHELL_INTEGRATION_DIR"
          printf 'ZDOTDIR=%s\\n' "$ZDOTDIR"
          printf 'BMUX_REAL_ZDOTDIR=%s\\n' "$BMUX_REAL_ZDOTDIR"
          ...
          if [ -r "$BMUX_SHELL_INTEGRATION_DIR/bmux-zsh-integration.zsh" ] && grep -q 'bmux_zsh_marker=1' "$BMUX_SHELL_INTEGRATION_DIR/bmux-zsh-integration.zsh"; then
            printf 'ZSH_INTEGRATION_HAS_MARKER=yes\\n'
          fi
          if [ -r "$BMUX_SHELL_INTEGRATION_DIR/bmux-bash-integration.bash" ] && grep -q 'bmux_bash_marker=1' "$BMUX_SHELL_INTEGRATION_DIR/bmux-bash-integration.bash"; then
            printf 'BASH_INTEGRATION_HAS_MARKER=yes\\n'
          fi
          if [ -r "$BMUX_FISH_INTEGRATION_FILE" ] && grep -q 'BMUX_FISH_MARKER' "$BMUX_FISH_INTEGRATION_FILE"; then
    ...
    private func writeExecutableBmuxFile(at url: URL, delay: TimeInterval) throws {
        try """
        #!/bin/sh
        if [ "$1" = "rpc" ]; then
    ...
    private func makeBundledIntegrationDir(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-shell-matrix-bundle-\(UUID().uuidString)")
