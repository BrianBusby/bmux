...
/// Behavior tests for the remote-tmux SSH auth path that backs `bmux ssh-tmux`:
...
@Suite struct RemoteTmuxAuthTests {
    ...
    @Test func controlModeArgumentsUseRemoteTmuxResolverAfterDestinationGuard() throws {
        ...
        #expect(remoteCommand.hasSuffix("'bmux-remote-tmux' '-CC' 'attach-session' '-t' 'work session'"))
    }
    ...
    @Test @MainActor func layoutChangePrunesRemovedPaneDiagnosticState() {
        ...
        connection.handleMessageForTesting(.subscriptionChanged(name: "bmux_reflow_4", value: "0|zsh"))
        connection.handleMessageForTesting(.subscriptionChanged(name: "bmux_reflow_5", value: "1|vim"))
    ...
    @Test func pastePaneCommandsProtectOptionLookingText() throws {
        ...
        #expect(commands.setBuffer == "set-buffer -b bmux-paste-7 -- '-n not-an-option'")
        #expect(commands.pasteBuffer == "paste-buffer -p -d -b bmux-paste-7 -t %7")
    }
    ...
    // MARK: - Interactive auth invocation (what `bmux ssh-tmux` runs in the tty)
