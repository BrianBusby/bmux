...
/// Regression coverage for https://github.com/manaflow-ai/bmux/issues/5887.
///
/// `NSWindow.bmux_performKeyEquivalent(with:)` force-dispatches certain key
...
struct WindowKeyDownReplayGuardTests {
    ...
    /// bypass in `bmux_performKeyEquivalent` force-dispatches this into the
    /// first responder's `keyDown`, which is the unguarded dispatch the
    /// https://github.com/manaflow-ai/bmux/issues/5887 crash looped through.
    ...
    func printableOptionTextKeyDownIsForceDispatchedExactlyOncePerEvent() {
        ...
        #expect(
            ...
            Comment(rawValue: "The same in-flight key event must not be force-dispatched into keyDown again while the first dispatch is still on the stack; unbounded re-dispatch is the infinite key-routing loop from https://github.com/manaflow-ai/bmux/issues/5887")
        )
    }
