...
struct CLICodexHookTimeoutRegressionTests {
    @Test func codexHookInstallReplacesSynchronousBundledHook() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-sync-hook-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let previousCommand = "bmux_cli=\"${BMUX_BUNDLED_CLI_PATH:-}\"; if [ -z \"$bmux_cli\" ] || [ ! -x \"$bmux_cli\" ]; then bmux_cli=\"$(command -v bmux 2>/dev/null || true)\"; fi; if [ -n \"$BMUX_SURFACE_ID\" ] && [ \"$BMUX_CODEX_HOOKS_DISABLED\" != \"1\" ] && [ -n \"$bmux_cli\" ]; then { if [ -n \"${BMUX_SOCKET_PATH:-}\" ]; then \"$bmux_cli\" --socket \"$BMUX_SOCKET_PATH\" hooks codex prompt-submit; else \"$bmux_cli\" hooks codex prompt-submit; fi; } || echo '{}'; else echo '{}'; fi"
        ...
        #expect(sessionStartHooks.allSatisfy { $0.body.contains("agent_pid=") && $0.body.contains("BMUX_CODEX_PID=") })
        ...
        #expect(promptHooks.allSatisfy { $0.body.contains("agent_pid=") && $0.body.contains("BMUX_CODEX_PID=") })
        ...
        #expect(stopHooks.allSatisfy { $0.body.contains("agent_pid=") && $0.body.contains("BMUX_CODEX_PID=") })
    ...
    @Test func codexInstalledHookReturnsBeforeSlowBmuxCommandFinishes() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-hook-async-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        ...
        try makeCodexHookExecutableShellFile(at: fakeCLI, lines: [
            "#!/bin/sh",
            "printf '%s\\n' \"$*\" > \"$BMUX_TEST_ARGS\"",
            "printf '%s\\n' \"$BMUX_CODEX_PID\" > \"$BMUX_TEST_PID\"",
            "cat > \"$BMUX_TEST_STDIN\"",
            "sleep 4",
            "printf done > \"$BMUX_TEST_DONE\"",
        ])
        ...
        let run = runCodexHookProcess(
            ...
            environment: [
                ...
                "TMPDIR": root.path,
                "BMUX_SURFACE_ID": "surface-123",
                "BMUX_SOCKET_PATH": "/tmp/bmux-test.sock",
                "BMUX_BUNDLED_CLI_PATH": fakeCLI.path,
                "BMUX_CODEX_PID": "4242",
                "BMUX_TEST_STDIN": capturedStdin.path,
                "BMUX_TEST_ARGS": capturedArgs.path,
                "BMUX_TEST_PID": capturedPID.path,
                "BMUX_TEST_DONE": doneFile.path,
            ],
        ...
        #expect(waitForFile(capturedArgs, containing: "--socket /tmp/bmux-test.sock hooks codex prompt-submit", timeout: 1))
        #expect(waitForFile(capturedPID, containing: "4242", timeout: 1))
        #expect(waitForFile(doneFile, containing: "done", timeout: 6))
    }

    @Test func codexInstalledStopHookReturnsBeforeSlowBmuxCommandFinishes() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-stop-hook-async-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux", isDirectory: false)
        let capturedStdin = root.appendingPathComponent("hook-stdin.json", isDirectory: false)
        let capturedArgs = root.appendingPathComponent("hook-args.txt", isDirectory: false)
        ...
        #expect(run.stdout == "{}\n")
        #expect(waitForFile(capturedStdin, containing: payload, timeout: 1))
        #expect(waitForFile(capturedArgs, containing: "--socket /tmp/bmux-test.sock hooks codex stop", timeout: 1))
        #expect(waitForFile(capturedPID, containing: "4242", timeout: 1))
    ...
    @Test func codexInstalledAsyncStopDoesNotMarkNewerTurnIdle() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-installed-stale-stop-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
    ...
    @Test func codexPromptSubmitDoesNotReviveStoppedTurn() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-stale-prompt-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexSessionStartDoesNotOverwriteExistingTurnState() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-stale-start-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexSessionStartRefreshesCompletedPriorTurn() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-fresh-start-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexSessionStartDoesNotReviveCompletedTurnFromSamePID() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-same-pid-start-\(UUID().uuidString)", isDirectory: true)
