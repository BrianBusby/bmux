...
/// bmux.json) and the foreground-command capture that feeds saved terminal
/// surfaces.
struct BmuxConfigActionSaverTests {

    private func temporaryRoot(_ label: String) throws -> URL {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-action-saver-\(label)-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    @Test func slugForTitle() {
        #expect(BmuxConfigActionSaver.slug(forTitle: "My Dev Setup!") == "my-dev-setup")
        #expect(BmuxConfigActionSaver.slug(forTitle: "  --  ") == "workspace")
        #expect(BmuxConfigActionSaver.slug(forTitle: "日本語 Dev") == "日本語-dev")
    }

    @Test func uniqueActionID() {
        #expect(BmuxConfigActionSaver.uniqueActionID(forTitle: "Dev", existingIDs: []) == "dev")
        #expect(
            BmuxConfigActionSaver.uniqueActionID(forTitle: "Dev", existingIDs: ["dev", "dev-2"]) == "dev-3"
        )
    }
    ...
    @Test func savePreservesCommentsAndDecodes() throws {
        ...
        let definition = BmuxWorkspaceDefinition(
            ...
            layout: .pane(BmuxPaneDefinition(surfaces: [
                BmuxSurfaceDefinition(type: .terminal, command: "claude", focus: true)
            ]))
        )
        let result = try BmuxConfigActionSaver.saveWorkspaceAction(
            title: "Dev",
    ...
    @Test func saveRejectsNonObjectActionsBlock() throws {
        ...
        #expect(throws: (any Error).self) {
            try BmuxConfigActionSaver.saveWorkspaceAction(
                title: "Nope",
                definition: BmuxWorkspaceDefinition(name: "Nope"),
                globalConfigPath: configPath
            )
        }
    ...
    @Test func savePreservesSymlinkedConfig() throws {
        ...
        defer { try? FileManager.default.removeItem(at: root) }
        let realConfig = root.appendingPathComponent("dotfiles-bmux.json")
        try "{}\n".write(to: realConfig, atomically: true, encoding: .utf8)
        let linkPath = root.appendingPathComponent("bmux.json").path
        try FileManager.default.createSymbolicLink(
            atPath: linkPath,
            withDestinationPath: realConfig.path
        )

        _ = try BmuxConfigActionSaver.saveWorkspaceAction(
            title: "Linked",
            definition: BmuxWorkspaceDefinition(name: "Linked"),
    ...
    @Test func saveRespectsReservedIDs() throws {
        ...
        let result = try BmuxConfigActionSaver.saveWorkspaceAction(
            title: "Dev",
            definition: BmuxWorkspaceDefinition(name: "Dev"),
            globalConfigPath: configPath,
    ...
    @Test func saveCreatesFileFromTemplateOwnerOnly() throws {
        ...
        defer { try? FileManager.default.removeItem(at: root) }
        let configPath = root.appendingPathComponent("nested/bmux.json").path

        let result = try BmuxConfigActionSaver.saveWorkspaceAction(
            title: "Fresh",
            definition: BmuxWorkspaceDefinition(name: "Fresh"),
            globalConfigPath: configPath
        )
    ...
    @Test func deleteRemovesActionAndPreservesComments() throws {
        ...
        try BmuxConfigActionSaver.deleteAction(id: "gone", globalConfigPath: configPath)
    ...
    @Test func deleteFirstOfTwoActionsKeepsValidJSON() throws {
        ...
        try BmuxConfigActionSaver.deleteAction(id: "first", globalConfigPath: configPath)
    ...
    @Test func deleteLastActionRemovesSeparatorComma() throws {
        ...
        try BmuxConfigActionSaver.deleteAction(id: "last", globalConfigPath: configPath)
    ...
    @Test func deleteSoleActionLeavesEmptyObject() throws {
        ...
        try BmuxConfigActionSaver.deleteAction(id: "only", globalConfigPath: configPath)
    ...
    @Test func deleteMissingActionThrowsAndPreservesFile() throws {
        ...
        #expect(throws: (any Error).self) {
            try BmuxConfigActionSaver.deleteAction(id: "ghost", globalConfigPath: configPath)
        }
        #expect(try String(contentsOfFile: configPath, encoding: .utf8) == original)
    }
    ...
    @Test func snapshotDisclosuresListEveryPersistedCommandURLAndEnvKey() {
        let snapshot = WorkspaceConfigActionSnapshot(
            definition: BmuxWorkspaceDefinition(
                name: "W",
                ...
                layout: .split(BmuxSplitDefinition(
                    ...
                    children: [
                        .pane(BmuxPaneDefinition(surfaces: [
                            BmuxSurfaceDefinition(type: .terminal, command: "claude", env: ["RUST_LOG": "debug"])
                        ])),
                        .pane(BmuxPaneDefinition(surfaces: [
                            BmuxSurfaceDefinition(type: .browser, url: "https://example.com/callback?code=abc"),
                            BmuxSurfaceDefinition(type: .terminal, command: "curl -H 'Authorization: Bearer x'"),
                        ])),
                    ]
                ))
            ),
        ...
        let plain = WorkspaceConfigActionSnapshot(
            definition: BmuxWorkspaceDefinition(name: "P"),
            skippedPanelCount: 0
        )
