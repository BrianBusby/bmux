...
extension BMUXCLIErrorOutputRegressionTests {
    @Test func testSessionsListTreatsTranscriptBackedClaudeRecordAsRestorable() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-claude-transcript-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func testSessionsListDoesNotTrustClaudeRestorableFlagWithoutTranscript() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-claude-no-transcript-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let repoDir = root.appendingPathComponent("repo", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: repoDir, withIntermediateDirectories: true)
    ...
    @Test func testSessionsListFindsClaudeTranscriptWhenRecordPathMissing() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-claude-lookup-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
    ...
    @Test func testSessionsListRejectsAmbiguousClaudeWorkflowContainerTranscripts() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-claude-workflow-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let repoDir = root.appendingPathComponent("repo", isDirectory: true)
        let claudeConfigDir = root.appendingPathComponent("claude-config", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
    ...
    @Test func testSessionsListIgnoresOutOfRangeStoredPID() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-pid-range-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
    ...
    @Test func testSessionsListFlagsExistingUnscopedPIDAs06417RestoreRisk() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-pid-reuse-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
    ...
    @Test func testSessionsListForkDiagnosticsUseForkCommandBuilder() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-fork-builder-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
