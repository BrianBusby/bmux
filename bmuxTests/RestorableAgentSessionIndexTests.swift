...
final class RestorableAgentSessionIndexTests: XCTestCase {
    func testClaudeHookSnapshotRequiresTranscriptFile() throws {
        ...
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-claude-restore-\(UUID().uuidString)", isDirectory: true)
    ...
    func testClaudeTranscriptCreatedAfterAbsentLoadInvalidatesSharedLookup() throws {
        ...
        let fixture = try makeClaudeTranscriptCacheFixture(
            prefix: "bmux-claude-cache-create",
            sessionId: sessionId
        )
    ...
    func testClaudeTranscriptDeletedBetweenLoadsInvalidatesSharedLookup() throws {
        ...
        let fixture = try makeClaudeTranscriptCacheFixture(
            prefix: "bmux-claude-cache-delete",
            sessionId: sessionId
        )
        defer { try? fm.removeItem(at: fixture.root) }
    ...
    func testClaudeTranscriptLookupIsStableAcrossUnchangedLoads() throws {
        ...
        let fixture = try makeClaudeTranscriptCacheFixture(
            prefix: "bmux-claude-cache-stable",
            sessionId: sessionId
        )
        defer { try? fm.removeItem(at: fixture.root) }
    ...
    func testClaudeZeroByteTranscriptIsRecheckedOnNextLoad() throws {
        ...
        let fixture = try makeClaudeTranscriptCacheFixture(
            prefix: "bmux-claude-cache-empty-file",
            sessionId: sessionId
        )
        defer { try? fm.removeItem(at: fixture.root) }
    ...
    func testClaudeNestedTranscriptCreatedWithoutProjectRootMtimeChangeIsFound() throws {
        ...
        let fixture = try makeClaudeTranscriptCacheFixture(
            prefix: "bmux-claude-cache-nested-create",
            sessionId: sessionId
        )
        defer { try? fm.removeItem(at: fixture.root) }
    ...
    func testClaudeNestedTranscriptDeletedWithoutProjectRootMtimeChangeStopsResolving() throws {
        ...
        let fixture = try makeClaudeTranscriptCacheFixture(
            prefix: "bmux-claude-cache-nested-delete",
            sessionId: sessionId
        )
        defer { try? fm.removeItem(at: fixture.root) }
    ...
    func testPanelFallbackUsesLatestHookRecord() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-claude-panel-fallback-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testClaudeForkResolvesDriftedCwdViaTranscriptPath() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-claude-fork-drift-path-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testClaudeForkResolvesDriftedCwdViaConfigScanWhenTranscriptPathMissing() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-claude-fork-drift-scan-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testClaudeResumePrefersLaunchCwdWhenTranscriptUnverifiable() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-claude-fallback-prefers-launch-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testDirectoryNamespacedNonClaudeAgentResolvesDriftToLaunchCwd() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-gemini-drift-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testPiDetectedLatestSessionDoesNotCollapseExactHookRecordsAcrossPanels() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-pi-restore-collapse-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        ...
        var registration = BmuxVaultAgentRegistration.builtInPi
        registration.sessionDirectory = sessionsRoot.path
        let registry = BmuxVaultAgentRegistry(registrations: [registration])
        let workspaceId = UUID()
    ...
    func testPiInferredLatestFallbackUsesSameKindPanelHookWhenAnotherKindIsNewer() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-pi-restore-kind-fallback-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        ...
        let index = RestorableAgentSessionIndex.load(
            homeDirectory: root.path,
            fileManager: fm,
            registry: BmuxVaultAgentRegistry(registrations: [.builtInPi]),
            detectedSnapshots: [
                key: (
    ...
    // classifier (in BMUXAgentLaunch) so the app-side resolver and the CLI surface-restore publisher
    // apply one policy. The shared resolver's own behavior is covered in BMUXAgentLaunchTests.
    ...
    func testClaudeWorkflowDirectorySessionUsesSiblingJsonlSessionForResume() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-claude-workflow-directory-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testCustomVaultAgentPreservesRuntimeCwdOnRestore() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-custom-cwd-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let launchCwd = root.appendingPathComponent("repo.main", isDirectory: true)
        ...
        let registry = BmuxVaultAgentRegistry(registrations: [
            BmuxVaultAgentRegistration(
                id: agentId,
                name: "My Agent",
                detect: BmuxVaultAgentDetectRule(processNames: [agentId]),
    ...
    func testForkCommandUsesPerAgentVerbAndSessionCwd() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-fork-agents-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testNonForkAgentsProduceNoForkCommand() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-nofork-agents-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func testReplacementRestoresNewestSessionForSurface() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-newest-wins-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func testRestoreIsIdempotentAcrossReloads() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-idempotent-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func testKilledSessionWithDeadProcessDoesNotRestore() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-killed-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        ...
        let registry = BmuxVaultAgentRegistry.load(homeDirectory: root.path, fileManager: fm)
        let index = RestorableAgentSessionIndex.load(
            homeDirectory: root.path,
            fileManager: fm,
            registry: registry,
    ...
    // from the frozen launch working directory, mirroring the production drift in CLI/bmux.swift.
    ...
    // A codex launched from inside a claude session inherits claude's BMUX_AGENT_LAUNCH_*
    ...
    func testCrossAgentLaunchCaptureIsDiscardedForResumeAndFork() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-cross-agent-capture-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        ...
        // Bare `codex` now routes through the codex wrapper token (BMUX_CODEX_WRAPPER_SHIM)
        // wrapped in `/bin/sh -c '…'` so the resumed session keeps bmux hooks (issue #5639).
        XCTAssertTrue(resume.contains("/bin/sh -c "), "codex resume must wrap the wrapper token for any login shell; got: \(resume)")
        XCTAssertTrue(resume.contains("BMUX_CODEX_WRAPPER_SHIM"), "codex resume must route through the codex wrapper shim; got: \(resume)")
        ...
        XCTAssertTrue(fork.contains("BMUX_CODEX_WRAPPER_SHIM"), "codex fork must route through the codex wrapper shim; got: \(fork)")
    ...
    func testShellWrapperArgvCaptureIsDiscardedForResumeAndFork() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-shell-argv-capture-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        ...
        record["launchCommand"] = [
            "launcher": "codex",
            "executablePath": "sh",
            "arguments": ["sh", "-c", "payload=\"${BMUX_HOOK_PAYLOAD:-}\"; eval \"$command\""],
        ...
        // (issue #5639) so the resumed session keeps bmux hooks.
        XCTAssertTrue(resume.contains("BMUX_CODEX_WRAPPER_SHIM"), "codex resume must route through the codex wrapper shim; got: \(resume)")
        XCTAssertTrue(resume.contains("resume") && resume.contains(sid), "codex resume must use the resume verb and session id; got: \(resume)")
        let fork = try XCTUnwrap(snapshot.forkCommand)
    ...
    func testWrapperLauncherCaptureStaysTrusted() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-wrapper-launcher-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        ...
        record["launchCommand"] = [
            ...
            "arguments": ["/usr/local/bin/bmux", "codex-teams"],
            "workingDirectory": dir.path,
            "capturedAt": 10,
            "source": "environment",
        ]
        ...
        XCTAssertTrue(
            ...
            "codexTeams capture must keep routing fork through the bmux wrapper; got: \(fork)"
        )
    }
    ...
    private func writeHookStore(
        ...
        let stateDir = root.appendingPathComponent(".bmuxterm", isDirectory: true)
