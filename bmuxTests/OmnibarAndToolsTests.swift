...
final class FinderServicePathResolverTests: XCTestCase {
    private func withTemporaryDirectory<T>(_ body: (URL) throws -> T) throws -> T {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-finder-service-tests-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/bmux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/bmux-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/bmux-services/../bmux-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/bmux-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/bmux-services/project",
                "/tmp/bmux-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/bmux-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/bmux-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/bmux-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/bmux-services/b/file.txt", isDirectory: false),
        ]
    ...
    func testOrderedUniqueDirectoriesSkipsBundleAndEmbeddedPathsWhenExcludingBundleRoot() {
        let bundleURL = URL(fileURLWithPath: "/Applications/Tools/../bmux.app", isDirectory: true)
        let input: [URL] = [
            bundleURL,
            URL(fileURLWithPath: "/Applications/bmux.app/Contents/MacOS/bmux", isDirectory: false),
            URL(fileURLWithPath: "/Applications/bmux.app/Contents/Resources/bin/bmux", isDirectory: false),
            URL(fileURLWithPath: "/Users/tester/Projects/bmux", isDirectory: true),
            URL(fileURLWithPath: "/Users/tester/Projects/bmux/README.md", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Users/tester/Projects/bmux",
            ]
        )
    }

    func testOrderedUniqueDirectoriesExclusionDoesNotFilterSiblingPaths() {
        let bundleURL = URL(fileURLWithPath: "/Applications/bmux.app", isDirectory: true)
        let input: [URL] = [
            URL(fileURLWithPath: "/Applications/bmux.app backup/project", isDirectory: true),
            URL(fileURLWithPath: "/Applications/bmux.app.beta/project/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Applications/bmux.app backup/project",
                "/Applications/bmux.app.beta/project",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesResolvesSymlinksOnlyForExcludedRootComparison() throws {
        try withTemporaryDirectory { root in
            ...
            let actualBundle = applicationsDirectory.appendingPathComponent("bmux.app", isDirectory: true)
            let actualBinary = actualBundle.appendingPathComponent("Contents/MacOS/bmux", isDirectory: false)
            ...
            let directories = FinderServicePathResolver.orderedUniqueDirectories(
                from: [
                    aliasApplications.appendingPathComponent("bmux.app", isDirectory: true),
                    aliasApplications.appendingPathComponent("bmux.app/Contents/MacOS/bmux", isDirectory: false),
                    aliasWorkspace,
                ],
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/bmux"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/bmux")
    }
...
final class BrowserOmnibarFieldEditorResolutionTests: XCTestCase {
    func testPanelIdResolutionUsesLiveOmnibarFieldWhenFieldEditorResponderChainIsStale() {
        ...
        let staleWebView = BmuxWebView(frame: NSRect(x: 0, y: 0, width: 420, height: 80), configuration: WKWebViewConfiguration())
