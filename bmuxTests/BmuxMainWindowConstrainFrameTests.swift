...
#if DEBUG
@MainActor
final class BmuxMainWindowConstrainFrameTests: XCTestCase {
    ...
    // on-screen; bmux never re-asserts its saved frame afterward, so the window
    // creeps each sleep cycle. BmuxMainWindow.constrainFrameRect must leave an
    ...
    func testPreservesFrameFullyInsideVisibleArea() {
        ...
        XCTAssertTrue(
            BmuxMainWindow.shouldPreserveFrameDuringConstrain(frame, visibleFrames: [visible])
        )
    }
    ...
    func testDoesNotPreserveWhenNoScreensAvailable() {
        ...
        XCTAssertFalse(
            BmuxMainWindow.shouldPreserveFrameDuringConstrain(frame, visibleFrames: [])
        )
    }
    ...
    func testDoesNotPreserveFrameWhoseTitlebarIsStrandedAboveTheOnlyScreen() {
        ...
        XCTAssertFalse(
            BmuxMainWindow.shouldPreserveFrameDuringConstrain(
                strandedAbove,
    ...
    func testPreservesFrameParkedAtSideEdgeWithGrabbableTitlebar() {
        ...
        XCTAssertTrue(
            BmuxMainWindow.shouldPreserveFrameDuringConstrain(parkedLeft, visibleFrames: [visible])
        )
    }
    ...
    func testPreservesFlushTopFrameOnLargeMenuBarDisplay() {
        ...
        XCTAssertTrue(
            BmuxMainWindow.shouldPreserveFrameDuringConstrain(flushTop, visibleFrames: [visible])
        )
    }
}
#endif
