...
final class AppDelegateShortcutRoutingTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        originalSettingsFileStore = KeyboardShortcutSettings.installIsolatedTestFileStore(prefix: "bmux-shortcut-routing")
    ...
    func testSettingsFileChordDispatchesNewWorkspaceShortcut() throws {
        ...
        RunLoop.main.run(until: Date(timeIntervalSinceNow: 0.05))
        XCTAssertEqual(manager.tabs.count, initialCount + 1, "bmux.json chord should dispatch the configured shortcut")
    }
    ...
    func testWelcomeWindowSidebarShortcutsUseSharedToggleCommands() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("bmux.main.\(windowId.uuidString)")
    ...
    func testAddWorkspaceInPreferredMainWindowPrunesOrphanedContextWithoutLiveWindow() {
        ...
        autoreleasepool {
            ...
            orphanWindow?.identifier = NSUserInterfaceItemIdentifier("bmux.main.\(orphanWindowId.uuidString)")
            appDelegate.registerMainWindow(
                orphanWindow!,
    ...
    func testCmdDPropagatesWhenSplitRightShortcutIsCleared() {
        ...
        withTemporaryShortcut(action: .splitRight, shortcut: .unbound) {
            ...
            XCTAssertFalse(
                appDelegate.debugHandleCustomShortcut(event: event),
                "Cleared Cmd+D split shortcut should not be consumed by bmux"
            )
#else
            XCTFail("debugHandleCustomShortcut is only available in DEBUG")
#endif
    ...
    func testRemappedCloseTabDoesNotLetCmdWReachGhosttyCloseSurfaceFallback() throws {
        ...
        probeWindow.identifier = NSUserInterfaceItemIdentifier("bmux.browser-popup")
        ...
        withTemporaryShortcut(action: .closeTab, shortcut: remappedCloseTab) {
            ...
            XCTAssertFalse(
                ...
                "After Close Tab is remapped, Cmd+W must not match the bmux Close Tab action"
            )
            ...
            XCTAssertEqual(
                ...
                1,
                "Remapped-away Cmd+W should reach the terminal as input instead of closing through bmux"
            )
            ...
            XCTAssertTrue(
                ...
                "The remapped Cmd+Option+W shortcut should match the bmux Close Tab action"
            )
#if DEBUG
            XCTAssertTrue(
                appDelegate.debugHandleShortcutMonitorEvent(event: remappedCmdOptionW),
                "The remapped Cmd+Option+W shortcut should trigger the bmux Close Tab action"
            )
#else
            XCTFail("debugHandleShortcutMonitorEvent is only available in DEBUG")
#endif
    ...
    func testGhosttyConfigDoesNotRetainNumberedGotoTabFallback() throws {
        // Regression for https://github.com/manaflow-ai/bmux/issues/5189.
        // bmux owns "Select Workspace 1…9" (default ⌘1–9) through KeyboardShortcutSettings.
        // Ghostty's built-in super+1…8 = goto_tab and super+9 = last_tab fallbacks must be
        // unbound — exactly like bmux already unbinds super+d / super+w — so the numbered
    ...
    func testRebindingSelectWorkspaceByNumberHonorsNewModifierAndDropsDefault() {
        ...
        // the bmux routing layer must drive the numbered shortcut from the configured
        // value, so a rebound modifier selects the workspace and the old ⌘ default no
        // longer routes through bmux.
        guard let appDelegate = AppDelegate.shared else {
            XCTFail("Expected AppDelegate.shared")
            return
        }
        ...
        withTemporaryShortcut(action: .selectWorkspaceByNumber, shortcut: rebound) {
            ...
            XCTAssertFalse(
                ...
                "After rebinding Select Workspace 1…9, the old ⌘1 must not be routed by bmux"
            )
#else
            XCTFail("debugHandleCustomShortcut is only available in DEBUG")
#endif
            ...
            XCTAssertTrue(
                ...
                "The rebound Ctrl+Option+1 shortcut should be routed by bmux"
            )
#else
            XCTFail("debugHandleCustomShortcut is only available in DEBUG")
#endif
    ...
    func testBrowserPopupPanelCloseShortcutFollowsCloseTabRemap() throws {
        ...
        withTemporaryShortcut(action: .closeTab, shortcut: remappedCloseTab) {
            ...
            panel.identifier = NSUserInterfaceItemIdentifier("bmux.browser-popup")
    ...
    func testCmdWClosesAuxiliaryWindowInsteadOfMainTerminalPanel() throws {
        ...
        auxiliaryWindow.identifier = NSUserInterfaceItemIdentifier("bmux.about")
        ...
        XCTAssertNotEqual(NSApp.keyWindow?.identifier?.rawValue, "bmux.about", "Closed auxiliary window should not remain key")
    }
    ...
    func testCmdWClosesMobilePairingWindowInsteadOfTerminalTab() throws {
        ...
        // window's identifier ever drops out of bmuxAuxiliaryWindowIdentifiers
    ...
    func testMinimalModeTitlebarPaddingOnlyCancelsHostingSafeArea() {
        XCTAssertEqual(
            ...
            accuracy: 0.5,
            "Standard mode should align terminal content with bmux's visual titlebar height even when AppKit reports a taller native titlebar zone"
        )
    ...
    func testAttachUpdateAccessoryHidesTitlebarAccessoryWhenMinimalModeEnabled() {
        ...
        let titlebarAccessory: () -> NSTitlebarAccessoryViewController? = {
            window.titlebarAccessoryViewControllers.first {
                $0.view.identifier?.rawValue == "bmux.titlebarControls"
            }
        }
    ...
    func testKeyboardShortcutSettingsSetShortcutPostsSpecificChangeNotification() {
        let notificationName = Notification.Name("bmux.keyboardShortcutSettingsDidChange")
    ...
    func testBrowserFirstFindShortcutRoutingExcludesAppOwnedFindCommands() {
        ...
        for testCase in cases {
            ...
            XCTAssertFalse(
                shouldRouteBrowserFindCommandEquivalentThroughWebContentFirst(event),
                "\(testCase.name) belongs to bmux find routing, not browser-first routing"
            )
        }
    }
    ...
    func testInlineVSCodeCommandPaletteShortcutDoesNotRouteForUntrackedLocalhostPage() {
        ...
        XCTAssertFalse(
            ...
            "A localhost page with a folder query must not steal bmux's command palette shortcut unless it is the tracked VS Code serve-web origin"
        )
    }
    ...
    func testInlineVSCodeCommandPaletteShortcutDoesNotRouteUnrelatedShortcut() {
        ...
        XCTAssertFalse(
            ...
            "Only the configured command palette shortcut should bypass bmux for inline VS Code"
        )
    }
    ...
    func testWindowPerformKeyEquivalentSuppressesRemappedCmdDStaleMenuShortcut() {
        ...
        withTemporaryShortcut(action: .splitRight, shortcut: remappedSplitRight) {
            XCTAssertFalse(
                probeWindow.performKeyEquivalent(with: event),
                "Remapped Cmd+D should not be consumed by stale bmux menu equivalents"
            )
        }
    ...
    func testCurrentGlobalSearchShortcutIsNotSuppressedAsStaleMenuShortcut() {
        ...
        withTemporaryShortcut(action: .globalSearch, shortcut: remappedGlobalSearch) {
            XCTAssertFalse(
                appDelegate.shouldSuppressStaleBmuxMenuShortcut(event: event),
    ...
    func testTextBoxMentionFileSuggestionsUseCommandPaletteSearchIndex() async throws {
        ...
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "bmux-textbox-mentions-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testTextBoxMentionFileSuggestionsRefreshCachedMisses() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "bmux-textbox-mentions-refresh-\(UUID().uuidString)",
            isDirectory: true
        )
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testTextBoxMentionSkillSuggestionsUseTypedDollarTrigger() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "bmux-textbox-skills-\(UUID().uuidString)",
            isDirectory: true
        )
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testTextBoxSubmitUsesLocalPreviewPathForClaudeRemoteImage() throws {
        ...
        let remotePath = "/tmp/bmux-upload/moon.png"
        let attachment = TextBoxAttachment(
            localURL: previewURL,
    ...
    func testTextBoxPasteboardRestorationSkipsAfterUserClipboardChange() throws {
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("bmux.textbox.restore.\(UUID().uuidString)"))
        defer {
            pasteboard.clearContents()
...
#if DEBUG
        XCTAssertFalse(
            bmuxCloseFocusedTerminalFindForEscape(event: escapeEvent, appDelegate: appDelegate),
    ...
    private func makeTemporaryPNGFile(named name: String) throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-textbox-attachment-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeBrowserFocusModeHarness(
        file: StaticString = #filePath,
        line: UInt = #line
    ) -> (windowId: UUID, window: NSWindow, panel: BrowserPanel, webView: BmuxWebView)? {
        guard let appDelegate = AppDelegate.shared else {
            XCTFail("Expected AppDelegate.shared", file: file, line: line)
            return nil
        }
        ...
        guard let window = window(withId: windowId),
              ...
              let webView = browserPanel.webView as? BmuxWebView else {
            closeWindow(withId: windowId)
    ...
    private func mainWindowIds() -> Set<UUID> {
        Set(NSApp.windows.compactMap { window in
            guard let raw = window.identifier?.rawValue,
                  raw.hasPrefix("bmux.main.") else {
                return nil
            }
            return UUID(uuidString: String(raw.dropFirst("bmux.main.".count)))
        })
    }
