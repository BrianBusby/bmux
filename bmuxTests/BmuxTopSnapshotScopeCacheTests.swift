...
// Regression coverage for the bmux-scope attribution cache used by system.top.
// See https://github.com/manaflow-ai/bmux/issues/5756.
//
// `.serialized` because these tests share the process-global bmux-top scope
// cache (`BmuxTopProcessSnapshot.cachedBMUXScope` / `pruneBMUXScopeCache`); they
// must not interleave with each other.
@Suite(.serialized)
struct BmuxTopSnapshotScopeCacheTests {
    ...
    static let afterNegativeTTLNanoseconds = nowNanoseconds
        + bmuxTopNegativeScopeTTLNanoseconds
        + 1

    // Before the fix, a process with no bmux scope was a permanent cache miss, so
    // every system.top poll re-ran the 3-sysctl probe for every non-bmux process
    // on the machine. The negative result must now be cached within the TTL.
    @Test func negativeScopeResultIsCachedWithinTTL() {
        BmuxTopProcessSnapshot.pruneBMUXScopeCache(activeKeys: [])
        let cacheKey = BmuxTopProcessScopeCacheKey(
            pid: 901_001,
            startSeconds: 1_700_000_001,
            startMicroseconds: 11
        )
        var probeCount = 0
        let probe: (Int, BmuxTopProcessScopeCacheKey) -> BmuxTopProcessScopeProbeResult = { _, _ in
            probeCount += 1
            return .resolved(nil)
        }

        let first = BmuxTopProcessSnapshot.cachedBMUXScope(
            for: 901_001, cacheKey: cacheKey, nowNanoseconds: Self.nowNanoseconds, probe: probe)
        let second = BmuxTopProcessSnapshot.cachedBMUXScope(
            for: 901_001, cacheKey: cacheKey, nowNanoseconds: Self.nowNanoseconds, probe: probe)

        #expect(first == nil)
        #expect(second == nil)
        #expect(probeCount == 1, "non-bmux process should be probed once per TTL, not on every poll")
    }
    ...
    @Test func positiveScopeResultIsCachedAcrossPolls() {
        ...
        let expected = BmuxTopProcessScope(
    ...
    @Test func transientProbeFailureIsNotCachedAndRetries() {
        ...
        let second = BmuxTopProcessSnapshot.cachedBMUXScope(
            for: 901_003, cacheKey: cacheKey, nowNanoseconds: Self.nowNanoseconds, probe: probe)
        let third = BmuxTopProcessSnapshot.cachedBMUXScope(
            for: 901_003, cacheKey: cacheKey, nowNanoseconds: Self.nowNanoseconds, probe: probe)
        let fourth = BmuxTopProcessSnapshot.cachedBMUXScope(
            for: 901_003, cacheKey: cacheKey, nowNanoseconds: Self.nowNanoseconds, probe: probe)
    ...
    // into a bmux-scoped command later, must be re-probed once the negative TTL
    // elapses so it is eventually attributed (https://github.com/manaflow-ai/bmux/issues/5756).
    @Test func negativeScopeIsReprobedAfterTTLExpiry() {
        ...
        let postExecScope = BmuxTopProcessScope(
            workspaceID: UUID(uuidString: "12121212-1212-1212-1212-121212121212"),
            surfaceID: nil,
            attributionReason: "bmux-environment"
        )
        var probeCount = 0
        var resolvedScope: BmuxTopProcessScope?
        let probe: (Int, BmuxTopProcessScopeCacheKey) -> BmuxTopProcessScopeProbeResult = { _, _ in
            probeCount += 1
        ...
        let preExec = BmuxTopProcessSnapshot.cachedBMUXScope(
        ...
        let withinTTL = BmuxTopProcessSnapshot.cachedBMUXScope(
            for: 901_004, cacheKey: cacheKey, nowNanoseconds: Self.nowNanoseconds, probe: probe)
        #expect(preExec == nil)
        #expect(withinTTL == nil)
        #expect(probeCount == 1)

        // Process has since execed into a bmux-scoped command; after the TTL the
        // negative entry expires and the next poll re-probes and attributes it.
        resolvedScope = postExecScope
        let afterExpiry = BmuxTopProcessSnapshot.cachedBMUXScope(
    ...
    @Test func liveProcessScopeProbeResolves() throws {
        ...
        let cacheKey = BmuxTopProcessSnapshot.scopeCacheKey(from: info)

        let result = BmuxTopProcessSnapshot.bmuxScopeProbe(for: pid, expectedCacheKey: cacheKey)

        switch result {
        case .resolved:
            break
        case .unavailable:
            Issue.record("live process probe should resolve, even when it has no bmux scope")
        }
    }
    ...
    @Test func negativeProbeDoesNotClobberConcurrentlyDiscoveredPositive() {
        ...
        let positive = BmuxTopProcessScope(
        ...
        let probe: (Int, BmuxTopProcessScopeCacheKey) -> BmuxTopProcessScopeProbeResult = { pid, key in
            _ = BmuxTopProcessSnapshot.cachedBMUXScope(
                for: pid, cacheKey: key, nowNanoseconds: Self.nowNanoseconds) { _, _ in .resolved(positive) }
            return .resolved(nil)
        }

        let result = BmuxTopProcessSnapshot.cachedBMUXScope(
        ...
        let nextPoll = BmuxTopProcessSnapshot.cachedBMUXScope(
