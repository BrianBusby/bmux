...
@Suite struct ClaudeWrapperResumeEnvironmentTests {
    @Test func bundledClaudeWrapperScrubsNestedSessionEnvironmentOnResume() throws {
        ...
        let wrapperURL = repoRoot.appendingPathComponent("Resources/bin/bmux-claude-wrapper", isDirectory: false)
        #expect(
            fileManager.isExecutableFile(atPath: wrapperURL.path),
            "Bundled bmux-claude-wrapper must exist and be executable for resume environment coverage"
        )
        ...
        let sandbox = URL(fileURLWithPath: "/tmp", isDirectory: true)
            .appendingPathComponent("bmux-resume-\(String(UUID().uuidString.prefix(8)))", isDirectory: true)
        ...
        let socketURL = sandbox.appendingPathComponent("bmux.sock", isDirectory: false)
        ...
        let fakeBmuxURL = binDir.appendingPathComponent("bmux", isDirectory: false)
        try writeExecutable(
            fakeBmuxURL,
            """
            #!/usr/bin/env bash
        ...
        process.environment = [
            ...
            "BMUX_SOCKET_PATH": socketURL.path,
            "BMUX_BUNDLED_CLI_PATH": fakeBmuxURL.path,
            "CLAUDECODE": "1",
            "CLAUDE_CODE": "1",
        ...
        try runWithBoundedWait(process, shellDescription: "bmux-claude-wrapper --resume")
