...
struct RestorableAgentSessionStalePIDTests {
    @Test func stalePIDHookRecordStillRestoresAndForks() throws {
        ...
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-stale-pid-fork-\(UUID().uuidString)", isDirectory: true)
        ...
        setenv("BMUX_AGENT_HOOK_STATE_DIR", stateDir.path, 1)
        defer {
    ...
    @Test func newerStalePIDHookRecordDoesNotReplaceLivePanelRecord() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-live-pid-preserved-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        let previousHookStateDir = getenv("BMUX_AGENT_HOOK_STATE_DIR").map { String(cString: $0) }
        ...
        let index = RestorableAgentSessionIndex.load(
            ...
            processArgumentsProvider: { pid in
                ...
                return BmuxTopProcessArguments(
                    ...
                    environment: [
                        "BMUX_WORKSPACE_ID": ws.uuidString,
                        "BMUX_SURFACE_ID": panel.uuidString,
                    ]
                )
            }
        )
