...
import BmuxTerminal
import BmuxBrowser
import struct BmuxSettings.IntegrationsCatalogSection
import enum BmuxSettings.KiroNotificationLevel
...
private typealias ShortcutStroke = bmux_DEV.ShortcutStroke
private typealias AppIconMode = bmux_DEV.AppIconMode
#elseif canImport(bmux)
@testable import bmux
private typealias StoredShortcut = bmux.StoredShortcut
private typealias ShortcutStroke = bmux.ShortcutStroke
private typealias AppIconMode = bmux.AppIconMode
#endif
...
final class SidebarSelectedWorkspaceColorTests: XCTestCase {
    func testLightModeUsesDefaultSelectedWorkspaceBackgroundColor() {
    ...
    func testDarkModeUsesDefaultSelectedWorkspaceBackgroundColor() {
        guard let color = sidebarSelectedWorkspaceBackgroundNSColor(
            for: .dark,
            sidebarSelectionColorHex: nil
        ).usingColorSpace(.sRGB) else {
    ...
    func testDefaultSelectedWorkspaceForegroundUsesNativeSelectionTextOnAccentBackground() {
        guard let color = sidebarSelectedWorkspaceForegroundNSColor(
            on: sidebarSelectedWorkspaceBackgroundNSColor(for: .light, sidebarSelectionColorHex: nil),
            opacity: 0.65
        ).usingColorSpace(.sRGB) else {
            XCTFail("Expected sRGB-convertible color")
            return
    ...
    @MainActor
    func testSolidFillUsesCustomWorkspaceColorForActiveWorkspaceRow() {
        ...
        XCTAssertEqual(background.opacity, 0.46, accuracy: 0.001)
        withExtendedLifetime(cancellable) {}
    }

    @MainActor
    func testLeftRailUsesClearBackgroundForActiveCustomColoredWorkspaceRow() {
        let manager = TabManager()
        guard let workspace = manager.tabs.first else {
            XCTFail("Expected TabManager to initialise with a workspace")
            return
...
final class KeyboardShortcutSettingsFileStoreTests: XCTestCase {
    ...
    func testSettingsFileStoreUsesLegacyFallbackWhenCanonicalConfigHasNoSetting() throws {
        ...
        try writeSettingsFile(
            """
            {
              "$schema": "https://raw.githubusercontent.com/manaflow-ai/bmux/main/web/data/bmux-settings.schema.json",
              "shortcuts": {
    ...
    @MainActor
    func testReloadConfigurationMenuActionReloadsRegisteredBmuxConfigStore() throws {
#if DEBUG
        let directoryURL = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directoryURL) }
        ...
        XCTAssertNotNil(bmuxConfigStore.resolvedAction(id: "first"))
        XCTAssertNil(bmuxConfigStore.resolvedAction(id: "second"))
        ...
        let appMenuItem = NSMenuItem(title: "bmux", action: nil, keyEquivalent: "")
        let appMenu = NSMenu(title: "bmux")
        ...
        XCTAssertEqual(
            ...
            NSUserInterfaceItemIdentifier("com.bmux.reloadConfiguration")
        )
        ...
        XCTAssertNil(bmuxConfigStore.resolvedAction(id: "first"))
        XCTAssertNotNil(bmuxConfigStore.resolvedAction(id: "second"))
#else
    ...
    func testBootstrapCreatesCommentedTemplateWhenPrimaryAndFallbackAreMissing() throws {
        ...
        let settingsFileURL = directoryURL
            .appendingPathComponent(".config/bmux", isDirectory: true)
        ...
        XCTAssertTrue(contents.contains(#""$schema": "https://raw.githubusercontent.com/manaflow-ai/bmux/main/web/data/bmux.schema.json""#))
    ...
    func testSettingsFileURLForEditingPrefersInvalidPrimaryForRepair() throws {
        ...
        let primaryURL = directoryURL.appendingPathComponent("primary/bmux.json", isDirectory: false)
        let fallbackURL = directoryURL.appendingPathComponent("fallback/bmux.json", isDirectory: false)
...
final class StoredShortcutMatchingTests: XCTestCase {
    ...
    func testSystemWideHotkeyNormalizationReportsBmuxActionConflictByRecordedPhysicalKey() {
        KeyboardShortcutSettings.resetAll()
        defer { KeyboardShortcutSettings.resetAll() }
...
final class WorkspaceCreationWorkingDirectoryInheritanceTests: XCTestCase {
    ...
    func testNewWorkspaceInheritsSourceWorkingDirectoryByDefault() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(nil) {
            let sourceCwd = "/tmp/bmux-source-\(UUID().uuidString)"
            let manager = TabManager(
    ...
    func testExplicitWorkspaceWorkingDirectoryWinsWhenInheritanceIsDisabled() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(false) {
            let sourceCwd = "/tmp/bmux-source-\(UUID().uuidString)"
            let explicitCwd = "/tmp/bmux-explicit-\(UUID().uuidString)"
            let manager = TabManager(
                initialWorkingDirectory: sourceCwd,
                autoWelcomeIfNeeded: false
            )
    ...
    func testDetachedWorkspaceTransferDirectoryWinsWhenInheritanceIsDisabled() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(false) {
            let sourceCwd = "/tmp/bmux-source-\(UUID().uuidString)"
            let transferCwd = "/tmp/bmux-detached-\(UUID().uuidString)"
            let manager = TabManager(
                initialWorkingDirectory: sourceCwd,
                autoWelcomeIfNeeded: false
            )
...
final class WorkspaceCreationConfigSanitizationTests: XCTestCase {
    private final class UnsafeConfigSnapshotTabManager: TabManager {
        ...
        func installInjectedConfig(fontSize: Float) {
            ...
            config.workingDirectory = "/tmp/bmux-workspace-snapshot"
            config.command = "echo snapshot"
            config.environmentVariables = ["BMUX_INHERITED_ENV": "1"]
...
final class WorkspaceReorderTests: XCTestCase {
    ...
    func testMoveTabToTopPublishesWorkspaceReorderedEvent() throws {
        ...
        let event = try XCTUnwrap(BmuxEventBus.shared.retainedSnapshot().last)
...
final class WorkspaceSplitWorkingDirectoryTests: XCTestCase {
    ...
    func testNewTerminalSplitFallsBackToRequestedWorkingDirectoryWhenReportedDirectoryIsStale() {
        ...
        let requestedDirectory = "/tmp/bmux-requested-split-cwd-\(UUID().uuidString)"
...
final class WorkspaceSidebarExtensionBrowserSurfaceTests: XCTestCase {
    ...
    func testCloudVMLoadingSurfaceSwapsToTerminalInPlace() throws {
        ...
        let command = "bmux vm-pty-connect --config /tmp/bmux.json --id vm_123"
    ...
    func testCloudVMLoadingFailureSummarizesRetrySpam() {
        ...
        \u{001B}[2K[bmux] Waiting for the Cloud VM service. Retrying in 2s (attempt 1/120).
        \u{001B}[2K[bmux] Waiting for the Cloud VM service. Retrying in 2s (attempt 2/120).
        \u{001B}[2K[bmux] Waiting for the Cloud VM service. Retrying in 2s (attempt 3/120).
        """)
        ...
        XCTAssertFalse(message.contains("[bmux]"))
...
final class WorkspacePanelGitBranchTests: XCTestCase {
    ...
    func testForkAgentConversationInRemoteWorkspaceUsesRemoteStartupCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/bmux-fork-remote.sock",
        ...
        XCTAssertEqual(forkPanel.surface.debugInitialCommand(), "ssh bmux-macmini")
        XCTAssertNil(forkPanel.requestedWorkingDirectory)
        XCTAssertEqual(workspace.panelDirectories[forkPanel.id], "/Users/bmux/project")
        XCTAssertEqual(forkPanel.surface.initialInput, snapshot.forkCommand.map { $0 + "\n" })
    ...
    func testForkAgentConversationInRemoteWorkspaceUsesFallbackDirectoryInForkCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/bmux-fork-fallback-remote.sock",
                terminalStartupCommand: "ssh bmux-macmini"
            ),
            autoConnect: false
        )
        workspace.currentDirectory = "/Users/bmux/fallback repo"
        let sourcePanelId = try XCTUnwrap(workspace.focusedPanelId)
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .codex,
            sessionId: "019dad34-d218-7943-b81a-eddac5c87951",
        ...
        XCTAssertEqual(forkPanel.surface.debugInitialCommand(), "ssh bmux-macmini")
        XCTAssertNil(forkPanel.requestedWorkingDirectory)
        XCTAssertEqual(workspace.panelDirectories[forkPanel.id], "/Users/bmux/fallback repo")
        XCTAssertEqual(
            forkPanel.surface.initialInput,
            "cd -- '/Users/bmux/fallback repo' 2>/dev/null || [ ! -d '/Users/bmux/fallback repo' ] && '/Users/example/.bun/bin/codex' 'fork' '019dad34-d218-7943-b81a-eddac5c87951'\n"
        )
    }
    ...
    func testSessionIndexRemoteSplitDoesNotInjectRemoteStartupCommand() throws {
        ...
        let hookStateRoot = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-session-drop-hook-state-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: hookStateRoot, withIntermediateDirectories: true)
        let previousHookStateDir = getenv("BMUX_AGENT_HOOK_STATE_DIR").map { String(cString: $0) }
        setenv("BMUX_AGENT_HOOK_STATE_DIR", hookStateRoot.path, 1)
        defer {
            if let previousHookStateDir {
                setenv("BMUX_AGENT_HOOK_STATE_DIR", previousHookStateDir, 1)
            } else {
                unsetenv("BMUX_AGENT_HOOK_STATE_DIR")
            }
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/bmux-session-drop-remote.sock",
                terminalStartupCommand: "ssh bmux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testForkAgentWorkspaceLaunchInRemoteWorkspacePreservesRemoteContext() throws {
        let workspace = Workspace()
        let agentSocketPath = "/tmp/bmux-fork-agent.sock"
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                destination: "bmux-macmini",
                port: 2222,
                identityFile: "/Users/example/.ssh/bmux",
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/bmux-fork-remote.sock",
                terminalStartupCommand: "ssh -p 2222 -i /Users/example/.ssh/bmux -o ServerAliveInterval=30 -o ForwardAgent=yes -tt bmux-macmini",
        ...
        XCTAssertEqual(launch.workingDirectory, "/Users/bmux/project")
        XCTAssertNil(launch.terminalWorkingDirectory)
        XCTAssertEqual(
            launch.initialTerminalCommand,
            "ssh -p 2222 -i /Users/example/.ssh/bmux -o ServerAliveInterval=30 -o ForwardAgent=yes -tt bmux-macmini"
        )
        ...
        XCTAssertEqual(launch.remoteConfiguration?.destination, "bmux-macmini")
        XCTAssertEqual(launch.remoteConfiguration?.port, 2222)
        XCTAssertEqual(launch.remoteConfiguration?.identityFile, "/Users/example/.ssh/bmux")
    ...
    func testForkAgentWorkspaceLaunchFromPersistentSSHPTYDoesNotReuseParentRelayOrDaemonSlot() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/bmux-fork-persistent.sock",
        ...
        XCTAssertEqual(
            startupCommand,
            "ssh -p 2222 -i /Users/example/.ssh/bmux -tt bmux-macmini"
        )
    }
    ...
    func testForkAgentWorkspaceLaunchInRemoteWorkspaceUsesFallbackDirectoryInForkCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/bmux-workspace-fallback-remote.sock",
                terminalStartupCommand: "ssh bmux-macmini"
            ),
            autoConnect: false
        )
        ...
        XCTAssertEqual(launch.workingDirectory, "/Users/bmux/fallback repo")
        XCTAssertNil(launch.terminalWorkingDirectory)
        XCTAssertEqual(launch.initialTerminalCommand, "ssh -tt bmux-macmini")
        XCTAssertEqual(
    ...
    func testForkAgentConversationInRemoteConfiguredLocalWorkspaceAllowsLauncherScript() throws {
        ...
        XCTAssertEqual(forkPanel.requestedWorkingDirectory, "/Users/bmux/project")
        ...
        XCTAssertEqual(launch.terminalWorkingDirectory, "/Users/bmux/project")
        XCTAssertNil(launch.initialTerminalCommand)
        XCTAssertFalse(launch.autoConnectRemoteConfiguration)
        XCTAssertNil(launch.remoteConfiguration)
    ...
    func testForkAgentConversationFromLocalTerminalInRemoteWorkspaceStaysLocal() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/bmux-fork-local-remote.sock",
                terminalStartupCommand: "ssh bmux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testUpdatingFocusedPanelPullRequestWithSameStateDoesNotRepublishWorkspace() {
        ...
        let pullRequestURL = URL(string: "https://github.com/manaflow-ai/bmux/pull/2388")!
    ...
    func testSidebarPullRequestsTrackFocusedPanelOnly() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1629,
            label: "PR",
            url: URL(string: "https://github.com/manaflow-ai/bmux/pull/1629")!,
            status: .open
        )
    ...
    func testSidebarDirectoryDisplayLabelUpgradesSharedRowWhileFilesystemVariantKeepsPath() {
        ...
        let sharedDirectory = "/tmp/bmux-display-label-shared"
    ...
    func testSidebarDerivedCollectionsMatchWhenUsingPrecomputedPanelOrder() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 101,
            label: "PR",
            url: URL(string: "https://github.com/manaflow-ai/bmux/pull/101")!,
            status: .open
        )
        workspace.updatePanelPullRequest(
            ...
            number: 18,
            label: "MR",
            url: URL(string: "https://gitlab.com/manaflow/bmux/-/merge_requests/18")!,
            status: .merged
        )
    ...
    func testForkConversationDefaultSettingFallsBackToRight() throws {
        let suiteName = "bmux.forkConversationDefault.\(UUID().uuidString)"
...
final class ExtensionWorktreePrototypeTests: XCTestCase {
    func testPipeOutputCollectorDrainsBufferedOutputOnFinish() async throws {
        let pipe = Pipe()
        let collector = BmuxExtensionPipeOutputCollector(fileHandle: pipe.fileHandleForReading)
    ...
    func testCreateWorktreeKeepsBmuxDirectoryLocallyIgnored() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-worktree-prototype-\(UUID().uuidString)", isDirectory: true)
        ...
        _ = try runGit([
            "-c", "user.name=bmux Test",
            "-c", "user.email=bmux@example.invalid",
            "commit",
            "-m",
            "initial"
        ], in: projectRoot)

        let result = try await BmuxExtensionWorktreePrototype.createWorktree(projectRootPath: projectRoot.path)

        XCTAssertTrue(FileManager.default.fileExists(atPath: result.worktreePath))
        XCTAssertTrue(result.workspaceTitle.hasPrefix("bmux-sidebar-"))
