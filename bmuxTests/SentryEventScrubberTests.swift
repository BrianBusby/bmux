...
@Suite struct SentryEventScrubberTests {
    ...
    @Test func scrubsFormattedOnlyMessageExceptionAndTransaction() {
        ...
        event.message = SentryMessage(formatted: "boom for lawrence@bmux.com")
    ...
    @Test func scrubsContextDictionaries() {
        let event = Event()
        // Mirrors scope.setContext(value:key:): bmux puts cwd / path data here.
        event.context = [
            "ui": ["cwd": "/Users/lawrence/dev/bmux", "action": "open"],
            "auth": ["token": "abcdef0123456789secretvalue"],
        ]

        let scrubbed = scrubber.scrub(event)
        #expect(scrubbed.context?["ui"]?["cwd"] as? String == "/Users/<redacted>/dev/bmux")
    ...
    @Test func scrubsStackFramePathsContextAndVarsButKeepsSymbols() {
        let event = Event()
        let frame = Frame()
        frame.fileName = "/Users/buildbot/work/bmux/Sources/AppDelegate.swift"
        frame.function = "applicationDidFinishLaunching(_:)"
        frame.package = "/Users/buildbot/Library/Developer/Xcode/DerivedData/bmux/Build/bmux.app"
        frame.lineNumber = 1325
        frame.contextLine = "let home = \"/Users/lawrence/secret\""
        frame.preContext = ["// open lawrence@bmux.com"]
        ...
        #expect(outFrame?.fileName == "/Users/<redacted>/work/bmux/Sources/AppDelegate.swift")
    ...
    @Test func scrubsDebugMetaCodeFilePaths() {
        let event = Event()
        let image = DebugMeta()
        image.codeFile = "/Users/lawrence/Library/Developer/Xcode/DerivedData/bmux/bmux.app/bmux"
