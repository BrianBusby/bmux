...
private extension GhosttyNSView {
    @objc func bmuxUnitTest_interpretKeyEvents(_ eventArray: [NSEvent]) {
        if let hook = cjkIMEInterpretKeyEventsHook, hook(self, eventArray) {
            return
        }
        bmuxUnitTest_interpretKeyEvents(eventArray)
    }

    @objc func bmuxUnitTest_paste(_ sender: Any?) {
        ghosttyPasteActionHook?(self, sender)
        bmuxUnitTest_paste(sender)
    }

    @objc func bmuxUnitTest_pasteAsPlainText(_ sender: Any?) {
        ghosttyPasteAsPlainTextActionHook?(self, sender)
        bmuxUnitTest_pasteAsPlainText(sender)
    }
}
...
func installCJKIMEInterpretKeyEventsSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.bmuxUnitTest_interpretKeyEvents(_:))
...
private func installGhosttyPasteActionSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.bmuxUnitTest_paste(_:))
    ...
    let plainTextSwizzledSelector = #selector(GhosttyNSView.bmuxUnitTest_pasteAsPlainText(_:))
...
// MARK: - Space release regression (Codex hold-to-talk in bmux)
...
final class GhosttyKeyEquivalentRegressionTests: XCTestCase {
    ...
    private func bmuxZshTerminalKeyboardResetSequence() throws -> Data {
        let repoRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let integrationPath = repoRoot
            .appendingPathComponent("Resources/shell-integration/bmux-zsh-integration.zsh")
            .path
        ...
        process.arguments = [
            ...
            if (( $+functions[_bmux_reset_terminal_keyboard_protocols] )); then
              _bmux_reset_terminal_keyboard_protocols
            fi
            """
        ]
        process.environment = [
            "BMUX_TEST_FORCE_KEYBOARD_RESET": "1"
        ]
        let output = Pipe()
    ...
    func testStaleKittyKeyboardAfterClearHistoryDoesNotEncodePlainLetterAsCSIU() throws {
        let captureReadyMarker = "BMUX_KBD_READY_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let captureMarker = "BMUX_KBD_HEX_\(UUID().uuidString.replacingOccurrences(of: "-", with: ""))"
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-kbd-capture-\(UUID().uuidString).py")
        let script = """
        import os
        import select
        import sys
        ...
        let keyboardResetData = try bmuxZshTerminalKeyboardResetSequence()
        XCTAssertEqual(
            keyboardResetData,
            Data("\u{1B}[>m\u{1B}[<8u".utf8),
            "bmuxZshTerminalKeyboardResetSequence must reset modifyOtherKeys and Kitty keyboard state"
        )
