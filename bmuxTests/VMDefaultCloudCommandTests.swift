...
extension CLINotifyProcessIntegrationRegressionTests {
    func testVMNewDefaultCreatesPinnedSSHDWorkspaceOverFreestyleSSH() throws {
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            case "vm.create":
                ...
                XCTAssertEqual(params["idempotency_key"] as? String, "bmux-default-freestyle-sshd-v1")
            ...
            case "workspace.create":
                ...
                XCTAssertTrue(decodedInitialCommand.contains("BMUX_CLOUD_RECONNECT_ATTEMPT"), decodedInitialCommand)
                XCTAssertFalse(decodedInitialCommand.contains("[bmux] ssh exited with status"), decodedInitialCommand)
            ...
            case "workspace.remote.configure":
                let params = payload["params"] as? [String: Any] ?? [:]
                XCTAssertEqual(params["workspace_id"] as? String, workspaceID)
                XCTAssertEqual(params["destination"] as? String, "\(vmID)+bmux@vm-ssh.freestyle.sh")
                ...
                XCTAssertTrue(decodedStartupCommand.contains("BMUX_CLOUD_RECONNECT_ATTEMPT"), decodedStartupCommand)
                XCTAssertFalse(decodedStartupCommand.contains("Cloud VM reconnecting"), decodedStartupCommand)
                XCTAssertFalse(decodedStartupCommand.contains("bmux_freestyle_notify_reconnect"), decodedStartupCommand)
                XCTAssertFalse(decodedStartupCommand.contains("[bmux] ssh exited with status"), decodedStartupCommand)
                ...
                XCTAssertEqual(params["persistent_daemon_slot"] as? String, "bmux-default-freestyle-sshd-v1")
                return self.v2Response(
                    id: id,
                    ok: true,
                    result: [
    ...
    func testVMNewExplicitFreestyleProviderCreatesSeparateDetachedVM() throws {
        ...
        let homeURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-vm-new-explicit-freestyle-\(UUID().uuidString)", isDirectory: true)
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            case "vm.create":
                ...
                XCTAssertEqual(params["provider"] as? String, "freestyle")
                XCTAssertNil(params["image"])
                XCTAssertNotEqual(params["idempotency_key"] as? String, "bmux-default-freestyle-sshd-v1")
                return self.v2Response(
                    id: id,
                    ok: true,
                    result: [
                        ...
                        "workspaces": [
                            [
                                ...
                                "remote": [
                                    ...
                                    "persistent_daemon_slot": "bmux-default-freestyle-sshd-v1",
                                ],
                            ],
                        ],
                    ]
    ...
    func testDefaultFreestyleSSHAttachScopesTmuxSessionToCallerSurface() throws {
        ...
        let tempDirectory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("bmux-fake-ssh-\(UUID().uuidString)", isDirectory: true)
        ...
        environment["BMUX_CLOUD_TMUX_SESSION"] = "bmux-cloud"
        environment["BMUX_FAKE_SSH_ARGS"] = capturedArgsPath
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        ...
        XCTAssertTrue(decodedRemoteBootstrap.contains("export BMUX_WORKSPACE_ID='\(workspaceID)'"), decodedRemoteBootstrap)
        XCTAssertTrue(decodedRemoteBootstrap.contains("export BMUX_SURFACE_ID='\(surfaceID)'"), decodedRemoteBootstrap)
        XCTAssertFalse(
        ...
        XCTAssertFalse(
            decodedRemoteBootstrap.contains("ln -sf /usr/local/bin/bmuxd-remote /usr/local/bin/bmux"),
            "interactive SSH attach must leave bmux CLI provisioning to vm.exec: \(decodedRemoteBootstrap)"
        )
        XCTAssertTrue(
            decodedRemoteBootstrap.contains("bmux-cloud-$bmux_cloud_tty_scope"),
            decodedRemoteBootstrap
        )
        XCTAssertTrue(
            decodedRemoteBootstrap.contains("unset BMUX_CLOUD_TMUX_SESSION"),
            decodedRemoteBootstrap
        )
        XCTAssertTrue(decodedRemoteBootstrap.contains("if [ \"$bmux_cloud_tty_scope\" = default ]; then"), decodedRemoteBootstrap)
        XCTAssertTrue(decodedRemoteBootstrap.contains("bmux_tmux_status=$?"), decodedRemoteBootstrap)
    ...
    func testDefaultFreestyleSSHAttachRejectedCredentialDoesNotExposePasswordPrompt() throws {
        ...
        IFS= read -r _bmux_password
        printf '\\nPermission denied, please try again.\\n' >&2
        printf "lease@vm-ssh.freestyle.sh's password: " >&2
        IFS= read -r _bmux_password_again
        exit 255
        """.write(toFile: fakeSSHPath, atomically: true, encoding: .utf8)
        chmod(fakeSSHPath, 0o755)
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_WORKSPACE_ID"] = "11111111-1111-1111-1111-111111111111"
        environment["BMUX_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["BMUX_CLOUD_TMUX_SESSION"] = "bmux-cloud"
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        environment["PATH"] = "\(tempDirectory.path):/usr/bin:/bin:/usr/sbin:/sbin"
    ...
    func testDefaultFreestyleSSHAttachRelaysAfterDelayedSuccessfulCredential() throws {
        ...
        XCTAssertTrue(result.stdout.contains("BMUX_DELAYED_RELAY_OK"), result.stdout + result.stderr)
    ...
    func testDefaultFreestyleSSHAttachFailsClosedWhenVMIsMissing() throws {
        ...
        XCTAssertTrue(result.stderr.contains("bmux vm new"), result.stderr)
        XCTAssertEqual(
            state.commands.compactMap { self.jsonObject($0)?["method"] as? String },
            ["vm.ssh_info"]
        )
    ...
    func testDefaultFreestyleSSHAttachReportsLocalServerRetryCountdown() throws {
            ...
            case "vm.ssh_info":
                ...
                return self.v2Response(
                    ...
                    error: [
                        ...
                        "message": """
                        Cannot reach the bmux Cloud VM service at http://localhost:3777.
        ...
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        environment["BMUX_DEFAULT_FREESTYLE_ATTACH_RETRY_LIMIT"] = "1"
        environment["BMUX_DEFAULT_FREESTYLE_ATTACH_RETRY_DELAY_SECONDS"] = "1"
        ...
        XCTAssertTrue(
            result.stderr.contains("Waiting for the local bmux web server at http://localhost:3777."),
            result.stderr
        )
    ...
    func testDefaultFreestyleSSHAttachHonorsGenericCloudRetryEnvironment() throws {
        ...
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        environment["BMUX_CLOUD_ATTACH_RETRY_LIMIT"] = "1"
        environment["BMUX_CLOUD_ATTACH_RETRY_DELAY_SECONDS"] = "0"
        environment["BMUX_DEFAULT_FREESTYLE_ATTACH_RETRY_LIMIT"] = "120"
        environment["BMUX_DEFAULT_FREESTYLE_ATTACH_RETRY_DELAY_SECONDS"] = "2"
    ...
    func testDefaultFreestyleSSHAttachHidesPersistentRetryLimitInCountdown() throws {
        ...
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        environment["BMUX_DEFAULT_FREESTYLE_ATTACH_RETRY_LIMIT"] = "86400"
        environment["BMUX_DEFAULT_FREESTYLE_ATTACH_RETRY_DELAY_SECONDS"] = "0.1"
