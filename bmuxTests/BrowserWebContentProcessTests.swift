...
struct BrowserWebContentProcessTests {
    private let recoveryURL = URL(string: "data:text/html,bmux-recovery")!
    ...
    func browserPanelInstallsWebAuthnBridgeWithoutExposingNativeHandler() async throws {
        ...
        let result = try await webView.evaluateJavaScript(
            """
            ({
              bridge: window.__bmuxWebAuthnBridgeInstalled === true,
              handler: !!(
                window.webkit &&
                window.webkit.messageHandlers &&
                window.webkit.messageHandlers.bmuxWebAuthn &&
                typeof window.webkit.messageHandlers.bmuxWebAuthn.postMessage === "function"
              )
            })
            """
        ) as? [String: Bool]
