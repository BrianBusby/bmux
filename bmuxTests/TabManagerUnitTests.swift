    ...
    func testDefaultFreestyleCloudSplitRepairsRawSSHStartupCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                terminalStartupCommand: "ssh -p 22 -tt 71smiccrg35sw9pydt8k+bmux@vm-ssh.freestyle.sh",
                preserveAfterTerminalExit: true,
    ...
    func testPaneCloseOnLastRemotePanelKeepsWorkspaceDisconnected() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: nil,
                localSocketPath: "/tmp/bmux-debug-test.sock",
                terminalStartupCommand: "bmux remote websocket"
            ),
            autoConnect: false
        )
    ...
    func testChildExitAfterPersistentAttachEndKeepsExitedSurfaceVisible() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayID: String(repeating: "a", count: 16),
                relayToken: String(repeating: "b", count: 64),
                localSocketPath: "/tmp/bmux-debug-attach-end-test.sock",
    ...
    func testChildExitOnSplitPersistentRemotePanelKeepsExitedSurfaceVisibleAndClearsOnlyThatPTYState() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayID: String(repeating: "a", count: 16),
                relayToken: String(repeating: "b", count: 64),
                localSocketPath: "/tmp/bmux-debug-split-test.sock",
                terminalStartupCommand: SSHPTYAttachStartupCommandBuilder.command(),
                preserveAfterTerminalExit: true,
...
final class TabManagerPullRequestProbeTests: XCTestCase {
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesIncludeMainAndMasterPanels() throws {
        ...
        let staleURL = try XCTUnwrap(URL(string: "https://github.com/manaflow-ai/bmux/pull/371"))
    ...
    func testSameDirectoryInitialGitMetadataProbesShareOneSnapshotRead() async throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-git-coalesced-probes-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testTrackedWorkspaceGitMetadataPollCandidatesExcludeDirectoriesWithoutResolvedGitMetadata() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "bmux-git-nonrepo-candidate-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testInheritedBackgroundWorkspaceFetchesGitBranchWithoutSelection() throws {
        ...
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "bmux-git-inherited-background-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testPeriodicWorkspaceGitMetadataRefreshUpdatesMainWorkspaceAfterCheckoutToFeatureBranch() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("bmux-git-main-refresh-\(UUID().uuidString)")
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
    ...
    func testPeriodicWorkspaceGitMetadataRefreshRestoresClearedBranchForStaleTerminal() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent(
            "bmux-git-stale-branch-refresh-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
    ...
    // BmuxProcessTests.resolvesCommandViaFallbackDirectoryOutsidePath when the
    // command runner was extracted into the BmuxProcess package.

    func testPeriodicWorkspaceGitMetadataRefreshClearsStalePullRequestAfterBranchReset() throws {
        let fileManager = FileManager.default
        let repoURL = fileManager.temporaryDirectory.appendingPathComponent("bmux-git-refresh-\(UUID().uuidString)")
        try fileManager.createDirectory(at: repoURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: repoURL) }
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 1052,
            label: "PR",
            url: try XCTUnwrap(URL(string: "https://github.com/manaflow-ai/bmux/pull/1052")),
            status: .open,
...
final class TabManagerCloseCurrentPanelTests: XCTestCase {
    ...
    func testCloseCurrentPanelHonorsWarnBeforeClosingTabDisabledFromBmuxJSON() throws {
    ...
    func testCloseCurrentPanelHonorsWarnBeforeClosingTabEnabledFromBmuxJSON() throws {
        try assertCloseCurrentPanelConfirmation(
    ...
    func testTabCloseButtonWarningHonorsBmuxJSON() throws {
    ...
    func testHideTabCloseButtonHonorsBmuxJSON() throws {
...
final class TabManagerSurfaceCreationTests: XCTestCase {
    ...
    func testNewBrowserSurfaceCanSelectBackgroundPaneWithoutTakingFocus() {
        ...
        guard let workspace = manager.selectedWorkspace,
              ...
              let url = URL(string: "file:///tmp/bmux-diff.html") else {
            XCTFail("Expected split setup to succeed")
            return
        }
    ...
    func testDuplicateBrowserPreservesDiffViewerChromeAndProxyBypass() throws {
        ...
        let url = try XCTUnwrap(URL(string: "http://127.0.0.1:49152/token/diff.html#bmux-diff-viewer"))
            ...
            .split(BmuxSplitDefinition(
                ...
                children: [
                    .pane(BmuxPaneDefinition(surfaces: [
                        BmuxSurfaceDefinition(type: .terminal, name: "Terminal A"),
                        BmuxSurfaceDefinition(type: .terminal, name: "Terminal B")
                    ])),
                    .pane(BmuxPaneDefinition(surfaces: [
                        BmuxSurfaceDefinition(type: .browser, name: "Browser", url: "https://example.com")
                    ]))
                ]
            )),
    ...
    func testEqualizeSplitsBalancesThreeSameAxisSiblingPanesIntoThirds() {
        ...
        workspace.applyCustomLayout(
            .split(BmuxSplitDefinition(
                ...
                children: [
                    .pane(BmuxPaneDefinition(surfaces: [
                        BmuxSurfaceDefinition(type: .terminal, name: "Left")
                    ])),
                    .split(BmuxSplitDefinition(
                        direction: .horizontal,
                        split: 0.8,
                        children: [
                            .pane(BmuxPaneDefinition(surfaces: [
                                BmuxSurfaceDefinition(type: .terminal, name: "Middle")
                            ])),
                            .pane(BmuxPaneDefinition(surfaces: [
                                BmuxSurfaceDefinition(type: .browser, name: "Right", url: "https://example.com")
                            ]))
                        ]
                    ))
                ]
            ...
            .split(BmuxSplitDefinition(
                ...
                children: [
                    .split(BmuxSplitDefinition(
                        ...
                        children: [
                            .pane(BmuxPaneDefinition(surfaces: [
                                BmuxSurfaceDefinition(type: .terminal, name: "Top Terminal")
                            ])),
                            .pane(BmuxPaneDefinition(surfaces: [
                                BmuxSurfaceDefinition(type: .terminal, name: "Bottom Terminal")
                            ]))
                        ]
                    )),
                    .pane(BmuxPaneDefinition(surfaces: [
                    ...
                    .split(BmuxSplitDefinition(
                        ...
                        children: [
                            .split(BmuxSplitDefinition(
                                ...
                                children: [
                                    .pane(BmuxPaneDefinition(surfaces: [
                                        BmuxSurfaceDefinition(type: .terminal, name: "Top Left")
                                    ])),
                                    .pane(BmuxPaneDefinition(surfaces: [
                                        BmuxSurfaceDefinition(type: .terminal, name: "Top Right")
                                    ]))
                                ]
                            )),
                            .pane(BmuxPaneDefinition(surfaces: [
                                BmuxSurfaceDefinition(type: .terminal, name: "Bottom")
                            ]))
                        ]
                    )),
                    .pane(BmuxPaneDefinition(surfaces: [
...
final class TabManagerReopenClosedBrowserFocusTests: XCTestCase {
    ...
    func testTemporaryDiffViewerTabCloseDoesNotStageRestoreSnapshot() throws {
        let workspace = Workspace()
        let diffViewerURL = try XCTUnwrap(URL(string: "http://127.0.0.1:49152/token/diff.html#bmux-diff-viewer"))
        guard let paneId = workspace.bonsplitController.focusedPaneId,
