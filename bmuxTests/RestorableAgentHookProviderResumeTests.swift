...
extension SocketListenerAcceptPolicyTests {
    ...
    func testRovoDevResumeCommandUsesRestoreAndPreservesYolo() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                environment: [
                    ...
                    "BMUX_ROVODEV_SESSIONS_DIR": "/tmp/rovo sessions"
                ],
                capturedAt: 123,
                source: "process"
            )
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/tmp/rovo repo' 2>/dev/null || [ ! -d '/tmp/rovo repo' ] && 'env' 'BMUX_ROVODEV_SESSIONS_DIR=/tmp/rovo sessions' '/opt/homebrew/bin/acli' 'rovodev' 'run' '--restore' 'session with space' '--yolo'"
        )
    }
    ...
    func testAdditionalHookAgentResumeCommandsUseVerifiedCLIResumeFlags() {
        ...
        let copilot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "copilot",
                executablePath: "/tmp/bmux-agent-upstreams/copilot-install/bin/copilot",
                arguments: [
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/copilot repo' 2>/dev/null || [ ! -d '/tmp/copilot repo' ] && 'env' 'COPILOT_HOME=/tmp/copilot home' '/tmp/bmux-agent-upstreams/copilot-install/bin/copilot' '--resume' 'copilot-session-123' '--model' 'gpt-5.4' '--allow-all-tools'"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            kiro.resumeCommand,
            "cd -- '/tmp/kiro repo' 2>/dev/null || [ ! -d '/tmp/kiro repo' ] && 'env' 'KIRO_HOME=/tmp/kiro home' '/Users/example/.cargo/bin/kiro-cli' 'chat' '--resume-id' 'kiro-session-123' '--agent' 'bmux' '--trust-tools' 'fs_read,fs_write'"
        )
        XCTAssertEqual(
            grok.resumeCommand,
