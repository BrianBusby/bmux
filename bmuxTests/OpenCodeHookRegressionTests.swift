...
final class OpenCodeHookRegressionTests: XCTestCase {
    ...
    func testOpenCodeInstallHooksIsIdempotentForLegacySetupAlias() throws {
        ...
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("bmux-opencode-hooks-\(UUID().uuidString)", isDirectory: true)
        ...
        try #"{"plugin":["other-plugin","./plugins/bmux-session.js"]}"#.write(to: configURL, atomically: true, encoding: .utf8)
        ...
        let pluginURL = configDir.appendingPathComponent("plugins", isDirectory: true).appendingPathComponent("bmux-session.js", isDirectory: false)
        let pluginSource = try String(contentsOf: pluginURL, encoding: .utf8)
        XCTAssertTrue(pluginSource.contains("bmux-opencode-session-plugin-marker"))
        ...
        XCTAssertFalse(secondResult.stdout.contains("Will write OpenCode bmux plugin"), secondResult.stdout)
        XCTAssertTrue(secondResult.stdout.contains("OpenCode hooks already up to date"), secondResult.stdout)
        XCTAssertTrue(try String(contentsOf: configDir.appendingPathComponent("plugins/bmux-feed.js"), encoding: .utf8).contains("bmux-feed-plugin-marker"))

        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: try Data(contentsOf: configURL), options: []) as? [String: Any])
        XCTAssertEqual(try XCTUnwrap(json["plugin"] as? [String]), ["other-plugin", "./plugins/bmux-session.js"])
    }
