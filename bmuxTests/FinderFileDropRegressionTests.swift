...
final class FinderFileDropRegressionTests: XCTestCase {
    ...
    func testLegacyFinderFilenameDropPlanInsertsEscapedLocalPath() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("bmux-test-legacy-filename-drop-\(UUID().uuidString)"))
    ...
    func testImageFileURLDropInsertsOriginalLocalImagePaths() throws {
        let imageDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux image file drop \(UUID().uuidString)")
        try FileManager.default.createDirectory(at: imageDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: imageDirectory) }

        let firstURL = imageDirectory.appendingPathComponent("bmux ssh 2.png")
        let secondURL = imageDirectory.appendingPathComponent("bmux ssh.png")
        try make1x1PNG(color: .systemRed).write(to: firstURL)
        try make1x1PNG(color: .systemGreen).write(to: secondURL)

        let pasteboard = NSPasteboard(name: .init("bmux-test-image-file-url-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageFileURLDropUploadsOriginalFilesForRemoteTerminal() throws {
        let imageDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux remote image file drop \(UUID().uuidString)")
        try FileManager.default.createDirectory(at: imageDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: imageDirectory) }
        ...
        let pasteboard = NSPasteboard(name: .init("bmux-test-remote-image-file-url-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
        XCTAssertTrue(pasteboard.writeObjects([firstURL as NSURL, secondURL as NSURL]))
    ...
    func testImagePasteboardDropMaterializesEveryImageForLocalInsertion() throws {
        let pasteboard = NSPasteboard(name: .init("bmux-test-multi-image-local-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
        let items = try [
    ...
    func testImagePasteboardItemWithDirectImageAndRTFDAttachmentMaterializesOnce() throws {
        let pasteboard = NSPasteboard(name: .init("bmux-test-image-rtfd-duplicate-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImagePasteboardItemTIFFDropNormalizesToPNGForLocalInsertion() throws {
        let pasteboard = NSPasteboard(name: .init("bmux-test-image-tiff-normalization-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImagePasteboardDropMaterializesEveryImageForRemoteUpload() throws {
        let pasteboard = NSPasteboard(name: .init("bmux-test-multi-image-remote-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
        let items = try [
            makeImagePasteboardItem(color: .systemRed),
            makeImagePasteboardItem(color: .systemGreen),
    ...
    func testFileExplorerPathInsertionEscapesMultiplePathsLikeTerminalDrop() {
        let paths = [
            "/tmp/bmux path/one file.txt",
            "/tmp/bmux path/quote's file.txt"
        ]
    ...
    func testFileURLTextInsertionIsExtensionAgnostic() {
        let urls = [
            URL(fileURLWithPath: "/tmp/bmux drop/image.png"),
            URL(fileURLWithPath: "/tmp/bmux drop/report.pdf"),
            URL(fileURLWithPath: "/tmp/bmux drop/movie.mov"),
            URL(fileURLWithPath: "/tmp/bmux drop/archive.zip")
        ]
    ...
    func testFilePreviewTransferRoutesToTextEvenWhenTargetPasteboardOmitsFileURLType() throws {
        let filePath = "/tmp/bmux drop/from image pane.png"
        let dragId = UUID()
        ...
        let pasteboard = NSPasteboard(name: .init("bmux-test-file-preview-transfer-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testFileExplorerRelativePathInsertionStandardizesMacOSSymlinkedRoots() {
        XCTAssertEqual(
            FileExplorerTerminalPathInsertion.relativePath(
                for: "/private/tmp/bmux-project/Sources/App.swift",
                rootPath: "/tmp/bmux-project"
            ),
            "Sources/App.swift"
        )
    }
