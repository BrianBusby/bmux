...
final class TaskManagerResourcesTests: XCTestCase {
    func testGrokCodingAgentDefinitionMatchesSymlinkLaunch() throws {
        let definition = try XCTUnwrap(BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
            processName: "grok",
    ...
    func testParsesTypedIntPIDArrayFromSummaryPayload() {
        ...
        let resources = BmuxTaskManagerResources(payload)
    ...
    func testSortOrderSortsSiblingRowsByCPUWhilePreservingHierarchy() {
        ...
        let sortedRows = BmuxTaskManagerSortOrder.defaultOrder.sortedRows(rows)
    ...
    func testSortOrderUsesAscendingNameSortWhenNameColumnIsSelected() {
        ...
        let sortedRows = BmuxTaskManagerSortOrder(
            column: .name,
            direction: .ascending
        ).sortedRows(rows)
    ...
    func testSortOrderSortsMemoryAndProcessColumnsDescending() {
        ...
        let memorySortedRows = BmuxTaskManagerSortOrder(
            column: .memory,
            direction: .descending
        ).sortedRows(rows)
        let processSortedRows = BmuxTaskManagerSortOrder(
            column: .processes,
            direction: .descending
        ).sortedRows(rows)
    ...
    func testSortOrderTogglesCurrentColumnAndUsesMetricDefaultsForNewColumns() {
        let cpuDescending = BmuxTaskManagerSortOrder.defaultOrder
        ...
        XCTAssertEqual(cpuAscending, BmuxTaskManagerSortOrder(column: .cpu, direction: .ascending))
        XCTAssertEqual(memoryDescending, BmuxTaskManagerSortOrder(column: .memory, direction: .descending))
        XCTAssertEqual(nameAscending, BmuxTaskManagerSortOrder(column: .name, direction: .ascending))
    }

    func testSnapshotBuildsProgramAggregateRowsForRepeatedProcessNames() throws {
        let snapshot = BmuxTaskManagerSnapshot(
            rows: [
                taskManagerRow(
                    "node-101-a",
                    kind: .process,
    ...
    func testSnapshotParsesProgramTotalsFromTopPayloadWithoutProcessRows() throws {
        let snapshot = BmuxTaskManagerSnapshot(payload: [
    ...
    func testSnapshotOnlyCountsAsLoadedAfterSamplingOrRowsArrive() {
        XCTAssertFalse(BmuxTaskManagerSnapshot.empty.hasLoadedResourceUsage)

        let sampledSnapshot = BmuxTaskManagerSnapshot(
            rows: [],
            total: .zero,
    ...
    func testCodingAgentMatcherUsesSupportedAgentNamesAndLaunchMetadata() {
        XCTAssertEqual(
            BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
                processName: "node",
                processPath: nil,
        ...
        XCTAssertTrue(BmuxTaskManagerCodingAgentDefinition.shouldReadArguments(
            processName: "2.1.140",
        ...
        XCTAssertNil(BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
            processName: "2.1.140",
            processPath: nil,
            arguments: [],
            environment: [:]
        ...
        XCTAssertEqual(
            BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
                ...
                environment: ["BMUX_AGENT_LAUNCH_KIND": "claudeTeams"]
            )?.id,
            "claude"
        )
        XCTAssertEqual(
            BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
                ...
                environment: ["BMUX_AGENT_LAUNCH_KIND": "omo"]
            )?.id,
            "opencode"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
                ...
                processPath: nil,
                arguments: ["node", "agent.js"],
                environment: ["BMUX_AGENT_LAUNCH_KIND": "omx"]
            )?.id,
            "codex"
        )
        XCTAssertNil(BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
    ...
    private func resourceSummary() -> BmuxTopResourceSummary {
    ...
    private func taskManagerRow(
        _ id: String,
        kind: BmuxTaskManagerRow.Kind = .workspace,
        level: Int,
        title: String? = nil,
        cpuPercent: Double = 0,
