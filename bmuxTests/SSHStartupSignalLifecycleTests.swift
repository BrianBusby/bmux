...
extension CLINotifyProcessIntegrationRegressionTests {
    func testSSHPaneCloseSignalDoesNotReportSessionEndToSharedTransport() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-pane-close-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        ...
        try writeShellFile(at: fakeCLI, lines: [
            "#!/bin/sh",
            "printf '%s\\n' \"$*\" >> \"${BMUX_TEST_SESSION_END_LOG}\"",
        ])
        try writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "kill -\"${BMUX_TEST_SIGNAL:?}\" \"${BMUX_SSH_STARTUP_PID:-$PPID}\"",
            "sleep 0.1",
            "exit 0",
        ])
        ...
        for signal in ["HUP", "INT", "TERM"] {
            ...
            environment["BMUX_BUNDLED_CLI_PATH"] = fakeCLI.path
            environment["BMUX_SOCKET_PATH"] = "/tmp/bmux-debug-test.sock"
            ...
            environment["BMUX_TEST_SESSION_END_LOG"] = logFile.path
            environment["BMUX_TEST_SIGNAL"] = signal
    ...
    func testSSHPaneCloseSignalDoesNotTerminateWrappedSSHChild() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-pane-close-child-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "trap 'printf \"%s\\n\" child-hup >> \"${BMUX_TEST_CHILD_SIGNAL_LOG}\"; exit 0' HUP",
            "trap 'printf \"%s\\n\" child-int >> \"${BMUX_TEST_CHILD_SIGNAL_LOG}\"; exit 0' INT",
            "trap 'printf \"%s\\n\" child-term >> \"${BMUX_TEST_CHILD_SIGNAL_LOG}\"; exit 0' TERM",
            "printf '%s\\n' child-started >> \"${BMUX_TEST_CHILD_SIGNAL_LOG}\"",
            "kill -\"${BMUX_TEST_SIGNAL:?}\" \"${BMUX_SSH_STARTUP_PID:-$PPID}\"",
            "sleep 0.2",
            "printf '%s\\n' child-completed >> \"${BMUX_TEST_CHILD_SIGNAL_LOG}\"",
            "exit 0",
        ])
        try fileManager.setAttributes([.posixPermissions: 0o700], ofItemAtPath: fakeCLI.path)
        try fileManager.setAttributes([.posixPermissions: 0o700], ofItemAtPath: fakeSSH.path)
        ...
        for signal in ["HUP", "INT", "TERM"] {
            ...
            environment["BMUX_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
            environment["BMUX_TEST_SESSION_END_LOG"] = logFile.path
            environment["BMUX_TEST_CHILD_SIGNAL_LOG"] = childSignalLog.path
            environment["BMUX_TEST_SIGNAL"] = signal
    ...
    func testSSHStartupRetriesTransientSSHExitBeforeReportingSessionEnd() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-reconnect-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "count=0",
            "if [ -r \"${BMUX_TEST_ATTEMPT_FILE}\" ]; then count=$(cat \"${BMUX_TEST_ATTEMPT_FILE}\"); fi",
            "count=$((count + 1))",
            "printf '%s\\n' \"$count\" > \"${BMUX_TEST_ATTEMPT_FILE}\"",
        ...
        environment["BMUX_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["BMUX_TEST_SESSION_END_LOG"] = logFile.path
        environment["BMUX_TEST_ATTEMPT_FILE"] = attemptFile.path
        ...
        // The regular `bmux ssh` bootstrap path runs one SSH command to install
    ...
    func testSSHStartupRemovesStaleBmuxControlSocketBeforeLaunchingPaneSSH() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-stale-control-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh.log")
        let staleControlPath = URL(fileURLWithPath: "/tmp", isDirectory: true)
            .appendingPathComponent("bmux-ssh-\(getuid())-\(UUID().uuidString.prefix(8)).sock")
        ...
        environment["BMUX_WORKSPACE_ID"] = "11111111-1111-1111-1111-111111111111"
        environment["BMUX_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["BMUX_TEST_CONTROL_PATH"] = staleControlPath.path
        environment["BMUX_TEST_SESSION_END_LOG"] = root.appendingPathComponent("ssh-session-end.log").path
        environment["BMUX_TEST_SSH_LOG"] = logFile.path
        environment["BMUX_SSH_RECONNECT_DELAY_SECONDS"] = "0"
    ...
    func testSSHStartupStopsAtConfiguredReconnectLimit() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-retry-limit-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        let attemptFile = root.appendingPathComponent("ssh-attempts.txt")
        ...
        environment["BMUX_TEST_ATTEMPT_FILE"] = attemptFile.path
        environment["BMUX_SSH_RECONNECT_DELAY_SECONDS"] = "0"
        environment["BMUX_SSH_RECONNECT_LIMIT"] = "2"
    ...
    func testSSHStartupDoesNotRetryNonTransientSSHExit() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-no-retry-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        let attemptFile = root.appendingPathComponent("ssh-attempts.txt")
    ...
    func testSSHSignalDerivedChildExitReportsSessionEnd() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-child-signal-exit-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
    ...
    func testSSHSignalDuringReconnectDelayDoesNotStartAnotherSSH() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-signal-during-reconnect-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        let attemptFile = root.appendingPathComponent("ssh-attempts.txt")
        ...
        try writeShellFile(at: fakeSSH, lines: [
            ...
            "printf '%s\\n' \"$count\" > \"${BMUX_TEST_ATTEMPT_FILE}\"",
            "if [ \"$count\" -eq 1 ]; then",
            "  ( sleep 0.2; kill -TERM \"${BMUX_SSH_STARTUP_PID:?}\" ) &",
            "  exit 255",
            "fi",
            "exit 0",
        ])
        ...
        environment["BMUX_TEST_SESSION_END_LOG"] = logFile.path
        environment["BMUX_TEST_ATTEMPT_FILE"] = attemptFile.path
        environment["BMUX_SSH_RECONNECT_DELAY_SECONDS"] = "2"
        environment["BMUX_SSH_RECONNECT_LIMIT"] = "2"
    ...
    func testSSHStartupPrintsFinalErrorBannerWhenStderrIsCaptured() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-error-banner-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        let fakeSSH = root.appendingPathComponent("ssh")
        let logFile = root.appendingPathComponent("ssh-session-end.log")
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 1, result.stderr)
        XCTAssertTrue(result.stderr.contains("[bmux] ssh exited with status 1."), result.stderr)
        XCTAssertTrue(result.stderr.contains("[bmux] press Enter to close this pane."), result.stderr)
    }

    func testSSHStartupForwardsStdinToBackgroundedSSH() throws {
        // Regression test for bmux ssh sessions where output flowed back from
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-stdin-forward-\(UUID().uuidString)", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("bmux")
        let fakeSSH = root.appendingPathComponent("ssh")
        ...
        environment["BMUX_WORKSPACE_ID"] = "11111111-1111-1111-1111-111111111111"
        environment["BMUX_SURFACE_ID"] = "22222222-2222-2222-2222-222222222222"
        environment["BMUX_TEST_SESSION_END_LOG"] = sessionEndLog.path
        environment["BMUX_TEST_STDIN_LOG"] = stdinCapture.path
        environment["BMUX_SSH_RECONNECT_DELAY_SECONDS"] = "0"
