...
final class AgentSessionAutoResumeSettingsTests: XCTestCase {
    func testDefaultsKeyAndNotificationOnFlip() throws {
        let suiteName = "bmux-agent-session-auto-resume-\(UUID().uuidString)"
    ...
    /// (wasAgentRunning == false), bmux must NOT inject the resume command on restore.
    ...
    /// (wasAgentRunning == true), bmux MUST auto-resume as before.
    @MainActor
    func testAgentRunningAtSnapshotAutoResumesWhenSettingEnabled() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            // Simulate: agent was still running when bmux quit
    ...
    func testRemoteWorkspaceAutoResumeKeepsRemoteStartupCommand() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let source = Workspace()
            let remoteCommand = "ssh bmux-macmini"
            let expectedRestoredRemoteCommand = "ssh -tt bmux-macmini"
            source.configureRemoteConnection(
                WorkspaceRemoteConfiguration(
                    ...
                    localSocketPath: "/tmp/bmux-auto-resume-remote.sock",
            ...
            XCTAssertFalse(input.contains("bmux-agent-resume"), input)
        ...
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            source.configureRemoteConnection(
                WorkspaceRemoteConfiguration(
                    ...
                    localSocketPath: "/tmp/bmux-auto-resume-long-remote.sock",
                    terminalStartupCommand: remoteCommand
                ),
                autoConnect: false
            )
            ...
            XCTAssertFalse(restoredInput.contains("bmux-agent-resume"), restoredInput)
            XCTAssertEqual(
                restored.restoredAgentResumeStatesByPanelId[restoredPanelId],
                .awaitingAutoResumeCommand
            )
    ...
    func testResumeLauncherReturnsToLaunchCwdAfterAgentExits() {
        ...
        let exec = "exec -l \"$_bmux_resume_shell\""
    ...
    private func assertAgentAutoResumeUsesStartupCommand(
        ...
        XCTAssertTrue(script.contains("BMUX_SHELL_INTEGRATION_DIR"), script, file: file, line: line)
        XCTAssertTrue(script.contains("BMUX_ZSH_ZDOTDIR"), script, file: file, line: line)
        XCTAssertTrue(script.contains("\"$_bmux_resume_shell\" -lic"), script, file: file, line: line)
        XCTAssertTrue(script.contains("csh|tcsh) \"$_bmux_resume_shell\" -c"), script, file: file, line: line)
        XCTAssertTrue(script.contains("exec -l \"$_bmux_resume_shell\""), script, file: file, line: line)
    }
    ...
    private func makeRestorableAgentIndex(
        ...
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-auto-resume-\(UUID().uuidString)", isDirectory: true)
        let previousHookStateDir = getenv("BMUX_AGENT_HOOK_STATE_DIR").map { String(cString: $0) }
        setenv("BMUX_AGENT_HOOK_STATE_DIR", home.appendingPathComponent("hook-state", isDirectory: true).path, 1)
        defer {
...
final class TerminalCopyOnSelectSettingsTests: XCTestCase {
    func testDefaultsNotificationAndGhosttyConfigMapping() throws {
        let suiteName = "bmux-terminal-copy-on-select-\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
