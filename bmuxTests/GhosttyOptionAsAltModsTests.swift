...
/// Regression coverage for https://github.com/manaflow-ai/bmux/issues/5993:
/// bmux ignored `macos-option-as-alt` left/right and captured Option before
...
/// bmux sends. If bmux maps both physical Option keys to the same generic
...
@Suite struct GhosttyOptionAsAltModsTests {
    ...
    @Test func rightOptionCarriesAltAndAltRightSideBit() {
        ...
        let mods = bmuxGhosttyModsFromFlags(modifierFlagsRawValue: raw)
    ...
    @Test func mouseModsNeverCarrySideBits() {
        ...
        let mods = bmuxGhosttyMouseModsFromFlags(modifierFlagsRawValue: raw)
        #expect(mods.rawValue & GHOSTTY_MODS_ALT.rawValue != 0)
        #expect(mods.rawValue & GHOSTTY_MODS_SHIFT.rawValue != 0)
        #expect(mods.rawValue & GHOSTTY_MODS_ALT_RIGHT.rawValue == 0)
        #expect(mods.rawValue & GHOSTTY_MODS_SHIFT_RIGHT.rawValue == 0)
    ...
    @Test func mouseOverLinkActionDecodesURLAndClearsEmptyHover() {
        var bytes = Array("https://example.com/path?q=bmux".utf8CString)
        ...
        #expect(decoded == "https://example.com/path?q=bmux")
    ...
    @Test func translationFlagsDropOptionWhenGhosttyStripsAlt() {
        ...
        let translated = bmuxTranslationModifierFlags(
            original: [.option],
