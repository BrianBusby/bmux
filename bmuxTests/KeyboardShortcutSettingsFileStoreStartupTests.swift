...
import enum BmuxSettings.BrowserSearchEngine
import struct BmuxSettings.BrowserSearchSettingsStore
...
final class KeyboardShortcutSettingsFileStoreStartupTests: XCTestCase {
    ...
    func testSettingsFileStoreDoesNotReachTerminalReloadThroughManagedAppearanceReplay() throws {
        ...
        try preservingDefaults(keys: [key, settingsFileBackupsDefaultsKey, importedManagedDefaultsKey]) {
            ...
            XCTAssertTrue(
                ...
                "BmuxSettingsFileStore must not synchronously route managed appearance replay into Ghostty reloadConfiguration"
            )
        }
    }
    ...
    func testConfirmQuitImportsEnumFromBmuxJSON() throws {
        let defaults = UserDefaults.standard
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-settings-startup-\(UUID().uuidString)",
            isDirectory: true
        )
