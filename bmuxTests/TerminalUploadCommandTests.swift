...
@Suite struct TerminalUploadCommandTests {
    ...
    // MARK: - Decoding (strict, whole-array) — the SettingCodable path used by bmux.json
    ...
    @Test func decodeRejectsScalarElementWithoutCrashing() {
        ...
        #expect(decode(#"["my-upload $BMUX_UPLOAD_LOCAL_PATH"]"#) == nil)
    ...
    @Test func emittedTextFallsBackWhenOutputIsOnlyControlCharacters() {
        ...
        let emitted = TerminalUploadCommand.emittedText(
            ...
            remotePath: "/tmp/bmux-drop-x.png"
        )
        #expect(emitted.contains("bmux-drop"))
    }
    ...
    @Test func environmentCarriesContext() {
        ...
        #expect(env["BMUX_UPLOAD_LOCAL_PATH"] == "/l")
        #expect(env["BMUX_UPLOAD_REMOTE_PATH"] == "/r")
        #expect(env["BMUX_UPLOAD_DESTINATION"] == "me@h")
        #expect(env["BMUX_UPLOAD_PORT"] == "2222")
        #expect(env["BMUX_UPLOAD_IDENTITY_FILE"] == "/id")
        #expect(env["BMUX_UPLOAD_SSH_OPTIONS"] == "A=B\nC=D")
    }
    ...
    @Test func environmentOmitsAbsentOptionalFields() {
        ...
        #expect(env["BMUX_UPLOAD_PORT"] == nil)
        #expect(env["BMUX_UPLOAD_IDENTITY_FILE"] == nil)
        #expect(env["BMUX_UPLOAD_SSH_OPTIONS"] == nil)
    }
}
...
@Suite struct TerminalCustomUploadRunnerTests {
    ...
    @Test func perFileStdoutJoinedWithSpaces() {
        let result = runner { _, env, _, _ in
            (0, "OUT:\(env["BMUX_UPLOAD_LOCAL_PATH"] ?? "")", "")
        }.runSync(
    ...
    @Test func emptyStdoutEmitsEscapedRemotePath() {
        ...
        // Falls back to the bmux-chosen remote path (escaped).
        #expect(text.contains("bmux-drop"))
    }
