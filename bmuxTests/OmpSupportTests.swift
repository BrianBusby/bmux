...
struct OmpSupportTests {
    ...
    @Test func ompRegistriesUsePiIconAsset() throws {
        let taskManagerDefinition = try #require(
            BmuxTaskManagerCodingAgentDefinition.builtIns.first { $0.id == "omp" }
        )
        #expect(taskManagerDefinition.assetName == "AgentIcons/Pi")
        #expect(BmuxVaultAgentRegistration.builtInOmp.iconAssetName == "AgentIcons/Pi")
    }
    ...
    @Test func directProcessDetectionUsesExplicitSessionSelectorsBeforeLatestFallback() throws {
        ...
        for selector in selectors {
            let root = try Self.makeTemporaryDirectory(prefix: "bmux-omp-explicit-")
    ...
    @Test func directProcessDetectionFallsBackToPartialSessionFileMatch() throws {
        let root = try Self.makeTemporaryDirectory(prefix: "bmux-omp-partial-session-")
        defer { try? FileManager.default.removeItem(at: root) }
        let workspace = root.appendingPathComponent("repo", isDirectory: true)
        let sessionsRoot = root.appendingPathComponent("sessions", isDirectory: true)
        let projectDirectory = try #require(PiSessionLocator.projectDirectoryName(for: workspace.path))
    ...
    @Test func directProcessDetectionUsesOmpAgentDirectorySessionsWhenNoSessionDirectoryIsSet() throws {
        let root = try Self.makeTemporaryDirectory(prefix: "bmux-omp-agent-dir-")
        defer { try? FileManager.default.removeItem(at: root) }
        let workspace = root.appendingPathComponent("repo", isDirectory: true)
    ...
    @Test func directProcessDetectionUsesPiConfigDirectoryAgentSessionsWhenAgentDirectoryIsUnset() throws {
        let root = try Self.makeTemporaryDirectory(prefix: "bmux-omp-config-dir-")
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func directProcessDetectionPreservesCustomSessionDirectoryBeforeOmpEnvironment() throws {
        let root = try Self.makeTemporaryDirectory(prefix: "bmux-omp-custom-session-dir-")
        defer { try? FileManager.default.removeItem(at: root) }
        let workspace = root.appendingPathComponent("repo", isDirectory: true)
        ...
        let registration = BmuxVaultAgentRegistration(
            id: "omp",
            name: "OMP",
            detect: BmuxVaultAgentDetectRule(processName: "omp"),
    ...
    @Test func vaultDetectsBunInvokedOmpPackage() throws {
        let root = try Self.makeTemporaryDirectory(prefix: "bmux-omp-bun-vault-")
        defer { try? FileManager.default.removeItem(at: root) }
        let workspace = root.appendingPathComponent("repo", isDirectory: true)
        let sessionsRoot = root.appendingPathComponent("sessions", isDirectory: true)
        let projectDirectory = try #require(PiSessionLocator.projectDirectoryName(for: workspace.path))
    ...
    @Test func hostedOmpIgnoresRuntimePreloadFlagsBeforeAgentScript() throws {
        let root = try Self.makeTemporaryDirectory(prefix: "bmux-omp-hosted-runtime-preload-")
        defer { try? FileManager.default.removeItem(at: root) }
        let workspace = root.appendingPathComponent("repo", isDirectory: true)
        let sessionsRoot = root.appendingPathComponent("sessions", isDirectory: true)
        let projectDirectory = try #require(PiSessionLocator.projectDirectoryName(for: workspace.path))
    ...
    @Test func hostedOmpParsesSessionSelectorsAfterAgentScript() throws {
        let root = try Self.makeTemporaryDirectory(prefix: "bmux-omp-hosted-runtime-session-")
        defer { try? FileManager.default.removeItem(at: root) }
        let workspace = root.appendingPathComponent("repo", isDirectory: true)
        let sessionsRoot = root.appendingPathComponent("sessions", isDirectory: true)
        let projectDirectory = try #require(PiSessionLocator.projectDirectoryName(for: workspace.path))
    ...
    @Test func taskManagerClassifiesOmpBeforeLegacyPiPackageNeedles() throws {
        let direct = try #require(BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
            processName: "omp",
        ...
        let hostedOmp = try #require(BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
            processName: "bun",
            processPath: "/opt/homebrew/bin/bun",
            arguments: [
                "/opt/homebrew/bin/bun",
        ...
        let legacyPi = try #require(BmuxTaskManagerCodingAgentDefinition.matchingDefinition(
            processName: "bun",
            processPath: "/opt/homebrew/bin/bun",
            arguments: [
                "/opt/homebrew/bin/bun",
    ...
    private static func detectedOmpSnapshot(
        ...
        registration: BmuxVaultAgentRegistration = .builtInOmp
        ...
        return RestorableAgentSessionIndex.processDetectedSnapshots(
            registry: BmuxVaultAgentRegistry(registrations: [registration]),
            fileManager: FileManager.default,
            processSnapshot: processSnapshot,
            capturedAt: 42,
            processArgumentsProvider: { requestedProcessId in
                guard requestedProcessId == processId else { return nil }
                return BmuxTopProcessArguments(arguments: arguments, environment: environment)
            }
        )[panelKey]?.snapshot
    }
