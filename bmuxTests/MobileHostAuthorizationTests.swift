...
struct MobileHostAuthorizationTests {
    ...
    @Test func testDebugStackAuthTokenPolicyRequiresConfiguredToken() {
        ...
        #expect(MobileHostDevStackAuthPolicy.authorize(
            providedToken: " bmux-dev-token ",
            acceptedToken: "bmux-dev-token"
        ))
    }
    @Test func testDebugConfiguredStackAuthTokenAuthorizesBroadWorkspaceList() async {
        let service = MobileHostService.shared
        service.debugConfigureAcceptedStackAuthTokenForTesting("bmux-dev-token")
        defer {
        ...
        let request = MobileHostRPCRequest(
            ...
            auth: MobileHostRPCAuth(
                attachToken: nil,
                stackAccessToken: "bmux-dev-token"
            )
        )
    ...
    @Test func testLiveAuthorizationRejectsWorkspaceScopedAttachTokenForMacScopedMutations() async throws {
        ...
        for (method, params) in [
            ...
            let request = MobileHostRPCRequest(
                ...
                method: method,
                params: params,
                auth: MobileHostRPCAuth(attachToken: attachToken, stackAccessToken: "bmux-dev-token")
            )
            let result = await service.debugAuthorizationError(for: request)
            guard case let .failure(error) = result else {
...
private final class MobileHostStartedTestSocket: @unchecked Sendable {
    ...
    init() throws {
        let queue = DispatchQueue(label: "dev.bmux.mobile-host-started-test-socket")
