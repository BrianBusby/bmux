...
struct WindowTitleTemplateTests {
    private let backupsDefaultsKey = "bmux.settingsFile.backups.v1"
    ...
    @Test func resolvesWindowPlaceholdersAndPreservesUnknownPlaceholders() throws {
        ...
        let template = WindowTitleTemplate(
            rawValue: "[bmux:{windowToken}] {activeWorkspace} {activeDirectory} {windowId} {defaultTitle} {appName} {unknown}"
        )
        ...
        #expect(resolved == "[bmux:01234567] Build /tmp/project 01234567-89ab-cdef-0123-456789abcdef Fallback bmux {unknown}")
    }
    ...
    @Test func settingsFileStoreAppliesAppWindowTitleTemplate() throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-window-title-template-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(defaults.string(forKey: WindowTitleTemplate.userDefaultsKey) == "[bmux:{windowToken}] {activeWorkspace}")
    }
    ...
    @Test func settingsFileStoreAppliesWorkspaceAutoNamingAutomationSetting() throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-workspace-auto-naming-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directoryURL) }
    ...
    @Test func settingsFileStoreAppliesAutoNamingAgentAutomationSetting() throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-auto-naming-agent-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directoryURL) }
    ...
    @Test func selectedWorkspaceDirectoryChangeRefreshesActiveDirectoryTitle() throws {
        ...
        defaults.set("[bmux:{windowToken}] {activeDirectory}", forKey: WindowTitleTemplate.userDefaultsKey)
        ...
        #expect(window.title == "[bmux:01234567] /tmp/old")
        ...
        #expect(window.title == "[bmux:01234567] /tmp/new")
    }
    ...
    @Test func remoteWorkspaceTitleUsesReportedDirectoryForActiveDirectoryTemplate() throws {
        ...
        manager.refreshWindowTitle()
        #expect(window.title == "[bmux:01234567] \(localDirectory)")
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/bmux-issue-7268-window-title.sock",
                terminalStartupCommand: sshCommand
            ),
            autoConnect: false
        )
        #expect(window.title == "[bmux:01234567]")
        ...
        #expect(window.title == "[bmux:01234567] \(remoteDirectory)")
    ...
    @Test func remoteWorkspaceTitleUsesFocusedLocalTerminalDirectoryForActiveDirectoryTemplate() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayID: "relay-\(UUID().uuidString)",
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/bmux-issue-7268-local-window-title.sock",
        ...
        manager.refreshWindowTitle()
        #expect(window.title == "[bmux:01234567] \(localTerminalDirectory)")
    ...
    @Test func remoteWorkspacePresentedDirectoryFallsBackToTrustedRemoteWhenAgentFocused() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayID: "relay-\(UUID().uuidString)",
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/bmux-issue-7268-agent-focus.sock",
                terminalStartupCommand: sshCommand
            ),
            autoConnect: false
        )
    ...
    private func sshRemoteConfiguration(command: String) -> WorkspaceRemoteConfiguration {
        WorkspaceRemoteConfiguration(
            ...
            relayID: "relay-\(UUID().uuidString)",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/bmux-window-title-\(UUID().uuidString).sock",
            terminalStartupCommand: command
        )
    }

    private func isolatedDefaults() throws -> UserDefaults {
        let suiteName = "bmux.WindowTitleTemplateTests.\(UUID().uuidString)"
