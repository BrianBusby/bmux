...
struct BrowserHiddenWebViewDiscardMediaPlaybackTests {
    /// Regression coverage for https://github.com/manaflow-ai/bmux/issues/5409:
    ...
    /// so the memory bound from https://github.com/manaflow-ai/bmux/issues/4539
    /// is preserved.
...
final class BrowserHiddenWebViewDiscardManagerTests: XCTestCase {
    ...
    // Regression coverage for https://github.com/manaflow-ai/bmux/issues/5261:
...
final class BrowserPanelVisualAutomationRestoreHostTests: XCTestCase {
    func testRestoredDiscardedHiddenWebViewGetsRestoreHostBeforeOffscreenCapture() {
        withHiddenWebViewDiscardPolicyEnabled {
            ...
            while (panel.webView.isLoading || panel.isLoading), Date() < deadline { RunLoop.main.run(until: Date(timeIntervalSinceNow: 0.01)) }
            XCTAssertFalse(panel.webView.isLoading || panel.isLoading, "Timed out waiting for about:blank to finish loading")
...
final class BrowserPanelInitialNavigationTests: XCTestCase {
    ...
    func testDiffViewerURLIsNotPersistedForSessionRestore() throws {
        let schemeURL = try XCTUnwrap(URL(string: "\(BmuxDiffViewerURLSchemeHandler.scheme)://token/index.html"))
        ...
        let loopbackURL = try XCTUnwrap(URL(string: "http://127.0.0.1:49152/token/diff.html#bmux-diff-viewer"))
        ...
        let aliasURL = try XCTUnwrap(URL(string: "http://bmux-loopback.localtest.me:49152/token/diff.html#bmux-diff-viewer"))
    ...
    func testDiffViewerURLIsNotRecordedInBrowserHistory() throws {
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-browser-history-\(UUID().uuidString).json")
...
final class BrowserPanelDiffViewerSchemeTests: XCTestCase {
    ...
    func testDiffViewerSchemeRegistrationIsIdempotentForCopiedConfiguration() {
        ...
        XCTAssertNotNil(config.urlSchemeHandler(forURLScheme: BmuxDiffViewerURLSchemeHandler.scheme))
    }
    ...
    func testDiffViewerSchemeLoadsSameOriginModuleFromAllowlist() throws {
        ...
        try BmuxDiffViewerURLSchemeHandler.shared.register(
            token: token,
            files: [
        ...
        let allowedURL = try XCTUnwrap(URL(string: "\(BmuxDiffViewerURLSchemeHandler.scheme)://\(token)/index.html"))
        let allowedPatchURL = try XCTUnwrap(URL(string: "\(BmuxDiffViewerURLSchemeHandler.scheme)://\(token)/index.patch"))
        let blockedURL = try XCTUnwrap(URL(string: "\(BmuxDiffViewerURLSchemeHandler.scheme)://\(token)/not-allowed.html"))
        let queryURL = try XCTUnwrap(URL(string: "\(BmuxDiffViewerURLSchemeHandler.scheme)://\(token)/index.html?copy=1"))
        XCTAssertNotNil(BmuxDiffViewerURLSchemeHandler.shared.registeredFile(for: allowedURL))
        XCTAssertNotNil(BmuxDiffViewerURLSchemeHandler.shared.registeredFile(for: allowedPatchURL))
        XCTAssertNil(BmuxDiffViewerURLSchemeHandler.shared.registeredFile(for: blockedURL))
        XCTAssertNil(BmuxDiffViewerURLSchemeHandler.shared.registeredFile(for: queryURL))
    ...
    func testDiffViewerSchemeRejectsSymlinkEscapeFromTrustedRoot() throws {
        ...
        let temporaryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-diff-viewer-security-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertThrowsError(try BmuxDiffViewerURLSchemeHandler.shared.register(
            token: token,
            files: [
    ...
    func testEnsureReactGrabActiveRefreshesBridgeSessionTokenWhenAlreadyActive() async throws {
        ...
        _ = try await panel.evaluateJavaScript(
            ...
            window['\(panel.reactGrabBridgeSessionUpdaterName)'] = function(token) {
                window.__bmuxTestRoundTripToken = token;
                return true;
            };
            true;
            """
        ...
        let refreshedToken = try await panel.evaluateJavaScript("window.__bmuxTestRoundTripToken") as? String
...
final class BrowserWindowPortalLifecycleTests: XCTestCase {
    private final class TrackingPortalWebView: WKWebView {
        ...
        @objc(_enterInWindow)
        func bmuxUnitTestEnterInWindow() {
            reattachRenderingStateCount += 1
        }

        @objc(_endDeferringViewInWindowChangesSync)
        func bmuxUnitTestEndDeferringViewInWindowChangesSync() {
            reattachRenderingStateCount += 1
        }
    }
    ...
    // Regression guard for https://github.com/manaflow-ai/bmux/issues/5733.
    ...
    // Crash regression for https://github.com/manaflow-ai/bmux/issues/5733.
    //
    ...
    // both `bmux_performKeyEquivalent` and `bmux_makeFirstResponder` ->
...
final class OmnibarNativeTextFieldCaretTests: XCTestCase {
    ...
    private func installOmnibarField(
        in window: NSWindow,
        stringValue: String = "https://github.com/manaflow-ai/bmux"
    ...
    /// Regression for https://github.com/manaflow-ai/bmux/issues/5268: a single,
