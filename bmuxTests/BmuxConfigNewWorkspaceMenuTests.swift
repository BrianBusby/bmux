...
struct BmuxConfigNewWorkspaceMenuTests {

    // MARK: - Store: plus-button menu auto-append

    @MainActor
    private func loadStore(globalJSON: String) throws -> (store: BmuxConfigStore, root: URL) {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-workspace-action-tests-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let globalConfigURL = root.appendingPathComponent("bmux.json")
    ...
    @MainActor
    private func menuActionIDs(_ store: BmuxConfigStore) -> [String] {
    ...
    @Test func storeAutoAppendsWorkspaceActionsToPlusButtonMenu() throws {
        ...
        // Defaults stay first.
        #expect(ids.first == BmuxSurfaceTabBarBuiltInAction.newWorkspace.configID)
    ...
    @Test func contextMenuIncludesBuiltInAgentChatActionBox() throws {
        ...
        let newWorkspaceIndex = try #require(menu.items.firstIndex { item in
            (item.representedObject as? NewWorkspaceContextMenuActionBox)?.action.id
                == BmuxSurfaceTabBarBuiltInAction.newWorkspace.configID
        })
        ...
        #expect(agentChatBox.action.id == BmuxSurfaceTabBarBuiltInAction.newAgentChat.configID)
    ...
    @Test func contextMenuHidesBuiltInAgentChatWhenActionOptsOut() throws {
        ...
        #expect(!ids.contains(BmuxSurfaceTabBarBuiltInAction.newAgentChat.configID))
    }
    ...
    @Test func customContextMenuControlsBuiltInAgentChatLikeOtherBuiltIns() throws {
        ...
        #expect(store.newWorkspaceContextMenuIsConfigured)
        let ids = try withNewWorkspaceContextMenu(store: store) { contextMenuActionIDs($0) }
        #expect(ids.contains(BmuxSurfaceTabBarBuiltInAction.newWorkspace.configID))
        #expect(!ids.contains(BmuxSurfaceTabBarBuiltInAction.newAgentChat.configID))
    }
    ...
    @Test func customContextMenuCanExplicitlyIncludeBuiltInAgentChat() throws {
        ...
        #expect(store.newWorkspaceContextMenuIsConfigured)
        let ids = try withNewWorkspaceContextMenu(store: store) { contextMenuActionIDs($0) }
        #expect(ids.filter { $0 == BmuxSurfaceTabBarBuiltInAction.newAgentChat.configID }.count == 1)
    }
