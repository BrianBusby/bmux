...
extension CLINotifyProcessIntegrationRegressionTests {
    func testRovoDevPromptSubmitInfersSessionIdFromWorkspaceMetadataAndPersistsLaunchCommand() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-rovo-hook-\(UUID().uuidString)", isDirectory: true)
        ...
        environment["BMUX_ROVODEV_SESSIONS_DIR"] = sessionsRoot.path
        environment["BMUX_AGENT_LAUNCH_KIND"] = "rovodev"
        environment["BMUX_AGENT_LAUNCH_EXECUTABLE"] = "/usr/local/bin/acli"
        ...
        environment["BMUX_AGENT_LAUNCH_CWD"] = workspace.path
    ...
    func testRovoDevPromptSubmitInfersNewestMatchingWorkspaceMetadata() throws {
        ...
        let listenerFD = try bindUnixSocket(at: socketPath)
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-rovo-newest-hook-\(UUID().uuidString)", isDirectory: true)
        let workspace = root.appendingPathComponent("repo", isDirectory: true)
        let sessionsRoot = root.appendingPathComponent("sessions", isDirectory: true)
        let workspaceId = "55555555-5555-5555-5555-555555555555"
        let surfaceId = "66666666-6666-6666-6666-666666666666"
        ...
        let result = runProcess(
            ...
            environment: [
                ...
                "BMUX_ROVODEV_SESSIONS_DIR": sessionsRoot.path,
                "BMUX_AGENT_LAUNCH_CWD": workspace.path,
                "BMUX_AGENT_LAUNCH_KIND": "rovodev",
                "BMUX_AGENT_LAUNCH_EXECUTABLE": "/usr/local/bin/acli",
    ...
    func testRovoDevPromptSubmitReadsConfiguredPersistenceDirWithCommentsHashAndApostrophePath() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-rovo-config-hook-\(UUID().uuidString)", isDirectory: true)
        let workspace = root.appendingPathComponent("repo", isDirectory: true)
    ...
    func testRovoDevPromptSubmitWithoutCwdDoesNotInferUnrelatedSession() throws {
        ...
        let listenerFD = try bindUnixSocket(at: socketPath)
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-rovo-nocwd-hook-\(UUID().uuidString)", isDirectory: true)
        let sessionsRoot = root.appendingPathComponent("sessions", isDirectory: true)
        let workspaceId = "55555555-5555-5555-5555-555555555555"
        let surfaceId = "66666666-6666-6666-6666-666666666666"
    ...
    func testRovoDevInstallDoesNotReplaceUnreadableConfigPath() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-rovo-install-\(UUID().uuidString)", isDirectory: true)
        let configDir = root.appendingPathComponent(".rovodev", isDirectory: true)
    ...
    func testRovoAliasCreatesConfigDirAndInstallsRovoDevHooksFromSetup() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-rovo-alias-install-\(UUID().uuidString)", isDirectory: true)
        let configDir = root.appendingPathComponent(".rovodev", isDirectory: true)
        ...
        XCTAssertTrue(config.contains(#"BMUX_BUNDLED_CLI_PATH"#), config)
    }

    func testSetupHooksRejectsConflictingAgentFilters() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-hooks-conflicting-agent-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSetupHooksRejectsMultiplePositionalTargets() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-hooks-multiple-targets-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
