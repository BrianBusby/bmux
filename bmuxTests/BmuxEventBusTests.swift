...
final class BmuxEventBusTests: XCTestCase {
    ...
    func testSubscribeReplaysEventsAfterSequenceAndReportsAck() throws {
        let bus = BmuxEventBus(retainedEventLimit: 4)
        bus.publish(
    ...
    func testSubscribeReportsGapWhenCursorFallsOutOfRetention() throws {
        let bus = BmuxEventBus(retainedEventLimit: 2)
    ...
    func testSubscriptionFiltersLiveEventsByCategory() {
        let bus = BmuxEventBus(retainedEventLimit: 8)
    ...
    func testSlowSubscriptionClosesWhenPendingQueueIsFull() {
        let bus = BmuxEventBus(retainedEventLimit: 8, maxPendingEventsPerSubscription: 2)
    ...
    func testEventEncodingIsSingleLineJSON() throws {
        ...
        let line = try XCTUnwrap(BmuxEventBus.encodeLine(event))
    ...
    func testEncodingPreservesZeroAndOneNumbers() throws {
        let line = try XCTUnwrap(BmuxEventBus.encodeLine([
    ...
    func testStrictSequenceParsingRejectsBooleanAndFloatFrames() throws {
        XCTAssertEqual(BmuxEventBus.int64(NSNumber(value: Int64(42))), 42)
        XCTAssertEqual(BmuxEventBus.int64("42"), 42)
        XCTAssertNil(BmuxEventBus.int64(true))
        XCTAssertNil(BmuxEventBus.int64(NSNumber(value: 1.5)))
    }

    func testOversizedEventPayloadIsTruncatedBeforeRetention() throws {
        let bus = BmuxEventBus(retainedEventLimit: 4, maxEventLineBytes: 1_024)
    ...
    func testWorkspaceReorderSocketMapperDoesNotDuplicateLifecycleEvent() throws {
        ...
        let snapshot = BmuxEventBus.shared.subscribe(
            afterSequence: nil,
            names: ["workspace.reordered"],
            categories: []
        )
        defer {
            BmuxEventBus.shared.unsubscribe(snapshot.subscription)
            BmuxEventBus.shared.resetForTesting()
        }
        ...
        BmuxSocketEventMapper.publish(command: command, response: response)
    ...
    func testPublishV2ReadTextResponseDoesNotAccumulateOnLongLivedThread() throws {
        ...
        XCTAssertTrue(BmuxEventBus.shared.retainedSnapshot().isEmpty)
    }
    ...
    func testNotificationSocketParamsRedactTextFields() throws {
        let redacted = BmuxSocketEventMapper.redactedNotificationParams([
            "title": "Secret title",
    ...
    func testV1NotifySurfacePublishesSurfaceIdWithoutWorkspaceId() throws {
        ...
        BmuxSocketEventMapper.publish(command: "notify_surface \(surfaceId.uuidString) done", response: "OK")
    ...
    func testV1MapperIgnoresNonSuccessResponses() throws {
        ...
        BmuxSocketEventMapper.publish(command: "notify title", response: "OKAY")
        BmuxSocketEventMapper.publish(command: "notify title", response: "queued")
        BmuxSocketEventMapper.publish(command: "notify title", response: "ERROR: failed")
    ...
    func testWorkstreamPayloadRedactsSensitiveFields() throws {
        ...
        let payload = BmuxEventBus.workstreamPayload(event)
        ...
        let line = try XCTUnwrap(BmuxEventBus.encodeLine(["payload": payload]))
        XCTAssertFalse(line.contains("secret"))
    }

    func testPublishAppendsDurableEventLog() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-event-log-\(UUID().uuidString)", isDirectory: true)
        let logURL = directory.appendingPathComponent("events.jsonl")
        let bus = BmuxEventBus(retainedEventLimit: 4, eventLogURL: logURL)
    ...
    func testDurableEventLogDropsOldestPendingLinesUnderBackpressure() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-event-log-backpressure-\(UUID().uuidString)", isDirectory: true)
        let logURL = directory.appendingPathComponent("events.jsonl")
        let bus = BmuxEventBus(
    ...
    func testDurableEventLogRotatesAtByteLimit() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-event-log-rotation-\(UUID().uuidString)", isDirectory: true)
        let logURL = directory.appendingPathComponent("events.jsonl")
        let bus = BmuxEventBus(
