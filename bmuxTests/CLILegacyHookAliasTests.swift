...
extension CLINotifyProcessIntegrationRegressionTests {
    func testLegacyCodexHookAliasReturnsJSONWithoutHelpAndPersistsSession() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-legacy-codex-hook-\(UUID().uuidString)", isDirectory: true)
        ...
        environment["BMUX_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated(["/usr/local/bin/codex", "--model", "gpt-5.4"])
    ...
    func testLegacyFeedHookAliasReturnsJSONWithoutHelpOutsideBmuxTerminal() throws {
        let cliPath = try bundledCLIPath()
        var environment = ProcessInfo.processInfo.environment
        environment["BMUX_SOCKET_PATH"] = makeSocketPath("legacy-feed")
