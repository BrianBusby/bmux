...
extension BMUXCLIErrorOutputRegressionTests {
    @Test func testSessionsListReportsForkDiagnosticsAndAcceptsWorkspaceRefs() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-fork-\(UUID().uuidString)", isDirectory: true)
            ...
            "sessions": [
                sessionId: [
                    ...
                    "launchCommand": [
                        "launcher": "codex",
                        "arguments": [],
                        "workingDirectory": "/tmp/bmux/debug",
                        "environment": [
        ...
        environment["BMUX_AGENT_HOOK_STATE_DIR"] = stateDir.path
    ...
    @Test func testSessionsListForkStartupInputAvailabilityCanDifferFromForkSupport() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-fork-startup-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
    ...
    @Test func testSessionsListIgnoresUntrustedLaunchCaptureForStartupInput() throws {
        ...
        let session = try sessionsListDiagnosticSession(
            ...
            arguments: ["/usr/local/bin/bmux", "claude-teams"],
    ...
    @Test func testSessionsListDoesNotReportLiveMatchingPIDAsStaleRestoreBlocker() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-live-pid-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
    ...
    func sessionsListDiagnosticSession(
        ...
        environment: [String: String] = [:], workingDirectory: String = "/tmp/bmux/debug", pid: Int? = nil,
        transcriptPath: String? = nil
    ) throws -> [String: Any] {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-codex-diag-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let sessionId = "019ef275-74e3-7777-9773-9dcb118ed5aa"
        ...
        for key in Array(processEnvironment.keys) where key.hasPrefix("BMUX_") { processEnvironment.removeValue(forKey: key) }
        processEnvironment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        processEnvironment["BMUX_AGENT_HOOK_STATE_DIR"] = stateDir.path
    ...
    @Test func testSessionsListForkStartupInputCountsSelectedEnvironment() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-fork-env-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func testSessionsListDoesNotReportLocalOpenCodeForkSupportedWithoutVersionProbe() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-opencode-fork-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
    ...
    @Test func testSessionsListDoesNotExecuteCapturedOpenCodeProbeCommand() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-opencode-probe-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let repoDir = root.appendingPathComponent("repo", isDirectory: true)
