...
struct WorkspaceForkConversationContextMenuTests {
    ...
    func liveAgentIndexLoaderUsesProcessDetectedPanelWhenHookBindingIsStale() throws {
        ...
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-live-agent-index-\(UUID().uuidString)", isDirectory: true)
        ...
        let processSnapshot = BmuxTopProcessSnapshot(
            processes: [
                BmuxTopProcessInfo(
                    ...
                    path: executable,
                    ttyDevice: nil,
                    bmuxWorkspaceID: liveWorkspaceId,
                    bmuxSurfaceID: livePanelId,
    ...
    func forkAvailabilitySnapshotRefreshesWhenProcessScopeChanges() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-live-agent-cache-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        ...
        let processSnapshotLock = OSAllocatedUnfairLock(initialState: BmuxTopProcessSnapshot(
            processes: [],
            sampledAt: Date(timeIntervalSince1970: 42),
            includesProcessDetails: true
        ))
        let sharedIndex = SharedLiveAgentIndex(
            indexLoader: {
                ...
                return SharedLiveAgentIndexLoader(
                    ...
                    processArgumentsProvider: { pid in
                        pid == processId
                            ? BmuxTopProcessArguments(arguments: [executable, "--session", sessionId], environment: ["PWD": cwd.path])
                            : nil
                    },
            ...
            hookStoreDirectoryProvider: {
                root.appendingPathComponent(".bmuxterm", isDirectory: true).path
            }
        )
    ...
    func forkAvailabilityProbeFailsClosedWhileSharedIndexRefreshes() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-live-agent-probe-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func forkAvailabilityProbeRefreshesMissingPanelSnapshotInsideCacheWindow() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-live-agent-missing-probe-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        try fm.createDirectory(at: root.appendingPathComponent(".bmuxterm", isDirectory: true), withIntermediateDirectories: true)
        ...
        let sharedIndex = SharedLiveAgentIndex(
            indexLoader: {
                SharedLiveAgentIndexLoader(
                    ...
                    processSnapshotProvider: {
                        BmuxTopProcessSnapshot(processes: [], sampledAt: now.withLock { $0 }, includesProcessDetails: true)
                    },
