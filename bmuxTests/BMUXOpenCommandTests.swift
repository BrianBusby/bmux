...
final class BMUXOpenCommandTests: XCTestCase {
    ...
    func testDiffCommandGeneratesCodeViewAndOpensBrowserSplit() throws {
        ...
        try FileManager.default.createDirectory(at: bmuxConfigURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: bmuxAppSupportConfigURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        ...
        try ghosttyConfigContents.write(to: bmuxAppSupportConfigURL, atomically: true, encoding: .utf8)
        try """
        {
          "shortcuts": {
            "bindings": {
        ...
        """.write(to: bmuxConfigURL, atomically: true, encoding: .utf8)
        try """
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            guard method == "browser.open_split",
                  ...
                  viewerURL.fragment == "bmux-diff-viewer" else {
                return Self.v2Response(id: id, ok: false, error: ["code": "unexpected", "message": method])
            }
            return Self.v2Response(
                id: id,
        ...
        XCTAssertEqual(viewerURL.fragment, "bmux-diff-viewer")
        ...
        XCTAssertTrue(html.contains("\"dark\":\"bmux-ghostty-dark-"), html)
        XCTAssertTrue(html.contains("\"light\":\"bmux-ghostty-light-"), html)
        ...
        try darkOnlyConfigContents.write(to: bmuxAppSupportConfigURL, atomically: true, encoding: .utf8)
    ...
    func testDiffCommandUsesTaggedSocketAppAssetsAndServer() throws {
        ...
        let socketPath = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("bmux-debug-\(tag).sock", isDirectory: false)
            .path
        unlink(socketPath)
        let listenerFD = try bindUnixSocket(at: socketPath)
        let rootURL = FileManager.default.temporaryDirectory
        ...
        try writeTestDiffViewerAssets(
            ...
            appMain: "export const bmuxTaggedSocketAssetMarker = 'target-\(tag)';\n"
        )
        try """
    ...
    func testDiffViewerServerBoundsDeferredWaitRequests() throws {
        ...
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-diff-viewer-wait-\(UUID().uuidString)", isDirectory: true)
        ...
        // BMUX_DIFF_VIEWER_WAIT_TIMEOUT_SECONDS=0.05 the server must give up on the
    ...
    func testDiffCommandShowsFriendlyEmptyStateForLastTurnWithoutBaseline() throws {
        ...
        let result = try runDiffCLIAndReadHTML(
            ...
            environmentOverrides: [
                "BMUX_AGENT_HOOK_STATE_DIR": rootURL.appendingPathComponent("hook-state", isDirectory: true).path,
                "BMUX_WORKSPACE_ID": UUID().uuidString.lowercased(),
                "BMUX_SURFACE_ID": UUID().uuidString.lowercased()
            ],
            currentDirectoryURL: repoURL,
            readPatchSidecar: false
        )
    ...
    func testDiffCommandShowsFriendlyEmptyStateForEmptyLastTurnDiff() throws {
        ...
        let result = try runDiffCLIAndReadHTML(
            ...
            arguments: ["diff", "--last-turn"],
            environmentOverrides: [
                "BMUX_AGENT_HOOK_STATE_DIR": stateURL.path,
    ...
    func testDiffCommandSupportsGitSourcesAndSurfaceScopedLastTurn() throws {
        ...
        try runGit(["config", "user.name", "bmux tests"], in: siblingRepoURL)
        try runGit(["config", "user.email", "bmux@example.invalid"], in: siblingRepoURL)
    ...
    func testAgentTurnDiffBaselineStoresUntrackedSnapshotsOutsideGit() throws {
        ...
        let untrackedRefs = try runGitStdout(
            ["for-each-ref", "--format=%(refname)", "refs/bmux/last-turn/untracked"],
            in: repoURL
        )
    ...
    func testDiffCommandOpensPendingViewerBeforeGitDiffCompletes() throws {
        ...
        XCTAssertTrue(pendingHTML.contains("data-bmux-diff-pending=\"true\""), pendingHTML)
        ...
        XCTAssertTrue(waitUntil(timeout: 5) {
            ...
            return html.contains("data-bmux-diff-redirect=")
        ...
        XCTAssertFalse(html.contains("data-bmux-diff-pending=\"true\""), html)
    ...
    func testTopCommandOutputsWindowLevelProcessRows() throws {
        ...
        XCTAssertEqual(outputLines(result.stdout), [
            ...
            "2.0\t2000\t1\tprocess\t4129\twindow:1\tbmux",
        ])
    }
    ...
    func testTopCommandSortsFlatWindowProcessesAndWorkspacesTogetherByMemory() throws {
        ...
        XCTAssertEqual(outputLines(result.stdout), [
            "2.0\t2000\t2\twindow\twindow:1\ttotal\t",
            "4.0\t10000\t1\tprocess\t4129\twindow:1\tbmux",
            "1.0\t1000\t1\tworkspace\tworkspace:low\twindow:1\t",
        ])
    }
    ...
    private static func diffViewerRedirectURL(from html: String) -> String? {
        let marker = "data-bmux-diff-redirect=\""
    ...
    private func diffViewerConfig(from html: String) throws -> [String: Any] {
        let marker = "<script id=\"bmux-diff-viewer-config\" type=\"application/json\">"
    ...
    private func runGit(_ arguments: [String], in directory: URL) throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        if result.status != 0 {
            throw NSError(domain: "BMUXOpenCommandTests.git", code: Int(result.status), userInfo: [NSLocalizedDescriptionKey: result.stderr])
        }
    }
    ...
    private func writeTestDiffViewerAssets(resourcesURL: URL, appMain: String) throws {
        ...
        try "self.bmuxWorkerFixture = true;\n".write(
    ...
    private func readLine(from handle: FileHandle, timeout: TimeInterval) throws -> String {
        ...
        guard finished.wait(timeout: .now() + timeout) == .success else {
            throw NSError(domain: "bmux.tests", code: 1, userInfo: [
    ...
    private func fetchData(from url: URL, timeout: TimeInterval) throws -> (data: Data, statusCode: Int) {
        ...
        guard finished.wait(timeout: .now() + timeout) == .success else {
            ...
            throw NSError(domain: "bmux.tests", code: 2, userInfo: [
