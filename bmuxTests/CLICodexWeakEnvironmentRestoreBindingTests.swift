...
extension CLINotifyProcessIntegrationRegressionTests {
    func testCodexHookDoesNotPublishResumeBindingForWeakEnvironmentOnlyCapture() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-weak-env-\(UUID().uuidString)", isDirectory: true)
        ...
        environment.removeValue(forKey: "BMUX_CLI_TTY_NAME")
    ...
    func testCodexWeakCurrentCapturePreservesDurableMappedResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-weak-env-preserve-\(UUID().uuidString)", isDirectory: true)
        let repo = root.appendingPathComponent("bmuxterm-hq", isDirectory: true)
    ...
    func testCodexPlainHookWithoutLaunchCapturePublishesDefaultResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-plain-no-launch-\(UUID().uuidString)", isDirectory: true)
        let repo = root.appendingPathComponent("bmuxterm-hq", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        environment["BMUX_AGENT_HOOK_STATE_DIR"] = root.path
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        for key in ["ANTHROPIC_BASE_URL", "CLAUDE_CONFIG_DIR", "CODEX_HOME", "BMUX_AGENT_LAUNCH_KIND", "BMUX_AGENT_LAUNCH_EXECUTABLE", "BMUX_AGENT_LAUNCH_ARGV_B64", "BMUX_AGENT_LAUNCH_CWD"] {
            environment.removeValue(forKey: key)
        }
