...
@Suite(.serialized) struct BMUXCLIErrorOutputRegressionTests {
    ...
    @Test func testCLIErrorPathDoesNotCrashWhenStderrIsClosed() throws {
        ...
        let result = runShell(
            "BMUX_CLI_SENTRY_DISABLED=1 \(shellSingleQuote(cliPath)) definitely-not-a-command 2>&-",
            timeout: 5
        )
    ...
    @Test func testAgentTeamsHelpDoesNotLaunchExternalAgentCLI() throws {
        ...
        for command in ["claude-teams", "codex-teams"] {
            ...
            XCTAssertTrue(result.stdout.contains("Usage: bmux \(command)"), result.stdout)
    ...
    @Test func testBundledCLIInTaggedDebugAppPrefersItsOwnSocketWithoutEnvironmentOverride() throws {
        ...
        let taggedSocketPath = "/tmp/bmux-debug-\(tagSlug).sock"
    ...
    @Test func testBundledCLIInTaggedDebugAppTreatsCaseVariantStableEnvSocketAsImplicitDefault() throws {
        ...
        let caseVariantStablePath = stableSocketURL
            ...
            .appendingPathComponent("BMUX.sock", isDirectory: false)
            .path
        ...
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "5"
        environment["BMUX_SOCKET_PATH"] = caseVariantStablePath
    ...
    @Test func testBundledCLIInTaggedDebugAppDoesNotFallBackToStableEnvSocketWhenTaggedSocketIsMissing() throws {
        ...
        let stableSocketURL = fixedHomeURL
            .appendingPathComponent(".local/state/bmux", isDirectory: true)
            .appendingPathComponent("bmux.sock", isDirectory: false)
        ...
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "0.1"
        environment["BMUX_SOCKET_PATH"] = stableSocketURL.path
    ...
    @Test func testBundledCLIInTaggedDebugAppTreatsUserScopedStableEnvSocketAsImplicitDefault() throws {
        let cliPath = try bundledCLIPath()
        let fixedHomeURL = URL(fileURLWithPath: "/tmp/bmux-cli-home-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: fixedHomeURL) }
        let stableSocketURL = fixedHomeURL
            .appendingPathComponent(".local/state/bmux", isDirectory: true)
            .appendingPathComponent("bmux-\(getuid()).sock", isDirectory: false)
        let stableSocketPath = stableSocketURL.path
        try FileManager.default.createDirectory(
            at: stableSocketURL.deletingLastPathComponent(),
            withIntermediateDirectories: true
        ...
        for alias in aliases {
            try autoreleasepool {
                ...
                environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
                environment["BMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "5"
                environment["BMUX_SOCKET_PATH"] = alias
                environment["CFFIXED_USER_HOME"] = fixedHomeURL.path
    ...
    @Test func testBundledStableCLIPreservesLiveUserScopedStableEnvSocket() throws {
        ...
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "5"
        environment["BMUX_SOCKET_PATH"] = userScopedStableSocketPath
        environment["CFFIXED_USER_HOME"] = fixedHomeURL.path
    ...
    @Test func testBundledStableCLIFallsBackFromSymlinkedLegacyStableEnvSocket() throws {
        ...
        let defaultStableSocketPath = socketDirectoryURL
            .appendingPathComponent("bmux.sock", isDirectory: false)
            .path
        let legacyStableSocketPath = "/tmp/bmux.sock"
        let symlinkTargetSocketPath = "/tmp/bmux-symlink-target-\(UUID().uuidString).sock"
        ...
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = "5"
        environment["BMUX_SOCKET_PATH"] = legacyStableSocketPath
        environment["CFFIXED_USER_HOME"] = fixedHomeURL.path
    ...
    @Test func testThemesSetReloadsRunningAppAfterEveryThemeWrite() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-themes-socket-\(UUID().uuidString)", isDirectory: true)
        ...
        let socketPath = "/tmp/bmux-theme-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(path: socketPath, response: "OK")
        defer { responder.stop() }
        let bundleIdentifier = "com.bmuxterm.app.debug.issue-4355-test"
        let reloadExpectation = expectation(description: "bmux themes set posts final reload notifications")
        ...
        let observer = DistributedNotificationCenter.default().addObserver(
            forName: Notification.Name("com.bmuxterm.themes.reload-config"),
            object: nil,
        ...
        environment["BMUX_BUNDLE_ID"] = bundleIdentifier
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
    ...
    @Test func testThemesSetTargetsResolvedTaggedSocketWhenBundleEnvironmentIsStale() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-themes-stale-bundle-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let socketPath = "/tmp/bmux-debug-active-theme-\(UUID().uuidString).sock"
        let staleBundleIdentifier = "com.bmuxterm.app.debug.stale.theme"
        let targetBundleIdentifier = "com.bmuxterm.app.debug.active.theme"
        let reloadExpectation = expectation(description: "bmux themes set targets the resolved socket bundle")
        let notificationQueue = OperationQueue()
        notificationQueue.maxConcurrentOperationCount = 1
        let notificationLock = NSLock()
        ...
        environment["GHOSTTY_RESOURCES_DIR"] = resourcesURL.path
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_BUNDLE_ID"] = staleBundleIdentifier
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
    ...
    @Test func testThemesSetNightlyOverridePathIsReadableByNightlyAppConfigResolution() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-themes-nightly-path-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let bundleIdentifier = "com.bmuxterm.app.nightly"
        var environment = ProcessInfo.processInfo.environment
        for key in Array(environment.keys) where key.hasPrefix("BMUX_") {
            environment.removeValue(forKey: key)
        }
        ...
        environment["HOME"] = root.path
        environment["GHOSTTY_RESOURCES_DIR"] = resourcesURL.path
        environment["BMUX_SOCKET_PATH"] = "/tmp/bmux-nightly.sock"
        environment["BMUX_BUNDLE_ID"] = bundleIdentifier
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        ...
        let appReadablePaths = GhosttyApp.bmuxAppSupportConfigURLs(
    ...
    @Test func testBareInteractiveThemesReloadsRunningAppAfterPickerExits() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-themes-picker-socket-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let socketPath = "/tmp/bmux-theme-picker-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(path: socketPath, response: "OK")
        defer { responder.stop() }
        let bundleIdentifier = "com.bmuxterm.app.debug.theme-picker.\(UUID().uuidString.lowercased())"
        let reloadExpectation = expectation(description: "bare bmux themes posts final reload notification")
        let notificationQueue = OperationQueue()
        notificationQueue.maxConcurrentOperationCount = 1
        let notificationLock = NSLock()
        var observedReloads: [(bundleIdentifier: String?, phase: String?)] = []
        ...
        let command = [
            ...
            "BMUX_SOCKET_PATH=\(shellSingleQuote(socketPath))",
            "BMUX_BUNDLE_ID=\(shellSingleQuote(bundleIdentifier))",
    ...
    @Test func testBareInteractiveThemesTreatsSigintAsSilentCancel() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-themes-picker-cancel-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let socketPath = "/tmp/bmux-theme-picker-cancel-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(path: socketPath, response: "OK")
        defer { responder.stop() }
    ...
    @Test func testBrowserDownloadWaitUsesRequestedTimeoutForSocketResponse() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = "/tmp/bmux-dw-\(UUID().uuidString.prefix(8)).sock"
    ...
    @Test func testDotPathOpenBypassesProtectedSocketForExternalCLI() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-cli-external-open-\(UUID().uuidString)", isDirectory: true)
        ...
        let socketPath = "/tmp/bmux-external-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
            response: "ERROR: Access denied — only processes started inside bmux can connect"
        )
        defer { responder.stop() }
        ...
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_SOCKET"] = "/tmp/bmux-stale-\(UUID().uuidString.prefix(8)).sock"
        environment["BMUX_SOCKET_PASSWORD"] = "stale-password"
        environment["BMUX_SOCKET_ENABLE"] = "0"
        environment["BMUX_SOCKET_MODE"] = "off"
        environment["BMUX_ALLOW_SOCKET_OVERRIDE"] = "1"
        environment["BMUX_WORKSPACE_ID"] = "workspace:stale"
        environment["BMUX_PANEL_ID"] = "panel:stale"
        environment["BMUX_SURFACE_ID"] = "surface:stale"
        environment["BMUX_TAB_ID"] = "tab:stale"
        environment["BMUX_TAG"] = "keepme"
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_TEST_OPEN_TOOL_PATH"] = fakeOpenURL.path
        environment["BMUX_TEST_OPEN_LOG"] = openLogURL.path
        environment["BMUX_TEST_OPEN_ENV_LOG"] = openEnvLogURL.path
        ...
        XCTAssertTrue(openEnvironment.contains("BMUX_TAG=keepme"), openEnvironment.joined(separator: "\n"))
    }

    @Test func testBareRelativeDirectoryPathOpenBypassesProtectedSocketForExternalCLI() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-cli-bare-open-\(UUID().uuidString)", isDirectory: true)
        let workingDirectory = root.appendingPathComponent("project", isDirectory: true)
        try FileManager.default.createDirectory(at: workingDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let socketPath = "/tmp/bmux-bare-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
            response: "ERROR: Access denied — only processes started inside bmux can connect"
        )
    ...
    @Test func testKnownCommandStillUsesSocketWhenMatchingBareRelativePathExists() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-cli-command-path-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(
        ...
        let socketPath = "/tmp/bmux-command-path-\(UUID().uuidString.prefix(8)).sock"
    ...
    @Test func testCaseVariantBareRelativeDirectoryPathOpenBypassesProtectedSocket() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-cli-case-path-\(UUID().uuidString)", isDirectory: true)
        ...
        let socketPath = "/tmp/bmux-case-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
            response: "ERROR: Access denied — only processes started inside bmux can connect"
        )
    ...
    @Test func testExplicitSocketPathOpenUsesRequestedSocket() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-cli-explicit-open-\(UUID().uuidString)", isDirectory: true)
        let workingDirectory = root.appendingPathComponent("project", isDirectory: true)
        try FileManager.default.createDirectory(at: workingDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let socketPath = "/tmp/bmux-explicit-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
    ...
    /// via the canonical ``BmuxStateDirectory`` and pass the same home to the
    /// spawned CLI via `CFFIXED_USER_HOME`, so they never touch (or bind over) the
    /// developer's real `~/.local/state/bmux` (issue #5146).
    private func makeTemporaryHome() throws -> URL {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-cli-home-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
        return home
    }

    /// The stable control-socket path under an injected (temp) home, resolved via
    /// the canonical ``BmuxStateDirectory`` so the test exercises the real layout.
    private func stableSocketURL(home: URL) throws -> URL {
        let directory = BmuxStateDirectory.url(homeDirectory: home)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        return directory.appendingPathComponent("bmux.sock", isDirectory: false)
    }
    ...
    private func fakeTaggedBundledCLIPath(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-cli-socket-\(UUID().uuidString)", isDirectory: true)
        let appURL = root.appendingPathComponent("bmux DEV \(tagSlug).app", isDirectory: true)
        ...
        let info: [String: Any] = [
            "CFBundleIdentifier": bundleIdentifier ?? "com.bmuxterm.app.debug.\(tagSlug.replacingOccurrences(of: "-", with: "."))",
            "CFBundleName": bundleName ?? "bmux DEV \(tagSlug)",
            "CFBundlePackageType": "APPL"
        ]
        ...
        let fakeCLIURL = binURL.appendingPathComponent("bmux", isDirectory: false)
    ...
    private func fakeOpenScript() -> String {
        ...
        if [ -n "${BMUX_TEST_OPEN_ENV_LOG:-}" ]; then
          env | LC_ALL=C sort | grep '^BMUX_' > "$BMUX_TEST_OPEN_ENV_LOG" || :
        fi
        for arg in "$@"; do
          printf '%s\\n' "$arg" >> "$BMUX_TEST_OPEN_LOG"
        done
        exit 0
        """
    }
...
private final class UnixSocketResponder {
    ...
    private let queue = DispatchQueue(label: "com.bmux.tests.unix-socket-responder")
