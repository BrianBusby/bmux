...
struct GhosttyTerminalStartupEnvironmentTests {
    @MainActor
    @Test
    func testTerminalSurfaceStartupEnvironmentIncludesBmuxContextValues() throws {
        ...
        let expectedContextValues = [
            "BMUX_WORKSPACE_ID": workspaceId.uuidString,
            "BMUX_SURFACE_ID": surface.id.uuidString,
            "BMUX_TAB_ID": workspaceId.uuidString,
            "BMUX_PANEL_ID": surface.id.uuidString,
        ]
        ...
        let socketPath = try #require(
            surface.startupEnvironmentValue("BMUX_SOCKET_PATH"),
            "BMUX_SOCKET_PATH should be present"
        )
    ...
    func testApplyManagedGitWatchEnvironmentDisablesShellGitWatch() {
        ...
        expectEqual(environment["BMUX_NO_GIT_WATCH"], "1")
        expectTrue(protectedKeys.contains("BMUX_NO_GIT_WATCH"))
    }
    ...
    func testApplyManagedGitWatchEnvironmentClearsInheritedOptOutWhenEnabled() {
        ...
        expectEqual(merged["BMUX_NO_GIT_WATCH"], "")
    }
    ...
    func testApplyManagedGitWatchEnvironmentDisablesShellPullRequestWatchWhenHidden() {
        ...
        expectEqual(environment["BMUX_NO_GIT_WATCH"], "")
        expectEqual(environment["BMUX_NO_PR_WATCH"], "1")
        expectTrue(protectedKeys.contains("BMUX_NO_GIT_WATCH"))
        expectTrue(protectedKeys.contains("BMUX_NO_PR_WATCH"))
    }
    ...
    func testInstallClaudeCommandShimCreatesExecutableOutsideBundleBin() throws {
        ...
        let bundleBin =
            root
            .appendingPathComponent("bmux.app", isDirectory: true)
        ...
        let wrapperURL = bundleBin.appendingPathComponent("bmux-claude-wrapper", isDirectory: false)
        try """
        #!/usr/bin/env bash
        set -euo pipefail
        {
            printf 'shim=%s\\n' "${BMUX_CLAUDE_WRAPPER_SHIM:-}"
            printf 'root=%s\\n' "${BMUX_CLAUDE_WRAPPER_SHIM_ROOT:-}"
            printf 'args=%s\\n' "$*"
        } > "$BMUX_TEST_LOG"
    ...
    func testClaudeCommandShimFallsBackToCurrentBundledWrapperWhenEmbeddedWrapperWasReaped() throws {
        ...
        let staleWrapperURL = oldBundleBin.appendingPathComponent("bmux-claude-wrapper", isDirectory: false)
        try writeExecutable(staleWrapperURL, """
        #!/usr/bin/env bash
        printf 'stale %s\\n' "$*" > "$BMUX_TEST_LOG"
        """)

        let currentWrapperURL = currentBundleBin.appendingPathComponent("bmux-claude-wrapper", isDirectory: false)
        try writeExecutable(currentWrapperURL, """
        #!/usr/bin/env bash
        printf 'current %s\\n' "$*" > "$BMUX_TEST_LOG"
        """)
        let currentCLIURL = currentBundleBin.appendingPathComponent("bmux", isDirectory: false)
        ...
        process.environment = [
            "PATH": "/usr/bin:/bin",
            "BMUX_BUNDLED_CLI_PATH": currentCLIURL.path,
            "BMUX_TEST_LOG": logURL.path,
        ]
        try process.run()
        process.waitUntilExit()
    ...
    @Test
    func testClaudeCommandShimFallbackSkipsInheritedBmuxShimRoots() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "GhosttyTerminalStartupEnvironmentTests-\(UUID().uuidString)", isDirectory: true)
        let bundleBin =
    ...
    @Test
    func testMergedStartupEnvironmentAllowsSessionReplayAndInitialEnvBMUXKeys() {
        let replayPath = "/tmp/bmux-replay-\(UUID().uuidString)"
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: [
                "PATH": "/usr/bin",
                "BMUX_SURFACE_ID": "managed-surface",
            ],
            protectedKeys: ["PATH", "BMUX_SURFACE_ID"],
            additionalEnvironment: [
                SessionScrollbackReplayStore.environmentKey: replayPath
            ],
            initialEnvironmentOverrides: [
                "BMUX_INITIAL_ENV_TOKEN": "token-123"
            ]
        )

        expectEqual(merged[SessionScrollbackReplayStore.environmentKey], replayPath)
        expectEqual(merged["BMUX_INITIAL_ENV_TOKEN"], "token-123")
    }
    ...
    func testMergedStartupEnvironmentProtectsManagedKeysOnly() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            ...
            protectedKeys: ["PATH", "BMUX_SURFACE_ID"],
            additionalEnvironment: [
                "BMUX_SURFACE_ID": "user-surface",
                "CUSTOM_FLAG": "1",
            ],
            initialEnvironmentOverrides: [
                "PATH": "/tmp/bin",
                "BMUX_SURFACE_ID": "override-surface",
            ]
        )

        expectEqual(merged["PATH"], "/usr/bin")
        expectEqual(merged["BMUX_SURFACE_ID"], "managed-surface")
    ...
    func testMergedStartupEnvironmentDoesNotDeriveHermesCodexBaseURLForGenericTerminals() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-hermes-codex-startup-\(UUID().uuidString)", isDirectory: true)
    ...
    // MARK: - Locale sanitization (https://github.com/manaflow-ai/bmux/issues/7152)
    //
    // A bmux-spawned shell must not inherit a malformed `LC_ALL`/`LC_CTYPE` — a
