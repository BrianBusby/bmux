...
struct ClaudeHookFeedTelemetrySwiftTests {
    @Test func sessionStartFeedTelemetryUsesResolvedTTYSurface() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-claude-feed-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(
            ...
            "Feed telemetry must use the resolved agent TTY surface, not leaked BMUX_SURFACE_ID; event=\(event)"
        )
    }
}
