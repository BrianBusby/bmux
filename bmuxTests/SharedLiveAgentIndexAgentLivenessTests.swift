...
struct SharedLiveAgentIndexAgentLivenessTests {
    ...
    func forkAvailabilityIgnoresDeadUnrelatedPanelChildProcess() async throws {
        ...
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-fork-agent-liveness-\(UUID().uuidString)", isDirectory: true)
        ...
        let processArguments = OSAllocatedUnfairLock(initialState: BmuxTopProcessArguments(
        ...
        processArguments.withLock {
            $0 = BmuxTopProcessArguments(
                arguments: [executable, "--session", sessionId],
                environment: [
                    "PWD": cwd.path,
                    "BMUX_WORKSPACE_ID": workspaceId.uuidString,
                    "BMUX_SURFACE_ID": UUID().uuidString,
                ]
            )
        }
        await sharedIndex.refreshForkAvailabilityNow(workspaceId: workspaceId, panelId: panelId)
    ...
    func forkAvailabilityReadsUseCachedValidationWithoutProcessInspection() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-fork-agent-read-cache-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func forkAvailabilityValidationUsesPanelFallbackAfterWorkspaceMove() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-fork-agent-panel-fallback-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        ...
        let processSnapshot = BmuxTopProcessSnapshot(
            processes: [
                BmuxTopProcessInfo(
                    ...
                    path: executable,
                    ttyDevice: nil,
                    bmuxWorkspaceID: originalWorkspaceId,
                    bmuxSurfaceID: panelId,
                    bmuxAttributionReason: "bmux-test",
                    processGroupID: nil,
                    terminalProcessGroupID: nil,
                ...
                SharedLiveAgentIndexLoader(
                    ...
                    processArgumentsProvider: { pid in
                        ...
                        return BmuxTopProcessArguments(
                            arguments: [executable, "--session", sessionId],
                            environment: [
                                "BMUX_WORKSPACE_ID": originalWorkspaceId.uuidString,
                                "BMUX_SURFACE_ID": panelId.uuidString,
                            ]
                        )
                    },
    ...
    func cachedAgentProcessIdentityRejectsInheritedScopeAndDifferentSession() {
        ...
        #expect(
            validator.currentProcess(
                BmuxTopProcessArguments(
                    arguments: [executable, "--session", sessionId],
                    environment: ["BMUX_AGENT_LAUNCH_KIND": agentId]
                ),
                matches: snapshot
            )
        )
        #expect(
            ...
            "Inherited bmux agent scope is not enough when argv no longer identifies the cached agent."
        )
        #expect(
            !validator.currentProcess(
                BmuxTopProcessArguments(
