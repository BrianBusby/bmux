...
final class BmuxSSHURLRequestTests: XCTestCase {
    deinit {}
    ...
    func testCommandPreviewIncludesSocketPathWhenProvided() throws {
        ...
        case .success(.some(let request)):
            XCTAssertEqual(
                request.cliPreview(socketPath: "/tmp/bmux-urlcmd.sock"),
                "bmux --socket /tmp/bmux-urlcmd.sock ssh --name \"Dev SSH\" dev.example.com"
            )
        case .success(nil):
            XCTFail("Expected SSH URL request")
        case .failure(let error):
    ...
    func testParsesStableNightlyAndDevSchemes() throws {
        for scheme in ["bmux", "bmux-nightly", "bmux-dev"] {
            var components = URLComponents()
            ...
            switch BmuxSSHURLRequest.parse(url, supportedSchemes: BmuxSSHURLRequest.supportedSchemes) {
            case .success(.some(let request)):
                XCTAssertEqual(request.destination, "dev.example.com")
            case .success(nil):
    ...
    func testDefaultParserIgnoresOtherProductSchemes() throws {
        let inactiveScheme = try XCTUnwrap(BmuxSSHURLRequest.supportedSchemes.first {
    ...
    func testAcceptsTextURLAtMaxLength() throws {
        let text = String(repeating: "a", count: BmuxTextURLRequest.maxTextLength)
        let url = try XCTUnwrap(textURL(host: "prompt", queryItems: [
        ...
        switch BmuxTextURLRequest.parse(url) {
        case .success(.some(let request)):
            XCTAssertEqual(request.text.count, BmuxTextURLRequest.maxTextLength)
        case .success(nil):
            XCTFail("Expected prompt URL request")
        case .failure(let error):
            XCTFail("Unexpected parse error: \(error)")
    ...
    func testRejectsTextURLExceedingMaxLength() throws {
        let text = String(repeating: "a", count: BmuxTextURLRequest.maxTextLength + 1)
        let url = try XCTUnwrap(textURL(host: "prompt", queryItems: [
            URLQueryItem(name: "text", value: text)
        ]))

        switch BmuxTextURLRequest.parse(url) {
        case .failure(.textTooLong(maxLength: BmuxTextURLRequest.maxTextLength)):
            break
        default:
    ...
    private func parsedOptional(_ url: URL) throws -> BmuxSSHURLRequest? {
        switch BmuxSSHURLRequest.parse(url) {
...
final class BmuxNavigationURLRequestTests: XCTestCase {
    private let supportedScheme = "bmux-test"
    ...
    func testGeneratedLinksRoundTrip() throws {
        let workspaceURL = try XCTUnwrap(URL(string: BmuxNavigationURLRequest.workspaceLink(workspaceId: workspaceId, scheme: supportedScheme)))
        let paneURL = try XCTUnwrap(URL(string: BmuxNavigationURLRequest.paneLink(workspaceId: workspaceId, paneId: paneId, scheme: supportedScheme)))
        let surfaceURL = try XCTUnwrap(URL(string: BmuxNavigationURLRequest.surfaceLink(workspaceId: workspaceId, surfaceId: surfaceId, scheme: supportedScheme)))
    ...
    func testIgnoresOtherBmuxRoutesAndInactiveSchemes() throws {
        ...
        let inactiveURL = try XCTUnwrap(URL(string: "bmux-other://workspace/\(workspaceId.uuidString)"))
    ...
    func testRejectsQueryFragmentAuthorityAndExtraPathComponents() throws {
        ...
        for rawURL in cases {
            let url = try XCTUnwrap(URL(string: rawURL))
            switch BmuxNavigationURLRequest.parse(url, supportedSchemes: [supportedScheme]) {
    ...
    private func parsedOptional(_ url: URL) throws -> BmuxNavigationURLRequest? {
        switch BmuxNavigationURLRequest.parse(url, supportedSchemes: [supportedScheme]) {
        case .success(let request):
            return request
        case .failure(let error):
    ...
    private func parsedTarget(_ url: URL) throws -> BmuxNavigationURLRequest.Target {
