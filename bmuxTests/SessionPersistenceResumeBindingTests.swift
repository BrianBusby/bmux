...
@Suite struct SessionPersistenceResumeBindingTests {
    ...
    @Test func decodingAgentHookBindingRewritesPersistedPATHManagedAgentExecutable() throws {
        ...
        #expect(startupInput.contains("BMUX_CLAUDE_WRAPPER_SHIM"), "\(startupInput)")
    ...
    @Test func agentHookBindingRewritesSupportedLocalManagedExecutablePaths() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-surface-resume-stale-managed-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let executablePaths = [
            Self.localManagedExecutablePath(root: root, executableName: "codex", ".fnm", "current", "bin"),
            "/tmp/bmux-cli-shims/\(UUID().uuidString)/codex",
    ...
    @Test func agentHookBindingWithDirectEnvironmentAssignmentRewritesMovedExecutable() throws {
        ...
        let binding = SurfaceResumeBindingSnapshot(
            kind: "codex",
            command: "BMUX_TRACE=1 '\(staleExecutablePath)' 'resume' 'session-env-cli'",
    ...
    @Test func agentHookBindingWithQuotedEnvAssignmentRewritesMovedExecutable() throws {
        ...
        let binding = SurfaceResumeBindingSnapshot(
            kind: "codex",
            command: "env 'BMUX_TRACE=1' '\(staleExecutablePath)' 'resume' 'session-quoted-env-cli'",
    ...
    @Test func agentHookClaudeBindingWithDirectEnvironmentAssignmentPreservesAssignmentSyntax() throws {
        ...
        let binding = SurfaceResumeBindingSnapshot(
            kind: "claude",
            command: "BMUX_TRACE='bar baz' '\(staleExecutablePath)' '--resume' 'session-env-cli'",
            checkpointId: "session-env-cli",
            source: "agent-hook",
            autoResume: true
        )
        ...
        #expect(startupInput.contains("/bin/sh -c"), "\(startupInput)")
        #expect(startupInput.contains("BMUX_CLAUDE_WRAPPER_SHIM"), "\(startupInput)")
        #expect(startupInput.contains("BMUX_TRACE="), "\(startupInput)")
    ...
    @Test func agentHookClaudeBindingWithShellOperatorKeepsOriginalCommandShape() throws {
        ...
        let redirection = "1>/tmp/bmux-claude-resume.log"
        let binding = SurfaceResumeBindingSnapshot(
            kind: "claude",
    ...
    @Test @MainActor func remoteWorkspaceLocalTerminalResumeBindingUsesLocalRepair() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-local-resume-binding-\(UUID().uuidString)", isDirectory: true)
        ...
        let suiteName = "bmux-session-resume-binding-\(UUID().uuidString)"
    ...
    @Test func agentHookSurfaceResumeStartupInputPreservesExistingPATHManagedAgentExecutable() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-surface-resume-existing-agent-\(UUID().uuidString)", isDirectory: true)
        let executable = root
    ...
    @Test func agentHookSurfaceResumeStartupInputFallsBackWhenRecordedAgentExecutableMoved() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-surface-resume-moved-agent-\(UUID().uuidString)", isDirectory: true)
        ...
        process.environment = [
            ...
            "BMUX_FAKE_CODEX_OUTPUT": outputURL.path,
        ]
        let stderr = Pipe()
    ...
    private func makeOhMyZshLauncherEnvironment(
        ...
        return [
            ...
            "BMUX_ZSH_ZDOTDIR": userZdotdirURL.path,
            "BMUX_SHELL_INTEGRATION": "1",
            "BMUX_SHELL_INTEGRATION_DIR": integrationDir.path,
            "BMUX_ZSH_RESTORE_TERM": "xterm-256color",
            "BMUX_CODEX_WRAPPER_SHIM": codexShimURL.path,
            "BMUX_FAKE_CODEX_OUTPUT": codexOutputURL.path,
    ...
    private func writeOhMyZshFixture(at root: URL) throws {
        ...
        try """
        autoload -Uz add-zsh-hook
        _bmux_test_autosuggest_precmd() { :; }
        _bmux_test_autosuggest_preexec() { :; }
        add-zsh-hook precmd _bmux_test_autosuggest_precmd
        add-zsh-hook preexec _bmux_test_autosuggest_preexec
        _bmux_test_autosuggest_self_insert() { zle .self-insert }
        zle -N self-insert _bmux_test_autosuggest_self_insert
        """.write(
        ...
        try """
        _bmux_test_syntax_highlighting_line_init() { :; }
        _bmux_test_syntax_highlighting_line_finish() { :; }
        zle -N zle-line-init _bmux_test_syntax_highlighting_line_init
        zle -N zle-line-finish _bmux_test_syntax_highlighting_line_finish
        """.write(
