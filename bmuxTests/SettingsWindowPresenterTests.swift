...
struct SettingsWindowPresenterTests {
    @Test func configureWindowLeavesPendingNavigationForSettingsViews() async throws {
        try await withCleanSettingsWindows {
            ...
            let settingsWindow = makeWindow(identifier: "bmux.unconfiguredSettings.\(UUID().uuidString)")
            var didOpen = false
            defer {
    ...
    @Test func doesNotAttachSettingsAsChildOfPreferredMainWindow() async throws {
        try await withCleanSettingsWindows {
            let presenter = SettingsWindowPresenter()
            let parentWindow = makeWindow(identifier: "bmux.main.\(UUID().uuidString)")
            let settingsWindow = makeWindow(identifier: SettingsWindowPresenter.windowIdentifier)
            defer {
                settingsWindow.orderOut(nil)
    ...
    @Test func focusingSettingsKeepsItAsPeerWhenPreferredMainWindowChanges() async throws {
        try await withCleanSettingsWindows {
            let presenter = SettingsWindowPresenter()
            let firstParent = makeWindow(identifier: "bmux.main.\(UUID().uuidString)")
            let secondParent = makeWindow(identifier: "bmux.main.\(UUID().uuidString)")
            let settingsWindow = makeWindow(identifier: SettingsWindowPresenter.windowIdentifier)
    ...
    @Test func adoptBmuxPeerWindowLevelBringsFloatingWindowToNormal() async throws {
        try await withCleanSettingsWindows {
            let window = makeWindow(identifier: "bmux.peer.\(UUID().uuidString)")
            defer {
                window.orderOut(nil)
                window.close()
            }
