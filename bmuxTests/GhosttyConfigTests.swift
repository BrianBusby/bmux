...
// The app target still declares legacy duplicates of these BmuxSettings
// value types; with BmuxSettings imported unconditionally the names are
// ambiguous. These tests exercise the app-side paths, so pin the app types.
private typealias BrowserThemeMode = bmux_DEV.BrowserThemeMode
#elseif canImport(bmux)
@testable import bmux
private typealias BrowserThemeMode = bmux.BrowserThemeMode
#endif
...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testLaunchGhosttyResourcesPreferCurrentBundleOverInheritedEnvironment() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-launch-resources-\(UUID().uuidString)")
        ...
        let resolved = bmuxApp.resolvedGhosttyResourcesDirectory(
    ...
    func testLaunchGhosttyResourcesKeepInheritedEnvironmentWhenBundleHasNoResources() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-launch-resource-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLaunchGhosttyResourcesKeepInheritedEnvironmentWhenBundleLacksThemes() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-launch-incomplete-resource-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLaunchGhosttyResourcesUseIncompleteBundleOnlyAsLastFallback() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-launch-incomplete-resource-last-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testThemeSearchPathsIncludeXDGDataDirsThemes() {
        let pathA = "/tmp/bmux-theme-a"
        let pathB = "/tmp/bmux-theme-b"
    ...
    func testThemeSearchPathsIncludeBmuxUserThemesDirectory() {
        let paths = GhosttyConfig.themeSearchPaths(
        ...
        XCTAssertTrue(
            paths.contains(
                "\(NSHomeDirectory())/Library/Application Support/com.bmuxterm.app/themes/Zag Light"
            )
        )
    }

    func testThemeSearchPathsIncludeBmuxUserThemesDirectoryFromFixedHome() {
        let fixedHome = "/tmp/bmux-fixed-home-\(UUID().uuidString)"
        let paths = GhosttyConfig.themeSearchPaths(
            forThemeName: "Zag Light",
        ...
        XCTAssertTrue(
            paths.contains(
                "\(fixedHome)/Library/Application Support/com.bmuxterm.app/themes/Zag Light"
            )
        )
    }

    func testThemesListIncludesBmuxUserThemesDirectory() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-user-theme-list-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        let themesDirectory = root
            .appendingPathComponent("Library/Application Support/com.bmuxterm.app/themes", isDirectory: true)
    ...
    func testBmuxDefaultThemeConfigContentsSkipsInvalidUTF8Candidate() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-managed-theme-search-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let contents = GhosttyConfig.bmuxDefaultThemeConfigContents(
            preferredColorScheme: .light,
    ...
    func testLoadReadsSymlinkedGhosttyConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-config-symlink-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadReadsBackgroundFromRecursiveConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-config-recursive-background-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadDoesNotReparseTopLevelConfigReferencedByConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-config-top-level-cycle-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadAllowsRecursiveConfigFileToReloadTopLevelConfigAsFinalOverride() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-config-top-level-reload-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadReadsOptionalQuotedConfigFilePath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-config-optional-quoted-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadIgnoresLegacyAppSupportConfigWhenConfigGhosttyIsNonEmpty() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-app-support-legacy-skip-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadUsesLegacyAppSupportConfigWhenConfigGhosttyIsEmpty() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-app-support-legacy-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadAppliesThemeBeforeLaterCursorColorOverride() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-theme-order-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadThemeReadsAbsoluteThemeFilePath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-absolute-theme-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadThemeResolvesPairedThemeValueByColorScheme() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-theme-pair-\(UUID().uuidString)")
    ...
    func testLoadThemeResolvesBuiltinAliasFromGhosttyResourcesDir() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-themes-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedLightAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-solarized-light-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedDarkAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ghostty-solarized-dark-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testUnparsedAppearanceFallbackIgnoresNativeLegacyBaselineWhenCurrentConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-native-legacy-baseline-\(UUID().uuidString)")
    ...
    func testUnparsedAppearanceFallbackKeepsNativeLegacyBaselineWhenCurrentConfigIsMissingOrEmpty() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-native-legacy-baseline-empty-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    func testRuntimeColorSchemeForBmuxSingleThemeReloadKeepsResolvedSchemeDuringConfigLoad() {
        XCTAssertEqual(
            GhosttyApp.runtimeColorSchemeForConfigLoad(
                source: GhosttySurfaceConfigurationRefresh.bmuxThemeReloadFinalSource,
                requestedColorScheme: .dark,
                effectiveTerminalColorScheme: .light,
                bmuxThemeValue: "light:3024 Day,dark:3024 Day"
            ),
            .light
        )
        XCTAssertEqual(
            GhosttyApp.runtimeColorSchemeForConfigLoad(
                source: GhosttySurfaceConfigurationRefresh.bmuxThemeReloadPreviewSource,
                requestedColorScheme: .dark,
                effectiveTerminalColorScheme: .light,
                bmuxThemeValue: "3024 Day"
            ),
            .light
        )
    }
    ...
    func testRuntimeColorSchemeForPairedThemeReloadUsesAppearanceDuringConfigLoad() {
        XCTAssertEqual(
            GhosttyApp.runtimeColorSchemeForConfigLoad(
                ...
                requestedColorScheme: .dark,
                effectiveTerminalColorScheme: .light,
                bmuxThemeValue: "light:3024 Day,dark:3024 Night"
            ),
            .dark
        )
        XCTAssertEqual(
    ...
    func testClaudeCodeIntegrationDefaultsToEnabledWhenUnset() {
        let suiteName = "bmux.tests.claude-hooks.\(UUID().uuidString)"
    ...
    func testKiroIntegrationDefaultsToEnabledWithStandardNotificationsWhenUnset() {
        let suiteName = "bmux.tests.kiro-hooks.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    func testSubagentNotificationSuppressionDefaultsToEnabledWhenUnset() {
        let suiteName = "bmux.tests.subagent-notifications.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    func testTelemetryDefaultsToEnabledWhenUnset() {
        let suiteName = "bmux.tests.telemetry.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
...
// bmuxShouldUseTransparentBackgroundWindow / bmuxShouldUseClearWindowBackground /
// bmuxShouldApplyWindowGlass) were lifted out of the app target into
// BmuxWorkspaceWindow's WindowBackgroundPolicy by the window-chrome tranche, and
// equivalent coverage now lives in
// Packages/macOS/BmuxWorkspaceWindow/Tests/BmuxWorkspaceWindowTests/WindowBackgroundPolicyTests.swift.
// The stale app-side test was left referencing the removed symbols, which broke
// the bmuxTests compile on the Swift 6 depot toolchain.
...
final class WorkspaceRemoteDaemonManifestTests: XCTestCase {
    func testParsesEmbeddedRemoteDaemonManifestJSON() throws {
        let manifestJSON = """
        {
          ...
          "appVersion": "0.62.0",
          "releaseTag": "v0.62.0",
          "releaseURL": "https://github.com/manaflow-ai/bmux/releases/tag/v0.62.0",
          "checksumsAssetName": "bmuxd-remote-checksums.txt",
          "checksumsURL": "https://github.com/manaflow-ai/bmux/releases/download/v0.62.0/bmuxd-remote-checksums.txt",
          "entries": [
            {
              "goOS": "linux",
              "goArch": "amd64",
              "assetName": "bmuxd-remote-linux-amd64",
              "downloadURL": "https://github.com/manaflow-ai/bmux/releases/download/v0.62.0/bmuxd-remote-linux-amd64",
              "sha256": "abc123"
            }
          ]
        }
        ...
        XCTAssertEqual(manifest?.entry(goOS: "linux", goArch: "amd64")?.assetName, "bmuxd-remote-linux-amd64")
    }
    ...
    func testRemoteDaemonCachePathIsVersionedByPlatform() throws {
        ...
        XCTAssertTrue(url.path.contains("/.local/state/bmux/remote-daemons/0.62.0/linux-arm64/"))
        XCTAssertEqual(url.lastPathComponent, "bmuxd-remote")
    }
}
...
final class RemoteLoopbackHTTPRequestRewriterTests: XCTestCase {
    func testRewritesLoopbackAliasHostHeadersToLocalhost() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: bmux-loopback.localtest.me:3000\r\n" +
                "Origin: http://bmux-loopback.localtest.me:3000\r\n" +
                "Referer: http://bmux-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        let rewritten = RemoteLoopbackHTTPRequestRewriter.rewriteIfNeeded(
            data: original,
            aliasHost: "bmux-loopback.localtest.me"
        )
        ...
        XCTAssertFalse(text.contains("bmux-loopback.localtest.me"))
    }
    ...
    func testRewritesLoopbackSubdomainAliasHostHeadersToOriginalLocalhostSubdomain() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: api.bmux-loopback.localtest.me:3000\r\n" +
                "Origin: http://api.bmux-loopback.localtest.me:3000\r\n" +
                "Referer: http://api.bmux-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        XCTAssertFalse(text.contains("api.bmux-loopback.localtest.me"))
    }

    func testRewritesAbsoluteFormRequestLineForLoopbackAlias() {
        let original = Data(
            (
                "GET http://bmux-loopback.localtest.me:3000/demo HTTP/1.1\r\n" +
                "Host: bmux-loopback.localtest.me:3000\r\n" +
                "\r\n"
            ).utf8
        )
    ...
    func testRewritesLoopbackResponseHeadersBackToAlias() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://bmux-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://bmux-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=bmux-loopback.localtest.me; Path=/"))
    }
    ...
    func testRewritesLoopbackSubdomainResponseHeadersBackToAliasSubdomain() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://api.bmux-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://api.bmux-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=api.bmux-loopback.localtest.me; Path=/"))
    }
    ...
    func testRewritesLeadingDotLoopbackCookieDomainsBackToAliasDomains() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Set-Cookie: root=1; Domain=.bmux-loopback.localtest.me; Path=/"))
        XCTAssertTrue(text.contains("Set-Cookie: api=1; Domain=.api.bmux-loopback.localtest.me; Path=/"))
    }
}
...
final class BrowserPanelWebViewLifecycleTests: XCTestCase {
    func testHiddenDiscardPolicyReadsUserDefaults() throws {
        let suiteName = "bmux.browserHiddenDiscardPolicyTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer {
            defaults.removePersistentDomain(forName: suiteName)
        }
        let hasEnabledEnvironmentOverride =
            ProcessInfo.processInfo.environment["BMUX_BROWSER_HIDDEN_WEBVIEW_DISCARD_ENABLED"] != nil
        let hasDelayEnvironmentOverride =
            ProcessInfo.processInfo.environment["BMUX_BROWSER_HIDDEN_WEBVIEW_DISCARD_DELAY_SECONDS"] != nil
...
final class BrowserDefaultsNormalizationTests: XCTestCase {
    ...
    func testNormalizeRewritesOutOfRangeAndLegacyValues() throws {
        let suiteName = "bmux.browserDefaultsNormalizationTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    ...
    func testNormalizePreservesValidValues() throws {
        ...
        // `BmuxSettings`, which declares a same-named enum.
...
final class BrowserNewTabNavigationSeedTests: XCTestCase {
    func testPreservesOriginalRequestHeadersMethodBodyAndBypassHost() throws {
        ...
        request.setValue("keep-me", forHTTPHeaderField: "X-Bmux-Test")
        ...
        XCTAssertEqual(
            seed.initialRequest.value(forHTTPHeaderField: "X-Bmux-Test"),
            "keep-me"
        )
...
final class BrowserPanelRemoteStoreTests: XCTestCase {
    ...
    func testRemoteWorkspaceDefersInitialNavigationUntilProxyEndpointIsReady() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "bmux-loopback.localtest.me")
    }
    ...
    func testRemoteWorkspacePreservesLocalhostSubdomainWhenAliasingLoopbackURL() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "api.bmux-loopback.localtest.me")
    }
    ...
    func testRemoteWorkspaceRuntimeBridgeAliasesMultipleLoopbackPortsFromSamePage() async throws {
        ...
        let baseURL = try XCTUnwrap(URL(string: "http://bmux-loopback.localtest.me:3000/"))
        ...
        let result = try await panel.evaluateJavaScript(
            """
            (() => {
              const rewrite = window.__bmuxRewriteRemoteLoopbackURL;
        ...
        XCTAssertEqual(
            result,
            #"["http://bmux-loopback.localtest.me:3000/frontend","http://bmux-loopback.localtest.me:8000/api","http://api.bmux-loopback.localtest.me:8000/v1","ws://bmux-loopback.localtest.me:5173/hmr","wss://localhost:5173/hmr","https://localhost:9443/secure"]"#
        )
    }
    ...
    func testBrowserMoveIntoRemoteWorkspaceRebuildsWebsiteDataStoreScope() throws {
        ...
        destination.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/bmux-store-dest.sock",
                terminalStartupCommand: "ssh bmux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testBrowserMoveOutOfRemoteWorkspaceRestoresDefaultWebsiteDataStore() throws {
        ...
        source.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/bmux-store-source.sock",
                terminalStartupCommand: "ssh bmux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testNewTerminalSurfaceStaysRemoteWhileBrowserPanelsKeepWorkspaceRemote() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            relayID: "relay-test",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/bmux-test.sock",
            terminalStartupCommand: "ssh bmux-macmini"
        )
...
final class WorkspaceRemoteConfigurationTransportKeyTests: XCTestCase {
    func testProxyBrokerTransportKeyIgnoresControlPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/bmux-ssh-501-64000-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64000,
            relayID: "relay-a",
            relayToken: "token-a",
            localSocketPath: "/tmp/bmux-a.sock",
            terminalStartupCommand: "ssh bmux-macmini"
        )
        let second = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/bmux-ssh-501-64001-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64001,
            relayID: "relay-b",
            relayToken: "token-b",
            localSocketPath: "/tmp/bmux-b.sock",
            terminalStartupCommand: "ssh bmux-macmini"
        )
    ...
    func testProxyBrokerTransportKeyIgnoresEphemeralAgentSocketPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/bmux-a.sock",
            terminalStartupCommand: "ssh bmux-macmini",
            agentSocketPath: "/tmp/bmux-agent-a.sock"
        )
        let second = WorkspaceRemoteConfiguration(
            destination: "bmux-macmini",
            port: 22,
            ...
            localSocketPath: "/tmp/bmux-b.sock",
            terminalStartupCommand: "ssh bmux-macmini",
            agentSocketPath: "/tmp/bmux-agent-b.sock"
        )
...
final class WorkspaceRemoteSSHCleanupTests: XCTestCase {
    func testOrphanedBMUXRemoteSSHPIDsMatchesOnlyParentOneRelayAndDaemonTransports() {
        let psOutput = """
          101 1 /usr/bin/ssh -N -T -S none -o ControlPath=/tmp/bmux-ssh-501-56080-%C -R 127.0.0.1:56080:127.0.0.1:64048 bmux-macmini
          102 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote serve --stdio'
          107 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-test''
          103 999 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 bmux-macmini
          104 1 /usr/bin/ssh -tt bmux-macmini
          105 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56082:127.0.0.1:64050 other-host
          106 1 /usr/bin/ssh -T -S none bmux-macmini /bin/sh
        """

        XCTAssertEqual(
            RemoteSessionCoordinator.orphanedBMUXRemoteSSHPIDs(
                psOutput: psOutput,
                destination: "bmux-macmini"
            ),
            [101, 102, 107]
    ...
    func testOrphanedBMUXRemoteSSHPIDsCanRestrictCleanupToSpecificRelayPort() {
        let psOutput = """
          201 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56080:127.0.0.1:64048 bmux-macmini
          202 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 bmux-macmini
          203 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote serve --stdio'
          204 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-test''
          205 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-other''
        """
    ...
    func testOrphanedBMUXRemoteSSHPIDsMatchesBackslashEscapedPersistentDaemonSlot() {
        let psOutput = """
          211 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec '\''.bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote'\'' '\''serve'\'' '\''--stdio'\'' '\''--persistent'\'' '\''--slot'\'' '\''ssh-test'\'''
          212 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec '\''.bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote'\'' '\''serve'\'' '\''--stdio'\'' '\''--persistent'\'' '\''--slot'\'' '\''ssh-other'\'''
        """
    ...
    func testOrphanedBMUXRemoteSSHPIDsMatchesEqualsQuotedPersistentDaemonSlot() {
        let psOutput = """
          221 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote serve --stdio --persistent --slot='ssh-test''
          222 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote serve --stdio --persistent --slot="ssh-other"'
        """
    ...
    func testOrphanedBMUXRemoteSSHPIDsWithSlotAndNoRelayKeepsGenericCleanup() {
        let psOutput = """
          301 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56080:127.0.0.1:64048 bmux-macmini
          302 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote serve --stdio'
          303 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-test''
          304 1 /usr/bin/ssh -T -S none -o RequestTTY=no bmux-macmini sh -c 'exec .bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote 'serve' '--stdio' '--persistent' '--slot' 'ssh-other''
          305 1 /usr/bin/ssh -T -S none -o RequestTTY=no other-host sh -c 'exec .bmux/bin/bmuxd-remote/0.63.1/darwin-arm64/bmuxd-remote serve --stdio'
        """
...
final class SocketControlSettingsTests: XCTestCase {
    func testMigrateModeSupportsExpandedSocketModes() {
        ...
        XCTAssertEqual(SocketControlSettings.migrateMode("bmuxOnly"), .bmuxOnly)
    ...
    func testInvalidEnvSocketModeDoesNotOverrideUserMode() {
        XCTAssertNil(
            SocketControlSettings.envOverrideMode(
                environment: ["BMUX_SOCKET_MODE": "definitely-not-a-mode"]
            )
        )
        XCTAssertEqual(
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "BMUX_TAG": "stray-tag",
                "BMUX_SOCKET_PATH": "/tmp/bmux-debug-issue-153-tmux-compat.sock",
            ],
    ...
    func testTaggedDebugLaunchUsesTagDefaultWhenNoOverrideIsProvided() {
        ...
        XCTAssertEqual(path, "/tmp/bmux-debug-my-tag.sock")
    }
    ...
    func testTaggedDebugLaunchStillHonorsSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "BMUX_TAG": "my-tag",
                "BMUX_SOCKET_PATH": "/tmp/bmux-debug-forced.sock",
            ],
            bundleIdentifier: "com.bmuxterm.app.debug",
            isDebugBuild: true
        )

        XCTAssertEqual(path, "/tmp/bmux-debug-forced.sock")
    }
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        ...
        XCTAssertEqual(path, "/tmp/bmux-nightly.sock")
    }

    func testTaggedDebugBundleKeepsMatchingSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "BMUX_SOCKET_PATH": "/tmp/bmux-debug-my-tag.sock",
            ],
            bundleIdentifier: "com.bmuxterm.app.debug.my-tag",
            isDebugBuild: false
        )
    ...
    func testTaggedDebugBundleIgnoresSocketOverrideInheritedFromDifferentBmuxBundle() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "BMUX_BUNDLE_ID": "com.bmuxterm.app.nightly",
                "BMUX_SOCKET_PATH": "/tmp/bmux-nightly.sock",
            ],
            bundleIdentifier: "com.bmuxterm.app.debug.issue.4355.bmux.themes.set.state.dependent",
            isDebugBuild: true
        )

        XCTAssertEqual(path, "/tmp/bmux-debug-issue-4355-bmux-themes-set-state-dependent.sock")
    }
    ...
    func testTaggedDebugBundleIgnoresMismatchedInheritedSocketOverride() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "com.bmuxterm.app.debug.fix-grok-notifications",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/bmux-debug-fix-grok-notifications.sock")
    }
    ...
    func testTaggedDebugBundleRefusesStableSocketOverrideEvenWithOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "BMUX_SOCKET_PATH": SocketControlSettings.stableDefaultSocketPath,
                "BMUX_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "com.bmuxterm.app.debug.sockguard",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/bmux-debug-sockguard.sock")
    }
    ...
    func testTaggedDebugBundleRefusesUserScopedStableSocketOverrideEvenWithOptInFlag() {
        let aliases = [
            ...
            "/private/tmp/bmux-501.sock",
        ]

        for alias in aliases {
            let path = SocketControlSettings.socketPath(
                environment: [
                    "BMUX_SOCKET_PATH": alias,
                    "BMUX_ALLOW_SOCKET_OVERRIDE": "1",
                ],
                bundleIdentifier: "com.bmuxterm.app.debug.sockguard",
                isDebugBuild: false,
            ...
            XCTAssertEqual(path, "/tmp/bmux-debug-sockguard.sock", alias)
        }
    }
    ...
    func testTaggedDebugBundleRefusesCanonicalLegacyStableSocketAliasEvenWithOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "BMUX_SOCKET_PATH": "/private/tmp/bmux.sock",
                "BMUX_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "com.bmuxterm.app.debug.sockguard",
            isDebugBuild: false
    ...
    func testTaggedDebugBundleRefusesCaseVariantStableSocketAliasesEvenWithOptInFlag() {
        let aliases = [
            ...
            "/private/tmp/BMUX.sock",
            SocketControlSettings.userScopedStableSocketPath(currentUserID: 501)
                .replacingOccurrences(of: "bmux-501.sock", with: "BMUX-501.sock"),
            SocketControlSettings.legacyUserScopedStableSocketPath(currentUserID: 501)
                .replacingOccurrences(of: "bmux-501.sock", with: "BMUX-501.sock"),
        ]
    ...
    func testTaggedDebugBundleRefusesLeafSymlinkToStableSocketEvenWithOptInFlag() throws {
        let alias = "/tmp/bmux-stable-alias-\(UUID().uuidString).sock"
    ...
    func testTaggedDebugBundleRefusesExcessiveSymlinkChainEvenWithOptInFlag() throws {
        let root = "/tmp/bmux-stable-chain-\(UUID().uuidString)"
    ...
    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "BMUX_SOCKET_PATH": "/tmp/bmux-staging-my-tag.sock",
            ],
            bundleIdentifier: "com.bmuxterm.app.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/bmux-staging-my-tag.sock")
    }
    ...
    func testDefaultSocketPathByChannel() {
        ...
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.bmuxterm.app.nightly.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/bmux-nightly-tag.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
            ...
            "/tmp/bmux-debug-tag.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.bmuxterm.app.staging.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/bmux-staging-tag.sock"
        )
    }
    ...
    func testInitialStableLaunchTreatsPrivateTmpLegacyStableAliasAsStablePath() {
        let path = SocketControlSettings.initialSocketPathBeforeListenerStart(
            preferredPath: "/private/tmp/bmux.sock",
            bundleIdentifier: "com.bmuxterm.app",
            isDebugBuild: false,
            currentUserID: 501,
            probeStableDefaultPathEntry: { socketPath in
                XCTAssertEqual(socketPath, "/private/tmp/bmux.sock")
    ...
    func testInitialSocketPathDoesNotProbeForTaggedDebugBuild() {
        let debugPath = "/tmp/bmux-debug-tag.sock"
        let path = SocketControlSettings.initialSocketPathBeforeListenerStart(
    ...
    func testTaggedDebugBundleAllowedWithoutLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.bmuxterm.app.debug.tests-v1",
                isDebugBuild: true
            )
        )
    }
    ...
    func testXCUITestLaunchEnvironmentIgnoresLaunchTagGate() {
        ...
        // The app receives BMUX_UI_TEST_* vars via XCUIApplication.launchEnvironment.
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["BMUX_UI_TEST_MODE": "1"],
                bundleIdentifier: "com.bmuxterm.app.debug",
                isDebugBuild: true
            )
        )
...
final class UITestLaunchManifestTests: XCTestCase {
    func testManifestPathReadsArgumentValue() {
        XCTAssertEqual(
            UITestLaunchManifest.manifestPath(
                from: ["bmux", "-bmuxUITestLaunchManifest", "/tmp/bmux-ui-test-launch.json"]
            ),
            "/tmp/bmux-ui-test-launch.json"
        )
    }
    ...
    func testManifestPathReturnsNilWithoutValue() {
        XCTAssertNil(
            UITestLaunchManifest.manifestPath(
                from: ["bmux", "-bmuxUITestLaunchManifest"]
            )
        )
    }
    ...
    func testApplyIfPresentDecodesEnvironmentPayload() {
        let payload = """
        {"environment":{"BMUX_TAG":"ui-tests-display","BMUX_SOCKET_PATH":"/tmp/bmux-ui-tests.sock"}}
        """.data(using: .utf8)!
        var applied: [String: String] = [:]

        UITestLaunchManifest.applyIfPresent(
            arguments: ["bmux", UITestLaunchManifest.argumentName, "/tmp/bmux-ui-test-launch.json"],
        ...
        XCTAssertEqual(applied["BMUX_TAG"], "ui-tests-display")
        XCTAssertEqual(applied["BMUX_SOCKET_PATH"], "/tmp/bmux-ui-tests.sock")
    }
}
...
final class GhosttyMouseFocusTests: XCTestCase {
    ...
    private func withTempConfig(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-\(UUID().uuidString)")
    ...
    func testUserConfigContainsCJKCodepointMapReturnsFalseForMissingFiles() {
        let path = NSTemporaryDirectory() + "bmux-nonexistent-\(UUID().uuidString)/config"
        XCTAssertFalse(
    ...
    func testUserConfigContainsCJKCodepointMapFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapFollowsRelativeIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-rel-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesOptionalInclude() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-opt-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesCyclicIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-cycle-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-font-family-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainIgnoresDuplicateFamilies() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-font-family-duplicate-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainMatchesGhosttyIncludeLoadOrder() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-font-family-order-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainRespectsConfigFileReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-font-family-config-file-reset-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testLoadedCJKScanPathsIncludesNativeGhosttyAppSupportWhenTaggedConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-cjk-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }

        let taggedDir = appSupport.appendingPathComponent("com.example.bmux-dev", isDirectory: true)
        ...
        let paths = GhosttyApp.loadedCJKScanPaths(
            currentBundleIdentifier: "com.example.bmux-dev",
            appSupportDirectory: appSupport
        )
    ...
    func testShouldApplyManagedDefaultAppearanceScansNativeGhosttyAppSupport() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-appearance-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    func testLoadedGhosttyConfigScanPathsSkipsNativeLegacyConfigWhenCurrentConfigIsNonEmpty() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-appearance-app-support-current-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    // Regression: https://github.com/manaflow-ai/bmux/issues/3459
    // `bmux themes set` always encodes the selection with conditional
    ...
    // stay at the white defaults), so bmux must inject the resolved plain theme
    ...
    // Regression: https://github.com/manaflow-ai/bmux/issues/3459
    // `bmux themes set --light X` encodes `theme = light:X`, which ghostty treats
    // as conditional config. bmux injects the resolved plain theme for the
    ...
    func testShouldApplyManagedDefaultAppearanceFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-theme-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testShouldApplyManagedDefaultAppearancePreservesQuotedQuestionMarkConfigFile() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-theme-quoted-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testShouldApplyManagedDefaultAppearanceProcessesIncludeQueuedAfterReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-theme-reset-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    func testGhosttyPromptHooksLoadWhenBmuxRequestsZshIntegration() throws {
        let output = try runInteractiveZsh(bmuxLoadGhosttyIntegration: true)
    ...
    func testGhosttyPromptHooksDoNotLoadWithoutBmuxHandoffFlag() throws {
        let output = try runInteractiveZsh(bmuxLoadGhosttyIntegration: false)
    ...
    func testGhosttySemanticPatchRetriesAfterDeferredInitCreatesLiveHooks() throws {
        let output = try runInteractiveZsh(
            bmuxLoadGhosttyIntegration: true,
            bmuxLoadShellIntegration: true,
            command: """
            _bmux_patch_ghostty_semantic_redraw
    ...
    func testShellIntegrationWinchGuardDoesNotPrintSpacerLineOnResize() throws {
        let output = try runInteractiveZsh(
            bmuxLoadGhosttyIntegration: false,
            bmuxLoadShellIntegration: true,
            command: """
            print -r -- BEFORE
            TRAPWINCH
    ...
    func testShellIntegrationPreservesStartupTermForThemeSelectionBeforeRestoringManagedTerm() throws {
        let output = try runPromptInteractiveZsh(
            ...
            bmuxLoadShellIntegration: true,
            command: """
            print -r -- "CMD=$TERM|${BMUX_ZSH_RESTORE_TERM-unset}" >> "$BMUX_TEST_OUTPUT"
            """,
            userZshRCContents: """
            export BMUX_STARTUP_THEME_TERM="$TERM"
            if [[ $TERM = (*256color|*rxvt*) ]]; then
              export BMUX_STARTUP_THEME_BRANCH=extended
            else
              export BMUX_STARTUP_THEME_BRANCH=basic
            fi

            bmux_test_ready() {
              [[ -e "$BMUX_TEST_READY" ]] && return 0
              print -r -- "PRE=$BMUX_STARTUP_THEME_TERM|$BMUX_STARTUP_THEME_BRANCH|$TERM|${BMUX_ZSH_RESTORE_TERM-unset}" > "$BMUX_TEST_OUTPUT"
              : > "$BMUX_TEST_READY"
              precmd_functions=(${precmd_functions:#bmux_test_ready})
            }
            precmd_functions+=(bmux_test_ready)
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermForInteractiveCommandMode() throws {
        let output = try runInteractiveZsh(
            ...
            bmuxLoadShellIntegration: true,
            command: """
            print -r -- "$BMUX_STARTUP_TERM|$TERM|${BMUX_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
            export BMUX_STARTUP_TERM="$TERM"
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenIntegrationDisabled() throws {
        let output = try runInteractiveZsh(
            bmuxLoadGhosttyIntegration: false,
            bmuxLoadShellIntegration: false,
            command: """
            print -r -- "$BMUX_STARTUP_TERM|$TERM|${BMUX_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenUserZshEnvDisablesIntegration() throws {
        let output = try runInteractiveZsh(
            ...
            bmuxLoadShellIntegration: true,
            command: """
            print -r -- "$BMUX_STARTUP_TERM|$TERM|${BMUX_ZSH_RESTORE_TERM-unset}|${BMUX_SHELL_INTEGRATION:-unset}"
            """,
            userZshEnvContents: """
            export BMUX_SHELL_INTEGRATION=0
            """,
            userZshRCContents: """
            export BMUX_STARTUP_TERM="$TERM"
            """
    ...
    func testShellIntegrationNormalizesClaudeConfigDirAfterUserZshrc() throws {
        let output = try runPromptInteractiveZsh(
            ...
            bmuxLoadShellIntegration: true,
            command: """
            print -r -- "CMD=$CLAUDE_CONFIG_DIR" >> "$BMUX_TEST_OUTPUT"
            """,
            userZshRCContents: """
            ...
            bmux_test_ready() {
              [[ -e "$BMUX_TEST_READY" ]] && return 0
              print -r -- "PRE=$CLAUDE_CONFIG_DIR" > "$BMUX_TEST_OUTPUT"
              : > "$BMUX_TEST_READY"
              precmd_functions=(${precmd_functions:#bmux_test_ready})
            }
            precmd_functions+=(bmux_test_ready)
    ...
    func testShellIntegrationRestoresManagedTermDuringPreexec() throws {
        let output = try runInteractiveZsh(
            ...
            bmuxLoadShellIntegration: true,
            command: """
            _bmux_preexec 'echo $TERM'
            print -r -- "$TERM|${BMUX_ZSH_RESTORE_TERM-unset}"
            """,
            extraEnvironment: [
                "TERM": "xterm-ghostty",
                "BMUX_ZSH_RESTORE_TERM": "xterm-256color",
            ]
        )
    ...
    func testShellIntegrationPublishesOnlyWorkspaceScopedBmuxEnvironmentToTmuxServerAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-tmux-publish-\(UUID().uuidString)")
        ...
        _ = try runInteractiveZsh(
            bmuxLoadGhosttyIntegration: false,
            bmuxLoadShellIntegration: true,
            command: "_bmux_preexec tmux; print -r -- READY",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "BMUX_SOCKET_PATH": "/tmp/bmux-current.sock",
                "BMUX_TAG": "feat-tmux-notification-attention-state",
                "BMUX_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "BMUX_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
        ...
        XCTAssertTrue(log.contains("set-environment -g BMUX_TAG feat-tmux-notification-attention-state"), log)
        XCTAssertTrue(log.contains("set-environment -g BMUX_SOCKET_PATH /tmp/bmux-current.sock"), log)
        XCTAssertTrue(log.contains("set-environment -g BMUX_WORKSPACE_ID 11111111-1111-1111-1111-111111111111"), log)
        XCTAssertFalse(log.contains("set-environment -g BMUX_SURFACE_ID"), log)
        XCTAssertFalse(log.contains("set-environment -g BMUX_PANEL_ID"), log)
    }

    func testShellIntegrationClearsStaleSurfaceScopedTmuxEnvironmentAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-tmux-clear-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("tmux.log", isDirectory: false)
        ...
        try writeExecutableScript(
            ...
            #!/bin/sh
            if [ "$1" = "show-environment" ] && [ "$2" = "-g" ]; then
              printf '%s\\n' 'BMUX_SURFACE_ID=99999999-9999-9999-9999-999999999999'
              printf '%s\\n' 'BMUX_PANEL_ID=99999999-9999-9999-9999-999999999999'
              exit 0
            fi
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
        ...
        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -gu BMUX_SURFACE_ID"), log)
        XCTAssertTrue(log.contains("set-environment -gu BMUX_PANEL_ID"), log)
    }

    func testShellIntegrationRefreshesWorkspaceScopedBmuxEnvironmentFromTmuxWithoutOverwritingSurfaceScope() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-tmux-refresh-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        let output = try runInteractiveZsh(
            bmuxLoadGhosttyIntegration: false,
            bmuxLoadShellIntegration: true,
            command: "_bmux_precmd; print -r -- \"$BMUX_TAG|$BMUX_SOCKET_PATH|$BMUX_WORKSPACE_ID|$BMUX_SURFACE_ID|$BMUX_PANEL_ID\"",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "TMUX": "/tmp/tmux-stale,123,0",
                "BMUX_SOCKET_PATH": "/tmp/bmux-stale.sock",
                "BMUX_TAG": "feat-tmux-integration-experiments",
                "BMUX_WORKSPACE_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "BMUX_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "BMUX_TAB_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "BMUX_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
        ...
        XCTAssertEqual(
            output,
            "feat-tmux-notification-attention-state|/tmp/bmux-current.sock|11111111-1111-1111-1111-111111111111|22222222-2222-2222-2222-222222222222|22222222-2222-2222-2222-222222222222"
        )
    }
    ...
    func testShellIntegrationReportsTTYFromTmuxWithoutUsingPanelScope() throws {
        let output = try runInteractiveZsh(
            ...
            bmuxLoadShellIntegration: true,
            command: """
            _BMUX_TTY_NAME=ttys999
            print -r -- "$(_bmux_report_tty_payload)"
            """,
            extraEnvironment: [
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        try writeExecutableScript(
            at: binDir.appendingPathComponent("bmux", isDirectory: false),
            contents: """
            #!/bin/sh
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
    ...
    func testShellIntegrationRelayPortsKickOmitsSurfaceIDUntilAvailableInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-relay-kick-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let output = try runInteractiveZsh(
            ...
            command: """
            : > "\(logPath.path)"
            _bmux_ports_kick_via_relay refresh
            repeat 20; do
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-relay-precmd-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let output = try runInteractiveZsh(
            ...
            command: """
            : > "\(logPath.path)"
            _BMUX_TTY_REPORTED=1
            _BMUX_PORTS_LAST_RUN=-999
            _bmux_precmd
            repeat 20; do
              [[ -s "\(logPath.path)" ]] && break
              sleep 0.05
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-bash-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
    ...
    func testShellIntegrationRelayPreexecWorksBeforeSurfaceIDExistsInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-bash-relay-preexec-no-surface-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let result = try runInteractiveBash(
            ...
            command: """
            : > "\(logPath.path)"
            _BMUX_TTY_NAME=ttys889
            _BMUX_TTY_REPORTED=0
            _bmux_preexec_command "python3 -m http.server 8899"
            for _bmux_i in $(seq 1 20); do
              [ -s "\(logPath.path)" ] && break
              sleep 0.05
            done
            cat "\(logPath.path)"
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInBashWithoutPromptNoise() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-bash-relay-prompt-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        XCTAssertFalse(result.stderr.contains("_bmux_report_tmux_state"), result.stderr)
        XCTAssertTrue(
    ...
    func testBashNoGitWatchSkipsHeadTrackingAndPRClear() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-bash-no-git-watch-\(UUID().uuidString)")
        ...
        let socketPath = root.appendingPathComponent("bmux-test.sock", isDirectory: false)
        ...
        let result = try runInteractiveBash(
            ...
            cd "\(repoA.path)"
            _BMUX_TTY_REPORTED=1
            _BMUX_PORTS_LAST_RUN=$(_bmux_now)
            _BMUX_PWD_LAST_PWD="$PWD"
            _BMUX_GIT_HEAD_LAST_PWD="$PWD"
            _BMUX_GIT_HEAD_PATH="$PWD/.git/HEAD"
            _BMUX_GIT_HEAD_SIGNATURE="$(_bmux_git_head_signature "$_BMUX_GIT_HEAD_PATH")"
            printf '%s\\n' 'ref: refs/heads/old-cleared' > "$_BMUX_GIT_HEAD_PATH"
            cd "\(repoB.path)"
            _BMUX_PWD_LAST_PWD="$PWD"
            _BMUX_LAST_PR_ACTION="checkout"
            _BMUX_LAST_PR_TARGET="feature"
            _bmux_prompt_command
            printf 'HEAD_PATH=%s\\n' "$_BMUX_GIT_HEAD_PATH"
            printf 'HEAD_LAST_PWD=%s\\n' "$_BMUX_GIT_HEAD_LAST_PWD"
            ...
            extraEnvironment: [
                "BMUX_NO_GIT_WATCH": "1",
                "BMUX_SOCKET_PATH": socketPath.path,
                "BMUX_TAB_ID": "22222222-2222-2222-2222-222222222222",
                "BMUX_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
    ...
    func testZshNoGitWatchSkipsHeadTrackingAndPRClear() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-no-git-watch-\(UUID().uuidString)")
        let repoA = root.appendingPathComponent("repo-a", isDirectory: true)
        let repoB = root.appendingPathComponent("repo-b", isDirectory: true)
        let logPath = root.appendingPathComponent("send.log", isDirectory: false)
        let socketPath = root.appendingPathComponent("bmux-test.sock", isDirectory: false)
    ...
    func testZshNoPullRequestWatchSkipsLegacyGhPRProbe() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-no-pr-watch-\(UUID().uuidString)")
        ...
        try writeExecutableScript(
            ...
            contents: """
            #!/bin/sh
            printf invoked > "$BMUX_GH_MARKER"
            printf '2746\\tOPEN\\thttps://github.com/manaflow-ai/bmux/pull/2746\\n'
            """
        )
        let socketFD = try bindUnixSocket(at: socketPath.path)
        defer {
        ...
        let output = try runInteractiveZsh(
            ...
            _bmux_send() { :; }
            _bmux_send_bg() { :; }
            _bmux_report_pr_for_path "\(repoURL.path)" || true
            [[ -e "\(markerURL.path)" ]] && print MARKER=1 || print MARKER=0
            """,
            extraEnvironment: [
                "BMUX_NO_PR_WATCH": "1",
                "BMUX_GH_MARKER": markerURL.path,
                "BMUX_SOCKET_PATH": socketPath.path,
    ...
    func testBashNoPullRequestWatchSkipsLegacyGhPRProbe() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-bash-no-pr-watch-\(UUID().uuidString)")
        let repoURL = root.appendingPathComponent("repo", isDirectory: true)
        let fakeBinURL = root.appendingPathComponent("fake-bin", isDirectory: true)
        let markerURL = root.appendingPathComponent("gh-pr-invoked", isDirectory: false)
        let socketPath = root.appendingPathComponent("bmux-test.sock", isDirectory: false)
    ...
    func testZshPromptResetsTerminalKeyboardProtocols() throws {
        let output = try runInteractiveZsh(
            ...
            extraEnvironment: [
                "BMUX_SOCKET_PATH": "127.0.0.1:64011",
                "BMUX_BUNDLED_CLI_PATH": "/usr/bin/true",
                "BMUX_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "BMUX_TAB_ID": "22222222-2222-2222-2222-222222222222",
                "BMUX_PANEL_ID": "22222222-2222-2222-2222-222222222222",
                "BMUX_TEST_FORCE_KITTY_RESET": "1",
            ]
        )
    ...
    private func runInteractiveZsh(bmuxLoadGhosttyIntegration: Bool) throws -> String {
        try runInteractiveZsh(
            bmuxLoadGhosttyIntegration: bmuxLoadGhosttyIntegration,
            bmuxLoadShellIntegration: false,
    ...
    private func runInteractiveZsh(
        bmuxLoadGhosttyIntegration: Bool,
        bmuxLoadShellIntegration: Bool,
        command: String,
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-shell-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let bmuxZdotdir = repoRoot.appendingPathComponent("Resources/shell-integration")
        ...
        process.environment = [
            ...
            "SHELL": "/bin/zsh",
            "USER": NSUserName(),
            "ZDOTDIR": bmuxZdotdir.path,
            "BMUX_ZSH_ZDOTDIR": userZdotdir.path,
            "BMUX_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
        ]
        if bmuxLoadGhosttyIntegration {
            process.environment?["BMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
        if bmuxLoadShellIntegration {
            process.environment?["BMUX_SHELL_INTEGRATION"] = "1"
            process.environment?["BMUX_SHELL_INTEGRATION_DIR"] = bmuxZdotdir.path
            process.environment?["BMUX_SOCKET_PATH"] = root.appendingPathComponent("bmux-test.sock").path
            process.environment?["BMUX_TAB_ID"] = "tab-test"
            process.environment?["BMUX_PANEL_ID"] = "panel-test"
        }
    ...
    private func runPromptInteractiveZsh(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zsh-prompt-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        process.environment = [
            ...
            "BMUX_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
            "BMUX_TEST_READY": readyPath.path,
            "BMUX_TEST_OUTPUT": outputPath.path,
        ]
        if bmuxLoadGhosttyIntegration {
            process.environment?["BMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
    ...
    private func runInteractiveBash(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-bash-shell-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let integrationPath = repoRoot.appendingPathComponent("Resources/shell-integration/bmux-bash-integration.bash")
        let rcfilePath = root.appendingPathComponent(".bashrc")
        let rcfileContents: String = {
            guard bmuxLoadShellIntegration else { return ":\n" }
            return """
...
final class BrowserInstallDetectorTests: XCTestCase {
    ...
    private func makeTemporaryHome() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("bmux-browser-detect-\(UUID().uuidString)")
    }
