...
final class WorkspaceRemoteConnectionTests: XCTestCase {
    ...
    private func runRelayZshHistfile(
        ...
        let home = fileManager.temporaryDirectory.appendingPathComponent("bmux-relay-zsh-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".bmux/relay/64011.shell")
    ...
    private func runGeneratedBashBootstrapMarkers(startupFiles: [String: String]) throws -> [String] {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("bmux-relay-bash-\(UUID().uuidString)")
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "TERM=xterm-256color",
                "USER=\(NSUserName())",
                "BMUX_BASH_MARKERS=\(markerFile.path)",
                "/bin/sh",
                "-c",
                script,
            ],
    ...
    func testGeneratedFallbackShellBootstrapPrependsBmuxBinOnce() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-fallback-shell-bootstrap-\(UUID().uuidString)")
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "TERM=xterm-256color",
                "USER=\(NSUserName())",
                "BMUX_CAPTURE_PATH=\(capturedPath.path)",
                "/bin/sh",
                "-c",
                script,
            ],
        ...
        let bmuxBinEntries = path.split(separator: ":")
            .filter { $0 == "\(home.path)/.bmux/bin" }
        XCTAssertEqual(bmuxBinEntries.count, 1, path)
    }

    func testRemoteRelayMetadataCleanupScriptRemovesMatchingSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("bmux-relay-cleanup-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".bmux/relay")
        let socketAddrURL = home.appendingPathComponent(".bmux/socket_addr")
    ...
    func testRemoteRelayMetadataCleanupScriptPreservesDifferentSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("bmux-relay-cleanup-preserve-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".bmux/relay")
        let socketAddrURL = home.appendingPathComponent(".bmux/socket_addr")
    ...
    func testRemoteStaleRelayListenerCleanupScriptKillsMatchingPersistentRelayListener() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("bmux-stale-relay-cleanup-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        ...
        try writeExecutableShellFile(
            ...
            34057 33681 /Users/bmux/.bmux/bin/bmuxd-remote/current/darwin-arm64/bmuxd-remote serve --stdio --persistent --slot ssh-c4ba8ab1
            34058 33681 /bin/zsh
            EOF
            """
        )
        ...
        let result = runProcess(
            ...
            arguments: [
                "PATH=\(bin.path):/usr/bin:/bin",
                "BMUX_KILL_LOG=\(killLog.path)",
                "/bin/sh",
                "-c",
                """
                kill() { printf '%s\\n' "$*" >> "$BMUX_KILL_LOG"; return 0; }
                \(script)
                """,
            ],
            timeout: 5
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(result.stdout.contains("bmux_stale_relay_killed pid=33681 children=34057 port=50446"), result.stdout)
    ...
    func testRemoteStaleRelayListenerCleanupScriptPreservesDifferentPersistentSlot() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("bmux-stale-relay-preserve-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        let killLog = root.appendingPathComponent("kill.log")
        try fileManager.createDirectory(at: bin, withIntermediateDirectories: true)
        try "".write(to: killLog, atomically: true, encoding: .utf8)
        ...
        try writeExecutableShellFile(
            ...
            cat <<'EOF'
            33681 1 /usr/sbin/sshd-session
            34057 33681 /Users/bmux/.bmux/bin/bmuxd-remote/current/darwin-arm64/bmuxd-remote serve --stdio --persistent --slot ssh-other
            EOF
            """
        )
    ...
    func testRemoteStaleRelayListenerCleanupScriptMatchesPersistentSlotExactly() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("bmux-stale-relay-slot-prefix-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        let killLog = root.appendingPathComponent("kill.log")
        try fileManager.createDirectory(at: bin, withIntermediateDirectories: true)
        try "".write(to: killLog, atomically: true, encoding: .utf8)
        ...
        try writeExecutableShellFile(
            ...
            cat <<'EOF'
            33681 1 /usr/sbin/sshd-session
            34057 33681 /Users/bmux/.bmux/bin/bmuxd-remote/current/darwin-arm64/bmuxd-remote serve --stdio --persistent --slot ssh-ab
            EOF
            """
        )
    ...
    func testRemoteStaleRelayListenerCleanupScriptKillsMetadataMatchedListenerWithoutChild() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("bmux-stale-relay-metadata-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        let relayDir = root.appendingPathComponent(".bmux/relay")
        let killLog = root.appendingPathComponent("kill.log")
        try fileManager.createDirectory(at: bin, withIntermediateDirectories: true)
        try fileManager.createDirectory(at: relayDir, withIntermediateDirectories: true)
        try "/Users/bmux/.bmux/bin/bmuxd-remote/current/darwin-arm64/bmuxd-remote".write(
        ...
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(
            result.stdout.contains("bmux_stale_relay_killed pid=33681 children= port=50446 reason=metadata"),
            result.stdout
        )
    ...
    func testRemoteStaleRelayListenerCleanupScriptPreservesMetadataMatchedDifferentPersistentSlot() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("bmux-stale-relay-metadata-preserve-\(UUID().uuidString)")
        let bin = root.appendingPathComponent("bin")
        let relayDir = root.appendingPathComponent(".bmux/relay")
        let killLog = root.appendingPathComponent("kill.log")
        try fileManager.createDirectory(at: bin, withIntermediateDirectories: true)
    ...
    func testDaemonSocketForwardArgumentsTargetBakedVMSocket() {
        ...
        let arguments = configuration.daemonSocketForwardArguments(
            localPort: 64123,
            remoteSocketPath: "/run/bmuxd-remote.sock"
        )
        ...
        XCTAssertTrue(arguments.contains("127.0.0.1:64123:/run/bmuxd-remote.sock"))
        XCTAssertEqual(arguments.last, "bmux-macmini")
    }
    ...
    func testProxyBrokerTransportKeySeparatesVMBakedSSHFromStandardSSH() {
        let standard = WorkspaceRemoteConfiguration(
            ...
            port: 2222,
            identityFile: "/Users/test/.ssh/id_ed25519",
            sshOptions: ["ControlPath /tmp/bmux-ssh-%C"],
            localProxyPort: nil,
            relayPort: 64099,
            relayID: "relay-a",
            relayToken: String(repeating: "a", count: 64),
            localSocketPath: "/tmp/bmux.sock",
        ...
        let vmWebSocket = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: nil,
            terminalStartupCommand: "bmux vm-pty-attach --id abcd1234",
    ...
    func testWebSocketVMWithoutDaemonEndpointSkipsProxyStartup() {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: nil,
            terminalStartupCommand: "bmux vm-pty-attach --id test-no-daemon",
            skipDaemonBootstrap: true
        )
    ...
    func testRemoteDaemonCapabilityErrorsUseUserFacingMessage() {
        ...
        XCTAssertEqual(
            message,
            "remote daemon does not support persistent SSH PTY sessions; reconnect the remote workspace to update bmux"
        )
        ...
        let rawError = NSError(domain: "bmux.remote.daemon", code: 43, userInfo: [
    ...
    func testWebSocketVMWithDaemonEndpointStartsProxyCapableConnection() {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: nil,
            terminalStartupCommand: "bmux vm-pty-attach --id test-with-daemon",
            daemonWebSocketEndpoint: WorkspaceRemoteWebSocketDaemonEndpoint(
    ...
    func testRemoteTerminalSurfaceLookupTracksOnlyActiveSSHSurfaces() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/bmux-debug-test.sock",
            terminalStartupCommand: "ssh bmux-macmini"
        )
    ...
    func testWebSocketRemoteTerminalEndLeavesConnectedStateWithinBoundedTime() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: "/tmp/bmux-debug-test.sock",
            terminalStartupCommand: "bmux vm-pty-attach --id issue-4509",
            daemonWebSocketEndpoint: WorkspaceRemoteWebSocketDaemonEndpoint(
    ...
    func testWebSocketRemoteTerminalEndWithoutStartupCommandStillDisconnects() throws {
        ...
        let initialConfig = WorkspaceRemoteConfiguration(
            ...
            relayToken: nil,
            localSocketPath: "/tmp/bmux-debug-test.sock",
            terminalStartupCommand: "bmux vm-pty-attach --id issue-4509-no-startup",
            daemonWebSocketEndpoint: WorkspaceRemoteWebSocketDaemonEndpoint(
                url: "wss://vm.example.invalid/daemon",
                headers: [:],
                token: "token",
    ...
    func testRemoteTerminalSessionEndPublishesDisconnectedDetailWithoutTransientNil() throws {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to bmux-macmini",
            target: "bmux-macmini"
        )
    ...
    func testRemoteReconnectingStateIsExposedInStatusPayload() {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .reconnecting,
            detail: "Reconnecting to bmux-macmini",
            target: "bmux-macmini"
        )
    ...
    func testTeardownRemoteConnectionRequestsControlMasterCleanupWhileStillConnecting() {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connecting,
            detail: "Connecting to bmux-macmini",
            target: "bmux-macmini"
        )
    ...
    func testTransferredRemoteSurfaceCleansUpControlMasterWhenSessionEndsInLocalWorkspace() throws {
        ...
        XCTAssertEqual(cleanupArguments.first?.suffix(2), ["exit", "bmux-macmini"])
    }
    ...
    func testRemoteDropPathUsesLowercasedExtensionAndProvidedUUID() throws {
        ...
        XCTAssertEqual(remotePath, "/tmp/bmux-drop-12345678-1234-1234-1234-1234567890ab.png")
    }
    ...
    func testDaemonBootstrapUploadUsesAbsoluteHomePathForScpDestination() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "bmux-remote-daemon-upload-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: directoryURL) }

        let fakeDaemonURL = directoryURL.appendingPathComponent("bmuxd-remote", isDirectory: false)
        ...
        let previousAllowLocalBuild = getenv("BMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD").map { String(cString: $0) }
        let previousDaemonBinary = getenv("BMUX_REMOTE_DAEMON_BINARY").map { String(cString: $0) }
        setenv("BMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD", "1", 1)
        setenv("BMUX_REMOTE_DAEMON_BINARY", fakeDaemonURL.path, 1)
        defer {
            if let previousAllowLocalBuild {
                setenv("BMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD", previousAllowLocalBuild, 1)
            } else {
                unsetenv("BMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD")
            }
            if let previousDaemonBinary {
                setenv("BMUX_REMOTE_DAEMON_BINARY", previousDaemonBinary, 1)
            } else {
                unsetenv("BMUX_REMOTE_DAEMON_BINARY")
            }
        }
            ...
            if executable == "/usr/bin/ssh" {
                ...
                if command.contains("uname -s") {
                    return (
                        status: 0,
                        stdout: """
                        __BMUX_REMOTE_HOME__=/home/test
                        __BMUX_REMOTE_OS__=Linux
                        __BMUX_REMOTE_ARCH__=x86_64
                        __BMUX_REMOTE_EXISTS__=no
                        """,
                        stderr: ""
                    )
                }
        ...
        XCTAssertTrue(
            destination.hasPrefix("test@hpc.example:/home/test/.bmux/bin/bmuxd-remote/"),
            "expected scp to target an absolute path under remote HOME, got \(destination)"
        )
        XCTAssertTrue(
            destination.contains("/linux-amd64/bmuxd-remote.tmp-"),
        ...
        let remoteProcessScript: RemoteProcessScript = { executable, arguments, _, _ in
            ...
            if executable == "/usr/bin/ssh" {
                ...
                if remoteDaemonServeCommand(command) {
                    return (
                        status: 0,
                        stdout: #"{"id":1,"ok":true,"result":{"name":"bmuxd-remote","version":"old","capabilities":["proxy.stream.push"]}}"# + "\n",
                        stderr: ""
                    )
                }
                if command.contains("mkdir -p") {
    ...
    func testPersistentReverseRelayCancelsStaleControlMasterForwardBeforeReusingRelayPort() throws {
        ...
        let remoteProcessScript: RemoteProcessScript = { executable, arguments, _, _ in
            ...
            if remoteDaemonServeCommand(command) {
                return (
                    status: 0,
                    stdout: #"{"id":1,"ok":true,"result":{"name":"bmuxd-remote","version":"dev","capabilities":["proxy.stream.push","pty.session","pty.session.token","pty.write.notification","pty.resize.notification","pty.session.persistent_daemon"]}}"# + "\n",
                    stderr: ""
                )
            }
            return (status: 0, stdout: "", stderr: "")
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/bmux-ssh-\(getuid())-64044-%C",
                "StrictHostKeyChecking=accept-new",
            ],
            localProxyPort: nil,
            relayPort: 64044,
            ...
            localSocketPath: "/tmp/bmux-stale-forward-test.sock",
            terminalStartupCommand: "ssh-pty-attach",
            preserveAfterTerminalExit: true,
    ...
    func testPersistentReverseRelayCleansStaleRemoteListenerAndRetriesControlMasterForward() throws {
        ...
        let remoteProcessScript: RemoteProcessScript = { executable, arguments, _, _ in
            ...
            let command = arguments.last ?? ""
            if command.contains("bmux_stale_relay_listener_cleanup=1") {
                lock.lock()
                cleanupInvoked = true
                ...
                return (
                    status: 0,
                    stdout: "bmux_stale_relay_killed pid=33681 children=34057 port=64045\n",
                    stderr: ""
                )
            }
            if command.contains("uname -s") {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "ControlMaster=auto",
                "ControlPersist=600",
                "ControlPath=/tmp/bmux-ssh-\(getuid())-64045-%C",
                "StrictHostKeyChecking=accept-new",
            ],
            localProxyPort: nil,
            relayPort: 64045,
            ...
            localSocketPath: "/tmp/bmux-stale-forward-retry.sock",
            terminalStartupCommand: "ssh-pty-attach",
            preserveAfterTerminalExit: true,
    ...
    func testDetachAttachDoesNotAdoptPersistentPTYSessionIDAcrossNilRelayWorkspaces() throws {
        ...
        let movedPanel = try XCTUnwrap(
            source.newTerminalSurface(
                inPane: sourcePaneID,
                focus: true,
                initialCommand: "bmux ssh-pty-attach",
                remotePTYSessionID: sessionID
            )
        )
    ...
    func testDetectedSSHUploadFailureCleansUpEarlierRemoteUploads() throws {
        let fileManager = FileManager.default
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "bmux-detected-ssh-upload-\(UUID().uuidString)",
            isDirectory: true
        )
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: directoryURL) }
    ...
    func testDetectsForegroundSSHSessionWithShortControlPathFlag() {
        ...
        XCTAssertEqual(session?.controlPath, "/tmp/bmux-ssh-%C")
        let scpArgs = session?.scpArgumentsForTesting(
            localPath: "/tmp/local.png",
            remotePath: "/tmp/bmux-drop-123.png"
        ) ?? []
        XCTAssertTrue(scpArgs.contains("ControlPath=/tmp/bmux-ssh-%C"))
    ...
    func testDetectsEternalTerminalSessionWithoutTreatingETPortAsSSHPort() {
        ...
        XCTAssertEqual(scpArgs.last, "lawrence@example.com:/tmp/bmux-drop-123.png")
    }
    ...
    func testDetectsEternalTerminalSessionWithBracketedIPv6ServerPortForSCP() {
        ...
        XCTAssertNil(session?.port)
        XCTAssertFalse(scpArgs.contains("-P"))
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1]:/tmp/bmux-drop-123.png")
    }
    ...
    func testDetectsEternalTerminalSessionWithFullIPv6ServerPortForSCP() {
        ...
        XCTAssertNil(session?.port)
        XCTAssertFalse(scpArgs.contains("-P"))
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8:0:0:0:0:0:1]:/tmp/bmux-drop-123.png")
    }
    ...
    func testDetectsEternalTerminalSessionPreservesAmbiguousCompressedIPv6LiteralForSCP() {
        ...
        XCTAssertNil(session?.port)
        XCTAssertFalse(scpArgs.contains("-P"))
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1:2022]:/tmp/bmux-drop-123.png")
    }
    ...
    func testDaemonTransportArgumentsReuseConfiguredControlPath() {
        ...
        let arguments = configuration.daemonTransportArguments(
            remotePath: "/remote/bmuxd-remote"
        )

        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains(where: { $0 == "ControlPath /tmp/bmux-ssh-%C" || $0 == "ControlPath=/tmp/bmux-ssh-%C" }))
        XCTAssertTrue(arguments.contains("bmux-macmini"))
        XCTAssertTrue(arguments.last?.contains("/remote/bmuxd-remote") ?? false)
    }
    ...
    func testReverseRelayControlMasterArgumentsReuseConfiguredControlSocket() throws {
        ...
        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains("ControlPath=/tmp/bmux-ssh-%C"))
    ...
    func testProxyOnlyErrorsKeepSSHWorkspaceConnectedAndLoggedInSidebar() {
        ...
        let proxyError = "Remote proxy to bmux-macmini unavailable: Failed to start local daemon proxy: daemon RPC timeout waiting for hello response (retry in 3s)"
        workspace.applyRemoteConnectionStateUpdate(.error, detail: proxyError, target: "bmux-macmini")

        XCTAssertEqual(workspace.remoteConnectionState, .connected)
        XCTAssertEqual(workspace.remoteConnectionDetail, proxyError)
        XCTAssertEqual(
            workspace.statusEntries["remote.error"]?.value,
            "Remote proxy unavailable (bmux-macmini): \(proxyError)"
        )
        ...
        workspace.applyRemoteConnectionStateUpdate(.connecting, detail: "Connecting to bmux-macmini", target: "bmux-macmini")
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to bmux-macmini via shared local proxy 127.0.0.1:9999",
            target: "bmux-macmini"
        )
    ...
    func testDefaultCloudProxyOnlyErrorsDoNotPolluteConnectedSidebar() {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            terminalStartupCommand: "bmux vm ssh-attach --id 71smiccrg35sw9pydt8k --default-freestyle-sshd",
            preserveAfterTerminalExit: true,
            persistentDaemonSlot: "bmux-default-freestyle-sshd-v1",
            skipDaemonBootstrap: true
        )
...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    func testAgentHookLaunchEnvironmentDoesNotPersistPathOrShell() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hook-\(UUID().uuidString)", isDirectory: true)
        ...
        environment["BMUX_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated([
            "/usr/local/bin/codex",
            "--model",
            "gpt-5.4",
            "old prompt"
        ])
        environment["BMUX_AGENT_LAUNCH_CWD"] = "/tmp/repo"
    ...
    func testCodexHookStopSetsRateLimitStatusFromTranscript() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-hook-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testCodexPromptSubmitRetiresPreviousMonitorLeaseForSameSession() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-monitor-leases-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testCodexHookMonitorSetsErrorStatusFromCompletedTranscriptWithoutAssistant() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-monitor-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testNotifyWithWorkspaceHandleKeepsCallerSurfaceFallback() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_WORKSPACE_ID"] = "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA"
        environment["BMUX_SURFACE_ID"] = staleSurface
    ...
    func testTriggerFlashFallsBackFromStaleCallerWorkspaceAndSurfaceIDs() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_WORKSPACE_ID"] = staleWorkspace
        environment["BMUX_SURFACE_ID"] = staleSurface
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testSSHCommandCreatesConfiguresAndSelectsRemoteWorkspaceViaCLI() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=bmux-macmini state=disconnected\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "bmux-macmini")
        ...
        XCTAssertTrue(sshOptions.contains("ControlPath /tmp/bmux-ssh-%C"))
        XCTAssertTrue(sshOptions.contains("StrictHostKeyChecking=accept-new"))

        // `bmux ssh` should land the user in the new SSH workspace immediately.
    ...
    func testSSHCommandDoesNotDeferReconnectWhenWhitespaceControlMasterDisablesMultiplexing() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=bmux-macmini state=connecting\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
    ...
    func testNotifyPrefersCallerTTYOverFocusedSurfaceWhenCallerIDsAreStale() throws {
        ...
        environment["BMUX_WORKSPACE_ID"] = staleWorkspace
        environment["BMUX_SURFACE_ID"] = staleSurface
        environment["BMUX_CLI_TTY_NAME"] = callerTTY
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
