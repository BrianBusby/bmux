...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    func testGenericHookAgentsPersistSanitizedLaunchCommandsForSessionRestore() throws {
        let scenarios: [GenericHookPersistenceScenario] = [
            ...
            GenericHookPersistenceScenario(
                ...
                executable: "/tmp/bmux-agent-upstreams/copilot-install/bin/copilot",
                launchArguments: [
                    "/tmp/bmux-agent-upstreams/copilot-install/bin/copilot",
                    "--model",
                    "gpt-5.4",
                    "--resume=old-session",
                    "--allow-all-tools",
    ...
    func testAntigravityStopAndNotificationsUseGenericNotificationPath() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-antigravity-notification-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertTrue(
            ...
            "Expected Antigravity permission notifications to publish through bmux, saw \(permissionCommands)"
        )
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            ...
            "Expected Antigravity Stop errors to publish through bmux, saw \(stopErrorCommands)"
        )
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            ...
            "Expected Antigravity error notifications to publish through bmux, saw \(errorCommands)"
        )
        XCTAssertTrue(
    ...
    func testHermesAgentNotificationsUseShellHookExtraPayload() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-hermes-notification-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testHermesAgentSessionEndIsTurnBoundaryButFinalizeTearsDown() throws {
        ...
        // separate `on_session_finalize` hook once at genuine teardown. bmux maps the
        ...
        // See https://github.com/manaflow-ai/bmux/issues/5000.
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("hermes-session-end")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-hermes-session-end-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testAntigravityHookInstallUsesNativeHooksJSONShape() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-antigravity-hook-install-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        ...
        let result = runProcess(
            ...
            environment: [
                "HOME": root.path,
                "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
                "BMUX_BUNDLED_CLI_PATH": root.path,
                "BMUX_CLI_SENTRY_DISABLED": "1",
            ],
            timeout: 5
        )
        ...
        let bmuxGroup = try XCTUnwrap(json["bmux"] as? [String: Any])
        let allCommands = bmuxGroup.values
        ...
        XCTAssertTrue(
            allCommands.allSatisfy { $0.contains("bmux-antigravity-hook-v2") },
        ...
        XCTAssertFalse(
            ...
            "Directory-valued BMUX_BUNDLED_CLI_PATH must not be embedded as a hook executable, saw \(allCommands)"
        )
        XCTAssertFalse(
            allCommands.contains { $0.contains(#"[ -n "$BMUX_SURFACE_ID" ]"#) },
            "Antigravity hooks must still dispatch when agy does not preserve BMUX_SURFACE_ID, saw \(allCommands)"
        )

        let preToolUse = try XCTUnwrap(bmuxGroup["PreToolUse"] as? [[String: Any]])
        ...
        let stop = try XCTUnwrap(bmuxGroup["Stop"] as? [[String: Any]])
        XCTAssertTrue(
            stop.contains {
        ...
        XCTAssertNotNil(bmuxGroup["SessionStart"])
        XCTAssertNotNil(bmuxGroup["SessionEnd"])
        XCTAssertNotNil(bmuxGroup["turn-completion"])
        XCTAssertNotNil(bmuxGroup["Notification"])
        XCTAssertNotNil(bmuxGroup["PostToolUse"])
    }

    func testKiroHookInstallUsesAgentConfigShapeAndPreservesDenyExit() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-kiro-hook-install-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(
            result.stdout.contains("kiro-cli chat --agent bmux"),
            "Expected Kiro install to print the --agent bmux activation hint, saw: \(result.stdout)"
        )
        ...
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: hookURL)) as? [String: Any])
        XCTAssertEqual(json["name"] as? String, "bmux")
        XCTAssertNil(json["version"], "Kiro agent configs should not receive Cursor's hooks version field")
        XCTAssertEqual(
            json["tools"] as? [String], ["*"],
            "Kiro bmux agent must grant the full tool set so `--agent bmux` can run tools and fire preToolUse hooks"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected Kiro preToolUse feed hook to preserve bmux's exit status for deny decisions, saw \(preToolUse)"
        )
    ...
    func testKiroFeedDenyUsesPreToolUseExitCodeTwo() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-kiro-feed-deny-\(UUID().uuidString)", isDirectory: true)
        ...
        let result = runProcess(
            ...
            environment: [
                ...
                "BMUX_WORKSPACE_ID": workspaceId,
                "BMUX_SURFACE_ID": surfaceId,
                "BMUX_KIRO_PID": "525252",
                "BMUX_KIRO_NOTIFICATION_LEVEL": "standard",
                "BMUX_CLI_SENTRY_DISABLED": "1",
            ],
        ...
        XCTAssertTrue(result.stderr.contains("User denied permission via bmux Feed."), result.stderr)
    ...
    func testKiroFeedAllowModesProceedAndUnknownModeDenies() throws {
        func runKiroDecision(mode: String) throws -> ProcessRunResult {
            ...
            let state = MockSocketServerState()
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("bmux-kiro-feed-mode-\(UUID().uuidString)", isDirectory: true)
            try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
            defer {
                Darwin.close(listenerFD)
                unlink(socketPath)
    ...
    func testKiroStandardLevelSuppressesReadOnlyToolFeedEvents() throws {
        func feedPushCount(forTool tool: String) throws -> Int {
            ...
            let state = MockSocketServerState()
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("bmux-kiro-suppress-\(UUID().uuidString)", isDirectory: true)
            try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
            defer {
                Darwin.close(listenerFD)
                unlink(socketPath)
    ...
    func testLowercaseGenericFeedToolsStayTelemetryOutsideKiro() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-generic-lowercase-feed-tool-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        let surfaceId = "44444444-4444-4444-4444-444444444444"
    ...
    func testAntigravityFeedHookMissingSessionIdUsesStableFallback() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-antigravity-feed-stable-session-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        let surfaceId = "44444444-4444-4444-4444-444444444444"
    ...
    func testGrokNotificationHookUsesPayloadMessageAndStopDoesNotSendGenericNotification() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-notification-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testGrokStopFallbackCompletionsFireForTwoConcurrentThreads() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-two-threads-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceIds = [
    ...
    func testGrokStopNotificationFallsBackWhenTranscriptCwdIsUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-stop-without-cwd-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testGrokNotificationStillFiresOnRepeatedPromptWhenFeedTelemetryDoesNotReply() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-repeat-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
    ...
    func testGrokSessionEndDoesNotDropRoutingForLaterChatMessages() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-turns-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        for index in 1...2 {
            ...
            XCTAssertTrue(
                ...
                "Expected Grok prompt \(index) to reuse the saved target without BMUX env, saw \(promptCommands)"
            )
            XCTAssertTrue(
    ...
    func testGrokCompletionDoesNotResetStatusWhileSiblingSessionRuns() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-sibling-status-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
    ...
    func testGrokCompletionResetsStatusWhenSiblingRunningRecordHasDeadPID() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-stale-sibling-status-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        ...
        let environment: [String: String] = [
            ...
            "BMUX_CLI_SENTRY_DISABLED": "1",
            "BMUX_WORKSPACE_ID": workspaceId,
            "BMUX_SURFACE_ID": completingSurfaceId,
        ]
    ...
    func testGrokHookInstallRoutesNotificationEventToNotificationSubcommand() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-hook-install-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
            ...
            "hooks": [
                "PostToolUse": [
                    [
                        "hooks": [
                            [
                                "command": "[ -n \"$BMUX_SURFACE_ID\" ] && [ \"$BMUX_GROK_HOOKS_DISABLED\" != \"1\" ] && command -v bmux >/dev/null 2>&1 && bmux hooks feed --source grok --event PostToolUse || echo '{}'",
                                "timeout": 120,
                                "type": "command",
                            ],
                        ],
                ...
                "Stop": [
                    [
                        "hooks": [
                            [
                                "command": "[ -n \"$BMUX_SURFACE_ID\" ] && [ \"$BMUX_GROK_HOOKS_DISABLED\" != \"1\" ] && command -v bmux >/dev/null 2>&1 && bmux hooks grok stop || echo '{}'",
                                "timeout": 5,
                                "type": "command",
                            ],
                        ],
        ...
        let hookURL = root
            .appendingPathComponent(".grok", isDirectory: true)
            .appendingPathComponent("hooks", isDirectory: true)
            .appendingPathComponent("bmux-session.json", isDirectory: false)
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: hookURL)) as? [String: Any])
        let hooks = try XCTUnwrap(json["hooks"] as? [String: Any])
        ...
        XCTAssertTrue(
            notificationCommands.contains { $0.contains("bmux hooks grok notification") },
            "Expected Grok Notification to dispatch to the notification handler, saw \(notificationCommands)"
        )
        XCTAssertFalse(
            notificationCommands.contains { $0.contains("bmux hooks grok stop") },
        ...
        XCTAssertFalse(
            allCommands.contains { $0.contains("[ -n \"$BMUX_SURFACE_ID\" ]") },
            "Grok strips BMUX_* from hook subprocesses, so installed commands must not gate on BMUX_SURFACE_ID. Saw \(allCommands)"
        )
        XCTAssertFalse(
            allCommands.contains { $0.contains("$BMUX_") },
            "Grok treats $VAR references as required hook environment, so installed commands must avoid BMUX variable interpolation. Saw \(allCommands)"
        )
        XCTAssertFalse(
            FileManager.default.fileExists(atPath: legacyHookURL.path),
            "Expected setup to remove legacy bmux-owned Grok hook file"
        )
    }

    func testGrokHookInstallPinsInstallingCLIAndSocketWithoutBMUXInterpolation() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-hook-pin-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let pinnedCLI = root.appendingPathComponent("bmux pinned dev cli", isDirectory: false)
        ...
        let socketPath = "/tmp/bmux-debug-grok-pin.sock"
        let result = runProcess(
            executablePath: cliPath,
            arguments: ["hooks", "grok", "install", "--yes"],
            environment: [
                "HOME": root.path,
                "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
                "BMUX_BUNDLED_CLI_PATH": pinnedCLI.path,
                "BMUX_SOCKET_PATH": socketPath,
                "BMUX_CLI_SENTRY_DISABLED": "1",
            ],
            timeout: 5
        ...
        XCTAssertFalse(allCommands.isEmpty)
        XCTAssertTrue(
            allCommands.allSatisfy { $0.contains("bmux-grok-hook-v2") },
        ...
        XCTAssertFalse(
            allCommands.contains { $0.contains("$BMUX_") },
            "Grok hook commands must not depend on BMUX environment interpolation, saw \(allCommands)"
        )
    }
    ...
    func testGrokHookInstallPreservesUserWrappedLegacyCommands() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-hook-preserve-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let preservedCommand = "bash -lc 'bmux hooks grok notification && ~/bin/after-grok'"
        let legacyHookJSON: [String: Any] = [
            "hooks": [
                "Notification": [
                    [
                        "hooks": [
                            ...
                            [
                                "command": "[ \"$BMUX_GROK_HOOKS_DISABLED\" != \"1\" ] && command -v bmux >/dev/null 2>&1 && bmux hooks grok notification || echo '{}'",
                                "timeout": 10,
                                "type": "command",
                            ],
                        ],
        ...
        XCTAssertFalse(
            commands.contains { $0.hasPrefix("[ \"$BMUX_GROK_HOOKS_DISABLED\"") },
            "Expected setup to remove only exact bmux-owned legacy commands, saw \(commands)"
        )
    }
    ...
    func testGrokHookInstallPreservesLegacyFileMetadataWhenPruningOwnedHooks() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-hook-metadata-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testCodexHookInstallPrefersLaunchingAppBundledCLI() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-hook-install-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }

        let previousBundledHookCommand = "bmux_cli=\"${BMUX_BUNDLED_CLI_PATH:-}\"; if [ -z \"$bmux_cli\" ] || [ ! -x \"$bmux_cli\" ]; then bmux_cli=\"$(command -v bmux 2>/dev/null || true)\"; fi; [ -n \"$BMUX_SURFACE_ID\" ] && [ \"$BMUX_CODEX_HOOKS_DISABLED\" != \"1\" ] && [ -n \"$bmux_cli\" ] && \"$bmux_cli\" hooks codex prompt-submit || echo '{}'"
        let legacyHookJSON: [String: Any] = [
            "hooks": [
                "UserPromptSubmit": [
                    [
        ...
        XCTAssertTrue(
            commandBodies.contains {
                $0.contains("BMUX_BUNDLED_CLI_PATH")
                    && $0.contains("\"$bmux_cli\" --socket \"$BMUX_SOCKET_PATH\" hooks codex prompt-submit")
            },
            "Codex hooks should route through the launching app's bundled CLI, saw \(commandBodies)"
        )
        XCTAssertFalse(
            commandBodies.contains { $0.contains("command -v bmux >/dev/null 2>&1 && bmux hooks codex") },
            "Codex hooks must not use the reload-global bmux shim directly, saw \(commandBodies)"
        )
        XCTAssertFalse(
            commandBodies.contains { $0 == previousBundledHookCommand },
            "Codex setup should replace bundled-CLI hooks that did not pin BMUX_SOCKET_PATH, saw \(commandBodies)"
        )
        XCTAssertEqual(
            allCommands.filter { $0.contains("hooks codex prompt-submit") }.count,
            1,
            "Codex setup should collapse duplicate bmux-owned prompt hooks to one entry, saw \(allCommands)"
        )
    }
    ...
    func testGrokHookInstallRejectsFileAtHooksDirectory() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-grok-hook-file-dir-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertTrue(
            result.stderr.contains("bmux could not create the hooks directory: a file exists at \(hooksPath.path); remove or rename the conflicting file and re-run `bmux hooks setup`"),
            result.stderr
        )
        XCTAssertFalse(
    ...
    func runGenericHookPersistenceScenario(_ scenario: GenericHookPersistenceScenario) throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-\(scenario.agent)-hook-\(UUID().uuidString)", isDirectory: true)
        ...
        var environment: [String: String] = [
            ...
            "BMUX_SURFACE_ID": surfaceId,
            "BMUX_AGENT_HOOK_STATE_DIR": root.path,
            "BMUX_AGENT_LAUNCH_KIND": scenario.agent,
            "BMUX_AGENT_LAUNCH_EXECUTABLE": scenario.executable,
            "BMUX_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(scenario.launchArguments),
            "BMUX_AGENT_LAUNCH_CWD": workspace.path,
            "BMUX_CLI_SENTRY_DISABLED": "1",
        ]
        ...
        if scenario.agent == "kiro" {
            ...
            XCTAssertEqual(
                ...
                "cd -- '\(workspace.path)' 2>/dev/null || [ ! -d '\(workspace.path)' ] && '\(scenario.executable)' 'chat' '--resume-id' '\(scenario.sessionId)' '--agent' 'bmux' '--trust-tools' 'fs_read,fs_write'"
            )
    ...
    /// G2 (https://github.com/manaflow-ai/bmux/issues/5350): plain `codex` under the subrouter account
    /// manager points CODEX_HOME at ~/.codex-accounts/<account>, not ~/.codex. When the launch argv
    /// can't be captured (no BMUX_AGENT_LAUNCH_ARGV_B64 and an exited PID), the session record used to
    ...
    func testCodexHookPreservesCodexHomeWhenLaunchCommandUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-home-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        environment["BMUX_SURFACE_ID"] = surfaceId
        environment["BMUX_CLI_TTY_NAME"] = ttyName
        environment["BMUX_AGENT_HOOK_STATE_DIR"] = root.path
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["CODEX_HOME"] = codexHome
        for key in ["BMUX_AGENT_LAUNCH_KIND", "BMUX_AGENT_LAUNCH_EXECUTABLE", "BMUX_AGENT_LAUNCH_ARGV_B64", "BMUX_AGENT_LAUNCH_CWD"] {
    ...
    /// G3 (https://github.com/manaflow-ai/bmux/issues/5333): the codex surface jumble. BMUX_SURFACE_ID
    ...
    func testCodexHookOverridesLeakedEnvSurfaceWithProcessTTYBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-surface-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let leakedSurfaceId = "22222222-2222-2222-2222-222222222222"   // env BMUX_SURFACE_ID (wrong)
        ...
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_WORKSPACE_ID"] = workspaceId
        environment["BMUX_SURFACE_ID"] = leakedSurfaceId
        environment["BMUX_CLI_TTY_NAME"] = ttyName
        environment["BMUX_AGENT_HOOK_STATE_DIR"] = root.path
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_AGENT_LAUNCH_KIND"] = "codex"
        environment["BMUX_AGENT_LAUNCH_EXECUTABLE"] = "/usr/local/bin/codex"
        environment["BMUX_AGENT_LAUNCH_CWD"] = root.path
        environment["BMUX_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated(["/usr/local/bin/codex"])
        ...
        XCTAssertEqual(
            ...
            "PID/TTY ground truth must override the leaked env BMUX_SURFACE_ID; params=\(params)"
        )
    }

    /// G3 stale-env variant (https://github.com/manaflow-ai/bmux/issues/5333): when the ambient
    /// BMUX_SURFACE_ID is stale/invalid (the surface was closed, or belongs to another workspace) it no
    ...
    func testCodexHookRecoversFromStaleEnvSurfaceViaProcessTTYBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-stale-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let staleSurfaceId = "22222222-2222-2222-2222-222222222222"   // env BMUX_SURFACE_ID, no longer exists
        ...
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_WORKSPACE_ID"] = workspaceId
        environment["BMUX_SURFACE_ID"] = staleSurfaceId
        environment["BMUX_CLI_TTY_NAME"] = ttyName
        environment["BMUX_AGENT_HOOK_STATE_DIR"] = root.path
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_AGENT_LAUNCH_KIND"] = "codex"
        ...
        XCTAssertEqual(
            params["surface_id"] as? String, ttySurfaceId,
            "a stale ambient BMUX_SURFACE_ID must fall through to the TTY pane; params=\(params)"
        )
    }
    ...
    func testCodexHookDoesNotPersistEnvOnlyRecordForNonRestorableExec() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-exec-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "11111111-1111-1111-1111-111111111111"
        let surfaceId = "22222222-2222-2222-2222-222222222222"
        ...
        environment["BMUX_AGENT_LAUNCH_ARGV_B64"] = base64NULSeparated(["/usr/local/bin/codex", "exec", "do a one-shot task"])
