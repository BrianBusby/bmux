...
final class SessionPersistenceTests: XCTestCase {
    ...
    private func sessionStore(
        bundleIdentifier: String? = "com.bmuxterm.tests",
    ...
    func testWorkspaceSessionSnapshotRestoresMarkdownPanel() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-session-markdown-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSaveAndLoadRoundTripWithCustomSnapshotPath() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-session-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testLoadReopenSessionSnapshotRequiresPreviousSnapshotFile() throws {
        ...
        let bundleIdentifier = "dev.bmux.tests.\(UUID().uuidString)"
    ...
    func testRestorePolicySkipsWhenLaunchHasExplicitArguments() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/bmux.app/Contents/MacOS/bmux", "--window", "window:1"],
            environment: [:]
        )
    ...
    func testRestorePolicyAllowsFinderStyleLaunchArgumentsOnly() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/bmux.app/Contents/MacOS/bmux", "-psn_0_12345"],
            environment: [:]
        )
    ...
    func testRestorePolicySkipsWhenRunningUnderXCTest() {
        let shouldRestore = SessionRestorePolicy.shouldAttemptRestore(
            arguments: ["/Applications/bmux.app/Contents/MacOS/bmux"],
    ...
    func testScrollbackReplayEnvironmentWritesReplayFile() {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-scrollback-replay-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    // Regression for https://github.com/manaflow-ai/bmux/issues/5165.
    //
    ...
    func testNormalizedExportedScreenPathAcceptsAbsoluteAndFileURL() {
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath("/tmp/bmux-screen.txt"),
            "/tmp/bmux-screen.txt"
        )
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath(" file:///tmp/bmux-screen.txt "),
            "/tmp/bmux-screen.txt"
        )
    }
    ...
    func testShouldRemoveExportedScreenDirectoryOnlyWithinTemporaryRoot() {
        let tempRoot = URL(fileURLWithPath: "/tmp")
            .appendingPathComponent("bmux-export-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeRestorableAgentIndex(
        ...
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hook-store-\(UUID().uuidString)", isDirectory: true)
...
final class SocketListenerAcceptPolicyTests: XCTestCase {
    func testClaudeResumeCommandRoutesThroughWrapperInsteadOfCapturedRealBinary() {
        ...
        // (BMUX_AGENT_LAUNCH_EXECUTABLE). Resuming with it directly bypasses
        // bmux's `claude` wrapper, which is what injects the hooks, so resumed
        // sessions silently lost SessionStart/Stop/Notification. Resume must use
        // the bare `claude` wrapper. https://github.com/manaflow-ai/bmux/issues/5427
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            sessionId: "claude-session-123",
            workingDirectory: "/tmp/bmux project",
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/bmux project' 2>/dev/null || [ ! -d '/tmp/bmux project' ] && /bin/sh -c "
                + shellQuotedForTest("'env' 'CLAUDE_CONFIG_DIR=/tmp/claude config' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR' \"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-123' '--model' 'sonnet' '--permission-mode' 'auto'")
        )
    ...
    func testClaudeForkCommandRoutesThroughWrapperInsteadOfCapturedRealBinary() throws {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                arguments: [
                    ...
                    "sonnet",
                    "--settings",
                    #"{"hooks":{"SessionStart":[{"hooks":[{"command":"bmux claude-hook session-start"}]}]}}"#,
                    "--session-id",
                    "old-session"
                ],
                workingDirectory: "/tmp/bmux project",
        ...
        // https://github.com/manaflow-ai/bmux/issues/5427
        ...
        XCTAssertFalse(command.contains("bmux claude-hook session-start"), command)
        XCTAssertFalse(command.contains("old-session"), command)
    }

    /// Regression for https://github.com/manaflow-ai/bmux/issues/5639.
    ///
    ...
    /// where bmux's shell integration (the `claude()` function + per-surface PATH shim)
    ...
    /// user's *real* binary, the bmux wrapper was bypassed, no hook `--settings` was
    ...
    /// and asserts the launched invocation routed through the bmux wrapper (its injected
    ...
    func testClaudeResumeCommandExecutesThroughWrapperInsideLoginShellLauncher() throws {
        ...
        let sandbox = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-5639-\(UUID().uuidString)", isDirectory: true)
        let shimDir = sandbox.appendingPathComponent("bmux-cli-shims", isDirectory: true)
        ...
        let wrapperURL = sandbox.appendingPathComponent("bmux-claude-wrapper", isDirectory: false)
        ...
        // Stand-in for bmux-claude-wrapper: re-inject the hook --settings on --resume
        ...
        for arg in "$@"; do
          if [[ "$arg" == "--resume" || "$arg" == "-r" ]]; then
            set -- --settings BMUX_HOOKS_JSON "$@"
            break
          fi
        done
        printf 'wrapper %s\\n' "$*" > \(shellQuotedForTest(recordURL.path))
        """)
        // Per-surface shim that bmux installs and points BMUX_CLAUDE_WRAPPER_SHIM at.
        ...
        // BMUX_CLAUDE_WRAPPER_SHIM is a managed terminal env var inherited by the -lic shell.
        environment["BMUX_CLAUDE_WRAPPER_SHIM"] = shimURL.path
        ...
        XCTAssertTrue(
            ...
            "Resumed claude must route through the bmux wrapper (which injects the hook --settings). Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
        XCTAssertTrue(
            recorded.hasPrefix("wrapper "),
            "Resume must exec the bmux wrapper, not the user's real claude binary. Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
    }
    ...
    /// `"$_bmux_resume_shell" -c <command>` for its `csh|tcsh` branch), and the
    ...
    /// under `zsh -lic`. https://github.com/manaflow-ai/bmux/issues/5639
    func testClaudeResumeCommandExecutesThroughWrapperInsideTcshLauncher() throws {
        ...
        XCTAssertTrue(
            recorded.hasPrefix("wrapper "),
            "tcsh-dispatched resume must parse and exec the bmux wrapper. Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
        XCTAssertTrue(
            recorded.contains("--settings"),
    ...
    /// is a shipped bmux integration (`Resources/shell-integration/fish/`). The restore
    /// launcher's `*)` branch dispatches `"$_bmux_resume_shell" -c <command>` for fish
    ...
    /// (`cd … || [ ! -d … ] && …`, https://github.com/manaflow-ai/bmux/issues/6285) is
    /// exercised too — older fish rejects the POSIX `{ …; }` grouping the guard used to
    /// emit. https://github.com/manaflow-ai/bmux/issues/5639
    func testClaudeResumeCommandExecutesThroughWrapperInsideFishLauncher() throws {
        ...
        XCTAssertTrue(
            recorded.hasPrefix("wrapper "),
            "fish-dispatched resume must parse and exec the bmux wrapper. Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
        XCTAssertTrue(
            recorded.contains("--settings"),
    ...
    /// Regression for the stale-shim fallback: `BMUX_CLAUDE_WRAPPER_SHIM` can outlive
    ...
    func testClaudeResumeCommandFallsBackToPathClaudeWhenShimFileIsStale() throws {
        ...
        let recorded = try runClaudeResumeCommand(
            ...
            environmentOverrides: [
                ...
                "BMUX_CLAUDE_WRAPPER_SHIM": reapedShimPath
            ]
        )
        XCTAssertTrue(
    ...
    /// restore-launcher regression tests: a recording stand-in for `bmux-claude-wrapper`
    /// (re-injects the hook `--settings` on `--resume`, as the real wrapper does), the
    /// per-surface shim that `BMUX_CLAUDE_WRAPPER_SHIM` points at, and a hook-less "real"
    ...
    /// profile sourcing, `BMUX_CLAUDE_WRAPPER_SHIM` from the managed terminal env).
    private func runClaudeResumeCommand(
        ...
        var environment = [
            ...
            "BMUX_CLAUDE_WRAPPER_SHIM": sandbox.shimURL.path
        ]
    ...
    func testRestorableAgentResumeStartupInputEscapesNonAsciiWorkingDirectoryAsAsciiShellInput() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-resume-\(UUID().uuidString)", isDirectory: true)
        let cwdURL = root
        ...
        // (https://github.com/manaflow-ai/bmux/issues/5639) exceed the inline
    ...
    func testSessionEntryClaudeResumeCommandChangesToSessionCwdBeforeResume() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-claude-resume-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testRestorableAgentStartupInputUsesLauncherScriptWhenCommandExceedsTerminalInputBudget() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-resume-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testClaudeResumeCommandPreservesDangerouslySkipPermissionsAndObservedEnvironment() {
        ...
        XCTAssertEqual(
            ...
            "cd -- '/Users/lawrence/fun' 2>/dev/null || [ ! -d '/Users/lawrence/fun' ] && /bin/sh -c "
                + shellQuotedForTest("'env' 'CLAUDE_CONFIG_DIR=/Users/lawrence/.codex-accounts/claude/_p1775010019397' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR' \"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' '24ec0052-450c-4914-b1dd-2ee80d4bc84b' '--dangerously-load-development-channels' 'server:custom-dev-channel' '--dangerously-skip-permissions'")
        )
    }
    ...
    func testCodexResumeCommandDropsStartupImagesAndPlacesSessionBeforeFlags() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            workingDirectory: "/Users/lawrence/fun/bmuxterm-hq",
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "codex",
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/Users/lawrence/fun/bmuxterm-hq' 2>/dev/null || [ ! -d '/Users/lawrence/fun/bmuxterm-hq' ] && '/Users/lawrence/.bun/bin/codex' 'resume' '019e2bb9-5544-7201-a517-d77bb00d724f' '-c' 'check_for_update_on_startup=false' '--yolo' '--model' 'gpt-5.4'"
        )
    }
    ...
    func testCodexTeamsResumeCommandUsesWrapperSubcommand() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "codexTeams",
                executablePath: "/usr/local/bin/bmux",
                arguments: [
                    "/usr/local/bin/bmux",
                    "codex-teams",
                    "--model",
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/Users/example/repo' 2>/dev/null || [ ! -d '/Users/example/repo' ] && 'env' 'CODEX_HOME=/tmp/codex home' '/usr/local/bin/bmux' 'codex-teams' 'resume' '019dad34-d218-7943-b81a-eddac5c87951' '-c' 'check_for_update_on_startup=false' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testCodexTeamsResumeCommandDropsOriginalForkTarget() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/Users/example/repo' 2>/dev/null || [ ! -d '/Users/example/repo' ] && 'env' 'CODEX_HOME=/tmp/codex home' '/usr/local/bin/bmux' 'codex-teams' 'resume' '019dad34-d218-7943-b81a-eddac5c87952' '-c' 'check_for_update_on_startup=false' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testForkCommandsUseVerifiedAgentForkSyntaxAndPreserveContext() {
        ...
        XCTAssertEqual(
            claude.forkCommand,
            "cd -- '/Users/lawrence/fun' 2>/dev/null || [ ! -d '/Users/lawrence/fun' ] && /bin/sh -c "
                + shellQuotedForTest("'env' 'CLAUDE_CONFIG_DIR=/Users/lawrence/.codex-accounts/claude/_p1775010019397' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR' \"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' '24ec0052-450c-4914-b1dd-2ee80d4bc84b' '--fork-session' '--dangerously-load-development-channels' 'server:custom-dev-channel' '--dangerously-skip-permissions'")
        )
        XCTAssertEqual(
            claudeFork.forkCommand,
            "cd -- '/Users/lawrence/fun' 2>/dev/null || [ ! -d '/Users/lawrence/fun' ] && /bin/sh -c "
                + shellQuotedForTest("'env' 'CLAUDE_CONFIG_DIR=/Users/lawrence/.codex-accounts/claude/_p1775010019397' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR' \"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-fork-child' '--fork-session' '--model' 'sonnet' '--dangerously-skip-permissions'")
        )
        XCTAssertEqual(
            codex.forkCommand,
        ...
        XCTAssertEqual(
            ...
            "cd -- '/Users/example/repo' 2>/dev/null || [ ! -d '/Users/example/repo' ] && 'env' 'CODEX_HOME=/tmp/codex home' '/usr/local/bin/bmux' 'codex-teams' 'fork' 'codex-teams-session' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/opencode repo' 2>/dev/null || [ ! -d '/tmp/opencode repo' ] && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/bmux' 'omo' '--session' 'opencode-session-123' '--fork' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
        XCTAssertEqual(
            omoOpenCodeFork.forkCommand,
            "cd -- '/tmp/opencode repo' 2>/dev/null || [ ! -d '/tmp/opencode repo' ] && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/bmux' 'omo' '--session' 'opencode-child-session' '--fork' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
    ...
    func testOpenCodeForkSupportProbesFromLaunchWorkingDirectory() async throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-opencode-probe-\(UUID().uuidString)", isDirectory: true)
    ...
    func testOpenCodeForkSupportSkipsLocalProbeForRemoteLikeContext() async {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            workingDirectory: "/remote/bmux/project-\(UUID().uuidString)",
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "opencode",
    ...
    func testAgentForkSupportRejectsRemoteForksThatNeedLauncherScript() async {
        let longPath = "/Users/bmux/" + String(repeating: "nested-project-", count: 120)
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .codex,
            sessionId: "019dad34-d218-7943-b81a-eddac5c87951",
            workingDirectory: "/Users/bmux/project",
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "codex",
                executablePath: "/Users/example/.bun/bin/codex",
                arguments: [
    ...
    func testOpenCodeForkSupportRejectsMissingLocalExecutable() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-opencode-missing-executable-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: root) }
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
    ...
    func testOpenCodeForkSupportCachesUnsupportedVersion() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-opencode-probe-cache-\(UUID().uuidString)", isDirectory: true)
        let executable = root.appendingPathComponent("opencode", isDirectory: false)
    ...
    func testProcessDetectedOpenCodeResolvesBareExecutableWithCapturedPath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-opencode-path-\(UUID().uuidString)", isDirectory: true)
    ...
    func testProcessDetectedOpenCodeWorkingDirectoryUsesProjectPositional() {
        ...
        XCTAssertEqual(
            RestorableAgentSessionIndex.openCodeWorkingDirectoryForProcess(
                ...
                environment: ["BMUX_AGENT_LAUNCH_CWD": "/tmp/hook-cwd", "PWD": "/tmp/shell-cwd"]
            ),
            "/tmp/hook-cwd"
        )
    }
    ...
    func testProcessDetectedOpenCodeLaunchArgumentsPreserveSafeForkContext() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-opencode-argv-\(UUID().uuidString)", isDirectory: true)
        let bin = root.appendingPathComponent("bin", isDirectory: true)
        let executable = bin.appendingPathComponent("opencode", isDirectory: false)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testClaudeTeamsResumeCommandPreservesRemoteControlLauncher() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                executablePath: "/Applications/bmux.app/Contents/Resources/bin/bmux",
                arguments: [
                ...
                workingDirectory: "/tmp/team repo",
                environment: [
                    "BMUX_CUSTOM_CLAUDE_PATH": "/opt/Claude Code/bin/claude",
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/tmp/team repo' 2>/dev/null || [ ! -d '/tmp/team repo' ] && 'env' 'BMUX_CUSTOM_CLAUDE_PATH=/opt/Claude Code/bin/claude' '/Applications/bmux.app/Contents/Resources/bin/bmux' 'claude-teams' '--resume' 'claude-team-session' '--teammate-mode' 'auto' '--model' 'sonnet' '--remote-control-session-name-prefix' 'bmux-team' '--permission-mode' 'auto'"
        )
    }
    ...
    func testClaudeResumeCommandHandlesOptionalDebugValueAndFilteredEnvironment() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "/bin/sh -c " + shellQuotedForTest("'env' 'NODE_OPTIONS=--max-old-space-size=4096' \"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-debug' '--debug' 'api,mcp' '--model' 'sonnet'")
        )
    }
    ...
    func testResumeCommandPreservesSafeProviderEnvironmentValuesOnly() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "/bin/sh -c " + shellQuotedForTest("'env' 'ANTHROPIC_BASE_URL=https://api.example.test' 'ANTHROPIC_MODEL=' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1' 'BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=ANTHROPIC_BASE_URL,ANTHROPIC_MODEL' \"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-env'")
        )
        XCTAssertFalse(snapshot.resumeCommand?.contains("ANTHROPIC_AUTH_TOKEN") ?? true)
    }

    func testClaudeResumeCommandStripsStaleBmuxNodeOptionsRestoreModule() {
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                workingDirectory: nil,
                environment: [
                    "NODE_OPTIONS": "--require=/tmp/bmux-claude-node-options/restore-node-options.cjs --max-old-space-size=4096 --trace-warnings"
                ],
                capturedAt: nil,
                source: nil
            )
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "/bin/sh -c " + shellQuotedForTest("'env' 'NODE_OPTIONS=--trace-warnings' \"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-node-options' '--model' 'sonnet'")
        )
    }

    func testClaudeResumeCommandDropsEmptyStaleBmuxNodeOptionsEnvironment() {
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                workingDirectory: nil,
                environment: [
                    "NODE_OPTIONS": "--require /tmp/bmux-claude-node-options/restore-node-options.cjs --max-old-space-size 4096"
                ],
                capturedAt: nil,
                source: nil
            )
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "/bin/sh -c " + shellQuotedForTest("\"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" '--resume' 'claude-session-empty-node-options' '--model' 'sonnet'")
        )
    }
    ...
    func testOpenCodeWrapperResumeCommandAndUnsupportedOhMyLaunchers() {
        ...
        let omx = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "omx",
                executablePath: "/usr/local/bin/bmux",
                arguments: ["/usr/local/bin/bmux", "omx", "team"],
                workingDirectory: nil,
                environment: nil,
                capturedAt: nil,
                source: nil
        ...
        let omc = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "omc",
                executablePath: "/usr/local/bin/bmux",
                arguments: ["/usr/local/bin/bmux", "omc", "team"],
                workingDirectory: nil,
                environment: nil,
                capturedAt: nil,
                source: nil
        ...
        XCTAssertEqual(
            omo.resumeCommand,
            "cd -- '/tmp/opencode repo' 2>/dev/null || [ ! -d '/tmp/opencode repo' ] && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/bmux' 'omo' '--session' 'opencode-session-123' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
        XCTAssertEqual(
    ...
    func testRestorableAgentIndexLoadsLaunchCommandFromHookStore() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hook-store-\(UUID().uuidString)", isDirectory: true)
        let storeDir = home.appendingPathComponent(".bmuxterm", isDirectory: true)
    ...
    func testAntigravityProcessDetectionDoesNotTreatTrailingFlagAsConversationID() throws {
        ...
        let processSnapshot = BmuxTopProcessSnapshot(
            processes: [
                BmuxTopProcessInfo(
                    pid: processId,
                    parentPID: 1,
                    ...
                    bmuxSurfaceID: panelId,
                    bmuxAttributionReason: "bmux-test",
                    processGroupID: nil,
        ...
        let registry = BmuxVaultAgentRegistry(registrations: [.builtInAntigravity])
...
extension SessionPersistenceTests {
    ...
    // Regression for https://github.com/manaflow-ai/bmux/issues/6285: the cwd guard runs
    ...
    // A binding persisted by an older bmux with the legacy braced guard must be
    ...
    func testAgentHookSurfaceResumeStartupInputRunsWhenSavedWorkingDirectoryWasDeleted() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-surface-resume-missing-cwd-\(UUID().uuidString)", isDirectory: true)
        let bin = root.appendingPathComponent("bin", isDirectory: true)
        ...
        try """
        #!/bin/zsh
        print -r -- "$PWD|$*" > "$BMUX_FAKE_CODEX_OUTPUT"
        ...
        environment["BMUX_FAKE_CODEX_OUTPUT"] = outputURL.path
        process.environment = environment
        let stderr = Pipe()
    ...
    func testSurfaceResumeBindingStartupInputUsesLauncherScriptWhenLong() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-surface-resume-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testSurfaceResumeApprovalMissingRecordResetsStalePromptPolicy() throws {
        ...
        let effectiveBinding = SurfaceResumeApprovalStore.applyingStoredApproval(
            to: binding,
            fileURL: URL(fileURLWithPath: "/tmp/bmux-missing-\(UUID().uuidString).json"),
    ...
    func testSurfaceResumeApprovalWritesRecordsIntoBmuxJSON() throws {
        let settingsURL = try makeSurfaceResumeApprovalBmuxSettingsURL()
        let secret = Data("approval-secret".utf8)
        let initialSettings = """
        {
          "$schema": "https://raw.githubusercontent.com/manaflow-ai/bmux/main/web/data/bmux.schema.json",
          // keep root comment
          "schemaVersion": 1,
          "terminal": {
    ...
    func testSurfaceResumeApprovalWritesNonUTF8BmuxJSON() throws {
        let settingsURL = try makeSurfaceResumeApprovalBmuxSettingsURL()
        let secret = Data("approval-secret".utf8)
        let initialSettings = """
        {
    ...
    func testSurfaceResumeApprovalMigratesLegacyRecordsIntoBmuxJSON() throws {
        let settingsURL = try makeSurfaceResumeApprovalBmuxSettingsURL()
    ...
    func testSurfaceResumeApprovalDoesNotOverwriteInvalidBmuxJSON() throws {
        let settingsURL = try makeSurfaceResumeApprovalBmuxSettingsURL()
        let legacyURL = settingsURL.deletingLastPathComponent()
            .appendingPathComponent("resume-commands.json", isDirectory: false)
        let secret = Data("approval-secret".utf8)
    ...
    func testHermesAgentHookSurfaceResumeBootstrapsSubrouterAndRewritesStaleCodexProvider() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-hermes-surface-resume-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeSurfaceResumeApprovalStoreURL() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-surface-resume-approvals-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        addTeardownBlock {
    ...
    private func makeSurfaceResumeApprovalBmuxSettingsURL() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-surface-resume-settings-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        addTeardownBlock {
            try? FileManager.default.removeItem(at: root)
        }
        return root.appendingPathComponent("bmux.json", isDirectory: false)
    }
    ...
    func testRestoreRunsSurfaceResumeBindingFromBindingCwd() throws {
        ...
        let bindingCwd = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-surface-binding-cwd-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRestoreDoesNotPassDeletedAgentHookCwdToTerminalRuntime() throws {
        try withAutoResumeAgentSessionsEnabled {
            ...
            let missingCwd = FileManager.default.temporaryDirectory
                .appendingPathComponent("bmux-deleted-agent-hook-cwd-\(UUID().uuidString)", isDirectory: true)
                .appendingPathComponent("repo", isDirectory: true)
            let bindingIndex = SurfaceResumeBindingIndex(bindingsByPanel: [
                SurfaceResumeBindingIndex.PanelKey(workspaceId: source.id, panelId: sourcePanelId): SurfaceResumeBindingSnapshot(
                    name: "Codex",
            ...
            // The guard is the fish-safe, brace-free form (https://github.com/manaflow-ai/bmux/issues/6285):
    ...
    func testRestorePreservesUnmountedVolumeCwdBindingsWhenInitialReportsAreScrambled() throws {
        try withAutoResumeAgentSessionsEnabled {
            ...
            let volumeName = "bmux-issue-5278-\(UUID().uuidString)"
    ...
    func testMarkdownFileLinkResolverPrefersCurrentMarkdownDirectory() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-markdown-link-resolver-\(UUID().uuidString)", isDirectory: true)
    ...
    func testMarkdownFileLinkResolverFallsBackToProcessWorkingDirectory() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-markdown-link-resolver-cwd-\(UUID().uuidString)", isDirectory: true)
        let docs = root.appendingPathComponent("docs", isDirectory: true)
        let openedFile = docs.appendingPathComponent("index.md")
    ...
    func testMarkdownFileLinkResolverRejectsMissingAndNonMarkdownFiles() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-markdown-link-resolver-reject-\(UUID().uuidString)", isDirectory: true)
