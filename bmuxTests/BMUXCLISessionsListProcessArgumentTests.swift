...
extension BMUXCLIErrorOutputRegressionTests {
    @Test func testSessionsListProcessArgumentsPreservesEmptyElements() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-live-argv-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func testSessionsListForkStartupInputCountsAsciiSafeNonASCIIQuoting() throws {
        let workingDirectory = "/tmp/bmux/\u{65e5}\u{672c}\u{8a9e}" + String(repeating: "x", count: 250)
        let session = try sessionsListDiagnosticSession(
            launcher: "codex",
    ...
    @Test func testSessionsListClaudeStartupInputCountsAuthPreservationEnvironment() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-sessions-list-claude-env-\(UUID().uuidString)", isDirectory: true)
