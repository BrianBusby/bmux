...
@Suite struct AutoNamingEngineTests {
    ...
    @Test func promptWithCurrentTitleRequiresKeepRenameDecision() {
        let prompt = engine.buildPrompt(
            currentTitle: "Debugging generated workspace title churn in bmux",
    ...
    @Test func promptRequestsSubjectStatementInsteadOfVagueTitleFragment() {
        let prompt = engine.buildPrompt(
            currentTitle: nil,
            context: "user: the title summary for bmux workspace tabs isn't good enough"
        )
    ...
    @Test func currentTitleTreatsTitleOnlyResponseAsKeep() {
        let currentTitle = "Debugging generated workspace title churn in bmux"
        let outcome = engine.sanitizeResponseOutcome(
            "Investigating unstable generated workspace names in bmux",
    ...
    @Test func currentTitleKeepsOnExplicitKeepDecision() {
        ...
        let outcome = engine.sanitizeResponseOutcome(
            ...
            Title: Investigating unstable generated workspace names in bmux
            """,
            currentTitle: currentTitle
        )
        #expect(outcome == .unchanged(currentTitle))
    ...
    @Test func summarizerEnvironmentScrubsRecursionVarsAndPreservesBackend() {
        ...
        #expect(env["BMUX_WORKSPACE_ID"] == nil)
        #expect(env["BMUX_SURFACE_ID"] == nil)
