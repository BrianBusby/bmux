...
final class RestorableAgentSessionIndexCodexWeakRecordTests: XCTestCase {
    func testCodexWeakEnvironmentOnlyRecordDoesNotOverrideTranscriptBackedSession() throws {
        ...
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-codex-weak-env-restore-\(UUID().uuidString)", isDirectory: true)
    ...
    func testCodexLegacyArgvRecordWithoutSourceIsRestorable() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-codex-legacy-argv-restore-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let repo = root.appendingPathComponent("bmuxterm-hq", isDirectory: true)
    ...
    func testCodexDefaultLaunchRecordIsRestorable() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("bmux-codex-default-restore-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let repo = root.appendingPathComponent("bmuxterm-hq", isDirectory: true)
        try fm.createDirectory(at: repo, withIntermediateDirectories: true)
