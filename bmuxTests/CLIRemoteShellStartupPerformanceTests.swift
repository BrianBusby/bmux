...
struct CLIRemoteShellStartupPerformanceTests {
    ...
    func generatedSSHStartupDoesNotBlockOnRelayRPCWarmup() throws {
        ...
        environment["BMUX_BUNDLED_CLI_PATH"] = root.bin.appendingPathComponent("bmux").path
        environment["BMUX_SOCKET_PATH"] = "/tmp/bmux-test.sock"
        environment["BMUX_WORKSPACE_ID"] = "workspace-cli-perf"
        environment["BMUX_SURFACE_ID"] = "surface-cli-perf"
        environment["BMUX_FAKE_SHELL_MARKER"] = shellMarker.path
        environment["BMUX_FAKE_RELAY_RPC_GATE"] = relayRPCGate.path
    ...
    private func makeFakeRemoteShellRoot() throws -> FakeRemoteShellRoot {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("bmux-cli-shell-perf-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        let bin = root.appendingPathComponent("bin")
        try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: bin, withIntermediateDirectories: true)
        try writeExecutable(at: bin.appendingPathComponent("bmux"), contents: """
        #!/bin/sh
        if [ "$1" = "rpc" ] && [ -n "${BMUX_FAKE_RELAY_RPC_GATE:-}" ]; then
          while [ ! -f "$BMUX_FAKE_RELAY_RPC_GATE" ]; do sleep 0.05; done
        fi
        exit 0
        """)
        ...
        #!/bin/sh
        if [ -n "${BMUX_FAKE_SHELL_MARKER:-}" ]; then printf started > "$BMUX_FAKE_SHELL_MARKER"; fi
        exit 0
        """)
    ...
    private func bundledCLIPath() throws -> String {
        ...
        let direct = appBundle.appendingPathComponent("Contents/Resources/bin/bmux")
        ...
        throw NSError(domain: "bmux.tests", code: 1, userInfo: [
            NSLocalizedDescriptionKey: "Bundled bmux CLI not found in \(appBundle.path)",
        ])
    }
