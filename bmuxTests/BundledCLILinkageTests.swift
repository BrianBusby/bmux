    ...
    static func bundledCLIURL(
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "bmux",
                  item.path.contains(".app/Contents/Resources/bin/bmux"),
                  fileManager.isExecutableFile(atPath: item.path) else { continue }
            return item
        }

        let message = "Bundled bmux CLI not found at \(expectedCLIURL.path)"
...
final class BundledCLILinkageTests: XCTestCase {
    ...
    func testBundledCLIDoesNotDependOnPrivateRPathFrameworks() throws {
        ...
        XCTAssertEqual(
            ...
            [],
            "The bundled bmux CLI is copied into Contents/Resources/bin as a standalone helper. Private @rpath framework dependencies abort in dyld before CLI code can run."
        )
    }
