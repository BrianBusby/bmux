...
/// Tests the pure request-shaping for `bmux remotes`: host:port parsing, the
...
@Suite struct RemotesClientTests {
    ...
    // Note: `BMUXCLI.sanitizeForTerminal` (used by `remotes list`) lives in the
    // CLI executable target, which `bmuxTests` does not link, so it cannot be
    // unit-tested here (referencing `BMUXCLI` breaks the test-target compile).
