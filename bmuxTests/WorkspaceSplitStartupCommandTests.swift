...
private func waitForWorkspaceSplitView(
    ...
    XCTFail(
        "Timed out waiting for rendered bmux.json split ratio \(expectedDividerPosition); last ratio: \(lastRatioDescription)",
        file: file,
        line: line
    )
...
final class WorkspaceSplitStartupCommandTests: XCTestCase {
    func testCustomLayoutSplitRatioSurvivesInitialBonsplitViewLayout() throws {
        ...
        let layout = BmuxLayoutNode.split(BmuxSplitDefinition(
            ...
            children: [
                ...
                .pane(BmuxPaneDefinition(surfaces: [
                    BmuxSurfaceDefinition(type: .terminal, name: "Right")
                ]))
            ]
        ))
        ...
        XCTAssertEqual(
            ...
            accuracy: 0.000_1,
            "bmux.json split ratio should be applied to the Bonsplit model before rendering"
        )
        ...
        XCTAssertEqual(
            ...
            expectedDividerPosition,
            accuracy: 0.000_1,
            "Bonsplit initial view layout should not rewrite the bmux.json split ratio back to 0.5"
        )
    }
    ...
    func testTabManagerSplitCarriesRequestedWorkingDirectoryAndStartupCommand() {
        ...
        let requestedDirectory = "/tmp/bmux-split-startup-\(UUID().uuidString)"
        let startupCommand = "/tmp/bmux-tmux-command-\(UUID().uuidString).sh"
    ...
    func testNewTerminalSurfaceCarriesRequestedWorkingDirectoryAndStartupCommand() {
        ...
        let requestedDirectory = "/tmp/bmux-surface-startup-\(UUID().uuidString)"
        let startupCommand = "/tmp/bmux-surface-command-\(UUID().uuidString).sh"
        let tmuxStartCommand = "node /opt/oh-my-codex/dist/omx.js hud --watch"
    ...
    func testRespawnTerminalSurfacePreservesPaneTabAndSurfaceIdentity() throws {
        ...
        let requestedDirectory = "/tmp/bmux-respawn-\(UUID().uuidString)"
        ...
        XCTAssertTrue(
            ...
            "Respawn should replace the registered terminal surface for the existing bmux surface id"
        )
    }
    ...
    func testSessionRestoreRelaunchesOMXHudTmuxStartCommand() throws {
        let workspace = Workspace()
        let sourcePanelId = try XCTUnwrap(workspace.focusedPanelId)
        let requestedDirectory = "/tmp/bmux-hud-restore-\(UUID().uuidString)"
        let originalStartupScript = "/tmp/bmux-tmux-command-\(UUID().uuidString).sh"
        ...
        XCTAssertTrue(restoredStartupScript.contains("bmux-session-terminal-command"))
    ...
    func testSessionSnapshotDoesNotPersistGenericTmuxStartCommand() throws {
        ...
        let panel = try XCTUnwrap(workspace.newTerminalSurface(
            inPane: paneId,
            focus: false,
            initialCommand: "/tmp/bmux-command-\(UUID().uuidString).sh",
