...
final class WorkspaceSSHFishShellTests: XCTestCase {
    ...
    func testSSHBootstrapStartupCommandPassesRemoteInstallScriptAsSingleSSHCommand() throws {
        ...
        let tempRoot = fileManager.temporaryDirectory.appendingPathComponent("bmux-ssh-bootstrap-\(UUID().uuidString)")
        ...
        args = sys.argv[1:]
        with open(os.environ["BMUX_FAKE_SSH_LOG"], "a", encoding="utf-8") as handle:
        ...
        if local_command:
            ...
            subprocess.run([os.environ["BMUX_TEST_LOCAL_SHELL"], "-c", local_command], check=False, env=os.environ.copy())
        PY
        cat >/dev/null
        exit 0
        """
        ...
        startupEnvironment["BMUX_FAKE_SSH_LOG"] = fakeSSHLog.path
        startupEnvironment["BMUX_TEST_PYTHON3"] = python3Path
        startupEnvironment["BMUX_TEST_LOCAL_SHELL"] = fishExecutable
        startupEnvironment["BMUX_SOCKET_PATH"] = socketPath
        startupEnvironment["BMUX_WORKSPACE_ID"] = workspaceID
        startupEnvironment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        startupEnvironment["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        ...
        let destinationIndex = try XCTUnwrap(firstInvocation.lastIndex(of: "bmux-macmini"))
        ...
        XCTAssertTrue(
            secondRemoteCommand.contains("bmux_bootstrap_tty=") || secondRemoteCommand.contains("bmux_bootstrap_tty\\\""),
