...
struct SSHStartupManualReconnectTests {
    ...
    @Test func manualReconnectReentersConnectLoop() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-ssh-manual-retry-\(UUID().uuidString)", isDirectory: true)
        ...
        try Self.writeShellFile(at: fakeSSH, lines: [
            "#!/bin/sh",
            "count=$(cat \"${BMUX_TEST_ATTEMPT_FILE}\" 2>/dev/null || printf 0)",
            "count=$((count + 1))",
            "printf '%s' \"$count\" > \"${BMUX_TEST_ATTEMPT_FILE}\"",
    ...
    @Test func reconnectRejectsUnendedTerminalSurfaceId() throws {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to bmux-macmini via shared local proxy 127.0.0.1:64007",
            target: "bmux-macmini"
        )
    ...
    @Test func reconnectDefersToInFlightReconnectForEndedPaneRetry() {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .reconnecting,
            detail: "Reconnecting to bmux-macmini via shared local proxy 127.0.0.1:64007",
            target: "bmux-macmini"
        )
    ...
    private static func testError(_ message: String) -> NSError {
        NSError(domain: "bmux.tests", code: 1, userInfo: [NSLocalizedDescriptionKey: message])
    }
}
