...
struct ShellStartupMissingBundleTests {
    ...
    // The spawn-env call site only advertises BMUX_SHELL_INTEGRATION /
    // BMUX_SHELL_INTEGRATION_DIR when the bundled dir actually exists, so a
    ...
    func shellIntegrationDirectoryExistsRequiresARealDirectory() throws {
        ...
        let missingDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-tests-deleted-bundle-\(UUID().uuidString)")
    ...
    // zshrc": spawn a real interactive zsh with the environment bmux produces
    ...
    func zshStillLoadsUserZshrcWhenBundledIntegrationDirIsDeleted() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-zshrc-deleted-bundle-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        try fileManager.createDirectory(at: home, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        try "export BMUX_TEST_USER_ZSHRC=1\n"
        ...
        let arguments = ["-i"] + environment.map { "\($0.key)=\($0.value)" } + [
            "/bin/zsh",
            "-ic",
            "print -rn -- \"user_zshrc_loaded=${BMUX_TEST_USER_ZSHRC:-no}\"",
        ]
    ...
    func zshReturnShellLoadsUserZshrcWhenBundledIntegrationDirIsDeleted() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-return-zshrc-deleted-bundle-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        try fileManager.createDirectory(at: home, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        try """
        export BMUX_RETURN_TOP_MARKER=top
        alias bmux_return_bottom_alias='echo bottom'
        export BMUX_RETURN_BOTTOM_MARKER=bottom
        """
            .write(to: home.appendingPathComponent(".zshrc"), atomically: true, encoding: .utf8)
        ...
        print -r -- "child_top=${BMUX_RETURN_TOP_MARKER:-missing} child_bottom=${BMUX_RETURN_BOTTOM_MARKER:-missing}"
        if (( $+aliases[bmux_return_bottom_alias] )); then print -r -- child_alias=present; else print -r -- child_alias=missing; fi
        """
        try (
            "#!/bin/zsh\n"
        ...
        let launchCommand = [
            ...
            "PATH=/usr/bin:/bin",
            "TERM=xterm-256color",
            "BMUX_SHELL_INTEGRATION_DIR=\(missingIntegrationDir)",
        ...
        zpty -w p $'print -r -- "top=${BMUX_RETURN_TOP_MARKER:-missing} bottom=${BMUX_RETURN_BOTTOM_MARKER:-missing}"\\n'
        zpty -w p $'if (( $+aliases[bmux_return_bottom_alias] )); then print -r -- alias=present; else print -r -- alias=missing; fi\\n'
        zpty -w p $'exit\\n'
        local output
    ...
    private func makeBundledIntegrationDir(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-shell-missing-bundle-\(UUID().uuidString)")
