...
struct DockControlDefinitionDecodingTests {
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-dock-config-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func projectConfigIdentityUsesResolvedDockFile() throws {
        ...
        let bmuxDirectory = root.appendingPathComponent(".bmux", isDirectory: true)
        try FileManager.default.createDirectory(at: bmuxDirectory, withIntermediateDirectories: true)
        let dockConfig = bmuxDirectory.appendingPathComponent("dock.json", isDirectory: false)
