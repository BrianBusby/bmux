...
/// saved by a bmux build that predates the codex update-check suppression would
/// replay verbatim on the first relaunch after updating bmux — the exact restart
...
@Suite struct SurfaceResumeBindingCodexUpdateCheckTests {
    ...
    @Test func staleCodexTeamsBindingGainsUpdateCheckSuppressionOnReplay() throws {
        let binding = SurfaceResumeBindingSnapshot(
            kind: "codex",
            command: "'/usr/local/bin/bmux' 'codex-teams' 'resume' 'team-stale-binding' '--model' 'gpt-5.4'",
