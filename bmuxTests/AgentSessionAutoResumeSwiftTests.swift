...
struct AgentSessionAutoResumeSwiftTests {
    ...
    @Test func claudeAgentHookResumeBindingRestoresFromLaunchCwdWhenRuntimeCwdDrifted() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let launchCwd = "/tmp/bmux-claude-launch"
            let runtimeCwd = "/tmp/bmux-claude-runtime"
            let agent = SessionRestorableAgentSnapshot(
                ...
                launchCommand: AgentLaunchCommandSnapshot(
                    ...
                    workingDirectory: launchCwd,
                    environment: ["CLAUDE_CONFIG_DIR": "/tmp/bmux-claude-config"],
    ...
    @Test func cmdTAfterAgentResumeRestoreKeepsProjectCwdDespiteSpuriousHomePwdReport() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("bmux-cmdt-resume-project-\(UUID().uuidString)", isDirectory: true)
                .path
    ...
    @Test func agentResumeRestoreAcceptsHomePwdReportWhenSavedDirectoryWasDeleted() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let deletedDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("bmux-cmdt-deleted-project-\(UUID().uuidString)", isDirectory: true)
                .path
    ...
    @Test func splitFromResumedAgentPaneInheritsSessionCwdWhenTrackedCwdClobbered() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-split-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
    ...
    @Test func newTabFromResumedAgentPaneInheritsSessionCwdWhenTrackedCwdClobbered() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-newtab-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
    ...
    @Test func splitFromBindingOnlyAutoResumedAgentPaneInheritsSessionCwdWhenTrackedCwdClobbered() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-binding-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
    ...
    @Test func splitAfterBindingOnlyAutoResumeExitsFollowsTrackedCwdAgain() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-binding-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
            let repairedDir = try makeTemporaryProjectDirectory(prefix: "bmux-binding-post-exit")
    ...
    @Test func splitFromLocalResumedAgentPaneInsideRemoteWorkspaceUsesSessionCwdRescue() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-remote-local-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
            ...
            restored.remoteConfiguration = WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/bmux-local-resume.sock",
    ...
    @Test func detachedResumedAgentPaneKeepsSessionCwdFallbackWhenReattached() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-detach-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
    ...
    @Test func splitAfterSecondRestoreOfClobberedRestorableAgentPaneUsesAgentCwd() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-second-restore-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
    ...
    /// The bmux-authored chat rebind must record the resume launcher's real
    ...
    @Test func secondRestoreOfClobberedRestorableAgentPaneRebindsChatSessionToAgentCwd() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-second-restore-chat-rebind")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
    ...
    @Test func registeredAgentWithCwdIgnoreDoesNotRescueFromLaunchCwd() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-cwd-ignore-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
    ...
    @Test func splitWhileSessionDirectoryVolumeUnmountedKeepsRescueArmed() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-unmounted-volume-resume")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
            ...
            let unmountedSessionDir = "/Volumes/bmux-missing-\(UUID().uuidString)/project"
    ...
    @Test func splitFromResumedAgentPanePrefersLiveForegroundProcessCwd() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-split-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
            let liveDir = try makeTemporaryProjectDirectory(prefix: "bmux-split-resume-live")
    ...
    @Test func splitFromResumedAgentPaneIgnoresRecreatedDeletedSessionDirectory() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-split-resume-deleted-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
    ...
    @Test func splitAfterResumedAgentExitsFollowsTrackedCwdAgain() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let projectDir = try makeTemporaryProjectDirectory(prefix: "bmux-split-resume-project")
            defer { try? FileManager.default.removeItem(atPath: projectDir) }
            let repairedDir = try makeTemporaryProjectDirectory(prefix: "bmux-split-post-exit")
            defer { try? FileManager.default.removeItem(atPath: repairedDir) }
    ...
    /// Builds a workspace that restores solely from a bmux-owned agent-hook
    ...
    @Test func claudeAgentHookResumeBindingIgnoresStaleRestoredAgentSnapshot() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let staleLaunchCwd = "/tmp/bmux-claude-stale-launch"
            let freshRuntimeCwd = "/tmp/bmux-claude-fresh-runtime"
    ...
    @Test func crossKindAgentHookResumeBindingDoesNotRetainStaleClaudeSnapshot() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let cwd = "/tmp/bmux-cross-kind-runtime"
    ...
    @Test func crossKindAgentHookResumeBindingIgnoresStaleClaudeHibernation() throws {
        try withRestoredDefaults(key: AgentSessionAutoResumeSettings.autoResumeAgentSessionsKey) {
            ...
            let cwd = "/tmp/bmux-cross-kind-hibernation-runtime"
            let claudeAgent = SessionRestorableAgentSnapshot(
                kind: .claude,
                sessionId: claudeSessionId,
                workingDirectory: cwd,
    ...
    @Test func claudeRestorableIndexFindsNestedTranscriptWithoutTranscriptPath() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-claude-nested-index-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func claudeRestorableIndexMapsNestedTranscriptPathToProjectCwd() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("bmux-claude-nested-path-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    private func assertAgentAutoResumeUsesStartupCommand(
        ...
        #expect(script.contains("BMUX_SHELL_INTEGRATION_DIR"), Comment(rawValue: script))
        #expect(script.contains("BMUX_ZSH_ZDOTDIR"), Comment(rawValue: script))
        #expect(script.contains("\"$_bmux_resume_shell\" -lic"), Comment(rawValue: script))
        #expect(script.contains("csh|tcsh) \"$_bmux_resume_shell\" -c"), Comment(rawValue: script))
        #expect(script.contains("exec -l \"$_bmux_resume_shell\""), Comment(rawValue: script))
    }
}
