...
/// and the managed `BMUX_*` variables can never be clobbered.
@Suite(.serialized)
@MainActor
struct WorkspaceEnvironmentTests {
    ...
    /// at `strdup`/Ghostty, so `"BMUX_SOCKET_PATH\0x"` would dodge the exact-match
    ...
    func sanitizedWorkspaceEnvironmentRejectsKeysThatTruncateAtTheCBoundary() {
        let result = Workspace.sanitizedWorkspaceEnvironment([
            "BMUX_SOCKET_PATH\u{0}x": "spoofed",  // NUL would truncate to BMUX_SOCKET_PATH
    ...
    // MARK: - Acceptance: managed BMUX_* variables cannot be clobbered
    ...
    @Test
    func workspaceEnvironmentCannotClobberProtectedBmuxVariables() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: ["BMUX_WORKSPACE_ID": "real-id", "TERM": "xterm-ghostty"],
            protectedKeys: ["BMUX_WORKSPACE_ID", "TERM"],
            additionalEnvironment: [
                "BMUX_WORKSPACE_ID": "spoofed",   // must be ignored
                "TERM": "dumb",                    // must be ignored
                "AWS_PROFILE": "prod",             // must be applied
            ],
            initialEnvironmentOverrides: ["BMUX_WORKSPACE_ID": "also-spoofed"],
            ambientEnvironment: [:]
        )
        #expect(merged["BMUX_WORKSPACE_ID"] == "real-id")
    ...
    // MARK: - Config entry point (bmux.json)

    @Test
    func bmuxWorkspaceDefinitionDecodesEnv() throws {
        let json = #"{"name":"Build","env":{"AWS_PROFILE":"prod","API_BASE":"https://api.example.com"}}"#
        let definition = try JSONDecoder().decode(BmuxWorkspaceDefinition.self, from: Data(json.utf8))
        #expect(definition.env == ["AWS_PROFILE": "prod", "API_BASE": "https://api.example.com"])
    }

    @Test
    func bmuxWorkspaceDefinitionEnvIsOptional() throws {
        let definition = try JSONDecoder().decode(BmuxWorkspaceDefinition.self, from: Data(#"{"name":"Build"}"#.utf8))
