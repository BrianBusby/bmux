...
final class AgentHibernationTests: XCTestCase {
    ...
    func testSocketLifecycleAcceptsRegisteredCustomAgentKey() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-custom-lifecycle-\(UUID().uuidString)", isDirectory: true)
        let configDirectory = root.appendingPathComponent(".bmux", isDirectory: true)
        ...
        """.write(to: configDirectory.appendingPathComponent("bmux.json"), atomically: true, encoding: .utf8)
    ...
    func testSettingsDefaultToOptInAndNotifyOnChanges() throws {
        let suiteName = "bmux-agent-hibernation-\(UUID().uuidString)"
    ...
    func testSessionIndexLoadsAgentLifecycleFromHookStore() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hibernation-index-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSessionIndexUsesLiveHookPIDAsProcessID() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hibernation-live-hook-pid-\(UUID().uuidString)", isDirectory: true)
        let storeURL = RestorableAgentKind.codex.hookStoreFileURL(homeDirectory: home.path)
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
        ...
        let index = RestorableAgentSessionIndex.load(
            ...
            processArgumentsProvider: { requestedPID in
                requestedPID == pid
                    ? BmuxTopProcessArguments(
                        arguments: ["/bin/sleep", "30"],
                        environment: [
                            "BMUX_WORKSPACE_ID": workspaceId.uuidString,
                            "BMUX_SURFACE_ID": panelId.uuidString,
                            "BMUX_AGENT_LAUNCH_KIND": RestorableAgentKind.codex.rawValue,
                        ]
                    )
                    : nil
            }
    ...
    func testSessionIndexAcceptsNodeBackedClaudeProcessAsLiveHookPID() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hibernation-claude-node-pid-\(UUID().uuidString)", isDirectory: true)
                ...
                requestedPID == pid
                    ? BmuxTopProcessArguments(
                        ...
                        environment: [
                            "BMUX_WORKSPACE_ID": workspaceId.uuidString,
                            "BMUX_SURFACE_ID": panelId.uuidString,
                            "BMUX_AGENT_LAUNCH_KIND": RestorableAgentKind.claude.rawValue,
                        ]
                    )
                    : nil
            }
    ...
    func testLiveProcessScopeMatchingAcceptsLegacyEnvironmentKeys() throws {
        let workspaceId = UUID()
        let panelId = UUID()
        let process = BmuxTopProcessArguments(
            arguments: ["/usr/bin/codex"],
            environment: [
                "BMUX_TAB_ID": workspaceId.uuidString,
                "BMUX_PANEL_ID": panelId.uuidString,
            ]
        )

        XCTAssertTrue(process.matchesBMUXScope(workspaceId: workspaceId, surfaceId: panelId))
        XCTAssertFalse(process.matchesBMUXScope(workspaceId: UUID(), surfaceId: panelId))
        XCTAssertFalse(process.matchesBMUXScope(workspaceId: workspaceId, surfaceId: UUID()))
    }
    ...
    func testProcessDetectedSnapshotPreservesMatchingHookLifecycleWithoutRefreshingActivity() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hibernation-detected-\(UUID().uuidString)", isDirectory: true)
    ...
    func testProcessDetectedSnapshotPreservesMatchingHookLifecycleWhenHookPIDIsStale() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hibernation-stale-pid-\(UUID().uuidString)", isDirectory: true)
        let storeURL = RestorableAgentKind.opencode.hookStoreFileURL(homeDirectory: home.path)
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
    ...
    func testProcessDetectedSnapshotPreservesHookLifecycleWhenRestoredPanelIDsChange() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hibernation-remapped-panel-\(UUID().uuidString)", isDirectory: true)
        let storeURL = RestorableAgentKind.opencode.hookStoreFileURL(homeDirectory: home.path)
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
    ...
    func testProcessDetectedOnlySnapshotDoesNotUseScanTimeAsActivity() {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hibernation-empty-home-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSupportedAgentSnapshotsHaveResumeCommandsForHibernation() {
        let cwd = "/tmp/bmux-agent-hibernation"
    ...
    func testCustomRegisteredAgentSnapshotCanHibernateWhenResumeCommandExists() {
        let registration = BmuxVaultAgentRegistration(
            id: "local-agent",
            name: "Local Agent",
            detect: BmuxVaultAgentDetectRule(processName: "local-agent"),
    ...
    func testInvalidatedIndexedAgentSnapshotIsNotEligibleForHibernation() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-hibernation-invalidated-index-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: home) }
