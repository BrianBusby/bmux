...
extension CLINotifyProcessIntegrationRegressionTests {
    func testVMSSHOpensManagedWorkspaceThroughSharedSSHPath() throws {
        ...
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=bmux@gateway.freestyle.sh state=connecting\n")
        ...
        XCTAssertTrue(initialScript.contains("bmux_ssh_cleanup_password() { rm -rf"), initialScript)
        XCTAssertTrue(initialScript.contains("bmux_ssh_session_end() {"), initialScript)
        XCTAssertTrue(initialScript.contains("bmux_ssh_cleanup_password;"), initialScript)
        ...
        XCTAssertFalse(initialScript.contains("trap 'rm -rf \"$bmux_ssh_askpass_dir\"'"), initialScript)
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "bmux@gateway.freestyle.sh")
    ...
    func testSSHCommandGlobalWindowOverridesCallerEnvironment() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_WORKSPACE_ID"] = callerWorkspaceID
        environment["BMUX_SURFACE_ID"] = callerSurfaceID
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testVMSSHInfoRemainsPrintOnly() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(result.stdout.contains("ssh bmux@gateway.freestyle.sh -p 2222"), result.stdout)
        XCTAssertTrue(result.stdout.contains("password:  <redacted; run `bmux vm ssh \(vmID)` to connect>"), result.stdout)
