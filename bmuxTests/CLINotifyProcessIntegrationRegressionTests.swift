...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    // https://github.com/manaflow-ai/bmux/issues/6606
    //
    ...
    func testCodexPromptSubmitDoesNotRefreshTerminalLastTurnDiffBaseline() throws {
        ...
        _ = try runGit(["config", "user.name", "bmux tests"])
        _ = try runGit(["config", "user.email", "bmux@example.invalid"])
    ...
    // MARK: - Forked conversation restore (https://github.com/manaflow-ai/bmux/issues/5908)
    //
    ...
    private func runClaudeHookWithoutServer(
        ...
        var environment = [
            ...
            "BMUX_WORKSPACE_ID": context.workspaceId,
            "BMUX_SURFACE_ID": context.surfaceId,
            "BMUX_CLAUDE_HOOK_STATE_PATH": context.root.appendingPathComponent("claude-hook-sessions.json").path,
            "BMUX_CLI_SENTRY_DISABLED": "1",
            "BMUX_CLAUDE_HOOK_SENTRY_DISABLED": "1",
        ]
    ...
    func testClaudeForkSessionStartWithoutSurfaceIdentityDoesNotRegisterPIDOnFallbackPane() throws {
        ...
        environment["BMUX_SURFACE_ID"] = ""
        environment["BMUX_CLAUDE_PID"] = "12345"
        let result = runClaudeHookListingSurfaces(
            context: context,
            surfaceIds: [parentSurfaceId, context.surfaceId],
            arguments: ["hooks", "claude", "session-start"],
    ...
    func testClaudeLegacyStoreBackfillsPaneBoundaryFromWorkspaceActiveSlot() throws {
        ...
        let paneBPrompt = runClaudeHookWithoutServer(
            ...
            extraEnvironment: ["BMUX_SURFACE_ID": paneB]
        )
        ...
        let lateStop = runClaudeHookWithoutServer(
            ...
            extraEnvironment: ["BMUX_SURFACE_ID": paneA]
        )
    ...
    func testClaudeForkedSessionPromptSubmitRecordsWhileParentTurnActive() throws {
        ...
        XCTAssertEqual(
            ...
            true,
            "The forked session must become restorable so a bmux restart resumes the fork, not the parent"
        )
    ...
    func testClaudeForkedSessionPromptSubmitRecordsWithSurfaceRefForm() throws {
        ...
        let result = runClaudeHookListingSurfaces(
            ...
            arguments: ["hooks", "claude", "prompt-submit"],
            standardInput: #"{"session_id":"\#(forkedSessionId)","turn_id":"fork-turn-1","cwd":"\#(context.root.path)","hook_event_name":"UserPromptSubmit","prompt":"diverge here"}"#,
            extraEnvironment: ["BMUX_SURFACE_ID": "surface:2"]
        )
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
    ...
    func testClaudeStaleStopFromClosedPaneStaysStaleWhenSurfaceResolutionFallsBack() throws {
        ...
        let result = runClaudeHookListingSurfaces(
            ...
            extraEnvironment: ["BMUX_SURFACE_ID": closedSurfaceId]
        )
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
    ...
    func testClaudeStaleStopStaysStaleAfterAnotherPaneBecomesWorkspaceActive() throws {
        ...
        func runHook(_ subcommand: String, stdin: String, surface: String) -> ProcessRunResult {
            runClaudeHookWithoutServer(
                ...
                standardInput: stdin,
                extraEnvironment: ["BMUX_SURFACE_ID": surface]
            )
        }
    ...
    func testClaudeParentPaneStopAppliesAfterForkedSessionPromoted() throws {
        ...
        let result = runClaudeHookListingSurfaces(
            ...
            extraEnvironment: ["BMUX_SURFACE_ID": parentSurfaceId]
        )
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
    ...
    func testClaudePromptSubmitResumeBindingPersistsSafeAuthSelectionValues() throws {
        ...
        let launchEnvironment = [
            ...
            "BMUX_AGENT_LAUNCH_EXECUTABLE": "/usr/local/bin/claude",
            "BMUX_AGENT_LAUNCH_CWD": context.root.path,
            "BMUX_AGENT_LAUNCH_ARGV_B64": base64NULSeparated([
        ...
        XCTAssertEqual(environment["BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV"] as? String, "1")
        XCTAssertEqual(
            environment["BMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS"] as? String,
    ...
    func testManagedCodexSubagentStopDoesNotReplaceResumeBinding() throws {
        ...
        let result = runCodexHook(
            ...
            extraEnvironment: codexLaunchEnvironment(context: context, sessionId: sessionId).merging([
                "BMUX_AGENT_MANAGED_SUBAGENT": "1",
                "BMUX_CODEX_TEAMS_THREAD_ID": "child-thread",
                "BMUX_CODEX_TEAMS_PARENT_THREAD_ID": "root-thread",
                "BMUX_CODEX_TEAMS_DEPTH": "1",
    ...
    func testRightSidebarInvalidCommandValidatesBeforeTargetResolution() throws {
        let cliPath = try bundledCLIPath()
        let missingSocketPath = "/tmp/bmux-test-missing-\(UUID().uuidString).sock"
        var environment = ProcessInfo.processInfo.environment
        environment["BMUX_SOCKET_PATH"] = missingSocketPath
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
    ...
    func testSSHForwardAgentRepeatedOptionUsesLastValue() throws {
        let run = try runMockedSSH(
            ...
            environmentOverrides: [
                "SSH_AUTH_SOCK": "/tmp/bmux-test-agent-\(UUID().uuidString).sock",
            ]
        )
        let createParams = try XCTUnwrap(params(for: "workspace.create", in: run.requests))
        let configureParams = try XCTUnwrap(params(for: "workspace.remote.configure", in: run.requests))
    ...
    func testSSHForwardAgentTildeSocketPathExpandsSocketPath() throws {
        let homeURL = try makeTemporaryDirectory(prefix: "bmux-ssh-home")
        let tildeSocketPath = "~/.ssh/bmux-test-agent.sock"
        let expandedSocketURL = homeURL.appendingPathComponent(".ssh/bmux-test-agent.sock")
    ...
    private func assertSSHPersistentPTYUsesReusableForegroundAuthControlConnection(
        ...
        XCTAssertTrue(initialScript.contains("BMUX_WORKSPACE_ID"), initialScript)
        XCTAssertTrue(initialScript.contains("BMUX_SURFACE_ID"), initialScript)
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            initialScript.contains("ssh-$bmux_ssh_pty_workspace_id-$bmux_ssh_pty_surface_id"),
            initialScript
        )
        XCTAssertTrue(initialScript.contains("254|255"), initialScript)
        assertSSHPTYAttachOmitsSurfaceArgument(initialScript)
        XCTAssertTrue(
            initialScript.contains("--workspace \"$bmux_ssh_pty_workspace_id\""),
            initialScript
        )
        XCTAssertEqual(
        ...
        XCTAssertTrue(terminalStartupScript.contains("BMUX_WORKSPACE_ID"), terminalStartupScript)
        XCTAssertTrue(terminalStartupScript.contains("BMUX_SURFACE_ID"), terminalStartupScript)
        XCTAssertTrue(
        ...
        XCTAssertTrue(
            terminalStartupScript.contains("ssh-$bmux_ssh_pty_workspace_id-$bmux_ssh_pty_surface_id"),
            terminalStartupScript
        )
        ...
        XCTAssertTrue(
            terminalStartupScript.contains("--workspace \"$bmux_ssh_pty_workspace_id\""),
            terminalStartupScript
        )
        XCTAssertEqual(
    ...
    func testSSHPersistentPTYFallsBackWhenForegroundAuthCannotBeReused() throws {
        let cases: [(name: String, arguments: [String])] = [
            ...
            ("local-command", ["--ssh-option", "LocalCommand=echo bmux-test"]),
    ...
    func testSSHPTYAttachWithoutSurfaceDoesNotSendLocalAttachEnd() throws {
        ...
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment.removeValue(forKey: "BMUX_SURFACE_ID")
    ...
    func testSSHPTYAttachWaitUsesCurrentTerminalSizeForBridgeHandshake() throws {
        ...
        guard openpty(&masterFD, &slaveFD, nil, nil, nil) == 0 else {
            throw NSError(domain: "bmux.tests", code: Int(errno), userInfo: [
    ...
    func testSSHSessionAttachCreatesSurfaceWithPersistedPTYSessionID() throws {
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            XCTAssertTrue(initialCommand.contains("BMUX_WORKSPACE_ID"), initialCommand)
            XCTAssertTrue(initialCommand.contains("BMUX_SURFACE_ID"), initialCommand)
    ...
    func testNotifyWithUUIDSurfaceDoesNotRequireCallerWorkspaceOrWindow() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_WORKSPACE_ID"] = callerWorkspace
        environment["BMUX_SURFACE_ID"] = callerSurface
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
    ...
    func testCodexPromptSubmitRebindsRestoredSessionToCurrentCallerSurface() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-rebind-\(UUID().uuidString)", isDirectory: true)
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_WORKSPACE_ID"] = currentWorkspaceId
        environment["BMUX_SURFACE_ID"] = currentSurfaceId
    ...
    func testCodexPromptSubmitWithForeignBmuxEnvDoesNotFallbackToSelectedWorkspace() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("codex-foreign-env")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-foreign-env-\(UUID().uuidString)", isDirectory: true)
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_WORKSPACE_ID"] = foreignWorkspaceId
        environment["BMUX_SURFACE_ID"] = foreignSurfaceId
        environment["BMUX_AGENT_HOOK_STATE_DIR"] = root.path
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["CODEX_HOME"] = root.appendingPathComponent("codex-home", isDirectory: true).path
        ...
        XCTAssertFalse(
            ...
            "Foreign bmux env must not mutate the selected workspace, saw \(state.commands)"
        )
    }

    func testCodexPromptSubmitWithForeignBmuxEnvIgnoresStaleMappedSession() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("codex-foreign-mapped")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-foreign-mapped-\(UUID().uuidString)", isDirectory: true)
        let foreignWorkspaceId = "11111111-1111-1111-1111-111111111111"
        let foreignSurfaceId = "22222222-2222-2222-2222-222222222222"
        let selectedWorkspaceId = "33333333-3333-3333-3333-333333333333"
        let selectedSurfaceId = "44444444-4444-4444-4444-444444444444"
        ...
        XCTAssertFalse(
            state.commands.contains { $0.contains("set_status codex Running") || $0.contains("notify_target_async") },
            "Foreign bmux env must not reuse stale mapped sessions, saw \(state.commands)"
        )
    }
    ...
    func testCodexPromptSubmitWithInvalidSurfaceDoesNotFallbackToFocusedSurface() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-invalid-surface-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
...
"""
    ...
    func testCodexPromptSubmitWithInvalidMappedWorkspaceDoesNotFallbackToSelectedWorkspace() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-invalid-mapped-\(UUID().uuidString)", isDirectory: true)
        let staleWorkspaceId = "11111111-1111-1111-1111-111111111111"
        let staleSurfaceId = "22222222-2222-2222-2222-222222222222"
        let selectedWorkspaceId = "33333333-3333-3333-3333-333333333333"
        ...
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["CODEX_HOME"] = root.appendingPathComponent("codex-home", isDirectory: true).path
        environment.removeValue(forKey: "BMUX_WORKSPACE_ID")
        environment.removeValue(forKey: "BMUX_SURFACE_ID")
    ...
    func testCodexTeamsForkPromptPublishesResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-codex-teams-resume-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        let surfaceId = "44444444-4444-4444-4444-444444444444"
        ...
        environment["BMUX_AGENT_HOOK_STATE_DIR"] = root.path
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_AGENT_LAUNCH_KIND"] = "codexTeams"
        environment["BMUX_AGENT_LAUNCH_EXECUTABLE"] = "/usr/local/bin/bmux"
        ...
        XCTAssertEqual(
            ...
            "cd -- '\(root.path)' 2>/dev/null || [ ! -d '\(root.path)' ] && '/usr/local/bin/bmux' 'codex-teams' 'resume' '\(sessionId)' '-c' 'check_for_update_on_startup=false' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testAgentPromptClearsSurfaceResumeBindingWhenResumeCommandUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-resume-unavailable-\(UUID().uuidString)", isDirectory: true)
            ...
            "sessions": [
                sessionId: [
                    ...
                    "launchCommand": [
                        "launcher": "omx",
                        "executablePath": "/usr/local/bin/bmux",
                        "arguments": ["/usr/local/bin/bmux", "omx", "hud"],
                        "workingDirectory": root.path,
                        "capturedAt": now,
                        "source": "test",
                    ],
    ...
    func testGenericAgentSessionEndClearsMatchingSurfaceResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-agent-resume-clear-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer {
            Darwin.close(listenerFD)
            unlink(socketPath)
    ...
    func testSurfaceResumeSetCLIDoesNotScopeExplicitSurfaceToEnvWorkspace() throws {
        ...
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment["BMUX_WORKSPACE_ID"] = staleWorkspaceId
        environment["BMUX_SURFACE_ID"] = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"
    ...
    private func agentLaunchEnvironment(
        ...
        [
            "BMUX_AGENT_LAUNCH_KIND": kind,
            "BMUX_AGENT_LAUNCH_EXECUTABLE": executable,
            "BMUX_AGENT_LAUNCH_CWD": context.root.path,
            "BMUX_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(arguments ?? [executable]),
        ]
    }
    ...
    private func runAgentHook(
        ...
        var environment = [
            ...
            "BMUX_WORKSPACE_ID": context.workspaceId,
            "BMUX_SURFACE_ID": context.surfaceId,
            "BMUX_AGENT_HOOK_STATE_DIR": context.root.path,
            "BMUX_CLI_SENTRY_DISABLED": "1",
        ]
    ...
    private func makeClaudeHookContext(name: String) throws -> ClaudeHookContext {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-\(name)-\(UUID().uuidString)", isDirectory: true)
    ...
    func testBrowserImportUsesInteractiveDialogOutsideCodingAgent() throws {
        ...
        environment["BMUX_SOCKET_PATH"] = socketPath
        environment["BMUX_CLI_SENTRY_DISABLED"] = "1"
        environment.removeValue(forKey: "BMUX_AGENT_LAUNCH_KIND")
    ...
    private func makeExistingAgentSocketPath() throws -> String {
        let directory = try makeTemporaryDirectory(prefix: "bmux-agent")
...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    // stamped the focused surface (A), desyncing BMUX_SURFACE_ID from BMUX_PANEL_ID and jumbling codex
    // into the wrong surface on reload.
    func testTmuxCompatEnvStampsLaunchSurfaceNotFocusedPane() throws {
        let cliPath = try bundledCLIPath()
        let tmpDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-spawn-id-e2e-\(UUID().uuidString)", isDirectory: true)
        ...
        let result = runProcess(
            ...
            environment: [
                "BMUX_SOCKET_PATH": socketPath,
                "BMUX_WORKSPACE_ID": launchWorkspace,
                "BMUX_SURFACE_ID": launchSurface,
                "BMUX_PANEL_ID": launchSurface,
                "BMUX_TAB_ID": launchTab,
                "HOME": tmpDir.path,
        ...
        XCTAssertTrue(
            result.stdout.contains("BMUX_SURFACE_ID=\(launchSurface)"),
            "launcher must stamp the LAUNCH surface; stdout:\n\(result.stdout)\nstderr:\n\(result.stderr)"
        )
        XCTAssertFalse(
            result.stdout.contains("BMUX_SURFACE_ID=\(focusedSurface)"),
            "launcher must NOT stamp the focused surface; stdout:\n\(result.stdout)"
        )
        XCTAssertTrue(result.stdout.contains("BMUX_WORKSPACE_ID=\(launchWorkspace)"), result.stdout)
        // Matched-pair invariant: SURFACE == PANEL (the desync is exactly the bug). The surface-scoped
        // tab id passes through untouched.
        XCTAssertTrue(result.stdout.contains("BMUX_PANEL_ID=\(launchSurface)"), result.stdout)
        XCTAssertTrue(result.stdout.contains("BMUX_TAB_ID=\(launchTab)"), result.stdout)
    }
}
