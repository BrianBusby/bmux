...
struct ClaudeHookSurfaceResolutionSwiftTests {
    @Test func claudeSessionStartOverridesLeakedEnvSurfaceWithTTYBinding() throws {
        ...
        let environment = [
            ...
            "BMUX_SURFACE_ID": leakedSurfaceId,
            "BMUX_CLI_TTY_NAME": ttyName,
            "BMUX_CLAUDE_PID": "42424",
            ...
            "BMUX_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(["/usr/local/bin/claude"]),
        ]
        ...
        #expect(
            ...
            "Claude must persist the agent TTY surface, not the leaked ambient BMUX_SURFACE_ID; params=\(request)"
        )
        #expect(
    ...
    @Test func claudeSessionStartOverridesLeakedEnvWorkspaceAndSurfaceWithTTYBinding() throws {
        ...
        let environment = [
            ...
            "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
            "BMUX_SOCKET_PATH": context.socketPath,
            "BMUX_WORKSPACE_ID": leakedWorkspaceId,
            "BMUX_SURFACE_ID": leakedSurfaceId,
            "BMUX_CLI_TTY_NAME": ttyName,
            "BMUX_CLAUDE_HOOK_STATE_PATH": context.root.appendingPathComponent("claude-hook-sessions.json").path,
            "BMUX_CLI_SENTRY_DISABLED": "1",
        ...
        #expect(
            ...
            "Claude must persist the agent TTY workspace, not the leaked ambient BMUX_WORKSPACE_ID; params=\(request)"
        )
        #expect(
            request["surface_id"] as? String == ttySurfaceId,
            "Claude must persist the agent TTY surface, not the leaked ambient BMUX_SURFACE_ID; params=\(request)"
    ...
    @Test func claudeSessionStartOverridesLeakedEnvWithClaudePIDBindingWhenTTYMissing() throws {
        ...
        let environment = [
            ...
            "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
            "BMUX_SOCKET_PATH": context.socketPath,
            "BMUX_SOCKET_PASSWORD": socketPassword,
            "BMUX_WORKSPACE_ID": leakedWorkspaceId,
            "BMUX_SURFACE_ID": leakedSurfaceId,
            "BMUX_CLAUDE_PID": "\(claudePID)",
            "BMUX_CLAUDE_HOOK_STATE_PATH": context.root.appendingPathComponent("claude-hook-sessions.json").path,
            "BMUX_CLI_SENTRY_DISABLED": "1",
            "BMUX_CLAUDE_HOOK_SENTRY_DISABLED": "1",
            "BMUX_AGENT_LAUNCH_KIND": "claude",
        ...
        #expect(
            ...
            "Claude PID binding must beat leaked ambient BMUX_WORKSPACE_ID when no TTY marker exists; params=\(request)"
        )
        #expect(
            request["surface_id"] as? String == pidSurfaceId,
            "Claude PID binding must beat leaked ambient BMUX_SURFACE_ID when no TTY marker exists; params=\(request)"
        )
        #expect(
    ...
    @Test func claudeSessionStartIgnoresStaleTTYWorkspaceWhenBoundSurfaceIsGone() throws {
        ...
        #expect(
            ...
            "Stale TTY workspace must not beat valid ambient BMUX_WORKSPACE_ID; params=\(request)"
        )
        #expect(
    ...
    private func bindUnixSocket(at path: String) throws -> Int32 {
        ...
        guard fd >= 0 else {
            throw NSError(domain: "bmux.tests", code: Int(errno))
        }
        ...
        guard utf8.count < maxPathLength else {
            Darwin.close(fd)
            throw NSError(domain: "bmux.tests", code: Int(ENAMETOOLONG))
        }
        ...
        guard bindResult == 0, Darwin.listen(fd, 1) == 0 else {
            let code = errno
            Darwin.close(fd)
            throw NSError(domain: "bmux.tests", code: Int(code))
        }
        return fd
    }
    ...
    func claudeHookEnvironment(context: ClaudeHookContext, surfaceId: String, ttyName: String, storeURL: URL) -> [String: String] {
        [
            ...
            "BMUX_CLI_TTY_NAME": ttyName,
            "BMUX_CLAUDE_HOOK_STATE_PATH": storeURL.path,
            "BMUX_CLI_SENTRY_DISABLED": "1",
            "BMUX_CLAUDE_HOOK_SENTRY_DISABLED": "1",
        ]
    }
    ...
    private func assertPromptSubmitRoutes(
        ...
        #expect(request["surface_id"] as? String != ambientSurfaceId, "Raw ambient BMUX_SURFACE_ID must not win")
        #expect(
            context.state.snapshot().contains {
                $0.hasPrefix("set_status claude_code Running ")
