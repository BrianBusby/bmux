...
final class BmuxConfigDecodingTests: XCTestCase {
    ...
    private func resolvedActions(
        from config: BmuxConfigFile,
        sourcePath: String? = nil
    ) -> [String: BmuxResolvedConfigAction] {
        Dictionary(
            uniqueKeysWithValues: config.actions.compactMap { id, definition in
                BmuxResolvedConfigAction.fromDefinition(
                    id: id,
    ...
    func testSurfaceTabBarActionReferenceUsesActionSourcePath() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-config-store-\(UUID().uuidString)", isDirectory: true)
        ...
        let globalConfigURL = globalDirectory.appendingPathComponent("bmux.json")
        let localConfigURL = localDirectory.appendingPathComponent("bmux.json")
        let globalJSON = """
        {
          "actions": {
    ...
    func testGlobalSVGIconAllowsNamespaceAndInternalGradient() throws {
        ...
        let configPath = root.appendingPathComponent("bmux.json").path
        ...
        let icon = BmuxButtonIcon.imagePath("icons/codex.svg")
        XCTAssertEqual(
            icon.bonsplitIcon(
    ...
    func testProjectLocalSVGIconRejectsExternalReferences() throws {
        ...
        let globalConfigPath = globalDirectory.appendingPathComponent("bmux.json").path
        let projectConfigPath = projectDirectory.appendingPathComponent("bmux.json").path
        ...
        let icon = BmuxButtonIcon.imagePath("icons/bad.svg")
        XCTAssertEqual(
            icon.bonsplitIcon(
    ...
    func testUntrustedProjectLocalIconUsesLockPlaceholder() throws {
        ...
        let icon = BmuxButtonIcon.imagePath("icons/safe.svg")
        XCTAssertEqual(
            icon.bonsplitIcon(
                configSourcePath: projectConfigPath,
                globalConfigPath: globalConfigPath,
    ...
    func testInvalidConfigExposesSchemaIssueAndClearsAfterFix() throws {
        ...
        let configURL = root.appendingPathComponent("bmux.json")
        let invalidJSON = """
        {
          "actions": {
            "bad": {
    ...
    func testConfigChangesRequireExplicitLoadByDefault() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-config-store-\(UUID().uuidString)",
            isDirectory: true
        )
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let store = BmuxConfigStore(globalConfigPath: configURL.path)
        store.loadAll()
        XCTAssertNotNil(store.resolvedAction(id: "first"))
        XCTAssertNil(store.resolvedAction(id: "second"))

        let didAutoReload = expectation(description: "bmux.json should not hot reload")
    ...
    @MainActor
    func testConfigStoreParsesGlobalBmuxJSONCSettingsAndActionSections() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-config-store-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let data = """
        {
          // bmux-owned app settings share the global bmux.json file.
          "app": {
            "appearance": "dark",
          },
          "actions": {
        ...
        let store = BmuxConfigStore(globalConfigPath: configURL.path, startFileWatchers: false)
        store.loadAll()
    ...
    func testLocalWatcherDetectsFirstCanonicalConfigAfterDirectoryCreation() async throws {
        ...
        let configDirectory = projectDirectory.appendingPathComponent(".bmux", isDirectory: true)
        let configURL = configDirectory.appendingPathComponent("bmux.json", isDirectory: false)
        let store = BmuxConfigStore(
            globalConfigPath: root.appendingPathComponent("missing-global.json").path,
            localConfigPath: configURL.path,
        ...
        let loaded = expectation(description: "created local bmux config is loaded")
        ...
        // watcher) to observe the .bmux directory creation, re-arm onto the new
        // bmux.json, reload, and republish loadedActions. vnode notification +
    ...
    @MainActor
    func testLocalWatcherDetectsFirstLegacyConfigWhenBmuxDirectoryExists() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "bmux-config-store-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let canonicalConfigURL = configDirectory.appendingPathComponent("bmux.json", isDirectory: false)
        let legacyConfigURL = projectDirectory.appendingPathComponent("bmux.json", isDirectory: false)
        let store = BmuxConfigStore(
            globalConfigPath: root.appendingPathComponent("missing-global.json").path,
        ...
        let loaded = expectation(description: "created legacy bmux config is loaded")
        loaded.assertForOverFulfill = false
        var cancellable: AnyCancellable?
        cancellable = store.$loadedActions.dropFirst().sink { actions in
        ...
        // The store observes the legacy bmux.json write via a DispatchSource vnode
    ...
    func testGlobalNewWorkspaceActionUsesLocalActionOverride() throws {
        ...
        let globalConfigURL = root.appendingPathComponent("global-bmux.json")
        let localConfigURL = root.appendingPathComponent("local-bmux.json")
        try """
        {
          "actions": {
    ...
    func testNotificationHooksAppendThroughConfigHierarchy() throws {
        ...
        let globalDirectory = root.appendingPathComponent("global", isDirectory: true)
        let projectDirectory = root.appendingPathComponent("project", isDirectory: true)
        let parentConfigDirectory = projectDirectory.appendingPathComponent(".bmux", isDirectory: true)
        let childDirectory = projectDirectory.appendingPathComponent("child", isDirectory: true)
        let childConfigDirectory = childDirectory.appendingPathComponent(".bmux", isDirectory: true)
        try FileManager.default.createDirectory(at: globalDirectory, withIntermediateDirectories: true)
        ...
        let globalConfigURL = globalDirectory.appendingPathComponent("bmux.json")
        let parentConfigURL = parentConfigDirectory.appendingPathComponent("bmux.json")
        let childConfigURL = childConfigDirectory.appendingPathComponent("bmux.json")
        try """
        {
          "notifications": {
    ...
    func testNotificationHooksIncludeExplicitLocalConfigOutsideDiscoveredHierarchy() throws {
        ...
        let globalConfigURL = globalDirectory.appendingPathComponent("bmux.json")
        let explicitConfigURL = explicitDirectory.appendingPathComponent("custom-bmux.json")
        try """
        {
          "notifications": {
            "hooks": [{ "id": "global", "command": "cat" }]
    ...
    func testSurfaceTabBarWorkspaceCommandButtonRoundTrips() throws {
        let original = BmuxSurfaceTabBarButton(
            id: "new-dev",
        ...
        let decoded = try JSONDecoder().decode(BmuxSurfaceTabBarButton.self, from: data)
...
final class BmuxCommandIdentityTests: XCTestCase {

    func testCommandIdIsDeterministic() {
        let cmd = BmuxCommandDefinition(name: "Run tests", command: "test")
        XCTAssertEqual(cmd.id, "bmux.config.command.Run%20tests")
    }

    func testCommandIdEncodesSpecialCharacters() {
        let cmd = BmuxCommandDefinition(name: "build & deploy", command: "make")
        XCTAssertTrue(cmd.id.hasPrefix("bmux.config.command."))
        XCTAssertFalse(cmd.id.contains("&"))
        XCTAssertFalse(cmd.id.contains(" "))
    }

    func testCommandIdIsUniqueForDifferentNames() {
        let cmd1 = BmuxCommandDefinition(name: "build", command: "make build")
        let cmd2 = BmuxCommandDefinition(name: "test", command: "make test")
        XCTAssertNotEqual(cmd1.id, cmd2.id)
    }

    func testCommandIdDoesNotCollideWithBuiltinPrefix() {
        let cmd = BmuxCommandDefinition(name: "palette.newWorkspace", command: "echo")
        XCTAssertTrue(cmd.id.hasPrefix("bmux.config.command."))
...
@MainActor
final class BmuxConfigWorkspaceCommandExecutionTests: XCTestCase {
    ...
    func testWorkspaceCommandCreatesNewWorkspaceByDefaultWhenNameAlreadyExists() {
        ...
        let command = BmuxCommandDefinition(
            name: "Dev command",
            workspace: BmuxWorkspaceDefinition(name: "Dev")
        )

        XCTAssertTrue(BmuxConfigExecutor.execute(
            command: command,
            tabManager: manager,
...
final class BmuxSplitDefinitionTests: XCTestCase {
    ...
    func testSplitOrientationVertical() {
        let split = BmuxSplitDefinition(direction: .vertical, split: nil, children: [])
...
@MainActor
final class BmuxConfigCwdResolutionTests: XCTestCase {

    private let baseCwd = "/Users/test/project"

    func testNilCwdReturnsBase() {
        XCTAssertEqual(
            BmuxConfigStore.resolveCwd(nil, relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testEmptyCwdReturnsBase() {
        XCTAssertEqual(
            BmuxConfigStore.resolveCwd("", relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testAbsolutePathReturnedAsIs() {
        XCTAssertEqual(
            BmuxConfigStore.resolveCwd("/tmp/other", relativeTo: baseCwd),
            "/tmp/other"
        )
    }
    ...
    func testRelativePathJoinedToBase() {
        XCTAssertEqual(
            BmuxConfigStore.resolveCwd("backend/src", relativeTo: baseCwd),
    ...
    func testTildeSlashExpandsToHomePlusPath() {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        XCTAssertEqual(
            BmuxConfigStore.resolveCwd("~/Documents/work", relativeTo: baseCwd),
    ...
    func testSingleSubdirectory() {
        XCTAssertEqual(
            BmuxConfigStore.resolveCwd("src", relativeTo: baseCwd),
...
final class BmuxLayoutEncodingTests: XCTestCase {

    func testPaneNodeRoundTrips() throws {
        let original = BmuxLayoutNode.pane(BmuxPaneDefinition(surfaces: [
            BmuxSurfaceDefinition(type: .terminal, name: "shell")
        ]))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(BmuxLayoutNode.self, from: data)
    ...
    func testSplitNodeRoundTrips() throws {
        let original = BmuxLayoutNode.split(BmuxSplitDefinition(
            direction: .vertical,
            split: 0.7,
            children: [
                .pane(BmuxPaneDefinition(surfaces: [BmuxSurfaceDefinition(type: .terminal)])),
                .pane(BmuxPaneDefinition(surfaces: [BmuxSurfaceDefinition(type: .browser, url: "http://localhost")]))
            ]
        ))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(BmuxLayoutNode.self, from: data)
