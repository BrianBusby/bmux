...
final class BmuxConfigNamedColorTests: XCTestCase {
    private func decode(_ json: String, colorDefaults: UserDefaults? = nil) throws -> BmuxConfigFile {
        let data = json.data(using: .utf8)!
        let decoder = JSONDecoder()
        if let colorDefaults {
            decoder.userInfo[.bmuxWorkspaceColorDefaults] = colorDefaults
        }
        return try decoder.decode(BmuxConfigFile.self, from: data)
    }

    func testDecodeWorkspaceCommandAcceptsNamedColor() throws {
        let suiteName = "bmux-config-named-color-\(UUID().uuidString)"
    ...
    func testDecodeWorkspaceCommandRejectsUnknownNamedColor() {
        let suiteName = "bmux-config-unknown-color-\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { defaults.removePersistentDomain(forName: suiteName) }
