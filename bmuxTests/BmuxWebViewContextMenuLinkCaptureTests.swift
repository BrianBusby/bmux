...
struct BmuxWebViewContextMenuLinkCaptureTests {
    ...
    func openLinkInDefaultBrowserOpensTheLinkUnderTheRightClick() async throws {
        ...
        BmuxWebView.contextMenuLinkCaptureAcceptsUntrustedEventsForTesting = true
        defer { BmuxWebView.contextMenuLinkCaptureAcceptsUntrustedEventsForTesting = false }
    ...
    func syntheticContextMenuEventCannotPlantDecoyLink() async throws {
        ...
        // point is identical because BmuxWebView (WKWebView) is flipped.
        let openedURL = try await openLinkInDefaultBrowser(
            webView,
            menuEventLocation: NSPoint(x: 60, y: 60),
            menuEventTimestamp: ProcessInfo.processInfo.systemUptime
    ...
    /// CSS (300,300)-(420,420).
    private func makeLoadedTwoLinkWebView() async throws -> BmuxWebView {
        _ = NSApplication.shared
        let webView = BmuxWebView(
            frame: NSRect(x: 0, y: 0, width: 800, height: 600),
            configuration: WKWebViewConfiguration()
