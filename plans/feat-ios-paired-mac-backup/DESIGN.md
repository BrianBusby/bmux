...
2. **Bundle id change** (external founders/beta `dev.bmux.app.beta` → a future
...
`BmuxSyncStore` local-first cache exactly like `devices`.
...
snapshot lands in `BmuxSyncStore`, `PairedMacRestore` merges each record into
