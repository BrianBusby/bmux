# Local-first sync for bmux (and the iOS device list as its first consumer)
...
2. **Presence DO** (`workers/presence`, `presence.bmux.dev`) is a realtime
...
3. Use **the most standard SQLite thing**: the repo already standardizes on the
   raw SQLite3 C API in `BmuxMobilePairedMac`. Extend that pattern; no new GRDB
   or FMDB dependency.
...
- **Wire protocol** (`sync/v1`): a snapshot+delta stream over the existing
  ...
  changed.
- **Local store** (`BmuxSyncStore`): one raw-SQLite3 database with a single
...
1. **t0 (launch):** UI reads the device collection straight from `BmuxSyncStore`
...
- **Across collections:** no cross-collection ordering guarantee. Each
  ...
  collections independent so adding one cannot perturb another, and no real bmux
...
One database, `bmux-sync.sqlite3`, opened exactly like
...
2. For each paired Mac it does **not** write the DO directly (the phone is not
   ...
   `BmuxSyncStore` `devices` collection with a provisional record derived from
...
- **Per-record, not per-field, by default.** Per-field merge (CRDT-ish) is more
  work and only pays off for genuinely concurrent multi-writer fields. bmux's
...
- **Why not CRDTs (yet).** CRDTs buy conflict-free *concurrent multi-writer*
  convergence without a server arbiter. bmux already has a natural per-team
...
overridable via env (`BMUX_MOBILE_DEVICE_LIST_LOCAL_FIRST`) and `UserDefaults`
...
|------|--------------|-------------|
| on   | yes          | `BmuxSyncStore` (instant) + live DO deltas. |
| on   | no           | `BmuxSyncStore` last-synced cache (labeled stale), retry DO. If cache empty, fall back to `GET /api/devices`, then local paired Macs. |
...
4. Add a typed facade over `BmuxSyncStore` if the UI wants a typed view.
...
- **Snapshot boundary for list subtrees.** The device list renders inside a
  ...
  `BmuxSyncStore` (no `@ObservedObject`, `@Bindable`, or a plain `let store`).
...
- iOS: new `Packages/Shared/BmuxSyncStore` raw-SQLite3 package mirroring
  `MobilePairedMacStore` (actor, `BmuxSyncStoring` protocol, `BmuxSyncStoreError`
