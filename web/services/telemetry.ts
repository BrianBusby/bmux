...
export async function withSpan<T>(
  ...
  return tracer.startActiveSpan(name, { attributes: cleanAttributes(attributes) }, async (span) => {
    ...
    try {
      ...
      span.setAttribute("bmux.duration_ms", Math.round((performance.now() - start) * 100) / 100);
      span.end();
    }
  });
}
...
export async function withApiRouteSpan<T extends Response>(
  ...
  return withSpan(
    "bmux-api",
    `bmux.api.${request.method} ${route}`,
    {
      "bmux.subsystem": "web",
      "bmux.runtime": "next-api",
    ...
    async (span) => {
      ...
      span.setAttribute("bmux.http.response_error", response.status >= 400);
...
export function recordSpanError(span: Span, err: unknown): void {
  if (err instanceof Error) {
    ...
    span.setAttributes({
      "bmux.error_name": err.name,
      "bmux.error_message": err.message,
    });
    return;
  }
  ...
  span.setAttributes({
    "bmux.error_name": "NonError",
    "bmux.error_message": message,
  });
}
