...
export interface ApnsNotificationInput {
   ...
   * `bmux.dismissedIds`) fanned out to every registered device.
   */
   ...
   * `bmux.notificationId` so an iOS swipe-dismiss can tell the Mac which
...
/**
 * APNs `aps.category` set on every bmux terminal push. iOS registers a
 ...
 * category id.
 */
export const BMUX_APNS_CATEGORY = "bmux.terminal";

/**
 * Build the APNs JSON payload. Adds `bmux.workspaceId`/`bmux.surfaceId`/
 * `bmux.macDeviceId`/`bmux.notificationId` custom keys so a tapped notification
...
export function buildApnsPayload(input: ApnsNotificationInput): Record<string, unknown> {
  ...
  const title = hidden ? "bmux" : input.title.trim() || "bmux";
  ...
  const aps: Record<string, unknown> = {
    ...
    category: BMUX_APNS_CATEGORY,
  };
  if (typeof input.badgeCount === "number") aps.badge = input.badgeCount;

  const bmux: Record<string, string> = {};
  if (input.workspaceId) bmux.workspaceId = input.workspaceId;
  if (input.surfaceId) bmux.surfaceId = input.surfaceId;
  if (input.macDeviceId) bmux.macDeviceId = input.macDeviceId;
  if (input.notificationId) bmux.notificationId = input.notificationId;

  return Object.keys(bmux).length > 0 ? { aps, bmux } : { aps };
}
 ...
 * the dismissed delivered banners listed under `bmux.dismissedIds`.
 *
...
function buildDismissPayload(input: ApnsNotificationInput): Record<string, unknown> {
  ...
  if (typeof input.badgeCount === "number") aps.badge = input.badgeCount;
  return { aps, bmux: { dismissedIds: [...(input.dismissedIds ?? [])] } };
}
