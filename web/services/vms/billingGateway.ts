...
export class VmBillingGateway extends Context.Tag("bmux/VmBillingGateway")<
  VmBillingGateway,
  VmBillingGatewayShape
>() {}

export const DEFAULT_FREE_CREATE_CREDIT_ITEM_ID = "bmux-vm-create-credit";
...
function createCreditItemId(
  ...
  const global = resolvedCreateCreditItemIdValue(env.BMUX_VM_CREATE_CREDIT_ITEM_ID);
...
function createCreditItemIdEnvKey(planId: string): string {
  return `BMUX_VM_PLAN_${planEnvKey(planId)}_CREATE_CREDIT_ITEM_ID`;
}
...
function createCreditCost(
  ...
  const providerKey = `BMUX_VM_CREATE_CREDIT_COST_${provider.toUpperCase()}`;
  const planProviderKey = `BMUX_VM_PLAN_${planKey}_CREATE_CREDIT_COST_${provider.toUpperCase()}`;
  const planKeyDefault = `BMUX_VM_PLAN_${planKey}_CREATE_CREDIT_COST`;
  const configured = firstConfiguredEnv(env, [
    planProviderKey,
    planKeyDefault,
    providerKey,
    "BMUX_VM_CREATE_CREDIT_COST",
  ]);
  const raw = configured?.value ?? "1";
  const key = configured?.key ??
    `${planProviderKey} or ${planKeyDefault} or ${providerKey} or BMUX_VM_CREATE_CREDIT_COST`;
...
function initialCreateCreditGrantAmount(
  ...
  const planKey = planEnvKey(planId);
  const configured = firstConfiguredEnv(env, [
    `BMUX_VM_PLAN_${planKey}_INITIAL_CREATE_CREDITS`,
    "BMUX_VM_INITIAL_CREATE_CREDITS",
  ]);
  return positiveInteger(
    configured?.value ?? String(DEFAULT_FREE_INITIAL_CREATE_CREDITS),
    configured?.key ?? `BMUX_VM_PLAN_${planKey}_INITIAL_CREATE_CREDITS or BMUX_VM_INITIAL_CREATE_CREDITS`,
  );
}
