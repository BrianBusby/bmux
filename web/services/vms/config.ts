...
export function assertVmCreateEnabled(
  ...
  if (isFalseFlag(env.BMUX_VM_CREATE_ENABLED)) {
...
export function providerEnabledEnvKey(provider: ProviderId): string {
  switch (provider) {
    case "e2b":
      return "BMUX_VM_E2B_ENABLED";
    case "freestyle":
      return "BMUX_VM_FREESTYLE_ENABLED";
    case "daytona":
      return "BMUX_VM_DAYTONA_ENABLED";
    default:
...
export function allowUnmanifestedImages(env: VmRuntimeEnv = process.env): boolean {
  return isTrueFlag(env.BMUX_VM_ALLOW_UNMANIFESTED_IMAGES) || !isDeployedRuntime(env);
}
