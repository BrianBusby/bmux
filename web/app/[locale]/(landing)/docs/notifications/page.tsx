  ...
  return (
    <>
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td><code>BMUX_NOTIFICATION_TITLE</code></td>
            <td>{t("envTitle")}</td>
          </tr>
          <tr>
            <td><code>BMUX_NOTIFICATION_SUBTITLE</code></td>
            <td>{t("envSubtitle")}</td>
          </tr>
          <tr>
            <td><code>BMUX_NOTIFICATION_BODY</code></td>
            <td>{t("envBody")}</td>
          </tr>
        </tbody>
      </table>
      <CodeBlock title="Examples" lang="bash">{`# Text-to-speech
say "$BMUX_NOTIFICATION_TITLE"

# Custom sound file
afplay /path/to/sound.aiff

# Log to file
echo "$BMUX_NOTIFICATION_TITLE: $BMUX_NOTIFICATION_BODY" >> ~/notifications.log`}</CodeBlock>
  ...
  "effects": {
    ...
    "paneFlash": true
      ...
      <CodeBlock lang="bash">{`bmux notify --title "Task Complete" --body "Your build finished"
bmux notify --title "Claude Code" --subtitle "Waiting" --body "Agent needs input"`}</CodeBlock>
...
printf '\\e]99;i=1;e=1;d=1;p=body:All tests passed\\e\\\\'`}</CodeBlock>
      ...
      <CodeBlock title="~/.claude/hooks/bmux-notify.sh" lang="bash">{`#!/bin/bash
# Skip if not in bmux
[ -S /tmp/bmux.sock ] || exit 0
...
case "$EVENT_TYPE" in
    "Stop")
        bmux notify --title "Claude Code" --body "Session complete"
        ;;
    "PostToolUse")
        [ "$TOOL" = "Task" ] && bmux notify --title "Claude Code" --body "Agent finished"
        ;;
esac`}</CodeBlock>
      <CodeBlock lang="bash">{`chmod +x ~/.claude/hooks/bmux-notify.sh`}</CodeBlock>
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/hooks/bmux-notify.sh"
          }
        ]
      }
    ],
  ...
  "hooks": {
    ...
    "agentStop": [
      {
        "type": "command",
        "bash": "if command -v bmux &>/dev/null; then bmux notify --title 'Copilot CLI' --body 'Done'; bmux set-status copilot_cli Idle; fi",
        "timeoutSec": 5
      }
    ],
    "errorOccurred": [
      {
        "type": "command",
        "bash": "if command -v bmux &>/dev/null; then bmux notify --title 'Copilot CLI' --subtitle 'Error' --body 'An error occurred'; bmux set-status copilot_cli Error; fi",
        "timeoutSec": 5
      }
    ],
    "sessionEnd": [
...
notify-after() {
  ...
  if [ $exit_code -eq 0 ]; then
    bmux notify --title "✓ Command Complete" --body "$1"
  else
    bmux notify --title "✗ Command Failed" --body "$1 (exit $exit_code)"
  fi
  return $exit_code
}
