    ...
    features: [
      {
        ...
        description:
          "Scale the entire bmux interface up or down with a single magnification control, not just the terminal font.",
      },
      {
        title: "Remote tmux Mirroring (Beta)",
        description:
          "Mirror a remote tmux session over SSH using -CC control mode, so remote windows show up as native bmux tabs that stay in sync with renames and reorders.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Opt in to have bmux name your workspaces and tabs from the agent conversation running inside them, so a wall of sessions stays readable at a glance.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Pair an iPhone with your Mac from the new Mobile Connect window and attach to your terminals from your phone, with opt-in forwarding of terminal notifications. The iOS beta ships on TestFlight as bmux BETA.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Custom sidebar extensions now run in their own process with an isolated interpreter, so a broken sidebar can't hang or crash bmux, and the interpreter covers a broader set of SwiftUI primitives.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Claude resume keeps bmux hooks attached so notifications and status tracking survive resumes, Agent Hibernation works for node-backed Claude sessions, and Codex resume preserves CODEX_HOME and pane order.",
      },
    ],
  },
  "0.64.13": {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "`bmux ssh` now forwards your local SSH agent, so remote sessions can use your local keys for git pushes and further hops without copying private keys onto the remote.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Open the diff viewer with a keyboard shortcut, configurable and editable in Settings alongside every other bmux shortcut.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Starship and other custom bash prompts no longer go static: the prompt bootstrap composes with your existing PROMPT_COMMAND instead of overwriting it. And `bmux ssh` now reports remote PTY allocation failures loudly instead of failing silently when a plain ssh would have worked.",
      },
      {
  ...
  "0.64.11": {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Fork Conversation moves into the tab right-click menu with configurable destinations, browser tabs gain an audio mute toggle, and `bmux diff` opens a CodeView diff viewer that streams large git diffs before full render.",
      },
    ],
  },
  "0.64.10": {
    ...
    features: [
      ...
      {
        title: "BmuxExtensionKit Sidebar Prototypes",
        description:
      ...
      },
      {
        ...
        description:
          "`bmux reorder-workspaces` accepts batch input, supports `--dry-run`, and emits reorder events so scripted layouts can plan a full sidebar reshuffle in one call and react to the changes via the socket.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Antigravity joins the supported coding-agent lineup with hook notifications, task manager attribution, and Vault session restore, the same way Claude, Codex, Gemini, and Grok already work in bmux.",
      },
      {
        title: "Native Grok Vault Resume",
        description:
          "Grok sessions can now be resumed natively from Vault. bmux parses registered Grok transcripts by layout, deduplicates sessions across shell-Grok homes, and quotes built-in resume commands so they round-trip cleanly.",
      },
      {
        title: "CLI Window Targeting",
        description:
          "bmux CLI commands now accept `--window` to scope workspace, pane, surface, SSH, VM, notifications, tree, and top flows to a specific window. Refs resolve inside the targeted window, and cross-window pane handles are rejected before they mutate state.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Grok Build joins the supported coding-agent lineup with notifications, task manager attribution, and session restore, the same way Claude, Codex, and Gemini already work in bmux.",
      },
      {
      ...
      },
      {
        ...
        description:
          "When bmux logs a crash, the notification now opens directly into the crash diagnostics view so you can inspect the breadcrumb trail without digging through state.",
      },
      {
        title: "Bug Fixes",
        description:
    ...
    features: [
      {
        ...
        description:
          "Fixes a critical regression in 0.64.5 where bmux ssh sessions would connect and render the remote prompt but drop every keystroke. The backgrounded ssh inside the startup wrapper now inherits the wrapper's stdin via <&0, so typing reaches the remote shell again. Thanks @kays0x for the community fix.",
      },
      {
      ...
      },
      {
        ...
        description:
          "bmux vm errors now include actionable next steps: sign-in instructions when not authenticated, suggested fixes for unknown flags, and usage examples for missing arguments.",
      },
    ],
  },
  "0.64.5": {
    ...
    features: [
      {
        ...
        description:
          "bmux codex-teams now maps Codex's subagent sessions into native bmux panes, the same way claude-teams does for Claude Code. Spawned subagents stack in a right column with sidebar metadata and notifications routed through bmux.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "The Files sidebar now follows SSH workspaces and shows the remote root instead of the local macOS path. SSH workspace descriptors restore on relaunch, and new guarded bmux://ssh deep links prompt before launching ssh so unfamiliar links can't run arbitrary commands.",
      },
      {
      ...
      },
      {
        ...
        description:
          "A new bmux browser cookies import CLI brings cookies from other browsers into bmux's browser panes so logged-in sessions follow you over.",
      },
      {
        title: "Quality-of-life Polish",
        description:
    ...
    features: [
      {
        ...
        description:
          "Closing the last window with the red X no longer drops your work. bmux restores prior panes on relaunch and resumes Claude Code, Codex, OpenCode, Gemini, and Rovo Dev sessions where you left off.",
      },
      {
        title: "Passkeys, WebAuthn, and FIDO2",
        description:
          "Sign in to passkey-protected sites directly inside bmux browser panes. Reworked inside-out signing keeps the notarized Developer ID build compatible with macOS authentication services.",
      },
      {
        title: "File Explorer",
        description:
      ...
      },
      {
        title: "Task Manager",
        description:
          "A built-in Task Manager window plus bmux top CLI shows a live snapshot of windows, workspaces, panes, surfaces, and browser webviews, with jumps from the manager into the matching surface.",
      },
    ],
  },
  "0.63.0": {
    ...
    features: [
      {
        title: "SSH",
        description:
          "bmux ssh user@remote creates a workspace for a remote machine. Browser panes route through the remote network so localhost just works. Drag an image into a remote session to upload via scp. Coding agent notifications come home to your local sidebar. Reconnects on drops.",
      },
      {
        title: "Claude Code Teams",
        description:
          "bmux claude-teams launches Claude Code's experimental teammate mode with one command. It sets up the environment, fakes a tmux session, and translates tmux commands into native bmux splits. Teammates stack vertically in a right column with sidebar metadata and notifications.",
      },
      {
        title: "oh-my-openagent",
        description:
          "bmux omo integrates oh-my-openagent (formerly oh-my-opencode), which orchestrates specialist agents across Claude, GPT, and Gemini in parallel. Same tmux shim as claude-teams, auto-installs the plugin, notifications route through bmux.",
      },
      {
        title: "Browser Profile Import",
        description:
          "Import cookies, history, and sessions from Chrome, Arc, Brave, Firefox, Safari, and 20+ browsers. The import wizard detects installed browsers, lets you pick profiles, and injects everything into bmux's browser panes so you're already logged in.",
      },
      {
        title: "Minimal Mode",
        description:
      ...
      },
      {
        ...
        description:
          "Define project-specific actions in bmux.json that launch from the command palette. One file per repo, no global config needed.",
      },
    ],
  },
  "0.62.0": {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Browser panels now support Cmd+F with inline find controls, so you can search long docs, dashboards, and issue threads without leaving bmux.",
      },
      {
        title: "Vi Copy Mode",
        description:
      ...
      },
      {
        ...
        description:
          "bmux now includes Japanese plus 16 additional languages, and a per-app language override lets you change the UI language without changing macOS system settings.",
      },
    ],
  },
  "0.61.0": {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Hit Cmd+Shift+P to open a searchable command palette. Every action in bmux is here: creating workspaces, toggling the sidebar, checking for updates, switching windows. Keyboard shortcuts are shown inline so you can learn them as you go.",
  ...
  "0.60.0": {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "The embedded browser now has full WebKit DevTools. Open them with the standard shortcut and they persist across tab switches. Inspect elements, debug JavaScript, and monitor network requests without leaving bmux.",
