      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>{t("release")}</td>
            <td>
              <code>/tmp/bmux.sock</code>
            </td>
          </tr>
          <tr>
            <td>{t("debug")}</td>
            <td>
              <code>/tmp/bmux-debug.sock</code>
            </td>
          </tr>
          <tr>
            <td>{t("taggedDebug")}</td>
            <td>
              <code>/tmp/bmux-debug-&lt;tag&gt;.sock</code>
            </td>
          </tr>
        </tbody>
      </table>
...
{"id":"req-1","ok":true,"result":{"workspaces":[...]}}`}</CodeBlock>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td>
              <strong>bmux processes only</strong>
            </td>
            <td>{t("bmuxOnlyMode")}</td>
            <td>{t("bmuxOnlyEnable")}</td>
          </tr>
          <tr>
            <td>
...
bmux current-workspace --json`}
      ...
      <Cmd
        ...
        cli={`bmux close-workspace --workspace <id>`}
...
bmux send "ls -la\\n"`}
      ...
      <Cmd
        ...
        cli={`bmux send-key --surface <id> enter`}
...
bmux list-notifications --json`}
      ...
      <Cmd
        ...
        cli={`bmux set-status build "compiling" --icon hammer --color "#ff9500" --priority 80
bmux set-status deploy "v1.2.3" --workspace workspace:2`}
        socket={`set_status build compiling --icon=hammer --color=#ff9500 --priority=80 --tab=<workspace-uuid>`}
      />
      <Cmd
        name="clear-status"
        desc={t("clearStatusDesc")}
        cli={`bmux clear-status build`}
        socket={`clear_status build --tab=<workspace-uuid>`}
      />
      <Cmd
        name="list-status"
        desc={t("listStatusDesc")}
        cli={`bmux list-status`}
        socket={`list_status --tab=<workspace-uuid>`}
      />
      <Cmd
        name="set-progress"
        desc={t("setProgressDesc")}
        cli={`bmux set-progress 0.5 --label "Building..."
bmux set-progress 1.0 --label "Done"`}
        socket={`set_progress 0.5 --label=Building... --tab=<workspace-uuid>`}
      />
      <Cmd
        name="clear-progress"
        desc={t("clearProgressDesc")}
        cli={`bmux clear-progress`}
        socket={`clear_progress --tab=<workspace-uuid>`}
      />
      <Cmd
        name="log"
        desc={t("logDesc")}
        cli={`bmux log "Build started"
bmux log --level error --source build "Compilation failed"
bmux log --level success -- "All 42 tests passed"`}
      ...
      <Cmd
        ...
        desc={t("listLogDesc")}
        cli={`bmux list-log
bmux list-log --limit 5`}
        socket={`list_log --limit=5 --tab=<workspace-uuid>`}
      />
      <Cmd
        name="sidebar-state"
        desc={t("sidebarStateDesc")}
        cli={`bmux sidebar-state
bmux sidebar-state --workspace workspace:2`}
...
bmux identify --json`}
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              <code>BMUX_SOCKET_PATH</code>
            </td>
            <td>{t("socketPathDesc")}</td>
          </tr>
          <tr>
            <td>
              <code>BMUX_SOCKET_ENABLE</code>
            </td>
            <td>{t("socketEnableDesc")}</td>
          </tr>
          <tr>
            <td>
              <code>BMUX_SOCKET_MODE</code>
            </td>
            <td>{t("socketModeDesc")}</td>
          </tr>
          <tr>
            <td>
              <code>BMUX_WORKSPACE_ID</code>
            </td>
            <td>{t("workspaceIdDesc")}</td>
          </tr>
          <tr>
            <td>
              <code>BMUX_SURFACE_ID</code>
            </td>
      ...
      <DocsHeading level={2} id="detecting-bmux">{t("detectingBmux")}</DocsHeading>
      <CodeBlock title="bash" lang="bash">{`# Prefer explicit socket path if set
SOCK="\${BMUX_SOCKET_PATH:-/tmp/bmux.sock}"
[ -S "$SOCK" ] && echo "Socket available"

# Check for the CLI
command -v bmux &>/dev/null && echo "bmux available"

# In bmux-managed terminals these are auto-set
[ -n "\${BMUX_WORKSPACE_ID:-}" ] && [ -n "\${BMUX_SURFACE_ID:-}" ] && echo "Inside bmux surface"

# Distinguish from regular Ghostty
[ "$TERM_PROGRAM" = "ghostty" ] && [ -n "\${BMUX_WORKSPACE_ID:-}" ] && echo "In bmux"`}</CodeBlock>
...
bmux_cmd '{"id":"ws","method":"workspace.list","params":{}}'
bmux_cmd '{"id":"notify","method":"notification.create","params":{"title":"Done","body":"Task complete"}}'`}</CodeBlock>

      <DocsHeading level={3} id="build-script-notification">{t("buildScriptNotification")}</DocsHeading>
      <CodeBlock title="bash" lang="bash">{`#!/bin/bash
npm run build
if [ $? -eq 0 ]; then
    bmux notify --title "✓ Build Success" --body "Ready to deploy"
else
    bmux notify --title "✗ Build Failed" --body "Check the logs"
fi`}</CodeBlock>
    </>
  );
}
