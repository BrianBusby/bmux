...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: "blog.bmuxSsh" });
  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    keywords: [
      "bmux", "SSH", "remote development", "terminal", "macOS",
    ...
    alternates: buildAlternates(locale, "/blog/bmux-ssh"),
  };
}

export default async function BmuxSshPage({
  params,
}: {
  ...
  const t = await getTranslations({ locale, namespace: "blog.posts.bmuxSsh" });
  const tc = await getTranslations({ locale, namespace: "common" });

  return (
    <>
      <BlogSchema postKey="bmuxSsh" path="/blog/bmux-ssh" datePublished="2026-03-30T00:00:00Z" />
      <div className="mb-8">
        <Link
          href="/blog"
      ...
      <video
        src="/blog/bmux-ssh-image-upload.mp4"
        width={1824}
        height={1080}
        autoPlay
        loop
      ...
      <ul className="mt-4 space-y-1">
        ...
        <li><code>bmux claude-teams</code> and <code>bmux omo</code> work over SSH, spawning teammate panes locally while computation runs remote</li>
