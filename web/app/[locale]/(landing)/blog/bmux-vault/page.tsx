...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "blog.bmuxVault" });
  ...
  return {
    ...
    alternates: buildAlternates(locale, "/blog/bmux-vault"),
  };
}

export default async function BmuxVaultPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations({ locale, namespace: "blog.posts.bmuxVault" });
  const tc = await getTranslations({ locale, namespace: "common" });

  return (
    <>
      <BlogSchema postKey="bmuxVault" path="/blog/bmux-vault" datePublished="2026-05-22T00:00:00Z" />
      <div className="mb-8">
        <Link
          href="/blog"
