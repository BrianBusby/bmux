...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: "blog.introducingBmux" });
  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    keywords: [
      "bmux", "terminal", "macOS", "Ghostty", "libghostty",
    ...
    alternates: buildAlternates(locale, "/blog/introducing-bmux"),
  };
}

export default function IntroducingBmuxPage() {
  const t = useTranslations("blog.posts.introducingBmux");
  const tc = useTranslations("common");

  return (
    <>
      <BlogSchema postKey="introducingBmux" path="/blog/introducing-bmux" datePublished="2026-02-12T00:00:00Z" />
      <div className="mb-8">
        <Link
          href="/blog"
