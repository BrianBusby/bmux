...
import {
  awesomeBmuxCategoryOrder,
  awesomeBmuxCuratedProjectRows,
  awesomeBmuxProjects,
  awesomeBmuxSourceUrl,
} from "./awesome-bmux-projects";
...
const categorySummaries = awesomeBmuxCategoryOrder
  .map((category) => ({
    category,
    count: awesomeBmuxProjects.filter((project) =>
      (project.categories as readonly string[]).includes(category),
    ).length,
  }))
  .filter(({ count }) => count > 0);

const categoryPlacementCount = awesomeBmuxProjects.reduce(
...
export default function CommunityPage() {
  ...
  const stats = [
    {
      value: awesomeBmuxProjects.length,
      ...
      <main className="w-full max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="mb-8 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
          ...
          <span className="text-muted">
            {numberFormatter.format(awesomeBmuxCuratedProjectRows)}{" "}
            {t("sourceRowsLabel")}
          </span>
        </div>
