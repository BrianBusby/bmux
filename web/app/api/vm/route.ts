...
export async function GET(request: Request): Promise<Response> {
  return withAuthedVmApiRoute(
    ...
    async ({ user, span }) => {
      ...
      try {
        if (requestedBillingTeamId || user.billingCustomerType === "team") {
          ...
          setSpanAttributes(span, {
            "bmux.billing.team_id_set": !!billingTeamId,
            "bmux.billing.customer_type": entitlements.billingCustomerType,
            "bmux.billing.plan_id": entitlements.planId,
          });
        }
      } catch (err) {
      ...
      setSpanAttributes(span, { "bmux.vm.count": entries.length });
  ...
  return withAuthedVmApiRoute(
    ...
    async ({ user: initialUser, span, authDurationMs, routeStartedAtMs, setResponseFinalizer }) => {
      ...
      let user: AuthedUser = initialUser;
      {
        ...
        // Idempotency-Key is standard HTTP; we also accept x-bmux-idempotency-key for CLI
        ...
        const rawKey = (
          ...
          request.headers.get("x-bmux-idempotency-key") ||
          ""
        ).trim();
        const idempotencyKey = rawKey ? rawKey.slice(0, 128) : undefined;
        setSpanAttributes(span, {
          "bmux.vm.provider": provider,
          "bmux.vm.image_set": image.length > 0,
          "bmux.vm.image_version": imageSelection.imageVersion,
          "bmux.vm.image_manifest": !!imageSelection.manifestEntry,
        ...
        setSpanAttributes(span, {
          ...
          "bmux.billing.customer_type": entitlements.billingCustomerType,
          "bmux.billing.plan_id": entitlements.planId,
          "bmux.billing.requested_team_id_set": !!requestedBillingTeamId,
          "bmux.vm.max_active": entitlements.maxActiveVms,
        });
        ...
        try {
          ...
          if (isVmCreateInProgressError(err)) {
            return vmErrorResponse({
              ...
              action: "Wait for the first `bmux vm new` to finish. If your terminal was interrupted, retry the same command and bmux will reuse the in-flight request.",
          ...
          if (isVmCreateFailedError(err)) {
            return vmErrorResponse({
              ...
              action: "Retry with a fresh `bmux vm new`. If it fails again, copy the details and contact support.",
              details: {
          ...
          if (isVmLimitExceededError(err)) {
            return vmErrorResponse({
              ...
              action: "Run `bmux vm ls`, then stop or delete an active VM with `bmux vm rm <id>` before creating another. Paused VMs do not count against this limit.",
        ...
        setSpanAttributes(span, { "bmux.vm.id": created.providerVmId });
        return jsonResponse({
...
function invalidTeamIdResponse(): Response {
  return vmErrorResponse({
    ...
    action: "Use a team id from `bmux auth status`, or omit `teamId` when the signed-in account has one team.",
    details: { field: "teamId" },
  });
}

function requestHasBlankVmTeamId(request: Request): boolean {
  for (const header of ["x-bmux-team-id", "x-bmux-billing-team-id"]) {
...
function billingTeamErrorResponse(err: {
  ...
  if (err.code === "vm_billing_team_not_found") {
    return vmErrorResponse({
      ...
      action: "Switch to a team you belong to, or run `bmux auth login` again and retry with the correct team id.",
  ...
  return vmErrorResponse({
    error: err.code,
    status: err.status,
    message: "bmux needs to know which team should own this Cloud VM.",
    action: "Select a team in bmux, or pass the team id with `X-Bmux-Team-Id`. If you do not see a team, run `bmux auth login` again.",
