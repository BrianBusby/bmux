// Device registry — register a Mac/host (and its running bmux app instance) and
...
// /api/device-tokens). Team scope: the caller picks a team via `X-Bmux-Team-Id`
 ...
 * member of. A requested team (`X-Bmux-Team-Id` / `?teamId=`) must appear in the
...
/**
 * Register (or refresh) a device and its running bmux app instance. Idempotent
...
export async function POST(request: Request): Promise<Response> {
  ...
  // validation AND is what `bmux remotes` uses to scope list/remove. Strip any
  ...
  // `manual: true` marks a user-initiated remote added through the bmux CLI
  // (`bmux remotes add`) rather than a Mac self-registering its own live
  ...
  // Persist the manual marker on the device so `bmux remotes` can scope list and
  ...
  const registered = await db.transaction(async (tx) => {
    ...
    // same bmux device UUID registering under a different team is a separate,
...
export async function GET(request: Request): Promise<Response> {
  ...
  const devicesPayload = deviceRows.map((device) => ({
    // The phone matches its stored `macDeviceID` (the bmux device UUID) against
...
export async function DELETE(request: Request): Promise<Response> {
  ...
  // was removed (e.g. `bmux remotes remove <not-owned-uuid>`).
