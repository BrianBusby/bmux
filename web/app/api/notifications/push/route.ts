...
function apnsConfig(): ApnsConfig | null {
  const keyP8 = env.BMUX_APNS_KEY_P8;
  const keyId = env.BMUX_APNS_KEY_ID;
  const teamId = env.BMUX_APNS_TEAM_ID;
...
async function sendPush(request: Request): Promise<Response> {
  ...
  if (process.env.VERCEL === "1" && env.BMUX_PUSH_RATE_LIMIT_ID) {
    const { error, rateLimited } = await checkRateLimit(env.BMUX_PUSH_RATE_LIMIT_ID, {
      request,
    ...
    if (error === "not-found") {
      console.error("notifications.push.rate_limit_not_found", env.BMUX_PUSH_RATE_LIMIT_ID);
    }
  }
