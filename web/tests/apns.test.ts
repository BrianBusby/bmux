...
describe("apns payload", () => {
  test("builds a time-sensitive alert with deep-link keys", () => {
    const payload = buildApnsPayload({
      ...
      surfaceId: "sf-2",
      macDeviceId: "mac-3",
    }) as { aps: Record<string, unknown>; bmux: Record<string, string> };
    ...
    expect(payload.bmux).toEqual({ workspaceId: "ws-1", surfaceId: "sf-2", macDeviceId: "mac-3" });
  });

  test("omits bmux block when no ids", () => {
    const payload = buildApnsPayload({ title: "t", body: "b" }) as Record<string, unknown>;
    expect("bmux" in payload).toBe(false);
  });
  ...
  test("carries the stable notification id and dismiss-sync category", () => {
    ...
    // The category is what arms iOS customDismissAction; the bmux key lets an
    // iOS swipe tell the Mac which notification was dismissed.
    expect(payload.aps.category).toBe(BMUX_APNS_CATEGORY);
    expect(payload.bmux).toEqual({ workspaceId: "ws-1", notificationId: "n-42" });
  });
  ...
  test("keeps the notification id even when content is hidden (id is not content)", () => {
    const payload = buildApnsPayload({
      ...
      notificationId: "n-9",
      hideContent: true,
    }) as { aps: { alert: Record<string, string>; category: string }; bmux: Record<string, string> };

    expect(payload.aps.alert.title).toBe("bmux");
    expect(payload.aps.category).toBe(BMUX_APNS_CATEGORY);
    expect(payload.bmux).toEqual({ notificationId: "n-9" });
  });
  ...
  test("hideContent redacts terminal content but keeps a generic compatibility body and deep-link", () => {
    const payload = buildApnsPayload({
      ...
      workspaceId: "ws-9",
      hideContent: true,
    }) as { aps: { alert: Record<string, string> }; bmux: Record<string, string> };

    expect(payload.aps.alert.title).toBe("bmux");
    expect(payload.aps.alert.body).toBe("An agent needs your attention");
    expect(payload.aps.alert.subtitle).toBeUndefined();
    expect(payload.bmux).toEqual({ workspaceId: "ws-9" });
  });

  test("empty title falls back to bmux", () => {
  ...
  test("dismiss push is banner-less: content-available + badge + dismissed ids only", () => {
    const payload = buildApnsPayload({
      ...
      badgeCount: 0,
    }) as { aps: Record<string, unknown>; bmux: Record<string, unknown> };
    ...
    expect(payload.bmux).toEqual({ dismissedIds: ["n-1", "n-2"] });
  });
});
...
describe("apns route policy", () => {
  test("allows only bmux iOS bundle IDs and derives the APNs environment", () => {
    expect(normalizeApnsBundle("com.bmuxterm.app")).toEqual({
      bundleId: "com.bmuxterm.app",
      environment: "production",
    });
    expect(normalizeApnsBundle("dev.bmux.app.beta")).toEqual({
      bundleId: "dev.bmux.app.beta",
      environment: "production",
    });
    expect(normalizeApnsBundle("dev.bmux.ios.push1")).toEqual({
      bundleId: "dev.bmux.ios.push1",
      environment: "sandbox",
    });

    expect(normalizeApnsBundle("com.example.app")).toBeNull();
    expect(normalizeApnsBundle("dev.bmux.ios.bad_topic")).toBeNull();
    expect(normalizeApnsBundle("dev.bmux.ios.-bad")).toBeNull();
  });
...
describe("apns sender transport", () => {
  test("starts sandbox and production host groups concurrently", async () => {
    ...
    const resultPromise = sendApnsNotification(
      ...
      [
        { deviceToken: "a".repeat(64), bundleId: "dev.bmux.ios.push1", environment: "sandbox" },
        { deviceToken: "b".repeat(64), bundleId: "com.bmuxterm.app", environment: "production" },
      ],
  ...
  test("keeps same-host successes when another request fails to start", async () => {
    ...
    const results = await sendApnsNotification(
      ...
      [
        { deviceToken: "a".repeat(64), bundleId: "com.bmuxterm.app", environment: "production" },
        { deviceToken: "b".repeat(64), bundleId: "dev.bmux.app.beta", environment: "production" },
      ],
      { title: "agent", body: "done" },
      1000,
      transport,
