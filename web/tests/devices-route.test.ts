...
function authHeaders(teamId?: string): Record<string, string> {
  ...
  if (teamId) base["x-bmux-team-id"] = teamId;
  return base;
}

function registerRequest(body: Record<string, unknown>, teamId?: string): Request {
  return new Request("https://bmux.test/api/devices", {
    method: "POST",
...
describe("device registry route", () => {
  dbTest("registers a Mac and its app instance, then lists it for the team", async () => {
    ...
    const listResponse = await GET(
      new Request("https://bmux.test/api/devices", { method: "GET", headers: authHeaders() }),
    );
  ...
  dbTest("re-registering the same (device, tag) refreshes routes in place (auto-pair path)", async () => {
    ...
    const list = (await (
      await GET(new Request("https://bmux.test/api/devices", { method: "GET", headers: authHeaders() }))
  ...
  dbTest("registers the same device UUID in two teams the user belongs to", async () => {
    ...
    // Same physical Mac (same bmux UUID), registered under team-a then team-b.
    ...
    const listA = (await (
      await GET(new Request("https://bmux.test/api/devices", { method: "GET", headers: authHeaders("team-a") }))
  ...
  dbTest("delete removes the device and cascades its instances", async () => {
    ...
    const del = await DELETE(
      new Request("https://bmux.test/api/devices", {
        method: "DELETE",
        headers: authHeaders(),
  ...
  // --- Manual remotes (bmux remotes add) ---
  ...
  dbTest("client-supplied labels.manual cannot spoof the manual marker", async () => {
    ...
    // The spoofed labels.manual was stripped, so `bmux remotes` (which filters on
  ...
  dbTest("marks manual remotes with labels.manual; self-registration is unmarked", async () => {
    ...
    // A manual remote (bmux remotes add) and a self-registered Mac.
    await POST(
      registerRequest({
        deviceId: DEVICE_A,
        platform: "mac",
    ...
    // the manual remote is listed/removable by `bmux remotes`.
