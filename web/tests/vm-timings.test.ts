...
describe("VM timing helpers", () => {
  ...
  test("finish is idempotent", () => {
    ...
    expect(attributes.filter((attribute) => attribute.key === "bmux.vm.timing.total_ms")).toHaveLength(1);
    expect(attributes.filter((attribute) => attribute.key === "bmux.vm.timing.total_count")).toHaveLength(1);
  });
});
