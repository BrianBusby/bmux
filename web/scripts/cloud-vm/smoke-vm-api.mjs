...
function vercelCurlFetch(url, init = {}, timeoutMs = REQUEST_TIMEOUT_MS) {
  ...
  const scratch = mkdtempSync(path.join(tmpdir(), "bmux-vercel-curl-"));
...
try {
  ...
  user = await app.createUser({
    primaryEmail: `bmux-${project.stackLabel}-smoke+${suffix}@manaflow.dev`,
    primaryEmailVerified: true,
    primaryEmailAuthEnabled: true,
    password: randomBytes(24).toString("base64url"),
    displayName: `bmux ${project.stackLabel} smoke`,
  });
