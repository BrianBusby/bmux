...
try {
  ...
  if ((env.BMUX_DB_SSL_CA_PEM || env.BMUX_DB_SSL_CA_PEM_BASE64) && process.env.BMUX_ALLOW_DB_CA_OVERRIDE !== "1") {
    throw new Error(
      "BMUX_DB_SSL_CA_PEM(_BASE64) is set. Current Vercel Aurora RDS certs chain to Amazon Root CA 1, so Node's default trust store should be used. Remove the override, redeploy, then retry. Set BMUX_ALLOW_DB_CA_OVERRIDE=1 only for a verified private CA.",
    );
  }
  ...
  const pool = new Pool({
    ...
    password: authToken,
    ssl: { rejectUnauthorized: parseBoolean(env.BMUX_DB_SSL_REJECT_UNAUTHORIZED, true) },
    max: 1,
  });
