...
choose_bmux_port() {
  local dev_offset="${BMUX_DB_PORT_OFFSET:-10000}"
  local test_offset="${BMUX_TEST_DB_PORT_OFFSET:-30000}"
  ...
  if (( max_candidate < min_candidate )); then
    echo "invalid BMUX_*_PORT_OFFSET values for port selection" >&2
    exit 1
  fi
  if [[ -n "${BMUX_MIGRATION_PREFLIGHT_PORT:-}" ]]; then
    printf '%s\n' "$BMUX_MIGRATION_PREFLIGHT_PORT"
    return
  fi
  ...
  echo "failed to find free BMUX_PORT for migration preflight" >&2
  exit 1
}

export BMUX_PORT="$(choose_bmux_port)"
echo "using isolated BMUX_PORT=$BMUX_PORT for migration preflight"
...
cleanup_test_db() {
  ...
  docker volume ls --format '{{.Name}}' \
    | grep -E "^bmux-postgres-.*-test-${BMUX_PORT}$" \
