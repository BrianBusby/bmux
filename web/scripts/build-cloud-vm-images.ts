...
const buildRoot = path.join(webRoot, ".bmux-cloud-build");
...
const PRIMARY_LINUX_USER = "bmux";
const NODE_MAJOR = String(positiveIntFromEnv("BMUX_CLOUD_IMAGE_NODE_MAJOR", 22));
const BUN_VERSION = semverFromEnv("BMUX_CLOUD_IMAGE_BUN_VERSION", "1.3.13");
const FREESTYLE_SNAPSHOT_CREATE_TIMEOUT_MS = positiveIntFromEnv(
  "BMUX_FREESTYLE_SNAPSHOT_CREATE_TIMEOUT_MS",
  20 * 60 * 1000,
);
const FREESTYLE_SNAPSHOT_RECOVERY_TIMEOUT_MS = positiveIntFromEnv(
  "BMUX_FREESTYLE_SNAPSHOT_RECOVERY_TIMEOUT_MS",
  10 * 60 * 1000,
);
const FREESTYLE_SNAPSHOT_RECOVERY_POLL_INTERVAL_MS = positiveIntFromEnv(
  "BMUX_FREESTYLE_SNAPSHOT_RECOVERY_POLL_INTERVAL_MS",
  5_000,
);
const FREESTYLE_SNAPSHOT_RECOVERY_CLOCK_SKEW_MS = positiveIntFromEnv(
  "BMUX_FREESTYLE_SNAPSHOT_RECOVERY_CLOCK_SKEW_MS",
  2 * 60 * 1000,
);
const DAYTONA_SNAPSHOT_CREATE_TIMEOUT_MS = positiveIntFromEnv(
  "BMUX_DAYTONA_SNAPSHOT_CREATE_TIMEOUT_MS",
  20 * 60 * 1000,
);
const DAYTONA_ENTRYPOINT_PATH = "/usr/local/bin/bmux-daytona-entrypoint";
const CLOUD_AGENT_TOOLS = [
  {
    name: "claude",
    envVar: "BMUX_CLOUD_IMAGE_CLAUDE_CODE_NPM_SPEC",
    packageSpec: "@anthropic-ai/claude-code@2.1.137",
    binaries: ["claude"],
  },
  {
    name: "opencode",
    envVar: "BMUX_CLOUD_IMAGE_OPENCODE_NPM_SPEC",
    packageSpec: "opencode-ai@1.14.41",
    binaries: ["opencode"],
  },
  {
    name: "codex",
    envVar: "BMUX_CLOUD_IMAGE_CODEX_NPM_SPEC",
    packageSpec: "@openai/codex@0.130.0",
    binaries: ["codex"],
  },
  {
    name: "pi",
    envVar: "BMUX_CLOUD_IMAGE_PI_NPM_SPEC",
...
async function main(): Promise<void> {
  ...
  const binaryPath = path.join(buildRoot, tag, "bmuxd-remote-linux-amd64");
  ...
  const imageMetadata = {
    ...
    bmuxdRemoteCommit: await gitRevParse(path.join(repoRoot, "daemon/remote")),
...
async function buildRemoteDaemon(outPath: string): Promise<void> {
  await runCommand(
    "go",
    ["build", "-trimpath", "-ldflags=-s -w", "-o", outPath, "./cmd/bmuxd-remote"],
    {
...
async function buildE2BTemplate(
  ...
  const template = Template({ fileContextPath })
    ...
    .copy(path.basename(daemonPath), "/usr/local/bin/bmuxd-remote", {
      forceUpload: true,
      mode: 0o755,
    })
    ...
    .setStartCmd(
      "/usr/local/bin/bmuxd-remote serve --ws --listen 0.0.0.0:7777 --auth-lease-file /tmp/bmux/attach-pty-lease.json --rpc-auth-lease-file /tmp/bmux/attach-rpc-lease.json --shell /bin/bash",
      waitForURL("http://127.0.0.1:7777/healthz", 200),
    );

  const name = `bmuxd-ws:${tag}`;
  ...
  return {
    ...
    manifestEntry: {
      ...
      version: `e2b-${tag}`,
      imageId: name,
      envVar: "E2B_BMUXD_WS_TEMPLATE",
      defaultForLocalDev: false,
      bmuxdRemoteCommit: metadata.bmuxdRemoteCommit,
...
async function buildFreestyleSnapshot(
  ...
  return {
    ...
    manifestEntry: {
      ...
      defaultForLocalDev: false,
      features: process.env.BMUX_FREESTYLE_ADMIN_SIGNING_PUBLIC_KEY?.trim()
  ...
  const snapshot = await daytona.snapshot.create(
    {
      ...
      // Also registered on the snapshot record so sandboxes restart bmuxd-remote on
 ...
 * packages, the bmux Linux user, agent tools, and bmuxd-remote on 7777 with lease files.
 *
 * No sshd is baked. bmux uses Daytona attach exclusively over preview-URL WebSockets, and even
...
export function daytonaSnapshotImage(daemonPath: string): Image {
  return Image.base("ubuntu:24.04")
    ...
    .addLocalFile(daemonPath, "/usr/local/bin/bmuxd-remote")
    .runCommands(
      ...[
        "chmod 0755 /usr/local/bin/bmuxd-remote",
...
export function daytonaEntrypointCommands(): string[] {
  ...
  // re-runs it on every sandbox start, which is what brings bmuxd-remote back after a
  // stop/start (bmux pause/resume) cycle.
  return [
    `cat > ${DAYTONA_ENTRYPOINT_PATH} <<'BMUX_DAYTONA_ENTRYPOINT'
#!/bin/sh
mkdir -p /tmp/bmux
chmod 700 /tmp/bmux
while true; do
  /usr/local/bin/bmuxd-remote serve --ws --listen 0.0.0.0:7777 --auth-lease-file /tmp/bmux/attach-pty-lease.json --rpc-auth-lease-file /tmp/bmux/attach-rpc-lease.json --shell /usr/local/bin/bmux-cloud-shell
  sleep 2
done
BMUX_DAYTONA_ENTRYPOINT`,
...
function cloudRootSetupCommands(): string[] {
  return [
    ...
    "mkdir -p /tmp/bmux && chown bmux:bmux /tmp/bmux && chmod 700 /tmp/bmux",
    "ln -sf /usr/local/bin/bmuxd-remote /usr/local/bin/bmux",
  ];
}

function cloudShellProfileCommands(): string[] {
  return [
    "mkdir -p /etc/bmux /home/bmux/.config/bmux /home/bmux/.bmux",
...
Type=simple
User=root
Environment=BMUXD_WS_ADMIN_ED25519_PUBLIC_KEY=${publicKey}
ExecStart=/usr/local/bin/bmuxd-remote serve --ws --listen 0.0.0.0:7777 --auth-lease-file /tmp/bmux/attach-pty-lease.json --rpc-auth-lease-file /tmp/bmux/attach-rpc-lease.json --shell /usr/local/bin/bmux-cloud-shell
Restart=always
RestartSec=2
...
BMUXD_WS_SERVICE`,
    "mkdir -p /etc/systemd/system/multi-user.target.wants && ln -sf /etc/systemd/system/bmuxd-ws.service /etc/systemd/system/multi-user.target.wants/bmuxd-ws.service",
  ];
}
...
export function cloudImageSmokeTestCommands(): string[] {
  const agentToolVersionChecks = cloudAgentToolPackageSpecs().flatMap((tool) =>
    tool.binaries.map((binary) => `${binary} --version >/tmp/bmux-${tool.name}-version.txt 2>&1`)
  );
  const toolchainEnv = toolchainSmokeEnvironmentPrefix();
  return [
    "printf 'int main(void) { return 0; }\\n' >/tmp/bmux-build-smoke.c && gcc /tmp/bmux-build-smoke.c -o /tmp/bmux-build-smoke && /tmp/bmux-build-smoke && g++ --version >/dev/null && make --version >/dev/null && pkg-config --version >/dev/null && rm -f /tmp/bmux-build-smoke.c /tmp/bmux-build-smoke",
    "openssl version -a >/tmp/bmux-openssl-version.txt 2>&1",
    "gh --version >/tmp/bmux-gh-version.txt 2>&1",
    "htop --version >/tmp/bmux-htop-version.txt 2>&1",
    "btop --version >/tmp/bmux-btop-version.txt 2>&1",
    "tmux -V >/tmp/bmux-tmux-version.txt 2>&1",
    "zsh --version >/tmp/bmux-zsh-version.txt 2>&1",
    ...
    "python3 -m pip --version >/tmp/bmux-pip-version.txt 2>&1",
    "python3 -m venv /tmp/bmux-venv-smoke && rm -rf /tmp/bmux-venv-smoke",
    `${toolchainEnv} test "$(command -v node)" = "${TOOLCHAIN_SHIMS_DIR}/node" && node --version >/tmp/bmux-node-version.txt 2>&1 && mise which node >/tmp/bmux-mise-node-path.txt 2>&1`,
    "npm --version >/tmp/bmux-npm-version.txt 2>&1",
    "bun --version >/tmp/bmux-bun-version.txt 2>&1",
    "go version >/tmp/bmux-go-version.txt 2>&1",
    "gh --version >/tmp/bmux-gh-version.txt 2>&1",
    `${toolchainEnv} rustup show active-toolchain >/tmp/bmux-rustup-toolchain.txt 2>&1 && grep -q '^stable' /tmp/bmux-rustup-toolchain.txt && rustc --version >/tmp/bmux-rustc-version.txt 2>&1 && cargo --version >/tmp/bmux-cargo-version.txt 2>&1`,
    "mise --version >/tmp/bmux-mise-version.txt 2>&1",
    "test -x /usr/local/bin/bmuxd-remote && test -x /usr/local/bin/bmux",
    "bmux --help >/tmp/bmux-cli-help.txt 2>&1",
    "bmux --socket /tmp/bmux-browser-smoke.sock browser >/tmp/bmux-browser-help.txt 2>&1; status=$?; test \"$status\" -eq 2 && grep -q 'requires a subcommand' /tmp/bmux-browser-help.txt",
    "bmuxd-remote version >/tmp/bmuxd-remote-version.txt 2>&1",
...
export function cloudToolInstallCommands(): string[] {
  ...
  return [
    ...
    toolPackages.length > 0
      ? `npm install -g --omit=dev --no-audit --fund=false ${toolPackages.map((tool) => shellQuote(tool.packageSpec)).join(" ")} >/tmp/bmux-npm-install.txt 2>&1`
      : "true",
    miseInstallCommand(),
...
function bunInstallCommand(): string {
  ...
  const commands = [
    "set -eu",
    "rm -rf /tmp/bmux-bun-install",
    "mkdir -p /tmp/bmux-bun-install",
    "cd /tmp/bmux-bun-install",
  ...
  return `{ ${commands.join(" && ")}; } >/tmp/bmux-bun-install.txt 2>&1`;
}
...
function miseInstallCommand(): string {
  ...
  return `{ ${commands.join(" && ")}; } >/tmp/bmux-mise-install.txt 2>&1`;
}
...
function rustupInstallCommand(): string {
  const commands = [
    ...
    "curl --proto '=https' --tlsv1.2 -fsSL https://sh.rustup.rs -o /tmp/bmux-rustup-init.sh",
    "sh /tmp/bmux-rustup-init.sh -y --profile minimal --default-toolchain stable --no-modify-path",
    "rm -f /tmp/bmux-rustup-init.sh",
    "rm -rf \"$RUSTUP_HOME/downloads\" \"$RUSTUP_HOME/tmp\" \"$CARGO_HOME/registry\"",
    "chmod -R a+rX \"$RUSTUP_HOME\" \"$CARGO_HOME\"",
  ];
  return `{ ${commands.join(" && ")}; } >/tmp/bmux-rustup-install.txt 2>&1`;
}
...
function toolchainProfileCommand(): string {
  ...
  return [
    ...
    `printf '%b\\n' ${shellQuote(profile)} >/etc/profile.d/bmux-toolchains.sh`,
    "chmod 0644 /etc/environment /etc/profile.d/bmux-toolchains.sh",
  ].join(" && ");
}
...
function freestylePythonOpenSSLCommands(): string[] {
  return [
    "apt-get update",
    "mkdir -p /tmp/bmux-libssl /opt/bmux/openssl/lib",
    "cd /tmp/bmux-libssl && apt-get download libssl3t64",
    "dpkg-deb -x /tmp/bmux-libssl/libssl3t64_*.deb /tmp/bmux-libssl/root",
    "cp /tmp/bmux-libssl/root/usr/lib/*-linux-gnu/libssl.so.3 /opt/bmux/openssl/lib/",
    "cp /tmp/bmux-libssl/root/usr/lib/*-linux-gnu/libcrypto.so.3 /opt/bmux/openssl/lib/",
    "cat <<'EOF' >/usr/local/bin/python3\n#!/bin/sh\nexport LD_LIBRARY_PATH=\"/opt/bmux/openssl/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}\"\nexec /usr/bin/python3 \"$@\"\nEOF",
    "chmod 0755 /usr/local/bin/python3",
    "ln -sf /usr/local/bin/python3 /usr/local/bin/python",
    "rm -rf /tmp/bmux-libssl /var/lib/apt/lists/*",
  ];
}
...
export function freestyleBaseDockerfileContent(daemonURL: string): string {
  return [
    ...
    `RUN curl -fsSL ${shellQuote(daemonURL)} -o /usr/local/bin/bmuxd-remote && chmod 0755 /usr/local/bin/bmuxd-remote`,
    ...
    "RUN ln -sf /etc/systemd/system/bmuxd-ws.service /etc/systemd/system/multi-user.target.wants/bmuxd-ws.service",
  ].join("\n");
}
...
function freestyleSystemdServiceCommand(): string {
  const service = [
    "[Unit]",
    "Description=bmuxd websocket daemon",
    "After=network.target",
    "",
    "[Service]",
    "Type=simple",
    ...
    "ExecStart=/usr/local/bin/bmuxd-remote serve --ws --listen 0.0.0.0:7777 --auth-lease-file /tmp/bmux/attach-pty-lease.json --rpc-auth-lease-file /tmp/bmux/attach-rpc-lease.json --shell /bin/bash",
    "Restart=always",
    "RestartSec=1",
    "",
    "[Install]",
  ...
  return `cat <<'EOF' >/etc/systemd/system/bmuxd-ws.service\n${service}\nEOF`;
}
...
async function remoteDaemonBuildURL(tag: string, daemonPath: string): Promise<string> {
  const explicit = process.env.BMUX_REMOTE_DAEMON_BUILD_URL?.trim();
  ...
  if (missing.length > 0) {
    throw new Error(
      `Freestyle snapshot build needs BMUX_REMOTE_DAEMON_BUILD_URL or R2 env vars; missing ${missing.join(", ")}`,
    );
  }

  const key = `bmux-build-artifacts/cloud-vm/${tag}/bmuxd-remote-linux-amd64`;
  const env = {
