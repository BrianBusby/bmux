...
dev_lock_key() {
  ...
  printf '%s-dev-%s' "$slug" "$BMUX_PORT"
}

claim_dev_lock() {
  local lock_dir
  lock_dir="${TMPDIR:-/tmp}/bmux-web-dev"
  mkdir -p "$lock_dir"
...
stop_local_services() {
  if ! owns_dev_lock; then
    echo "bmux web dev: skipped local service stop because another dev process owns BMUX_PORT=$BMUX_PORT"
    return
  fi
  bash "$ROOT_DIR/scripts/db-local.sh" down >/dev/null 2>&1 || true
  echo "bmux web dev: stopped local Postgres for BMUX_PORT=$BMUX_PORT"
}

start_cleanup_watcher() {
  if [[ "${BMUX_DEV_STOP_DB_ON_EXIT:-1}" == "0" ]]; then
    return
  fi
  ...
  (
    ...
    while kill -0 "$parent_pid" >/dev/null 2>&1; do
      if owns_dev_lock && ! bash "$ROOT_DIR/scripts/db-local.sh" ready >/dev/null 2>&1; then
        echo "bmux web dev: local Postgres unavailable; restarting for BMUX_PORT=$BMUX_PORT"
...
cleanup() {
  ...
  if [[ "$started_db" == "1" && "${BMUX_DEV_STOP_DB_ON_EXIT:-1}" != "0" ]]; then
...
redacted_database_url="postgres://${BMUX_DB_USER}:<redacted>@localhost:${BMUX_DB_PORT}/${BMUX_DB_NAME}"
cat <<EOF
bmux web dev
  BMUX_PORT=$BMUX_PORT
  BMUX_VM_API_BASE_URL=$BMUX_VM_API_BASE_URL
  DATABASE_URL=$redacted_database_url
  BMUX_WEB_SECRET_ENV_FILE=$BMUX_WEB_SECRET_ENV_FILE
  BMUX_WEB_EXTRA_SECRET_ENV_FILE=${BMUX_WEB_EXTRA_SECRET_ENV_FILE:-}
EOF

next dev --port "$BMUX_PORT" &
next_pid=$!
