...
function shouldUseAwsRdsIam(env: Env): boolean {
  const requestedDriver = envValue(env, "BMUX_DB_DRIVER");
  if (requestedDriver) {
    if (requestedDriver === "aws-rds-iam") return true;
    if (requestedDriver === "url" || requestedDriver === "postgres-url") return false;
    throw new Error("BMUX_DB_DRIVER must be url, postgres-url, or aws-rds-iam");
  }
...
export function cloudDbConfig(env: Env = process.env): CloudDbConfig {
  const poolMax = parsePositiveInteger(envValue(env, "BMUX_DB_POOL_MAX"), "BMUX_DB_POOL_MAX", 5);
  ...
  if (shouldUseAwsRdsIam(env)) {
    ...
    return {
      ...
      poolMax,
      sslRejectUnauthorized: parseBoolean(envValue(env, "BMUX_DB_SSL_REJECT_UNAUTHORIZED"), true),
      sslCaPem: envValue(env, "BMUX_DB_SSL_CA_PEM") ??
        decodeBase64Env(envValue(env, "BMUX_DB_SSL_CA_PEM_BASE64"), "BMUX_DB_SSL_CA_PEM_BASE64"),
    };
  }
