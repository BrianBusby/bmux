...
import (
	...
	"github.com/manaflow-ai/bmux/vault/internal/authflow"
	"github.com/manaflow-ai/bmux/vault/internal/authstore"
	"github.com/manaflow-ai/bmux/vault/internal/resume"
	"github.com/manaflow-ai/bmux/vault/internal/state"
	"github.com/manaflow-ai/bmux/vault/internal/syncer"
)
...
func run(args []string, stdout, stderr io.Writer) int {
	global := flag.NewFlagSet("bmux-vault", flag.ContinueOnError)
	global.SetOutput(stderr)
	apiBase := global.String("api-base", defaultAPIBase(), "bmux web API base URL")
	...
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "bmux-vault: %v\n", err)
		return 1
	}
	var warnings []string
	...
	case "help", "-h", "--help":
		...
		return 0
	default:
		_, _ = fmt.Fprintf(stderr, "bmux-vault: unknown command %q\n", cmd)
		usage(stderr)
		return 2
	}
}
...
func runSync(ctx context.Context, args []string, env agentdirs.Environ, apiBase string, jsonOutput bool, stdout, stderr io.Writer) int {
	...
	if tokens == nil && !*dryRun {
		_, _ = fmt.Fprintln(stderr, "not logged in; run bmux-vault login")
		return 1
	}
...
func defaultAPIBase() string {
	if value := strings.TrimSpace(os.Getenv("BMUX_VAULT_API_BASE")); value != "" {
		return value
	}
...
func usage(w io.Writer) {
	_, _ = fmt.Fprintln(w, `Usage: bmux-vault [--api-base URL] [--json] <command> [options]
...
Commands:
  ...
  resume     Restore a missing session from bmux Vault and print the resume command
