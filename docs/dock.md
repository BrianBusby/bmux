...
Dock is the bmux right sidebar rendered as a full panel container. It uses the **same surface and split system as the main content area** — terminals *and* browsers, tiled with the same split affordances — just docked on the right. Each Dock terminal runs in its own Ghostty-backed surface, so TUIs keep normal keyboard behavior such as arrow keys, `j` / `k`, and `Ctrl-C`. Dock browsers share the same browser stack as main-area browser panes (cookies, profile, devtools, navigation).

Dock is useful for project dashboards, git views, logs, queues, local services, test watchers, dev servers, custom TUIs, and reference web pages. Feed can be added as one optional terminal with `bmux feed tui --opentui`, but Dock is not limited to Feed.

Every bmux window has its own independent Dock. Opening a new window seeds that window's Dock fresh from your Dock config (exactly like a fresh app launch), multiple windows can show their Docks side by side, and closing a window closes its Dock terminals and browsers with it.
...
```sh
bmux new-pane --placement dock                              # split a terminal into the Dock
bmux new-pane --type browser --placement dock --url https://example.com
bmux new-surface --type browser --placement dock --url https://example.com   # add a Dock tab
```
...
bmux looks for Dock config in this order:

1. `.bmux/dock.json` in the current project or a parent directory
2. `~/.config/bmux/dock.json`

Use `.bmux/dock.json` for repo-specific controls that should be shared with teammates. Commit it to the repo when the commands are safe and portable.

Use `~/.config/bmux/dock.json` for personal defaults, machines without a repo, or controls that are specific to your local setup.

Nested project configs apply to their directory tree. If a nested project has its own `.bmux/dock.json`, use that nearest config for work inside the nested project. Do not put unrelated project controls into the global config just because a repo is absent.

If neither file exists, Dock opens empty and offers a prompt to create a starter config. bmux does not add Dock controls automatically.

Relative `cwd` values resolve from the config base. For `.bmux/dock.json`, that base is the project directory containing `.bmux`. For the global config, that base is the home directory.

## Trust

Project Dock configs can start commands. The first time bmux sees a project Dock config, it shows a trust gate before launching controls. Changing the config changes the trust fingerprint and asks again.

Global Dock config at `~/.config/bmux/dock.json` is treated as personal config and starts without a project trust gate.
...
> Bring your team's TUIs into the bmux Dock.
