...
## Goal
Ensure commands run through the bmux Unix socket/CLI do not steal user focus from the current UI workflow.
...
- [x] Add explicit `rename-tab` command (defaults to `BMUX_TAB_ID` / `BMUX_SURFACE_ID` / `BMUX_WORKSPACE_ID` when flags omitted).
