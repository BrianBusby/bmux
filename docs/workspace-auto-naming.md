...
- At the end of an agent turn, cmux summarizes up to the last 25 recent user-led exchanges into an informative subject statement of up to 12 words (in the conversation's language) and applies it to the workspace. The title is based on the recent thread, not just the last prompt. When a workspace has multiple tabs, the agent's own tab is named too.
