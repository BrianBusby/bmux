...
- At the end of an agent turn, cmux summarizes the session's recent conversation into an informative subject statement of up to 80 characters (in the conversation's language) and applies it to the workspace. When a workspace has multiple tabs, the agent's own tab is named too.
