...
Tracks the redesign of how bmux tracks whether a terminal surface has a coding
agent (Claude Code / Codex) session, for the iOS GUI ("coding agent") mode.

## Goal

The iOS GUI must reliably know, for any bmux surface: is there an agent session,
...
2. **Deterministic binding, no heuristics.** Surface to session binding is
   established by construction at terminal/agent start, keyed by a bmux-minted
...
`BMUX_SURFACE_ID` / `BMUX_WORKSPACE_ID` / `BMUX_PANEL_ID` / `BMUX_TAB_ID` as
PROTECTED env keys at spawn (`TerminalSurface+StartupEnvironment.swift:45-62`,
`applyManagedBmuxContextEnvironment`). Remote shells inject the same via
placeholder substitution (`RemoteInteractiveShellBootstrapBuilder.swift:191-196`).
So a hand-typed `claude`/`codex` in any bmux terminal already inherits the
surface token.

Hook surface resolution is multi-signal and bmux-owned
(`CLI/bmux.swift:22901-22935`, `resolveCallerTerminalBindingByTTY:24336`,
...
2. inherited env `BMUX_SURFACE_ID` / `BMUX_WORKSPACE_ID`,
...
surface bmux owns. This is the deterministic binding the redesign blesses.

Entrypoint coverage (each ends with a hook whose surface resolves by the above):
- `bmux claude-teams` / hand-typed `claude`: COVERED. Hooks injected via
  `--settings` by `bmux-claude-wrapper`; env inherited; tty/pid backstop.
- `bmux new-surface --type agent-session` (UI/CLI): COVERED. App injects
  `BMUX_SURFACE_ID` into the launch env (`AppDelegate.swift:6256`).
...
- `codex` (bare or `codex-teams`): PARTIAL. No `--settings`-style auto-inject
  wrapper; relies on `bmux hooks setup codex` having installed `~/.codex` hooks.
  Once installed, the fired hook resolves the surface identically. GAP = ensure
  setup ran; not a per-launch binding gap.
- Degraded mode: `bmux-claude-wrapper` execs claude WITHOUT hook injection when
  the socket ping fails (`bmux-claude-wrapper:414`). Acceptable: the agent still
...
bmux owns every bmux terminal's environment. When it spawns a surface's shell,
it injects:

- `BMUX_SURFACE_ID` — stable, bmux-owned, the binding key.
- `BMUX_WORKSPACE_ID`.
- the agent hook config, so `bmux hooks <agent> <event>` fires for any agent
  started in that shell and carries the surface token.

Because the token lives at the shell/env layer (which bmux always controls for
...
fallback: there is no gap for a bmux-owned surface. An agent in a terminal bmux
does NOT own (ssh/remote/external) is not a bmux surface and has nothing to show
in GUI mode, so no fallback is needed.

Open task: enumerate every way an agent can start in a bmux surface (new-agent
...
## Identity and the binding key (durability of bmux IDs)

Reality today (evidence: `Sources/Workspace.swift`, `Sources/TabManager.swift`):

- **Surface id** (`BMUX_SURFACE_ID` / `BMUX_PANEL_ID`; == panel id == ghostty
  ...
  it is stable.
- **Workspace id** (`BMUX_WORKSPACE_ID` / `BMUX_TAB_ID`): ALWAYS regenerated on
...
- **Window**: OS-managed (`NSWindow`), not a bmux model identity.
...
- The binding key is the SURFACE id, and the design REQUIRES it to be a durable
  ...
  re-id'ing it), so `BMUX_SURFACE_ID` is invariant for a terminal's whole life.
...
```
AgentSession {
  surfaceID        // binding key, stable, bmux-owned
  workspaceID
  ...
  pid              // process bmux spawned (or adopted via the env token)
...
socket, each carrying `BMUX_SURFACE_ID`, so each event maps unambiguously to one
...
- Backstops that do NOT depend on hook delivery:
  - **Owned-process exit.** bmux spawned (or adopted) the agent pid, so a child
...
- On restore, re-attach by token (the live agent process still carries
  `BMUX_SURFACE_ID`; bmux can re-adopt the pid it owns), or mark `ended` if the
...
- `~/.bmuxterm/<agent>-hook-sessions.json` as a read-side source of truth (it may
...
- **Slice B — Deterministic process-exit backstop. DONE.** Implemented as
  ...
  new iOS UI needed. Design: bmux does NOT own a
  ...
  on the agent pid bmux already knows (from hook events / the store). On `.exit`,
...
- **Status note (after A/B/D):** Slices A, B, D are implemented, build-green on
  ...
  - **F (codex hook auto-setup): deferred, needs a product decision.** The only
    ...
    `bmux hooks setup` (the existing onboarding step). Safe future options: an
...
- **Slice D — Delete the unreliable heuristics. IN PROGRESS.** Remove
  ...
  title-key helpers. REVISED SCOPE: the hook-store stays — it is bmux-written,
...
- **Slice E — Surface-id invariance (only if needed).** Close the collision
  regeneration (restore-into-live / duplicate-workspace) so `BMUX_SURFACE_ID` is
...
- **Slice F — Codex parity.** Guarantee `bmux hooks setup codex` is in effect so
  codex hooks fire and resolve the surface like claude.

## Verification

- Build/compile via tagged `-derivedDataPath /tmp/bmux-<tag>` (never untagged).
