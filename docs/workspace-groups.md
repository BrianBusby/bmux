...
Press `⌃⌘G` to create a new empty workspace group. bmux inserts a fresh anchor workspace as the group header and auto-names it `Group 1`, `Group 2`, … (rename anytime via the header context menu).
...
All group operations are scriptable via `bmux workspace-group <subcommand>`. The hyphenated form ships first; once the broader `bmux workspace <noun>` namespace lands, `bmux workspace group ...` will be the canonical form with the hyphenated form kept as an alias forever.

### Subcommands

```bash
bmux workspace-group list [--json]
bmux workspace-group create --name "manaflow" [--cwd ~/projects/manaflow] [--from <id>,<id>]
bmux workspace-group ungroup <group-id>
bmux workspace-group delete  <group-id>   # destructive: closes every member workspace
bmux workspace-group rename <group-id> --name "new name"
bmux workspace-group collapse <group-id>
bmux workspace-group expand <group-id>
bmux workspace-group pin <group-id>
bmux workspace-group unpin <group-id>
bmux workspace-group add --group <group-id> --workspace <workspace-id>
bmux workspace-group remove --workspace <workspace-id>
bmux workspace-group set-anchor --group <group-id> --workspace <workspace-id>
bmux workspace-group new-workspace <group-id> [--placement afterCurrent|top|end]
```
...
```bash
bmux workspace-group create --name manaflow
```

Spin up a new workspace inside an existing group (e.g. wired to a worktree script):

```bash
bmux workspace-group new-workspace workspace_group:1
```

List groups in the focused window:

```bash
bmux workspace-group list
```

## Configuration

Per-group configuration is keyed by the anchor's working directory in `~/.config/bmux/bmux.json` (this surface lands in a follow-up; the file location is reserved). The intent:
...
```jsonc
{
  "workspaceGroups": {
    ...
    "byCwd": {
      "/Users/you/manaflow/bmux": {
        "color": "#7A4FD8",
        "icon": "ladybug.fill",
        "newWorkspacePlacement": "top",
        "contextMenu": [
          // Entries reference actions defined elsewhere in bmux.json (in the
...
1. Explicit `--placement afterCurrent|top|end` on `bmux workspace-group new-workspace`, or `"placement"` in the v2 `workspace.group.new_workspace` params.
2. The per-cwd entry above.
3. Global default via Settings > App > Group New Workspace Placement or `workspaceGroups.newWorkspacePlacement` in `bmux.json` (defaults to `afterCurrent`).
...
Neither knob is wired up yet. The current build keeps the sidebar's existing iMessage-mode behavior unchanged regardless of groups. A follow-up will add the `sidebar.imessageMode.*` keys to `bmux.json`, the schema, and the Settings UI; this section is documented here so the eventual JSON shape is decided up front.

## Persistence

Groups (name, anchor, pin state, collapse state, color, icon) round-trip through `~/Library/Application Support/bmux/session-<bundle-id>.json` alongside workspaces. Membership lives on `Workspace.groupId`. Writes are atomic via the existing `SessionPersistenceStore` rename-into-place pattern.
