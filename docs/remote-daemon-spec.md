...
Tracking issue: https://github.com/manaflow-ai/bmux/issues/151
Primary PR: https://github.com/manaflow-ai/bmux/pull/1296
CLI relay PR: https://github.com/manaflow-ai/bmux/pull/374
...
- `DONE` `bmux ssh` creates remote-tagged workspaces and does not require `--name`.
- `DONE` scoped shell niceties are applied only for `bmux ssh` launches.
...
- `DONE` local app probes remote platform, verifies a release-pinned `bmuxd-remote` artifact by embedded manifest SHA-256, uploads it when missing, and runs `serve --stdio`.
...
- `DONE` bootstrap installs `~/.bmux/bin/bmux` wrapper (also tries `/usr/local/bin/bmux`) so `bmux` is available in PATH on the remote.
- `DONE` normal `bmux ssh` launches `bmuxd-remote serve --stdio --persistent --slot <slot>`, where the stdio process proxies to a long-lived authenticated daemon with slot credentials under `~/.bmux/daemon/<version>/<slot>/` and a short per-user socket path under `/tmp/bmuxd-remote-<uid>/`.
- `DONE` persistent daemon slots advertise `pty.session.persistent_daemon`; bmux requires that capability before preserving a saved remote PTY session ID across app relaunch.

### 3.5 CLI Relay (Running bmux Commands From Remote)
- `DONE` `bmuxd-remote` includes a table-driven CLI relay (`cli` subcommand) that maps CLI args to v1 text or v2 JSON-RPC messages.
- `DONE` busybox-style argv[0] detection: when invoked as `bmux` via wrapper/symlink, auto-dispatches to CLI relay.
...
- `DONE` relay address written to `~/.bmux/socket_addr` on the remote only after the reverse forward survives startup validation.
- `DONE` Go CLI no longer polls for relay readiness. It dials the published relay once and only refreshes `~/.bmux/socket_addr` a single time to recover from a stale shared address rewrite.
- `DONE` `bmux ssh` startup exports session-local `BMUX_SOCKET_PATH=127.0.0.1:<relay_port>` so parallel sessions pin to their own relay instead of racing on shared socket_addr.
- `DONE` session snapshots persist the relay port for persistent SSH PTYs and mint fresh relay credentials on restore, so a reattached remote shell can keep using its existing `BMUX_SOCKET_PATH=127.0.0.1:<relay_port>` after app relaunch.
- `DONE` relay startup writes `~/.bmux/relay/<relay_port>.daemon_path`; remote `bmux` wrapper uses this to select the right daemon binary per session, including mixed local bmux versions.
- `DONE` relay startup writes `~/.bmux/relay/<relay_port>.auth` with a relay ID and token; the local relay requires HMAC-SHA256 challenge-response before forwarding any command to the real local socket.
- `DONE` SSH agent forwarding is opt-in. `bmux ssh` preserves its live `SSH_AUTH_SOCK` for app-launched OpenSSH transports so `ForwardAgent yes` from ssh_config works normally, and accepts `-A` / `--forward-agent` or `-a` / `--no-forward-agent` for explicit forwarding control.
...
### 3.6 Artifact Trust
- `DONE` release and nightly workflows publish `bmuxd-remote` assets for `darwin/linux × arm64/amd64`.
- `DONE` release and nightly apps embed a compact `BMUXRemoteDaemonManifestJSON` in `Info.plist` with exact asset URLs and SHA-256 digests.
- `DONE` `bmux remote-daemon-status` exposes the current manifest entry, local cache verification state, release download command, and GitHub attestation verification command.
...
1. `DONE` `bmuxd-remote` now supports proxy stream RPC (`proxy.open`, `proxy.close`, `proxy.write`, `proxy.stream.subscribe`) with pushed `proxy.stream.data/eof/error` events.
...
|---|---|---|---|
| M-001 | `bmux ssh` workspace creation + metadata + optional `--name` | DONE | Covered by `tests_v2/test_ssh_remote_cli_metadata.py` |
...
| M-004b | CLI relay: run bmux commands from within SSH sessions | DONE | Reverse TCP forward + Go CLI relay + bootstrap wrapper |
...
| M-007 | Remote proxy stream RPC in `bmuxd-remote` | DONE | `proxy.open/close/write/proxy.stream.subscribe` plus pushed stream events implemented |
...
| M-011 | Detachable persistent `bmux ssh` PTY sessions | IN PROGRESS | Persistent remote daemon slots keep PTY sessions alive across local surface close and app relaunch; coverage includes Go daemon auth/reattach tests, Swift restore tests, CLI contract tests, and `tests_v2/test_ssh_remote_detachable_pty.py` |
...
| ID | Scenario | Status |
|---|---|---|
| C-001 | `bmux ping` from remote session | DONE |
| C-002 | `bmux list-workspaces --json` from remote | DONE |
| C-003 | `bmux new-workspace` from remote | DONE |
| C-004 | `bmux rpc system.capabilities` passthrough | DONE |
...
| ID | Scenario | Status |
|---|---|---|
| DP-001 | `bmux ssh` creates a persistent daemon slot and PTY session ID | IN PROGRESS |
| DP-002 | closing the local SSH surface detaches the attachment without killing the remote shell | IN PROGRESS |
| DP-003 | `bmux ssh-session-list` reports detached persisted sessions with bounded scrollback metadata | IN PROGRESS |
| DP-004 | `bmux ssh-session-attach` reattaches to the same remote shell PID and env | IN PROGRESS |
...
| DP-006 | `bmux ssh-session-cleanup` terminates persisted PTY sessions explicitly | DONE |
...
1. `BMUX_SSH_TEST_DOCKER_HOST` sets the SSH destination host/IP used by docker-backed SSH fixtures (default `127.0.0.1`).
2. `BMUX_SSH_TEST_DOCKER_BIND_ADDR` sets the bind address used in fixture container publish mappings (default `127.0.0.1`).
