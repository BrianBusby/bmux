...
bmux exposes a reconnectable event stream for local tools that need to observe
workspace, pane, surface, notification, browser, Feed, and agent-hook activity.

The same events are appended to `~/.bmuxterm/events.jsonl` as newline-delimited
JSON. The live stream is delivered over the existing bmux socket. Clients call
...
```bash
bmux events --cursor-file ~/.cache/bmux/events.seq --reconnect
bmux events --category window --category workspace --category pane --category surface
bmux events --category notification
bmux events --category feed --category agent --no-heartbeat
```
...
`bmux events --cursor-file`. If bmux restarts, `boot_id` changes and the server
...
`source: "workspace.lifecycle"` are emitted from the bmux model, so they cover
...
`resume.gap` is `true` when the requested cursor is older than bmux still keeps
...
| `boot_id` | UUID process-boot identifier for this in-memory event log. Changes when bmux restarts. |
| `id` | Stable event id for the current bmux process. Use it for dedupe. |
...
1. Connect to the bmux socket and authenticate if required.
...
client stops reading and falls behind that queue, bmux closes that subscription
with a `slow_consumer` error. The client should reconnect with the last `seq` it
successfully processed.

The durable event log is bounded too. bmux writes current events to
`~/.bmuxterm/events.jsonl`, rotates the previous file to
`~/.bmuxterm/events.jsonl.1`, and caps each file at 16 MiB. Disk writes are
batched behind a bounded 1,024-line queue. Under sustained disk backpressure,
bmux drops the oldest pending disk-only lines and keeps the live socket stream
...
writes its specialized long-term audit log to `~/.bmuxterm/workstream.jsonl`.

## CLI

`bmux events` prints the stream as newline-delimited JSON.
...
| Name | Trigger |
| --- | --- |
| `window.created` | A main bmux window is registered in the app model. Covers startup, session restore, shortcuts, menus, and socket commands. |
| `window.focused` | A bmux window focus request succeeded. This is an app-level focus action, not necessarily a new AppKit key transition. |
| `window.keyed` | AppKit reported a main bmux window became the key window. Use this to track the window receiving keyboard input. |
| `window.unkeyed` | AppKit reported a main bmux window resigned key status. |
| `window.closed` | A main bmux window was unregistered during close. |
...
`extension.sidebar.snapshot`, then subscribe to `bmux events --category
...
| Name | Trigger |
| --- | --- |
| `notification.requested` | Socket command asked bmux to create a notification. |
| `notification.clear_requested` | Socket command asked bmux to clear notifications. |
| `notification.dismiss_requested` | Socket command asked bmux to remove one notification or already-read notifications. |
| `notification.mark_read_requested` | Socket command asked bmux to mark notifications read. |
| `notification.open_requested` | Socket command asked bmux to open a notification by id. |
| `notification.jump_to_unread_requested` | Socket command asked bmux to jump to the latest unread notification. |
...
Agent integrations use `bmux hooks feed --source <agent>` or an equivalent
