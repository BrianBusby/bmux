...
This is the scoped todo list for making the Cloud VM backend production-ready with application logic running in the existing Vercel `manaflow/bmux` project.

## Current State

- Vercel project exists: `manaflow/bmux`.
- Vercel root directory is `web`.
- Production URL is `https://bmux.com`.
- Vercel custom `staging` environment exists for the `manaflow/bmux` project and tracks the
  `staging` git branch.
...
- A separate `manaflow/bmux-staging` Vercel project exists for staging.
...
- [x] Copy minimal DB migration variables from Vercel into both GitHub Cloud VM environments:
  ...
  - `PGUSER`
  - `PGDATABASE`
  - `BMUX_DB_SSL_REJECT_UNAUTHORIZED`
...
- [ ] Publish a new Freestyle snapshot with bmuxd-remote started with `--rpc-auth-lease-file`.
...
- [x] Staging migration role is scoped to `repo:manaflow-ai/bmux:environment:cloud-vm-staging` and the staging Aurora cluster resource id.
- [x] Production migration role is scoped to `repo:manaflow-ai/bmux:environment:cloud-vm-production` and the production Aurora cluster resource id.
- [x] Staging and production Cloud VM default provider are set to E2B.
- [x] Freestyle creates are disabled in staging and production with `BMUX_VM_FREESTYLE_ENABLED=0`.
...
- `RESEND_API_KEY`
- `BMUX_FEEDBACK_FROM_EMAIL`
- `BMUX_FEEDBACK_RATE_LIMIT_ID`
- `NEXT_PUBLIC_STACK_PROJECT_ID`
- `NEXT_PUBLIC_STACK_PUBLISHABLE_CLIENT_KEY`
- `STACK_SECRET_SERVER_KEY`
...
- [x] Add a global VM create kill switch, `BMUX_VM_CREATE_ENABLED`.
- [x] Add per-provider kill switches:
  - `BMUX_VM_E2B_ENABLED`
  - `BMUX_VM_FREESTYLE_ENABLED`
- [x] Set kill switches to enabled values in `manaflow/bmux` production and
  `manaflow/bmux-staging` production.
...
- [ ] Set `BMUX_VM_DEFAULT_PROVIDER` in Vercel development, preview, and production.
- [ ] Set `E2B_API_KEY` in Vercel preview and production.
- [ ] Set `FREESTYLE_API_KEY` in Vercel preview and production.
- [ ] Set `E2B_BMUXD_WS_TEMPLATE` in Vercel preview and production.
...
- [ ] Confirm Stack Auth callback and trusted domains include:
  ...
  - local `BMUX_PORT` development callback URLs
- [ ] Redeploy Vercel preview after env injection.
- [ ] Smoke test preview:
  - `bmux auth login`
  - `bmux vm new --provider freestyle`
  - `bmux vm new --provider e2b`
  - `bmux vm attach <id>`
...
- [ ] Keep local Stack/web runtime secrets in `~/.secrets/bmuxterm-dev.env`.
- [ ] Keep production Stack/web runtime secrets in `~/.secrets/bmuxterm.env`.
- [ ] Keep provider image-build secrets in `~/.secrets/bmux.env`.
- [ ] Add runtime VM vars to the relevant `~/.secrets/bmuxterm*.env` file:
  - `BMUX_VM_DEFAULT_PROVIDER`
...
- [x] Document the split between `~/.secrets/bmuxterm-dev.env`, `~/.secrets/bmuxterm.env`, and
  `~/.secrets/bmux.env` in `AGENTS.md`.
...
- [x] Build E2B template and Freestyle snapshot from the same bmuxd-remote commit.
- [x] Record artifact provenance:
  - bmuxd-remote git commit
  - bmuxd-remote build command
  - binary SHA256
...
- [x] Add BMUX_PORT-derived local Postgres so parallel worktrees do not collide.
...
- [ ] Connect the Vercel Marketplace Aurora resource to `manaflow/bmux` for both `staging` and production so these env names are present:
  - `AWS_ROLE_ARN`
  - `AWS_REGION`
  - `PGHOST`
  - `PGPORT`
...
- [ ] Add `BMUX_VM_CREATE_ENABLED` and provider kill-switch docs.
...
- [ ] Add a future `bmux-infra` or `backend-rollout` skill so agents follow this workflow consistently.
