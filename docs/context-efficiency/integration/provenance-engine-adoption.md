...
| Session tree CLI | read, presentation | `bmux provenance sessions tree <session-id>` | `CLI/BMUXCLI+Provenance.swift` constructs `WorkProvenanceStore` and calls `BmuxLegacyProvenanceClient.sessionTree(...)`. | `sessionTree`. | Local `sessions`, `session_relationships`, and `session_external_identities` projections through `WorkProvenanceStore.sessionTree(...)` and `externalIdentities(...)`. | Yes: `sessionTree`. | 1, Slice C. | `WorkProvenanceStore.sessionTree(rootSessionID:limit:)`, relationship tree helpers used only by that read path, the `sessionTree` method on `BmuxLegacyProvenanceClient`, and local CLI test seeding for this path once replaced by public engine seeding. |
...
- The accepted external `0.1.0` baseline already exposes APIs for the remaining durable read paths and normalized lifecycle writes. Slice C should not need engine feature expansion unless implementation inspection finds a compatibility defect.
...
Exact Slice C scope: migrate only `bmux provenance sessions tree <session-id>` to construct the external SQLite-backed engine client and call `ProvenanceEngineClient.sessionTree(ProvenanceSessionTreeRequest(...))`; preserve existing JSON/text/no-database/no-session behavior and bounds; seed tests through public engine APIs; remove the legacy local `sessionTree` method and any session-tree-only local query helpers that become unused.

Do not begin file-explanation, current-context, lifecycle-write, capture, observability, data-migration, daemon, UI, or engine-expansion work in Slice C.
