...
- Phase 2 has not moved code into an independent engine repository, created a daemon, moved storage/schema, added data migration, or added daemon/SDK packaging. Three read-only CLI paths now consume in-process `ProvenanceEngineClient` contracts instead of their direct SQLite reader paths: `bmux provenance sessions tree` uses `sessionTree(...)`, `bmux provenance explain` uses `fileExplanation(...)`, and `bmux provenance worktrees list` uses `worktrees(...)`.
