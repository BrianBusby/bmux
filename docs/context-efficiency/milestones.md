...
migration remains ADR-001 Phase 5. Legacy boundary clarification and the session-tree read migration are now accepted. Slice D file-explanation read adoption is implemented on branch `slice-d-file-explanation-sdk-migration` against Provenance Engine PR 5 readiness revision `384026e36087dda576e25343907c3e06d8a4d594`, but it is not accepted until the engine readiness PR and bmux adoption PR complete review. Slice C accepted an explicit GitHub Actions waiver because bmux PR-event runs produced zero jobs/check runs and manual workflow runs queued without assignment to `blacksmith-4vcpu-ubuntu-2404`; that infrastructure issue remains outside Slice D scope.
