...
- Phase 2 has not moved code into an independent engine repository, created a daemon, moved storage/schema, added data migration, or added daemon/SDK packaging. One read-only CLI path, `bmux provenance sessions tree`, now consumes the in-process `ProvenanceEngineClient.sessionTree(...)` contract instead of its direct SQLite reader path.
