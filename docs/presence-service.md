...
Realtime device presence for bmux: every team member sees which of the team's
devices (Macs, iPhones, and their tagged bmux app instances) are online or
...
- **Auth** (`src/auth.ts`): `Authorization: Bearer <Stack access token>`,
  ...
  team scoping via `X-Bmux-Team-Id` / `?teamId=` and a membership check,
...
(https://github.com/manaflow-ai/bmux/pull/5626); this service adds no Aurora
...
A dev/staging instance (`bmux-presence-dev`, dev Stack project) is live at
`https://bmux-presence-dev.debussy.workers.dev` on the team Cloudflare
account; see `workers/presence/README.md` for how to redeploy it and point a
dev Mac build at it. Production deploys serve `https://presence.bmux.dev`
(a `custom_domain` route in `wrangler.toml`; the bmux.dev zone lives on the
...
- **Mac sender** (`Sources/Cloud/PresenceHeartbeatClient.swift`): Debug builds
  ...
  `presenceServiceURL` / `BMUX_PRESENCE_BASE_URL`). Follows the
...
- **iOS** (`Packages/iOS/BmuxMobileShell/Sources/BmuxMobileShell/PresenceClient.swift`):
  ...
  (https://github.com/manaflow-ai/bmux/pull/5648) rows, writes pushed routes
...
```bash
set -a; source ~/.secrets/bmuxterm-dev.env; set +a
STACK_PROJECT_ID="$NEXT_PUBLIC_STACK_PROJECT_ID" \
STACK_PUBLISHABLE_CLIENT_KEY="$NEXT_PUBLIC_STACK_PUBLISHABLE_CLIENT_KEY" \
STACK_EMAIL="$BMUX_DOGFOOD_STACK_EMAIL" \
STACK_PASSWORD="$BMUX_DOGFOOD_STACK_PASSWORD" \
