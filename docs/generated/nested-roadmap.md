    ...
    - **Three-view Coding Session Experience** (`three_view_coding_session_experience`) - phase; status: active; owner: Bmux; repositories: Bmux, Provenance Engine; concept: semantic understanding; layer: consumer presentation; execution: current / Bmux; parallelism: safe
      ...
      - **Coding Session View Surfaces** (`coding_session_view_surfaces`) - milestone; status: active; owner: Bmux; repositories: Bmux, Provenance Engine; concept: semantic understanding; layer: consumer presentation; execution: current / Bmux; parallelism: safe
        ...
        - **Shared-session Chat and permanent Terminal** (`shared_session_chat`) - slice; status: active; owner: Bmux; repositories: Bmux; concept: execution telemetry; layer: consumer presentation; execution: current / Bmux; parallelism: serial; delivery: proposed; acceptance: under observation; maturity: active
          ...
          Execution notes: User authorized opt-in newly connected Codex sessions after the shared-host proof. Existing ordinary CLI sessions remain read-only in Chat. Queue delivery and expected-turn steering passed live protocol verification with the original TUI. Build 572 dogfood verified one acknowledged Chat submission and matching original Terminal output on the same thread. A temporary startup ambiguity recovered without restart; its cause and broader UI/recovery acceptance remain open. The existing dogfood build remains untouched.
          Rationale: Preserve the original PTY and reuse transcript observation for Chat. Ordinary Codex 0.154.0 attachment via app-server proxy failed because no control socket exists; submit, steer, queue, interrupt, approvals, questions and settings remain gated. See docs/product/shared-session-control-decision.md. Read-only native UI, late output, view continuity and raw-terminal interruption were exercised; A separate new shared-host TUI probe accepted a second-client prompt but failed expected-turn interruption; an old turn ID interrupted a newer turn in Codex 0.154.0. Opt-in new shared-host queue and expected-turn steering are implemented with live native transport proof and automated ownership/delivery tests; basic connected UI delivery now passed in build 572; temporary startup ambiguity and broader recovery acceptance remain open. Shared-host interrupt remains disabled. Codex turn states use explicit provider events. Older-history paging, broader recovery acceptance and shared controls remain open. This does not complete React Chat productization or three-view control acceptance.
