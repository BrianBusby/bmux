    ...
    - **Three-view Coding Session Experience** (`three_view_coding_session_experience`) - phase; status: active; owner: Bmux; repositories: Bmux, Provenance Engine; concept: semantic understanding; layer: consumer presentation; execution: current / Bmux; parallelism: safe
      ...
      - **Coding Session View Surfaces** (`coding_session_view_surfaces`) - milestone; status: active; owner: Bmux; repositories: Bmux, Provenance Engine; concept: semantic understanding; layer: consumer presentation; execution: current / Bmux; parallelism: safe
        ...
        - **Shared-session Chat and permanent Terminal** (`shared_session_chat`) - slice; status: active; owner: Bmux; repositories: Bmux; concept: execution telemetry; layer: consumer presentation; execution: current / Bmux; parallelism: serial; delivery: proposed; acceptance: under observation; maturity: active
          ...
          Execution notes: User authorized opt-in newly connected Codex sessions after the shared-host proof. Existing ordinary CLI sessions remain read-only in Chat. Queue delivery and expected-turn steering passed live protocol verification with the original TUI. Build 593 (tag stabilize-shared-session-final-v9, commit b949e6dfa) verified the connected composer transition from empty/pre-ingestion Conversation unavailable to one accepted user message and matching assistant response in Chat and the original connected Terminal; a draft survived Terminal-to-Chat and two-workspace switching. DEBUG-only, session-scoped history partial/unavailable and control-transport disconnect hooks are covered by native regressions and compiled out of Release. Build 595 (tag stabilize-shared-session-acceptance-v11, commit 596420f12) exercised a real connected target and post-disconnect delivery; the dark WebKit hook still does not change rendered colors and needs follow-up. Current-head web, native, activation, project-truth, and workflow checks are green; release-build remains in progress. A temporary startup ambiguity was not reproduced; conservative rejection remains. Application-restored connected ownership, structured interruption, approvals/questions/settings, and live VoiceOver/dark-mode acceptance remain explicit gates. The existing dogfood build remains untouched.
