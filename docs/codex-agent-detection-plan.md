...
Done: `bmux-codex-wrapper` (PATH shim, Claude-parity per-invocation `[hooks]`
...
- bmux's Codex hooks (`~/.codex/hooks.json`) are NOT installed.
- Codex has a single legacy `notify` slot, and it is taken by Computer Use:
  `notify = [".../SkyComputerUseClient", "turn-ended"]`. bmux's notify-based
  events cannot fire.
...
- The 13 "codex" entries in `~/.bmuxterm/codex-hook-sessions.json` are stale,
  not live-updating.

Why Claude works, for contrast: Claude Code has a `SessionStart` hook. bmux's
`bmux-claude-wrapper` is a PATH shim that injects bmux's hooks per-invocation
...
bmux owns the terminal environment, so it can mediate any agent the user launches
in its terminal, per-invocation, without touching global config. Detection must
depend only on what bmux controls — the wrapper, the injected env, the pid it
...
A `bmux-codex-wrapper`, mirroring `bmux-claude-wrapper`:

1. bmux prepends a shim dir to `PATH` when it spawns its terminal shells (the
...
2. Before exec'ing the real codex, the wrapper emits the launch signal itself:
   `bmux hooks codex session-start` carrying `BMUX_SURFACE_ID`, the cwd, and its
   child pid. THIS is the reliable detection signal, and it needs nothing from
   Codex — the wrapper, which bmux controls, is the source. (More robust than the
...
4. Outside bmux (no `BMUX_SURFACE_ID` / socket), the wrapper no-ops and execs the
   real codex, so it is invisible to non-bmux usage.
...
- Chain: read the user's existing `notify` from `config.toml`; bmux's injected
...
Super reliable:
- Detection of any Codex that bmux launches OR the user types in a bmux terminal.
  Anchored to the wrapper-emitted session-start (surface + pid bmux owns).
...
- Deliberate bypass is not caught: absolute path (`/usr/local/bin/codex`), an
  ...
  a host bmux does not own. Not surfacing a truly-unmediated agent is correct
  behavior, not a bug.
...
1. Does a `bmux-claude-wrapper`-style PATH shim already exist, and where is the
...
4. Does bmux's existing Codex launch path (`codex-teams` /
...
2. Add `bmux-codex-wrapper` to the same shim dir + PATH injection Claude uses.
3. Wrapper emits `bmux hooks codex session-start` (surface / pid / cwd) before
   exec, and no-ops cleanly outside bmux.
...
8. Build + dogfood: type `codex` in a bmux terminal, confirm it appears in the
