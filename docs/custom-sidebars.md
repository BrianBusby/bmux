# Custom sidebars: vibe-code your own bmux sidebar

bmux lets you build your own sidebar UI by writing a small SwiftUI-style file.
It is interpreted at runtime (no Xcode, no build step, no signing), renders as
native SwiftUI in the real sidebar, hot-reloads on save, binds to live bmux
state, and can run bmux commands on tap. This guide is the authoring contract
...
- Make it interactive by default. Rows that represent something you can open
  should be tappable and run the matching `bmux(...)` action (e.g. selecting a
...
- Iterate by saving the file and opening it as a pane with
  `bmux sidebar open <name>`; it hot-reloads there while you edit. Verify it
...
Write a named file (the name becomes the menu label; use short kebab-case):
    ...
    ~/.config/bmux/sidebars/<name>.swift     # interpreted Swift (preferred)
    ~/.config/bmux/sidebars/<name>.json      # declarative JSON (simpler, static)
...
for the left sidebar, or run `bmux sidebar open <name>` to show it in a pane;
...
as real SwiftUI inside the bmux window, so hover styling, focus, keyboard,
...
crash or hang caused by the interpreted file cannot take down bmux, but
input is limited to forwarded clicks (no hover, focus, or keyboard).

Set it in **Settings → Custom Sidebars**, or in `~/.config/bmux/bmux.json`:
...
Install one from a bmux checkout:

    mkdir -p ~/.config/bmux/sidebars
    cp Examples/CustomSidebars/status-board.swift ~/.config/bmux/sidebars/status-board.swift
    cp Examples/CustomSidebars/finder.swift ~/.config/bmux/sidebars/finder.swift

Then validate and open it as a Bonsplit pane:

    bmux sidebar validate status-board
    bmux sidebar open status-board

`bmux sidebar select <name>` still previews a custom sidebar in the left
sidebar picker. Use `bmux sidebar open <name>` when you want the sidebar as a
...
- `workspaces` — array, one per workspace. Always present: `id`, `title`,
  ...
  the same shape with every pull request bmux knows for the workspace),
...
## Actions (run real bmux commands on tap)

A button or `.onTapGesture` body calls `bmux("<method>", param: value)`. On tap
it runs that bmux command through the same dispatcher as the `bmux` CLI:

    Button(action: { bmux("workspace.select", workspace_id: w.id) }) { ... }
    ...onTapGesture { bmux("surface.focus", surface_id: t.id) }

Use real method and parameter names. Common ones: `workspace.select`
(`workspace_id`), `surface.focus` (`surface_id`), `workspace.reorder`
(`workspace_id` + `index`). Run `bmux docs api` to discover the full command
surface.
...
runs the `move` command, which both reorders and persists (bmux remembers
workspace order):
    ...
    HSplitView {
        VStack(alignment: .leading) {
            for i in 0..<workspaces.count {
                Button(action: { bmux("workspace.select", workspace_id: workspaces[i].id) }) {
            ...
            for i in 0..<workspaces.count {
                if workspaces[i].selected {
                    for j in 0..<workspaces[i].tabs.count {
                        Button(action: { bmux("surface.focus", surface_id: workspaces[i].tabs[j].id) }) {
...
(`TextField`, `Toggle`, `Slider`, `Picker`) — buttons/taps that run `bmux(...)`
...
is live; data bmux doesn't track (custom domain collections) won't appear.
