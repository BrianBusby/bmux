...
Goal: ship the iOS path from current bmux main with Swift-owned app, session, transport, and storage code. Ghostty remains the terminal engine through GhosttyKit.
...
- `Packages/Shared/BMUXMobileCore` owns the cross-platform Swift protocol layer: attach tickets, routes, frame codec, and Ghostty snapshot models.
- The iOS shell depends on `BMUXMobileCore` and a `CmxByteTransport`, not on Tailscale, Iroh, WebSocket, or daemon details.
...
1. Land `BMUXMobileCore` with route selection, frame codec, snapshot schema, and tests.
