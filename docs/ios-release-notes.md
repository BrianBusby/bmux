...
How bmux tells iOS TestFlight testers what changed in a build, so an install or
...
- **Trigger**: on launch, compare the running `CFBundleShortVersionString`
  ...
  `UserDefaults` (e.g. `bmux.whatsNew.lastSeenVersion`). If the running version is
