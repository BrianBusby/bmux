...
const ALPN: &[u8] = b"dev.bmux.mobile.terminal/0";
...
pub struct BmuxIrohEndpoint {
    endpoint: Endpoint,
}

pub struct BmuxIrohConnection {
...
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_endpoint_bind(
    enable_relay: bool,
    accept_connections: bool,
    err_buf: *mut c_char,
    err_cap: usize,
) -> *mut BmuxIrohEndpoint {
    ...
    match result {
        Ok(endpoint) => Box::into_raw(Box::new(BmuxIrohEndpoint { endpoint })),
        Err(error) => {
...
/// Free with `bmux_iroh_string_free`.
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_endpoint_id(endpoint: *const BmuxIrohEndpoint) -> *mut c_char {
...
/// (id, direct addrs, relay URL). Free with `bmux_iroh_string_free`.
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_endpoint_route_json(endpoint: *const BmuxIrohEndpoint) -> *mut c_char {
    let Some(endpoint) = (unsafe { endpoint.as_ref() }) else {
        return ptr::null_mut();
    };
...
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_endpoint_online(
    endpoint: *mut BmuxIrohEndpoint,
    timeout_ms: u64,
    err_buf: *mut c_char,
    err_cap: usize,
) -> c_int {
...
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_endpoint_accept(
    endpoint: *mut BmuxIrohEndpoint,
    timeout_ms: u64,
    err_buf: *mut c_char,
    err_cap: usize,
) -> *mut BmuxIrohConnection {
    let Some(endpoint) = (unsafe { endpoint.as_ref() }) else {
        set_error(err_buf, err_cap, "null endpoint");
        return ptr::null_mut();
    };
...
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_endpoint_connect(
    endpoint: *mut BmuxIrohEndpoint,
...
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_connection_recv(
    connection: *mut BmuxIrohConnection,
    buf: *mut u8,
    cap: usize,
    err_buf: *mut c_char,
    err_cap: usize,
...
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_connection_send(
    connection: *mut BmuxIrohConnection,
    bytes: *const u8,
    len: usize,
    err_buf: *mut c_char,
...
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_connection_close(connection: *mut BmuxIrohConnection) {
...
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_endpoint_close(endpoint: *mut BmuxIrohEndpoint) {
    if endpoint.is_null() {
        return;
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn bmux_iroh_string_free(string: *mut c_char) {
    if string.is_null() {
        return;
    }
...
fn finish_connection(
    ...
    match result {
        Ok((connection, send, recv)) => Box::into_raw(Box::new(BmuxIrohConnection {
            connection,
            send: Mutex::new(send),
            recv: Mutex::new(recv),
        })),
