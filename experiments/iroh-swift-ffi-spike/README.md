...
bmux iOS-to-Mac transport: prove a Swift process can bind an iroh endpoint,
...
plain bridging header (`include/bmux_iroh_ffi.h`).
...
- staticlib `libbmux_iroh_ffi.a`: 15 MB per target (`opt-level = "s"`, fat
