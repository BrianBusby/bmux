// Swift harness for the bmux iroh FFI spike.
//
...
func takeString(_ raw: UnsafeMutablePointer<CChar>?) -> String {
    ...
    defer { bmux_iroh_string_free(raw) }
...
func bindEndpoint(acceptConnections: Bool) -> OpaquePointer {
    ...
    guard let endpoint = bmux_iroh_endpoint_bind(true, acceptConnections, &err, errCap) else {
...
func waitOnline(_ endpoint: OpaquePointer) {
    var err = [CChar](repeating: 0, count: errCap)
    guard bmux_iroh_endpoint_online(endpoint, 30_000, &err, errCap) == 0 else {
...
func runListen() {
    ...
    waitOnline(endpoint)
    print("endpoint-id: \(takeString(bmux_iroh_endpoint_id(endpoint)))")
    print("route: \(takeString(bmux_iroh_endpoint_route_json(endpoint)))")
    print("listening; dial me with: swift-harness dial <endpoint-id>")

    var err = [CChar](repeating: 0, count: errCap)
    guard let connection = bmux_iroh_endpoint_accept(endpoint, 180_000, &err, errCap) else {
    ...
    var total = 0
    while true {
        let read = bmux_iroh_connection_recv(connection, &buf, buf.count, &err, errCap)
        if read < 0 { fail("recv: \(lastError(err))") }
        if read == 0 { break }
        total += read
        guard bmux_iroh_connection_send(connection, buf, read, &err, errCap) == 0 else {
            fail("send: \(lastError(err))")
        }
    }
    print("echoed \(total) byte(s); peer closed stream")
    bmux_iroh_connection_close(connection)
    bmux_iroh_endpoint_close(endpoint)
}
...
func runDial(endpointID: String, payload: String) {
    ...
    guard let connection = endpointID.withCString({ id in
        bmux_iroh_endpoint_connect(endpoint, id, nil, nil, 0, 60_000, &err, errCap)
    }) else {
    ...
    guard bmux_iroh_connection_send(connection, message, message.count, &err, errCap) == 0 else {
        fail("send: \(lastError(err))")
    }
