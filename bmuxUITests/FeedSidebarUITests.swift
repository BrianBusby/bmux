...
final class FeedSidebarUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        socketPath = "/tmp/bmux-debug-\(UUID().uuidString).sock"
        diagnosticsPath = "/tmp/bmux-feed-sidebar-\(UUID().uuidString).json"
        feedResultPath = "/tmp/bmux-feed-sidebar-result-\(UUID().uuidString).json"
        feedTUIReadyPath = "/tmp/bmux-feed-sidebar-tui-ready-\(UUID().uuidString).json"
        dockRenderReadyPath = "/tmp/bmux-dock-render-ready-\(UUID().uuidString).json"
        dockConfigPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-feed-sidebar-dock-\(UUID().uuidString).json")
            .path
    ...
    func testFeedReceivesAndResolvesPermissionRequest() throws {
        ...
        app.launchEnvironment["BMUX_ALLOW_SOCKET_OVERRIDE"] = "1"
        ...
        app.launchEnvironment["BMUX_UI_TEST_PORTAL_STATS"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_FEED_SIDEBAR_RESULT_PATH"] = feedResultPath
        app.launchEnvironment["BMUX_UI_TEST_FEED_SIDEBAR_REQUEST_ID"] = requestId
        app.launchEnvironment["BMUX_UI_TEST_FEED_TUI_READY_PATH"] = feedTUIReadyPath
        app.launchEnvironment["BMUX_UI_TEST_DOCK_CONFIG_PATH"] = dockConfigPath
    ...
    private func waitForFeedTUIReady(timeout: TimeInterval) -> Bool {
        return pollUntil(timeout: timeout, interval: 0.5) {
            ...
            guard payload["stage"] == "opentui-ready",
                  ...
                  let cwd = payload["cwd"],
                  !cwd.contains(".bmuxterm/feed-tui-opentui") else {
                return false
            }
            return true
        }
    ...
    private func launchAndEnsureUsable(_ app: XCUIApplication) {
        ...
        XCTAssertTrue(
            ...
            "bmux failed to launch for Feed UI test. state=\(app.state.rawValue)"
        )
    }
    ...
    private func writeFeedDockConfig(bunPath: String?) throws {
        var env = ["BMUX_FEED_TUI_READY_PATH": feedTUIReadyPath]
        if let bunPath, !bunPath.isEmpty {
            env["BMUX_FEED_TUI_BUN_PATH"] = bunPath
        }
    ...
    private func writeDockRenderConfig() throws {
        ...
        let config: [String: Any] = [
            "controls": [[
                ...
                "command": command,
                "height": 320,
                "env": ["BMUX_DOCK_RENDER_READY_PATH": dockRenderReadyPath]
            ]]
        ]
        let data = try JSONSerialization.data(withJSONObject: config, options: [.prettyPrinted, .sortedKeys])
        try data.write(to: URL(fileURLWithPath: dockConfigPath), options: .atomic)
