...
/// it to a local fixture page under `bmuxUITests/BrowserFixtures/`.
///
...
class BrowserFixtureSocketTestCase: XCTestCase {
    ...
    override func setUp() {
        ...
        diagnosticsPath = "/tmp/bmux-ui-test-browser-fixtures-\(UUID().uuidString).json"
...
final class BrowserFixtureInteractionUITests: BrowserFixtureSocketTestCase {
    ...
    func testEventTrustAndOrder() throws {
        ...
        XCTAssertTrue(
            try evalBool(
                "window.__bmuxLog.some(e => e.type === 'click' && e.target === '#btn')",
                surfaceID: sid
            ),
        ...
        XCTAssertTrue(
            try evalBool(
                "window.__bmuxLog.some(e => e.type === 'input' && e.target === '#field' && e.value === 'abc')",
                surfaceID: sid
            ),
        ...
        XCTAssertFalse(
            try evalBool("window.__bmuxLog.some(e => e.isTrusted === true)", surfaceID: sid),
        ...
        XCTAssertFalse(
            try evalBool("window.__bmuxLog.some(e => e.type === 'keydown')", surfaceID: sid),
    ...
    func testIframeNested() throws {
        ...
        XCTAssertTrue(
            try evalBool(
                "window.__bmuxLog.some(e => e.type === 'click' && e.target === '#mid-btn')",
                surfaceID: sid
            ),
    ...
    func testCustomDropdowns() throws {
        ...
        XCTAssertTrue(
            try evalBool(
                "window.__bmuxLog.some(e => e.type === 'change' && e.target === '#native' && e.value === 'c')",
                surfaceID: sid
            ),
