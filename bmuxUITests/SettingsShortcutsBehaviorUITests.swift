...
/// `shortcuts.bindings` in bmux.json (the JSON config store), and the
...
/// - `SettingsKeyboardShortcutsOpenSettingsFileButton` — open bmux.json.
...
final class SettingsShortcutsBehaviorUITests: SettingsUITestCase {
    ...
    ///   - off: "Turn this on to show or hide all bmux windows from any app."
    ///   - on:  "Press the shortcut from any app to show or hide all bmux windows."
    ...
    func testEnableToggleSwapsSubtitleCopy() {
        ...
        let offSubtitle = "Turn this on to show or hide all bmux windows from any app."
        let onSubtitle = "Press the shortcut from any app to show or hide all bmux windows."
    ...
    /// starting state, so the test leaves bmux.json clean.
...
//   chord shows/hides all bmux windows from any app. That registration and
...
//   ("Open bmux.json") — Calls SettingsHostActions.openConfigInExternalEditor(),
//   which runs NSWorkspace.shared.open(configFileURL) to hand the bmux.json
...
//   the bmux UI surface to assert against. Not e2e-testable from XCUITest;
...
//   A SwiftUI Link to https://bmux.com/docs/keyboard-shortcuts#shortcut-chords.
