...
/// (`Packages/macOS/BmuxSettingsUI/.../Sections/BrowserSection.swift`).
///
...
/// `BMUX_UI_TEST_BROWSER_IMPORT_FIXTURE` env seam, so we can launch with a
...
final class SettingsBrowserBehaviorUITests: SettingsUITestCase {
    ...
    private func makeLaunchedAppWithImportFixture() -> XCUIApplication {
        ...
        app.launchEnvironment["BMUX_UI_TEST_BROWSER_IMPORT_FIXTURE"] =
    ...
    // TIER 2 (needs runtime seam): Enable bmux Browser (BrowserEnabledToggle) —
    ...
    //   never writes (only the legacy `bmuxApp.browserEnabledBinding`
    //   mirrors both keys). So toggling Enable bmux Browser in the new
    //   Settings UI produces no XCUITest-observable change (the command
    //   palette still shows "Disable bmux Browser"). Verifying this
    ...
    //   bmux without a warning prompt. The effect (warning prompt shown or
    ...
    //   only via dedicated BMUX_UI_TEST_BROWSER_IMPORT_HINT_* launch seams,
