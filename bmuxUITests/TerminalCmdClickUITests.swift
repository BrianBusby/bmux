...
final class TerminalCmdClickUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        fixtureDirectoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ui-test-terminal-cmd-click-\(UUID().uuidString)", isDirectory: true)
    ...
    func testCmdClickPngOpensInBmuxFilePreviewWhenEnabled() throws {
        let app = launchApp(
            displayMode: .raw,
            fileName: "Command Click Fixture.png",
            captureOpenPaths: true,
            captureHoverDiagnostics: false,
            openSupportedFilesInBmux: true
        )
        defer { app.terminate() }
        ...
        XCTAssertEqual(
            ...
            "1",
            "Expected supported-file routing to create a bmux file preview. result=\(result)"
        )
        XCTAssertTrue(
            waitForOpenCountToStay(0, timeout: 0.75),
            "Expected bmux file preview routing to avoid the external opener. opened=\(loadCapturedOpenPaths())"
        )
    }
    ...
    func testCmdClickMarketingSkillMarkdownPathWithTrailingPeriodOpensMarkdownViewer() throws {
        ...
        XCTAssertEqual(
            ...
            "1",
            "Expected Markdown paths to open in the bmux Markdown viewer. result=\(result)"
        )
        XCTAssertEqual(
            result["lastCommandOpenedInFilePreview"] as? String,
            "0",
        ...
        XCTAssertTrue(
            waitForOpenCountToStay(0, timeout: 0.75),
            "Expected bmux Markdown routing to avoid the external opener. opened=\(loadCapturedOpenPaths())"
        )
    }
    ...
    func testCmdClickQuotedAbsoluteMarkdownPathWithTrailingPeriodOpensMarkdownViewer() throws {
        ...
        XCTAssertEqual(
            result["lastCommandOpenedInMarkdownViewer"] as? String,
            "1",
            "Expected absolute Markdown paths to open in the bmux Markdown viewer. result=\(result)"
        )
        XCTAssertEqual(
            result["lastCommandOpenedInFilePreview"] as? String,
            "0",
    ...
    private func launchApp(
        ...
        openSupportedFilesInBmux: Bool = false,
        openMarkdownInBmuxViewer: Bool? = nil,
        quicklookOverride: String? = nil,
        viewportOffsetDelta: Int? = nil
    ) -> XCUIApplication {
        let app = XCUIApplication()
        app.launchEnvironment["BMUX_TAG"] = "ui-test-terminal-cmd-click"
        app.launchEnvironment["BMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_SETUP"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_PATH"] = setupDataPath
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_COMMAND_PATH"] = commandPath
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_FIXTURE_DIR"] = fixtureDirectoryURL.path
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_FILE_NAME"] = fileName
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_MODE"] = displayMode.rawValue
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_LINE_FORMAT"] = lineFormat.rawValue
        app.launchEnvironment["BMUX_UI_TEST_OPEN_SUPPORTED_FILES_IN_BMUX"] = openSupportedFilesInBmux ? "1" : "0"
        if !displaySuffix.isEmpty {
            app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_SUFFIX"] = displaySuffix
        }
        if displayAsAbsolutePath {
            app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_AS_ABSOLUTE_PATH"] = "1"
        }
        if let openMarkdownInBmuxViewer {
            app.launchEnvironment["BMUX_UI_TEST_OPEN_MARKDOWN_IN_BMUX_VIEWER"] = openMarkdownInBmuxViewer ? "1" : "0"
        }
        if !linePrefix.isEmpty {
            app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_LINE_PREFIX"] = linePrefix
        }
        if !extraFileNames.isEmpty,
           let data = try? JSONSerialization.data(withJSONObject: extraFileNames, options: [.sortedKeys]),
           let json = String(data: data, encoding: .utf8) {
            app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_EXTRA_FILE_NAMES_JSON"] = json
        }
        if captureOpenPaths {
            app.launchEnvironment["BMUX_UI_TEST_CAPTURE_OPEN_PATH"] = openCapturePath
        }
        if lineFormat == .osc8 {
            app.launchEnvironment["BMUX_UI_TEST_CAPTURE_OPEN_URL_PATH"] = openURLCapturePath
        }
        if captureHoverDiagnostics {
            app.launchEnvironment["BMUX_UI_TEST_CMD_HOVER_DIAGNOSTICS_PATH"] = hoverDiagnosticsPath
        }
        if let quicklookOverride {
            app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_QUICKLOOK_OVERRIDE"] = quicklookOverride
        }
        if let viewportOffsetDelta {
            app.launchEnvironment["BMUX_UI_TEST_TERMINAL_CMD_CLICK_VIEWPORT_OFFSET_DELTA"] = String(viewportOffsetDelta)
        }
