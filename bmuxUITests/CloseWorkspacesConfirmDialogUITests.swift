...
final class CloseWorkspacesConfirmDialogUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        socketPath = "/tmp/bmux-ui-test-close-workspaces-\(UUID().uuidString).sock"
        diagnosticsPath = "/tmp/bmux-ui-test-close-workspaces-\(UUID().uuidString).json"
    ...
    func testCmdShiftWUsesSidebarMultiSelectionSummaryDialog() {
        ...
        configureSocketLaunchEnvironment(app)
        app.launchEnvironment["BMUX_UI_TEST_FORCE_CONFIRM_CLOSE_WORKSPACE"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_SIDEBAR_SELECTED_WORKSPACE_INDICES"] = "0,1"
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
    ...
    func testCmdShiftWCloseWorkspacesPromptIsWindowModalSheet() {
        let app = XCUIApplication()
        let recorderPath = "/tmp/bmux-ui-test-close-workspaces-presentation-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: recorderPath)
        configureSocketLaunchEnvironment(app)
        app.launchEnvironment["BMUX_UI_TEST_KEYEQUIV_PATH"] = recorderPath
        app.launchEnvironment["BMUX_UI_TEST_FORCE_CONFIRM_CLOSE_WORKSPACE"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_SIDEBAR_SELECTED_WORKSPACE_INDICES"] = "0,1"
        app.launch()
        XCTAssertTrue(
        ...
        XCTAssertEqual(
            ...
            "sheet",
            "Workspace close confirmation should be attached to the bmux window so it cannot get stranded as a separate app-modal alert. recorder=\(presentation ?? loadJSON(atPath: recorderPath) ?? [:])"
        )
        XCTAssertEqual(
