...
final class SidebarHelpMenuUITests: XCTestCase {
    ...
    func testHelpMenuCheckForUpdatesTriggersSidebarUpdatePill() {
        ...
        app.launchEnvironment["BMUX_UI_TEST_FEED_MODE"] = "available"
...
final class FeedbackComposerShortcutUITests: XCTestCase {
    ...
    func testCmdOptionFWorksFromSettingsWindow() {
        let app = XCUIApplication()
        app.launchEnvironment["BMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_SHOW_SETTINGS"] = "1"
        app.launch()
        app.activate()
...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    private let debugDefaultsDomain = "com.bmuxterm.app.debug"
    private let hiddenSurfaceToken = "bmux-command-palette-hidden-surface"
    private let visibleSurfaceToken = "bmux-command-palette-visible-surface"
    private let noMatchWorkspaceQuery = "bmux-command-palette-no-match"

    override func setUp() {
        super.setUp()
        continueAfterFailure = false
        socketPath = "/tmp/bmux-ui-test-command-palette-\(UUID().uuidString).sock"
    ...
    func testMinimalModeToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/bmux-ui-test-settings-focus-\(UUID().uuidString).json"
        ...
        let diagnostics = waitForDiagnostics(
            ...
            data["keyWindowIdentifier"] == "bmux.settings" && data["settingsWindowIsKey"] == "1"
        }
    ...
    func testMenuBarOnlyToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/bmux-ui-test-menu-bar-only-focus-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: diagnosticsPath)
