...
/// (no `BMUX_UI_TEST_BONSPLIT_TAB_DRAG_SETUP` / `_SHOW_RIGHT_SIDEBAR`
...
final class SettingsSidebarBetaBehaviorUITests: SettingsUITestCase {
    ...
    // SettingsPickerRow at bmuxApp.swift / SidebarSection.swift).
    ...
    //   with no workspaces (no BMUX_UI_TEST_BONSPLIT setup env), so no
    ...
    //   which requires BMUX_UI_TEST_BONSPLIT_SHOW_RIGHT_SIDEBAR=1 plus the
