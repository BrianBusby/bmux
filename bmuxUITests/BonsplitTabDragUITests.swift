...
final class BonsplitTabDragUITests: XCTestCase {
    ...
    func testRightSidebarCloseButtonLivesInsideSidebarChrome() {
        ...
        XCTAssertTrue(
            ...
            "Expected bmux to be foreground before toggling the right sidebar shortcut. state=\(app.state.rawValue)"
        )
    ...
    func testMinimalModeSidebarControlsRemainVisibleWhileNotificationsPopoverIsShown() {
        ...
        XCTAssertTrue(
            ensureAppForegroundForKeyboardInteraction(app, timeout: 6.0),
            "Expected bmux to be foreground before opening notifications shortcut. state=\(app.state.rawValue)"
        )
    ...
    func testManyPaneTabBarActionsUseTrailingWhitespaceBeforeClipping() {
        ...
        let firstActionButton = app.descendants(matching: .any)
            .matching(identifier: "paneTabBarControl.custom.bmux-ui-test-action-1")
            .firstMatch
        let lastActionButton = app.descendants(matching: .any)
            .matching(identifier: "paneTabBarControl.custom.bmux-ui-test-action-\(actionButtonCount)")
            .firstMatch
    ...
    private func launchConfiguredApp(
        ...
        let dataPath = "/tmp/bmux-ui-test-bonsplit-tab-drag-\(UUID().uuidString).json"
        ...
        if startWithHiddenSidebar {
            app.launchEnvironment["BMUX_UI_TEST_BONSPLIT_START_WITH_HIDDEN_SIDEBAR"] = "1"
        }
        if let windowSize {
            app.launchEnvironment["BMUX_UI_TEST_BONSPLIT_WINDOW_SIZE"] = windowSize
        }
        if let actionButtonCount {
            app.launchEnvironment["BMUX_UI_TEST_BONSPLIT_ACTION_BUTTON_COUNT"] = String(actionButtonCount)
        }
        if showRightSidebar {
            app.launchEnvironment["BMUX_UI_TEST_BONSPLIT_SHOW_RIGHT_SIDEBAR"] = "1"
        }
