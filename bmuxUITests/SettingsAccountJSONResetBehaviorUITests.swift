...
/// "Account + bmux.json + Reset" group: the Account identity row, the
/// bmux.json config-file/docs card, and the Reset All Settings button.
///
/// These three sidebar sections (`Account`, `bmux.json`, `Reset`) are
...
/// control whose effect is observable purely inside the bmux app surface
...
/// - TIER 2: bmux.json "Open" button (opens the config file in an
///   external editor — leaves the bmux process, no in-app element).
/// - TIER 3: Account Sign In / Sign Out (browser auth round trip),
///   bmux.json "Open Docs" link (external browser).
///
/// The Account identity row and the bmux.json card *layout* are still
...
final class SettingsAccountJSONResetBehaviorUITests: SettingsUITestCase {
    ...
    /// (TIER 3 below), which XCUITest cannot observe inside bmux.
    ...
    /// The bmux.json card renders its two action rows: the User config file
    /// row with an "Open" button (`SettingsJSONOpenButton`) and the
    /// Documentation row with an "Open Docs" link (`SettingsJSONDocsLink`).
    /// Presence guard only — both controls leave the bmux process when
    ...
    func testSettingsJSONCardRendersOpenAndDocsControls() {
        ...
        let openButton = requireElement(
            ...
            timeout: 5.0,
            description: "bmux.json Open button"
        )
        ...
        let docsLink = requireElement(
            ...
            timeout: 4.0,
            description: "bmux.json Open Docs link"
        )
    ...
    // TIER 2 (needs runtime seam): bmux.json "Open" button (SettingsJSONOpenButton)
    ...
    //   user's default editor for ~/.config/bmux/bmux.json in a *separate*
    //   application. bmux opens no in-app window and changes no in-app element,
    //   so XCUITest (scoped to the bmux process) cannot observe the effect.
    ...
    //   bmux sign-in page in the system browser for an OAuth-style round trip;
    ...
    // TIER 3 (not e2e): bmux.json "Open Docs" link (SettingsJSONDocsLink)
    //   — a SwiftUI `Link` to https://bmux.com/docs/configuration#bmux-json that
    //   opens in the system browser. Cross-app navigation with no bmux-side
