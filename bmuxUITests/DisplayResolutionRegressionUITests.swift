...
final class DisplayResolutionRegressionUITests: XCTestCase {
    private let defaultDisplayHarnessManifestPath = "/tmp/bmux-ui-test-display-harness.json"
    ...
    override func setUp() {
        ...
        let tempPrefix = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ui-test-display-\(token)")
            .path
        launchTag = "ui-tests-display-resolution-\(token.prefix(8))"
        diagnosticsPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ui-test-display-churn-\(token).json")
            .path
    ...
    private let prelaunchManifestPath = "/tmp/bmux-ui-test-prelaunch.json"
    ...
    private func loadPrebuiltHelperBinaryPath(_ env: [String: String]) -> String? {
        guard let helperBinaryPath = env["BMUX_UI_TEST_DISPLAY_HELPER_BINARY_PATH"],
    ...
    private func loadExternalHarnessFromEnvironment(_ env: [String: String]) -> ExternalDisplayHarness? {
        guard let readyPath = env["BMUX_UI_TEST_DISPLAY_READY_PATH"], !readyPath.isEmpty,
              let displayIDPath = env["BMUX_UI_TEST_DISPLAY_ID_PATH"], !displayIDPath.isEmpty,
              let donePath = env["BMUX_UI_TEST_DISPLAY_DONE_PATH"], !donePath.isEmpty else {
            return nil
        }
        ...
        return ExternalDisplayHarness(
            ...
            startPath: env["BMUX_UI_TEST_DISPLAY_START_PATH"],
            donePath: donePath,
            logPath: env["BMUX_UI_TEST_DISPLAY_LOG_PATH"],
            helperBinaryPath: nil
        )
    }

    private func loadExternalHarnessFromManifest(_ env: [String: String]) -> ExternalDisplayHarness? {
        let manifestPath = env["BMUX_UI_TEST_DISPLAY_HARNESS_MANIFEST_PATH"] ?? defaultDisplayHarnessManifestPath
    ...
    private func launchAppProcess(targetDisplayID: String) throws {
        ...
        let logPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ui-test-app-\(launchTag).log").path
    ...
    private func resolveAppBinaryPath() throws -> String {
        ...
        //   .../Build/Products/Debug/bmuxUITests-Runner.app/Contents/PlugIns/bmuxUITests.xctest
        // The app binary is at:
        //   .../Build/Products/Debug/bmux DEV.app/Contents/MacOS/bmux DEV
        let testBundle = Bundle(for: Self.self)
        let productsDir = testBundle.bundleURL
            .deletingLastPathComponent()  // -> .../Contents/PlugIns
            .deletingLastPathComponent()  // -> .../Contents
            .deletingLastPathComponent()  // -> .../bmuxUITests-Runner.app
            .deletingLastPathComponent()  // -> .../Debug
        let binaryPath = productsDir
            .appendingPathComponent("bmux DEV.app")
            .appendingPathComponent("Contents/MacOS/bmux DEV")
            .path
    ...
    private func launchEnvironment(targetDisplayID: String) -> [String: String] {
        [
            "BMUX_UI_TEST_MODE": "1",
            "BMUX_UI_TEST_DIAGNOSTICS_PATH": diagnosticsPath,
            "BMUX_UI_TEST_DISPLAY_RENDER_STATS": "1",
            "BMUX_UI_TEST_TARGET_DISPLAY_ID": targetDisplayID,
            "BMUX_TAG": launchTag,
        ]
    }
