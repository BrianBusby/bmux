...
final class RightSidebarChromeHeightUITests: XCTestCase {
    func testSecondaryBarMatchesModeBarAndPaneTabs() {
        ...
        let dataPath = "/tmp/bmux-ui-test-right-sidebar-chrome-\(UUID().uuidString).json"
    ...
    func testMatchedTerminalBackgroundKeepsSidebarBackgroundsAndBordersUnified() {
        let app = XCUIApplication()
        let dataPath = "/tmp/bmux-ui-test-sidebar-appearance-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: dataPath)
...
final class TerminalViewportUITests: XCTestCase {
    func testTerminalSurfaceUsesAvailableViewportAndTracksWindowResize() {
        let dataPath = "/tmp/bmux-ui-test-terminal-viewport-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: dataPath)

        let app = XCUIApplication()
        app.launchEnvironment["BMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_VIEWPORT_SETUP"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_VIEWPORT_PATH"] = dataPath
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_VIEWPORT_WINDOW_SIZE"] = "900x620"
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_VIEWPORT_RESIZE_WINDOW_SIZE"] = "1180x780"
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_VIEWPORT_HIDE_SIDEBAR"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_TERMINAL_VIEWPORT_HIDE_RIGHT_SIDEBAR"] = "1"
        app.launchArguments += ["-workspacePresentationMode", "minimal"]
