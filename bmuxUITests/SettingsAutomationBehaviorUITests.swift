...
///
/// The Automation section (`AutomationSection` in `BmuxSettingsUI`) exposes:
...
/// env injection, which `rg` binary Find resolves, the `BMUX_PORT` range a new
...
///     not exist in the default `bmux processes only` mode, and choosing a
...
final class SettingsAutomationBehaviorUITests: SettingsUITestCase {
    ...
    private enum L {
        ...
        static let modeBmuxOnly = "bmux processes only"
    ...
    /// Default mode is `bmux processes only`, so the Socket Password row must
    /// be absent. Selecting `Password mode` must reveal it (title + the
    /// "no password set" subtitle), and selecting `bmux processes only`
    ...
    func testPasswordModeRevealsAndHidesSocketPasswordRow() {
        ...
        // Default (bmuxOnly): no password row.
        XCTAssertFalse(
            passwordTitle.waitForExistence(timeout: 1.0),
            "Socket Password row should be hidden in the default bmux-only mode"
        )
        ...
        // Switch back to bmux-only → row disappears again.
        XCTAssertTrue(selectMode(window, app, L.modeBmuxOnly), "could not select bmux-only mode")
        XCTAssertTrue(
            poll(timeout: 4.0) { !passwordTitle.exists },
    ...
    //
    // TIER 2 (needs runtime seam): Socket Control Mode (bmuxOnly/automation/
    ...
    //   connection handler (consumed in AppDelegate/bmuxApp socket setup), not
    ...
    //   toggles — when enabled, bmux injects session-tracking + notification
    //   hook env into newly launched `claude`/`cursor`/`gemini` processes
    //   inside bmux terminals. The observable consequence (sidebar session
    ...
    //   these select which `claude` / `rg` executable bmux resolves
    ...
    //   BMUX_PORT / BMUX_PORT_END environment range a new terminal inherits.
    //   The only true verification is reading the env of a freshly spawned
    //   terminal (e.g. `echo $BMUX_PORT`), which requires driving the Ghostty
