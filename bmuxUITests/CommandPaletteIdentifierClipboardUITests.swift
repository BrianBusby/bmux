...
final class CommandPaletteIdentifierClipboardUITests: XCTestCase {
    ...
    func testCmdShiftPOpenBmuxJSONOpensUserConfigFile() throws {
        let app = XCUIApplication()
        let capturePath = "/tmp/bmux-ui-test-open-bmux-json-\(UUID().uuidString).txt"
        ...
        app.launchArguments += ["-AppleLanguages", "(en)", "-AppleLocale", "en_US", "-menuBarOnly", "false"]
        app.launchEnvironment["BMUX_UI_TEST_MODE"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_CAPTURE_OPEN_PATH"] = capturePath
        launchAndActivate(app)
        ...
        searchField.typeText("open bmux json")

        let predicate = NSPredicate(
            format: "identifier BEGINSWITH %@ AND value == %@",
            "CommandPaletteResultRow.",
            "palette.openBmuxSettingsFile"
        )
        let row = app.descendants(matching: .any)
            .matching(predicate)
            .firstMatch
        XCTAssertTrue(row.waitForExistence(timeout: 5.0), "Expected row for Open bmux.json")
        try? FileManager.default.removeItem(atPath: capturePath)
        row.click()
        ...
        let expectedPath = (loginHomeDirectoryPath() as NSString)
            .appendingPathComponent(".config/bmux/bmux.json")
