...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/bmux-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/bmux-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["BMUX_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["BMUX_TAG"] = launchTag
        ...
        XCTAssertTrue(
            ...
            "Expected bmux to be foreground before sending notification shortcut. state=\(app.state.rawValue)"
        )
    ...
    func testNotificationsPopoverCanCloseViaShortcutAndEscape() {
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected bmux to be foreground before opening notifications popover. state=\(app.state.rawValue)"
        )
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["BMUX_SOCKET_PATH"] = socketPath
        app.launchEnvironment["BMUX_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["BMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["BMUX_TAG"] = launchTag
        launchAllowingHeadlessBackgroundActivation(app)
        XCTAssertTrue(
            ensureAppRunningAfterLaunch(app, timeout: 12.0),
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected bmux to be foreground before opening empty notifications popover. state=\(app.state.rawValue)"
        )
        app.typeKey("i", modifierFlags: [.command])
        ...
        let marker = "bmux_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["BMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["BMUX_TAG"] = launchTag
        launchAllowingHeadlessBackgroundActivation(app)
        XCTAssertTrue(
            ensureAppRunningAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveBmuxCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled bmux CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `bmux notify` script. " +
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected bmux to be foreground before typing delayed notify command. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected bmux to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `bmux notify` command to finish without foregrounding bmux. state=\(app.state.rawValue)"
        )
        ...
        // bundled `bmux notify` command has finished while the app stayed
        ...
        XCTAssertFalse(
            ...
            "Expected bmux to remain in background after bundled `bmux notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `bmux notify` launched from the in-app shell to succeed. " +
    ...
    func testOpenNotificationCLISelectsWorkspaceSurfaceAndMarksRowRead() throws {
        ...
        let notifyResult = runBmuxCommand(
    ...
    func testNewWorkspaceCLIWindowFlagTargetsSpecificWindow() throws {
        ...
        app.launchEnvironment["BMUX_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_WINDOW_ROUTE_CLI"] = "1"
        app.launchEnvironment["BMUX_UI_TEST_WINDOW_ROUTE_CLI_TITLE"] = title
        app.launchEnvironment["BMUX_TAG"] = launchTag
        launchAllowingHeadlessBackgroundActivation(app)
        XCTAssertTrue(
            ensureAppRunningAfterLaunch(app, timeout: 12.0),
        ...
        XCTAssertEqual(
            ...
            "1",
            "Expected bundled `bmux --window` route test hook to finish. failure=\(setup["windowRouteFailure"] ?? "")"
        )
    ...
    private func waitForBmuxPing(timeout: TimeInterval) -> (stdout: String?, stderr: String?) {
        var lastStdout: String?
        var lastStderr: String?
        let didSucceed = waitForCondition(timeout: timeout) {
            let result = self.runBmuxCommand(
        ...
        let result = runBmuxCommand(socketPath: socketPath, arguments: ["ping"], responseTimeoutSeconds: 2.0)
        let stdout = result.stdout.isEmpty ? nil : result.stdout
        let stderr = result.stderr.isEmpty ? nil : result.stderr
    ...
    private func runBmuxCommand(
        socketPath: String,
        arguments: [String],
        responseTimeoutSeconds: Double = 3.0,
        cliStrategy: BmuxCLIStrategy = .any
    ) -> (terminationStatus: Int32, stdout: String, stderr: String) {
        var args = ["--socket", socketPath]
        args.append(contentsOf: arguments)
        var environment = ProcessInfo.processInfo.environment
        environment["BMUXTERM_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)

        let cliPaths = resolveBmuxCLIPaths(strategy: cliStrategy)
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled bmux CLI"
            )
        }

        var lastPermissionFailure: (terminationStatus: Int32, stdout: String, stderr: String)?
        for cliPath in cliPaths {
            let result = executeBmuxCommand(
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled bmux CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["bmux"] + args
        let fallbackResult = executeBmuxCommand(
    ...
    private enum BmuxCLIStrategy: Equatable {
        case any
        case bundledOnly
    }
    ...
    private func resolveBmuxCLIPaths(strategy: BmuxCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["BMUX_UI_TEST_CLI_PATH", "BMUXTERM_CLI"] {
        ...
        candidates.append("/tmp/bmux-\(launchTag)/Build/Products/Debug/bmux DEV.app/Contents/Resources/bin/bmux")
        candidates.append("/tmp/bmux-\(launchTag)/Build/Products/Debug/bmux.app/Contents/Resources/bin/bmux")
        if strategy == .any {
            candidates.append("/tmp/bmux-\(launchTag)/Build/Products/Debug/bmux")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        strategy: BmuxCLIStrategy,
        to candidates: inout [String]
    ) {
        candidates.append("\(productsDir)/bmux DEV.app/Contents/Resources/bin/bmux")
        candidates.append("\(productsDir)/bmux.app/Contents/Resources/bin/bmux")
        if strategy == .any {
            candidates.append("\(productsDir)/bmux")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/bmux")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "bmux" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeBmuxCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run bmux command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested BMUX_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/bmux-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/bmux-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/bmux.sock")
        }
    ...
    private func stableSocketPath() -> String {
        // Mirrors BmuxStateDirectory.url() + bmux.sock (non-TCC ~/.local/state/bmux;
        // see https://github.com/manaflow-ai/bmux/issues/5146).
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("bmux") && $0.hasSuffix(".sock") }
