...
- Ad hoc file logging or stdout/stderr logging for production diagnostics when `Logger` or the existing bmux debug log should be used.
...
- Debug-only `NSLog` or bmux event logging guarded by `#if DEBUG`.
