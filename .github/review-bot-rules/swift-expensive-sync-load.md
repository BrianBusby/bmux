...
- Read the off-main, cached accessor instead (in bmux: `SharedLiveAgentIndex.shared`), which loads on a background task and serves a cached result.
...
Background: this rule exists because a synchronous `RestorableAgentSessionIndex.load()` added to the workspace/tab close paths froze the UI 350ms-1.8s on every close (https://github.com/manaflow-ai/bmux/pull/5669).
