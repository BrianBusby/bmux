...
Flag clear new uses of legacy asynchronous patterns in bmux-owned Swift code when Swift concurrency would be the correct design.
...
- Completion-handler APIs in new internal code when the caller and callee are both under bmux control and can use `async throws`.
