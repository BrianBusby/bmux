...
The canonical case is https://github.com/manaflow-ai/bmux/pull/6452: a `#if DEBUG` `debugQueuedRequestCount()` accessor was added to the production `MobileCoreRPCSession`/`MobileCoreRPCClient` sources so a test could read the actor's private writer-queue state. The correct fix removed the production seam entirely, widened the queue state `private` -> `internal`, and observed it from `BmuxMobileRPCTests` via `@testable import BmuxMobileRPC`.
