...
- A bmux-owned package `.gitignore` ignores `Package.resolved`.
- A bmux-owned `Package.swift` dependency change resolves new or changed external pins without the matching package-local `Package.resolved` diff.
- A `bmux.xcodeproj` SwiftPM package-reference change omits the root Xcode `Package.resolved` diff.
- A review treats `bmux.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved` as sufficient proof for standalone package resolution.

## Pass

- bmux-owned package-local `Package.resolved` files are committed with SwiftPM dependency changes.
