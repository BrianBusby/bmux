...
- Changes a homepage FAQ answer (`home.faq*` in `web/messages/en.json`) so it contradicts a claim in `README.md` (for example FAQ says bmux is free while the README implies otherwise, or the FAQ describes a capability the README denies).
