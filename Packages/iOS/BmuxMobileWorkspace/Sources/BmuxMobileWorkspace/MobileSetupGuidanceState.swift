...
/// *same* bmux account as the Mac. Each gate that is not yet cleared is its own
...
public enum MobileSetupGuidanceState: Hashable, Sendable, CaseIterable {
    ...
    /// in with the same bmux account the Mac uses.
    case notSignedIn

    /// Signed in, but this device has never paired a Mac and none is reachable.
    /// The next step is to get the bmux Mac app installed and running, then pair.
    ...
    /// different bmux account than the Mac. The next step is to sign this device
