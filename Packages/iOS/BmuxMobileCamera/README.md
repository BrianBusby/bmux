...
iOS camera service for bmux mobile QR-code pairing.
...
- ``QRCodeScanStream`` — an `AsyncStream<String>` of decoded QR payloads, fed by
  ...
  injected predicate (bmux pairing only accepts `bmux-ios://` links).
...
stream.yield("bmux-ios://example")
stream.finish()
var seen: [String] = []
for await code in stream.codes { seen.append(code) }
#expect(seen == ["bmux-ios://example"])
```
