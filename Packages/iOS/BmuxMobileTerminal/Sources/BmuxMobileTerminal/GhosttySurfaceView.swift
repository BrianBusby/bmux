...
private let log = Logger(subsystem: "ai.manaflow.bmux.ios", category: "ghostty.surface")
...
public final class GhosttySurfaceView: UIView, TerminalSurfaceHosting {
    ...
    private func applyPendingFontSizeIfNeeded() -> Bool {
        ...
        outputQueue.async(surface: surface) { surfaceHandle in
    ...
    #if DEBUG
    /// Repro hook for the `BMUX_ZOOM_STRESS` harness: drive one font-zoom
    ...
    private func processOutput(
        ...
        workQueue.async(surface: surface) { [weak self] surfaceHandle in
    ...
    func enqueueScrollToBottom() {
        ...
        outputQueue.async(surface: surface) { [weak self] surfaceHandle in
            action.withCString { pointer in
                _ = ghostty_surface_binding_action(surfaceHandle.surface, pointer, UInt(action.utf8.count))
            }
            DispatchQueue.main.async {
    ...
    private func enqueueSurfaceFree(
        ...
        queue.async(surface: surface) { surfaceHandle in
