...
private let log = Logger(subsystem: "ai.manaflow.bmux.ios", category: "ghostty.runtime")
...
private func bmuxIOSRuntimeReadClipboardCallback(
...
public final class GhosttyRuntime {
    ...
    init() throws {
        ...
        runtimeConfig.read_clipboard_cb = bmuxIOSRuntimeReadClipboardCallback
    ...
    nonisolated static func iOSConfigURLs(
        ...
        #if os(iOS)
        var urls: [URL] = []
        if let overridePath = processInfo.environment["BMUX_GHOSTTY_CONFIG_PATH"], !overridePath.isEmpty {
