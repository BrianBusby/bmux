...
enum TerminalInputDebugLog {
    #if DEBUG
    private static let isEnabled = ProcessInfo.processInfo.environment["BMUX_INPUT_DEBUG"] == "1"
    private static let logger = Logger(subsystem: "ai.manaflow.bmux.ios", category: "ghostty.input")
    #endif

    /// Logs an input-trace line in DEBUG builds when `BMUX_INPUT_DEBUG=1`.
