...
public final class TerminalAccessoryConfiguration {
    ...
    public static let didChangeNotification = Notification.Name("bmux.terminal.accessoryConfigurationDidChange")
    ...
    private static let orderDefaultsKey = "bmux.terminal.toolbar.order.v3"
    private static let enabledDefaultsKey = "bmux.terminal.toolbar.enabled.v3"
    // Custom actions are schema-stable across v2 and v3, so the v2 key is reused.
    private static let customDefaultsKey = "bmux.terminal.toolbar.custom.v2"
    ...
    private static let legacyV2OrderDefaultsKey = "bmux.terminal.toolbar.order.v2"
    private static let legacyV2EnabledDefaultsKey = "bmux.terminal.toolbar.enabled.v2"
    // v1 schema (parallel [Int] arrays of built-in rawValues), read once to
    // forward-migrate an upgrading user's existing arrangement.
    private static let legacyV1OrderDefaultsKey = "bmux.terminal.accessory.displayOrder.v1"
    private static let legacyV1EnabledDefaultsKey = "bmux.terminal.accessory.enabled.v1"
