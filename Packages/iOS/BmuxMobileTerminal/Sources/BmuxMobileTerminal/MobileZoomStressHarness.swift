...
///
/// Enable with `BMUX_ZOOM_STRESS=1`; see `bmuxApp`. DEBUG-only.
...
private struct ZoomStressRepresentable: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, GhosttySurfaceViewDelegate {
        ...
        private static func promptRedrawBurst(cols: Int) -> Data {
            ...
            let cwd = "\u{1b}]7;file://mac/Users/lawrence/fun/bmuxterm-hq/worktrees/feat-ios-swift-mobile-core/ios\u{07}"
            for i in 0..<160 {
                s += "\u{1b}]133;A\u{07}"                                   // prompt start
                s += cwd                                                    // cwd (OSC 7)
                s += "\u{1b}]0;lawrence@mac: ~/fun/bmuxterm-hq (\(i))\u{07}" // title (OSC 0)
