...
struct ChatComposerDebugAutofocusBridge: UIViewRepresentable {
    ...
    private func scheduleChatComposerDebugAutofocus(from view: UIView) {
        guard let delay = chatComposerDebugTimeInterval("BMUX_UITEST_CHAT_AUTOFOCUS_DELAY") else {
            return
        }
        UIView.animate(withDuration: 0, delay: max(0, delay), options: [.allowUserInteraction]) {
        } completion: { _ in
            MainActor.assumeIsolated {
                let root = view.window ?? view.bmuxRootView()
                let input = root.bmuxFirstFocusableTextInput(preferredIdentifier: "ChatComposerField")
    ...
    private func scheduleChatComposerDebugDismissAndRefocus(for input: UIView?) {
        guard let autoDismissDelay = chatComposerDebugTimeInterval("BMUX_UITEST_CHAT_AUTO_DISMISS_DELAY") else {
            return
        }
        UIView.animate(withDuration: 0, delay: max(0, autoDismissDelay), options: [.allowUserInteraction]) {
            ...
            MainActor.assumeIsolated {
                ...
                guard let autoRefocusDelay = chatComposerDebugTimeInterval("BMUX_UITEST_CHAT_AUTO_REFOCUS_AFTER_DISMISS_DELAY") else {
                    return
                }
...
private extension UIView {
    ...
    @MainActor
    func bmuxFirstFocusableTextInput(preferredIdentifier: String) -> UIView? {
        ...
        for subview in subviews {
            if let found = subview.bmuxFirstFocusableTextInput(preferredIdentifier: preferredIdentifier),
