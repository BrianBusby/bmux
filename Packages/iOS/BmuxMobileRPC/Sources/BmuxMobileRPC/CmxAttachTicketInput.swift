...
/// Decodes a scanned or pasted `bmux-ios://` pairing/attach URL into a
/// validated ``CmxAttachTicket``.
public struct CmxAttachTicketInput {
    private init() {}

    /// Decode and validate a `bmux-ios://pair` or `bmux-ios://attach` URL.
    ///
    ...
    /// by Stack account, not ticket age). Only the ancient `bmux-ios://pair`
    ...
    public static func decode(_ rawValue: String) throws -> CmxAttachTicket {
        ...
        // Accept any channel's pairing scheme (bmux-ios for release builds,
        // bmux-ios-dev for development); cross-channel pairing still works when
        ...
        // A QR minted by a newer bmux whose grammar version this build does not
