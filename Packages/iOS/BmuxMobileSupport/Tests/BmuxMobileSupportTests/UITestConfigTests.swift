...
@Suite struct UITestConfigTests {
    ...
    @Test func explicitEnableTurnsOnMockData() {
        let env = ["BMUX_UITEST_MOCK_DATA": "1"]
        #if DEBUG
    ...
    @Test func valueReturnsTrimmedNonEmptyWhenMockEnabled() {
        let env = [
            "BMUX_UITEST_MOCK_DATA": "1",
            "BMUX_UITEST_ADD_DEVICE_NAME": "  Work Mac  ",
        ]
        #if DEBUG
        #expect(UITestConfig.value(for: "BMUX_UITEST_ADD_DEVICE_NAME", env: env) == "Work Mac")
        #else
        #expect(UITestConfig.value(for: "BMUX_UITEST_ADD_DEVICE_NAME", env: env) == nil)
        #endif
    }

    @Test func valueIsNilWhenMockDisabled() {
        let env = ["BMUX_UITEST_ADD_DEVICE_NAME": "Work Mac"]
        #expect(UITestConfig.value(for: "BMUX_UITEST_ADD_DEVICE_NAME", env: env) == nil)
    }
    ...
    @Test func valueIsNilWhenBlank() {
        let env = [
            "BMUX_UITEST_MOCK_DATA": "1",
            "BMUX_UITEST_ADD_DEVICE_HOST": "   ",
        ]
        #expect(UITestConfig.value(for: "BMUX_UITEST_ADD_DEVICE_HOST", env: env) == nil)
    }
    ...
    @Test func dogfoodAttachURLReturnedWithMockDisabled() {
        let env = [
            "BMUX_UITEST_MOCK_DATA": "0",
            "BMUX_DOGFOOD_ATTACH_URL": "bmux-ios://attach?v=1&payload=abc",
        ]
        #if DEBUG
        #expect(UITestConfig.dogfoodAttachURL(from: env) == "bmux-ios://attach?v=1&payload=abc")
        #else
    ...
    /// (`BMUX_UITEST_ATTACH_URL`) stays nil, which is exactly why the dedicated
    ...
    @Test func legacyAttachURLStaysNilWithMockDisabledButDogfoodDoesNot() {
        let env = [
            "BMUX_UITEST_MOCK_DATA": "0",
            "BMUX_UITEST_ATTACH_URL": "bmux-ios://attach?v=1&payload=legacy",
            "BMUX_DOGFOOD_ATTACH_URL": "bmux-ios://attach?v=1&payload=dogfood",
        ]
        #expect(UITestConfig.value(for: "BMUX_UITEST_ATTACH_URL", env: env) == nil)
        #if DEBUG
        #expect(UITestConfig.dogfoodAttachURL(from: env) == "bmux-ios://attach?v=1&payload=dogfood")
        #else
        #expect(UITestConfig.dogfoodAttachURL(from: env) == nil)
        #endif
    }
    ...
    @Test func dogfoodAttachURLIsTrimmed() {
        let env = ["BMUX_DOGFOOD_ATTACH_URL": "  bmux-ios://attach?v=1&payload=zzz  "]
        #if DEBUG
        #expect(UITestConfig.dogfoodAttachURL(from: env) == "bmux-ios://attach?v=1&payload=zzz")
        #else
        #expect(UITestConfig.dogfoodAttachURL(from: env) == nil)
        #endif
    }
    ...
    @Test func dogfoodAttachURLIsNilWhenBlank() {
        let env = ["BMUX_DOGFOOD_ATTACH_URL": "   "]
        #expect(UITestConfig.dogfoodAttachURL(from: env) == nil)
    }

    @Test func workspaceDetailRefreshingTerminalMenuFlagIsDebugOnly() {
        let env = ["BMUX_UITEST_WORKSPACE_DETAIL_REFRESHING_TERMINAL_MENU": "1"]
        #if DEBUG
    ...
    @Test func workspaceDetailRefreshingTerminalMenuFlagRequiresOne() {
        ...
        #expect(UITestConfig.workspaceDetailRefreshingTerminalMenuPreviewEnabled(
            from: ["BMUX_UITEST_WORKSPACE_DETAIL_REFRESHING_TERMINAL_MENU": "0"]
        ) == false)
    }

    @Test func agentChatPreviewFlagIsDebugOnly() {
        let env = ["BMUX_UITEST_AGENT_CHAT_PREVIEW": "1"]
    ...
    @Test func agentChatPreviewFlagRequiresOne() {
        ...
        #expect(UITestEnvironmentConfig(
            environment: ["BMUX_UITEST_AGENT_CHAT_PREVIEW": "0"]
        ).agentChatPreviewEnabled == false)
    }

    @Test func agentChatInlinePreviewFlagIsDebugOnly() {
        let env = ["BMUX_UITEST_AGENT_CHAT_INLINE_PREVIEW": "1"]
        let config = UITestEnvironmentConfig(environment: env)
        #if DEBUG
    ...
    @Test func agentChatInlinePreviewFlagRequiresOne() {
        ...
        #expect(UITestEnvironmentConfig(
            environment: ["BMUX_UITEST_AGENT_CHAT_INLINE_PREVIEW": "0"]
