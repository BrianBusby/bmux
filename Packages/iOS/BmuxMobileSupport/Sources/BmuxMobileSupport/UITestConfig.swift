...
public struct UITestConfig {
    ...
    /// fires under the XCUITest mock harness), this reads `BMUX_DOGFOOD_ATTACH_URL`
    /// *without* the mock gate. The dev-launch tooling
    /// (`scripts/mobile-dev-launch.sh`, `scripts/dev-setup.sh`) signs in for real
    /// against the live backend (`BMUX_UITEST_MOCK_DATA=0`) and wants the phone to
    ...
    /// - Returns: The trimmed value of `BMUX_DOGFOOD_ATTACH_URL` when present and
    ...
    public static func dogfoodAttachURL(from env: [String: String]) -> String? {
        ...
        // URL is returned with BMUX_UITEST_MOCK_DATA=0 (the real-backend
        // dev-launch path) and iOS auto-pair actually fires.
        let value = env["BMUX_DOGFOOD_ATTACH_URL"]?.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    ///
    /// When `BMUX_UITEST_TERMINAL_PREVIEW=1`, the root view renders a standalone
    ...
    public static var terminalLayoutPreviewEnabled: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["BMUX_UITEST_TERMINAL_PREVIEW"] == "1"
        #else
        return false
        #endif
    }
    ...
    ///
    /// When `BMUX_UITEST_WORKSPACE_LIST_PREVIEW=1`, the root view renders a
    ...
    public static var workspaceListLayoutPreviewEnabled: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["BMUX_UITEST_WORKSPACE_LIST_PREVIEW"] == "1"
            ...
            || ProcessInfo.processInfo.arguments.contains("BMUX_UITEST_WORKSPACE_LIST_PREVIEW=1")
        #else
        return false
        #endif
    }
    ...
    ///
    /// When `BMUX_UITEST_WORKSPACE_DETAIL_DELAYED_TERMINAL=1`, the root view renders
    ...
    public static var workspaceDetailDelayedTerminalPreviewEnabled: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["BMUX_UITEST_WORKSPACE_DETAIL_DELAYED_TERMINAL"] == "1"
        #else
        return false
        #endif
    }
    ...
    /// preview is enabled.
    ///
    /// When `BMUX_UITEST_WORKSPACE_DETAIL_CREATE_DELAYED_TERMINAL=1`, the root
    ...
    public static var workspaceDetailCreateDelayedTerminalPreviewEnabled: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["BMUX_UITEST_WORKSPACE_DETAIL_CREATE_DELAYED_TERMINAL"] == "1"
        #else
        return false
        #endif
    }
    ...
    ///
    /// When `BMUX_UITEST_STREAMING_CHAT_PREVIEW=1`, the root view renders a
    ...
    public static var streamingChatPreviewEnabled: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["BMUX_UITEST_STREAMING_CHAT_PREVIEW"] == "1"
        #else
        return false
        #endif
    }
    ...
    ///
    /// When `BMUX_UITEST_AGENT_CHAT_PREVIEW=1`, the root view renders the
    ...
    ///
    /// When `BMUX_UITEST_AGENT_CHAT_INLINE_PREVIEW=1`, the root view renders the
    ...
    /// `BMUX_UITEST_MOCK_DATA` of `0`/`1` wins; otherwise the presence of
    ...
    public static func mockDataEnabled(from env: [String: String]) -> Bool {
        #if DEBUG
        if env["BMUX_UITEST_MOCK_DATA"] == "0" {
            return false
        }
        if env["BMUX_UITEST_MOCK_DATA"] == "1" {
            return true
        }
