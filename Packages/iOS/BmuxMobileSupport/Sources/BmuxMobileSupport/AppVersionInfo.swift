...
/// (the `BMUXDevTag` / `BMUXGitSHA` Info.plist keys), e.g.
...
public struct AppVersionInfo: Sendable, Equatable {
    ...
    /// The dev `--tag` (`BMUXDevTag`); empty for release/TestFlight builds.
    public let devTag: String

    /// The short git SHA (`BMUXGitSHA`); empty for release/TestFlight builds.
    ///
    ...
    public init(infoDictionary: [String: Any]?, isDevBuild: Bool) {
        func trimmedString(_ key: String) -> String {
            ...
            // Unexpanded build-setting placeholders (e.g. "$(BMUX_GIT_SHA)" when
        ...
        self.devTag = trimmedString("BMUXDevTag")
        self.gitSHA = trimmedString("BMUXGitSHA")
