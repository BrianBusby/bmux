...
/// is set (`BMUX_UITEST_TERMINAL_PREVIEW=1`). It renders a real, blank
...
private struct TerminalLayoutPreviewSurface: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UIView {
        ...
        let fakeHeight = ProcessInfo.processInfo.environment["BMUX_UITEST_FAKE_KEYBOARD_HEIGHT"]
            .flatMap(Double.init) ?? 0
        view.debugSetKeyboardHeightForLayoutPreview(CGFloat(max(0, fakeHeight)))
        if ProcessInfo.processInfo.environment["BMUX_UITEST_SHOW_ZOOM"] == "1" {
