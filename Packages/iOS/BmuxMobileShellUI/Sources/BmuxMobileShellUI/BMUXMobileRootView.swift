...
struct BMUXMobileRootView: View {
    ...
    #if os(iOS)
    init(store: BMUXMobileShellStore, onboardingStore: MobileOnboardingStore) {
        self.store = store
        self.onboardingStore = onboardingStore
        _hasSeenOnboarding = State(initialValue: onboardingStore.hasSeenOnboarding)
    }
    #else
    init(store: BMUXMobileShellStore) {
        self.store = store
    }
    #endif
    ...
    /// authorization grounds (a different bmux account, or a token it could not
    ...
    private func connectUITestAttachURLIfNeeded() -> Bool {
        ...
        //   - BMUX_DOGFOOD_ATTACH_URL (UITestConfig.dogfoodAttachURL): NOT gated on
        ...
        //     for real (BMUX_UITEST_STACK_* with BMUX_UITEST_MOCK_DATA=0) and wants
        ...
        //   - BMUX_UITEST_ATTACH_URL (UITestConfig.attachURL): gated on mock data,
