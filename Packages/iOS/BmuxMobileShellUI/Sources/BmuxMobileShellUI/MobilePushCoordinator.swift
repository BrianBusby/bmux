...
/// ``BmuxAuthRuntime/PushRegistrationService``, and routes foreground
/// presentation + taps to the active ``BMUXMobileShellStore`` for "mirror macOS"
...
public final class MobilePushCoordinator {
    ...
    private static let enabledKey = "bmux.notifications.pushEnabled"

    /// APNs `aps.category` the web sets on every bmux terminal push (see
    /// `BMUX_APNS_CATEGORY` in `web/services/apns/payload.ts`). The matching
    ...
    public static let dismissSyncCategoryIdentifier = "bmux.terminal"

    @ObservationIgnored private weak var store: BMUXMobileShellStore?
    ...
    public func bind(store: BMUXMobileShellStore) {
        self.store = store
    ...
    ///     with `bmux.notificationId` as a fallback.
    ///   - macDeviceId: The Mac that owns the notification, from the `bmux`
