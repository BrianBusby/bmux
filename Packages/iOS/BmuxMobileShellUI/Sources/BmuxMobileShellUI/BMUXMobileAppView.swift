...
public struct BMUXMobileAppView: View {
    @State private var store: BMUXMobileShellStore
    ...
    public init(
        store: BMUXMobileShellStore = .preview(),
    ...
    public var body: some View {
        #if os(iOS)
        BMUXMobileRootView(store: store, onboardingStore: onboardingStore)
            .environment(browserStore)
        #else
        BMUXMobileRootView(store: store)
            .environment(browserStore)
        #endif
    }
}
