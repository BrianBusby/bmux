...
@testable import BmuxMobileShellModel
...
struct MobileFeedbackRouteTests {
    ...
    @Test func debugBuildIsAlwaysDev() {
        #expect(MobileBuildType.resolve(isDebugBuild: true, bundleIdentifier: "dev.bmux.app.beta") == .dev)
        #expect(MobileBuildType.resolve(isDebugBuild: true, bundleIdentifier: "dev.bmux.app") == .dev)
        #expect(MobileBuildType.resolve(isDebugBuild: true, bundleIdentifier: nil) == .dev)
    }

    @Test func releaseBetaBundleIsBeta() {
        #expect(MobileBuildType.resolve(isDebugBuild: false, bundleIdentifier: "dev.bmux.app.beta") == .beta)
    }

    @Test func releaseNonBetaBundleIsProd() {
        #expect(MobileBuildType.resolve(isDebugBuild: false, bundleIdentifier: "dev.bmux.app") == .prod)
    ...
    @Test func subjectSuffixStampsBuildTypeAndVersion() {
        let beta = MobileFeedbackStamp(
            ...
            bundleIdentifier: "dev.bmux.app.beta", osVersion: "iOS 18.5", deviceModel: "iPhone16,2"
        )
        #expect(beta.subjectSuffix == "[Beta 0.64.13 (42)]")

        let prod = MobileFeedbackStamp(
            buildType: .prod, appVersion: "1.0.0", appBuild: "",
            bundleIdentifier: "dev.bmux.app", osVersion: "", deviceModel: ""
        )
    ...
    @Test func agentBuildStampDropsEmptyFields() {
        ...
        let sparse = MobileFeedbackStamp(
            ...
            bundleIdentifier: "dev.bmux.ios", osVersion: "", deviceModel: ""
        )
    ...
    private func makeStamp(version: String, build: String) -> MobileFeedbackStamp {
        MobileFeedbackStamp(
            ...
            appVersion: version,
            appBuild: build,
            bundleIdentifier: "dev.bmux.app.beta",
            osVersion: "iOS 18.5",
