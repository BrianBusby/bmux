...
/// One tagged bmux app instance running on a registered device, as returned by
...
public struct RegistryAppInstance: Equatable, Sendable, Identifiable {
    /// The bmux build tag this instance runs (`"stable"`, a dev tag, or
...
/// registry, with its running bmux app instances.
///
...
/// `deviceId` here is the bmux-generated device UUID (the wire `deviceId`), not
...
public struct RegistryDevice: Equatable, Sendable, Identifiable {
    /// Stable, cross-platform bmux device UUID (matches `MobileHostIdentity` /
    ...
    public var lastSeenAt: Date
    /// The device's running bmux app instances (tags), newest-first.
