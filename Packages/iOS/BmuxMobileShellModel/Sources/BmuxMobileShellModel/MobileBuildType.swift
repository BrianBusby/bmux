...
/// identifier (the beta TestFlight bundle is `dev.bmux.app.beta`). Both `beta`
...
public enum MobileBuildType: String, Equatable, Sendable {
    ...
    case dev
    /// A Release build distributed for beta dogfooding (bundle id `dev.bmux.app.beta`).
    case beta
    ...
    /// TestFlight bundle is `dev.bmux.app.beta`; anything else is treated as
    /// ``prod``.
    ///
    /// - Parameters:
    ...
    public static func resolve(isDebugBuild: Bool, bundleIdentifier: String?) -> MobileBuildType {
        ...
        if bundleIdentifier == "dev.bmux.app.beta" {
            return .beta
        }
        return .prod
    }
