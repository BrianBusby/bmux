...
///
/// Construct one at the composition root and hand it to views (bmux injects it
/// through the `\.tailscaleStatusMonitor` SwiftUI environment key in
/// `BmuxMobileShellUI`). Inject a custom provider to drive tests.
@MainActor
@Observable
public final class TailscaleStatusMonitor {
    ...
    public init(
        ...
        monitor.start(queue: DispatchQueue(label: "dev.bmux.tailscale-status", qos: .utility))
    }
