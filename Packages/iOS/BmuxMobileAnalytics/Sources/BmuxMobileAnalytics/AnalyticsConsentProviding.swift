...
///
/// The analytics package must not depend on the settings domain (`BmuxSettings`),
...
/// `BmuxSettings.catalog.app.sendAnonymousTelemetry`; tests provide a fixed
...
///
/// The iOS app cannot import the macOS-only `BmuxSettings` package, so this reads
/// the same backing key that `BmuxSettings.catalog.app.sendAnonymousTelemetry`
