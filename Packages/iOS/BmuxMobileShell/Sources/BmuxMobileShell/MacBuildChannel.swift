...
/// `BMUX_TAG`, so any non-`"default"` tag means a DEV build and the tag is the
...
public struct MacBuildChannel: Sendable {
    ...
    public func label(bundleID: String?, tag: String?) -> String? {
        ...
        // build appends a further `.slug` (e.g. `com.bmuxterm.app.nightly.my-tag`,
        // `com.bmuxterm.app.rc`), so match the component, not the suffix. Mirrors
        ...
        // A non-`com.bmuxterm.app` bundle that is clearly a dev build (e.g. the iOS
        // dev bundle `dev.bmux.*`).
        if bundle.hasPrefix("dev.bmux") { return "DEV" }
        return nil
    }
}
