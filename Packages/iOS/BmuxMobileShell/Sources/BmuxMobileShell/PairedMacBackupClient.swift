...
private let pairedMacBackupLog = Logger(subsystem: "com.bmuxterm.app", category: "PairedMacBackup")
...
/// `X-Bmux-Team-Id`, with tokens supplied through ``PresenceTokenSource``.
public actor PairedMacBackupClient: PairedMacBackingUp {
    ...
    private func makeRequest(
        ...
        if let scope = await clientScope() {
            request.setValue(scope, forHTTPHeaderField: "X-Bmux-Client-Scope")
        }
        if let body {
