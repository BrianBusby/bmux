...
/// One running bmux app instance on a device, as reported by the presence
/// service (`workers/presence`). Identities match the durable registry:
/// `deviceId` is the bmux device UUID (`devices.device_uuid`) and `tag` the
/// app-instance tag (`device_app_instances.tag`).
public struct PresenceInstance: Codable, Equatable, Sendable {
    /// bmux device UUID, matching the registry's `devices.device_uuid`.
