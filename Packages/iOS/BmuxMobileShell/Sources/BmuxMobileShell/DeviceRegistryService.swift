...
private let deviceRegistryLog = Logger(subsystem: "com.bmuxterm.app", category: "DeviceRegistry")
...
/// optional `X-Bmux-Team-Id` so the server scopes to the chosen team (defaults to
...
public actor DeviceRegistryService: DeviceRegistryRefreshing {
    ...
    /// - Parameters:
    ///   - apiBaseURL: The bmux web API base URL (no trailing slash).
    ...
    private static let deviceIDKey = "bmux.deviceRegistry.iosDeviceID"

    /// This iOS device's stable bmux identity for the device registry.
    ///
    /// A bmux-GENERATED persisted UUID (NOT `identifierForVendor`, which resets
    /// when the last bmux app is removed, and NOT a hardware fingerprint).
    ...
    private func makeRequest(method: String, path: String, body: [String: Any]?) async -> URLRequest? {
        ...
        if let teamID = await teamIDProvider(), !teamID.isEmpty {
            request.setValue(teamID, forHTTPHeaderField: "X-Bmux-Team-Id")
        }
        if let body {
