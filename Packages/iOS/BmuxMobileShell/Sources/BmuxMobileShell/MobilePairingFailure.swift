...
internal import BmuxMobileRPC
internal import BmuxMobileSupport
internal import BmuxMobileTransport
import Foundation
...
public enum MobilePairingFailureCategory: Equatable, Sendable {
    ...
    /// The address was reachable but nothing accepted the connection: bmux is not
    ...
    /// The Mac is signed in to a different bmux account than this device.
    ...
    /// Mac's channel (release Macs emit `bmux-ios`, dev Macs `bmux-ios-dev`,
    ...
    /// (https://github.com/manaflow-ai/bmux/issues/7145). Same-channel
    ...
    case ticketExpired
    /// The scanned/typed input was not a pairing QR bmux recognizes (malformed,
    /// not a bmux code, or an unreadable URL).
    case invalidCode
    /// The QR is a bmux pairing code, but it uses a grammar version newer than
    /// this build understands (the Mac is on a newer bmux). The fix is updating
    ...
    public var message: String {
        ...
        case let .listenerNotRunning(host, port):
            ...
            return L10n.string(
                ...
                defaultValue: "Your Mac is reachable, but bmux isn't running there (or mobile pairing is off). Open bmux on the Mac, then try again."
            )
        case .localNetworkBlocked:
            return L10n.string(
                "mobile.pairing.localNetworkPermission",
                defaultValue: "iOS blocked the connection. Allow bmux to use the Local Network in iOS Settings, then try again."
            )
        ...
        case .accountMismatch:
            return L10n.string(
                ...
                defaultValue: "This Mac is signed in to a different bmux account. Sign out and sign back in with the account that owns this Mac."
            )
        ...
        case let .authEnvironmentMismatch(macChannelIsRelease):
            if macChannelIsRelease {
                return L10n.string(
                    ...
                    defaultValue: "This dev build is signed in to bmux's development auth environment, so its account can never match a Mac running the release app — even with the same email."
                )
            }
            return L10n.string(
                "mobile.pairing.authEnvironmentMismatch.devMac",
                defaultValue: "This iPhone uses bmux's production sign-in, but this Mac runs a dev build on the development auth environment, so their accounts can never match — even with the same email."
            )
        case .ticketExpired:
            return L10n.string(
        ...
        case .invalidCode:
            return L10n.string(
                ...
                defaultValue: "This isn't a bmux pairing QR. Scan the code shown in the Pair iPhone window on your Mac."
            )
        case .unrecognizedVersion:
            return L10n.string(
                "mobile.pairing.unrecognizedVersion",
                defaultValue: "This QR needs a newer version of bmux. Update the app and try again."
            )
        case .loopbackRejected:
            return L10n.string(
    ...
    public var guidance: String? {
        ...
        case .hostUnreachable, .dnsFailed, .handshakeTimedOut:
            return L10n.string(
                ...
                defaultValue: "Check that this phone and your Mac are on the same Wi-Fi or both running Tailscale, that the Mac is awake, and that bmux is open on it."
            )
        case .listenerNotRunning, .connectionDropped:
            return L10n.string(
                "mobile.pairing.guidance.openMacApp",
                defaultValue: "Open bmux on your Mac, then scan the QR or link from it again."
            )
        case .localNetworkBlocked:
            return L10n.string(
                "mobile.pairing.guidance.localNetwork",
                defaultValue: "Settings > bmux > Local Network, then try again."
            )
        case .accountMismatch, .emailMismatch, .authFailed:
            return L10n.string(
                "mobile.pairing.guidance.sameAccount",
                defaultValue: "Both devices must be signed in to the same bmux account."
            )
        case let .authEnvironmentMismatch(macChannelIsRelease):
            if macChannelIsRelease {
                return L10n.string(
            ...
            return L10n.string(
                ...
                defaultValue: "Pair with a Mac running the release bmux app, or use a development-channel iPhone build for dev Macs."
            )
        ...
        case .unrecognizedVersion:
            return L10n.string(
                ...
                defaultValue: "Update bmux from the App Store (or TestFlight), then scan again."
            )
