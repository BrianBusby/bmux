...
/// authoritative `bmux.notificationId` payload key (with the request
...
public struct SystemDeliveredNotificationClearer: DeliveredNotificationClearing {
    ...
    /// `bmux.notificationId` payload key when present (authoritative — the Mac
    ...
    static func macNotificationID(for request: UNNotificationRequest) -> String {
        if let bmux = request.content.userInfo["bmux"] as? [String: Any],
