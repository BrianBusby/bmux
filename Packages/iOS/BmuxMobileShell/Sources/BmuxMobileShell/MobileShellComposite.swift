public import BMUXMobileCore
public import BmuxAgentChat
internal import BmuxMobileDiagnostics
public import BmuxMobilePairedMac
public import BmuxMobileRPC
public import BmuxMobileShellModel
internal import BmuxMobileSupport
public import BmuxMobileTransport
public import Foundation
import Observation
internal import OSLog

private let mobileShellLog = Logger(
    subsystem: Bundle.main.bundleIdentifier ?? "dev.bmux.ios",
    category: "mobile-shell"
)

/// Transitional alias for the decomposed shell facade.
///
/// The iOS views and push coordinator still bind to `BMUXMobileShellStore`;
...
public typealias BMUXMobileShellStore = MobileShellComposite
...
public final class MobileShellComposite: MobileTerminalOutputSinking {
    ...
    private static let hasKnownPairedMacDefaultsKey = "bmux.mobile.hasKnownPairedMac"
    ...
    public static func preview(runtime: (any MobileSyncRuntime)? = nil) -> BMUXMobileShellStore {
        BMUXMobileShellStore(
            runtime: runtime,
    ...
    /// The team's registered devices and their bmux app instances (tags), for the
    ...
    /// The bmux device id of the Mac the live connection currently targets, or
    ...
    private func connectPairingURLResult(
        ...
        do {
            ...
            if case MobileSyncPairingPayloadError.loopbackRouteRejected = error {
                ...
                // A real bmux QR whose grammar version this build predates: the
    ...
    static let foregroundAnonymousKey = "__bmux_foreground__"
    ...
    private func versionWarning(for ticket: CmxAttachTicket) -> String? {
        ...
        let format = L10n.string(
            ...
            defaultValue: "This iPhone is running bmux %@, but the Mac is running bmux %@. Pairing across different compatibility levels can break terminal input, workspace sync, or notifications. Continue only if you trust this Mac and accept that some features may fail."
        )
        return String(
            format: format,
    ...
    private func registerTerminalOutput(
        ...
        #if DEBUG
        mobileShellLog.info("BMUX_REPLAY register sink surface=\(surfaceID, privacy: .public) connected=\(self.connectionState == .connected, privacy: .public) hasClient=\(self.remoteClient != nil, privacy: .public) workspaceCount=\(self.workspaces.count, privacy: .public)")
        #endif
    ...
    func requestTerminalReplay(
        ...
        guard let client = remoteClient else {
            ...
            #if DEBUG
            mobileShellLog.error("BMUX_REPLAY skip surface=\(surfaceID, privacy: .public) reason=no_remote_client")
            #endif
            return
        }
        guard let workspaceID = workspaceID(forTerminalID: surfaceID) else {
            ...
            #if DEBUG
            mobileShellLog.error("BMUX_REPLAY skip surface=\(surfaceID, privacy: .public) reason=workspace_not_found")
            #endif
            return
        }
        let remoteWorkspaceID = remoteWorkspaceID(for: workspaceID)
        if let replayBarrierTokenForRequest {
            guard terminalReplayBarrierTokensInFlightBySurfaceID[surfaceID] != replayBarrierTokenForRequest else {
                #if DEBUG
                mobileShellLog.info("BMUX_REPLAY skip surface=\(surfaceID, privacy: .public) reason=barrier_in_flight")
                #endif
                return
            }
        } else {
            guard !terminalReplaySurfaceIDsInFlight.contains(surfaceID) else {
                #if DEBUG
                mobileShellLog.info("BMUX_REPLAY skip surface=\(surfaceID, privacy: .public) reason=in_flight")
                #endif
                return
            }
        }
        ...
        let replayTask = Task { @MainActor [weak self] in
            ...
            case .success(let data):
                guard self.terminalReplayRequestIDsInFlightBySurfaceID[surfaceID] == replayRequestID else {
                    MobileDebugLog.anchormux("BMUX_REPLAY stale_request surface=\(surfaceID)")
                    return
                }
                ...
                if let replayBarrierTokenForRequest {
                    guard self.terminalReplayBarrierTokensBySurfaceID[surfaceID] == replayBarrierTokenForRequest else {
                        MobileDebugLog.anchormux("BMUX_REPLAY barrier_stale surface=\(surfaceID)")
                        return
                    }
                }
                #if DEBUG
                ...
                if let replaySeq {
                    ...
                    if deliveredSeq > replaySeq
                        ...
                        MobileDebugLog.anchormux("BMUX_REPLAY stale surface=\(surfaceID) delivered=\(deliveredSeq) replay=\(replaySeq)")
                ...
                if let renderGrid {
                    deliverBytes = nil
                    MobileDebugLog.anchormux("BMUX_REPLAY render_grid surface=\(surfaceID) spans=\(renderGrid.rowSpans.count) seq=\(renderGrid.stateSeq)")
                } else if let snapshotBytes, !snapshotBytes.isEmpty {
                    deliverBytes = Self.terminalSnapshotReplacementBytes(snapshotBytes)
                    MobileDebugLog.anchormux("BMUX_REPLAY snapshot surface=\(surfaceID) bytes=\(snapshotBytes.count) seq=\(replaySeq ?? 0)")
                } else {
                    deliverBytes = bytes
                    MobileDebugLog.anchormux("BMUX_REPLAY raw_tail surface=\(surfaceID) bytes=\(bytes?.count ?? -1) seq=\(replaySeq ?? 0)")
                }
                if let renderGrid {
            ...
            case .failure(let error):
                guard self.terminalReplayRequestIDsInFlightBySurfaceID[surfaceID] == replayRequestID else {
                    MobileDebugLog.anchormux("BMUX_REPLAY stale_request_failed surface=\(surfaceID)")
                    return
                }
                mobileShellLog.error("BMUX_REPLAY failed surface=\(surfaceID, privacy: .public) error=\(String(describing: error), privacy: .private)")
                guard self.remoteClient === client else {
                    self.clearTerminalReplayInFlightIfCurrent(
                        surfaceID: surfaceID,
                        requestID: replayRequestID
    ...
    private func handleTerminalRenderGridEvent(_ event: MobileEventEnvelope) {
        ...
        #if DEBUG
        mobileShellLog.info("BMUX_REPLAY live render_grid surface=\(renderGrid.surfaceID, privacy: .public) full=\(renderGrid.full, privacy: .public) spans=\(renderGrid.rowSpans.count, privacy: .public) cleared=\(renderGrid.clearedRows.count, privacy: .public) seq=\(renderGrid.stateSeq, privacy: .public) hasSink=true")
        #endif
    ...
    private func handleTerminalBytesEvent(_ event: MobileEventEnvelope) {
        ...
        mobileShellLog.info("BMUX_REPLAY live bytes surface=\(surfaceID, privacy: .public) byteCount=\(bytes.count, privacy: .public) seq=\(debugSeq, privacy: .public) hasSink=\(self.hasTerminalOutputSink(surfaceID: surfaceID), privacy: .public)")
        #endif
