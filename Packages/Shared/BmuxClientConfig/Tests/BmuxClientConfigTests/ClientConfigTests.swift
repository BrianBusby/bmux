import Foundation
import Testing
@testable import BmuxClientConfig
...
@Suite(.serialized) struct ClientConfigTests {
    @Test func decodesFlagValuesAndPayloads() throws {
        ...
        #expect(config.value(.bmuxForWindows) == true)
    ...
    @Test func typedFlagsFallBackOnMissingOrMismatchedValues() {
        ...
        #expect(config.value(.bmuxForWindows) == false)
    ...
    @Test func httpLoaderPostsToClientConfigRoute() async throws {
        ...
        #expect(request?.url?.absoluteString == "https://bmux.test/api/client-config")
