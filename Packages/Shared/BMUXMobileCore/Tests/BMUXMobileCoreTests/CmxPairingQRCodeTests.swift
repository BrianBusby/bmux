...
@Suite struct CmxPairingQRCodeTests {
    ...
    @Test func roundTripsSingleRoute() throws {
        ...
        // The scheme is channel-specific: a release Mac emits bmux-ios, a dev
        // Mac emits bmux-ios-dev, so the system camera routes each channel's QR
    ...
    func decodeRejectsLoopbackHosts(host: String) throws {
        ...
        let url = "bmux-ios://attach?v=2&r=\(encodedHost):58465"
    ...
    @Test func decodeRejectsLoopbackEvenWhenARealRouteIsPresent() throws {
        ...
        let url = "bmux-ios://attach?v=2&r=100.64.0.5:58465&r=127.0.0.1:58465"
        #expect(throws: MobileSyncPairingPayloadError.loopbackRouteRejected) {
            try CmxPairingQRCode().decode(try components(url))
        }
    }
    ...
    @Test func decodeRejectsMalformedRoutes() throws {
        let malformed = [
            "bmux-ios://attach?v=2",
            "bmux-ios://attach?v=2&r=",
            "bmux-ios://attach?v=2&r=hostonly",
            "bmux-ios://attach?v=2&r=host:0",
            "bmux-ios://attach?v=2&r=host:99999",
            "bmux-ios://attach?v=2&r=host:not-a-port",
            "bmux-ios://attach?v=2&r=[::1:58465",
        ]
    ...
    @Test func decodeCapsHostileRouteCounts() throws {
        ...
        #expect(throws: MobileSyncPairingPayloadError.invalidURL) {
            try CmxPairingQRCode().decode(try components("bmux-ios://attach?v=2&\(routes)"))
        }
    }

    @Test func versionedURLDetectionDistinguishesGrammars() throws {
        #expect(CmxPairingQRCode().isPairingCodeURLString("bmux-ios://attach?v=2&r=100.64.0.5:58465"))
        #expect(!CmxPairingQRCode().isPairingCodeURLString("bmux-ios://attach?v=1&payload=abc"))
        #expect(!CmxPairingQRCode().isPairingCodeURLString("bmux-ios://pair?v=1&payload=abc"))
    ...
    @Test func decodedTicketStillPairsLongAfterMint() throws {
        ...
        let url = "bmux-ios://attach?v=2&r=100.64.0.5:58465"
        let decoded = try CmxPairingQRCode().decode(try components(url))
        #expect(decoded.expiresAt == nil)
    ...
    @Test func reportsPayloadBytesAndQRVersionBeforeAfter() throws {
        ...
        for (label, ticket) in [("1-route", oneRoute), ("2-route", twoRoutes)] {
            ...
            let base64 = compactPayload.base64EncodedString()
                ...
                .replacingOccurrences(of: "=", with: "")
            let before = "bmux-ios://attach?v=1&payload=\(base64)"
