...
/// It lives in `BMUXMobileCore` (the universally-imported, dependency-free base
/// package) so the lowest fire-site — `MobileClientIDRepository` in
/// `BmuxMobileShellModel` — can depend on the seam without any upward edge. The
/// concrete `AnalyticsEmitter` actor lives in `BmuxMobileAnalytics`, is built
