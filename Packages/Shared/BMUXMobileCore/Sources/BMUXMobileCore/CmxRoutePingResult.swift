...
public enum CmxRoutePingResult: Sendable, Equatable {
    ...
    /// listening on the port (bmux not running, or mobile pairing off).
    case refused
...
extension CmxRoutePingResult {
    ...
    /// listening on the port. Use ``isListening`` for "the bmux port answered".
    ...
    /// Whether a service actually accepted the connection on the bmux port (only
