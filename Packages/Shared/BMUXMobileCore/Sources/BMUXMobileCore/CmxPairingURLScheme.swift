...
/// The channel-specific URL scheme carried by bmux pairing/attach deep links.
///
/// All builds used to register and emit one scheme (`bmux-ios`), so scanning a
...
///
/// The iOS app's registered scheme comes from `BMUX_IOS_URL_SCHEME` in
...
public struct CmxPairingURLScheme {
    ...
    public static let release = "bmux-ios"

    /// The scheme development (DEBUG/tagged) builds register and emit.
    public static let development = "bmux-ios-dev"

    /// Every scheme any bmux build may emit; parsers accept all of them.
    ...
    /// Whether `scheme` is a pairing scheme from any bmux channel.
