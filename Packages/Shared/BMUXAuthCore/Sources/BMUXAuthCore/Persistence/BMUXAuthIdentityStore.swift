...
/// Persists the signed-in ``BMUXAuthUser`` across launches.
///
...
/// ``BMUXAuthKeyValueStore`` under a caller-chosen key (JSON-encoded), so each
...
public final class BMUXAuthIdentityStore: @unchecked Sendable {
    private let keyValueStore: BMUXAuthKeyValueStore
    ...
    public init(keyValueStore: BMUXAuthKeyValueStore, key: String) {
    ...
    public func save(_ user: BMUXAuthUser) throws {
    ...
    public func load() throws -> BMUXAuthUser? {
        guard let data = keyValueStore.data(forKey: key) else {
            return nil
        }
        return try decoder.decode(BMUXAuthUser.self, from: data)
    }
