...
/// sign-in.
public struct BMUXAuthAutoLoginCredentials: Equatable, Sendable {
    /// The account email.
    ...
    /// (`BMUX_UITEST_STACK_EMAIL` / `BMUX_UITEST_STACK_PASSWORD`), or `nil`
    ...
    public init?(
        ...
        guard let email = environment["BMUX_UITEST_STACK_EMAIL"], !email.isEmpty else {
            return nil
        }
        guard let password = environment["BMUX_UITEST_STACK_PASSWORD"], !password.isEmpty else {
            return nil
        }
