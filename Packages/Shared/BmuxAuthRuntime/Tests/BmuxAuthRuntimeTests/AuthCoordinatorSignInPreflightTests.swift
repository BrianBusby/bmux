...
@Suite struct AuthCoordinatorSignInPreflightTests {
    ...
    private func makeCoordinator(
        ...
        cachedUser: BMUXAuthUser? = nil,
        hasCachedTokens: Bool = false
    ) -> AuthCoordinator {
        let store = FakeKeyValueStore()
        let sessionCache = BMUXAuthSessionCache(keyValueStore: store, key: "has_tokens")
        let userCache = BMUXAuthIdentityStore(keyValueStore: store, key: "cached_user")
