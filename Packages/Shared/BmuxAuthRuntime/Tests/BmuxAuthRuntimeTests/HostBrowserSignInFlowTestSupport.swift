...
struct HostBrowserSignInFlowHarness {
    ...
    init(
        ...
        self.flow = HostBrowserSignInFlow(
            ...
            makeSignInURL: { URL(string: "https://example.test/handler/sign-in?bmux_auth_state=\($0)")! },
            callbackScheme: { "bmux-dev" },
    ...
    func callbackURL(state: String) -> URL {
        URL(string: "bmux-dev://auth-callback?stack_refresh=refresh-1&stack_access=access-1&bmux_auth_state=\(state)")!
    }

    func fallbackCallbackURL() -> URL {
        URL(string: "bmux-dev://auth-callback?stack_refresh=refresh-1&stack_access=access-1")!
    }
    ...
    func callbackState(_ session: FakeBrowserAuthSession) -> String {
        URLComponents(url: session.signInURL, resolvingAgainstBaseURL: false)?
            .queryItems?
            .first(where: { $0.name == "bmux_auth_state" })?
            .value ?? ""
    }
