...
@Suite struct AuthCoordinatorTests {
    private func makeCoordinator(
        ...
        let coordinator = AuthCoordinator(
            client: client,
            sessionCache: BMUXAuthSessionCache(keyValueStore: store, key: "has_tokens"),
            userCache: BMUXAuthIdentityStore(keyValueStore: store, key: "cached_user"),
            teamSelection: BMUXAuthTeamSelectionStore(keyValueStore: store, key: "selected_team"),
            anchor: FakeAnchor(),
            config: .test,
            launch: launch,
            clock: clock,
    ...
    @Test func passwordSignInAuthenticatesAndCaches() async throws {
        let user = BMUXAuthUser(id: "u1", primaryEmail: "a@b.com", displayName: "A")
    ...
    @Test func foregroundRevalidationWhileWaitingForCodeKeepsPendingNonce() async throws {
        ...
        // Reading the email backgrounds the iOS app. Returning to bmux triggers
    ...
    @Test func refreshOnlySignOutMintsAccessTokenForTeardown() async throws {
        ...
        // stored read, so it sees nil access, and the bmux API authenticates
    ...
    @Test func devAuthFortyTwoShortcutSignsIn() async throws {
        let user = BMUXAuthUser(id: "debug", primaryEmail: "l@l.com", displayName: "L")
        let client = FakeAuthClient(user: user)
    ...
    @Test func devAuthAccessTokenFallbackDoesNotWaitOnItsOwnTokenPhase() async throws {
        ...
        coordinator.debugCredentials = BMUXAuthAutoLoginCredentials(email: "l@l.com", password: "pw")
    ...
    @Test func signInRefreshesTeamsAndResolvesSelection() async throws {
        ...
        await client.setTeams([
            BMUXAuthTeam(id: "team_a", displayName: "Alpha"),
            BMUXAuthTeam(id: "team_b", displayName: "Beta"),
        ])
        let (coordinator, store) = makeCoordinator(client: client)
    ...
    @Test func staleTeamSelectionFallsBackToFirstTeam() async throws {
        let user = BMUXAuthUser(id: "u1", primaryEmail: "a@b.com", displayName: "A")
        let client = FakeAuthClient(user: user)
        await client.setTeams([BMUXAuthTeam(id: "team_a", displayName: "Alpha")])
        let (coordinator, _) = makeCoordinator(client: client)
