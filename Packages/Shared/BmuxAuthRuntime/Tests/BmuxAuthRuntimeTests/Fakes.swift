...
final class FakeKeyValueStore: BMUXAuthKeyValueStore, @unchecked Sendable {
...
actor FakeAuthClient: AuthClient {
    ...
    var user: BMUXAuthUser?
    var teams: [BMUXAuthTeam] = []
    ...
    init(access: String? = nil, refresh: String? = nil, user: BMUXAuthUser? = nil) {
        self.access = access
        self.refresh = refresh
        self.user = user
    }

    func setUser(_ user: BMUXAuthUser?) { self.user = user }
    ...
    func setTeams(_ teams: [BMUXAuthTeam]) { self.teams = teams }
    ...
    func currentUser(throwOnMissing: Bool) async throws -> BMUXAuthUser? {
        if let throwOnCurrentUser { throw throwOnCurrentUser }
        return user
    }

    func listTeams() async throws -> [BMUXAuthTeam] {
...
extension AuthConfig {
    static let test = AuthConfig(
        stack: BMUXAuthConfig(projectId: "test", publishableClientKey: "test"),
