...
/// (https://github.com/manaflow-ai/bmux/issues/7145).
@MainActor
@Suite struct AuthCoordinatorEnvironmentSwitchClearTests {
    private let staleUser = BMUXAuthUser(id: "stale-dev-user", primaryEmail: "dev@x.com", displayName: "Dev")
    ...
    private func makeCoordinatorWithStaleSession(
        ...
        try BMUXAuthIdentityStore(keyValueStore: store, key: "cached_user").save(staleUser)
    ...
    @Test func switchClearStillRunsDevAutoLoginOnTheSameLaunch() async throws {
        ...
        let freshUser = BMUXAuthUser(id: "dogfood", primaryEmail: "dog@x.com", displayName: "Dog")
        ...
        let (coordinator, _) = try makeCoordinatorWithStaleSession(
            ...
            launch: AuthLaunchOptions(
                ...
                mockDataEnabled: false,
                environment: [
                    "BMUX_UITEST_STACK_EMAIL": "dog@x.com",
                    "BMUX_UITEST_STACK_PASSWORD": "pw",
                ],
    ...
    @Test func uiTestClearKeepsSuppressingAutoLogin() async throws {
        // The pre-existing BMUX_UITEST_CLEAR_AUTH contract is untouched: it
