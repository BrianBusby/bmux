...
///
/// Consolidates the Stack project credentials (from ``BMUXAuthConfig``) with the
/// magic-link callback URL and the bmux web API base URL, so both apps build
...
public struct AuthConfig: Equatable, Sendable {
    ...
    public let stack: BMUXAuthConfig
    /// The auth callback URL the magic-link email should target.
    public let magicLinkCallbackURL: String
    /// The base URL of the bmux web API (device-token registration, push).
    public let apiBaseURL: String

    /// Creates an auth configuration from its resolved parts.
    public init(stack: BMUXAuthConfig, magicLinkCallbackURL: String, apiBaseURL: String) {
        self.stack = stack
    ...
    public init(
        environment: BMUXAuthEnvironment,
        overrides: [String: String] = [:]
    ) {
        let stack = BMUXAuthConfig(
        ...
        case .production:
            callbackURL = "https://bmux.com/auth/callback"
            defaultAPIBaseURL = "https://bmux.com"
        }
