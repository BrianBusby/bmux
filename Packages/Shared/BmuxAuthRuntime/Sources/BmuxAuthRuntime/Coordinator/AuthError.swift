...
/// The shared, localized error vocabulary for bmux authentication flows.
///
/// Lifted out of the iOS-only `BmuxMobileAuth` package so both the macOS and iOS
...
/// previously surfaced through `BMUXMobileCore.L10n`.
public enum AuthError: Error, LocalizedError, Equatable, Sendable {
    ...
    /// The OS/browser auth session closed before a valid callback reached bmux.
