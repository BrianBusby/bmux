public import BMUXAuthCore
import Foundation
public import Observation
import OSLog

private let authLog = Logger(subsystem: "ai.manaflow.bmux", category: "auth")

/// The shared, injected auth orchestrator for bmux.
///
...
///     sessionCache: BMUXAuthSessionCache(keyValueStore: defaults, key: "auth_has_tokens"),
///     userCache: BMUXAuthIdentityStore(keyValueStore: defaults, key: "auth_cached_user"),
///     teamSelection: BMUXAuthTeamSelectionStore(keyValueStore: defaults, key: "auth_selected_team"),
...
public final class AuthCoordinator {
    ...
    public internal(set) var currentUser: BMUXAuthUser?
    ...
    public private(set) var availableTeams: [BMUXAuthTeam] = []
    /// The user's selected team id. Writes persist through the injected
    /// ``BMUXAuthCore/BMUXAuthTeamSelectionStore``.
    ...
    private let userCache: BMUXAuthIdentityStore
    private let teamSelection: BMUXAuthTeamSelectionStore
    ...
    var debugCredentials: BMUXAuthAutoLoginCredentials?
    ...
    public init(
        ...
        sessionCache: BMUXAuthSessionCache,
        userCache: BMUXAuthIdentityStore,
    ...
    public func sendCode(to email: String) async throws {
        ...
        if launch.includesDevAuth,
           email.trimmingCharacters(in: .whitespacesAndNewlines) == "42" {
            let creds = BMUXAuthAutoLoginCredentials(email: "l@l.com", password: "abc123")
    ...
    public func verifyCode(_ code: String) async throws {
        ...
        let fullCode = BMUXAuthMagicLinkCode(code: code, nonce: nonce).composed
        do {
            let client = self.client
    ...
    func applySignedInUser(_ user: BMUXAuthUser) async {
    ...
    func apply(_ state: BMUXAuthState) {
        currentUser = state.currentUser
        isAuthenticated = state.isAuthenticated
        isRestoringSession = state.isRestoringSession
    }

    func loadCachedUser() -> BMUXAuthUser? {
        do {
    ...
    func saveCachedUser(_ user: BMUXAuthUser) {
        do {
    ...
    var autoLoginCredentials: BMUXAuthAutoLoginCredentials? {
    ...
    static let uiTestMockUser = BMUXAuthUser(
        id: "uitest_user",
        primaryEmail: "uitest@bmux.local",
        displayName: "UI Test"
    )
}
