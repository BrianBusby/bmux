...
public struct StackAuthClient: AuthClient {
    ...
    public func currentUser(throwOnMissing: Bool) async throws -> BMUXAuthUser? {
    ...
    public func listTeams() async throws -> [BMUXAuthTeam] {
        guard let user = try await stack.getUser(or: .returnNull) else {
            return []
        }
        let teams = try await user.listTeams()
        var summaries: [BMUXAuthTeam] = []
        summaries.reserveCapacity(teams.count)
        for team in teams {
            summaries.append(BMUXAuthTeam(id: team.id, displayName: await team.displayName))
        }
        return summaries
    }
    ...
    private static func mapped(_ user: CurrentUser) async -> BMUXAuthUser {
        let id = await user.id
        let email = await user.primaryEmail
        let name = await user.displayName
        return BMUXAuthUser(id: id, primaryEmail: email, displayName: name)
    }
}
