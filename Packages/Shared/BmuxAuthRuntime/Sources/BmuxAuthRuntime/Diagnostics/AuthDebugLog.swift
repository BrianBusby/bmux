...
///
/// Logs to the unified log (`com.bmuxterm.app` / `auth`) in all builds. macOS
/// DEBUG builds additionally append to `/tmp/bmux-auth-debug.log` (0600) so a
...
public struct AuthDebugLog: Sendable {
    ...
    private static let logger = Logger(subsystem: "com.bmuxterm.app", category: "auth")

    #if DEBUG && os(macOS)
    private static let debugLogPath = "/tmp/bmux-auth-debug.log"
    ...
    static func debugLogPaths(environment: [String: String]) -> [String] {
        ...
        if let taggedPath = environment["BMUX_DEBUG_LOG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !taggedPath.isEmpty,
    ...
    public static func redacted(_ message: String) -> String {
        ...
        let replacements: [(pattern: String, replacement: String)] = [
            (#"(?i)\b(stack_access|stack_refresh|access_token|refresh_token|id_token|token|login_code|polling_code|code|state|bmux_auth_state)=([^\s&#,)]+)"#, "$1=<redacted>"),
            (#"(?i)(stack_access|stack_refresh|access_token|refresh_token|id_token|token|login_code|polling_code|code|state|bmux_auth_state)%253[dD]([^\s&#,)]+)"#, "$1%253D<redacted>"),
            (#"(?i)(stack_access|stack_refresh|access_token|refresh_token|id_token|token|login_code|polling_code|code|state|bmux_auth_state)%3[dD]([^\s&#,)]+)"#, "$1%3D<redacted>"),
