...
/// Recognizes and parses bmux auth callback URLs.
///
/// Accepts the built-in bmux callback schemes (`bmux`, `bmux-nightly`,
/// `bmux-dev`) plus an optional runtime override (e.g. a per-tag Debug build's
...
public struct AuthCallbackRouter: Sendable {
    private static let builtInSchemes: Set<String> = ["bmux", "bmux-nightly", "bmux-dev"]
    ...
    ///   `BMUX_AUTH_CALLBACK_SCHEME` override round-trips).
    public init(extraAllowedScheme: String? = nil) {
        self.extraAllowedScheme = extraAllowedScheme?.lowercased()
    }

    /// Whether `url` is a bmux auth callback (`<scheme>://auth-callback`).
