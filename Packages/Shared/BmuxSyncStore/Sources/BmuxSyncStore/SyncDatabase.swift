...
///
/// `BmuxSyncStore` holds exactly one `SyncDatabase` as a `private let` and is an
...
final class SyncDatabase: @unchecked Sendable {
    ...
    init(path: String) throws {
        ...
        guard rc == SQLITE_OK, let handle else {
            ...
            throw BmuxSyncStoreError.openFailed(rc)
        }
        for pragma in ["PRAGMA foreign_keys = ON;", "PRAGMA journal_mode = WAL;"] {
            ...
            guard prc == SQLITE_OK else {
                ...
                throw BmuxSyncStoreError.stepFailed(prc, "")
            }
        }
        self.handle = handle
    }
    ...
    func prepare(_ sql: String) throws -> OpaquePointer? {
        ...
        guard rc == SQLITE_OK else { throw BmuxSyncStoreError.prepareFailed(rc, lastErrorMessage()) }
        return statement
    }
    ...
    func userVersion() throws -> Int32 {
        ...
        guard step == SQLITE_ROW else { throw BmuxSyncStoreError.stepFailed(step, lastErrorMessage()) }
    ...
    func exec(_ sql: String, binding parameters: [BindValue] = []) throws {
        if parameters.isEmpty {
            ...
            guard rc == SQLITE_OK else { throw BmuxSyncStoreError.stepFailed(rc, lastErrorMessage()) }
            return
        }
        var statement: OpaquePointer?
        defer { sqlite3_finalize(statement) }
        ...
        guard step == SQLITE_DONE || step == SQLITE_ROW else {
            throw BmuxSyncStoreError.stepFailed(step, lastErrorMessage())
        }
    }
