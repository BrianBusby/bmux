...
import Foundation
import Testing
@testable import BmuxSyncStore

// Sync apply/transport/codec/flag/migration suites. Shared helpers (TEAM, COLL,
// makeStore, deviceRecord, sortKey) live in BmuxSyncStoreTests.swift.
...
@Suite struct FlagTests {
    @Test func envOverrideWins() {
        #expect(MobileDeviceListLocalFirst.resolved(environment: ["BMUX_MOBILE_DEVICE_LIST_LOCAL_FIRST": "1"], defaults: UserDefaults(suiteName: "flag-1")!, isDebugBuild: false).isEnabled)
        #expect(!MobileDeviceListLocalFirst.resolved(environment: ["BMUX_MOBILE_DEVICE_LIST_LOCAL_FIRST": "0"], defaults: UserDefaults(suiteName: "flag-2")!, isDebugBuild: true).isEnabled)
    }
