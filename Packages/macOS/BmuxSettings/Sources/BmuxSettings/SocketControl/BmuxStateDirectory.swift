...
/// The per-user directory that holds bmux's control-plane runtime state: the
...
/// the bmux app (bundle id `com.bmuxterm.app`) and the standalone `bmux` CLI
/// installed at `/usr/local/bin/bmux`. On macOS Sequoia, a non-sandboxed process
...
///
/// This directory therefore resolves to `~/.local/state/bmux`, a plain dotfolder
/// macOS does **not** treat as protected app data. It is the sibling of the
/// existing `~/.local/state/bmux/crash` breadcrumb directory.
///
/// ```swift
...
/// let socket = BmuxStateDirectory.url(homeDirectory: home).appendingPathComponent("bmux.sock")
/// ```
public enum BmuxStateDirectory {
    /// The directory name segment under `~/.local/state` (and the legacy name
    /// under `~/Library/Application Support`).
    public static let directoryName = "bmux"

    /// The bmux state directory: `<home>/.local/state/bmux`.
    ///
