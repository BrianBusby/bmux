...
/// Pure resolution policy for the bmux control socket: where its path lives, which
...
public struct SocketControlSettings {
    ...
    /// Environment key that, when truthy, permits honoring `BMUX_SOCKET_PATH` overrides.
    public static let allowSocketPathOverrideKey = "BMUX_ALLOW_SOCKET_OVERRIDE"
    /// Environment key carrying the socket password (highest-priority password source).
    public static let socketPasswordEnvKey = "BMUX_SOCKET_PASSWORD"
    /// Environment key carrying the dev build's launch tag.
    public static let launchTagEnvKey = "BMUX_TAG"
    /// Base bundle identifier shared by all debug builds.
    public static let baseDebugBundleIdentifier = "com.bmuxterm.app.debug"
    private static let stableSocketFileName = "bmux.sock"
    /// Legacy stable socket path used before the Application Support location.
    public static let legacyStableDefaultSocketPath = "/tmp/bmux.sock"

    /// The stable build's default socket path (within ``BmuxStateDirectory``, falling back to `/tmp`).
    ...
    public static func shouldBlockUntaggedDebugLaunch(
        ...
        // so isRunningUnderXCTest() misses it. Check for any BMUX_UI_TEST_ env var.
    ...
    /// The control-socket path to use, honoring `BMUX_SOCKET_PATH` overrides only when safe.
    public static func socketPath(
        ...
        guard let override = environment["BMUX_SOCKET_PATH"], !override.isEmpty else {
            return fallback
        }
    ...
    public static func initialSocketPathBeforeListenerStart(
        ...
        guard !isDebugBuild,
              normalizedBundleIdentifier(bundleIdentifier) == "com.bmuxterm.app",
    ...
    private static func shouldReserveStableSocketPath(bundleIdentifier: String?, isDebugBuild: Bool) -> Bool {
        ...
        return normalizedBundleIdentifier(bundleIdentifier) != "com.bmuxterm.app"
    }
    ...
    /// The per-user stable socket path (`bmux-<uid>.sock` in ``BmuxStateDirectory``, `/tmp` fallback).
    public static func userScopedStableSocketPath(currentUserID: uid_t = getuid()) -> String {
        stableSocketDirectoryURL()?
            ...
            .path ?? "/tmp/bmux-\(currentUserID).sock"
    }
    ...
    /// Whether a `BMUX_SOCKET_PATH` override should be honored for this build.
    ...
    private static func inheritedBundleIdentifierConflicts(
        ...
        guard let inheritedBundleIdentifier = normalizedBundleIdentifier(environment["BMUX_BUNDLE_ID"]),
    ...
    public static func isDebugLikeBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        ...
        return bundleIdentifier == "com.bmuxterm.app.debug"
            || bundleIdentifier.hasPrefix("com.bmuxterm.app.debug.")
    }

    /// Whether the bundle identifier is a tagged DEV build (`com.bmuxterm.app.debug.<tag>`).
    ...
    public static func isStagingBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        guard let bundleIdentifier else { return false }
        return bundleIdentifier == "com.bmuxterm.app.staging"
            || bundleIdentifier.hasPrefix("com.bmuxterm.app.staging.")
    }

    /// The directory holding the control socket and its marker files.
    ///
    /// Resolves to ``BmuxStateDirectory`` (`~/.local/state/bmux`) rather than
    /// Application Support: the separately-signed `bmux` CLI connects to this
    ...
    /// from other apps" prompt (https://github.com/manaflow-ai/bmux/issues/5146).
    public static func stableSocketDirectoryURL(fileManager: FileManager = .default) -> URL? {
        BmuxStateDirectory.url(homeDirectory: fileManager.homeDirectoryForCurrentUser)
    }

    /// The stable control socket file URL (within ``BmuxStateDirectory``), if it can be resolved.
    ...
    /// The `BMUX_SOCKET_ENABLE` override as a tri-state (`true`/`false`/`nil` when unset/invalid).
    public static func envOverrideEnabled(
        environment: [String: String] = ProcessInfo.processInfo.environment
    ) -> Bool? {
        guard let raw = environment["BMUX_SOCKET_ENABLE"], !raw.isEmpty else {
            return nil
        }
    ...
    /// The `BMUX_SOCKET_MODE` override as a ``SocketControlMode``, or `nil` when unset/invalid.
    public static func envOverrideMode(
        environment: [String: String] = ProcessInfo.processInfo.environment
    ) -> SocketControlMode? {
        guard let raw = environment["BMUX_SOCKET_MODE"], !raw.isEmpty else {
            return nil
        }
        return parseMode(raw)
    }

    /// The effective mode after applying `BMUX_SOCKET_ENABLE`/`BMUX_SOCKET_MODE` overrides.
    /// - Parameters:
    ...
    public static func effectiveMode(
        ...
        if let overrideEnabled = envOverrideEnabled(environment: environment) {
            ...
            return userMode == .off ? .bmuxOnly : userMode
        }
