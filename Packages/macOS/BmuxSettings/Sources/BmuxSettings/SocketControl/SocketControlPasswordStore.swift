...
///
/// The password sources, highest priority first, are the `BMUX_SOCKET_PASSWORD`
...
public struct SocketControlPasswordStore: Sendable {
    ...
    public static let didChangeNotification = Notification.Name("bmux.socketControlPasswordDidChange")
    ...
    private static let legacyKeychainService = "com.bmuxterm.app.socket-control"
    ...
    /// - Parameters:
    ///   - environment: The process environment used to read `BMUX_SOCKET_PASSWORD`.
    ...
    /// The default password file URL within the bmux state directory.
    ///
    /// Resolves to `<directory>/socket-control-password`, where `directory`
    /// defaults to ``BmuxStateDirectory`` (`~/.local/state/bmux`). The file lives
    /// outside Application Support so the separately-signed `bmux` CLI can read it
    ...
    ///     ``BmuxStateDirectory`` location is resolved via `fileManager`.
    ...
    /// (`~/Library/Application Support/bmux/socket-control-password`) into the
    /// current ``BmuxStateDirectory`` location, once.
    ///
    ...
    public static func migrateLegacyApplicationSupportPasswordFileIfNeeded(
        ...
        guard let destination = defaultPasswordFileURL(fileManager: fileManager),
              let legacyDirectory = BmuxStateDirectory.legacyApplicationSupportURL(fileManager: fileManager) else {
            return
        }
        migratePasswordFile(
    ...
    private static func loadLegacyPasswordFromKeychain() -> String? {
        ...
        let query: [CFString: Any] = [
            ...
            kSecAttrService: "com.bmuxterm.app.socket-control",
