...
public enum SocketPathMarkerFiles {
    ...
    public static let stableTmpPath = "/tmp/bmux-last-socket-path"
    public static let nightlyBundleIdentifier = "com.bmuxterm.app.nightly"
    public static let stagingBundleIdentifier = "com.bmuxterm.app.staging"
    public static let defaultBaseDebugBundleIdentifier = "com.bmuxterm.app.debug"
    public static let defaultDebugSocketPath = "/tmp/bmux-debug.sock"
    public static let defaultNightlySocketPath = "/tmp/bmux-nightly.sock"
    public static let defaultStagingSocketPath = "/tmp/bmux-staging.sock"
    ...
    public static func variant(
        ...
        if bundleId == baseDebugBundleIdentifier {
            if let tag = normalized(environment["BMUX_TAG"]),
    ...
    public static func defaultSocketPath(
        ...
        case .nightly(let slug):
            if let slug {
                return "/tmp/bmux-nightly-\(slug).sock"
            }
            return nightlySocketPath
        case .staging(let slug):
            if let slug {
                return "/tmp/bmux-staging-\(slug).sock"
            }
