...
/// `UserDefaults` under the catalog's `app.openMarkdownInBmuxViewer` and
/// `app.openSupportedFilesInBmux` keys.
///
...
public struct FileRouteSettingsStore: FileRouteSettingsReading {
    ...
    public static let markdownRouteDidChange = Notification.Name("bmux.cmdClickMarkdownRouteDidChange")

    /// Posted after the supported-file route toggle changes.
    public static let supportedFileRouteDidChange = Notification.Name("bmux.cmdClickSupportedFileRouteDidChange")
    ...
    public func setSupportedFileRouteEnabled(_ enabled: Bool) {
        keys.openSupportedFilesInBmux.set(enabled, in: defaults)
