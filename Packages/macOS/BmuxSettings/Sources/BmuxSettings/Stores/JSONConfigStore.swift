...
/// Typed read/write/observe access to settings persisted in the bmux JSON config file.
///
...
///
/// Observation uses a primary ``BmuxFileWatch/FileWatcher`` on the configured
...
public actor JSONConfigStore {
    ...
    // Resolution identity the cache was loaded under. A bmux.json symlink can be
    ...
    ///     ``BmuxConfigLocation/userConfigFile`` for the standard bmux path.
    ...
    /// ``BmuxFileWatch/FileWatcher`` and fans out to every registered
    ...
    ///
    /// When `url` is a symlink — e.g. a `bmux.json` symlinked into a dotfiles
