...
/// Conventional on-disk locations for the bmux JSON config.
///
...
///
/// ```swift
/// let locations = BmuxConfigLocation()
/// let store = JSONConfigStore(fileURL: locations.userConfigFile)
/// ```
public struct BmuxConfigLocation: Sendable, Hashable {
    /// The primary bmux config file: `<home>/.config/bmux/bmux.json`.
    public let userConfigFile: URL

    /// The legacy fallback: `<home>/.config/bmux/settings.json`. The app's
    ...
    public init(home: URL = FileManager.default.homeDirectoryForCurrentUser) {
        ...
        self.userConfigFile = home.appending(path: ".config/bmux/bmux.json")
        self.legacyFallbackFile = home.appending(path: ".config/bmux/settings.json")
    }
}
