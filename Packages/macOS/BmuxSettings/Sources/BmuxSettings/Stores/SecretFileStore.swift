...
/// `bmux.json`: each secret is written to its own file with owner-only
...
/// store with `~/.config/bmux` (the same directory as `bmux.json`); tests pass
...
public actor SecretFileStore {
    ...
    public static let didChangeNotification = Notification.Name("bmux.secretFileStoreDidChange")
    ...
    ///   (created on first write if absent). The app uses `~/.config/bmux`.
