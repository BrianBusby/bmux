...
/// `bmux.json` config file and into a secure secret sink, then removes the
/// plaintext from `bmux.json`.
///
/// The socket-control password used to be storable as a plaintext string in
/// `bmux.json` (an editable, copyable, versionable file). Convergence makes the
...
/// very start of launch, before any managed-config layer reads `bmux.json`, so
...
public enum PlaintextSecretMigration {
    ...
    public enum Outcome: Equatable, Sendable {
        /// `bmux.json` does not exist or could not be read; nothing to do.
        case noConfigFile
        ...
        /// Copying the plaintext into the secret sink failed, so `bmux.json` was
        ...
        /// removed from `bmux.json`. Also covers an empty/whitespace plaintext.
        case scrubbedWithoutCopy
    }

    /// Lifts a plaintext secret at `plaintextKeyPath` out of `bmux.json` into the
    /// secret sink (only when the sink is empty), then removes the key from
    /// `bmux.json` after backing the file up.
    ///
    ...
    ///   - configURL: The `bmux.json` location.
    ...
    ///     plaintext is left in `bmux.json` (``Outcome/saveFailedLeftIntact``) so
        ...
        if let plaintext, !plaintext.isEmpty {
            ...
            if existing == nil || existing?.isEmpty == true {
                do {
                    ...
                    // The secure write failed; leave the plaintext in bmux.json so a
    ...
    /// that appear outside of string literals, so a JSONC `bmux.json` can be
