...
///
/// Use this shared formatter anywhere bmux shows a ``StoredShortcut`` so named
...
public struct ShortcutDisplayFormatter: Sendable {
    ...
    /// Formats modifier booleans in bmux's standard `Control`, `Option`, `Shift`, `Command` order.
    ///
    /// - Parameters:
    ///   - command: Whether the Command modifier is present.
    ///   - shift: Whether the Shift modifier is present.
