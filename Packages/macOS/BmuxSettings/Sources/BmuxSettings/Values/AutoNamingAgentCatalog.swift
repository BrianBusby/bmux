...
/// power user can name a custom agent in `~/.config/bmux/bmux.json` even if it
...
public struct AutoNamingAgentOption: Sendable, Hashable {
    ...
    /// `<agent>` segment of `bmux hooks <agent> auto-name`.
    public let slug: String
    /// Brand display name shown in the picker (proper noun — not localized).
    public let displayName: String
    /// Whether bmux currently knows how to drive this agent as a summarizer.
...
///
/// Lives in `BmuxSettings` (imported by both the app's Settings UI and the
/// bundled `bmux` CLI) so the picker and the summarizer dispatch share one
...
public enum AutoNamingAgentCatalog {
    ...
    /// Agents whose binary bmux knows how to invoke for a one-shot
    ...
    /// agent until bmux learns to drive them).
    ...
    /// True only when bmux can drive `slug` as a summarizer. Custom/unknown
