...
/// Controls which clients may connect to the bmux automation socket.
///
/// Stored under the catalog entry ``SettingCatalog/automationSocketControlMode``.
/// The cases mirror the on-disk strings used by `~/.config/bmux/bmux.json` and
...
public enum SocketControlMode: String, CaseIterable, Sendable, SettingCodable {
    ...
    case off
    /// Only the bundled `bmux` CLI may connect.
    case bmuxOnly
