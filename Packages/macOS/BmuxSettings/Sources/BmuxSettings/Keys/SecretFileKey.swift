...
///
/// `SecretFileKey` is the third key flavor in ``BmuxSettings``, alongside
/// ``DefaultsKey`` (UserDefaults) and ``JSONKey`` (the bmux JSON config). It
/// exists for values that must not live in the shared, user-editable
/// `bmux.json` (for example, the socket-control password): each secret is
...
/// store (the app uses `~/.config/bmux`, the same directory as `bmux.json`).
///
/// ```swift
