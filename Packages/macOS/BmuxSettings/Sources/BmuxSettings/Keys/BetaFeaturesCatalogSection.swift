...
/// pattern `<feature-domain>.beta.<flag-name>` so the bmux.json view
...
public struct BetaFeaturesCatalogSection: SettingCatalogSection {
    ...
    /// JSON) discovered from `~/.config/bmux/sidebars/` and selectable in the
    ...
    /// Remote tmux: mirror a remote host's tmux sessions in the bmux sidebar
    ...
    /// create/close propagate to the remote, while quitting bmux leaves the
