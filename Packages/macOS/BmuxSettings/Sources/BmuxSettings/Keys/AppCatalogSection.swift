...
public struct AppCatalogSection: SettingCatalogSection {
    ...
    public let openSupportedFilesInBmux = DefaultsKey<Bool>(
        id: "app.openSupportedFilesInBmux",
        defaultValue: true,
        userDefaultsKey: "openSupportedFilesInBmux"
    )
    ...
    public let openMarkdownInBmuxViewer = DefaultsKey<Bool>(
        id: "app.openMarkdownInBmuxViewer",
        defaultValue: true,
        userDefaultsKey: "openMarkdownInBmuxViewer"
    )

    /// App-wide magnification percent for bmux-owned fonts and chrome.
    ...
    /// Shared, cross-tag default display that DEBUG bmux builds open new
    /// windows on, identified by the display's `localizedName` (e.g.
    /// `"LG HDR 4K"`). Empty means the system default placement.
    ///
    /// JSON-backed (`JSONKey`) on purpose: `bmux.json` lives at a fixed path
