...
/// signed in to the bmux backend.
///
/// The sign-in flow, token storage, and `BMUXAuthUser` model itself
/// live in the host app's auth package and reach the package's
/// ``AccountSection`` via an injected delegate (see ``AccountFlow``
/// in BmuxSettingsUI), not through this catalog.
