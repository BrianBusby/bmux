...
Strongly-typed, migratable settings storage for bmux. Depends only on
Foundation and `BmuxFileWatch` (for config-file reload watching). Modern
...
- **`JSONConfigStore`** — wraps `~/.config/bmux/bmux.json`. Structured config
...
- **JSON config** lives at `~/.config/bmux/bmux.json` by default
  (`BmuxConfigLocation().userConfigFile`). The store creates the file (and
  parent directory) on first write. File is missing on first launch, in
  which case every read returns the key's default value.

A populated bmux.json looks like this — pretty-printed, sorted keys, JSONC
...
let jsonConfigStore = JSONConfigStore(
    fileURL: BmuxConfigLocation().userConfigFile
)
...
let tempDir = FileManager.default.temporaryDirectory
    .appending(path: "bmux-readme-\(UUID().uuidString)")
try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
let fileURL = tempDir.appending(path: "bmux.json")
...
1. Pick the catalog section under `Sources/BmuxSettings/Keys/` (e.g.
...
1. Create `Sources/BmuxSettings/Keys/FooCatalogSection.swift`:
   ```swift
...
- **`JSONConfigStore`** — `actor`. Async reads/writes/observe. Owns one
  `BmuxFileWatch.FileWatcher` and fans out file-change events to per-subscriber
  bounded signal streams (no `N × parse` work under burst changes). File
  watching itself lives in the `BmuxFileWatch` package.
...
is the production registry. See `Tests/BmuxSettingsTests/` for patterns.
...
comparisons. See `Tests/BmuxSettingsTests/ShortcutWhenClauseTests.swift`.

## Concurrency

The bmux repo enforces a strict modern-concurrency policy
(see the bmux `CLAUDE.md` "Modern Swift concurrency" section): no locks,
