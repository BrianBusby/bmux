...
struct SettingCatalogTests {
    ...
    @Test func allReachesEverySection() {
        ...
        #expect(ids.contains("integrations.linear.apiKey"))
        #expect(ids.contains("integrations.linear.authorizationHeader"))
    }
    ...
    @Test func keyIdsMatchTheirSectionPrefix() {
        ...
        for key in catalog.integrations.all { #expect(key.id.hasPrefix("integrations.")) }
