...
@Suite struct BmuxStateDirectoryTests {
    ...
    @Test func socketPathStaysWellWithinUnixPathLimit() {
        ...
        let socket = BmuxStateDirectory.url(homeDirectory: home)
    ...
    @Test func legacyApplicationSupportURLPointsAtOldLocation() {
        let legacy = BmuxStateDirectory.legacyApplicationSupportURL(fileManager: .default)
        // Only used for one-time migration; it must still address the old folder.
        #expect(legacy?.lastPathComponent == "bmux")
        #expect(legacy?.path.contains("/Application Support/bmux") == true)
    }
}
...
@Suite(.serialized) struct SocketControlPasswordMigrationTests {
    private func tempDir() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-pw-migrate-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    /// TCC-protected directory (https://github.com/manaflow-ai/bmux/issues/5146).
