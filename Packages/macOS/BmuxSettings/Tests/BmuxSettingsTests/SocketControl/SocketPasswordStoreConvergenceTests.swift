...
/// without touching the real Application Support, keychain, or `bmux.json`.
@Suite struct SocketPasswordStoreConvergenceTests {
    private let secretKey = SettingCatalog().automation.socketPassword

    /// A throwaway control directory standing in for ``BmuxStateDirectory``.
    private func tempBaseDirectory() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-convergence-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    /// the password file's parent directory. This mirrors `bmuxApp.init`, which
    ...
    @Test func bothStoresResolveIdenticalFileURL() async throws {
        ...
        // Exactly the derivation bmuxApp.init performs.
