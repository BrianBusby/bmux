...
@Suite struct UpdateStateModelTests {
    ...
    @Test func errorDetailsIncludesLogPath() {
        let err = NSError(domain: "bmux.update", code: 7, userInfo: [NSLocalizedDescriptionKey: "boom"])
    ...
    @Test func installerAgentFailureHasOwnTitleAndOffersManualDownload() {
        ...
        #expect(UpdateManualDownloadRecovery().url(for: err)?.absoluteString.hasSuffix("bmux-macos.dmg") == true)
    }
