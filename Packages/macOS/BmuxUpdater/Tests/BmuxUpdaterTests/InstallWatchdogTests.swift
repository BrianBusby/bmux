...
@Suite struct InstallWatchdogTests {
    ...
    @Test func manualDownloadRoutesToTheActiveChannel() throws {
        let didNotStart = NSError(domain: UpdateStateModel.updateErrorDomain, code: UpdateStateModel.installDidNotStartCode)
        let nightlyFeed = "https://github.com/manaflow-ai/bmux/releases/download/nightly/appcast.xml"
        let recovery = UpdateManualDownloadRecovery()

        let nightlyURL = try #require(recovery.url(for: didNotStart, feedURLString: nightlyFeed))
        #expect(nightlyURL.absoluteString.hasSuffix("/releases/download/nightly/bmux-nightly-macos.dmg"))
        #expect(!nightlyURL.absoluteString.contains("latest/download"))

        let stableURL = try #require(recovery.url(for: didNotStart, feedURLString: "https://bmux.com/appcast.xml"))
        ...
        #expect(sparkleNightlyURL.absoluteString.hasSuffix("/releases/download/nightly/bmux-nightly-macos.dmg"))
    }
    ...
    @Test func unresolvedDelegateFeedStillRoutesWatchdogRecoveryToNightly() throws {
        ...
        #expect(recoveryURL.absoluteString == "https://github.com/manaflow-ai/bmux/releases/download/nightly/bmux-nightly-macos.dmg")
    ...
    /// An unrecognized bmux.update code renders the generic failure title instead of silently
    /// masquerading as "Updater Not Ready".
    @Test func unknownBmuxUpdateCodeFallsBackToGenericTitle() {
    ...
    @Test func watchdogErrorRendersDedicatedCopy() {
        let error = NSError(
            ...
            userInfo: [NSLocalizedDescriptionKey: "bmux couldn’t start the update. Check your internet connection and try again."]
        )
