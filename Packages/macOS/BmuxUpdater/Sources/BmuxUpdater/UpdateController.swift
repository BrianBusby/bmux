...
/// Coordinates bmux's custom Sparkle update flow: owns the `SPUUpdater` and its
...
public final class UpdateController {
    ...
    /// Whether the running build is a bmux DEV/staging build that must never be compared against
    ...
    ///   - settings: The Sparkle defaults/migration configuration. Defaults to bmux's hourly check.
    ...
    init(log: any UpdateLogging,
        ...
        if isDevLikeBundle {
            // DEV (`com.bmuxterm.app.debug[.<tag>]`) and staging (`com.bmuxterm.app.staging[.<tag>]`)
            ...
            // bmux's launch/background probe is short-circuited by the `automaticallyChecksForUpdates`
    ...
    public func startUpdaterIfNeeded() {
        ...
        if ProcessInfo.processInfo.environment["BMUX_UI_TEST_RESET_SPARKLE_PERMISSION"] == "1" {
    ...
    private func recordUITestTimestamp(key: String) {
        ...
        guard env["BMUX_UI_TEST_MODE"] == "1" else { return }
        guard let path = env["BMUX_UI_TEST_TIMING_PATH"] else { return }
...
extension UpdateController {
    /// Whether `bundleIdentifier` is a bmux DEV (`com.bmuxterm.app.debug[.<tag>]`) or staging
    /// (`com.bmuxterm.app.staging[.<tag>]`) build.
    ///
    ...
    /// `isStagingBundleIdentifier` (in the BmuxSettings package). The classification is
    /// duplicated here deliberately to avoid introducing a `BmuxUpdater → BmuxSettings` package
