...
/// transitions for bmux's custom (non-Sparkle-UI) update surface.
///
...
final class UpdateDriver: NSObject, @preconcurrency SPUUserDriver {
    ...
    /// Whether the running build is a bmux DEV/staging build that is not on the public release
    ...
    func show(_ request: SPUUpdatePermissionRequest,
        ...
        if env["BMUX_UI_TEST_TRIGGER_UPDATE_CHECK"] == "1" || env["BMUX_UI_TEST_AUTO_ALLOW_PERMISSION"] == "1" {
...
#endif
        // Never show Sparkle's permission UI. bmux always enables scheduled checks and keeps
    ...
    func showUpdateReleaseNotes(with downloadData: SPUDownloadData) {
        // bmux uses Sparkle's UI for release notes links instead.
    }
    ...
    func showUpdateInFocus() {
        // No-op; bmux never shows Sparkle dialogs.
    }
