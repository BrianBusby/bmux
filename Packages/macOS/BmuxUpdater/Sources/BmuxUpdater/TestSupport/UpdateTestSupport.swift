...
///
/// DEBUG-only test scaffolding, gated on `BMUX_UI_TEST_*` environment variables. Constructed by
...
public struct UpdateTestSupport {
    ...
    public func applyIfNeeded() {
        ...
        if let detectedVersion = env["BMUX_UI_TEST_DETECTED_UPDATE_VERSION"],
        ...
        guard let state = env["BMUX_UI_TEST_UPDATE_STATE"] else { return }
        switch state {
        case "available":
            let version = env["BMUX_UI_TEST_UPDATE_VERSION"] ?? "9.9.9"
        ...
        case "error":
            let message = env["BMUX_UI_TEST_UPDATE_ERROR_MESSAGE"] ?? "Test update error"
            let error = NSError(domain: "bmux.update.uitest", code: 1, userInfo: [NSLocalizedDescriptionKey: message])
            transition(to: .error(.init(
                error: error,
                retry: {},
                dismiss: {},
                technicalDetails: "ui-test error",
                feedURLString: env["BMUX_UI_TEST_FEED_URL"]
            )))
        default:
            break
        }
    ...
    /// Runs a mock feed check against `BMUX_UI_TEST_FEED_URL` if requested. Returns whether it ran.
    public func performMockFeedCheckIfNeeded() -> Bool {
        let env = ProcessInfo.processInfo.environment
        guard env["BMUX_UI_TEST_TRIGGER_UPDATE_CHECK"] == "1" else { return false }
        guard let feedURLString = env["BMUX_UI_TEST_FEED_URL"],
        ...
        let task = URLSession.shared.dataTask(with: feedURL) { data, _, _ in
            ...
            let delayMilliseconds = Int(env["BMUX_UI_TEST_MOCK_FEED_DELAY_MS"] ?? "") ?? 0
            Task { @MainActor in
