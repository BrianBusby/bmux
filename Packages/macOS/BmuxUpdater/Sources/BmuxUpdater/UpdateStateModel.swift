...
/// Observation framework; appearance (color) derivations live in the `BmuxUpdaterUI` package.
///
...
public final class UpdateStateModel {
    ...
    // MARK: - bmux-originated update errors

    /// The `NSError` domain for update errors that bmux itself raises (as opposed to Sparkle or
    /// `NSURLError`). Errors in this domain carry user-ready, already-localized copy in their
    /// `localizedDescription`.
    public nonisolated static let updateErrorDomain = "bmux.update"
    ...
    public func debugShowUpdateError(_ scenario: DebugUpdateErrorScenario) {
        setOverrideState(.error(.init(
            ...
            feedURLString: "https://github.com/manaflow-ai/bmux/releases/latest/download/appcast.xml"
        )))
    }
    #endif
...
public enum DebugUpdateErrorScenario: String, CaseIterable, Hashable, Sendable {
    ...
    case noInternet
    /// bmux.update install-watchdog trip: "Update Didn't Start" (the install-loop guard).
    case installDidNotStart
    /// bmux.update readiness timeout: "Updater Not Ready".
        ...
        case .installDidNotStart:
            return NSError(domain: UpdateStateModel.updateErrorDomain, code: UpdateStateModel.installDidNotStartCode, userInfo: [
                NSLocalizedDescriptionKey: String(
                    ...
                    defaultValue: "bmux couldn’t start the update. Check your internet connection and try again."
                ),
            ])
        case .updaterNotReady:
