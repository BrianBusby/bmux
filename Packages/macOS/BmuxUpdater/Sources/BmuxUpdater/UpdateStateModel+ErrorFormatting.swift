...
extension UpdateStateModel {
    ...
    public static func userFacingErrorTitle(for error: any Swift.Error) -> String {
        ...
        if nsError.domain == updateErrorDomain {
            ...
            default:
                // An unrecognized bmux.update code falls through to the generic title below
    ...
    public static func userFacingErrorMessage(for error: any Swift.Error) -> String {
        let nsError = error as NSError
        if nsError.domain == updateErrorDomain {
            // bmux-originated errors already carry user-ready, localized copy.
        ...
        if let networkError = networkError(from: nsError) {
            switch networkError.code {
            case NSURLErrorNotConnectedToInternet:
                return String(localized: "update.error.noInternet.message", defaultValue: "bmux can’t reach the update server. Check your internet connection and try again.")
            case NSURLErrorTimedOut:
            ...
            case NSURLErrorCannotConnectToHost:
                return String(localized: "update.error.serverUnreachable.message", defaultValue: "bmux couldn’t connect to the update server. Check your connection or try again later.")
            case NSURLErrorNetworkConnectionLost:
        ...
        if nsError.domain == SUSparkleErrorDomain {
            switch nsError.code {
            case 2001:
                return String(localized: "update.error.feedDownload.message", defaultValue: "bmux couldn't download the update feed. Check your connection and try again.")
            case 1000, 1002:
            ...
            case 1003, 1005:
                return String(localized: "update.error.permissionError.message", defaultValue: "Move bmux into Applications and relaunch to enable updates.")
            case 4005, 4010:
                return String(localized: "update.error.installRecovery.message", defaultValue: "Move bmux into Applications and relaunch to enable updates. If it’s already in Applications, restart your Mac and try again, or download the latest version below.")
            default:
                break
            }
        }
        // Catch-all: keep user-facing copy in bmux terms; raw vendor descriptions, domains, and
