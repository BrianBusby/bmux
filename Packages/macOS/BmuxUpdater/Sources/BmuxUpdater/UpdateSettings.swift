...
/// Registers bmux's Sparkle preference defaults and performs the one-time migration that
...
/// configuration, so this is a value type constructed with them (defaulting to bmux's
...
public struct UpdateSettings: Sendable {
    ...
    /// bmux's marker that the v2 automatic-checks migration already ran.
    public static let migrationKey = "bmux.sparkle.automaticChecksMigration.v2"
    ...
    /// The scheduled-check interval bmux registers (1h by default).
    public let scheduledCheckInterval: TimeInterval

    /// Creates the settings with bmux's defaults.
    ///
