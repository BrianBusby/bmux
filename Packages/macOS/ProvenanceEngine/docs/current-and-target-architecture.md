...
## Three-view Coding Session Model

bmux's target coding-agent experience has three distinct views over one
underlying provider session identity:

- Native: the provider-native surface, kept for fidelity, debugging, provider
  features bmux has not normalized, and escape-hatch workflows.
- Terminal: bmux's React live interaction surface. It renders conversation,
  streaming response state, tool and command lifecycle, provider controls,
  approvals, interrupts, skills, modes, and other live execution interaction.
- Session: bmux's React smart summary surface. It answers what the work means,
  what has completed, what is happening now, what is planned next, and whether
...
The current factual Session UI work in bmux is useful groundwork for the
factual consumer shape and diagnostics. It is not the final Smart Session
product by itself, especially where it is implemented as native Swift UI. The
intended user-facing Smart Session information architecture is React, with
native code hosting, routing, and preserving session identity where needed.
...
For bmux's Smart Session view, completed turns and the current turn are distinct
presentation needs. Completed turns should eventually be compact, chronological,
and expandable with provenance. The current turn should update from new factual
and semantic evidence without becoming a second live transcript. Detailed token,
tool, and command-output replay belongs in the React Terminal view or the
provider-native view.
