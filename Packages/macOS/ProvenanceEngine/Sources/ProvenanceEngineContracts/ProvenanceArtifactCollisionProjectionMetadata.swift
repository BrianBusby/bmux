import Foundation

/// Revision and rule metadata for artifact-collision awareness.
public struct ProvenanceArtifactCollisionProjectionMetadata: Codable, Equatable, Sendable {
    /// Stable revision identifier for this projection content.
    public let revisionID: String

    /// Stable deterministic projection rule identifier.
    public let projectionRuleID: String

    /// Version of the deterministic projection rule.
    public let projectionRuleVersion: String

    /// Stable fingerprint of bounded query options.
    public let requestFingerprint: String

    /// Stable fingerprint of returned candidate content.
    public let contentFingerprint: String

    /// Result limit applied by this projection.
    public let resultLimit: Int

    /// Related-session limit applied before artifact candidate discovery.
    public let relatedSessionLimit: Int

    /// Exclusion explanation limit applied by this projection.
    public let exclusionLimit: Int

    /// Optional observed artifact path filter supplied by the request.
    public let artifactPath: String?

    /// Normalized artifact path filter applied by deterministic rules.
    public let normalizedArtifactPath: String?

    /// Optional lower bound for candidate latest artifact observation.
    public let updatedAfter: Date?

    /// Optional boundary used to classify stale candidates.
    public let staleBefore: Date?

    /// Latest accepted event-ledger sequence evaluated by the projection.
    public let sourceEvidenceWatermark: Int?

    /// Deterministic generation timestamp from accepted evidence.
    public let generatedAt: Date?

    /// Creates projection metadata.
    public init(
        revisionID: String,
        projectionRuleID: String,
        projectionRuleVersion: String,
        requestFingerprint: String,
        contentFingerprint: String,
        resultLimit: Int,
        relatedSessionLimit: Int,
        exclusionLimit: Int,
        artifactPath: String?,
        normalizedArtifactPath: String?,
        updatedAfter: Date?,
        staleBefore: Date?,
        sourceEvidenceWatermark: Int?,
        generatedAt: Date?
    ) {
        self.revisionID = revisionID
        self.projectionRuleID = projectionRuleID
        self.projectionRuleVersion = projectionRuleVersion
        self.requestFingerprint = requestFingerprint
        self.contentFingerprint = contentFingerprint
        self.resultLimit = resultLimit
        self.relatedSessionLimit = relatedSessionLimit
        self.exclusionLimit = exclusionLimit
        self.artifactPath = artifactPath
        self.normalizedArtifactPath = normalizedArtifactPath
        self.updatedAfter = updatedAfter
        self.staleBefore = staleBefore
        self.sourceEvidenceWatermark = sourceEvidenceWatermark
        self.generatedAt = generatedAt
    }
}
