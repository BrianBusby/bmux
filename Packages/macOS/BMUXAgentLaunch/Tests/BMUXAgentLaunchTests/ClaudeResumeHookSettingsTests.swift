...
/// Regression coverage for https://github.com/manaflow-ai/bmux/issues/5427.
///
/// bmux injects Claude Code's hooks from the `Resources/bin/claude` wrapper,
/// which is first on `PATH` inside bmux terminals and re-injects the hook
/// `--settings` whenever it sees `--resume`. The captured launch executable,
/// however, is the *real* claude binary (`BMUX_AGENT_LAUNCH_EXECUTABLE`).
...
@Suite("Claude resume routes through the bmux wrapper")
struct ClaudeResumeHookSettingsTests {
    ...
    func staleHookSettingsStripped() throws {
        ...
        // dropped; the wrapper re-applies bmux's current hooks at exec time.
        let argv = try #require(
            AgentResumeArgv().builtInKind(
                kind: "claude",
                sessionId: "s",
                ...
                arguments: [
                    ...
                    "opus",
                    "--settings",
                    "/tmp/bmux-claude-hook-x/settings.json"
                ]
            )
        )
        #expect(argv == ["claude", "--resume", "s", "--model", "opus"])
    ...
    func mergedHookSettingsKeepsUserKeysOnResume() throws {
        let mergedSettings = #"{"effortLevel":"max","preferredNotifChannel":"notifications_disabled","hooks":{"SessionStart":[{"matcher":"","hooks":[{"type":"command","command":"bmux hooks claude session-start","timeout":10}]}]}}"#
