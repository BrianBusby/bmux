...
struct AgentSpawnIdentityTests {
    ...
    // focused surface A (which would desync from BMUX_PANEL_ID and restore into the wrong surface).
    ...
    func orphanOwnSurfaceIsNotPairedWithFocusedWorkspace() {
        // Only BMUX_SURFACE_ID was inherited (no BMUX_WORKSPACE_ID), e.g. partial env scrubbing, while
