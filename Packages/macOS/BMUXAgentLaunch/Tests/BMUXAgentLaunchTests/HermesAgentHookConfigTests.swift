...
struct HermesAgentHookConfigTests {
    ...
    func installsHooksIntoEmptyConfig() {
        let events = [
            HermesAgentHookConfig.Event(name: "on_session_start", command: "sh -c 'bmux hooks hermes-agent session-start'"),
            HermesAgentHookConfig.Event(name: "pre_tool_call", command: "sh -c 'bmux hooks feed --source hermes-agent --event pre_tool_call'", timeout: 120),
        ]

        let installed = HermesAgentHookConfig.installing(events: events, in: "")

        #expect(installed.contains("# bmux hooks hermes-agent begin\nhooks:\n  on_session_start:"))
        #expect(installed.contains("    - command: \"sh -c 'bmux hooks hermes-agent session-start'\""))
        #expect(installed.contains("      timeout: 120"))
        #expect(HermesAgentHookConfig.uninstalling(from: installed) == "")
    }

    @Test("Coalesces multiple bmux hooks for the same missing Hermes event")
    func coalescesMultipleHooksForSameMissingEvent() {
        let events = [
            HermesAgentHookConfig.Event(name: "pre_approval_request", command: "sh -c 'bmux hooks hermes-agent notification'"),
            HermesAgentHookConfig.Event(name: "pre_approval_request", command: "sh -c 'bmux hooks feed --source hermes-agent --event pre_approval_request'", timeout: 120),
            HermesAgentHookConfig.Event(name: "post_llm_call", command: "sh -c 'bmux hooks hermes-agent agent-response'"),
        ]
        ...
        #expect(
            installed.contains("""
              pre_approval_request:
                - command: "sh -c 'bmux hooks hermes-agent notification'"
                  timeout: 5
                - command: "sh -c 'bmux hooks feed --source hermes-agent --event pre_approval_request'"
                  timeout: 120
            """)
        )
    ...
    @Test("Coalesces multiple bmux hooks for the same existing Hermes event")
    func coalescesMultipleHooksForSameExistingEvent() {
        ...
        #expect(
            installed.contains("""
              pre_approval_request:
                # bmux hooks hermes-agent begin
                - command: "sh -c 'bmux hooks hermes-agent notification'"
                  timeout: 5
                - command: "sh -c 'bmux hooks feed --source hermes-agent --event pre_approval_request'"
                  timeout: 120
                # bmux hooks hermes-agent end
                - command: "echo user"
                  timeout: 10
            """)
        )
    ...
    func preservesExistingHookEventsWithoutDuplicatingKeys() {
        ...
        let events = [
            HermesAgentHookConfig.Event(name: "pre_tool_call", command: "sh -c 'bmux hooks feed --source hermes-agent --event pre_tool_call'", timeout: 120),
            HermesAgentHookConfig.Event(name: "on_session_end", command: "sh -c 'bmux hooks hermes-agent stop'"),
        ]

        let installed = HermesAgentHookConfig.installing(events: events, in: existing)

        #expect(installed.components(separatedBy: "\n  pre_tool_call:").count == 2)
        #expect(installed.contains("  pre_tool_call:\n    # bmux hooks hermes-agent begin\n    - command: \"sh -c 'bmux hooks feed --source hermes-agent --event pre_tool_call'\""))
    ...
    func installsIntoMultipleExistingEventsWithoutShiftingLaterIndexes() {
        ...
        """
        let events = [
            HermesAgentHookConfig.Event(name: "pre_tool_call", command: "sh -c 'bmux hooks feed --source hermes-agent --event pre_tool_call'"),
            HermesAgentHookConfig.Event(name: "post_tool_call", command: "sh -c 'bmux hooks feed --source hermes-agent --event post_tool_call'"),
        ]
    ...
    func uninstallsInlineEmptyHooksRoot() {
        ...
        #expect(installed.contains("hooks:\n  # bmux hooks hermes-agent begin restore-line-base64:"))
        #expect(installed.contains("  pre_tool_call:"))
        #expect(HermesAgentHookConfig.uninstalling(from: installed) == existing)
    }

    @Test("Allowlist install and uninstall only touches bmux commands")
    func allowlistInstallAndUninstallOnlyTouchesBmuxCommands() throws {
        let existing = """
        {
          "approvals": [
            {
