...
struct AgentResumeArgvTests {
    ...
    func opencodeResumeDropsInternalTUISettingsSelector() {
        ...
        #expect(
            ...
            ) == .resolved(["bmux", "omo", "--session", "SID", "--model", "anthropic/claude-sonnet-4-6"])
        )
        #expect(
            AgentResumeArgv().builtInKind(
                kind: "opencode",
    ...
    func codexResumeSuppressesStartupUpdatePrompt() {
        ...
        // "Update available!" picker before restoring the session — a bmux relaunch that
        // auto-restores codex panes lands them on that prompt instead of the conversation.
        // The per-invocation `-c` override keeps bmux-driven restores non-interactive
        ...
        #expect(
            AgentResumeArgv().launcherResolution(
                ...
                sessionId: "SID",
                executablePath: nil,
                arguments: ["bmux", "codex-teams", "--model", "gpt-5.4"]
            ) == .resolved(["bmux", "codex-teams", "resume", "SID"] + overrides + ["--model", "gpt-5.4"])
        )
    }
    ...
    func codexResumeRespectsExplicitUpdateCheckSetting() {
        ...
        #expect(
            AgentResumeArgv().launcherResolution(
                ...
                sessionId: "SID",
                executablePath: nil,
                arguments: ["bmux", "codex-teams", "-c", "check_for_update_on_startup=true"]
            ) == .resolved(["bmux", "codex-teams", "resume", "SID", "-c", "check_for_update_on_startup=true"])
        )
    }

    @Test("bmux wrapper launchers resolve before per-kind verbs")
    func launcherWrappers() {
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "claudeTeams", sessionId: "SID", executablePath: nil, arguments: ["bmux", "claude-teams"]
            ) == .resolved(["bmux", "claude-teams", "--resume", "SID"])
        )
        #expect(
            AgentResumeArgv().launcherResolution(
        ...
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "codexTeams", sessionId: "SID", executablePath: nil, arguments: ["bmux", "codex-teams"]
            ) == .resolved(["bmux", "codex-teams", "resume", "SID", "-c", "check_for_update_on_startup=false"])
        )
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omo", sessionId: "SID", executablePath: nil, arguments: ["bmux", "omo"]
            ) == .resolved(["bmux", "omo", "--session", "SID"])
        )
        // One-shot wrappers have no resumable form (omx and omc share an arm; exercise each).
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omx", sessionId: "SID", executablePath: nil, arguments: ["bmux", "omx"]
            ) == .resolved(nil)
        )
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omc", sessionId: "SID", executablePath: nil, arguments: ["bmux", "omc"]
            ) == .resolved(nil)
        )
    ...
    func renderedPortableClaudeResumeShellCommand() {
        ...
        // Launcher resumes that resolve to bmux's own CLI emit no bare `claude`:
        // already-portable quoted words stay unwrapped.
        #expect(
            AgentResumeArgv.renderedPortableClaudeResumeShellCommand(
                parts: ["/Applications/bmux.app/Contents/Resources/bin/bmux", "claude-teams", "--resume", "SID"],
                quote: quote
            ) == "'/Applications/bmux.app/Contents/Resources/bin/bmux' 'claude-teams' '--resume' 'SID'"
        )
    }

    @Test("Codex wrapper token resolves BMUX_CODEX_WRAPPER_SHIM, degrading to bare codex")
    ...
    func renderedPortableCodexResumeShellCommand() {
        ...
        #expect(
            AgentResumeArgv.renderedPortableCodexResumeShellCommand(
                parts: ["/Applications/bmux.app/Contents/Resources/bin/bmux", "codex-teams", "resume", "SID"],
                quote: quote
            ) == "'/Applications/bmux.app/Contents/Resources/bin/bmux' 'codex-teams' 'resume' 'SID'"
        )
    }
}
