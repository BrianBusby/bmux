...
struct AgentForkArgvTests {
    ...
    @Test("bmux wrapper launchers use fork verbs")
    func launcherWrappersUseForkVerbs() {
        #expect(
            AgentForkArgv().launcherResolution(
                ...
                sessionId: "SID",
                executablePath: nil,
                arguments: ["bmux", "claude-teams", "--worktree", "/tmp/team repo"]
            ) == .resolved(["bmux", "claude-teams", "--resume", "SID", "--fork-session", "--worktree", "/tmp/team repo"])
        )
        #expect(
            AgentForkArgv().launcherResolution(
                launcher: "codexTeams",
                sessionId: "SID",
                executablePath: nil,
                arguments: ["bmux", "codex-teams", "--model", "gpt-5"]
            ) == .resolved(["bmux", "codex-teams", "fork", "SID", "--model", "gpt-5"])
        )
        #expect(
            AgentForkArgv().launcherResolution(
                launcher: "omo",
                sessionId: "SID",
                executablePath: nil,
                arguments: ["bmux", "omo", "--model", "anthropic/claude-sonnet-4-6"]
            ) == .resolved(["bmux", "omo", "--session", "SID", "--fork", "--model", "anthropic/claude-sonnet-4-6"])
        )
        #expect(
            AgentForkArgv().launcherResolution(
                launcher: "omx",
