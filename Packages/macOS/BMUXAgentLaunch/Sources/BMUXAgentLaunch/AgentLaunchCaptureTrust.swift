...
///
/// `BMUX_AGENT_LAUNCH_*` is exported into the agent's process environment by the
/// bmux launch wrappers and therefore leaks to every descendant process: an agent
...
public enum AgentLaunchCaptureTrust {
    ...
    /// and the bmux app executable, from becoming persisted resume commands.
