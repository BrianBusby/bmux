...
public enum AgentLaunchEnvironmentPolicy {
    ...
    private static func selectedNodeOptions(from env: [String: String]) -> String? {
        switch normalizedValue(env["BMUX_ORIGINAL_NODE_OPTIONS_PRESENT"]) {
        case "1":
            return sanitizedNodeOptions(env["BMUX_ORIGINAL_NODE_OPTIONS"])
        case "0":
            return nil
        default:
    ...
    private static func sanitizedNodeOptions(_ rawValue: String?) -> String? {
        ...
        while index < tokens.count {
            ...
            if isRequireOption(token), index + 1 < tokens.count,
               isBmuxNodeOptionsRestoreModulePath(tokens[index + 1]) {
                index += 2
                shouldDropInjectedHeapCap = true
                continue
            }
            if let path = inlineRequireOptionPath(token),
               isBmuxNodeOptionsRestoreModulePath(path) {
                index += 1
                shouldDropInjectedHeapCap = true
                continue
            }
    ...
    private static func isBmuxNodeOptionsRestoreModulePath(_ value: String) -> Bool {
        let trimmed = value.trimmingCharacters(in: CharacterSet(charactersIn: "'\""))
        guard URL(fileURLWithPath: trimmed).lastPathComponent == "restore-node-options.cjs" else {
            return false
        }
        return trimmed.contains("/bmux-")
    }
