...
/// bmux socket, then materialized into a `WorkstreamItem` by the store.
///
...
/// `_opencode_request_id`. `context` is bmux-specific and optional.
public struct WorkstreamEvent: Codable, Sendable, Equatable {
    ...
    /// bmux hook wrappers already emit on stdin so no translation layer is
    /// needed.
