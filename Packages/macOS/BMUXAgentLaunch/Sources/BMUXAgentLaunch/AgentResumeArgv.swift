...
/// (`AgentResumeCommandBuilder` in the app target) and the standalone `bmux-cli` surface-restore
...
///
/// Resolution order mirrors the historical app builder: a bmux wrapper launcher
...
public struct AgentResumeArgv: Sendable, Equatable {
    ...
    /// The shell token that resolves bmux's `claude` wrapper at exec time.
    ///
    /// The claude resume/fork argv emits a bare `claude` executable and relies on
    /// bmux's `claude` wrapper (which re-injects the hook `--settings` on `--resume`)
    /// being reachable. Inside bmux terminals the wrapper is reached two ways: a
    ...
    ///
    /// `BMUX_CLAUDE_WRAPPER_SHIM` is a *managed* terminal environment variable (set by
    ...
    /// `-lic` launcher. https://github.com/manaflow-ai/bmux/issues/5639
    ///
    ...
    /// fallback used when the variable is unset outside bmux.
    ///
    ...
    public static let claudeWrapperShellExecutableToken =
        "\"$([ -x \"${BMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\""

    /// The shell token that resolves bmux's `codex` wrapper at exec time.
    ///
    ...
    /// `bmux-codex-wrapper` and dropping every bmux hook (no `SessionStart`, the
    ...
    /// environment variable `BMUX_CODEX_WRAPPER_SHIM` (set by
    ...
    /// the hooks fire. https://github.com/manaflow-ai/bmux/issues/5639
    ///
    ...
    /// variable is unset outside bmux.
    ///
    ...
    public static let codexWrapperShellExecutableToken =
        "\"$([ -x \"${BMUX_CODEX_WRAPPER_SHIM:-}\" ] && printf '%s' \"$BMUX_CODEX_WRAPPER_SHIM\" || printf codex)\""

    /// Per-invocation config override appended to every bmux-generated codex resume argv.
    ///
    /// codex's TUI shows a blocking "Update available!" picker at startup whenever no
    /// initial prompt is passed — exactly the shape of `codex resume <id>` — so a bmux
    ...
    /// `"$_bmux_resume_shell" -c <command>` for its `csh|tcsh` and `*` branches), and the
    ...
    /// apply), `sh` inherits `BMUX_CLAUDE_WRAPPER_SHIM` from the managed terminal
    /// environment and resolves the token, and outside bmux the unset variable still
    ...
    /// composing on the outside. https://github.com/manaflow-ai/bmux/issues/5639
    ...
    ///
    /// Claude-launcher resumes that resolve to bmux's own CLI (`bmux claude-teams --resume …`)
    /// emit no bare `claude` executable: they were already portable quoted words, the bmux
    ...
    /// layer lets the app resume builder and the bmux-cli surface-restore publisher share one
    /// wrapper-routing seam. https://github.com/manaflow-ai/bmux/issues/5639
    ...
    /// identically while `sh` still inherits `BMUX_CODEX_WRAPPER_SHIM` from the
    ...
    /// Call only for the codex kind. https://github.com/manaflow-ai/bmux/issues/5639
    ...
    /// The result of resolving a bmux wrapper launcher (the `claude-teams` / `codex-teams` / `omo`
    /// style launchers bmux injects), checked before the per-kind verb.
    public enum LauncherResolution: Sendable, Equatable {
        /// The launcher is a bmux wrapper; the associated value is its resume argv, or `nil` when the
    ...
    /// Resolves a resume argv from a bmux wrapper launcher, or ``LauncherResolution/passthrough`` when
    ...
    public func launcherResolution(
        ...
        switch launcher {
        case "claudeTeams":
            let parts = commandParts(executablePath: executablePath, arguments: arguments, fallbackExecutable: "bmux")
            var tail = parts.tail
    ...
    /// Builds the claude resume argv, routing it through bmux's `claude` wrapper
    /// so bmux hooks fire on the resumed session.
    ///
    /// bmux injects Claude Code's hook `--settings` from the `bmux-claude-wrapper`,
    ...
    /// *real* claude binary (`BMUX_AGENT_LAUNCH_EXECUTABLE`), so resuming with it
    ...
    /// the `BMUX_CLAUDE_WRAPPER_SHIM` managed env var), because relying on the wrapper
    /// being first on `PATH` does not hold inside the `$SHELL -lic` restore launcher
    /// (https://github.com/manaflow-ai/bmux/issues/5639). The captured executable is
    /// intentionally ignored for claude; the wrapper resolves the real binary
    /// (honouring `BMUX_CUSTOM_CLAUDE_PATH`).
