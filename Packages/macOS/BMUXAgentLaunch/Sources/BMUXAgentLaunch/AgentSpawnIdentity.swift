...
///
/// bmux's CLI agent launchers (the `claude-teams` / `oh-my-codex` / `codex-teams` family) run inside a
/// terminal surface and inherit that surface's own `BMUX_SURFACE_ID` / `BMUX_WORKSPACE_ID`. They also
...
/// pane desyncs `BMUX_SURFACE_ID` from the inherited `BMUX_PANEL_ID`, so an agent launched in surface B
/// while surface A is focused records surface A and later restores into the wrong surface
/// (https://github.com/manaflow-ai/bmux/issues/4920, the codex "jumble after reload" symptom).
///
...
public struct AgentSpawnIdentity: Sendable, Equatable {
    ...
    /// for launches that have no own identity (e.g. started outside a bmux-managed terminal). Inputs are
    ...
    /// that inherits only `BMUX_WORKSPACE_ID` (no surface) while focus is in a *different* workspace
    ...
    ///
    /// - Parameters:
    ///   - ownWorkspaceId: the launching process's inherited `BMUX_WORKSPACE_ID` (the launch surface).
    ///   - ownSurfaceId: the launching process's inherited `BMUX_SURFACE_ID` (the launch surface).
    ...
    public func resolve(
        ...
        if let ownSurface, ownWorkspace != nil {
            ...
            // An orphan own surface (inherited BMUX_SURFACE_ID with no BMUX_WORKSPACE_ID, e.g. partial
