...
/// Builds and removes bmux-owned Kimi Code hook blocks in TOML config files.
...
public enum KimiCodeHookConfig {
    ...
    /// Returns TOML content with exactly one bmux-owned Kimi Code hooks block.
    /// - Parameters:
    ...
    public static func installing(events: [Event], in existing: String) -> String {
        ...
        removeBmuxKimiHooksBlock(from: &lines)
    ...
    /// Returns TOML content after removing bmux-owned Kimi Code hooks blocks.
    /// - Parameter existing: Existing TOML config content.
    /// - Returns: The TOML content without bmux-owned Kimi Code hook blocks.
    ...
    private static func removeBmuxKimiHooksBlock(from lines: inout [String]) {
        var index = 0
