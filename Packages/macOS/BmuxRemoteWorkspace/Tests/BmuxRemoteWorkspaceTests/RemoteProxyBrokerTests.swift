...
struct RemoteProxyBrokerTests {
    ...
    func acquireStartsTunnel() throws {
        ...
        let lease = broker.acquire(configuration: makeConfiguration(), remotePath: "/usr/local/bin/bmuxd-remote") {
            recorder.record($0)
        }
        defer { lease.release() }
        ...
        #expect(tunnel.remotePath == "/usr/local/bin/bmuxd-remote")
    ...
    func ptyOperationsForwardOrThrow() throws {
        ...
        do {
            ...
            #expect(nsError.domain == "bmux.remote.pty")
