...
import Foundation
import Testing
@testable import BmuxRemoteWorkspace
...
struct RemoteDaemonManifestRepositoryTests {
    ...
    private func temporaryHome() throws -> URL {
        let home = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("bmux-manifest-repo-tests-\(UUID().uuidString.prefix(8))", isDirectory: true)
    ...
    private func makeManifestJSON(port: Int, assetPath: String, sha256: String, appVersion: String = "0.99.0") -> String {
        """
        {
          ...
          "checksumsURL": "http://127.0.0.1:\(port)/bmuxd-remote-checksums.txt",
          "entries": [
            {
              "goOS": "linux",
              "goArch": "amd64",
    ...
    @Test("the cache path is versioned by platform under the bmux state directory")
    func cachePathShape() throws {
        ...
        #expect(url.path == home.path + "/.local/state/bmux/remote-daemons/0.62.0/linux-arm64/bmuxd-remote")
        var isDirectory: ObjCBool = false
        let rootExists = FileManager.default.fileExists(
            atPath: home.appendingPathComponent(".local/state/bmux/remote-daemons").path,
    ...
    func fetchManifestStatuses() throws {
        ...
        server.setResponse(path: "/bmuxd-remote-manifest.json", body: Data(manifestJSON.utf8))
        ...
        #expect(manifest?.entry(goOS: "linux", goArch: "amd64")?.assetName == "bmuxd-remote-linux-amd64")

        server.setResponse(path: "/bmuxd-remote-manifest.json", status: 500, body: Data())
    ...
    func downloadHappyPath() throws {
        ...
        server.setResponse(path: "/bmuxd-remote-linux-amd64", body: binary)
        let entry = makeEntry(port: server.port, assetPath: "/bmuxd-remote-linux-amd64", sha256: sha256Hex(binary))
    ...
    func checksumMismatchThrows() throws {
        ...
        let home = try temporaryHome()
        let repository = makeRepository(home: home)
        server.setResponse(path: "/bmuxd-remote-linux-amd64", body: Data("real bytes".utf8))
        let entry = makeEntry(port: server.port, assetPath: "/bmuxd-remote-linux-amd64", sha256: String(repeating: "0", count: 64))
        ...
        do {
            ...
            #expect(nsError.domain == "bmux.remote.daemon")
            #expect(nsError.code == 28)
            #expect(nsError.localizedDescription == "remote daemon checksum mismatch for bmuxd-remote-linux-amd64")
        }
    ...
    func checksumLiveManifestFallback() throws {
        ...
        server.setResponse(path: "/bmuxd-remote-linux-amd64", body: binary)
        server.setResponse(
            path: "/bmuxd-remote-manifest.json",
            body: Data(makeManifestJSON(port: server.port, assetPath: "/bmuxd-remote-linux-amd64", sha256: sha256Hex(binary)).utf8)
        )
        let staleEntry = makeEntry(port: server.port, assetPath: "/bmuxd-remote-linux-amd64", sha256: String(repeating: "f", count: 64))
    ...
    func downloadHTTPErrorThrows() throws {
        ...
        let home = try temporaryHome()
        let repository = makeRepository(home: home)
        server.setResponse(path: "/bmuxd-remote-linux-amd64", status: 503, body: Data())
        let entry = makeEntry(port: server.port, assetPath: "/bmuxd-remote-linux-amd64", sha256: String(repeating: "0", count: 64))
