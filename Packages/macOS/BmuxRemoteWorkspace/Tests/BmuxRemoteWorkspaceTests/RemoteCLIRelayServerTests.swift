...
/// Minimal one-shot unix socket server standing in for the local bmux
...
private final class FakeUnixSocketServer: @unchecked Sendable {
    ...
    init(response: Data) throws {
        ...
        path = NSTemporaryDirectory() + "bmux-relay-test-\(UUID().uuidString.prefix(8)).sock"
...
struct RemoteCLIRelayServerTests {
    ...
    func authAndForwardRoundTrip() throws {
        ...
        #expect(challenge["protocol"] as? String == "bmux-relay-auth")
