...
final class RemoteDaemonProxySession: @unchecked Sendable {
    ...
    private func parseSocksRequest(from data: Data) throws -> SocksRequest? {
        ...
        guard bytes[0] == 0x05 else {
            throw NSError(domain: "bmux.remote.proxy", code: 1, userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS version"])
        }
        ...
        default:
            throw NSError(domain: "bmux.remote.proxy", code: 2, userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS address type"])
        }

        guard !host.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            throw NSError(domain: "bmux.remote.proxy", code: 3, userInfo: [NSLocalizedDescriptionKey: "empty SOCKS host"])
        }
        ...
        guard port > 0 && port <= 65535 else {
            throw NSError(domain: "bmux.remote.proxy", code: 4, userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS port"])
        }
