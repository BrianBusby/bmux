public import BmuxCore
public import BmuxRemoteDaemon
internal import BmuxSettings
...
public final class RemoteDaemonProxyTunnel: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.bmux.remote-ssh.daemon-tunnel.\(UUID().uuidString)", qos: .utility)
    ...
    public func start() throws {
        ...
        queue.sync {
            guard !isStopped else {
                capturedError = NSError(domain: "bmux.remote.proxy", code: 20, userInfo: [
    ...
    public func listPTY() throws -> [[String: Any]] {
        try queue.sync {
            guard let rpcClient, !isStopped else {
                throw NSError(domain: "bmux.remote.pty", code: 30, userInfo: [
    ...
    public func closePTY(sessionID: String) throws {
        try queue.sync {
            guard let rpcClient, !isStopped else {
                throw NSError(domain: "bmux.remote.pty", code: 31, userInfo: [
                    NSLocalizedDescriptionKey: "remote daemon tunnel is not ready",
                ])
            }
    ...
    public func resizePTY(sessionID: String, attachmentID: String, attachmentToken: String, cols: Int, rows: Int) throws {
        try queue.sync {
            guard let rpcClient, !isStopped else {
                throw NSError(domain: "bmux.remote.pty", code: 32, userInfo: [
                    NSLocalizedDescriptionKey: "remote daemon tunnel is not ready",
                ])
            }
    ...
    public func detachPTY(sessionID: String, attachmentID: String, attachmentToken: String) throws {
        try queue.sync {
            guard let rpcClient, !isStopped else {
                throw NSError(domain: "bmux.remote.pty", code: 34, userInfo: [
                    NSLocalizedDescriptionKey: "remote daemon tunnel is not ready",
                ])
            }
    ...
    public func startPTYBridge(sessionID: String, attachmentID: String, command: String?, requireExisting: Bool) throws -> RemotePTYBridgeServer.Endpoint {
        try queue.sync {
            guard let rpcClient, !isStopped else {
                throw NSError(domain: "bmux.remote.pty", code: 33, userInfo: [
                    NSLocalizedDescriptionKey: "remote daemon tunnel is not ready",
                ])
            }
            let bridgeID = UUID()
    ...
    /// keeps VM processes from becoming arbitrary local bmux socket clients.
    ...
    internal static func cloudCLIAuthLoginRequest(password: String) throws -> Data {
        ...
        guard JSONSerialization.isValidJSONObject(request) else {
            throw NSError(domain: "bmux.remote.cli-bridge", code: 7, userInfo: [
                NSLocalizedDescriptionKey: "failed to encode local bmux socket auth request",
            ])
        }
    ...
    private static func roundTripUnixSocket(socketPath: String, request: Data) throws -> Data {
        ...
        guard fd >= 0 else {
            throw NSError(domain: "bmux.remote.cli-bridge", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "failed to create local bmux socket",
            ])
        }
        ...
        guard pathBytes.count <= MemoryLayout.size(ofValue: address.sun_path) else {
            throw NSError(domain: "bmux.remote.cli-bridge", code: 2, userInfo: [
                NSLocalizedDescriptionKey: "local bmux socket path is too long",
            ])
        }
        ...
        guard connectResult == 0 else {
            throw NSError(domain: "bmux.remote.cli-bridge", code: 3, userInfo: [
                NSLocalizedDescriptionKey: "failed to connect to local bmux socket",
            ])
        }
        ...
        if let socketPassword = SocketControlPasswordStore().configuredPassword(allowLazyKeychainFallback: true),
           !socketPassword.isEmpty {
            ...
            guard cloudCLIAuthResponseSucceeded(authResponse) else {
                throw NSError(domain: "bmux.remote.cli-bridge", code: 8, userInfo: [
                    NSLocalizedDescriptionKey: "local bmux socket auth rejected cloud CLI bridge",
                ])
            }
        }
    ...
    private static func writeAll(_ data: Data, to fd: Int32) throws {
        try data.withUnsafeBytes { rawBuffer in
            ...
            while bytesRemaining > 0 {
                ...
                if written <= 0 {
                    throw NSError(domain: "bmux.remote.cli-bridge", code: 4, userInfo: [
    ...
    private static func readLineFromUnixSocket(fd: Int32) throws -> Data {
        ...
        while true {
            ...
            if errno == EAGAIN || errno == EWOULDBLOCK {
                ...
                throw NSError(domain: "bmux.remote.cli-bridge", code: 5, userInfo: [
                    NSLocalizedDescriptionKey: "timed out waiting for local bmux response",
                ])
            }
            throw NSError(domain: "bmux.remote.cli-bridge", code: 6, userInfo: [
                NSLocalizedDescriptionKey: "failed to read local bmux response",
            ])
        }
    }
    ...
    private static func makeLoopbackListener(port: Int) throws -> NWListener {
        guard let localPort = NWEndpoint.Port(rawValue: UInt16(port)) else {
            throw NSError(domain: "bmux.remote.proxy", code: 21, userInfo: [
