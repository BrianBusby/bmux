...
/// Mediates the bmuxd-remote release manifest and the local cache of
...
public struct RemoteDaemonManifestRepository: Sendable {
    ...
    /// Creates a repository rooted at `homeDirectory`'s bmux state cache.
    ///
    /// - Parameters:
    ...
    /// (`<state>/remote-daemons/<version>/<goOS>-<goArch>/bmuxd-remote`),
    /// creating the cache root directory if needed.
    ///
    /// Cached under the non-TCC bmux state directory (matching the CLI's
    /// `remoteDaemonCacheURL`) rather than Application Support, so the
    /// separately-signed CLI can read it on `bmux ssh` without tripping the
    /// macOS Sequoia "access data from other apps" prompt
    /// (https://github.com/manaflow-ai/bmux/issues/5146).
    ...
    public func fetchManifest(releaseURL: String, version: String) -> WorkspaceRemoteDaemonManifest? {
        guard let manifestURL = URL(string: "\(releaseURL)/bmuxd-remote-manifest.json") else { return nil }
        let request = NSMutableURLRequest(url: manifestURL)
        request.timeoutInterval = 15
        request.setValue("bmux/\(version)", forHTTPHeaderField: "User-Agent")
    ...
    public func downloadBinary(
        ...
        guard let url = URL(string: entry.downloadURL) else {
            throw NSError(domain: "bmux.remote.daemon", code: 25, userInfo: [
        ...
        session.downloadTask(with: request as URLRequest) { localURL, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse,
               !(200...299).contains(httpResponse.statusCode) {
                downloadError = NSError(domain: "bmux.remote.daemon", code: 26, userInfo: [
        ...
        guard let downloadedURL else {
            throw NSError(domain: "bmux.remote.daemon", code: 27, userInfo: [
        ...
        if downloadedSHA != entry.sha256.lowercased() {
            ...
            if let releaseURL,
               ...
               downloadedSHA == liveEntry.sha256.lowercased() {
                ...
                throw NSError(domain: "bmux.remote.daemon", code: 28, userInfo: [
    ...
    private func cacheRoot() throws -> URL {
        let cacheRoot = BmuxStateDirectory.url(homeDirectory: homeDirectory)
