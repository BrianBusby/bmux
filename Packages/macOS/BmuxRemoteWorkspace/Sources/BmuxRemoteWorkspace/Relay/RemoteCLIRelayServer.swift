...
/// Loopback TCP relay that lets the remote host's `bmux` CLI reach the local
/// bmux control socket: it listens on `127.0.0.1`, authenticates each
...
///
/// Wire behavior is pinned: the `bmux-relay-auth` challenge JSON (protocol,
...
public final class RemoteCLIRelayServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.bmux.remote-ssh.cli-relay.\(UUID().uuidString)", qos: .utility)
    ...
    ///
    /// - Parameters:
    ///   - localSocketPath: Path of the local bmux control unix socket.
    ///   - relayID: Public identifier echoed in the auth challenge.
    ///   - relayTokenHex: Hex-encoded shared secret; throws
    ///     `bmux.remote.relay` code 7 when invalid or empty.
    ...
    public init(
        ...
        guard let relayToken = Session.hexData(from: relayTokenHex), !relayToken.isEmpty else {
            throw NSError(domain: "bmux.remote.relay", code: 7, userInfo: [
    ...
    public func start() throws -> Int {
        ...
        if waitResult != .success {
            ...
            listener.cancel()
            throw NSError(domain: "bmux.remote.relay", code: 8, userInfo: [
