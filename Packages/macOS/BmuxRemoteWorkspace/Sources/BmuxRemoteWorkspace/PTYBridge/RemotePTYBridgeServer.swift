...
/// Single-use loopback TCP server bridging one local PTY client (the bmux
...
public final class RemotePTYBridgeServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.bmux.remote-ssh.pty-bridge.\(UUID().uuidString)", qos: .userInitiated)
    ...
    public func start() throws -> Endpoint {
        ...
        guard readySemaphore.wait(timeout: .now() + 5.0) == .success else {
            listener.cancel()
            throw NSError(domain: "bmux.remote.pty", code: 20, userInfo: [
        ...
        guard let startupPort = outcome.port, startupPort > 0 else {
            listener.cancel()
            throw NSError(domain: "bmux.remote.pty", code: 21, userInfo: [
