...
/// git/PR subsystem moved into `BmuxGit`. Exercises remote-slug derivation
...
@Suite struct GitConfigIncludeTests {
    ...
    @Test func prioritizesUpstreamThenOriginAndDeduplicates() {
        let output = """
        origin https://github.com/austinwang/bmux.git (fetch)
        origin https://github.com/austinwang/bmux.git (push)
        upstream git@github.com:manaflow-ai/bmux.git (fetch)
        upstream git@github.com:manaflow-ai/bmux.git (push)
        backup ssh://git@github.com/manaflow-ai/bmux.git (fetch)
        mirror https://gitlab.com/manaflow-ai/bmux.git (fetch)
        """
        #expect(
            GitMetadataService.githubRepositorySlugs(fromGitRemoteVOutput: output)
                == ["manaflow-ai/bmux", "austinwang/bmux"]
        )
    }
    ...
    @Test func ignoresInlineComments() {
        ...
        [remote "origin"] ; user's main fork
            url = git@github.com:austinwang/bmux.git # main origin
            fetch = +refs/heads/*:refs/remotes/origin/*
        [remote "upstream"] # canonical repo
            url = https://github.com/manaflow-ai/bmux.git ; upstream source
            fetch = +refs/heads/*:refs/remotes/upstream/*
        """
        #expect(slugs(fromConfig: config) == ["manaflow-ai/bmux", "austinwang/bmux"])
    }
    ...
    @Test func usesLastRemoteURLValue() {
        ...
        """
        #expect(slugs(fromConfig: config) == ["manaflow-ai/bmux"])
    }
    ...
    @Test func appliesIncludesInPlace() throws {
        ...
        #expect(slugs(forDirectory: fixture.root.path) == ["manaflow-ai/bmux"])
    }
    ...
    @Test func treatsTrailingSlashGitdirAsRecursive() throws {
        ...
        let parent = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmuxgit-recursive-\(UUID().uuidString)", isDirectory: true)
        let repoRoot = parent.appendingPathComponent("teams/bmux", isDirectory: true)
        ...
        #expect(slugs(forDirectory: repoRoot.path) == ["manaflow-ai/bmux"])
    }
