...
@Suite struct MultiWindowRouterTests {
    ...
    private func makeScript(_ body: String) throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("BmuxIPCServiceTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        let url = directory.appendingPathComponent("fake-bmux")
    ...
    @Test func childEnvironmentIsExactlyTheInjectedOne() async throws {
        ...
        let script = try makeScript(#"printf '%s|%s' "$BMUX_TEST_MARKER" "${HOME:-unset}""#)
        let router = MultiWindowRouter(
            cliURL: script,
            socketPath: "/tmp/route.sock",
            environment: ["BMUX_TEST_MARKER": "marker-value"]
        )
        let result = try await router.route(arguments: [])
        #expect(result.terminationStatus == 0)
    ...
    @Test func throwsLaunchErrorPreservingUnderlyingDescription() async {
        let router = MultiWindowRouter(
            cliURL: URL(fileURLWithPath: "/nonexistent/bmux-cli-\(UUID().uuidString)"),
            socketPath: "/tmp/route.sock",
            environment: [:]
        )
        do {
