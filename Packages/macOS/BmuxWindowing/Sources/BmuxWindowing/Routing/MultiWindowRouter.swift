...
nonisolated private let logger = Logger(subsystem: "com.bmuxterm.app", category: "MultiWindowRouter")

/// Runs the bundled bmux CLI against the app's control socket to route a
...
/// that made `BmuxProcess.CommandRunner` a stateless struct). The legacy
...
public struct MultiWindowRouter: MultiWindowRouting, Sendable {
    ...
    /// - Parameters:
    ///   - cliURL: The bundled bmux CLI executable.
