...
The bmux control-socket domain: the listener and the transport/policy layer under the Unix-domain socket that external programs (the bmux CLI, agents, tests) use to drive the app.
...
#expect(server.start(socketPath: path, accessMode: .bmuxOnly))
...
Run with `swift test --package-path Packages/macOS/BmuxControlSocket`.
