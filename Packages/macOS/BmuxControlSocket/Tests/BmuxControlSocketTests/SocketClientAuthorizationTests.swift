import BmuxControlSocket
import Testing

@Suite("Socket client authorization")
struct SocketClientAuthorizationTests {
    private let authorization = SocketClientAuthorization()

    @Test func bmuxOnlyFailsClosedWhenPeerPidIsUnavailable() {
        #expect(!authorization.isBmuxOnlyClientAllowed(
            peerProcessID: nil,
            peerHasSameUID: true,
            isDescendant: { _ in true }
        ))
    }

    @Test func bmuxOnlyAllowsDescendantPeerPid() {
        #expect(authorization.isBmuxOnlyClientAllowed(
            peerProcessID: 123,
            peerHasSameUID: false,
            isDescendant: { $0 == 123 }
        ))
    }

    @Test func bmuxOnlyRejectsNonDescendantPeerPid() {
        #expect(!authorization.isBmuxOnlyClientAllowed(
            peerProcessID: 123,
            peerHasSameUID: true,
            isDescendant: { _ in false }
        ))
    }
}
