...
extension SocketTransport {
    ...
    /// reusable lock marker, based on the well-known bmux socket filenames
    ...
    func canReplaceUnmarkedRefusedSocket(at path: String) -> Bool {
        let filename = URL(fileURLWithPath: (path as NSString).standardizingPath)
            .lastPathComponent
            .lowercased()
        let recoverablePrefixes = ["bmux-debug-", "bmux-nightly-", "bmux-staging-"]
        return filename == "bmux-debug.sock" ||
            filename == "bmux-nightly.sock" ||
            filename == "bmux-staging.sock" ||
