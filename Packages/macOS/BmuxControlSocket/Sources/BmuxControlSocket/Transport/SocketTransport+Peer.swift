...
public extension SocketTransport {
    ...
    ///
    /// Used with ``isProcessDescendant(_:of:)`` to enforce the `bmuxOnly`
    ...
    ///     // refuse: client was not started inside bmux
    /// }
    /// ```
    ...
    ///
    /// Pairs with ``peerProcessID(of:)`` for the `bmuxOnly` ancestry check;
    /// see that symbol for a usage example.
    /// - Parameters:
    ///   - pid: The process to test.
    ///   - ancestorPid: The candidate ancestor (the bmux process).
