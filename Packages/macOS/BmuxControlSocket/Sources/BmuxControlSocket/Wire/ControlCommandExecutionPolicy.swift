...
public enum ControlCommandExecutionPolicy: Sendable, Equatable {
    ...
    ///
    /// `remotes.*` (the `bmux remotes` device-registry verbs) and
    ...
    static let socketWorkerMethods: Set<String> = [
        ...
        // (https://github.com/manaflow-ai/bmux/issues/5757), so it runs on
        ...
        // (bmux-zsh-integration.zsh `_bmux_report_tty_once`) still returns
    ...
    /// inline-collapsing hop each (bmuxTests drive workspace.set_auto_title
    ...
    static let mainThreadCallableSocketWorkerMethods: Set<String> = [
        ...
        // safe by construction — and bmuxTests drive them through
    ...
    /// In-process main-thread callers and bmuxTests exercise these verbs via
    /// `handleSocketLine` on the main actor.
    ///
    ...
    static let mainThreadCallableSocketWorkerV1Commands: Set<String> = [
        ...
        // send_workspace is REQUIRED to be callable: bmuxTests drive it
