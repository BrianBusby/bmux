...
/// Ring-buffer event log used by bmux debug builds.
///
...
public final class DebugEventLog: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "bmux.debug-event-log")
    ...
    private static func resolveLogPath() -> String {
        ...
        if let explicit = env["BMUX_DEBUG_LOG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !explicit.isEmpty {
            return explicit
        }

        if let tag = env["BMUX_TAG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !tag.isEmpty {
            return "/tmp/bmux-debug-\(sanitizePathToken(tag)).log"
        }

        if let socketPath = env["BMUX_SOCKET_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !socketPath.isEmpty {
            let socketBase = URL(fileURLWithPath: socketPath).deletingPathExtension().lastPathComponent
            if socketBase.hasPrefix("bmux-debug-") {
                return "/tmp/\(socketBase).log"
            }
        }

        if let bundleId = Bundle.main.bundleIdentifier,
           bundleId != "com.bmuxterm.app.debug" {
            return "/tmp/bmux-debug-\(sanitizePathToken(bundleId)).log"
        }
