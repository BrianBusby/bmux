import Foundation
import Testing
@testable import BmuxNotifications
...
struct NotificationClickPerformerTests {
    ...
    func selectsExistingFile() {
        ...
        finder.existingPaths = ["/tmp/bmux/report.txt"]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "/tmp/bmux/report.txt")))
        #expect(finder.selected == ["/tmp/bmux/report.txt"])
    ...
    func opensContainingDirectory() {
        ...
        finder.existingPaths = ["/tmp/bmux"]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "/tmp/bmux/missing.txt")))
        #expect(finder.selected.isEmpty)
        #expect(finder.opened == ["/tmp/bmux"])
    }
    ...
    func failsWhenNothingExists() {
        ...
        #expect(performer.perform(.revealInFinder(path: "/tmp/bmux/missing.txt")) == false)
        #expect(finder.selected.isEmpty)
        #expect(finder.opened.isEmpty)
    }
    ...
    func expandsTilde() {
        ...
        let expectedPath = expandedHome + "/bmux-file.txt"
        finder.existingPaths = [expectedPath]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "~/bmux-file.txt")))
