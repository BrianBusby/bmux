...
struct NotificationDeliveryCoordinatorTests {
    ...
    func configureInstallsCategoriesAndDelegate() throws {
        ...
        let fullPermission = try #require(categories["BMUXFeedPermissionOnceAlwaysAll"])
        ...
        let denyOnlyPermission = try #require(categories["BMUXFeedPermissionDeny"])
        #expect(denyOnlyPermission.actions.map(\.identifier) == ["feed.permission.deny"])

        let exitPlan = try #require(categories["BMUXFeedExitPlan"])
        ...
        let question = try #require(categories["BMUXFeedQuestion"])
    ...
    func feedPermissionAlwaysFallsBackToOnce() {
        ...
        coordinator.handle(NotificationDeliveryResponse(
            categoryIdentifier: "BMUXFeedPermissionOnceAlways",
    ...
    func feedPermissionUnsupportedModeDoesNotReply() {
        ...
        coordinator.handle(NotificationDeliveryResponse(
            categoryIdentifier: "BMUXFeedPermissionAll",
    ...
    func feedMissingRequestIdIsConsumed() {
        ...
        coordinator.handle(NotificationDeliveryResponse(
            categoryIdentifier: "BMUXFeedQuestion",
    ...
    func terminalDefaultClickActionPerformsAndMarksRead() {
        ...
        coordinator.handle(NotificationDeliveryResponse(
            ...
            userInfo: [
                ...
                "bmuxClickAction": "revealInFinder",
                "bmuxRevealInFinderPath": "/tmp/report.txt",
            ]
        ))
