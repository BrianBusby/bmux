import Testing
@testable import BmuxSwiftRender
...
@Suite struct SwiftViewInterpreterTests {
    ...
    @Test func capturesButtonBmuxActionWithNamedParams() {
        let node = interp.evaluate("""
        VStack {
            for i in 0..<2 {
                Button("select \\(i)") { bmux("workspace.select", workspace_id: "ws-\\(i)") }
            }
        }
        """)
        #expect(node?.children.count == 2)
        #expect(node?.children.first?.action?.commands == [.bmux(method: "workspace.select", params: ["workspace_id": "ws-0"])])
        #expect(node?.children.last?.action?.commands == [.bmux(method: "workspace.select", params: ["workspace_id": "ws-1"])])
    }
    ...
    @Test func labelFormButtonCapturesActionAndLabel() {
        let node = interp.evaluate("""
        VStack {
            Button(action: { bmux("workspace.select", workspace_id: "w-9") }) {
        ...
        #expect(button?.action?.commands == [.bmux(method: "workspace.select", params: ["workspace_id": "w-9"])])
    ...
    @Test func capturesOnTapGestureActionOnRichRow() {
        ...
        VStack {
            HStack { Text("●"); Text("home") }
                .onTapGesture { bmux("workspace.select", workspace_id: "abc-123") }
        }
        """)
        #expect(node?.children.first?.kind == .hstack)
        #expect(node?.children.first?.action?.commands == [.bmux(method: "workspace.select", params: ["workspace_id": "abc-123"])])
    }

    @Test func memberAccessOnObjectAndArray() {
        let data = SwiftValue.object(["name": .string("bmux"), "tabs": .array([.int(1), .int(2), .int(3)])])
        let node = interp.evaluate(#"VStack { Text("\(data.name): \(data.tabs.count)") }"#, state: ["data": data])
        #expect(node?.children.first?.text == "bmux: 3")
    }
    ...
    @Test func progressGaugeMenuAndContextMenu() {
        ...
        Text("row").contextMenu {
            Button("Open") { bmux("workspace.select") }
        }
        """)
    ...
    /// crashing the process (https://github.com/manaflow-ai/bmux/pull/5275
    /// review finding).
