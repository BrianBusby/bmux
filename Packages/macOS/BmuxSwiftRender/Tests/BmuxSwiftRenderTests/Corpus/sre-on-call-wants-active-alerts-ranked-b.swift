    ...
    VStack(spacing: 10) {
        ...
        if incident.active {
            VStack(spacing: 4) {
                ...
                Text(incident.title)
                    .font(.caption)
                HStack {
                    Button("Ack") { bmux("incident.ack", incident_id: incident.id) }
                        .padding(4)
                        .foregroundColor("#0A84FF")
                    Button("Bridge") { bmux("link.open", url: incident.bridgeUrl) }
                        .padding(4)
                        .foregroundColor("#0A84FF")
                    Spacer()
        ...
        ForEach(alerts) { a in
            VStack(spacing: 3) {
                ...
                HStack {
                    Button("Ack") { bmux("alert.ack", alert_id: a.id) }
                        .font(.caption)
                        .padding(3)
                        .foregroundColor("#0A84FF")
                    Button("Runbook") { bmux("link.open", url: a.runbookUrl) }
                        .font(.caption)
                        .padding(3)
                        .foregroundColor("#0A84FF")
                    Button("Logs") { bmux("sre.tail", service: a.service) }
                        .font(.caption)
                        .padding(3)
                        .foregroundColor("#0A84FF")
                    Spacer()
            ...
            .padding(6)
            .onTapGesture { bmux("sre.tail", service: a.service) }
            Divider()
        }
        ...
        ForEach(runbooks) { r in
            ...
            .padding(4)
            .onTapGesture { bmux("link.open", url: r.url) }
        }
    }
    .padding(8)
    ...
    VStack(spacing: 4) {
        HStack {
            ...
            Text("LOGS · \(tailService)")
                .font(.caption).fontWeight(.semibold)
            Spacer()
            Button(action: { bmux("sre.tail.terminal", service: tailService) }) {
                HStack {
