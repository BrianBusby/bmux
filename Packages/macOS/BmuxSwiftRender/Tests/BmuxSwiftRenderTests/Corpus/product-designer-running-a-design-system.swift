...
HSplitView(spacing: 0) {
  ...
  VStack(alignment: .leading, spacing: 14) {
      ...
      VStack(alignment: .leading, spacing: 6) {
        ...
        if ws.selected {
          ForEach(ws.tabs) { tab in
            ...
            .onTapGesture { bmux("surface.focus", surface_id: tab.id) }
          }
        }
      }
    }
    ...
    Divider()
    Button(action: { bmux("workspace.create", template: "design-export") }) {
      HStack(spacing: 6) {
  ...
  VStack(alignment: .leading, spacing: 16) {
    ...
    VStack(alignment: .leading, spacing: 8) {
      ForEach(palette) { c in
        ...
        .onTapGesture { bmux("clipboard.write", text: c["hex"]) } // clipboard verb -> missing
      }
    }
    ...
    VStack(alignment: .leading, spacing: 8) {
      ForEach(recentAssets) { a in
        ...
        .onTapGesture { bmux("file.reveal", path: "exports/\(a["file"])") }
      }
    }
    ...
    ForEach(spacing) { s in
      ...
      .onTapGesture { bmux("clipboard.write", text: s["v"]) }
    }
    ...
    Divider()
    Button(action: { bmux("surface.run", command: "npm run tokens:build") }) {
      HStack(spacing: 6) {
