                ...
                ForEach(Array(reviewable.enumerated()), id: \.offset) { idx, ws in
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        HStack(alignment: .top, spacing: 8) {
                            ...
                            // actions menu
                            Menu("") {
                                Button("Open PR #\(pr.number)") { bmux("openURL", param: pr.url) }
                                Button("Focus workspace") { bmux("selectWorkspace", param: ws.id) }
                                Button("Approve") { bmux("prApprove", param: pr.number) }
                                Button("Request changes") { bmux("prRequestChanges", param: pr.number) }
                                Button("Refresh PR") { bmux("prRefresh", param: pr.number) }
                                if pr.status == "approved" {
                                    Button("Merge") { bmux("prMerge", param: pr.number) }
                                }
                            }
                        }
                    ...
                    .contextMenu {
                        Button("Open PR #\(pr.number)") { bmux("openURL", param: pr.url) }
                        Button("Focus workspace") { bmux("selectWorkspace", param: ws.id) }
                        Button("Refresh") { bmux("prRefresh", param: pr.number) }
                    }
                    .onTapGesture { bmux("selectWorkspace", param: ws.id) }
