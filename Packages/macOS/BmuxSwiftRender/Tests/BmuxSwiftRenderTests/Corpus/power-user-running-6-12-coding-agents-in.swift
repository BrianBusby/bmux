HSplitView {
  VStack(alignment: .leading, spacing: 10) {
    ...
    ForEach(agents.flatMap { $0.approvals }) { req in
      VStack(alignment: .leading, spacing: 6) {
        ...
        HStack {
          Button { bmux("agent.approve", request_id: req.id); log("ok") } label: { Text("Approve") }.background("#1F2335").foregroundColor("#9ECE6A").cornerRadius(8).keyboardShortcut(.return, modifiers: [])
          Button { bmux("agent.deny", request_id: req.id) } label: { Text("Deny") }.foregroundColor("#F7768E")
        }
    ...
    ScrollView {
      VStack(alignment: .leading, spacing: 6) {
        ForEach(agents.sorted { $0.statusRank < $1.statusRank }) { a in
          ...
          }.padding(8).background(a.selected ? "#1F2335" : "#16161E").cornerRadius(8).onTapGesture { bmux("workspace.select", workspace_id: a.workspaceId) }.contextMenu { Button("Stop agent") { bmux("agent.stop", workspace_id: a.workspaceId) }; Button("Re-run") { bmux("agent.requeue", workspace_id: a.workspaceId) } }
        }
      }
    }
    Spacer()
  ...
  VStack(alignment: .leading, spacing: 10) {
    HStack { Text("Task Queue").font(.title2).bold(); Spacer(); Button { bmux("task.new") } label: { Image(systemName: "plus.circle.fill").foregroundColor("#7AA2F7") } }
    ForEach(["running", "queued", "done"]) { g in
      ...
      ForEach(items) { t in
        HStack {
          ...
          Spacer()
          if t.state == "running" { Button { bmux("task.stop", task_id: t.id) } label: { Image(systemName: "stop.fill").foregroundColor("#F7768E") } }
          else if t.state == "queued" { Button { bmux("task.promote", task_id: t.id) } label: { Image(systemName: "arrow.up.to.line").foregroundColor("#7AA2F7") } }
          else { Button { bmux("task.requeue", task_id: t.id) } label: { Image(systemName: "arrow.clockwise").foregroundColor("#7AA2F7") } }
        }.padding(8).background("#16161E").cornerRadius(8).opacity(t.state == "done" ? 0.55 : 1.0).onTapGesture { bmux("task.focus", task_id: t.id) }
      }
      Divider()
    }
    Spacer()
