// bmux pomodoro study sidebar — single view expression.
...
//   - tasks: each row is bound to one of my real bmux workspaces by index.
//
...
VStack(alignment: .leading, spacing: 14) {
    ...
    HStack(spacing: 8) {
      Image(systemName: t["done"] ? "checkmark.circle.fill" : "circle")
        ...
        .onTapGesture {
          ...
          bmux("workspace.select", workspace_id: workspaces[t["ws"]].id)
          log("toggled: \(t["title"])")
        }
      Text(t["title"])
        .font(.caption)
  ...
  HStack(spacing: 6) {
    ...
    Button(action: { bmux("pomodoro.toggle"); log("start/pause") }) {
      HStack(spacing: 4) {
