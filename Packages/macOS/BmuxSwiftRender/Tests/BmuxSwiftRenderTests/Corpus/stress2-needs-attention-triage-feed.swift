    ...
    ForEach(needsAttention) { ws in
      ...
      HStack(alignment: .top, spacing: 9) {
        ...
        VStack(alignment: .leading, spacing: 7) {
          ...
          HStack(spacing: 6) {
            Button { bmux("workspace.select", workspace_id: ws.id) } label: {
              HStack(spacing: 4) {
            ...
            Button { bmux("workspace.markRead", workspace_id: ws.id) } label: {
            ...
            if ws.pr.number > 0 {
              Menu("PR") {
                Button("Open #\(ws.pr.number)") { bmux("pr.open", url: ws.pr.url) }
                Button("Copy branch") { bmux("workspace.copyBranch", workspace_id: ws.id) }
              }
              .font(.system(size: 10))
            }
          }
      ...
      .onTapGesture { bmux("workspace.select", workspace_id: ws.id) }
      .contextMenu {
        Button("Triage") { bmux("workspace.select", workspace_id: ws.id) }
        Button("Mark read") { bmux("workspace.markRead", workspace_id: ws.id) }
        Button(ws.pinned ? "Unpin" : "Pin") { bmux("workspace.togglePin", workspace_id: ws.id) }
      }
    }
