...
func row(_ w, _ i) -> some View {
  ...
  .opacity(g == "R" ? 0.6 : 1.0)
  .onTapGesture { bmux("workspace.select", workspace_id: w.id) }
  .keyboardShortcut(.return)
  .contextMenu {
    Button("Select") { bmux("workspace.select", workspace_id: w.id) }
    Button("Stop agent") { bmux("workspace.stop", workspace_id: w.id) }
    Button(w.pinned == true ? "Unpin" : "Pin") { bmux("workspace.pin", workspace_id: w.id) }
    if w.pr != nil {
      Button("Open PR") { bmux("workspace.openPR", workspace_id: w.id) }
    }
  }
