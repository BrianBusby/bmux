    ...
    VStack(alignment: .leading, spacing: 14) {
                    ...
                    if let pr = w.pr {
                        ...
                        VStack(alignment: .leading, spacing: 9) {
                            ...
                            HStack(alignment: .top, spacing: 8) {
                                ...
                                Menu {
                                    Button("Open #\(pr.number)") { bmux("pr.open", url: pr.url) }
                                    Button("Focus workspace") { bmux("workspace.select", workspace_id: w.id) }
                                    Divider()
                                    switch pr.status {
                                    case "review_required":
                                        Button("Approve") { bmux("pr.approve", number: pr.number) }
                                        Button("Request changes") { bmux("pr.requestChanges", number: pr.number) }
                                    case "changes_requested":
                                        Button("Re-review") { bmux("pr.review", number: pr.number) }
                                    case "approved":
                                        Button("Merge") { bmux("pr.merge", number: pr.number) }
                                    default:
                                        Button("View checks") { bmux("pr.checks", number: pr.number) }
                                    }
                                    if pr.stale {
                                        Button("Refresh (stale)") { bmux("pr.refresh", number: pr.number) }
                                    }
                                } label: {
                        ...
                        .contextMenu {
                            Button("Open #\(pr.number)") { bmux("pr.open", url: pr.url) }
                            Button("Focus workspace") { bmux("workspace.select", workspace_id: w.id) }
                            Button("Refresh") { bmux("pr.refresh", number: pr.number) }
                        }
                        .onTapGesture {
                            bmux(pr.stale ? "pr.refresh" : "workspace.select", workspace_id: w.id)
                        }
        ...
        ViewThatFits {
            HStack(spacing: 10) {
                ...
                Spacer()
                Button {
                    bmux("pr.refreshAll", count: total)
                } label: {
                    HStack(spacing: 4) {
            ...
            HStack {
                Text("\(total) in queue")
                    ...
                    .foregroundColor(.secondary)
                Spacer()
                Button("Refresh") { bmux("pr.refreshAll", count: total) }
                    .font(.caption)
                    .fontWeight(.semibold)
            }
        }
