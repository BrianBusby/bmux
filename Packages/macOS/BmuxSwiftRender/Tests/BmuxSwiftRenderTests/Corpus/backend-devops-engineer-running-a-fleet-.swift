HSplitView {
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        ForEach(services) { svc in
            HStack(spacing: 8) {
                ...
                // restart
                Button(action: { bmux("service.restart", service_id: svc.id) }) {
                    Image(systemName: "arrow.clockwise")
                }
                // tail logs (focuses/opens the service's log surface)
                Button(action: { bmux("surface.focus", surface_id: svc.logSurfaceId) }) {
                    Image(systemName: "text.alignleft")
                }
            }
            .padding(4)
            .onTapGesture { bmux("service.open", service_id: svc.id) }
        }
        ...
        ForEach(ports) { p in
            HStack {
                ...
                Spacer()
                Button(action: { bmux("port.kill", port: p.port) }) {
        ...
        HStack(spacing: 8) {
            Button("Restart all") { bmux("service.restartAll") }
                .foregroundColor("#FF9F0A")
            Button("Stop all") { bmux("service.stopAll") }
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        HStack(spacing: 8) {
            ...
            Spacer()
            Button(action: { bmux("ci.rerun", run_id: build.runId) }) {
                Image(systemName: "arrow.clockwise")
            }
            Button(action: { bmux("ci.open", run_id: build.runId) }) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack {
                    ...
                    Button(d.env == "prod" ? "Ship prod" : "Deploy") {
                        bmux("deploy.start", env: d.env, sha: build.sha)
                    }
