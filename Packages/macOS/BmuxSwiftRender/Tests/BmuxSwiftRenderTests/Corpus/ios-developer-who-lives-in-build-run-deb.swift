HSplitView {
    ...
    VStack(alignment: .leading, spacing: 14) {
        ...
        HStack(spacing: 8) {
            ...
            Text(activeConfig)
                ...
                .onTapGesture {
                    ...
                    // to write back a build config; modeled as a bmux action.
                    bmux("ios.config.cycle", current: activeConfig)
                }
        }
        ...
        Text("Switch scheme")
            .font(.caption)
            .foregroundColor("#8E8E93")
        let schemes = ["bmux", "bmux DEV", "bmuxUITests", "BMUXMobileSyncCore"]
        ForEach(schemes) { scheme in
            ...
            .padding(4)
            .onTapGesture {
                bmux("ios.scheme.select", scheme: scheme)
            }
        }
            ...
            HStack(spacing: 8) {
                ...
                if sim["state"] != "Booted" {
                    Image(systemName: "play.circle.fill")
                        .foregroundColor("#0A84FF")
                        .onTapGesture {
                            bmux("ios.sim.boot", name: sim["name"], os: sim["os"])
                        }
                }
            }
            .padding(4)
            .onTapGesture {
                bmux("ios.destination.select", name: sim["name"], os: sim["os"], kind: "simulator")
            }
        }
        ...
        ForEach(devices) { dev in
            ...
            .onTapGesture {
                if dev["state"] == "Connected" {
                    bmux("ios.destination.select", name: dev["name"], os: dev["os"], kind: "device")
                } else {
    ...
    VStack(alignment: .leading, spacing: 14) {
        ...
        HStack(spacing: 8) {
            // Build
            Button(action: { bmux("ios.build", scheme: activeScheme, config: activeConfig) }) {
                VStack(spacing: 4) {
                    Image(systemName: "hammer.fill")
            ...
            Button(action: { bmux("ios.run", scheme: activeScheme, config: activeConfig) }) {
                VStack(spacing: 4) {
        ...
        HStack(spacing: 8) {
            Button(action: { bmux("ios.test", scheme: activeScheme) }) {
                VStack(spacing: 4) {
            ...
            Button(action: { bmux("ios.clean", scheme: activeScheme) }) {
                VStack(spacing: 4) {
                ...
                if ws.selected {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        ForEach(ws.tabs) { tab in
                            ...
                            .padding(4)
                            .onTapGesture {
                                bmux("surface.focus", surface_id: tab.id)
                            }
                        }
                    }
                }
        ...
        ForEach(recents) { r in
            ...
            .padding(3)
            .onTapGesture {
                bmux("ios.destination.select", name: r)
            }
        }
