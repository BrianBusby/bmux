HSplitView {
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        ForEach(workspaces) { ws in
            VStack(alignment: .leading, spacing: 4) {
                Button(action: {
                    bmux("workspace.select", workspace_id: ws.id)
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        ForEach(devServers) { srv in
            Button(action: { bmux("surface.focus", surface_id: srv.surfaceId) }) {
                HStack(spacing: 8) {
        ...
        ForEach(checks) { c in
            ...
            .padding(3)
            .onTapGesture { bmux("surface.focus", surface_id: c.surfaceId) }
        }
        Divider()
