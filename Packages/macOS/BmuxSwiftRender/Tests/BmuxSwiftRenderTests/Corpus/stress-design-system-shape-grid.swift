...
func swatch(_ hex: String, _ kind: Int, _ label: String, _ angle: Double) -> some View {
  ...
  .help("\(label) — \(hex)")
  .contextMenu {
    Button("Apply \(hex)") { bmux("noop", value: hex) }
    Button("Copy name") { bmux("noop", value: label) }
  }
}
  ...
  VStack(alignment: .leading) {
    ...
    Menu("Palette actions") {
      ForEach(Array(palette().prefix(4).enumerated()), id: \.offset) { i, hex in
        Button("Set accent \(shadeName(i))") { bmux("noop", value: hex) }
      }
      Button("Reset") { bmux("noop", value: "reset") }
    }
    .padding(.top, 4)
