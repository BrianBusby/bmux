...
func prPill(_ ws: Any) -> some View {
    return Group {
        if ws.pr != nil {
            ...
            .help("PR \(ws.pr.label)")
            .onTapGesture { bmux("openURL", param: ws.pr.url) }
        } else {
...
func railRow(_ ws: Any) -> some View {
    ...
    .contextMenu {
        Button("Focus") { bmux("workspace.focus", param: ws.id) }
        Button(ws.pinned ? "Unpin" : "Pin") { bmux("workspace.togglePin", param: ws.id) }
        Button("Mark read") { bmux("workspace.markRead", param: ws.id) }
    }
    .help(ws.directory)
    .onTapGesture { bmux("workspace.select", param: ws.id) }
}
...
func tabCard(_ tab: Any) -> some View {
    ...
    .contextMenu {
        Button("Focus tab") { bmux("tab.focus", param: tab.id) }
        Button(tab.pinned ? "Unpin tab" : "Pin tab") { bmux("tab.togglePin", param: tab.id) }
        Button("Close tab") { bmux("tab.close", param: tab.id) }
    }
    .onTapGesture { bmux("tab.focus", param: tab.id) }
}
...
func cockpitHeader() -> some View {
    return HStack(spacing: 8) {
        ...
        Spacer()
        Menu("•••") {
            Button("Collapse all reads") { bmux("workspace.markAllRead") }
            Button("New workspace") { bmux("workspace.create") }
            Section("Sort") {
                Button("By unread") { bmux("workspace.sort", param: "unread") }
                Button("By recent") { bmux("workspace.sort", param: "recent") }
            }
        }
        .foregroundColor(.secondary)
        Text(clockLabel())
    ...
    HSplitView {
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                Button("+") { bmux("tab.create", param: selectedId) }
                    .foregroundColor(.secondary)
