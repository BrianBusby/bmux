HSplitView {
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        .padding(4)
        .onTapGesture { bmux("writer.sprint.toggle", minutes: 25) }
        ...
        for ws in workspaces {
            if ws.selected {
                for tab in ws.tabs {
                    Button(action: { bmux("surface.focus", surface_id: tab.id) }) {
                        HStack(spacing: 6) {
                            if tab.focused {
        ...
        HStack(spacing: 8) {
            Button("＋ Section") { bmux("writer.insert", snippet: "## ") }
                .font(.caption).foregroundColor("#7AA2F7")
            Button("Frontmatter") { bmux("writer.insert", snippet: "frontmatter") }
                .font(.caption).foregroundColor("#7AA2F7")
        }
        .padding(4)
    }
    ...
    VStack(alignment: .leading, spacing: 12) {
        ...
        Button(action: { bmux("writer.format") }) {
            HStack(spacing: 6) {
        ...
        Button(action: { bmux("writer.preview") }) {
            HStack(spacing: 6) {
        ...
        Button(action: { bmux("writer.commit", message: "docs: writing session") }) {
            HStack(spacing: 6) {
        ...
        if lintClean {
            Button(action: { bmux("writer.publish") }) {
                HStack(spacing: 6) {
