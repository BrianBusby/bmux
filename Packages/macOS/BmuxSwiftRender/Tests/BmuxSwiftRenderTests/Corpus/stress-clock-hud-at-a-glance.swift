    ...
    if busy.count == 0 {
      ...
      ForEach(busy.prefix(6)) { w in
        ...
        HStack(spacing: 8) {
          ...
          Spacer()
          Button { bmux("workspace.markRead", workspace_id: w.id) } label: {
        ...
        .contextMenu {
          Button("Open") { bmux("workspace.select", workspace_id: w.id) }
          Button("Mark read") { bmux("workspace.markRead", workspace_id: w.id) }
        }
      }
    }
  }
