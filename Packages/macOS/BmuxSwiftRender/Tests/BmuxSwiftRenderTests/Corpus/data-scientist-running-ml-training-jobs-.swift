HSplitView {
  ...
  VStack(alignment: .leading, spacing: 10) {
    ...
    ForEach(gpus) { gpu in
      ...
      .onTapGesture { bmux("surface.focus", surface_id: gpu.ownerSurfaceId) }
    }
  ...
  VStack(alignment: .leading, spacing: 10) {
    ...
    if runs.isEmpty {
      ...
      ForEach(runs) { run in
        ...
        .padding(6)
        .background(RoundedRectangle(cornerRadius: 8).fill("#161a22"))
        .onTapGesture { bmux("surface.focus", surface_id: run.surfaceId) }
      }
    }
    ...
    ForEach(datasets) { ds in
      Button(action: {
        bmux("workspace.select", workspace_id: ds.workspaceId)
        bmux("surface.run", surface_id: ds.surfaceId, command: ds.mountCommand)
      }) {
        HStack(spacing: 8) {
