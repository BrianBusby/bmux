HSplitView {
    ...
    VStack(alignment: .leading, spacing: 10) {
        ...
        Button(action: { bmux("review.refresh") }) {
            HStack(spacing: 6) {
        ...
        if redCount > 0 {
            ...
            ForEach(reviewQueue) { pr in
                if pr.ciState == "red" {
                    Button(action: { bmux("workspace.select", workspace_id: pr.workspaceId) }) {
        ...
        if reviewCount > 0 {
            ...
            ForEach(reviewQueue) { pr in
                if pr.needsMyReview && pr.ciState != "red" {
                    Button(action: { bmux("pr.open", url: pr.url) }) {
                        VStack(alignment: .leading, spacing: 2) {
                            HStack(spacing: 6) {
                                Text("#\(pr.number)")
                                    .font(.caption)
                ...
                if pr.mergeable {
                    HStack(spacing: 6) {
                        Text("#\(pr.number) \(pr.title)")
                            .font(.caption)
                        Spacer()
                        Button(action: { bmux("pr.merge", url: pr.url) }) {
                            Text("Merge")
                                .font(.caption)
                                .bold()
                                .foregroundColor("#46D17F")
    ...
    VStack(alignment: .leading, spacing: 8) {
        ...
        ForEach(workspaces) { w in
            VStack(alignment: .leading, spacing: 3) {
                ...
                if w.selected {
                    ForEach(w.tabs) { t in
                        Button(action: { bmux("surface.focus", surface_id: t.id) }) {
                            HStack(spacing: 6) {
                                if t.focused {
