...
///
/// - **Forward** — every bmux.json path a `SettingsCardRow` declares via
...
/// a bmux.json path (pickers and action buttons that don't write a
...
struct SettingsRowAnchorResolutionTests {
    /// Every singular bmux.json path declared by an *unconditionally
    ...
    /// (no single bmux.json path): pickers and action buttons. Each must
