// Tests/BmuxSettingsUITests/ShortcutListModelTests.swift
import Foundation
import Testing
import BmuxSettings
...
@Suite struct ShortcutListModelTests {
    ...
    private func makeStore() -> (JSONConfigStore, SettingCatalog, SettingsErrorLog) {
        ...
        let fileURL = tempDir.appendingPathComponent("bmux.json")
    ...
    @Test func failedWriteRollsBackOptimisticBinding() async throws {
        ...
        // JSON store rejected; corrupt or unwritable bmux.json should surface
        ...
        let store = JSONConfigStore(fileURL: blockedParent.appendingPathComponent("bmux.json"))
    ...
    @Test func externalChangePrunesStaleConflictRejection() async throws {
        // WHY: a user editing bmux.json externally to remove a conflicting binding
    ...
    @Test func externalConflictMoveRefreshesConflictTarget() async throws {
        // WHY: if bmux.json moves the rejected shortcut to a different action,
