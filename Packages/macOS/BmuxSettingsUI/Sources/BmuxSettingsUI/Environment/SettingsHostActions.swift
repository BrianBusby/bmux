...
/// opening the user's editor on bmux.json, sending feedback, posting
...
public protocol SettingsHostActions: AnyObject {
    ...
    /// Opens the bmux JSON config file in the user's preferred
    ...
    /// Opens System Settings to the bmux notifications pane so the
    /// user can grant / revoke OS-level notification permission.
    func openSystemNotificationSettings()

    /// Restarts the bmux app. Used after the user changes the
    ...
    /// Opens the bmux terminal config preview window (the legacy
