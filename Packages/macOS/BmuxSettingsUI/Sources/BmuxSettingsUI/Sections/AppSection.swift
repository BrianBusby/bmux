...
/// bmux, Terminal Config link, Open Markdown in bmux Viewer,
...
public struct AppSection: View {
    ...
    public init(
        ...
        _openSupported = State(initialValue: DefaultsValueModel(store: defaultsStore, key: catalog.app.openSupportedFilesInBmux))
        _openMarkdown = State(initialValue: DefaultsValueModel(store: defaultsStore, key: catalog.app.openMarkdownInBmuxViewer))
    ...
    /// Languages legacy `AppLanguage` exposes (bmuxApp.swift line
    /// 4338). The shared `BmuxSettings.AppLanguage` adds `.vi` for a
    ...
    private var globalFontMagnificationSubtitle: String {
        ...
        return String(
            ...
            defaultValue: "Scale every font in bmux by the same percentage. 100% = design size."
        )
    }
        ...
        SettingsCard {
            ...
            SettingsCardRow(
                ...
                subtitle: languageAtAppear != nil && language.current != languageAtAppear
                    ? String(localized: "settings.app.language.restartSubtitle", defaultValue: "Restart bmux to apply")
                    : nil,
            ...
            SettingsCardRow(
                ...
                subtitle: firstClick.current
                    ? String(localized: "settings.app.paneFirstClickFocus.subtitleOn", defaultValue: "When bmux is inactive, clicking a pane activates the window and focuses that pane in one click.")
                    : String(localized: "settings.app.paneFirstClickFocus.subtitleOff", defaultValue: "When bmux is inactive, the first click only activates the window. Click again to focus the pane.")
            ) {
            ...
            // Open Supported Files in bmux
            SettingsCardRow(
                configurationReview: .json("app.openSupportedFilesInBmux"),
                String(localized: "settings.app.openSupportedFilesInBmux", defaultValue: "Open Supported Files in bmux"),
                subtitle: String(localized: "settings.app.openSupportedFilesInBmux.subtitle", defaultValue: "Cmd-clicking readable files opens text, code, PDFs, images, audio, video, and Quick Look previews in bmux.")
            ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.app.configWindow.subtitle", defaultValue: "Open the bmux terminal config and generated preview in one utility window."),
                controlWidth: Self.columnWidth
            ) {
            ...
            // Open Markdown in bmux Viewer
            SettingsCardRow(
                configurationReview: .json("app.openMarkdownInBmuxViewer"),
                String(localized: "settings.app.openMarkdownInBmuxViewer", defaultValue: "Open Markdown in bmux Viewer"),
                subtitle: String(localized: "settings.app.openMarkdownInBmuxViewer.subtitle", defaultValue: "When supported file routing is on, Cmd-clicking Markdown files opens the rendered bmux markdown viewer instead of the generic file preview.")
            ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.app.menuBarOnly.subtitle", defaultValue: "Hide the Dock icon and Cmd+Tab entry. Use the menu bar item to show bmux.")
            ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.app.showInMenuBar.subtitle", defaultValue: "Keep bmux in the menu bar for unread notifications and quick actions.")
            ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash a blue outline when bmux highlights a pane.")
            ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.notifications.command.subtitle", defaultValue: "Run a shell command when a notification arrives. $BMUX_NOTIFICATION_TITLE, $BMUX_NOTIFICATION_SUBTITLE, $BMUX_NOTIFICATION_BODY are set.")
            ) {
                TextField(
            ...
            SettingsCardRow(
                ...
                subtitle: (telemetryAtAppear != nil && telemetry.current != telemetryAtAppear)
                    ...
                    : String(localized: "settings.app.telemetry.subtitle", defaultValue: "Share anonymized crash and usage data to help improve bmux.")
            ) {
    ...
    /// Standard macOS notification sound names plus bmux-specific
