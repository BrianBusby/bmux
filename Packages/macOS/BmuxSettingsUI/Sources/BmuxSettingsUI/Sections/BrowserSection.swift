...
/// row-for-row inside a single `SettingsCard`: Enable bmux Browser,
...
public struct BrowserSection: View {
    ...
    public init(
        ...
        _openTermLinks = State(initialValue: DefaultsValueModel(store: defaultsStore, key: catalog.browser.openTerminalLinksInBmuxBrowser))
        _interceptOpen = State(initialValue: DefaultsValueModel(store: defaultsStore, key: catalog.browser.interceptTerminalOpenCommandInBmuxBrowser))
    ...
    private var mainCard: some View {
        SettingsCard {
            ...
            SettingsCardRow(
                ...
                String(localized: "settings.browser.enabled", defaultValue: "Enable bmux Browser"),
            ...
            if engine.current == .custom {
                ...
                SettingsCardRow(
                    ...
                    subtitle: String(localized: "settings.browser.customSearchEngineURLTemplate.subtitle", defaultValue: "Use {query} or %s for the search terms. Without a placeholder, bmux appends q=."),
                    controlWidth: 330
                ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.browser.hiddenWebViewDiscardDelay.subtitle", defaultValue: "How long a browser tab must stay hidden before bmux frees its page memory. Active downloads, popups, developer tools, fullscreen, and loading pages are skipped."),
                controlWidth: Self.columnWidth
            ) {
                HStack(spacing: 8) {
            ...
            // Open Terminal Links
            SettingsCardRow(
                configurationReview: .json("browser.openTerminalLinksInBmuxBrowser"),
                String(localized: "settings.browser.openTerminalLinks", defaultValue: "Open Terminal Links in bmux Browser"),
            ...
            // Intercept open
            SettingsCardRow(
                configurationReview: .json("browser.interceptTerminalOpenCommandInBmuxBrowser"),
            ...
            if openTermLinks.current || interceptOpen.current {
                ...
                hostnameEditor(
                    ...
                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in bmux. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in bmux."),
    ...
    private func httpAllowlistRow(model: DefaultsValueModel<String>) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in bmux without a warning prompt. Defaults include localhost, *.localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me."))
                .bmuxFont(.caption)
