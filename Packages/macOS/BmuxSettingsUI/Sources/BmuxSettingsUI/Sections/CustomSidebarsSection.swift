    ...
    public var body: some View {
        Group {
            ...
            SettingsCard {
                ...
                SettingsCardDivider()
                SettingsCardNote(
                    String(localized: "settings.customSidebars.note", defaultValue: "Custom sidebars are SwiftUI-style files in ~/.config/bmux/sidebars. Pick one from the sidebar toggle button's right-click menu; edits hot-reload on save. Use the in-app renderer only for sidebars you trust.")
                )
            }
        }
    ...
    private var enabledRow: some View {
        SettingsCardRow(
            ...
            subtitle: enabled.current
                ? String(localized: "settings.customSidebars.enabled.subtitleOn", defaultValue: "Lists your sidebars from ~/.config/bmux/sidebars in the sidebar picker.")
