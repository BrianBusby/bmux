...
public struct AutomationSection: View {
    ...
    private var socketControlCard: some View {
        ...
        SettingsCard {
            ...
            SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: BMUX_SOCKET_ENABLE, BMUX_SOCKET_MODE, and BMUX_SOCKET_PATH (set BMUX_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: claudeCodeModel.current
                    ...
                    : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without bmux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.claudeCode.note", defaultValue: "When enabled, bmux wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: codexModel.current
                    ...
                    : String(localized: "settings.automation.codex.subtitleOff", defaultValue: "Codex runs without bmux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.codex.note", defaultValue: "When enabled, bmux wraps the codex command to inject session tracking and notification hooks. Disable if you prefer to manage Codex hooks yourself. bmux still tracks live Codex sessions it can observe even when this is off. To also track Codex launched through a custom launcher that bypasses the wrapper (e.g. a subrouter), run `bmux hooks setup --agent codex`, which installs hooks into ~/.codex/hooks.json."))
        }
    }
    ...
    private var autoNamingCard: some View {
        SettingsCard {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.workspaceAutoNaming.note", defaultValue: "When enabled, bmux summarizes supported agent sessions into short workspace and tab names using each agent's own binary, refreshed as the topic shifts. Manual renames always win and stop auto-naming for that workspace or tab. Uses your agent account for the short summarization calls."))
            if autoNamingModel.current,
               !claudeCodeModel.current,
               autoNamingAgentModel.current == AutoNamingAgentCatalog.autoSlug || autoNamingAgentModel.current == "claude" {
                autoNamingFootnote(String(localized: "settings.automation.workspaceAutoNaming.hooksOffWarning", defaultValue: "Claude Code Integration is off, so Claude sessions will not be auto-named. Other supported agents still name when their bmux hooks are installed."))
            }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: ampModel.current
                    ...
                    : String(localized: "settings.automation.amp.subtitleOff", defaultValue: "Amp runs without bmux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.amp.note", defaultValue: "Hooks must be installed with `bmux hooks amp install`. They no-op outside bmux terminals. When disabled, the installed Amp plugin stays inactive without needing to be removed."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: cursorModel.current
                    ...
                    : String(localized: "settings.automation.cursor.subtitleOff", defaultValue: "Cursor runs without bmux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.cursor.note", defaultValue: "Hooks must be installed with `bmux hooks cursor install`. They no-op outside bmux terminals."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: geminiModel.current
                    ...
                    : String(localized: "settings.automation.gemini.subtitleOff", defaultValue: "Gemini runs without bmux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.gemini.note", defaultValue: "Hooks must be installed with `bmux hooks gemini install`. They no-op outside bmux terminals."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: kiroModel.current
                    ...
                    : String(localized: "settings.automation.kiro.subtitleOff", defaultValue: "Kiro runs without bmux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.kiro.note", defaultValue: "Hooks must be installed with `bmux hooks kiro install`, then run Kiro with `kiro-cli chat --agent bmux` (or set it as your default agent). They no-op outside bmux terminals."))
        }
    }
    ...
    private var portCard: some View {
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for BMUX_PORT env var."),
                controlWidth: Self.columnWidth
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets BMUX_PORT and BMUX_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
        }
    }
