    ...
    private var customSidebarsRow: some View {
        SettingsCardRow(
            ...
            subtitle: customSidebars.current
                ? String(localized: "settings.betaFeatures.customSidebars.subtitleOn", defaultValue: "Lists your sidebars from ~/.config/bmux/sidebars in the sidebar picker, rendered in an isolated helper process.")
    ...
    private var remoteTmuxRow: some View {
        SettingsCardRow(
            ...
            subtitle: remoteTmux.current
                ? String(localized: "settings.betaFeatures.remoteTmux.subtitleOn", defaultValue: "Mirrors a remote host's tmux sessions in the sidebar over ssh tmux -CC; sessions become workspaces and windows become tabs. Quitting bmux leaves the remote tmux server running.")
