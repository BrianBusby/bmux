...
/// **bmux.json** section — mirrors the legacy in-app section: a card
...
public struct SettingsJSONSection: View {
    ...
    public var body: some View {
        Group {
            SettingsSectionHeader(String(localized: "settings.section.settingsJSON", defaultValue: "bmux.json"), section: .settingsJSON)
    ...
    private var userConfigFileRow: some View {
        SettingsCardRow(
            ...
            subtitle: String(localized: "settings.settingsJSON.file.subtitle", defaultValue: "Edit bmux-owned app settings, shortcuts, automation, sidebar, notifications, and browser behavior."),
            controlWidth: 330
        ) {
            HStack(spacing: 8) {
                Text(displayPath)
    ...
    private var documentationRow: some View {
        SettingsCardRow(
            ...
            Link(
                ...
                destination: URL(string: "https://bmux.com/docs/configuration#bmux-json")!
            )
            .bmuxFont(.caption)
