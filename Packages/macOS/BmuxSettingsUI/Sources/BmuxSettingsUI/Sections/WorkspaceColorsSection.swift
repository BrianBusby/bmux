...
public struct WorkspaceColorsSection: View {
    ...
    private var mainCard: some View {
        SettingsCard {
            ...
            SettingsCardNote(
                String(localized: "settings.workspaceColors.dictionaryNote", defaultValue: "Edit bmux.json to add or remove named colors. \"Choose Custom Color...\" still adds local Custom N entries.")
            )

            let entries = effectivePaletteEntries(overrides: paletteModel.current)
            if entries.isEmpty {
                SettingsCardNote(
                    String(localized: "settings.workspaceColors.emptyPalette", defaultValue: "No palette entries. Add colors in bmux.json or use \"Choose Custom Color...\" from a workspace context menu.")
                )
            } else {
        ...
        SettingsCardRow(
            ...
            HStack(spacing: 8) {
                ...
                Text(isCustom ? model.current : String(localized: "settings.sidebarAppearance.defaultLabel", defaultValue: "Default"))
                    .bmuxFont(size: 12, weight: .medium, design: .monospaced)
    ...
    /// bmux-themed accent color used as the live ColorPicker fallback
    /// when the selection or notification badge has no custom hex.
    /// Mirrors the legacy `bmuxAccentColor()` helper (see
    ...
    private static func bmuxAccentColor() -> Color {
