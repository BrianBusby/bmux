...
/// localized row text, the bmux.json paths owned by that row, and a
...
/// `[CuratedSettingEntry].bmuxDefault` constant.
public struct CuratedSettingEntry: Sendable, Hashable {
    ...
    /// Dotted bmux.json paths that should scroll to this search entry's
    ...
    /// Dotted bmux.json path used as the scroll/highlight anchor.
    ///
