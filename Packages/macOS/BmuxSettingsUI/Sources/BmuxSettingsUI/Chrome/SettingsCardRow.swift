...
/// underlying bmux.json path next to the row when the host enables
/// "show config paths".
///
...
public struct SettingsCardRow<Trailing: View>: View {
    ...
    // map the bmux.json path(s) it declares via `configurationReview`
    ...
    /// custom-control rows that don't write a single bmux.json key);
