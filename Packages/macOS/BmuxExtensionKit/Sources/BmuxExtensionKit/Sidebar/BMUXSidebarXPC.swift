...
@_spi(BmuxHostTransport) @objc public protocol BMUXSidebarHostXPC: NSObjectProtocol {
...
@_spi(BmuxHostTransport) @objc public protocol BMUXSidebarExtensionXPC: NSObjectProtocol {
...
public enum BmuxSidebarXPCCodec {
    ...
    public static func encodeSnapshot(_ snapshot: BmuxSidebarSnapshot) throws -> NSData {
        try encode(snapshot, kind: "snapshot", maximumBytes: maximumSnapshotPayloadBytes)
    }

    public static func decodeSnapshot(_ payload: NSData) throws -> BmuxSidebarSnapshot {
        try validatePayloadSize(payload, kind: "snapshot", maximumBytes: maximumSnapshotPayloadBytes)
        return try JSONDecoder().decode(BmuxSidebarSnapshot.self, from: payload as Data)
    }

    public static func encodeManifest(_ manifest: BmuxExtensionManifest) throws -> NSData {
        try encode(manifest, kind: "manifest", maximumBytes: maximumManifestPayloadBytes)
    }

    public static func decodeManifest(_ payload: NSData) throws -> BmuxExtensionManifest {
        try validatePayloadSize(payload, kind: "manifest", maximumBytes: maximumManifestPayloadBytes)
        return try JSONDecoder().decode(BmuxExtensionManifest.self, from: payload as Data)
    }

    public static func encodeAction(_ action: BmuxSidebarAction) throws -> NSData {
        try encode(action, kind: "action", maximumBytes: maximumActionPayloadBytes)
    }

    public static func decodeAction(_ payload: NSData) throws -> BmuxSidebarAction {
        try validatePayloadSize(payload, kind: "action", maximumBytes: maximumActionPayloadBytes)
        return try JSONDecoder().decode(BmuxSidebarAction.self, from: payload as Data)
    }

    public static func encodeActionResult(_ result: BmuxSidebarActionResult) throws -> NSData {
        try encode(result, kind: "actionResult", maximumBytes: maximumActionResultPayloadBytes)
    }

    public static func decodeActionResult(_ payload: NSData) throws -> BmuxSidebarActionResult {
        try validatePayloadSize(payload, kind: "actionResult", maximumBytes: maximumActionResultPayloadBytes)
        return try JSONDecoder().decode(BmuxSidebarActionResult.self, from: payload as Data)
    }
    ...
    private static func validatePayloadSize(_ payload: NSData, kind: String, maximumBytes: Int) throws {
        guard payload.length <= maximumBytes else {
            throw BmuxExtensionValidationError.payloadTooLarge(
                kind: kind,
