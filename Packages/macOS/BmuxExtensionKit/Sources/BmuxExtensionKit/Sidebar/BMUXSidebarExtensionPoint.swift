...
/// Public identifiers for the BMUX sidebar ExtensionKit surface.
@_spi(BmuxHostTransport)
public enum BmuxSidebarExtensionPoint {
    ...
    /// per build tag (e.g. `com.bmuxterm.app.debug.my-tag.bmux.sidebar`) so that concurrent debug
    ...
    /// committed to source.
    public static let baseIdentifier = "com.bmuxterm.app.bmux.sidebar"

    /// Info.plist key a bundle may declare to override the extension point identifier.
    ///
    /// Populated at build time from the `BMUX_SIDEBAR_EXTENSION_POINT_ID` build setting
    ...
    public static let identifierInfoPlistKey = "BMUXSidebarExtensionPointIdentifier"
    ...
    /// Default ExtensionKit scene identifier hosted inside the bmux sidebar.
