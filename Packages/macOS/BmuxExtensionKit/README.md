...
`BmuxExtensionKit` is the zero-dependency public SDK for BMUX sidebar extensions.
...
Sidebar extensions are ExtensionKit app extensions. `BmuxExtensionKit` and the reference projects target macOS 14+, matching BMUX.
...
4. Keep the extension point identifier as `com.bmuxterm.app.bmux.sidebar`.
5. Build and launch the containing app once so macOS registers the embedded extension.
6. In BMUX, open Sidebar Extensions from the puzzle button next to the sidebar help button and enable your extension.
...
<dict>
  ...
  <string>com.bmuxterm.app.bmux.sidebar</string>
</dict>
```

Define your ExtensionKit entrypoint by conforming directly to `BmuxSidebarExtension`.
...
@Observable
@MainActor
final class ExampleSidebarExtension: BmuxSidebarExtension {
...
`BmuxSidebarExtension` is the public extension protocol. It refines `AppExtension`,
requires a manifest and SwiftUI `body`, and delivers `BmuxSidebarContext`, which
contains the filtered `BmuxSidebarSnapshot` and a typed `BmuxSidebarHost` command
channel.

The lower-level transport lives behind BMUX host SPI. New sidebar extensions should
conform to `BmuxSidebarExtension` and should not handle XPC directly.
...
host-side callbacks are SPI for BMUX's own host implementation.
Creating or splitting a browser surface with a URL requires both the surface
action scope and `openURL`.

## Permissions

List every scope and action your extension needs in its manifest. BMUX filters the
...
If your extension does not appear, confirm the containing app has been launched, the embedded appex is signed by your team, the extension point identifier is unchanged, and BMUX's Sidebar Extensions browser shows the extension as enabled.
