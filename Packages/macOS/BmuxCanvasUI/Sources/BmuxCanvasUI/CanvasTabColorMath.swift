...
extension NSColor {
    ...
    func bmuxCanvasLighten(by amount: CGFloat) -> NSColor {
        blended(withFraction: amount, of: .white) ?? self
    }

    /// Darkens toward black by `amount` (0...1).
    func bmuxCanvasDarken(by amount: CGFloat) -> NSColor {
        blended(withFraction: amount, of: .black) ?? self
    }

    /// The active (selected) tab fill for a bar of this background color.
    var bmuxCanvasActiveTabFill: NSColor {
        bmuxCanvasIsLight ? bmuxCanvasDarken(by: 0.065) : bmuxCanvasLighten(by: 0.12)
    }

    /// The hovered (unselected) tab fill for a bar of this background color.
    var bmuxCanvasHoverTabFill: NSColor {
        let adjusted = bmuxCanvasIsLight ? bmuxCanvasDarken(by: 0.03) : bmuxCanvasLighten(by: 0.07)
