...
public struct WorkspaceRemoteConfiguration: Equatable, Sendable {
    ...
    /// CLI relay port for remote `bmux` command forwarding.
    ...
    /// Provider-issued Cloud VM id for bmux-managed Cloud VM workspaces.
    ...
    /// True for cloud-VM remotes (Freestyle snapshots) where bmuxd-remote is pre-baked in
    ...
    /// the baked daemon through an SSH local forward to `/run/bmuxd-remote.sock`.
