...
struct WorkspaceRemoteConfigurationSSHBatchCommandsTests {
    ...
    func daemonTransportArgumentsWithoutSlot() {
        let arguments = configuration().daemonTransportArguments(remotePath: "/remote/bmuxd-remote")
        let expectedCommand = #"sh -c 'exec '"'"'/remote/bmuxd-remote'"'"' '"'"'serve'"'"' '"'"'--stdio'"'"''"#
        #expect(
            arguments == ["-T", "-o", "RemoteCommand=none"]
                + expectedBatchArguments
                + ["-o", "RequestTTY=no", "bmux-macmini", expectedCommand]
        )
    }
    ...
    func daemonTransportArgumentsWithSlot() {
        ...
        ).daemonTransportArguments(remotePath: "/remote/bmuxd-remote")
        let expectedCommand = #"sh -c 'exec '"'"'/remote/bmuxd-remote'"'"' '"'"'serve'"'"' '"'"'--stdio'"'"' '"'"'--persistent'"'"' '"'"'--slot'"'"' '"'"'ws-1'"'"''"#
        #expect(
            arguments == ["-T", "-o", "RemoteCommand=none"]
                + expectedBatchArguments
                + ["-o", "RequestTTY=no", "bmux-macmini", expectedCommand]
    ...
    func daemonTransportArgumentsOverrideHostConfiguredRemoteCommand() {
        ...
        let destinationIndex = arguments.firstIndex(of: "bmux-macmini")
        ...
        #expect(
            arguments == ["-N", "-T", "-S", "none"]
                ...
                + [
                    ...
                    "-o", "RequestTTY=no",
                    "-L", "127.0.0.1:64123:/run/bmuxd-remote.sock",
                    "bmux-macmini",
                ]
        )
    }
    ...
    func reverseRelayControlMasterArguments() throws {
        ...
        #expect(
            arguments == expectedBatchArguments
                + ["-O", "forward", "-R", "127.0.0.1:64007:127.0.0.1:54321", "bmux-macmini"]
        )
    }
    ...
    func reverseRelayControlMasterCancelArguments() throws {
        ...
        #expect(
            arguments == expectedBatchArguments
                + ["-O", "cancel", "-R", "127.0.0.1:64007", "bmux-macmini"]
        )
    }
