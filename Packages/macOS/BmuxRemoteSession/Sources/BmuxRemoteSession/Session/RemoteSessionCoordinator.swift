...
public import BmuxRemoteWorkspace
...
/// orchestration, bmuxd-remote bootstrap and install, the reverse CLI relay,
...
public final class RemoteSessionCoordinator: @unchecked Sendable {
    ...
    let queue = DispatchQueue(label: "com.bmux.remote-ssh.\(UUID().uuidString)", qos: .utility)
    ...
    ///   - manifestRepository: bmuxd-remote manifest/binary-cache repository.
    ...
    /// The capabilities advertised by the bmuxd-remote baked into the Freestyle snapshot
    ...
    static func bakedVMDaemonHello() -> DaemonHello {
        DaemonHello(
            ...
            remotePath: "/usr/local/bin/bmuxd-remote"
        )
    }
    ...
    func beginConnectionAttemptLocked() {
        ...
        do {
            ...
            if configuration.skipDaemonBootstrap {
                // Cloud-VM path: bmuxd-remote is pre-baked in the image and exposed via
                // systemd socket activation at /run/bmuxd-remote.sock. We skip the probe,
            ...
            guard missingCapabilities.isEmpty else {
                throw NSError(domain: "bmux.remote.daemon", code: 43, userInfo: [
