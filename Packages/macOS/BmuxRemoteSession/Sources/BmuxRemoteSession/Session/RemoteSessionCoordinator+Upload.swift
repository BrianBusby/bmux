...
// composition, the `/tmp/bmux-drop-<uuid>` path shape, cleanup script text,
...
extension RemoteSessionCoordinator {
    /// Uploads dropped local files to `/tmp/bmux-drop-*` paths on the remote
    ...
    /// The `/tmp/bmux-drop-<uuid>[.ext]` remote path a dropped local file
    ...
    public static func remoteDropPath(for fileURL: URL, uuid: UUID = UUID()) -> String {
        ...
        return "/tmp/bmux-drop-\(uuid.uuidString.lowercased())\(lowercasedSuffix)"
    }
