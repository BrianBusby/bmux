...
extension RemoteSessionCoordinator {
    ...
    private func scanRemotePortsByPanelLocked(ttyNamesByPanel: [UUID: String]) throws -> [UUID: [Int]] {
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "bmux.remote.ports", code: 90, userInfo: [
    ...
    private func shouldUseFallbackRemotePortPollingLocked() -> Bool {
        // `bmux ssh` owns the remote shell bootstrap and can report the remote
    ...
    private func shouldUseTTYFallbackRemotePortPollingLocked() -> Bool {
        // `bmux ssh` can still land in shells without our command hooks, such as
    ...
    static func remotePortScanScript(ttyNames: [String], excluding ports: Set<Int>) -> String {
        ...
        return """
        set -eu
        bmux_tracked_ttys=" \(ttySet) "
        bmux_tty_csv='\(ttyCSV)'
        bmux_excluded_ports=" \(excludedPorts) "

        bmux_emit_port() {
          bmux_tty="$1"
          bmux_port="$2"
          case "$bmux_tracked_ttys" in
            *" $bmux_tty "*) ;;
            *) return 0 ;;
          esac
          case "$bmux_excluded_ports" in
            *" $bmux_port "*) return 0 ;;
          esac
          [ "$bmux_port" -ge 1024 ] && [ "$bmux_port" -le 65535 ] || return 0
          printf '%s\\t%s\\n' "$bmux_tty" "$bmux_port"
        }

        bmux_used_ss=0
        if [ -d /proc ] && command -v ss >/dev/null 2>&1; then
          bmux_ss_output="$(ss -ltnpH 2>/dev/null || true)"
          case "$bmux_ss_output" in
            *pid=*)
              bmux_used_ss=1
              printf '%s\\n' "$bmux_ss_output" | while IFS= read -r bmux_line; do
                [ -n "$bmux_line" ] || continue
                bmux_port="$(printf '%s\\n' "$bmux_line" | awk '{print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ { print $1; exit }')"
                [ -n "$bmux_port" ] || continue
                printf '%s\\n' "$bmux_line" | awk '
                  {
                    line = $0
                ...
                ' | while IFS= read -r bmux_pid; do
                  [ -n "$bmux_pid" ] || continue
                  bmux_tty_path="$(readlink "/proc/$bmux_pid/fd/0" 2>/dev/null || true)"
                  [ -n "$bmux_tty_path" ] || continue
                  bmux_tty="${bmux_tty_path##*/}"
                  [ -n "$bmux_tty" ] || continue
                  bmux_emit_port "$bmux_tty" "$bmux_port"
                done
              done
              ;;
          esac
        ...
        if [ "$bmux_used_ss" -eq 0 ] && command -v lsof >/dev/null 2>&1 && [ -n "$bmux_tty_csv" ]; then
          bmux_tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t bmux-ports)"
          trap 'rm -rf "$bmux_tmpdir"' EXIT INT TERM
          bmux_pid_tty_map="$bmux_tmpdir/pid_tty"
          ps -t "$bmux_tty_csv" -o pid=,tty= 2>/dev/null | awk '
            NF >= 2 {
              tty = $2
              sub(/^.*\\//, "", tty)
              print $1 "\\t" tty
          ...
          ' > "$bmux_pid_tty_map"
          [ -s "$bmux_pid_tty_map" ] || exit 0
          bmux_pid_csv="$(awk '{print $1}' "$bmux_pid_tty_map" | paste -sd, -)"
          [ -n "$bmux_pid_csv" ] || exit 0
          lsof -nP -a -p "$bmux_pid_csv" -iTCP -sTCP:LISTEN -Fpn 2>/dev/null | awk -v map="$bmux_pid_tty_map" '
            BEGIN {
              while ((getline < map) > 0) {
                pid_to_tty[$1] = $2
              }
          ...
          ' | while IFS=$'\\t' read -r bmux_tty bmux_port; do
            [ -n "$bmux_tty" ] || continue
            [ -n "$bmux_port" ] || continue
            bmux_emit_port "$bmux_tty" "$bmux_port"
          done
    ...
    static func remoteAllPortsScanScript(excluding ports: Set<Int>) -> String {
        ...
        if command -v ss >/dev/null 2>&1; then
          ss -ltnH 2>/dev/null | awk '{print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r bmux_port; do
            [ -n "$bmux_port" ] || continue
            bmux_emit_port "$bmux_port"
          done
        elif command -v netstat >/dev/null 2>&1; then
          netstat -lnt 2>/dev/null | awk 'NR > 2 {print $4}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r bmux_port; do
            [ -n "$bmux_port" ] || continue
            bmux_emit_port "$bmux_port"
          done
        elif command -v lsof >/dev/null 2>&1; then
          lsof -nP -iTCP -sTCP:LISTEN 2>/dev/null | awk 'NR > 1 {print $9}' | sed -E 's/.*:([0-9]+)$/\\1/' | awk '/^[0-9]+$/ {print $1}' | while IFS= read -r bmux_port; do
            [ -n "$bmux_port" ] || continue
            bmux_emit_port "$bmux_port"
          done
        fi
