...
// bmuxd-remote serve-stdio children reparented to launchd after a crash or
...
extension RemoteSessionCoordinator {
    ...
    /// orphaned (PPID 1) bmux-owned ssh transports for `destination`,
    ...
    public static func orphanedBMUXRemoteSSHPIDs(
        psOutput: String,
        destination: String,
        relayPort: Int? = nil,
        ...
        return psOutput
            ...
            .compactMap { line -> Int? in
                ...
                guard isOrphanedBMUXRemoteSSHCommand(
                    parsed.command,
    ...
    static func killOrphanedRemoteSSHProcesses(
        ...
        for pid in orphanedBMUXRemoteSSHPIDs(
            psOutput: output,
    ...
    private static func isOrphanedBMUXRemoteSSHCommand(
        _ command: String,
        destination: String,
        relayPort: Int?,
        ...
        if let relayPort {
            ...
            return isBMUXRemotePersistentDaemonServeStdioCommand(
                trimmed,
        ...
        if let trimmedPersistentDaemonSlot {
            if isBMUXRemotePersistentDaemonServeStdioCommand(
                trimmed,
                slot: trimmedPersistentDaemonSlot
            ) {
                return true
            }
            return isBMUXRemoteNonPersistentDaemonServeStdioCommand(trimmed)
        }
        if isBMUXRemoteDaemonServeStdioCommand(trimmed) {
            return true
        }
        return false
    }

    private static func isBMUXRemoteDaemonServeStdioCommand(_ command: String) -> Bool {
        guard command.contains("bmuxd-remote") else { return false }
    ...
    private static func isBMUXRemoteNonPersistentDaemonServeStdioCommand(_ command: String) -> Bool {
        guard isBMUXRemoteDaemonServeStdioCommand(command) else { return false }
        let normalized = command
            .replacingOccurrences(of: "'", with: " ")
            .replacingOccurrences(of: "\"", with: " ")
    ...
    private static func isBMUXRemotePersistentDaemonServeStdioCommand(
        _ command: String,
        slot: String
    ) -> Bool {
        guard isBMUXRemoteDaemonServeStdioCommand(command) else { return false }
