...
// relay shell hook to `~/.bmux/relay/<port>.tty`), with a bounded retry.
...
extension RemoteSessionCoordinator {
    ...
    func requestBootstrapRemoteTTYIfNeededLocked() {
        ...
        let command = "sh -c \("tty_path=\"$HOME/.bmux/relay/\(relayPort).tty\"; if [ -r \"$tty_path\" ]; then cat \"$tty_path\"; fi".shellSingleQuoted)"
        do {
