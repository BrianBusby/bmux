internal import BmuxCore
internal import BmuxFoundation
internal import CryptoKit
public import Foundation

// bmuxd-remote bootstrap: probe the remote platform and existing install,
...
extension RemoteSessionCoordinator {
    static let remotePlatformProbeHomeMarker = "__BMUX_REMOTE_HOME__="
    static let remotePlatformProbeOSMarker = "__BMUX_REMOTE_OS__="
    static let remotePlatformProbeArchMarker = "__BMUX_REMOTE_ARCH__="
    static let remotePlatformProbeExistsMarker = "__BMUX_REMOTE_EXISTS__="
    ...
    static func remotePlatformProbeScript(version: String) -> String {
        ...
        return """
        bmux_uname_os="$(uname -s)"
        bmux_uname_arch="$(uname -m)"
        printf '%s%s\\n' '\(Self.remotePlatformProbeHomeMarker)' "$HOME"
        printf '%s%s\\n' '\(Self.remotePlatformProbeOSMarker)' "$bmux_uname_os"
        printf '%s%s\\n' '\(Self.remotePlatformProbeArchMarker)' "$bmux_uname_arch"
        case "$bmux_uname_os" in
          Linux|linux|LINUX) bmux_go_os=linux ;;
          Darwin|darwin|DARWIN) bmux_go_os=darwin ;;
          FreeBSD|freebsd|FREEBSD) bmux_go_os=freebsd ;;
          *) exit 70 ;;
        esac
        case "$bmux_uname_arch" in
          x86_64|X86_64|amd64|AMD64) bmux_go_arch=amd64 ;;
          aarch64|AARCH64|arm64|ARM64) bmux_go_arch=arm64 ;;
          armv7l|ARMV7L|armv7|ARMV7) bmux_go_arch=arm ;;
          *) exit 71 ;;
        esac
        bmux_remote_path="$HOME/.bmux/bin/bmuxd-remote/\(scriptVersion)/${bmux_go_os}-${bmux_go_arch}/bmuxd-remote"
        if [ -x "$bmux_remote_path" ]; then
    ...
    func probeRemoteBootstrapStateLocked(version: String) throws -> RemoteBootstrapState {
        ...
        guard let unameOS, let unameArch, let homeDirectory else {
            ...
            throw NSError(domain: "bmux.remote.daemon", code: 11, userInfo: [
        ...
        guard let goOS = Self.mapUnameOS(unameOS),
            ...
            throw NSError(domain: "bmux.remote.daemon", code: 12, userInfo: [
        ...
        if result.status != 0, binaryExists == nil {
            let detail = Self.bestErrorLine(stderr: result.stderr, stdout: userFacingStdout) ?? "ssh exited \(result.status)"
            throw NSError(domain: "bmux.remote.daemon", code: 13, userInfo: [
    ...
    static func allowLocalDaemonBuildFallback(environment: [String: String] = ProcessInfo.processInfo.environment) -> Bool {
        environment["BMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1"
    }
    ...
    static func explicitRemoteDaemonBinaryURL(environment: [String: String] = ProcessInfo.processInfo.environment) -> URL? {
        ...
        guard let path = environment["BMUX_REMOTE_DAEMON_BINARY"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !path.isEmpty else {
            return nil
        }
    ...
    static func versionedRemoteDaemonBuildURL(goOS: String, goArch: String, version: String) -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("bmux-remote-daemon-build", isDirectory: true)
            .appendingPathComponent(version, isDirectory: true)
            .appendingPathComponent("\(goOS)-\(goArch)", isDirectory: true)
            .appendingPathComponent("bmuxd-remote", isDirectory: false)
    }
    ...
    func buildLocalDaemonBinary(goOS: String, goArch: String, version: String) throws -> URL {
        ...
        guard Self.allowLocalDaemonBuildFallback() else {
            throw NSError(domain: "bmux.remote.daemon", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "this build does not include a verified bmuxd-remote manifest for \(goOS)-\(goArch). Use a release/nightly build, or set BMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 for a dev-only fallback.",
            ])
        }

        guard let repoRoot = findRepoRoot() else {
            throw NSError(domain: "bmux.remote.daemon", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "cannot locate bmux repo root for dev-only bmuxd-remote build fallback",
            ])
        }
        ...
        guard FileManager.default.fileExists(atPath: goModPath) else {
            throw NSError(domain: "bmux.remote.daemon", code: 21, userInfo: [
                NSLocalizedDescriptionKey: "missing daemon module at \(goModPath)",
            ])
        }
        guard let goBinary = Self.which("go") else {
            throw NSError(domain: "bmux.remote.daemon", code: 22, userInfo: [
                NSLocalizedDescriptionKey: "go is required for the dev-only bmuxd-remote build fallback",
            ])
        }
        ...
        let result = try runProcess(
            executable: goBinary,
            arguments: ["build", "-trimpath", "-buildvcs=false", "-ldflags", ldflags, "-o", output.path, "./cmd/bmuxd-remote"],
            environment: env,
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "bmux.remote.daemon", code: 23, userInfo: [
                NSLocalizedDescriptionKey: "failed to build bmuxd-remote: \(detail)",
            ])
        }
        guard FileManager.default.isExecutableFile(atPath: output.path) else {
            throw NSError(domain: "bmux.remote.daemon", code: 24, userInfo: [
                NSLocalizedDescriptionKey: "bmuxd-remote build output is not executable",
            ])
        }
    ...
    func uploadRemoteDaemonBinaryLocked(localBinary: URL, location: RemoteDaemonInstallLocation) throws {
        ...
        guard mkdirResult.status == 0 else {
            ...
            throw NSError(domain: "bmux.remote.daemon", code: 30, userInfo: [
        ...
        guard scpResult.status == 0 else {
            ...
            throw NSError(domain: "bmux.remote.daemon", code: 31, userInfo: [
                NSLocalizedDescriptionKey: "failed to upload bmuxd-remote: \(detail)",
            ])
        }
        ...
        guard finalizeResult.status == 0 else {
            ...
            throw NSError(domain: "bmux.remote.daemon", code: 32, userInfo: [
    ...
    func helloRemoteDaemonLocked(remotePath: String) throws -> DaemonHello {
        ...
        guard result.status == 0 else {
            ...
            throw NSError(domain: "bmux.remote.daemon", code: 40, userInfo: [
        ...
        guard !responseLine.isEmpty,
            ...
            throw NSError(domain: "bmux.remote.daemon", code: 41, userInfo: [
        ...
        if let ok = payload["ok"] as? Bool, !ok {
            ...
            throw NSError(domain: "bmux.remote.daemon", code: 42, userInfo: [
        ...
        return DaemonHello(
            name: (name?.isEmpty == false ? name! : "bmuxd-remote"),
    ...
    static func remoteDaemonPath(version: String, goOS: String, goArch: String) -> String {
        ".bmux/bin/bmuxd-remote/\(version)/\(goOS)-\(goArch)/bmuxd-remote"
    }
    ...
    static func absoluteRemotePath(homeDirectory: String, relativePath: String) throws -> String {
        ...
        guard normalizedHome.hasPrefix("/"), !normalizedHome.isEmpty, !normalizedRelative.isEmpty else {
            throw NSError(domain: "bmux.remote.daemon", code: 14, userInfo: [
    ...
    func findRepoRoot() -> URL? {
        ...
        let compileTimeRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent() // BmuxRemoteSession
        candidates.append(compileTimeRoot)
        let environment = ProcessInfo.processInfo.environment
        if let envRoot = environment["BMUX_REMOTE_DAEMON_SOURCE_ROOT"],
        ...
        if let envRoot = environment["BMUXTERM_REPO_ROOT"],
           !envRoot.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            candidates.append(URL(fileURLWithPath: envRoot, isDirectory: true))
        }
