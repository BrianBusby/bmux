import Foundation
import Testing
@testable import BmuxRemoteSession

// Ported from the app-target WorkspaceRemotePlatformProbeTests (#6056) when
// the remote bootstrap probe was lifted into BmuxRemoteSession. The probe must
...
struct RemotePlatformProbeScriptTests {
    ...
    func probeScriptWorksWhenTrLacksCharacterClasses() throws {
        ...
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "bmux-remote-platform-probe-\(UUID().uuidString)",
            isDirectory: true
        )
        let home = root.appendingPathComponent("home", isDirectory: true)
        let bin = root.appendingPathComponent("bin", isDirectory: true)
        let daemonURL = home
            .appendingPathComponent(".bmux/bin/bmuxd-remote/test-version/linux-amd64", isDirectory: true)
    ...
    func probeScriptSanitizesVersionBeforeShellInterpolation() throws {
        ...
        let bin = root.appendingPathComponent("bin", isDirectory: true)
        let daemonURL = home
            .appendingPathComponent(".bmux/bin/bmuxd-remote/dev/linux-amd64", isDirectory: true)
            .appendingPathComponent("bmuxd-remote", isDirectory: false)
        try fileManager.createDirectory(at: daemonURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        try fileManager.createDirectory(at: bin, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let result = try Self.runProcess(
            ...
            arguments: [
                ...
                RemoteSessionCoordinator.remotePlatformProbeScript(
                    version: #""; printf "__BMUX_INJECTED__\n"; #"#
                ),
            ]
        )
        ...
        let stdoutComment = Comment(rawValue: result.stdout)
        #expect(result.status == 0, outputComment)
        #expect(!result.stdout.contains("__BMUX_INJECTED__"), stdoutComment)
        #expect(
            result.stdout.contains("\(RemoteSessionCoordinator.remotePlatformProbeExistsMarker)yes"),
            stdoutComment
        )
