...
// `bmux.remote.process` error codes 1 and 2.
//
...
struct RemoteSessionProcessRunnerTests {
    ...
    @Test("Launch failure throws the pinned bmux.remote.process code 1")
    func launchFailurePinsErrorCode() {
        ...
        #expect {
            try runner.run(
                RemoteProcessRequest(
                    executable: "/nonexistent/bmux-no-such-binary",
                    arguments: [],
                    timeout: 2
                ),
                operation: nil
            ...
            return nsError.domain == "bmux.remote.process"
                && nsError.code == 1
                && nsError.localizedDescription.hasPrefix("Failed to launch bmux-no-such-binary:")
        }
    }
