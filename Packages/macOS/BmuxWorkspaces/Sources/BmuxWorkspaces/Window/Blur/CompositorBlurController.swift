...
private func bmuxCGSDefaultConnectionForThread() -> UnsafeMutableRawPointer?
...
@discardableResult
private func bmuxCGSSetWindowBackgroundBlurRadius(
...
///
/// Lifted faithfully from the `bmuxResetCompositorBackgroundBlur` free function
...
/// the caller's isolation domain (the legacy `bmuxResetCompositorBackgroundBlur`
...
public struct CompositorBlurController: Sendable {
    ...
    /// `bmuxResetCompositorBackgroundBlur(on:)`. The caller resolves
    /// `window.windowNumber` in its own (main-actor) isolation domain.
    public func resetBackgroundBlur(windowNumber: Int) {
        _ = bmuxCGSSetWindowBackgroundBlurRadius(
            bmuxCGSDefaultConnectionForThread(),
            UInt(windowNumber),
            0
        )
    }
