import Foundation
import Testing
@testable import BmuxWorkspaces
...
struct SessionSnapshotRepositoryTests {
    ...
    @Test("snapshot file paths derive from the sanitized bundle identifier under bmux/")
    func snapshotFileURLShape() throws {
        let dir = try makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: dir) }
        let repository = makeRepository(appSupport: dir, bundleIdentifier: "com.bmux odd/id")

        let primary = try #require(repository.defaultSnapshotFileURL())
        let backup = try #require(repository.manualRestoreSnapshotFileURL())
        #expect(primary.path == dir.appendingPathComponent("bmux/session-com.bmux_odd_id.json").path)
        #expect(backup.path == dir.appendingPathComponent("bmux/session-com.bmux_odd_id-previous.json").path)
    }

    @Test("nil and blank bundle identifiers fall back to com.bmuxterm.app")
    func bundleIdentifierFallback() throws {
        ...
        for identifier in [nil, "  "] as [String?] {
            ...
            let primary = try #require(repository.defaultSnapshotFileURL())
            #expect(primary.lastPathComponent == "session-com.bmuxterm.app.json")
        }
    }
