...
private final class FakeHistoryProvider: BrowserProfileHistoryProviding {
    ...
    var defaultURL: URL? = URL(fileURLWithPath: "/tmp/bmux-test-default/browser_history.json")
...
private func makeRepository(
    suiteName: String = "bmux.browserprofiles.test.\(UUID().uuidString)",
    ...
    bundleIdentifier: String = "ai.manaflow.bmux.test"
...
struct BrowserProfileRepositoryTests {
    ...
    @Test func historyFileURLDefaultAndPerProfileShape() {
        let history = FakeHistoryProvider()
        let (repo, _) = makeRepository(history: history, bundleIdentifier: "ai.manaflow.bmux.test")
        ...
        #expect(url.path.contains("ns-ai.manaflow.bmux.test"))
    ...
    @Test func loadDedupesPersistedBuiltInDefault() {
        let suite = "bmux.browserprofiles.test.dedup.\(UUID().uuidString)"
