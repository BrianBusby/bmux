...
extension BrowserOmnibarPageFocusRepository {
    ...
    /// `window.__bmuxAddressBarFocusState` (mirrored to the top frame) and tags
    /// the element with a stable `data-bmux-addressbar-focus-id`. Returns a
    /// `captured:<id>` / `cleared:*` / `error` status string.
    static let captureScript = """
    (() => {
      try {
        const syncState = (state) => {
          window.__bmuxAddressBarFocusState = state;
          try {
            if (window.top && window.top !== window) {
              window.top.postMessage({ bmuxAddressBarFocusState: state }, "*");
            } else if (window.top) {
              window.top.__bmuxAddressBarFocusState = state;
            }
          } catch (_) {}
        };
        ...
        let id = active.getAttribute("data-bmux-addressbar-focus-id");
        if (!id) {
          id = "bmux-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
          active.setAttribute("data-bmux-addressbar-focus-id", id);
        }
    ...
    (() => {
      try {
        if (window.__bmuxAddressBarFocusTrackerInstalled) return true;
        ...
        if (window.top === window && !window.__bmuxAddressBarFocusMessageBridgeInstalled) {
          window.__bmuxAddressBarFocusMessageBridgeInstalled = true;
          window.addEventListener("message", (ev) => {
            try {
              const data = ev ? ev.data : null;
              if (!data || !Object.prototype.hasOwnProperty.call(data, "bmuxAddressBarFocusState")) return;
              window.__bmuxAddressBarFocusState = data.bmuxAddressBarFocusState || null;
            } catch (_) {}
          }, true);
        }
        ...
        const ensureFocusId = (el) => {
          let id = el.getAttribute("data-bmux-addressbar-focus-id");
          if (!id) {
            id = "bmux-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 8);
            el.setAttribute("data-bmux-addressbar-focus-id", id);
          }
          return id;
        };
    ...
    (() => {
      try {
        const readState = () => {
          let state = window.__bmuxAddressBarFocusState;
          try {
            if ((!state || typeof state.id !== "string" || !state.id) &&
                window.top && window.top.__bmuxAddressBarFocusState) {
              state = window.top.__bmuxAddressBarFocusState;
            }
          } catch (_) {}
          return state;
        };
        ...
        const clearState = () => {
          window.__bmuxAddressBarFocusState = null;
          try {
            if (window.top && window.top !== window) {
              window.top.postMessage({ bmuxAddressBarFocusState: null }, "*");
            } else if (window.top) {
              window.top.__bmuxAddressBarFocusState = null;
            }
          } catch (_) {}
        };
        ...
        const selector = '[data-bmux-addressbar-focus-id="' + state.id + '"]';
