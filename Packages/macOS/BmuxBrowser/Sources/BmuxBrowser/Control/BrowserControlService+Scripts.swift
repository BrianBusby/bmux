...
extension BrowserControlService {
    ...
    public func findScript(finderBody: String) -> String {
        ...
        \(finderBody)
          ...
          if (!__bmuxFound) return { ok: false, error: 'not_found' };
          const selector = __bmuxCssPath(__bmuxFound);
          if (!selector) return { ok: false, error: 'not_found' };
          return {
            ok: true,
            selector,
            tag: String(__bmuxFound.tagName || '').toLowerCase(),
            text: String(__bmuxFound.textContent || '').trim()
          };
        })()
        """
    }
