...
public struct BrowserFindScript: Sendable, Equatable {
    ...
    public static func search(query: String) -> BrowserFindScript {
        ...
        (() => {
          const MARK_CLASS = '__bmux-find';
          const CURRENT_CLASS = '__bmux-find-current';
          ...
          window.__bmuxFindMatches = matches;
          window.__bmuxFindIndex = 0;
          ...
          if (!document.getElementById('__bmux-find-style')) {
            const style = document.createElement('style');
            style.id = '__bmux-find-style';
            style.textContent = `
              mark.__bmux-find { background: #facc15; color: #000; border-radius: 2px; }
              mark.__bmux-find.__bmux-find-current { background: #f97316; color: #fff; }
            `;
    ...
    public static func next() -> BrowserFindScript {
        ...
        (() => {
          const matches = window.__bmuxFindMatches || [];
          if (matches.length === 0) return JSON.stringify({ total: 0, current: 0 });
          let idx = window.__bmuxFindIndex || 0;
          if (!matches[idx] || !matches[idx].isConnected) {
            window.__bmuxFindMatches = [];
            window.__bmuxFindIndex = 0;
            return JSON.stringify({ total: 0, current: 0 });
          }
          matches[idx].classList.remove('__bmux-find-current');
          ...
          matches[idx].classList.add('__bmux-find-current');
          matches[idx].scrollIntoView({ block: 'center', behavior: 'smooth' });
          window.__bmuxFindIndex = idx;
    ...
    public static func clear() -> BrowserFindScript {
        ...
        (() => {
          ...
          window.__bmuxFindMatches = [];
          window.__bmuxFindIndex = 0;
          const style = document.getElementById('__bmux-find-style');
    ...
    document.querySelectorAll('mark.__bmux-find').forEach(mark => {
