...
/// stateless `BmuxGit` ``BmuxGit/PullRequestProbeService`` pipeline.
///
...
public final class PullRequestPollService: PullRequestProbing {
    ...
    // Resolves slugs for candidate seeds (stateless BmuxGit reader).
    let gitMetadataService: GitMetadataService
    // Fetches and matches GitHub PRs (stateless BmuxGit pipeline).
