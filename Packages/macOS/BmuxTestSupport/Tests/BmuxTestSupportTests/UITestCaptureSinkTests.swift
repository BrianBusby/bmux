import Foundation
import Testing
@testable import BmuxTestSupport

@Suite("UITestCaptureSink")
struct UITestCaptureSinkTests {
    private func makeScratchPath(_ name: String = "capture.txt") -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-test-support-\(UUID().uuidString)", isDirectory: true)
            .appendingPathComponent(name)
    }

    @Test func unconfiguredEnvironmentIsANoOp() {
        let sink = UITestCaptureSink(environment: [:])
        #expect(!sink.appendLineIfConfigured(envKey: "BMUX_UI_TEST_X", line: "hello"))
        #expect(!sink.mutateJSONObjectIfConfigured(envKey: "BMUX_UI_TEST_X") { $0["k"] = 1 })
    }

    @Test func blankOrWhitespacePathIsANoOp() {
        let sink = UITestCaptureSink(environment: ["BMUX_UI_TEST_X": "  \n "])
        #expect(!sink.appendLineIfConfigured(envKey: "BMUX_UI_TEST_X", line: "hello"))
    }
    ...
    @Test func appendCreatesParentDirectoryAndAppendsLines() throws {
        ...
        let sink = UITestCaptureSink(environment: ["BMUX_UI_TEST_X": url.path])

        #expect(sink.appendLineIfConfigured(envKey: "BMUX_UI_TEST_X", line: "first"))
        #expect(sink.appendLineIfConfigured(envKey: "BMUX_UI_TEST_X", line: "second"))
    ...
    @Test func mutateJSONMergesIntoExistingObject() throws {
        ...
        #expect(sink.mutateJSONObjectIfConfigured(envKey: "BMUX_UI_TEST_X") { payload in
            payload["b"] = 2
        })
        #expect(sink.mutateJSONObjectIfConfigured(envKey: "BMUX_UI_TEST_X") { payload in
            payload["a"] = 1
