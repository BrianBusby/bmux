...
final class NucleoLibrary {
    ...
    private static let libraryFileName = "libbmux_command_palette_nucleo_ffi.dylib"
    ...
    private init(path: String) throws {
        ...
        guard resolvedVersion == Self.supportedVersion else {
            ...
            throw NSError(
                ...
                code: 6,
                userInfo: [
                    NSLocalizedDescriptionKey: "unsupported bmux_nucleo_ffi_version \(resolvedVersion)"
                ]
            )
        }
    }
    ...
    private static func defaultLibraryPaths(environment: [String: String]) -> [String] {
        var paths: [String] = []
        if let environmentPath = environment["BMUX_NUCLEO_FFI_LIB"], !environmentPath.isEmpty {
        ...
        // This file lives at Packages/macOS/BmuxCommandPalette/Tests/BmuxCommandPaletteTests/,
        ...
        paths.append(
            crateTarget
                .appendingPathComponent("bmux-nucleo-ffi")
                .appendingPathComponent(libraryFileName)
                .path
        )
        paths.append(
    ...
    init(library: NucleoLibrary, entries: [FixtureEntry]) throws {
        ...
        guard let pointer = blob.withUnsafeBufferPointer({ blobBuffer in
            ...
            throw NSError(
                domain: "CommandPaletteNucleoFFITests",
                code: 3,
                userInfo: [NSLocalizedDescriptionKey: "bmux_nucleo_index_create returned null"]
            )
        }
    ...
    func search(query: String, limit: Int) throws -> [NucleoResult] {
        ...
        guard status == 0 else {
            throw NSError(
                domain: "CommandPaletteNucleoFFITests",
                code: Int(status),
                userInfo: [NSLocalizedDescriptionKey: "bmux_nucleo_index_search failed with \(status)"]
            )
        }
        ...
        guard count >= 0, count <= matches.count, count <= limit else {
            throw NSError(
                ...
                code: 4,
                userInfo: [
                    NSLocalizedDescriptionKey: "bmux_nucleo_index_search returned invalid count \(count) for limit \(limit)"
                ]
            )
        }
        ...
        return try matches.prefix(count).map { match in
            guard entries.indices.contains(match.index) else {
                throw NSError(
                    ...
                    code: 5,
                    userInfo: [
                        NSLocalizedDescriptionKey: "bmux_nucleo_index_search returned invalid index \(match.index)"
                    ]
                )
            }
