...
public enum CommandPaletteRequestKind: String, Sendable, CaseIterable {
    ...
    ///
    /// These strings are byte-identical to the `bmux.*` `Notification.Name`
    /// literals the app target previously posted inline, so observers keyed on
    /// the existing names are unaffected.
    public var notificationName: String {
        switch self {
        case .commands:
            return "bmux.commandPaletteRequested"
        case .switcher:
            return "bmux.commandPaletteSwitcherRequested"
        case .renameTab:
            return "bmux.commandPaletteRenameTabRequested"
        case .renameWorkspace:
            return "bmux.commandPaletteRenameWorkspaceRequested"
        case .editWorkspaceDescription:
            return "bmux.commandPaletteEditWorkspaceDescriptionRequested"
        }
    }
