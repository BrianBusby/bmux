...
// search state.
/// Loaded `bmux-nucleo-ffi` dylib handle with resolved C entry points.
///
...
final class CommandPaletteNucleoSearchLibrary: @unchecked Sendable {
    ...
    private static func load(path: String) -> CommandPaletteNucleoSearchLibrary? {
        ...
        guard let createIndex = symbol("bmux_nucleo_index_create", from: handle, as: CreateIndex.self),
              let destroyIndex = symbol("bmux_nucleo_index_destroy", from: handle, as: DestroyIndex.self),
              let searchIndexWithBoosts = symbol(
                "bmux_nucleo_index_search_with_boosts",
                from: handle,
                as: SearchIndexWithBoosts.self
              ),
              let version = symbol("bmux_nucleo_ffi_version", from: handle, as: Version.self) else {
            dlclose(handle)
            return nil
        }
    ...
    private static func defaultLibraryPaths() -> [String] {
        var paths: [String] = []
        let environmentPath = ProcessInfo.processInfo.environment["BMUX_NUCLEO_FFI_LIB"]
        ...
        // Packages/macOS/BmuxCommandPalette/Sources/BmuxCommandPalette/Search/Nucleo/.
        ...
        paths.append(
            sourceRoot
                .appendingPathComponent("Native/CommandPaletteNucleoFFI/target/bmux-nucleo-ffi")
                .appendingPathComponent(Self.libraryFileName)
                .path
        )
        paths.append(
