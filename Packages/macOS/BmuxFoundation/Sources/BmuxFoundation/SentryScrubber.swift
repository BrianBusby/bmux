...
public struct SentryScrubber: Sendable {
    ...
    ///
    /// This is bmux's equivalent of relay's `@common` `@urlauth` rule
    ...
    /// ``ScrubberDenylists/valuePatterns`` because bmux's redaction loop treats
    ...
    /// `@common` ports and bmux's own provider-key / JWT / AWS rules live in one
    ...
    /// sentry-python `DEFAULT_DENYLIST`). bmux's substring-contains semantics
