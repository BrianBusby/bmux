...
/// App-wide magnification for bmux-owned chrome and terminal configuration.
///
/// Stored as an integer percent (100 = default, 150 = 1.5x, 200 = 2x).
/// SwiftUI call sites should use ``View/bmuxFont(size:weight:design:monospacedDigit:)``
/// or ``View/bmuxFont(_:weight:design:)``. AppKit call sites should use the
...
public struct GlobalFontMagnification {
    ...
    /// Capped at 200% so bmux fixed-size chrome does not clip or overflow.
    ...
    public static let didChangeNotification = Notification.Name("bmux.globalFontMagnification.didChange")
