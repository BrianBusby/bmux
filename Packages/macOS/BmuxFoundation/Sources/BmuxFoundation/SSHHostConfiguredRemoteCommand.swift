...
/// Neutralizes a host-configured OpenSSH `RemoteCommand` for bmux-controlled
...
/// (https://github.com/manaflow-ai/bmux/issues/7246). Every bmux-built ssh
/// argv that appends its own command — auth probes, bootstrap installers,
/// the bmuxd stdio transport, port scans, tmux mirror commands, cleanup
...
/// option, which would break bmux plumbing the same way. Session
/// invocations that intentionally carry bmux's own
