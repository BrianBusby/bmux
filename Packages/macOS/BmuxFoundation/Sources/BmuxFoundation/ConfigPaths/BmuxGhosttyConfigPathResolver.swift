...
/// Resolves which bmux-managed Ghostty config file under Application Support is
...
public struct BmuxGhosttyConfigPathResolver {
    /// The bundle identifier of the released bmux app, used as the canonical
    /// config location and the fallback for dev/nightly/staging channels.
    public static let releaseBundleIdentifier = "com.bmuxterm.app"
    private static let releaseFallbackChannelSuffixes = ["debug", "nightly", "staging"]

    public init() {}

    /// The path bmux writes edits to: always `config.ghostty` under the active
    ...
    /// The ordered list of bmux-managed config files to load, applying the
