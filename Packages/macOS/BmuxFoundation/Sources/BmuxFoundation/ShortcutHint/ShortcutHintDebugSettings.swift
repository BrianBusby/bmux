...
public struct ShortcutHintDebugSettings {
    ...
    /// `shortcuts.showModifierHoldHints` toggle. `BmuxFoundation` is a leaf
    /// module and cannot import `BmuxSettings`, so the key is duplicated here;
    ...
    public var alwaysShowHints: Bool {
        Self.defaultAlwaysShowHints || environment["BMUX_UI_TEST_SHORTCUT_HINTS_ALWAYS_SHOW"] == "1"
    }
