...
/// from Sentry's relay `@common` scrubbers, plus bmux's own provider-key / JWT /
...
public struct ScrubberDenylists: Sendable {
    ...
    /// sentry-python itself credits as "stolen from relay". bmux's substring
    ...
    static let sensitiveKeyMarkers: [String] = [
        ...
        // bmux-original markers retained (not in sentry-python's list) so no
    ...
    /// (PII keys added because bmux runs with `sendDefaultPii = false`), with the
    ...
    static let sensitiveKeyExactMarkers: Set<String> = [
        ...
        // sentry-python DEFAULT_PII_DENYLIST (bmux sends sendDefaultPii = false).
    ...
    ///
    /// The first seven entries are bmux-original and partly exceed relay (relay
    ...
    /// value rules bmux was missing, ported with no capture group so bmux's
    ...
    /// members are intentionally NOT duplicated here: bmux already covers email /
    ...
    static let valuePatterns: [SentryRegexPattern] = [
        // --- bmux-original rules (kept verbatim; partly exceed relay) ---
        ...
        // (relay-pii/src/regexes.rs:341-345), hand-tuned for bmux's free text.
        //
        ...
        // prefixes. No relay equivalent — bmux-specific add for dev secrets.
        ...
        // --- relay @common value rules bmux was missing (ported, no group) ---
        // @pemkey — a PEM private/public key block. relay captures group 1 (the
        // body) and replaces only the body; bmux redacts the WHOLE block (header,
        ...
        // dotall flag (bmux's SentryRegexPattern sets only .caseInsensitive).
