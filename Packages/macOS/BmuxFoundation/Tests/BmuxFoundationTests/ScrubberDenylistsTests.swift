...
@Suite struct ScrubberDenylistsTests {
    ...
    static let sensitiveKeys: [String] = [
        ...
        "_xsrf", "XSRF-TOKEN",
        // PII (bmux runs sendDefaultPii = false)
    ...
    /// representative secrets/PII (relay `@common` corpora plus bmux's own); each
    ...
    static let valueRedactions: [(input: String, expected: String)] = [
        ...
        // @urlauth equivalent (bmux redactURLCredentials) — userinfo stripped.
        ...
        // bmux-original provider key / JWT / AWS (exceed relay).
    ...
    /// inversion: relay redacts group 1 = body, bmux redacts the whole match).
    ...
    static let preservedStrings: [String] = [
        // UUIDs bmux logs constantly (workspace/surface ids). @uuid intentionally not ported.
