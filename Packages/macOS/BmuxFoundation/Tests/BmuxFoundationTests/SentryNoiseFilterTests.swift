...
final class SentryNoiseFilterTests: XCTestCase {
    ...
    func testDropsExpectedCLISocketDisconnectsInSocketStages() {
        ...
        XCTAssertTrue(filter.isExpectedCLISocketTransportFailure(
            ...
            message: "Failed to connect to socket at /tmp/bmux.sock (Connection refused, errno 61)"
        ))
        XCTAssertTrue(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_connect",
            message: "Socket not found at /tmp/bmux.sock"
        ))
    }
    ...
    func testKeepsActionableSocketFailures() {
        ...
        XCTAssertFalse(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_connect",
            message: "Failed to connect to socket at /tmp/bmux.sock (Permission denied, errno 13)"
        ))
    }
    ...
    func testErrnoMatchingRequiresExactCode() {
        XCTAssertFalse(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_connect",
            message: "Failed to connect to socket at /tmp/bmux.sock (Invalid argument, errno 22)"
        ))
        XCTAssertFalse(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_command",
        ...
        XCTAssertTrue(filter.isExpectedCLISocketTransportFailure(
            stage: "socket_connect",
            message: "Failed to connect to socket at /tmp/bmux.sock (errno=2)"
        ))
    }
