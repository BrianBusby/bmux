import Foundation
import Testing
@testable import BMUXProjectModel

@Suite("XcodeProjectAdapter against bmux.xcodeproj")
struct XcodeProjectAdapterTests {
    ...
    init() {
        ...
        if let override = env["BMUX_PROJECT_FIXTURE"] {
            let base = URL(fileURLWithPath: override)
            self.workspaceURL = base.pathExtension.lowercased() == "xcworkspace" ? base : base.appendingPathComponent("bmux.xcworkspace")
            self.projectURL = base.pathExtension.lowercased() == "xcodeproj" ? base : base.appendingPathComponent("bmux.xcodeproj")
        } else {
            ...
            let worktreeRoot = here
                ...
                .deletingLastPathComponent()
                .deletingLastPathComponent()
            self.workspaceURL = worktreeRoot.appendingPathComponent("bmux.xcworkspace")
            self.projectURL = worktreeRoot.appendingPathComponent("bmux.xcodeproj")
        }
    }

    @Test
    func loadsBmuxXcodeprojIntoOneModule() throws {
    ...
    @Test
    func findsBmuxAppTargetWithApplicationProductType() throws {
        let adapter = XcodeProjectAdapter()
        let model = try adapter.load(at: projectURL)
        let module = try #require(model.modules.first)
        let bmuxTarget = module.targets.first(where: { $0.displayName == "bmux" })
        let summary = try #require(bmuxTarget)
    ...
    func bundleIdentifierIsEitherResolvedOrExplicitlyNilNotFabricated() throws {
        ...
        let model = try adapter.load(at: projectURL)
        let module = try #require(model.modules.first)
        let bmux = try #require(module.targets.first(where: { $0.displayName == "bmux" }))
        if let bundle = bmux.bundleIdentifier {
