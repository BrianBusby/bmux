...
static bool bmux_test_needs_confirm_quit = false;
static uint64_t bmux_test_foreground_pid = 0;
static const char* bmux_test_tty_name = NULL;

void bmux_test_ghostty_runtime_stubs_reset(void) {
    bmux_test_needs_confirm_quit = false;
    bmux_test_foreground_pid = 0;
    bmux_test_tty_name = NULL;
}

void bmux_test_ghostty_runtime_stubs_set_close_state(bool needs_confirm, uint64_t foreground_pid, const char* tty_name) {
    bmux_test_needs_confirm_quit = needs_confirm;
    bmux_test_foreground_pid = foreground_pid;
    bmux_test_tty_name = tty_name;
}
...
uint64_t ghostty_surface_foreground_pid(void *surface) {
    (void)surface;
    return bmux_test_foreground_pid;
}
...
bool ghostty_surface_needs_confirm_quit(void *surface) {
    (void)surface;
    return bmux_test_needs_confirm_quit;
}
...
ghostty_string_s ghostty_surface_tty_name(void *surface) {
    (void)surface;
    if (bmux_test_tty_name == NULL) {
        return (ghostty_string_s){0};
    }
    return (ghostty_string_s){.ptr = bmux_test_tty_name, .len = strlen(bmux_test_tty_name), .sentinel = false};
}
