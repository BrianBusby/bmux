...
internal import BMUXAgentLaunch
...
extension TerminalSurface {
    ...
    /// Applies the managed bmux context identity keys and protects them.
    public static func applyManagedBmuxContextEnvironment(
        _ context: BmuxContextEnvironment,
        to environment: inout [String: String],
        protectedKeys: inout Set<String>
    ) {
        let values = [
    ...
    public static func applyManagedGitWatchEnvironment(
        ...
        environment["BMUX_NO_GIT_WATCH"] = watchGitStatusEnabled ? "" : "1"
        protectedKeys.insert("BMUX_NO_GIT_WATCH")
        environment["BMUX_NO_PR_WATCH"] = (watchGitStatusEnabled && showPullRequestsEnabled) ? "" : "1"
        protectedKeys.insert("BMUX_NO_PR_WATCH")
    }
    ...
    public static func installClaudeCommandShimIfPossible(
        ...
        do {
            ...
            let script = """
            #!/usr/bin/env bash
            bmux_wrapper=\(shellSingleQuoted(wrapperURL.path))
            ...
            export BMUX_CLAUDE_WRAPPER_SHIM=\(shellSingleQuoted(shimURL.path))
            export BMUX_CLAUDE_WRAPPER_SHIM_ROOT=\(shellSingleQuoted(shimDirectory.path))
            if [[ -x "$bmux_wrapper" ]]; then
                exec "$bmux_wrapper" "$@"
            fi
            ...
            // dir so typed `codex` resolves to bmux-codex-wrapper through the
            ...
            // so runtime surface creation can export BMUX_CODEX_WRAPPER_SHIM into
            // the managed env, which a resumed `codex` session needs to route its
            // resume through the wrapper and keep bmux hooks.
    ...
    /// bundled `bmux-codex-wrapper` exists alongside `bmux-claude-wrapper`. The
    /// shim resolves and execs the codex wrapper; if the wrapper is gone it
    /// strips every bmux shim dir from `PATH` and execs the real `codex`, so the
    ...
    public static func installCodexCommandShimIfPossible(
        ...
        let codexWrapperURL = claudeWrapperURL
            ...
            .appendingPathComponent("bmux-codex-wrapper", isDirectory: false)
            .standardizedFileURL
        ...
        do {
            let script = """
            #!/usr/bin/env bash
            bmux_wrapper=\(shellSingleQuoted(codexWrapperURL.path))
            if [[ ! -x "$bmux_wrapper" && -n "${BMUX_BUNDLED_CLI_PATH:-}" ]]; then
                bmux_candidate="$(dirname "$BMUX_BUNDLED_CLI_PATH")/bmux-codex-wrapper"
                if [[ -x "$bmux_candidate" ]]; then
                    bmux_wrapper="$bmux_candidate"
                fi
            fi
            if [[ ! -x "$bmux_wrapper" ]]; then
                bmux_cli="$(command -v bmux 2>/dev/null || true)"
                if [[ -n "$bmux_cli" ]]; then
                    bmux_candidate="$(dirname "$bmux_cli")/bmux-codex-wrapper"
                    if [[ -x "$bmux_candidate" ]]; then
                        bmux_wrapper="$bmux_candidate"
                    fi
                fi
            fi
            export BMUX_CODEX_WRAPPER_SHIM=\(shellSingleQuoted(shimURL.path))
            export BMUX_CODEX_WRAPPER_SHIM_ROOT=\(shellSingleQuoted(shimDirectory.path))
            if [[ -x "$bmux_wrapper" ]]; then
                exec "$bmux_wrapper" "$@"
            fi
            bmux_path_without_shim=""
            ...
            IFS=:
            for bmux_entry in ${PATH:-}; do
                if [[ "$bmux_entry" == "$BMUX_CODEX_WRAPPER_SHIM_ROOT" || "$bmux_entry" == */bmux-cli-shims/* || "$bmux_entry" == */bmux-cli-shims ]]; then
                    continue
                fi
                if [[ -z "$bmux_path_without_shim" ]]; then
                    bmux_path_without_shim="$bmux_entry"
    ...
    /// the `C` fallback (see https://github.com/manaflow-ai/bmux/issues/7152).
    ...
    /// untouched. See https://github.com/manaflow-ai/bmux/issues/7152.
    ...
    public static func applyManagedFishStartupEnvironment(
        ...
        environment["BMUX_FISH_INTEGRATION_FILE"] = integrationFile
        environment["BMUX_FISH_USER_CONFIG_ALREADY_LOADED"] = "1"
        protectedKeys.insert("BMUX_FISH_INTEGRATION_FILE")
        protectedKeys.insert("BMUX_FISH_USER_CONFIG_ALREADY_LOADED")
    }
    ...
    /// callers must not advertise it (BMUX_SHELL_INTEGRATION_DIR) or redirect
    /// shell startup at it.
    public static func shellIntegrationDirectoryExists(
        ...
        Logger(subsystem: "com.bmuxterm.app", category: "ghostty.initialization")
            .error("bmux shell-integration dir missing at \(integrationDir, privacy: .private); spawning shell without bmux shell integration so the user's shell config still loads")
        return false
    }
    ...
    public static func applyManagedShellSpecificStartupEnvironment(
        ...
        func bundledBootstrapIsReadable(_ relativePath: String) -> Bool {
            ...
            Logger(subsystem: "com.bmuxterm.app", category: "ghostty.initialization")
                .error("bmux \(shellName, privacy: .public) bootstrap unreadable at \(path, privacy: .private); skipping bmux shell-startup redirection so the user's shell config still loads")
            return false
        }
        switch shellName {
        case "zsh":
            guard bundledBootstrapIsReadable(".zshenv") else { return nil }
            if userGhosttyShellIntegrationMode != "none" { setManagedEnvironmentValue("BMUX_LOAD_GHOSTTY_ZSH_INTEGRATION", "1") }
            ...
            if let candidateZdotdir, !candidateZdotdir.isEmpty {
                ...
                if candidateZdotdir != ghosttyZdotdir { setManagedEnvironmentValue("BMUX_ZSH_ZDOTDIR", candidateZdotdir) }
            }
            setManagedEnvironmentValue("ZDOTDIR", integrationDir)
        case "bash":
            if userGhosttyShellIntegrationMode != "none" { setManagedEnvironmentValue("BMUX_LOAD_GHOSTTY_BASH_INTEGRATION", "1") }
            let bashBootstrapPath = (integrationDir as NSString).appendingPathComponent("bmux-bash-bootstrap.bash")
            do {
                ...
                Logger(subsystem: "com.bmuxterm.app", category: "ghostty.initialization")
                    .error("bmux bash bootstrap unreadable at \(bashBootstrapPath, privacy: .private): \(error.localizedDescription, privacy: .public); bash shell integration will not load")
            }
        case "fish":
    ...
    /// The managed fish launch command sourcing the bmux integration file.
    public static func managedFishShellCommand(shell: String) -> String {
        let initCommand = #"source "$BMUX_FISH_INTEGRATION_FILE""#
