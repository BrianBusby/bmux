...
public struct TerminalSurfaceSpawnPolicy: Sendable {
    /// Whether Claude Code hooks are enabled (`BMUX_CLAUDE_HOOKS_DISABLED`).
    ...
    /// (`BMUX_CUSTOM_CLAUDE_PATH`), if set.
    ...
    /// Whether Cursor hooks are enabled (`BMUX_CURSOR_HOOKS_DISABLED`).
    public var cursorHooksEnabled: Bool

    /// Whether Gemini hooks are enabled (`BMUX_GEMINI_HOOKS_DISABLED`).
    public var geminiHooksEnabled: Bool

    /// Whether Kiro hooks are enabled (`BMUX_KIRO_HOOKS_DISABLED`).
    public var kiroHooksEnabled: Bool

    /// The Kiro notification level raw value
    /// (`BMUX_KIRO_NOTIFICATION_LEVEL`).
    public var kiroNotificationLevel: String

    /// Whether Amp hooks are enabled (`BMUX_AMP_HOOKS_DISABLED`).
    public var ampHooksEnabled: Bool

    /// Whether bmux shell integration is enabled (the
    /// `sidebarShellIntegration` setting).
    public var shellIntegrationEnabled: Bool

    /// Whether sidebar git-status watching is enabled (`BMUX_NO_GIT_WATCH`).
    public var watchGitStatusEnabled: Bool

    /// Whether sidebar pull-request watching is enabled (`BMUX_NO_PR_WATCH`).
