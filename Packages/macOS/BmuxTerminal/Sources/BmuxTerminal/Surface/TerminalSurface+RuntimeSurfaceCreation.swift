...
internal import BmuxTerminalCore
internal import BMUXAgentLaunch
internal import Darwin
#if DEBUG
internal import BMUXDebugLog
#endif
...
extension TerminalSurface {
    ...
    func createNativeRuntimeSurface(
        ...
        var baseConfig = configTemplate ?? BmuxSurfaceConfigTemplate()
        var surfaceConfig = ghostty_surface_config_new()
        let magnificationPercent = globalFontMagnificationPercent()
        surfaceConfig.font_size = BmuxSurfaceConfigTemplate.runtimeFontSize(
        ...
        setManagedEnvironmentValue("BMUX_SOCKET", "")
        ...
        do {
            ...
            setManagedEnvironmentValue("BMUX_PORT", String(startPort))
            setManagedEnvironmentValue("BMUX_PORT_END", String(startPort + sessionPortRangeSize - 1))
            setManagedEnvironmentValue("BMUX_PORT_RANGE", String(sessionPortRangeSize))
        }

        let spawnPolicy = spawnPolicyProvider.currentSpawnPolicy()
        let claudeHooksEnabled = spawnPolicy.claudeHooksEnabled
        if !claudeHooksEnabled {
            setManagedEnvironmentValue("BMUX_CLAUDE_HOOKS_DISABLED", "1")
        }
        ...
        if !spawnPolicy.cursorHooksEnabled {
            setManagedEnvironmentValue("BMUX_CURSOR_HOOKS_DISABLED", "1")
        }
        if !spawnPolicy.geminiHooksEnabled {
            setManagedEnvironmentValue("BMUX_GEMINI_HOOKS_DISABLED", "1")
        }
        if !spawnPolicy.kiroHooksEnabled {
            setManagedEnvironmentValue("BMUX_KIRO_HOOKS_DISABLED", "1")
        }
        setManagedEnvironmentValue("BMUX_KIRO_NOTIFICATION_LEVEL", spawnPolicy.kiroNotificationLevel)
        if !spawnPolicy.ampHooksEnabled {
            setManagedEnvironmentValue("BMUX_AMP_HOOKS_DISABLED", "1")
        }
        ...
        if let claudeShim {
            setManagedEnvironmentValue("BMUX_CLAUDE_WRAPPER_SHIM", claudeShim.executablePath)
            setManagedEnvironmentValue("BMUX_CLAUDE_WRAPPER_SHIM_ROOT", claudeShim.directoryPath)
            ...
            // session resolves the codex executable through BMUX_CODEX_WRAPPER_SHIM
            // (see AgentResumeArgv.codexWrapperShellExecutableToken), so without
            // this the resumed codex bypasses bmux-codex-wrapper and loses its
            ...
            if let codexShim = claudeShim.codexCommandShim {
                setManagedEnvironmentValue("BMUX_CODEX_WRAPPER_SHIM", codexShim.executablePath)
                setManagedEnvironmentValue("BMUX_CODEX_WRAPPER_SHIM_ROOT", codexShim.directoryPath)
            }
            let currentPath = env["PATH"]
                ?? getenv("PATH").map { String(cString: $0) }
                ?? ProcessInfo.processInfo.environment["PATH"]
        ...
        if spawnPolicy.shellIntegrationEnabled,
           ...
           Self.shellIntegrationDirectoryExists(integrationDir) {
            setManagedEnvironmentValue("BMUX_SHELL_INTEGRATION", "1")
            setManagedEnvironmentValue("BMUX_SHELL_INTEGRATION_DIR", integrationDir)
