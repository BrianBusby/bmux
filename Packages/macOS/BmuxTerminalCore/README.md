...
The terminal domain's core leaf: pure and Sendable terminal logic with no view dependency, lifted out of the app target's `GhosttyTerminalView.swift`. Higher terminal packages (engine, surface model, surface views) and the app depend on this; it depends only on GhosttyKit, `BmuxTerminalCopyMode`, and the DEBUG-only event log.

## Layout

- `Interop/` — `GhosttyRuntimeCInterop`, the one sanctioned seam for `@_silgen_name` libghostty bindings.
- `KeyEvents/` — `ghostty_input_action_e.modifierActionForFlagsChanged` (flagsChanged press/release resolution) and the `NSEvent.ModifierFlags` adapter for `BmuxTerminalCopyMode`.
