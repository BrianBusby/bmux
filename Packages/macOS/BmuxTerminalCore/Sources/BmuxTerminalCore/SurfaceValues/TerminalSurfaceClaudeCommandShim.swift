...
/// resolves to the bmux wrapper; both paths are exported to the shell as
/// `BMUX_CLAUDE_WRAPPER_SHIM` / `BMUX_CLAUDE_WRAPPER_SHIM_ROOT`.
public struct TerminalSurfaceClaudeCommandShim: Equatable, Sendable {
    ...
    /// directory, if the bundled `bmux-codex-wrapper` was available. Carried
    /// alongside the claude shim so the runtime surface creation can export
    /// `BMUX_CODEX_WRAPPER_SHIM` into the managed terminal environment (the
    /// same way it exports `BMUX_CLAUDE_WRAPPER_SHIM`), which a resumed codex
    /// session needs to route its `codex resume` through the wrapper and keep
    /// bmux hooks. `nil` when the codex wrapper was absent.
