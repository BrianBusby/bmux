...
public import BmuxFoundation
public import Foundation

/// The resolved bmux terminal configuration: fonts, colors, theme, scrollback,
/// and sidebar appearance parsed from the user's ghostty config files plus
/// bmux's managed defaults.
///
...
/// resolves themes by light/dark color scheme, and folds in bmux's managed
...
public struct GhosttyConfig {
    ...
    public static let bmuxDefaultLightThemeName = "Apple System Colors Light"
    /// Native fallback dark theme name used for fresh installs before the user
    /// has chosen terminal colors.
    public static let bmuxDefaultDarkThemeName = "Apple System Colors"
    ...
    public static let defaultSidebarFontSize = CGFloat(BmuxGhosttyConfigSettingEditor.defaultSidebarFontSize)
    /// The minimum sidebar font size the parser will clamp to.
    public static let minSidebarFontSize = CGFloat(BmuxGhosttyConfigSettingEditor.minSidebarFontSize)
    /// The maximum sidebar font size the parser will clamp to.
    public static let maxSidebarFontSize = CGFloat(BmuxGhosttyConfigSettingEditor.maxSidebarFontSize)
    /// The default surface tab-bar font size, in points.
    public static let defaultSurfaceTabBarFontSize = CGFloat(BmuxGhosttyConfigSettingEditor.defaultSurfaceTabBarFontSize)
    /// The minimum surface tab-bar font size the parser will clamp to.
    public static let minSurfaceTabBarFontSize = CGFloat(BmuxGhosttyConfigSettingEditor.minSurfaceTabBarFontSize)
    /// The maximum surface tab-bar font size the parser will clamp to.
    public static let maxSurfaceTabBarFontSize = CGFloat(BmuxGhosttyConfigSettingEditor.maxSurfaceTabBarFontSize)
    ...
    /// Creates a config with bmux's built-in default appearance, before any
    ...
    private static func bmuxConfigPaths(
        ...
        return BmuxGhosttyConfigPathResolver().loadConfigURLs(
    ...
    static func loadFromDisk(preferredColorScheme: ColorSchemePreference) -> GhosttyConfig {
        ...
        configPaths.append(contentsOf: bmuxConfigPaths())
    ...
    /// Applies bmux's managed default appearance (when the user has not chosen a
    ...
    /// Applies bmux's managed default theme for `preferredColorScheme` by parsing
    ...
    public mutating func applyBmuxDefaultAppearance(
        environment: [String: String],
        bundleResourceURL: URL?,
        preferredColorScheme: ColorSchemePreference
    ) {
        parse(
            Self.bmuxDefaultThemeConfigContents(
                preferredColorScheme: preferredColorScheme,
                environment: environment,
                bundleResourceURL: bundleResourceURL
            )
    ...
    /// The bmux managed default theme name for `preferredColorScheme`.
    public static func bmuxDefaultThemeName(preferredColorScheme: ColorSchemePreference) -> String {
        switch preferredColorScheme {
        case .light:
            return bmuxDefaultLightThemeName
        case .dark:
            return bmuxDefaultDarkThemeName
        }
    }

    /// The bmux managed default theme's config contents for `preferredColorScheme`,
    /// read from the resolved theme file when available, otherwise a built-in
    /// fallback palette.
    public static func bmuxDefaultThemeConfigContents(
        preferredColorScheme: ColorSchemePreference,
        environment: [String: String] = ProcessInfo.processInfo.environment,
        bundleResourceURL: URL? = Bundle.main.resourceURL
    ) -> String {
        if let url = bmuxDefaultThemeConfigURL(
            preferredColorScheme: preferredColorScheme,
            environment: environment,
            bundleResourceURL: bundleResourceURL
        ...
        return bmuxDefaultFallbackConfigContents(preferredColorScheme: preferredColorScheme)
    }

    /// The on-disk URL of the bmux managed default theme for `preferredColorScheme`,
    /// or `nil` when no candidate theme file exists.
    public static func bmuxDefaultThemeConfigURL(
        preferredColorScheme: ColorSchemePreference,
        environment: [String: String] = ProcessInfo.processInfo.environment,
        bundleResourceURL: URL? = Bundle.main.resourceURL
    ) -> URL? {
        let themeName = bmuxDefaultThemeName(preferredColorScheme: preferredColorScheme)
    ...
    private static func bmuxDefaultFallbackConfigContents(
        preferredColorScheme: ColorSchemePreference
    ) -> String {
        switch preferredColorScheme {
        case .light:
    ...
    public mutating func parse(
        ...
        for line in lines {
            ...
            // ignored, matching `BmuxGhosttyConfigSettingEditor.parsedSetting`.
    ...
    /// directives that determine whether bmux should apply its managed default
    ...
    public struct UserAppearanceConfigSummary {
        ...
        /// Whether bmux should apply its managed default appearance: true unless
        ...
        /// (https://github.com/manaflow-ai/bmux/issues/7161).
    ...
    /// Whether bmux should inject its managed default appearance: true unless
    ...
    public static func clampedSidebarFontSize(_ value: CGFloat) -> CGFloat {
        CGFloat(BmuxGhosttyConfigSettingEditor().clampedSidebarFontSize(Double(value)))
    }
    ...
    public static func clampedSurfaceTabBarFontSize(_ value: CGFloat) -> CGFloat {
        CGFloat(BmuxGhosttyConfigSettingEditor().clampedSurfaceTabBarFontSize(Double(value)))
    }
    ...
    /// `.light` and `nil` for `.dark`. bmux injects a resolved plain `theme = X`
    ...
    /// https://github.com/manaflow-ai/bmux/issues/3459). Injecting for an unset
    ...
    /// The ordered list of filesystem paths bmux searches for the given theme
    ...
    public static func themeSearchPaths(
        ...
        for appSupportDirectory in BmuxApplicationSupportDirectories(environment: environment).userDirectories {
            appendUniquePath(
                appSupportDirectory
                    .appendingPathComponent(BmuxGhosttyConfigPathResolver.releaseBundleIdentifier, isDirectory: true)
