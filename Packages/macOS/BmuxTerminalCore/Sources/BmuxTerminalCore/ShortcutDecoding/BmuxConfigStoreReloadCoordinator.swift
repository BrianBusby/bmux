/// Orchestrates reloading every per-window bmux configuration store and refreshing
/// window titles, as the app delegate's `reloadBmuxConfigStores(source:)` did.
///
/// Owned by the app delegate and given a weak reference to it through
/// `BmuxConfigStoreReloadEnvironment`, so the delegate retaining the coordinator
...
@MainActor
public final class BmuxConfigStoreReloadCoordinator {
    private weak var environment: (any BmuxConfigStoreReloadEnvironment)?
    ...
    public init(
        environment: any BmuxConfigStoreReloadEnvironment,
