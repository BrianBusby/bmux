...
private func bmux_ghostty_surface_clear_selection(_ surface: ghostty_surface_t) -> Bool

/// The one sanctioned seam for libghostty symbols that are linked by name
/// rather than imported through the GhosttyKit header.
///
/// bmux's libghostty fork exports a small number of symbols that are not part
...
public struct GhosttyRuntimeCInterop {
    ...
    ///
    /// Mirrors `ghostty_surface_clear_selection` from the bmux libghostty
    ...
    public static func clearSelection(_ surface: ghostty_surface_t) -> Bool {
        bmux_ghostty_surface_clear_selection(surface)
    }
