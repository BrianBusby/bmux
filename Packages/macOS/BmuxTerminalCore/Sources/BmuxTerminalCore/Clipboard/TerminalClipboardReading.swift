...
///
/// Implemented by `TerminalPasteboardService` in `BmuxTerminalServices` and
...
public protocol TerminalClipboardReading: AnyObject, Sendable {
    ...
    /// locations bmux does not support.
    ...
    /// applying bmux's flavor-priority rules (file URLs, image-only guard,
