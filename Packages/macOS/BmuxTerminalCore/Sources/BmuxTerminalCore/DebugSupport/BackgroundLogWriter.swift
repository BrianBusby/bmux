...
/// Asynchronous, append-only sink for bmux's opt-in background diagnostics log.
///
...
private func consumeBackgroundLog(
    ...
    for await entry in stream {
        ...
        let line =
            "\(formatter.string(from: entry.date)) seq=\(sequence) t+\(String(format: "%.3f", entry.uptimeMs))ms thread=\(entry.threadLabel) frame60=\(frame60) frame120=\(frame120) bmux bg: \(entry.message)\n"
