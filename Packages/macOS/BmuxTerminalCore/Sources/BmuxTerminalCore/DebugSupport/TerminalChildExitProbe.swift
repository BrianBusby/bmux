...
/// `BMUX_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP=1` and a probe path; the surface
...
public struct TerminalChildExitProbe: Sendable {
    ...
    public func probePath() -> String? {
        guard environment["BMUX_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] == "1",
              let path = environment["BMUX_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"],
              !path.isEmpty else {
            return nil
        }
        return path
