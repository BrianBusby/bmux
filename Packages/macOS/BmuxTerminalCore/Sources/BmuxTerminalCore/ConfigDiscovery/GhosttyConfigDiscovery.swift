...
/// Resolves where bmux's Ghostty configuration lives and what it should inject,
...
/// includes) for the font and appearance directives that drive bmux's
...
public struct GhosttyConfigDiscovery {
    ...
    /// own identifier.
    public static let releaseBundleIdentifier = BmuxGhosttyConfigPathResolver.releaseBundleIdentifier

    private let fileReader: any GhosttyConfigFileReading
    private let fontProbe: any GhosttyFontProbing
    private let pathResolver: BmuxGhosttyConfigPathResolver
    ...
    public init(
        ...
        pathResolver: BmuxGhosttyConfigPathResolver = BmuxGhosttyConfigPathResolver()
    ) {
    ...
    /// already covers the ranges bmux would otherwise auto-map.
    ...
    /// Returns only the CJK mappings bmux should auto-inject after respecting
    ...
    /// application-support config paths bmux may load at runtime.
    ...
    /// Whether bmux should inject its managed CJK `font-codepoint-map` fallback.
    ...
    /// Whether bmux should apply its managed default appearance across the
    ...
    /// Computes the resolved plain-theme override bmux must inject when the
    ...
    public func conditionalThemeOverrideConfigContents(
        ...
        // syntax, even when both sides resolve to the same theme. `bmux themes
        ...
        // https://github.com/manaflow-ai/bmux/issues/3459. Only override sides the
    ...
    /// summarizes the font directives relevant to bmux's injected CJK fallback.
    ...
    /// `config-file` processing, including native Ghostty, legacy, and bmux
    ...
    public func loadedGhosttyConfigScanPaths(
        ...
        let appSupportConfigURLs = bmuxAppSupportConfigURLs(
    ...
    /// Whether bmux should load the legacy `config` file because the new
    ...
    /// Resolves the bmux Application Support Ghostty config file URLs for the
    /// running build, via the shared ``BmuxGhosttyConfigPathResolver``.
    public func bmuxAppSupportConfigURLs(
