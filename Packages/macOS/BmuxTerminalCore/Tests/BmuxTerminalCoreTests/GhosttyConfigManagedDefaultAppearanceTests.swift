...
import Foundation
import Testing
@testable import BmuxTerminalCore

/// Regression coverage for https://github.com/manaflow-ai/bmux/issues/7161.
///
/// bmux applies its managed default terminal theme ("Apple System Colors")
...
/// directive made bmux skip the managed theme entirely, so Ghostty silently
...
@Suite struct GhosttyConfigManagedDefaultAppearanceTests {
    private func withTempConfigDir(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-7161-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeManagedThemesRoot(in dir: URL) throws -> URL {
        ...
        cursor-color = #778899
        """
        for name in [GhosttyConfig.bmuxDefaultDarkThemeName, GhosttyConfig.bmuxDefaultLightThemeName] {
    ...
    @Test func explicitThemeDirectiveSkipsManagedDefaultBase() throws {
        ...
        try loadResolvedConfig(userConfig: "theme = Bmux Nonexistent Theme 7161\n") { config in
            #expect(config.theme == "Bmux Nonexistent Theme 7161")
