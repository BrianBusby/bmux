...
@MainActor
private final class FakeConfigStore: BmuxConfigStoreReloading {
    private(set) var loadAllCount = 0
    func loadAll() { loadAllCount += 1 }
}

@MainActor
private final class FakeEnvironment: BmuxConfigStoreReloadEnvironment {
    var stores: [FakeConfigStore]
    private(set) var titleRefreshCount = 0

    init(stores: [FakeConfigStore]) { self.stores = stores }

    var reloadableConfigStores: [any BmuxConfigStoreReloading] { stores }
    func refreshWindowTitlesAfterConfigReload() { titleRefreshCount += 1 }
}

@MainActor
@Suite struct BmuxConfigStoreReloadCoordinatorTests {
    @Test func reloadsEachDistinctStoreOnceAndRefreshesTitles() {
        ...
        let coordinator = BmuxConfigStoreReloadCoordinator(environment: env) { source, count in
    ...
    @Test func reloadWithNoStoresStillRefreshesTitles() {
        ...
        var reportedCount: Int?
        let coordinator = BmuxConfigStoreReloadCoordinator(environment: env) { _, count in
            reportedCount = count
        }
