...
@Suite struct TerminalChildExitProbeTests {
    @Test func loadReturnsEmptyPayloadForMissingFile() {
        #expect(TerminalChildExitProbe().load(at: "/tmp/bmux-termcore-missing-\(UUID().uuidString)") == [:])
    }

    @Test func loadRoundTripsJSONPayload() throws {
        let path = NSTemporaryDirectory() + "bmux-termcore-probe-\(UUID().uuidString).json"
    ...
    @Test func writeIsInertWithoutEnvironmentOptIn() {
        // The test process does not set BMUX_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP,
