...
@Suite struct TerminalLinkRouterTests {
    ...
    @Test func resolvesHostlessHTTPSAsExternal() throws {
        let target = try #require(router.resolveOpenURLTarget("https:///tmp/bmux.txt"))
        guard case let .external(url) = target else {
