...
struct SurfaceTabBarFontSizeConfigTests {
    ...
    @Test func editorParsesLastSurfaceTabBarValueAndClamps() {
        ...
        #expect(BmuxGhosttyConfigSettingEditor().parsedSurfaceTabBarFontSize(in: contents)
            == BmuxGhosttyConfigSettingEditor.maxSurfaceTabBarFontSize)
    }

    @Test func editorReturnsNilWhenSurfaceTabBarValueAbsent() {
        #expect(BmuxGhosttyConfigSettingEditor().parsedSurfaceTabBarFontSize(in: "sidebar-font-size = 14") == nil)
    }

    @Test func editorFormatsSurfaceTabBarValueTrimmingTrailingZeros() {
        #expect(BmuxGhosttyConfigSettingEditor().formattedSurfaceTabBarFontSize(12) == "12")
        #expect(BmuxGhosttyConfigSettingEditor().formattedSurfaceTabBarFontSize(12.5) == "12.5")
    }

    @Test func editorWriteSettingRoundTripsSurfaceTabBarValue() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bmux-surface-tab-bar-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(BmuxGhosttyConfigSettingEditor().parsedSurfaceTabBarFontSize(in: contents) == 13)
    }
}
