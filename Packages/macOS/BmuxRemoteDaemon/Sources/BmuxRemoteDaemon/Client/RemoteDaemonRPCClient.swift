...
/// Synchronous JSON-RPC client for a bmuxd-remote daemon over one of three
...
public final class RemoteDaemonRPCClient: @unchecked Sendable {
    ...
    static let bakedVMDaemonSocketPath = "/run/bmuxd-remote.sock"
    ...
    let transportKeepaliveQueue = DispatchQueue(label: "com.bmux.remote-ssh.daemon-rpc.keepalive.\(UUID().uuidString)")
    let writeQueue = DispatchQueue(label: "com.bmux.remote-ssh.daemon-rpc.write.\(UUID().uuidString)")
    let stateQueue = DispatchQueue(label: "com.bmux.remote-ssh.daemon-rpc.state.\(UUID().uuidString)")
    let cliRequestQueue = DispatchQueue(label: "com.bmux.remote-ssh.daemon-rpc.cli.\(UUID().uuidString)", qos: .utility, attributes: .concurrent)
    ...
    ///   - remotePath: Remote path of the bmuxd-remote binary (stdio
    ///     transport only).
    ...
    public func start() throws {
        ...
        do {
            ...
            guard missingCapabilities.isEmpty else {
                throw NSError(domain: "bmux.remote.daemon.rpc", code: 2, userInfo: [
