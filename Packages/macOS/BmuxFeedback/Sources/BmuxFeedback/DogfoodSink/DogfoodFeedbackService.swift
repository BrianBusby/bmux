...
/// `~/.cache/bmux-dogfood-feedback/<ISO8601>_<shortid>/` (a `bundle.json`
...
public struct DogfoodFeedbackService: Sendable {
    ...
    public init(
        ...
        self.cacheRoot = cacheRoot ?? FileManager.default.homeDirectoryForCurrentUser
            ...
            .appendingPathComponent("bmux-dogfood-feedback", isDirectory: true)
        self.now = now
    }

    /// The privileged feedback domain. Mirrors `isManaflowEmail` in
    /// `BmuxMobileShellModel` (the phone's routing source of truth) but is
    ...
    private static func writeBundle(
        ...
        do {
            ...
            let manifest: [String: Any] = [
                "schema": "bmux.dogfood.feedback.v1",
