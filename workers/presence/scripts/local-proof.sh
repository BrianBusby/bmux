...
# Optional explicit team to scope every request to (X-Bmux-Team-Id); both
...
if [ -n "${PROOF_TEAM_ID:-}" ]; then
  printf 'header = "x-bmux-team-id: %s"\n' "$PROOF_TEAM_ID" >>"$AUTH_CFG"
fi
AUTH=(-K "$AUTH_CFG")
...
PRESENCE_TOKEN="$ACCESS_TOKEN" PRESENCE_TEAM_ID="${PROOF_TEAM_ID:-}" bun -e '
  ...
  if (process.env.PRESENCE_TEAM_ID) headers["x-bmux-team-id"] = process.env.PRESENCE_TEAM_ID;
