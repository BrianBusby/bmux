...
accept optional team scoping via `X-Bmux-Team-Id` or `?teamId=` (must be a
...
A dev instance runs as `bmux-presence-dev` on the team Cloudflare account
(the same one the regatta subrouter deploys to), configured with the dev
Stack project's Worker secrets:

```
https://bmux-presence-dev.debussy.workers.dev
```
...
> [!IMPORTANT]
> Use `--config wrangler.dev.toml`, NOT `--name bmux-presence-dev`. The default
> `wrangler.toml` carries the **production** `presence.bmux.dev` custom domain;
> `--name` only overrides the worker name, so it inherits that route and STEALS
> the production domain (detaching it from `bmux-presence`, which breaks prod
...
> own `*.workers.dev` URL.
Point a dev Mac build at it with the `BMUX_PRESENCE_BASE_URL` env override or
...
`bmux-presence-dev` is a **single shared instance** — last deploy wins, and an
...
```
./scripts/deploy-dev.sh           # deploys bmux-presence-dev-<your-id>
```

Each `bmux-presence-dev-<slug>` is a separate worker with its **own Durable
...
```
export BMUX_PRESENCE_BASE_URL=https://bmux-presence-dev-<slug>.<subdomain>.workers.dev
```
...
- **Mac:** `BMUX_PRESENCE_BASE_URL` env, or `defaults write <tagged-bundle>
...
- **iOS:** a tapped device app sees no shell env, so the override is read from the
  app's **Info.plist key `BMUXPresenceBaseURL`** (and from `presenceServiceURL`
  UserDefaults / the `BMUX_PRESENCE_BASE_URL` launch env). Resolution precedence:
  env → UserDefaults → Info.plist → Debug default. `ios/scripts/reload.sh` bakes
  `$BMUX_PRESENCE_BASE_URL` into that Info.plist key (next to `BMUXDevTag`, via the
  `BMUX_PRESENCE_BASE_URL` build setting in `ios/Config/Shared.xcconfig` +
...
Leave `BMUX_PRESENCE_BASE_URL` unset to use the shared `bmux-presence-dev`
