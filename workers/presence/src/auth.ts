...
// from `X-Bmux-Team-Id` / `?teamId=` with a membership check, defaulting to the
...
/** Requested team from `X-Bmux-Team-Id` (or legacy billing header) or the
...
export function requestedTeamIdFromRequest(request: Request): string | null {
  const fromHeader =
    normalized(request.headers.get("x-bmux-team-id")) ??
    normalized(request.headers.get("x-bmux-billing-team-id"));
