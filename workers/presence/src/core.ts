// Pure presence state machine for the bmux device presence service.
//
...
export interface PresenceInstance {
  /** bmux-generated persisted device UUID (same identity as the Aurora
   * `devices.device_uuid` registry column). */
  deviceId: string;
  /** Build tag of the running bmux app instance ("default" for stable). */
  tag: string;
