// bmux device presence service — worker entry.
//
...
// optional `X-Bmux-Team-Id` / `?teamId=` team scoping, verified in auth.ts the
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/healthz") {
      return json({ ok: true, service: "bmux-presence" });
    }
    ...
    if (url.pathname === "/v1/sync/paired-macs") {
      ...
      const team = await resolveTeamOr403(request, env);
      if (!team.ok) return team.response;
      const rawClientScope = request.headers.get("x-bmux-client-scope");
