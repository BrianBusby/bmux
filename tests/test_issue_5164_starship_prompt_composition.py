#!/usr/bin/env python3
"""
Regression coverage for https://github.com/manaflow-ai/bmux/issues/5164.

Using Starship as the bash prompt inside a bmux session breaks: Starship's
status line stops updating after the first command. The root cause is the local
macOS bash bootstrap that bmux injects as ``PROMPT_COMMAND`` (see
``Resources/shell-integration/bmux-bash-bootstrap.bash``). The user's startup
...
BOOTSTRAP = REPO_ROOT / "Resources/shell-integration/bmux-bash-bootstrap.bash"
        ...
        env.update(
            {
                ...
                "BMUX_BOOTSTRAP_FILE": str(lean_bootstrap),
                "BMUX_SHELL_INTEGRATION": "1",
                "BMUX_SHELL_INTEGRATION_DIR": str(INTEGRATION_DIR),
                "BMUX_LOAD_GHOSTTY_BASH_INTEGRATION": "0",
                "GHOSTTY_RESOURCES_DIR": "",
                "BMUX_TAB_ID": "tab-test",
                "BMUX_PANEL_ID": "panel-test",
                # No unix socket: _bmux_prompt_command's body early-returns, which
                # keeps this test focused on PROMPT_COMMAND/PS1 composition.
                "BMUX_SOCKET_PATH": "",
                "STARSHIP_COUNTER_FILE": str(tmp_path / "counter"),
                "BMUX_CD_TARGET": str(cd_target),
                "BMUX_WITH_STARSHIP": "1" if with_starship else "0",
            }
        )
...
def test_starship_precmd_survives_under_bmux_bash_bootstrap() -> None:
    ...
    # bmux must compose, not clobber: after the one-shot bootstrap runs, the
    ...
    for i in (1, 2, 3):
        ...
        assert "starship_precmd" in pc, (
            ...
            f"bmux took exclusive ownership instead of composing. PROMPT_COMMAND=<{pc}>"
            + debug
        )
        assert "_bmux_prompt_command" in pc, (
            f"bmux's own prompt hook missing from PROMPT_COMMAND at prompt {i}: <{pc}>"
            + debug
        )
...
def test_plain_bash_bootstrap_installs_bmux_prompt_command() -> None:
    """No-regression guard: with no user PROMPT_COMMAND hook, the bootstrap must
    still install _bmux_prompt_command (and remove its own marker)."""
    assert BOOTSTRAP.exists(), f"missing bootstrap file: {BOOTSTRAP}"
    ...
    for i in (1, 2, 3):
        ...
        # stays green if bmux-bash-integration.bash ever tweaks PROMPT_COMMAND.
        assert "_bmux_prompt_command" in pc, (
            f"plain-bash bootstrap did not install _bmux_prompt_command at prompt {i}: <{pc}>"
            + debug
        )
        assert "__bmux_bash_bootstrap_marker__" not in pc, (
...
if __name__ == "__main__":
    test_starship_precmd_survives_under_bmux_bash_bootstrap()
    test_plain_bash_bootstrap_installs_bmux_prompt_command()
    print("PASS: bmux bash bootstrap composes with (and without) user prompt hooks")
