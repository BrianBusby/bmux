    ...
    env.update(
        {
            ...
            "BMUX_TEST_LOG": str(log_path),
            "BMUX_TEST_STATE": str(state_path),
            "BMUX_TEST_BIN_DIR": str(bin_dir),
        }
    )
    command = f"""
source "{integration_path}"
PATH="$BMUX_TEST_BIN_DIR:$PATH"
hash -r 2>/dev/null || true
: > "{log_path}"
rm -f "{state_path}"
_BMUX_TTY_NAME={tty_name}
_BMUX_TTY_REPORTED=0
_bmux_report_tty_once || true
first="${{_BMUX_TTY_REPORTED}}:$(wc -l < "{log_path}" | tr -d ' ')"
_bmux_report_tty_once || true
second="${{_BMUX_TTY_REPORTED}}:$(wc -l < "{log_path}" | tr -d ' ')"
printf '%s\\n' "$first|$second"
""".strip()
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-ssh-relay-report-tty-retry-") as td:
        tmp = Path(td)
        bin_dir = tmp / "bin"
        ...
        _write_executable(
            ...
            """#!/bin/sh
count=0
if [ -r "$BMUX_TEST_STATE" ]; then
    count=$(cat "$BMUX_TEST_STATE")
fi
count=$((count + 1))
printf '%s' "$count" > "$BMUX_TEST_STATE"
printf '%s\n' "$*" >> "$BMUX_TEST_LOG"
if [ "$count" -eq 1 ]; then
...
""",
        ...
        cases = [
            (
                "zsh",
                ["-f", "-c"],
                SHELL_DIR / "bmux-zsh-integration.zsh",
                "ttys777",
            ),
            (
                "bash",
                ["--noprofile", "--norc", "-c"],
                SHELL_DIR / "bmux-bash-integration.bash",
                "ttys888",
            ),
        ]
