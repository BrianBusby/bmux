...
overlay used for focus tracking. Fix: BmuxWebView.mouseDown posts a notification
...
def _marker(name: str) -> Path:
    return MARKER_DIR / f"bmux_mwf_{name}_{os.getpid()}"
...
def _verify_responsive(c: bmux, marker: Path, surface_idx: int, retries: int = 3) -> bool:
...
def _wait_terminal_in_window(c: bmux, surface_idx: int, timeout: float = 5.0) -> bool:
    start = time.time()
    while time.time() - start < timeout:
        try:
...
def test_multi_workspace_terminal_responsive(c: bmux) -> None:
    """
...
def test_three_workspaces_non_last_responsive(c: bmux) -> None:
    """
...
def test_rapid_workspace_switching_preserves_focus(c: bmux) -> None:
    """
...
def test_browser_panel_focus_and_return(c: bmux) -> None:
    """
    ...
    if not surfaces:
        raise bmuxError("No surfaces after new_workspace")
...
def test_browser_focus_across_workspaces(c: bmux) -> None:
    """
    ...
    with bmux() as c:
        ...
        for name, test_fn in tests:
            ...
            try:
                ...
                print("PASS")
                passed += 1
            except (AssertionError, bmuxError) as e:
                print(f"FAIL: {e}")
                failed += 1
