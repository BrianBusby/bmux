...
def _write_fake_codex(fake_bin_dir: Path) -> None:
    ...
    fake_codex.write_text(
        "#!/bin/sh\n"
        "printf 'BMUX_FAKE_CODEX_RESUME:%s\\n' \"$*\"\n",
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    if not cli_path.exists():
        print(f"SKIP: bundled bmux CLI not found at {cli_path}")
        return 0

    bundle_id = _bundle_id(app_path)
    socket_path = Path(f"/tmp/bmux-restore-session-codex-{bundle_id.replace('.', '-')}.sock")
    snapshot = _snapshot_path(bundle_id)
    previous_snapshot = _snapshot_path(bundle_id, suffix="-previous")

    failures: list[str] = []

    with tempfile.TemporaryDirectory(prefix="bmux-restore-session-codex-") as td:
        ...
        try:
            ...
            try:
                ...
                restore_env["BMUX_AGENT_HOOK_STATE_DIR"] = str(hook_state_dir)
                ...
                marker = "BMUX_FAKE_CODEX_RESUME:resume codex-session-restore-2923"
