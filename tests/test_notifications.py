...
def wait_for_notifications(client: bmux, expected: int, timeout: float = 2.0) -> list[dict]:
    start = time.time()
...
def wait_for_flash_count(client: bmux, surface: str, minimum: int = 1, timeout: float = 2.0) -> int:
...
def wait_for_notification_read(
    client: bmux, surface_id: str, timeout: float = 4.0
) -> Optional[dict]:
...
def ensure_two_surfaces(client: bmux) -> list[tuple[int, str, bool]]:
...
def focused_surface_index(client: bmux) -> int:
    surfaces = client.list_surfaces()
...
def send_osc(client: bmux, sequence: str, surface: Optional[int] = None) -> None:
...
def test_clear_prior_notifications(client: bmux) -> TestResult:
...
def test_suppress_when_focused(client: bmux) -> TestResult:
...
def test_not_suppressed_when_inactive(client: bmux) -> TestResult:
...
def test_kitty_notification_simple(client: bmux) -> TestResult:
...
def test_kitty_notification_chunked(client: bmux) -> TestResult:
...
def test_rxvt_notification_osc777(client: bmux) -> TestResult:
...
def test_mark_read_on_focus_change(client: bmux) -> TestResult:
...
def test_mark_read_on_app_active(client: bmux) -> TestResult:
...
def test_mark_read_on_tab_switch(client: bmux) -> TestResult:
...
def test_flash_on_tab_switch(client: bmux) -> TestResult:
...
def test_focus_on_notification_click(client: bmux) -> TestResult:
...
def test_restore_focus_on_tab_switch(client: bmux) -> TestResult:
...
def test_clear_on_tab_close(client: bmux) -> TestResult:
