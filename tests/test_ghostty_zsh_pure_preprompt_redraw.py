...
typeset -g BMUX_TOP='%F{4}%~%f'
typeset -g BMUX_LAST_PROMPT=''
typeset -gi BMUX_ASYNC_DONE=0
typeset -g BMUX_ASYNC_FD=''
...
bmux_render_prompt() {
  ...
  PROMPT="${BMUX_TOP}${prompt_newline}${cleaned_ps1:-%F{5}❯%f }"

  local expanded_prompt="${(S%%)PROMPT}"
  if [[ ${1:-} == precmd ]]; then
    print
  elif [[ $BMUX_LAST_PROMPT != $expanded_prompt ]]; then
    zle && zle .reset-prompt
  fi
  typeset -g BMUX_LAST_PROMPT=$expanded_prompt
}

bmux_async_ready() {
  emulate -L zsh
  local fd="${1:-$BMUX_ASYNC_FD}"
  if [[ -n $fd ]]; then
    zle -F "$fd"
    exec {fd}<&-
  fi
  ...
  (( BMUX_ASYNC_DONE )) && return
  BMUX_ASYNC_DONE=1
  BMUX_TOP='%F{4}%~%f %F{242}main%f%F{218}*%f %F{6}⇣⇡%f'
  bmux_render_prompt async
}

precmd() {
  BMUX_ASYNC_DONE=0
  bmux_render_prompt precmd
}

bmux_line_init() {
  if (( !BMUX_ASYNC_DONE )) && [[ -z $BMUX_ASYNC_FD ]]; then
    exec {BMUX_ASYNC_FD}< <(
      sleep 0.05
      printf 'ready\n'
    )
    zle -F "$BMUX_ASYNC_FD" bmux_async_ready
  fi
}

zle -N zle-line-init bmux_line_init
PROMPT='%F{5}❯%f '
""".lstrip()
...
def _capture_session(
    *,
    ...
    base = Path(tempfile.mkdtemp(prefix="bmux_ghostty_pure_preprompt_"))
    try:
        home = base / "home"
