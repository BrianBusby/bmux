...
release = module.load_release("manaflow-ai/bmux", "nightly")
assert release == {"assets": []}
module.delete_assets("manaflow-ai/bmux", [module.ReleaseAsset(asset_id=123, name="old.dmg", build=1)])
assert requests == [
    ("GET", "https://api.example.test/repos/manaflow-ai/bmux/releases/tags/nightly"),
    ("DELETE", "https://api.example.test/repos/manaflow-ai/bmux/releases/assets/123"),
]
...
module.delete_assets("manaflow-ai/bmux", [module.ReleaseAsset(asset_id=456, name="older.dmg", build=1)])
assert gh_calls == [
    ["gh", "api", "-X", "DELETE", "repos/manaflow-ai/bmux/releases/assets/456"],
]
PY
