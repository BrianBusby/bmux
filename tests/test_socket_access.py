...
In bmuxOnly mode (default), only processes descended from the bmux
app process can connect. External processes (e.g., SSH) are rejected.

Test strategy:
  Phase 1: bmuxOnly — external processes get rejected
  Phase 2: bmuxOnly — internal process CAN connect (inject via shell rc)
...
def _preferred_worktree_slug():
    env_slug = os.environ.get("BMUX_TAG") or os.environ.get("BMUX_BRANCH_SLUG")
    if env_slug:
...
def _derived_app_candidates_for_current_worktree():
    project_path = os.path.realpath(os.path.join(os.getcwd(), "bmux.xcodeproj"))
    info_paths = glob.glob(os.path.expanduser(
        "~/Library/Developer/Xcode/DerivedData/bmux-*/info.plist"
    ))
    matches = []
...
def _find_app():
    explicit = os.environ.get("BMUX_APP_PATH")
...
def _find_cli(preferred_app_path: str = ""):
    ...
    if preferred_app_path:
        ...
        sibling = os.path.join(debug_dir, "bmux")
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/bmux"
    )))
    candidates.extend(glob.glob("/tmp/bmux-*/Build/Products/Debug/bmux"))
    candidates.extend(glob.glob("/private/tmp/bmux-*/Build/Products/Debug/bmux"))
...
def _launch_bmux(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    if os.path.exists(socket_path):
        try:
    ...
    env_args = []
    if mode:
        env_args = ["--env", f"BMUX_SOCKET_MODE={mode}"]
    launch_env = {
...
def test_internal_process_allowed(socket_path: str, app_path: str) -> TestResult:
    """
    Verify a bmux-spawned terminal process CAN connect in bmuxOnly mode.
    Inject a test via the shell rc file, then launch bmux in bmuxOnly mode.
    The shell (a descendant of bmux) runs the test on startup.
    """
    result = TestResult("Internal process can connect (bmuxOnly)")
    marker = os.path.join(tempfile.gettempdir(), f"bmux_internal_{os.getpid()}")
    hook_file = os.path.join(tempfile.gettempdir(), f"bmux_rc_hook_{os.getpid()}.sh")
...
""")
        ...
        # Kill existing bmux, launch in bmuxOnly mode (default)
        _kill_bmux(app_path)
        _launch_bmux(app_path, socket_path, mode="bmuxOnly")
        ...
        if content == "OK":
            result.success("Internal process pinged socket successfully in bmuxOnly mode")
        else:
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify BMUX_SOCKET_MODE=allowAll bypasses ancestry check."""
    result = TestResult("allowAll mode allows external")
    try:
        _kill_bmux(app_path)
        _launch_bmux(app_path, socket_path, mode="allowAll")
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    password = f"bmux-pass-{os.getpid()}"
    try:
        _kill_bmux(app_path)
        _launch_bmux(
            app_path,
            socket_path,
            mode="password",
            extra_env={"BMUX_SOCKET_PASSWORD": password}
        )
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find bmux CLI binary")
            return result
...
def run_tests():
    print("=" * 60)
    print("bmux Socket Access Control Tests")
    print("=" * 60)
    print()
    ...
    socket_path = f"/tmp/bmux-test-socket-access-{os.getpid()}.sock"
    try:
        os.unlink(socket_path)
    except OSError:
        pass
    ...
    # ── Phase 1: bmuxOnly — external rejection ──
    print("Phase 1: bmuxOnly mode — external rejection")
    print("-" * 50)

    # Ensure bmux is running in bmuxOnly mode
    _kill_bmux(app_path)
    print("  Launching bmux in bmuxOnly mode...")
    _launch_bmux(app_path, socket_path, mode="bmuxOnly")
    ...
    # ── Phase 2: bmuxOnly — internal process CAN connect ──
    print("Phase 2: bmuxOnly mode — internal process allowed")
    print("-" * 50)
    ...
    # ── Cleanup: leave bmux in bmuxOnly mode ──
    _kill_bmux(app_path)
    _launch_bmux(app_path, socket_path, mode="bmuxOnly")
