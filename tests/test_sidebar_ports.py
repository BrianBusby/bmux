...
Run with a tagged instance to avoid unix socket conflicts:
  BMUX_TAG=<tag> python3 tests/test_sidebar_ports.py
"""
...
def _start_external_server(base: Path, port: int) -> subprocess.Popen:
    """
    Start an http.server outside bmux and ensure it is actually listening.
...
def _start_agent_server(base: Path, port: int, pid_file: Path, log_file: Path) -> subprocess.Popen:
    """
    Start a long-lived "agent" shell outside bmux. The shell owns a child
...
def _wait_for_port(client: bmux, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
...
def _wait_for_port_absent(client: bmux, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
        state = _parse_sidebar_state(client.sidebar_state())
        raw = state.get("ports", "")
        if raw == "none" or not raw:
...
def _assert_port_absent_for_duration(client: bmux, port: int, duration: float = 6.0, interval: float = 0.15) -> None:
    """
...
def main() -> int:
    ...
    try:
        ...
        # Start a listening server outside bmux. A fresh tab should NOT show this port,
        ...
        with bmux() as client:
            ...
            client.send("echo bmux_ports_test\n")
    ...
    except (bmuxError, AssertionError, RuntimeError, ValueError) as e:
