...
# Catches issues like https://github.com/manaflow-ai/bmux/issues/110 where a race
# condition caused the cask to contain the SHA of a 404 page instead of the DMG.
set -euo pipefail

CASK_FILE="$(dirname "$0")/../homebrew-bmux/Casks/bmux.rb"

if [ ! -f "$CASK_FILE" ]; then
  echo "SKIP: homebrew-bmux submodule not initialized"
  exit 0
fi
