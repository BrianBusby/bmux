#!/usr/bin/env python3
"""
Regression test (#6447): `bmux claude-teams` grants the Claude trust-gate bypass
...
def run(cli_path: str, args: list[str], extra_env: dict[str, str]) -> tuple[str, str]:
    with tempfile.TemporaryDirectory(prefix="bmux-teams-trust-") as td:
        tmp = Path(td)
        real_bin = tmp / "bin"
        real_bin.mkdir()
        ...
        fake.write_text(
            ...
            'printf "%s" "${BMUX_CLAUDE_TEAMS_SANDBOXED-__UNSET__}" > "$L2"\n',
            encoding="utf-8",
        )
        fake.chmod(0o755)
        env = dict(os.environ)
...
def main() -> int:
    cli = resolve_bmux_cli()

    # Opted in: both the Claude env var and the bmux propagation marker are set.
    ...
    sandboxed, marker = run(
        cli,
        ["--version"],
        {"CLAUDE_CODE_SANDBOXED": "1", "BMUX_CLAUDE_TEAMS_SANDBOXED": "1"},
    )
    ...
    print("PASS: bmux claude-teams gates the trust bypass on the current opt-in and clears ambient markers")
    return 0
