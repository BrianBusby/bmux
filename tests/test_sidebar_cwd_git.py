...
Run with a tagged instance to avoid unix socket conflicts:
  BMUX_TAG=<tag> python3 tests/test_sidebar_cwd_git.py
"""
...
def _init_git_repo(repo: Path) -> None:
    ...
    _git(repo, "init")
    _git(repo, "config", "user.email", "bmux-test@example.com")
    _git(repo, "config", "user.name", "bmux-test")
...
def main() -> int:
    ...
    except (bmuxError, subprocess.CalledProcessError, AssertionError) as e:
