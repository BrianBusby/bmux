#!/usr/bin/env python3
"""
Regression test: `bmux claude-teams` injects the tmux-style auto-mode env.
"""
...
def run_claude_teams(
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-claude-teams-env-") as td:
        tmp = Path(td)
...
printf '%s\\n' "${BMUX_CLAUDE_TEAMS_SANDBOXED-__UNSET__}" > "$FAKE_SANDBOXED_MARKER_LOG"
command -v tmux > "$FAKE_TMUX_PATH_LOG"
printf '%s\\n' "${BMUX_CLAUDE_TEAMS_BMUX_BIN-__UNSET__}" > "$FAKE_BMUX_BIN_LOG"
printf '%s\\n' "$@" > "$FAKE_ARGV_LOG"
...
printf '%s\\n' "${BMUX_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_PATH_LOG"
printf '%s\\n' "${BMUX_SOCKET_PASSWORD-__UNSET__}" > "$FAKE_SOCKET_PASSWORD_LOG"
...
""",
        ...
        env["FAKE_BMUX_BIN_LOG"] = str(bmux_bin_log)
        ...
        if tmpdir is not None:
            env["TMPDIR"] = tmpdir
        explicit_socket_path = str(tmp / "explicit-bmux.sock")
        ...
        # The launcher records the opt-in in BMUX_CLAUDE_TEAMS_SANDBOXED so teammate
        ...
        if marker_value != "1":
            print(f"FAIL: expected BMUX_CLAUDE_TEAMS_SANDBOXED=1 opt-in marker, got {marker_value!r}")
            raise SystemExit(1)
        ...
        if tmux_path.startswith(str(real_bin)):
            print(f"FAIL: expected bmux tmux shim to shadow PATH, got {tmux_path!r}")
            raise SystemExit(1)

        bmux_bin_value = read_text(bmux_bin_log)
        if not bmux_bin_value or bmux_bin_value == "__UNSET__":
            print("FAIL: missing BMUX_CLAUDE_TEAMS_BMUX_BIN")
            raise SystemExit(1)

        if not os.path.exists(bmux_bin_value):
            print(f"FAIL: BMUX_CLAUDE_TEAMS_BMUX_BIN does not exist: {bmux_bin_value!r}")
            raise SystemExit(1)
        ...
        # #6447: so a plain `bmux claude-teams "make a demo team"` actually opens
        ...
        if socket_path_value != explicit_socket_path:
            print(f"FAIL: expected BMUX_SOCKET_PATH={explicit_socket_path!r}, got {socket_path_value!r}")
            raise SystemExit(1)

        socket_password_value = read_text(socket_password_log)
        if socket_password_value != explicit_socket_password:
            print(
                "FAIL: expected BMUX_SOCKET_PASSWORD to preserve the explicit CLI override, "
...
def main() -> int:
    ...
    if proc.returncode != 0:
        print("FAIL: `bmux claude-teams --version` exited non-zero")
    ...
    if proc.returncode != 0:
        print("FAIL: `bmux claude-teams --version` with existing heap flag exited non-zero")
        print(f"exit={proc.returncode}")
        print(f"stdout={proc.stdout.strip()}")
        print(f"stderr={proc.stderr.strip()}")
        return 1
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-claude-teams-bad-tmp-") as td:
    ...
    if proc.returncode != 0:
        print("FAIL: `bmux claude-teams --version` should still succeed when TMPDIR is unusable")
        print(f"exit={proc.returncode}")
        print(f"stdout={proc.stdout.strip()}")
        print(f"stderr={proc.stderr.strip()}")
        return 1
    ...
    print("PASS: bmux claude-teams restores child NODE_OPTIONS while injecting the auto-mode tmux env")
    return 0
