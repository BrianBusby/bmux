...
def _git_stub() -> str:
    return textwrap.dedent(
        ...
        if [ "$1" = "remote" ] && [ "$2" = "get-url" ] && [ "$3" = "origin" ]; then
          printf 'https://github.com/manaflow-ai/bmux.git\\n'
          exit 0
        fi
...
def _gh_stub() -> str:
    return textwrap.dedent(
        """\
        #!/bin/sh
        args_log="${BMUX_TEST_GH_ARGS_LOG:?}"
        printf '%s\\n' "$*" >> "$args_log"
        exit 0
        """
    )
...
def _shell_command(kind: str, scenario: str) -> str:
    shared = {
        ...
        "merge_action": (
            'cd "$BMUX_TEST_REPO"\n'
            '_bmux_pr_command_entry "gh pr merge"\n'
            '_bmux_prompt_entry\n'
            'sleep 1\n'
            '_bmux_cleanup\n'
        ),
        "close_action_target": (
            'cd "$BMUX_TEST_REPO"\n'
            '_bmux_pr_command_entry "gh pr close 2580"\n'
            '_bmux_prompt_entry\n'
            'sleep 1\n'
            '_bmux_cleanup\n'
        ),
        ...
        "non_pr_gh_no_action": (
            'cd "$BMUX_TEST_REPO"\n'
            '_bmux_pr_command_entry "gh issue view 1"\n'
            '_bmux_prompt_entry\n'
            'sleep 1\n'
            '_bmux_cleanup\n'
        ),
    ...
    if kind == "zsh":
        return textwrap.dedent(
            f"""\
            source "$BMUX_TEST_SCRIPT"
            _bmux_send() {{ print -r -- "$1" >> "$BMUX_TEST_SEND_LOG"; }}
            _bmux_prompt_entry() {{ _bmux_precmd; }}
            _bmux_pr_command_entry() {{ _bmux_preexec "$1"; }}
            _bmux_cleanup() {{ _bmux_zshexit; }}
            {shared}"""
        )
    ...
    if kind == "bash":
        return textwrap.dedent(
            f"""\
            source "$BMUX_TEST_SCRIPT"
            _bmux_send() {{ printf '%s\\n' "$1" >> "$BMUX_TEST_SEND_LOG"; }}
            _bmux_prompt_entry() {{ _bmux_prompt_command; }}
            _bmux_pr_command_entry() {{ _bmux_bash_preexec_hook "$1"; }}
            _bmux_cleanup() {{ type _bmux_bash_cleanup >/dev/null 2>&1 && _bmux_bash_cleanup; }}
            {shared}"""
        )
...
def _run_case(base: Path, *, shell: str, shell_args: list[str], script: Path, scenario: str) -> tuple[int, str]:
    ...
    env["BMUX_TAB_ID"] = "00000000-0000-0000-0000-000000000001"
    env["BMUX_PANEL_ID"] = "00000000-0000-0000-0000-000000000002"
    env["BMUX_TEST_SCRIPT"] = str(script)
    env["BMUX_TEST_REPO"] = str(repo)
    env["BMUX_TEST_SEND_LOG"] = str(send_log)
    env["BMUX_TEST_GH_ARGS_LOG"] = str(gh_args_log)
    env["BMUX_TEST_HEAD_FILE"] = str(head_file)
...
def main() -> int:
    ...
    cases = [
        ("zsh", ["-f", "-c"], root / "Resources" / "shell-integration" / "bmux-zsh-integration.zsh"),
        ("bash", ["--noprofile", "--norc", "-c"], root / "Resources" / "shell-integration" / "bmux-bash-integration.bash"),
    ]
    scenarios = [
        "prompt_does_not_call_gh",
        "merge_action",
    ...
    base = Path("/tmp") / f"bmux_issue_1138_pr_poll_{os.getpid()}"
    try:
