...
SCRIPT="$ROOT_DIR/Resources/bin/start-bmux-profiling"
TMP_DIR="$(mktemp -d)"
...
EOF
  : > "$path/Contents/MacOS/bmux"
}

stable_app="$TMP_DIR/bmux.app"
nightly_app="$TMP_DIR/bmux NIGHTLY.app"
dev_app="$TMP_DIR/bmux DEV dog.app"
make_app "$stable_app" "com.bmuxterm.app" "bmux"
make_app "$nightly_app" "com.bmuxterm.app.nightly" "bmux NIGHTLY"
make_app "$dev_app" "com.bmuxterm.app.debug.dog" "bmux DEV dog"
...
EOF
chmod +x "$plist_buddy"
export BMUX_PROFILE_PLIST_BUDDY="$plist_buddy"
...
export BMUX_PROFILE_DEFAULTS="$defaults_bin"
...
export BMUX_PROFILE_SYSTEM_PROFILER="$system_profiler_bin"

ps_file="$TMP_DIR/ps.txt"
cat > "$ps_file" <<EOF
101 $stable_app/Contents/MacOS/bmux
202 $nightly_app/Contents/MacOS/bmux
303 $dev_app/Contents/MacOS/bmux
EOF

dry_run="$("$SCRIPT" --dry-run --test-ps-file "$ps_file" --channel dev --tag dog --duration 7 --out "$TMP_DIR/out")"
if [[ "$dry_run" != *"Target: pid=303 channel=dev bundle=com.bmuxterm.app.debug.dog name=bmux DEV dog"* ]]; then
...
if "$SCRIPT" --dry-run --test-ps-file "$ps_file" --out "$TMP_DIR/ambiguous" >/tmp/bmux-profile-ambiguous.log 2>&1; then
  echo "FAIL: unqualified selection should reject multiple bmux processes" >&2
  exit 1
fi
if ! grep -Fq "multiple bmux processes are running" /tmp/bmux-profile-ambiguous.log; then
  echo "FAIL: ambiguous selection did not explain how to discriminate instances" >&2
  cat /tmp/bmux-profile-ambiguous.log >&2
  exit 1
fi

list_output="$("$SCRIPT" --list-targets --test-ps-file "$ps_file")"
if [[ "$list_output" != *"pid=101 channel=stable bundle=com.bmuxterm.app"* ]] ||
   [[ "$list_output" != *"pid=202 channel=nightly bundle=com.bmuxterm.app.nightly"* ]] ||
   [[ "$list_output" != *"pid=303 channel=dev bundle=com.bmuxterm.app.debug.dog"* ]]; then
...
HOME="$TMP_DIR" PATH="$fake_bin:$PATH" BMUX_PROFILE_TOC_TIMEOUT_SECONDS=1 "$SCRIPT" \
...
if ! grep -Fq "System:" "$timeout_out/summary.md" ||
   ! grep -Fq "App: ~/bmux DEV dog.app" "$timeout_out/summary.md" ||
...
if grep -Fq "$TMP_DIR/bmux DEV dog.app" "$timeout_out/summary.md" ||
   grep -Fq "$TMP_DIR/bmux DEV dog.app" "$timeout_out/system-info.txt"; then
...
PATH="$fake_bin:$PATH" BMUX_PROFILE_SYSTEM_PROFILER="$failing_system_profiler" "$SCRIPT" \
  --test-ps-file "$ps_file" \
  --channel dev \
  --tag dog \
  --duration 1 \
...
PATH="$fake_bin:$PATH" BMUX_PROFILE_SYSTEM_PROFILER="$sleep_system_profiler" BMUX_PROFILE_SYSTEM_PROFILER_TIMEOUT_SECONDS=1 "$SCRIPT" \
  --test-ps-file "$ps_file" \
  --channel dev \
  --tag dog \
  --duration 1 \
...
if PATH="$fail_bin:$PATH" "$SCRIPT" \
  ...
  --template "Time Profiler" \
  --no-submit \
  --out "$all_failed_out" >/tmp/bmux-profile-all-failed.log 2>&1; then
  echo "FAIL: all-failed profiling run should exit nonzero" >&2
  exit 1
fi
if ! grep -Fq "all profiling templates failed" /tmp/bmux-profile-all-failed.log ||
  ...
  cat /tmp/bmux-profile-all-failed.log >&2
  cat "$all_failed_out/summary.md" >&2
  exit 1
fi

submit_output="$("$ROOT_DIR/Resources/bin/submit-bmux-profile" --dry-run --profile "$timeout_out" --target-name "bmux DEV dog" --target-pid 303 --channel dev --bundle-id com.bmuxterm.app.debug.dog --reply-to "user@example.com")"
if [[ "$submit_output" != *"Recipient: founders@manaflow.com"* ]] ||
   [[ "$submit_output" != *"Reply-to: user@example.com"* ]] ||
   [[ "$submit_output" != *"Subject: bmux profiling capture: bmux DEV dog"* ]]; then
...
"$ROOT_DIR/Resources/bin/submit-bmux-profile" --dry-run --profile "$timeout_out" --target-name "bmux DEV dog" >/dev/null
...
EOF
chmod +x "$sleep_ditto"
BMUX_PROFILE_DITTO="$sleep_ditto" "$ROOT_DIR/Resources/bin/submit-bmux-profile" \
  --profile "$timeout_out" \
  --package-only &
...
package_output="$(BMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-bmux-profile" --profile "$timeout_out" --package-only)"
...
BMUX_PROFILE_OSASCRIPT="$cancel_bin" BMUX_PROFILE_OPEN="$open_bin" BMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-bmux-profile" \
  --profile "$timeout_out" \
  --target-name "bmux DEV dog" \
  --target-pid 303 \
  --channel dev \
  --bundle-id com.bmuxterm.app.debug.dog
...
BMUX_PROFILE_OSASCRIPT="$sleep_osascript" BMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-bmux-profile" \
  --profile "$timeout_out" \
  --target-name "bmux DEV dog" \
  --target-pid 303 \
  --channel dev \
...
if BMUX_PROFILE_OSASCRIPT="$cancel_bin" BMUX_PROFILE_OPEN="$open_bin" BMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-bmux-profile" \
  --profile "$timeout_out" \
  --target-name "bmux DEV dog" \
  --target-pid 303 \
  --channel dev \
  --bundle-id com.bmuxterm.app.debug.dog \
  --send >/tmp/bmux-profile-send-cancel.log 2>&1; then
  echo "FAIL: submit helper send mode should fail when Mail send is canceled" >&2
  exit 1
fi
if ! grep -Fq "User canceled" /tmp/bmux-profile-send-cancel.log; then
  echo "FAIL: submit helper send cancellation did not preserve the AppleScript error" >&2
  cat /tmp/bmux-profile-send-cancel.log >&2
  exit 1
fi
...
HOME="$TMP_DIR" BMUX_PROFILE_FEEDBACK_EMAIL=wrong@example.com BMUX_PROFILE_LOCALE=ja_JP BMUX_PROFILE_OSASCRIPT="$capture_osascript" BMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-bmux-profile" \
  --profile "$timeout_out" \
  --target-name "bmux DEV dog" \
  --target-pid 303 \
  --channel dev \
  ...
  --note-file "$note_file" \
  --send
if ! grep -Fq "bmuxプロファイルを送信" "$captured_args" ||
   ! grep -Fq "下書きを開く" "$captured_args" ||
   ! grep -Fq "bmuxプロファイリングキャプチャ" "$captured_args" ||
...
echo "PASS: start-bmux-profiling target selection and default templates"
