...
bmux also injects its own zsh integration by setting ZDOTDIR to
Resources/shell-integration. If bmux incorrectly treats Ghostty's injected
...
def main() -> int:
    ...
    if not (bmux_wrapper_dir / ".zshenv").exists():
        print(f"SKIP: missing bmux wrapper .zshenv at {bmux_wrapper_dir}")
        return 0
    if not (ghostty_zsh_dir / ".zshenv").exists():
        print(f"SKIP: missing Ghostty zsh .zshenv at {ghostty_zsh_dir}")
        return 0

    base = Path("/tmp") / f"bmux_histfile_ghostty_stack_{os.getpid()}"
    try:
        ...
        # Simulate the buggy situation: bmux stores Ghostty's injected ZDOTDIR
        # as the "original" ZDOTDIR, then sets ZDOTDIR to its own wrapper.
        env["BMUX_ORIGINAL_ZDOTDIR"] = str(ghostty_zsh_dir)
        env["ZDOTDIR"] = str(bmux_wrapper_dir)
        env["BMUX_SHELL_INTEGRATION"] = "0"
        ...
        if seen != expected:
            ...
            print(f"  bmux_wrapper_dir={bmux_wrapper_dir}")
