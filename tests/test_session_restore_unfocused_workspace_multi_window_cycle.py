...
def _snapshot_path(bundle_id: str) -> Path:
    ...
    return Path.home() / "Library/Application Support/bmux" / f"session-{safe_bundle}.json"
...
def _socket_candidates(app_path: Path, preferred: Path) -> list[Path]:
    ...
    if app_name.startswith(prefix):
        ...
        candidates.append(Path(f"/tmp/bmux-debug-{slug}.sock"))
    deduped: list[Path] = []
    seen: set[str] = set()
...
def _launch(app_path: Path, preferred_socket_path: Path) -> Path:
    ...
    subprocess.run(
        [
            ...
            str(app_path),
            "--env",
            f"BMUX_SOCKET_PATH={preferred_socket_path}",
            "--env",
...
def _wait_for_marker(client: bmux, marker: str, timeout: float = 8.0) -> bool:
    deadline = time.time() + timeout
    while time.time() < deadline:
...
def _consume_visible_markers(client: bmux, remaining: set[str], timeout: float = 4.0) -> None:
    if not remaining:
        return
    deadline = time.time() + timeout
...
def _ensure_workspaces(client: bmux, count: int) -> None:
    while len(client.list_workspaces()) < count:
        client.new_workspace()
        time.sleep(0.3)


def _list_windows(client: bmux) -> list[str]:
...
def _new_window(client: bmux) -> str:
...
def _focus_window(client: bmux, window_id: str) -> None:
...
def main() -> int:
    ...
    socket_path = Path(f"/tmp/bmux-mw-restore-{bundle_suffix}.sock")

    markers = {
        "w1_ws0": "BMUX_MW_RESTORE_W1_WS0",
        "w1_ws1": "BMUX_MW_RESTORE_W1_WS1",
        "w2_ws0": "BMUX_MW_RESTORE_W2_WS0",
        "w2_ws1": "BMUX_MW_RESTORE_W2_WS1",
    }
    failures: list[str] = []
