...
#
# Verifies the lint script reports "ok" on the real bmux repo and correctly
...
#   (d) Test file is a member of a non-bmuxTests target (e.g. bmuxUITests).
#       Its "in Sources" lines exist in the pbxproj, but not inside the
#       bmuxTests Sources build phase; Xcode does not compile it into the
#       bmuxTests bundle.
#   (e) Test file's basename is a suffix of another file already wired into
#       the bmuxTests Sources phase. An unanchored grep would match the
...
# Helper: write a minimal pbxproj that contains a bmuxTests PBXNativeTarget with
...
# eye; the lint only cares about the `bmuxTests` target marker, the Sources
...
/* Begin PBXNativeTarget section */
		AAAA000000000000000000T1 /* bmuxTests */ = {
			isa = PBXNativeTarget;
			buildPhases = (
...
mkdir -p "$SANDBOX_B/bmuxTests" "$SANDBOX_B/bmux.xcodeproj"
cat > "$SANDBOX_B/bmuxTests/FakeOrphanTests.swift" <<'SWIFT'
import XCTest
final class FakeOrphanTests: XCTestCase { func testNoop() { XCTAssert(true) } }
SWIFT
write_base_pbxproj "$SANDBOX_B/bmux.xcodeproj/project.pbxproj"
...
mkdir -p "$SANDBOX_C/bmuxTests" "$SANDBOX_C/bmux.xcodeproj"
cat > "$SANDBOX_C/bmuxTests/FakeGroupOnlyTests.swift" <<'SWIFT'
import XCTest
final class FakeGroupOnlyTests: XCTestCase { func testNoop() { XCTAssert(true) } }
SWIFT
write_base_pbxproj "$SANDBOX_C/bmux.xcodeproj/project.pbxproj" "
...
/* Begin PBXGroup section */
		BBBB000000000000000000G1 /* bmuxTests */ = {
			isa = PBXGroup;
			children = (
...
# ---------------------------------------------------------------------------
# (d) File is in bmuxUITests target's Sources phase, NOT in bmuxTests.
SANDBOX_D="$SANDBOX_PARENT/d"
mkdir -p "$SANDBOX_D/bmuxTests" "$SANDBOX_D/bmux.xcodeproj"
cat > "$SANDBOX_D/bmuxTests/FakeWrongTargetTests.swift" <<'SWIFT'
import XCTest
final class FakeWrongTargetTests: XCTestCase { func testNoop() { XCTAssert(true) } }
SWIFT

# Write a pbxproj that contains BOTH a bmuxTests target (with empty Sources
# phase) AND a separate bmuxUITests target whose Sources phase wires
# FakeWrongTargetTests.swift. The file therefore appears in two `in Sources`
# lines (PBXBuildFile + bmuxUITests Sources phase), satisfying a naive global
# grep, but it is NOT a member of the bmuxTests Sources phase.
cat > "$SANDBOX_D/bmux.xcodeproj/project.pbxproj" <<'PBX'
...
/* Begin PBXNativeTarget section */
		...
		CCCC000000000000000000T1 /* bmuxUITests */ = {
			isa = PBXNativeTarget;
			buildPhases = (
...
if "$LINT" --repo-root "$SANDBOX_D" >"$SANDBOX_D/out" 2>&1; then
  echo "test_ci_pbxproj_test_wiring: (d) lint should have failed on file wired to wrong target (bmuxUITests instead of bmuxTests)" >&2
  cat "$SANDBOX_D/out" >&2
  exit 1
fi
...
if ! grep -q "bmuxTests target's Sources build phase" "$SANDBOX_D/out"; then
  echo "test_ci_pbxproj_test_wiring: (d) lint output missing bmuxTests-target diagnostic" >&2
  cat "$SANDBOX_D/out" >&2
  exit 1
fi
...
# is wired into the bmuxTests Sources phase. The shorter file should be
...
mkdir -p "$SANDBOX_E/bmuxTests" "$SANDBOX_E/bmux.xcodeproj"
cat > "$SANDBOX_E/bmuxTests/FooTests.swift" <<'SWIFT'
import XCTest
final class FooTests: XCTestCase { func testNoop() { XCTAssert(true) } }
SWIFT
cat > "$SANDBOX_E/bmuxTests/PrefixFooTests.swift" <<'SWIFT'
import XCTest
final class PrefixFooTests: XCTestCase { func testNoop() { XCTAssert(true) } }
SWIFT

# pbxproj: bmuxTests target's Sources phase only wires PrefixFooTests.swift.
# FooTests.swift has no entries; it should be flagged.
cat > "$SANDBOX_E/bmux.xcodeproj/project.pbxproj" <<'PBX'
