...
When a bmux shell leaves a git repository for a non-git directory, an async
...
guard every git-branch payload with ``_bmux_git_report_path_is_active`` so a
...
def _shell_command(kind: str) -> str:
    ...
    if kind == "zsh":
        return textwrap.dedent(
            ...
            cd "$BMUX_TEST_NONREPO"
            setopt noclobber
            _bmux_set_git_active_pwd "$PWD"
            unsetopt noclobber
            _bmux_report_git_branch_for_path "$BMUX_TEST_REPO" &
            wait
...
def _run_case(
    ...
    *,
    ...
    socket_path = base / shell / "bmux.sock"
    ...
    env["BMUX_TEST_NONREPO"] = str(nonrepo)
    ...
    env.pop("_BMUX_GIT_ACTIVE_PWD_FILE", None)
...
def _run_same_repo_case(
    ...
    *,
    ...
    socket_path = base / f"{shell}-samerepo" / "bmux.sock"
...
def _run_zsh_chpwd_keeps_watch(base: Path, *, script: Path) -> tuple[int, str]:
    ...
    command = textwrap.dedent(
        ...
        sleep 5 &
        watch_pid=$!
        _BMUX_GIT_HEAD_WATCH_PID=$watch_pid
        cd "$BMUX_TEST_NONREPO"
...
def main() -> int:
    ...
    base = Path("/tmp") / f"bmux_shell_git_branch_stale_cwd_{os.getpid()}"
    try:
