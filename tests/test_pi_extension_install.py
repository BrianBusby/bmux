#!/usr/bin/env python3
"""
Regression test: the generated Pi extension is importable and emits bmux hook calls.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-pi-extension-") as td:
        root = Path(td)
        ...
        extension_path = config_dir / "extensions" / "bmux-session.ts"
        if not extension_path.exists():
            print(f"FAIL: expected extension at {extension_path}")
            return 1
        extension_text = extension_path.read_text(encoding="utf-8")
        if "bmux-pi-session-extension-marker" not in extension_text:
        ...
        make_executable(
            ...
            """#!/usr/bin/env bash
set -euo pipefail
printf '%s\n' "$*" >> "$BMUX_TEST_PI_ARGS_LOG"
payload="$(cat)"
printf '%s' "$payload" >> "$BMUX_TEST_PI_STDIN_LOG"
printf '\n---\n' >> "$BMUX_TEST_PI_STDIN_LOG"
{
  ...
  if [ -n "${BMUX_LEAK_TOKEN-}" ]; then printf 'BMUX_LEAK_TOKEN=present\n'; fi
  ...
  if [ -n "${BMUX_TEST_PI_TOKEN-}" ]; then printf 'BMUX_TEST_PI_TOKEN=present\n'; fi
} >> "$BMUX_TEST_PI_ENV_LOG"
case "$*" in
  *"hooks pi notification"*)
    ...
    ;;
  *"surface resume get"*)
    if [ -f "$BMUX_TEST_PI_BINDING_FILE" ]; then
      cat "$BMUX_TEST_PI_BINDING_FILE"
    else
...
""",
        ...
        check_env["BMUX_TEST_PI_EXTENSION_PATH"] = str(extension_path)
        check_env["BMUX_SURFACE_ID"] = "surface-pi-test"
        check_env["BMUX_WORKSPACE_ID"] = "workspace-pi-test"
        check_env["BMUX_PI_BMUX_BIN"] = str(fake_bmux)
        check_env["BMUX_TEST_PI_ARGS_LOG"] = str(fake_args_log)
        check_env["BMUX_TEST_PI_STDIN_LOG"] = str(fake_stdin_log)
        check_env["BMUX_TEST_PI_ENV_LOG"] = str(fake_env_log)
        check_env["BMUX_TEST_PI_BINDING_FILE"] = str(fake_binding)
        ...
        check_env["BMUX_LEAK_TOKEN"] = "bmux-secret-should-not-leak"
        ...
        check_env["BMUX_TEST_PI_TOKEN"] = "test-token-should-not-leak"
        check_source = """
const extensionPath = process.env.BMUX_TEST_PI_EXTENSION_PATH;
...
process.env.BMUX_PI_HOOKS_DISABLED = "1";
const disabledCtx = {
  cwd: "/tmp/pi-project",
  sessionManager: {
...
delete process.env.BMUX_PI_HOOKS_DISABLED;
...
"""
        ...
        if stop_payload.get("turn_id") != prompt_turn_id:
            ...
            return 1
        if stop_payload.get("bmux_notification_routed") is not True:
        ...
        if fallback_stop_payload is None:
            ...
            return 1
        if fallback_stop_payload.get("bmux_notification_routed") is True:
            print(
        ...
        if interrupted_stop_payload is None:
            ...
            return 1
        if interrupted_stop_payload.get("bmux_notification_routed") is True:
            print(
    ...
    print("PASS: generated Pi extension installs and emits bmux hooks")
    return 0
