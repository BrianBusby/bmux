...
def test_wrapper_skips_bmux_shims_and_bundled_claude(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-wrapper-resolution-") as td:
        root = Path(td)
        bundle_bin = root / "bmux.app" / "Contents" / "Resources" / "bin"
        shim_bin = root / "shim-bin"
        real_bin = root / "real-bin"
        for directory in (bundle_bin, shim_bin, real_bin):
            directory.mkdir(parents=True, exist_ok=True)

        wrapper = bundle_bin / "bmux-claude-wrapper"
...
""",
        ...
        write_executable(
            shim,
            f"""#!/bin/sh
export BMUX_CLAUDE_WRAPPER_SHIM="{shim}"
export BMUX_CLAUDE_WRAPPER_SHIM_ROOT="{shim_bin}"
exec "{wrapper}" "$@"
""",
        )

        env = minimal_env(f"{shim_bin}:{bundle_bin}:{real_bin}:/usr/bin:/bin")
        env["BMUX_CLAUDE_WRAPPER_SHIM"] = str(shim)
        env["BMUX_CLAUDE_WRAPPER_SHIM_ROOT"] = str(shim_bin)
        env["BMUX_CUSTOM_CLAUDE_PATH"] = str(bundle_bin / "claude")
...
def test_wrapper_skips_inherited_bmux_cli_shim_roots(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-wrapper-inherited-shim-") as td:
        root = Path(td)
        wrapper_bin = root / "wrapper-bin"
        current_shim_root = root / "tmp" / "bmux-cli-shims" / "current-surface"
        inherited_shim_root = root / "tmp" / "bmux-cli-shims" / "old-surface"
        real_bin = root / "real-bin"
        for directory in (wrapper_bin, current_shim_root, inherited_shim_root, real_bin):
            directory.mkdir(parents=True, exist_ok=True)

        wrapper = wrapper_bin / "bmux-claude-wrapper"
        wrapper.write_bytes(WRAPPER.read_bytes())
        wrapper.chmod(0o755)
...
""",
        ...
        env["BMUX_CLAUDE_WRAPPER_SHIM"] = str(current_shim)
        env["BMUX_CLAUDE_WRAPPER_SHIM_ROOT"] = str(current_shim_root)
        ...
        if output != "real-claude --version":
            failures.append(f"expected inherited bmux shim roots to be skipped, got {output!r}")


def test_shell_integration_does_not_shim_grok(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-grok-wrapper-resolution-") as td:
        root = Path(td)
        real_bin = root / "real-bin"
...
""",
        ...
        base_env["BMUX_SHELL_INTEGRATION_DIR"] = str(SHELL_INTEGRATION_DIR)
        ...
        shell_commands = [
            [
                ...
                "--norc",
                "-c",
                'source "$BMUX_SHELL_INTEGRATION_DIR/bmux-bash-integration.bash"; grok --version',
            ],
            [
                "/bin/zsh",
                "-f",
                "-c",
                'source "$BMUX_SHELL_INTEGRATION_DIR/bmux-zsh-integration.zsh"; grok --version',
            ],
        ]
...
def test_shell_integration_preserves_empty_path_components(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-shell-path-components-") as td:
        root = Path(td)
        tmpdir = root / "tmp"
        ...
        shim_root = tmpdir / "bmux-cli-shims" / surface_id
        expected_path = f"{shim_root}::{first}::{last}:"

        input_path = f":{first}::{shim_root}:{last}:"
        base_env = minimal_env("/usr/bin:/bin", tmpdir)
        base_env["BMUX_SHELL_INTEGRATION_DIR"] = str(SHELL_INTEGRATION_DIR)
        base_env["BMUX_SURFACE_ID"] = surface_id
        base_env["BMUX_TEST_INPUT_PATH"] = input_path
        ...
        shell_commands = [
            [
                ...
                "--norc",
                "-c",
                'PATH="$BMUX_TEST_INPUT_PATH"; '
                'source "$BMUX_SHELL_INTEGRATION_DIR/bmux-bash-integration.bash"; '
                'printf "%s\\n" "$PATH"',
            ],
            [
                "/bin/zsh",
                ...
                "-c",
                'PATH="$BMUX_TEST_INPUT_PATH"; '
                'source "$BMUX_SHELL_INTEGRATION_DIR/bmux-zsh-integration.zsh"; '
                'printf "%s\\n" "$PATH"',
            ],
        ]
        for argv in shell_commands:
...
def main() -> int:
    failures: list[str] = []
    test_wrapper_skips_bmux_shims_and_bundled_claude(failures)
    test_wrapper_skips_inherited_bmux_cli_shim_roots(failures)
