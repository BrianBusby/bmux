...
NEW_WORKSPACES = int(os.environ.get("BMUX_LAG_NEW_WORKSPACES", "20"))
SWITCH_PASSES = int(os.environ.get("BMUX_LAG_SWITCH_PASSES", "1"))
SWITCH_DELAY_S = float(os.environ.get("BMUX_LAG_SWITCH_DELAY_S", "0.06"))
HISTORY_SEED_LINES = int(os.environ.get("BMUX_LAG_HISTORY_LINES", "120"))
KEY_EVENTS = int(os.environ.get("BMUX_LAG_KEY_EVENTS", "180"))
KEY_DELAY_S = float(os.environ.get("BMUX_LAG_KEY_DELAY_S", "0.0"))
KEY_COMBO = os.environ.get("BMUX_LAG_KEY_COMBO", "up")
...
BURST_REPEATS = max(1, int(os.environ.get("BMUX_LAG_BURST_REPEATS", "3")))

MAX_P95_RATIO = float(os.environ.get("BMUX_LAG_MAX_P95_RATIO", "1.70"))
MAX_AVG_RATIO = float(os.environ.get("BMUX_LAG_MAX_AVG_RATIO", "1.70"))
MAX_CHURN_P95_MS = float(os.environ.get("BMUX_LAG_MAX_CHURN_P95_MS", "35.0"))
MAX_P95_DELTA_MS = float(os.environ.get("BMUX_LAG_MAX_P95_DELTA_MS", "20.0"))
MAX_AVG_DELTA_MS = float(os.environ.get("BMUX_LAG_MAX_AVG_DELTA_MS", "12.0"))
MIN_BASELINE_P95_MS_FOR_RATIO = float(os.environ.get("BMUX_LAG_MIN_BASELINE_P95_MS_FOR_RATIO", "12.0"))
MIN_BASELINE_AVG_MS_FOR_RATIO = float(os.environ.get("BMUX_LAG_MIN_BASELINE_AVG_MS_FOR_RATIO", "8.0"))
MAX_CPU_PERCENT = float(os.environ.get("BMUX_LAG_MAX_CPU_PERCENT", "180.0"))
ENFORCE_CPU = os.environ.get("BMUX_LAG_ENFORCE_CPU", "0") == "1"
ALLOW_MAIN_SOCKET = os.environ.get("BMUX_LAG_ALLOW_MAIN_SOCKET", "0") == "1"
...
class RawSocketClient:
    ...
    def command(self, command: str, timeout_s: float = 2.0) -> str:
        if self.sock is None:
            raise bmuxError("Raw socket client not connected")
        ...
        while True:
            ...
            if remaining <= 0:
                raise bmuxError(f"Timed out waiting for response to: {command}")
            ...
            if not chunk:
                raise bmuxError("Socket closed while waiting for response")
...
def resolve_target_socket() -> str:
    socket_path = os.environ.get("BMUX_SOCKET_PATH")
    if not socket_path:
        raise bmuxError(
            "BMUX_SOCKET_PATH is required. Point it to a tagged dev socket (for example /tmp/bmux-debug-<tag>.sock)."
        )
    base = os.path.basename(socket_path)
    if not ALLOW_MAIN_SOCKET and base in {"bmux.sock", "bmux-debug.sock"}:
        raise bmuxError(
            f"Refusing to run against main socket '{socket_path}'. Set BMUX_SOCKET_PATH to a tagged dev instance."
        )
    return socket_path
...
def keep_only_first_workspace(client: bmux) -> str:
...
def create_workspaces(client: bmux, count: int) -> list[str]:
    created: list[str] = []
...
def focused_terminal_panel(client: bmux) -> str:
    surfaces = client.list_surfaces()
    if not surfaces:
        raise bmuxError("No surfaces available in selected workspace")
...
def seed_history(client: bmux, lines: int) -> None:
    for i in range(lines):
        client.send_line(f"echo bmux-lag-seed-{i}")
...
def latency_failures(baseline: LatencyStats, churn: LatencyStats, cpu_max: float) -> list[str]:
...
def main() -> int:
    ...
    try:
        ...
        failures = latency_failures(baseline, churn, cpu_max)

        if failures:
            print("\nFAIL")
            for item in failures:
                print(f"  - {item}")
            sample_path = maybe_write_sample(pid, "bmux_workspace_churn_up_arrow_lag")
            if sample_path:
        ...
        print(f"FAIL: {e}")
        sample_path = maybe_write_sample(pid, "bmux_workspace_churn_up_arrow_error")
        if sample_path:
            print(f"sample_path: {sample_path}")
        return 1
