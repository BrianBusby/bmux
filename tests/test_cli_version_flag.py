#!/usr/bin/env python3
"""
Regression test: `bmux --version` should print version text without requiring a socket.
"""
...
def main() -> int:
    ...
    if not out:
        print("FAIL: `bmux --version` produced empty stdout")
        return 1
    ...
    if out2 != out:
        print("FAIL: `bmux --version` and `bmux version` differ")
        print(f"--version: {out!r}")
        print(f"version:   {out2!r}")
        return 1

    print(f"PASS: bmux version command works ({out})")
    return 0
