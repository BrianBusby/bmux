...
def run_claude_hook(
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"bmux claude-hook {subcommand} failed:\n"
...
def main() -> int:
    ...
    state_path = Path(tempfile.gettempdir()) / f"bmux_claude_hook_state_{os.getpid()}.json"
    ...
    project_dir = Path(tempfile.gettempdir()) / f"bmux_claude_map_project_{os.getpid()}"
