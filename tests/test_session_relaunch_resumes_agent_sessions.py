...
1) Launch bmux and seed workspaces with tracked Claude/Codex/OpenCode/Pi sessions.
2) Quit the app normally so the session snapshot is saved.
3) Relaunch bmux the next day.
...
def main() -> int:
    ...
    socket_path = Path(f"/tmp/bmux-session-relaunch-agents-{bundle_id.replace('.', '-')}.sock")
    snapshot = _snapshot_path(bundle_id)
    previous_snapshot = _snapshot_path(bundle_id, suffix="-previous")
    codex_expected = "BMUX_FAKE_CODEX_RESUME:resume codex-session-relaunch-2923"
    # The bmux claude wrapper inserts its own arguments around --resume, so
    # claude expectations are order-agnostic tokens that must share one line.
    claude_expected_tokens = (
        "BMUX_FAKE_CLAUDE_RESUME:",
        "--resume claude-session-relaunch-2923",
        "--dangerously-skip-permissions",
    )
    opencode_expected = "BMUX_FAKE_OPENCODE_RESUME:--session opencode-session-relaunch-2923"
    pi_expected = "BMUX_FAKE_PI_RESUME:--session pi-session-relaunch-2923"

    failures: list[str] = []

    with tempfile.TemporaryDirectory(prefix="bmux-session-relaunch-agents-") as td:
        ...
        _write_fake_agent(fake_bin_dir, "pi", "BMUX_FAKE_PI_RESUME")
        ...
        try:
            ...
            # Prove the relaunch uses the persisted bmux snapshot, not the live hook files.
            claude_hook_state.unlink(missing_ok=True)
            codex_hook_state.unlink(missing_ok=True)
            opencode_hook_state.unlink(missing_ok=True)
            pi_hook_state.unlink(missing_ok=True)
