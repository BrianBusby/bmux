...
shell integrations should dispatch `claude` through bmux's wrapper even when
...
def prepare_bundle(tmp: Path) -> tuple[Path, Path]:
    ...
    for name in (".zshenv", ".zprofile", ".zshrc", "bmux-zsh-integration.zsh", "bmux-bash-integration.bash"):
...
def run_zsh(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    env["BMUX_ZSH_ZDOTDIR"] = str(orig)
    env["BMUX_SHELL_INTEGRATION"] = "1"
    env["BMUX_SHELL_INTEGRATION_DIR"] = str(shell_dir)
    env["BMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "0"
    env["BMUX_TEST_LOG"] = str(log_path)
    env["BMUX_TEST_REAL_BIN"] = str(real_bin)
    env["PATH"] = f"{real_bin}:/usr/bin:/bin"
    env.pop("GHOSTTY_BIN_DIR", None)

    result = subprocess.run(
        ["zsh", "-d", "-i", "-c", 'PATH="$BMUX_TEST_REAL_BIN:$PATH"; claude zsh-case'],
        env=env,
        capture_output=True,
        text=True,
        timeout=30,
...
def run_zsh_with_alias(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            "zsh",
            "-fic",
            f'alias claude="echo alias"; source "{shell_dir / "bmux-zsh-integration.zsh"}"; '
            'PATH="$BMUX_TEST_REAL_BIN:$PATH"; claude zsh-alias-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_zsh_with_late_user_function(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            "zsh",
            "-fic",
            f'source "{shell_dir / "bmux-zsh-integration.zsh"}"; '
            'claude() { "$BMUX_TEST_REAL_BIN/user-claude-function" "$@"; }; '
            '_bmux_fix_path; PATH="$BMUX_TEST_REAL_BIN:$PATH"; claude zsh-late-function-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-c",
            f'source "{shell_dir / "bmux-bash-integration.bash"}"; PATH="$BMUX_TEST_REAL_BIN:$PATH"; claude bash-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash_with_alias(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-ic",
            f'alias claude="$BMUX_TEST_REAL_BIN/user-claude"\n'
            f'source "{shell_dir / "bmux-bash-integration.bash"}"\n'
            'PATH="$BMUX_TEST_REAL_BIN:$PATH"; claude bash-alias-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash_with_function(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-ic",
            f'claude() {{ "$BMUX_TEST_REAL_BIN/user-claude-function" "$@"; }}; '
            f'source "{shell_dir / "bmux-bash-integration.bash"}"; '
            'PATH="$BMUX_TEST_REAL_BIN:$PATH"; claude bash-function-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_zsh_with_stale_original_wrapper(
    ...
    env["BMUX_TEST_LOG"] = str(log_path)
    env["BMUX_TEST_REAL_BIN"] = str(real_bin)
    env["BMUX_TEST_OLD_WRAPPER"] = str(old_wrapper)
    env["BMUX_BUNDLED_CLI_PATH"] = str(current_cli)
    env["PATH"] = f"{real_bin}:/usr/bin:/bin"
    env.pop("GHOSTTY_BIN_DIR", None)
    ...
    result = subprocess.run(
        [
            ...
            "-fic",
            f'source "{shell_dir / "bmux-zsh-integration.zsh"}"; '
            'rm -f "$BMUX_TEST_OLD_WRAPPER"; '
            'PATH="$BMUX_TEST_REAL_BIN:$PATH"; claude zsh-stale-wrapper-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash_with_stale_original_wrapper(
    ...
    result = subprocess.run(
        [
            ...
            f'source "{shell_dir / "bmux-bash-integration.bash"}"; '
            'rm -f "$BMUX_TEST_OLD_WRAPPER"; '
            'PATH="$BMUX_TEST_REAL_BIN:$PATH"; claude bash-stale-wrapper-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_fish_with_stale_original_wrapper(
    ...
    result = subprocess.run(
        [
            ...
            'rm -f "$BMUX_TEST_OLD_WRAPPER"; '
            'set -gx PATH "$BMUX_TEST_REAL_BIN" $PATH; claude fish-stale-wrapper-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-issue-2448-") as td:
        tmp = Path(td)
        ...
        write_executable(
            bundle_bin / "bmux-claude-wrapper",
            """#!/bin/sh
set -eu
printf 'wrapper:%s\n' "$*" >> "$BMUX_TEST_LOG"
""",
...
""",
        ...
        write_executable(
            current_bin / "bmux-claude-wrapper",
            """#!/bin/sh
set -eu
...
""",
    ...
    if failures:
        print("FAIL: shell integration did not keep claude on the bmux wrapper")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("PASS: zsh, bash, and fish integrations dispatch claude through the bmux wrapper")
    return 0
