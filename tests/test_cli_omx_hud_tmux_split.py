#!/usr/bin/env python3
"""
Regression tests for OMX HUD panes through bmux's tmux compatibility shim.
"""
...
def run_cli(
    ...
    env["BMUX_OMX_BMUX_BIN"] = cli_path
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="bmux-omx-hud-split-") as td:
            tmp = Path(td)
        ...
        with tempfile.TemporaryDirectory(prefix="bmux-omx-hud-disabled-") as td:
            tmp = Path(td)
            socket_path = tmp / "fake-bmux.sock"
            fake_home = tmp / "home"
            fake_home.mkdir(parents=True, exist_ok=True)
