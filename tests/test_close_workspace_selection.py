...
def _wait_for_selection(
    client: bmux, expected_id: str, deadline: float = 5.0, interval: float = 0.05
) -> List[WorkspaceTuple]:
    """
...
def _ensure_workspaces(client: bmux, count: int) -> List[str]:
    """
...
def test_close_middle_selects_next(client: bmux) -> TestResult:
...
def test_close_last_selects_previous(client: bmux) -> TestResult:
