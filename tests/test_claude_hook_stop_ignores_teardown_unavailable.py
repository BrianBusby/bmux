        ...
        try:
            ...
            with conn:
                ...
                if not self.commands:
                    raise RuntimeError("bmux CLI never sent a command to the teardown test socket")
...
def main() -> int:
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="bmux-claude-hook-stop-")
    try:
        ...
        env["BMUX_WORKSPACE_ID"] = str(uuid.uuid4())
        env["BMUX_SURFACE_ID"] = str(uuid.uuid4())
        env["BMUX_CLAUDE_HOOK_STATE_PATH"] = str(state_path)
