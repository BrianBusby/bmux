...
A client library for programmatically controlling bmux via Unix socket.
...
# The control socket, markers, and password live in the non-TCC bmux state
# directory (~/.local/state/bmux), not Application Support, so the separately
# signed bmux CLI can touch them without the macOS "access data from other apps"
# prompt (https://github.com/manaflow-ai/bmux/issues/5146).
_STATE_DIR = os.path.expanduser("~/.local/state/bmux")
_STABLE_SOCKET_PATH = os.path.join(_STATE_DIR, "bmux.sock")
_LEGACY_STABLE_SOCKET_PATH = "/tmp/bmux.sock"
_STABLE_BUNDLE_ID = "com.bmuxterm.app"
_NIGHTLY_BUNDLE_ID = "com.bmuxterm.app.nightly"
_STAGING_BUNDLE_ID = "com.bmuxterm.app.staging"
_DEFAULT_DEBUG_BUNDLE_ID = "com.bmuxterm.app.debug"
...
def _is_known_bmux_bundle_id(bundle_id: str) -> bool:
    return (
...
def _default_bundle_id() -> str:
    override = (os.environ.get("BMUX_BUNDLE_ID") or "").strip()
    if override and _is_known_bmux_bundle_id(override):
        return override

    tag = os.environ.get("BMUX_TAG")
...
def _socket_variant() -> Tuple[str, Optional[str]]:
    ...
    if bundle_id == _DEFAULT_DEBUG_BUNDLE_ID:
        tag = os.environ.get("BMUX_TAG", "").strip()
...
def _last_socket_path_files() -> List[str]:
    ...
    if variant == "nightly":
        ...
        tmp_marker = f"/tmp/bmux-nightly-{slug}-last-socket-path" if slug else "/tmp/bmux-nightly-last-socket-path"
    elif variant == "staging":
        marker = f"staging-{slug}-last-socket-path" if slug else "staging-last-socket-path"
        tmp_marker = f"/tmp/bmux-staging-{slug}-last-socket-path" if slug else "/tmp/bmux-staging-last-socket-path"
    elif variant == "dev":
        marker = f"dev-{slug}-last-socket-path" if slug else "dev-last-socket-path"
        tmp_marker = f"/tmp/bmux-dev-{slug}-last-socket-path" if slug else "/tmp/bmux-dev-last-socket-path"
...
def _variant_socket_candidates() -> List[str]:
    variant, slug = _socket_variant()
    if variant == "nightly":
        return [f"/tmp/bmux-nightly-{slug}.sock"] if slug else ["/tmp/bmux-nightly.sock"]
    if variant == "staging":
        return [f"/tmp/bmux-staging-{slug}.sock"] if slug else ["/tmp/bmux-staging.sock"]
    if variant == "dev":
        if slug:
            return [f"/tmp/bmux-debug-{slug}.sock"]
        return ["/tmp/bmux-debug.sock"]
...
def _default_socket_path() -> str:
    ...
    if bundle_id == _DEFAULT_DEBUG_BUNDLE_ID:
        ...
        tagged.extend(glob.glob(os.path.join(_STATE_DIR, "bmux*.sock")))
        tagged = [
            p for p in tagged
            if os.path.exists(p)
            and os.path.basename(p).startswith("bmux-debug-")
        ]
        if tagged:
...
class bmux:
    """Client for controlling bmux via Unix socket"""
    ...
    def _send_command(self, command: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - start >= 5.0:
                        raise bmuxError("Command timed out")
                    continue
                if not chunk:
                    break
                data += chunk.decode()
            ...
            raise bmuxError(f"Socket error: {e}")
    ...
    def seed_drag_pasteboard_types(self, types: List[str]) -> None:
        ...
        if not types:
            raise bmuxError("seed_drag_pasteboard_types requires at least one type")
        payload = ",".join(t.strip() for t in types if t and t.strip())
        if not payload:
            raise bmuxError("seed_drag_pasteboard_types requires at least one non-empty type")
    ...
    def layout_debug(self) -> dict:
        ...
        try:
            ...
            raise bmuxError(f"layout_debug JSON decode failed: {e}: {payload[:200]}")
    ...
    def render_stats(self, panel: Union[str, int, None] = None) -> dict:
        ...
        try:
            ...
            raise bmuxError(f"render_stats JSON decode failed: {e}: {payload[:200]}")
    ...
    def panel_snapshot(self, panel: Union[str, int], label: str = "") -> dict:
        ...
        if len(parts) != 5:
            raise bmuxError(f"panel_snapshot parse failed: {response}")
...
def main():
    """CLI interface for bmux"""
    import sys
    import argparse

    parser = argparse.ArgumentParser(description="bmux CLI")
    ...
    try:
        with bmux(args.socket) as client:
            if not args.command:
                # Interactive mode
                print("bmux CLI (type 'help' for commands, 'quit' to exit)")
                while True:
                    try:
                        line = input("> ").strip()
