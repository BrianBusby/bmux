...
typeset -gi _bmux_redraw_done=0
typeset -g _bmux_redraw_fd=''
...
_bmux_redraw_ready() {
  emulate -L zsh
  local fd="${1:-$_bmux_redraw_fd}"
  if [[ -n "$fd" ]]; then
    zle -F "$fd"
    exec {fd}<&-
  fi
  _bmux_redraw_fd=''
  (( _bmux_redraw_done )) && return 0
  _bmux_redraw_done=1
  zle reset-prompt
}

_bmux_redraw_line_init() {
  if (( !_bmux_redraw_done )) && [[ -z "$_bmux_redraw_fd" ]]; then
    exec {_bmux_redraw_fd}< <(
      sleep 0.05
      printf 'ready\\n'
    )
    zle -F "$_bmux_redraw_fd" _bmux_redraw_ready
  fi
}

add-zsh-hook precmd _bmux_redraw_precmd
zle -N zle-line-init _bmux_redraw_line_init
""".lstrip(),
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    base = Path(tempfile.mkdtemp(prefix="bmux_ghostty_prompt_redraw_"))
    try:
        home = base / "home"
