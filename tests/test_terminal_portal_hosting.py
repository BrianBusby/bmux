...
def _wait_portal_terminals(c: bmux, expected: int = 2, timeout: float = 8.0):
    ...
    bmuxError with the last-seen snapshot if it never settles within `timeout`.
    """
    start = time.time()
    terminals: list = []
    ...
    with bmux(SOCKET_PATH) as c:
        ...
        for row in terminals:
            if not row.get("in_window", False):
                raise bmuxError(f"terminal not attached to window: {row}")
            if row.get("portal") is not True:
                raise bmuxError(f"terminal is not portal-hosted: {row}")
            depth = row.get("view_depth")
            if not isinstance(depth, int):
                raise bmuxError(f"missing view_depth in surface_health: {row}")
            if depth > 8:
                raise bmuxError(f"terminal view depth too deep ({depth}): {row}")
