...
WRAPPER = ROOT / "Resources" / "bin" / "bmux-claude-wrapper"
...
def build_mutual_shim_tree(root: Path) -> tuple[Path, dict[str, str]]:
    bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-loop"
    delimit_primary_dir = root / "home" / ".delimit" / "shims"
    delimit_secondary_dir = root / "home" / ".delimit" / "managed-shims"
    real_dir = root / "real-bin"
    for directory in (bmux_shim_dir, delimit_primary_dir, delimit_secondary_dir, real_dir):
        directory.mkdir(parents=True, exist_ok=True)

    bmux_shim = bmux_shim_dir / "claude"
    shutil.copy2(WRAPPER, bmux_shim)
    bmux_shim.chmod(0o755)
...
""",
    ...
    managed_path = f"{bmux_shim_dir}:{delimit_primary_dir}:{delimit_secondary_dir}:{real_dir}:/usr/bin:/bin"
    env = {
        "HOME": str(root / "home"),
        "PATH": managed_path,
        "DELIMIT_MANAGED_PATH": managed_path,
        "BMUX_CLAUDE_WRAPPER_SHIM": str(bmux_shim),
        "BMUX_CLAUDE_WRAPPER_SHIM_ROOT": str(bmux_shim_dir),
    }
    return bmux_shim, env


def test_wrapper_stops_mutual_foreign_shim_loop(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-mutual-shim-loop-") as td:
        ...
        if "BMUX_CUSTOM_CLAUDE_PATH" not in combined_output:
            failures.append(f"expected BMUX_CUSTOM_CLAUDE_PATH remedy, got: {combined_output!r}")


def test_wrapper_stops_node_based_foreign_shim_loop(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-node-mutual-shim-loop-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-node-loop"
        node_primary_dir = root / "home" / ".node-shim" / "primary"
        node_secondary_dir = root / "home" / ".node-shim" / "secondary"
        real_dir = root / "real-bin"
        for directory in (bmux_shim_dir, node_primary_dir, node_secondary_dir, real_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        managed_path = f"{bmux_shim_dir}:{node_primary_dir}:{node_secondary_dir}:{real_dir}:{inherited_path}"
        env = {
            "HOME": str(root / "home"),
            "PATH": managed_path,
...
def test_wrapper_stops_indirect_shell_foreign_shim_loop(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-indirect-mutual-shim-loop-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-indirect-loop"
        shell_primary_dir = root / "home" / ".indirect-shim" / "primary"
        shell_secondary_dir = root / "home" / ".indirect-shim" / "secondary"
        real_dir = root / "real-bin"
        for directory in (bmux_shim_dir, shell_primary_dir, shell_secondary_dir, real_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        inherited_path = os.environ.get("PATH", "/usr/bin:/bin")
        managed_path = f"{bmux_shim_dir}:{shell_primary_dir}:{shell_secondary_dir}:{real_dir}:{inherited_path}"
        env = {
            "HOME": str(root / "home"),
            "PATH": managed_path,
...
def test_wrapper_guard_allows_child_claude_process(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-child-reentry-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-child"
        real_dir = root / "real-bin"
        for directory in (bmux_shim_dir, real_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        env = {
            "HOME": str(root / "home"),
            "PATH": f"{bmux_shim_dir}:{real_dir}:{inherited_path}",
            "BMUX_CLAUDE_WRAPPER_SHIM": str(bmux_shim),
            "BMUX_CLAUDE_WRAPPER_SHIM_ROOT": str(bmux_shim_dir),
        }
        result = subprocess.run(
...
def test_wrapper_allows_finite_layered_foreign_shims(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-finite-shim-chain-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-finite"
        shim_a_dir = root / "foreign-a"
        shim_b_dir = root / "foreign-b"
        real_dir = root / "real-bin"
        for directory in (bmux_shim_dir, shim_a_dir, shim_b_dir, real_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        env = {
            "HOME": str(root / "home"),
            "PATH": f"{bmux_shim_dir}:{shim_a_dir}:{shim_b_dir}:{real_dir}:/usr/bin:/bin",
            "BMUX_CLAUDE_WRAPPER_SHIM": str(bmux_shim),
            "BMUX_CLAUDE_WRAPPER_SHIM_ROOT": str(bmux_shim_dir),
        }
        result = subprocess.run(
...
def test_passthrough_real_node_claude_does_not_receive_guard_env(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-passthrough-guard-env-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-passthrough"
        foreign_shim_dir = root / "foreign-shim"
        real_dir = root / "real-bin"
        for directory in (bmux_shim_dir, foreign_shim_dir, real_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        write_executable(
            real_dir / "claude",
            """#!/usr/bin/env node
const upperGuard = process.env.BMUX_CLAUDE_WRAPPER_REEXEC_GUARD ?? "__unset__";
const upperTargets = process.env.BMUX_CLAUDE_WRAPPER_REEXEC_TARGETS ?? "__unset__";
const lowerGuard = process.env.bmux_claude_wrapper_reexec_guard ?? "__unset__";
const lowerTargets = process.env.bmux_claude_wrapper_reexec_targets ?? "__unset__";
const surface = process.env.BMUX_SURFACE_ID ?? "__unset__";
...
""",
        ...
        env = {
            "HOME": str(root / "home"),
            "PATH": f"{bmux_shim_dir}:{foreign_shim_dir}:{real_dir}:{inherited_path}",
            "BMUX_CLAUDE_WRAPPER_SHIM": str(bmux_shim),
            "BMUX_CLAUDE_WRAPPER_SHIM_ROOT": str(bmux_shim_dir),
            "BMUX_SOCKET_PATH": str(root / "missing.sock"),
            "BMUX_SURFACE_ID": "surface-passthrough",
        }
        result = subprocess.run(
            [str(bmux_shim), "--version"],
            env=env,
...
def test_custom_shim_path_with_colon_is_tracked_as_one_target(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-custom-shim-target-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-custom-colon"
        custom_shim_dir = root / "custom:shim"
        real_dir = root / "real-bin"
        for directory in (bmux_shim_dir, custom_shim_dir, real_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
export PATH="{bmux_shim_dir}:{real_dir}:/usr/bin:/bin"
exec claude "$@"
""",
        )
...
""",
        ...
        env = {
            "HOME": str(root / "home"),
            "PATH": f"{bmux_shim_dir}:{real_dir}:/usr/bin:/bin",
            "BMUX_CLAUDE_WRAPPER_SHIM": str(bmux_shim),
            "BMUX_CLAUDE_WRAPPER_SHIM_ROOT": str(bmux_shim_dir),
            "BMUX_CUSTOM_CLAUDE_PATH": str(custom_shim),
        }
        try:
            result = subprocess.run(
                [str(bmux_shim), "--version"],
...
def test_real_shell_claude_launcher_allows_child_claude_process(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-real-shell-launcher-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-shell-launcher"
        real_dir = root / "real-bin"
        package_dir = real_dir / "node_modules" / "@anthropic-ai" / "claude-code"
        for directory in (bmux_shim_dir, package_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
const { spawnSync } = require("node:child_process");
if (process.argv[2] === "child") {
  const guard = process.env.bmux_claude_wrapper_reexec_guard ?? "__unset__";
  const targets = process.env.bmux_claude_wrapper_reexec_targets ?? "__unset__";
...
def test_custom_shell_wrapper_execing_real_claude_allows_child_claude(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-custom-real-wrapper-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-custom-real"
        custom_dir = root / "custom-bin"
        real_dir = root / "real-bin"
        for directory in (bmux_shim_dir, custom_dir, real_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        env = {
            ...
            "BMUX_CLAUDE_WRAPPER_SHIM": str(bmux_shim),
            "BMUX_CLAUDE_WRAPPER_SHIM_ROOT": str(bmux_shim_dir),
            "BMUX_CUSTOM_CLAUDE_PATH": str(custom_wrapper),
        }
        result = subprocess.run(
            [str(bmux_shim)],
            env=env,
...
def test_interactive_finite_shim_chain_does_not_duplicate_hooks(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-interactive-finite-shim-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-interactive"
        foreign_shim_dir = root / "foreign-shim"
        real_dir = root / "real-bin"
        for directory in (bmux_shim_dir, foreign_shim_dir, real_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        socket_path = str(root / "bmux.sock")
        test_socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        try:
            test_socket.bind(socket_path)
            env = {
                "HOME": str(root / "home"),
                "PATH": f"{bmux_shim_dir}:{foreign_shim_dir}:{real_dir}:/usr/bin:/bin",
                "BMUX_CLAUDE_WRAPPER_SHIM": str(bmux_shim),
                "BMUX_CLAUDE_WRAPPER_SHIM_ROOT": str(bmux_shim_dir),
                "BMUX_SURFACE_ID": "surface-interactive",
        ...
        if len(session_start) != 1 or len(stop) != 3:
            failures.append(
                "expected one bmux hook injection after finite shim chain, "
...
def test_interactive_spawning_shim_chain_refreshes_claude_pid(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="bmux-claude-interactive-spawn-shim-") as td:
        root = Path(td)
        bmux_shim_dir = root / "tmp" / "bmux-cli-shims" / "surface-spawn"
...
""",
        ...
        write_executable(
            ...
            f"""#!/usr/bin/env node
const {{ spawnSync }} = require("node:child_process");
process.env.PATH = "{bmux_shim_dir}:{real_dir}:{inherited_path}";
const child = spawnSync("claude", process.argv.slice(2), {{
  env: process.env,
  encoding: "utf8",
}});
...
""",
        ...
        for line in lines:
            if line.startswith("arg="):
                ...
                values[key] = value
        if values.get("real_pid") != values.get("bmux_pid"):
            failures.append(f"expected BMUX_CLAUDE_PID to match real process pid, got: {combined_output!r}")
            return
        node_options = values.get("node_options", "")
        if "--require=" not in node_options or "--max-old-space-size=4096" not in node_options:
            failures.append(f"expected reentry to reinstall bmux NODE_OPTIONS, got: {combined_output!r}")
            return
