#!/usr/bin/env python3
"""
Regression test: the generated OMP extension is importable and emits bmux hook calls with complete payloads.
"""
...
class MockBmuxSocket:
    ...
    def __enter__(self) -> "MockBmuxSocket":
        try:
            self.path.unlink()
...
def verify_hook_persistence(cli_path: str, root: Path, base_env: dict[str, str]) -> bool:
    ...
    socket_path = Path("/tmp") / f"bmux-omp-hook-{os.getpid()}-{time.monotonic_ns()}.sock"
    launch_argv = [
    ...
    hook_env.update(
        {
            "PWD": str(workspace),
            "BMUX_SOCKET_PATH": str(socket_path),
            "BMUX_WORKSPACE_ID": workspace_id,
            "BMUX_SURFACE_ID": surface_id,
            "BMUX_AGENT_HOOK_STATE_DIR": str(hook_state_dir),
            "BMUX_AGENT_LAUNCH_KIND": "omp",
            "BMUX_AGENT_LAUNCH_EXECUTABLE": launch_argv[0],
            "BMUX_AGENT_LAUNCH_ARGV_B64": base64.b64encode(
                b"".join(value.encode("utf-8") + b"\0" for value in launch_argv)
            ).decode("ascii"),
            "BMUX_AGENT_LAUNCH_CWD": str(workspace),
    ...
    with MockBmuxSocket(socket_path, workspace_id=workspace_id, surface_id=surface_id) as server:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-omp-extension-") as td:
        root = Path(td)
        home = root / "home"
        home.mkdir()
        shared_agent_dir = root / "shared-agent-dir"
        shared_pi_extension = shared_agent_dir / "extensions" / "bmux-session.ts"
        shared_pi_extension.parent.mkdir(parents=True)
        shared_pi_extension.write_text("// bmux-pi-session-extension-marker v1\n", encoding="utf-8")
        ...
        # Pi's different bmux-session.ts filename in the same extensions folder.
        ...
        extension_path = shared_agent_dir / "extensions" / "bmux-omp-session.ts"
        if not extension_path.exists():
            print(f"FAIL: expected extension at {extension_path}")
            return 1
        extension_text = extension_path.read_text(encoding="utf-8")
        if "bmux-omp-session-extension-marker" not in extension_text:
            print(f"FAIL: expected bmux marker in {extension_path}")
            return 1
        if shared_pi_extension.read_text(encoding="utf-8") != "// bmux-pi-session-extension-marker v1\n":
...
""",
        ...
        check_env = env.copy()
        check_env["BMUX_TEST_OMP_EXTENSION_PATH"] = str(extension_path)
        check_env["BMUX_SURFACE_ID"] = "surface-omp-test"
        check_env["BMUX_OMP_BMUX_BIN"] = str(fake_bmux)
        ...
        check_source = """
const extensionPath = process.env.BMUX_TEST_OMP_EXTENSION_PATH;
...
"""
        ...
        if uninstall_foreign.returncode != 0 or not foreign_path.exists() or "Refusing to remove" not in uninstall_foreign.stdout:
            print("FAIL: omp extension uninstall did not preserve non-bmux file")
        ...
        invalid_extension_bytes = b"\xff\xfe\x00bmux-not-utf8"
        ...
        if "Failed to read" not in install_invalid_output or "not a bmux extension" in install_invalid_output:
        ...
        if "Failed to read" not in uninstall_invalid_output or "not a bmux extension" in uninstall_invalid_output:
        ...
        config_extension_path = config_override / "agent" / "extensions" / "bmux-omp-session.ts"
        ...
        if config_uninstall.returncode != 0 or config_extension_path.exists():
            ...
            return 1
    print("PASS: generated OMP extension installs, emits complete bmux hook payloads, and persists hook sessions")
    return 0
