...
bmux wrapper .zshenv should only source Ghostty zsh integration when Ghostty
...
def _run_case(
    *,
    ...
    env["BMUX_TEST_OUT"] = str(out_path)

    # Keep input deterministic and local to this test.
    for key in (
        "GHOSTTY_ZSH_ZDOTDIR",
        "BMUX_ZSH_ZDOTDIR",
        "BMUX_ORIGINAL_ZDOTDIR",
    ...
    if ghostty_enabled:
        ...
        env["BMUX_ZSH_ZDOTDIR"] = str(orig_zdotdir)
...
def main() -> int:
    ...
    base = Path("/tmp") / f"bmux_issue_734_{os.getpid()}"
    try:
