#!/usr/bin/env python3
"""
Regression test: `bmux claude-teams` preserves fallback provider dirs in PATH.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-claude-teams-fallback-path-") as td:
        tmp = Path(td)
        home = tmp / "home"
...
""",
        ...
        env.pop("BMUX_CUSTOM_CLAUDE_PATH", None)
        ...
        if proc.returncode != 0:
            print("FAIL: `bmux claude-teams --version` failed with Claude in a fallback dir")
    ...
    print("PASS: bmux claude-teams preserves fallback provider dirs in PATH")
    return 0
