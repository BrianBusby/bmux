...
def main() -> int:
    with bmux(SOCKET_PATH) as c:
        ...
        panes = c.list_panes()
        if len(panes) < 2:
            raise bmuxError(f"expected >=2 panes after first split, got {len(panes)}: {panes}")
        ...
        if underflows != 0:
            ...
            raise bmuxError(f"bonsplit arranged-subview underflow observed ({underflows}); screenshot: {shot}")
