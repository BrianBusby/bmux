#!/usr/bin/env python3
"""Behavior checks for the no-socket `bmux config doctor` command."""
...
def resolve_bmux_cli() -> str:
    ...
    candidates = [
        path
        for path in glob.glob(os.path.expanduser("~/Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/bmux"))
...
def run_cli(
    ...
    env["BMUX_SOCKET_PATH"] = str(home / "missing.sock")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-config-doctor-") as temp:
        home = Path(temp)
        workspace = home / "workspace" / "child"
        workspace.mkdir(parents=True)
        (home / "bmux.json").write_text('{"homeLevel": true,,}\n', encoding="utf-8")
        config_path = home / ".config" / "bmux" / "bmux.json"
        config_path.parent.mkdir(parents=True)
        config_path.write_text(
            """
            {
              // JSONC comments and trailing commas are valid in bmux.json.
              "schemaVersion": 1,
              "app": {
                "appearance": "system",
              },
        ...
        if default_result.returncode != 0:
            ...
            if payload is not None:
                ...
                if not isinstance(findings, list):
                    ...
                    if any(
                        isinstance(finding, dict) and finding.get("path") == str(home / "bmux.json")
                        for finding in findings
                    ):
                        failures.append(f"default scan included home-level bmux.json: {default_result.stdout}")
        ...
        if bad_result.returncode == 0:
            ...
            if "bmux config doctor found 1 error(s)" not in bad_result.stderr:
    ...
    print("PASS: bmux config doctor validates JSONC and reports syntax errors")
    return 0
