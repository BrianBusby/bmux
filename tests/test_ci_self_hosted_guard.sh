...
check_build_lag_deriveddata_cache_path() {
  if ! awk '
    ...
    in_prepare && /DERIVED_DATA_PATH="\$RUNNER_TEMP\/bmux-deriveddata-tests-build-and-lag"/ { saw_prepare_path=1 }
...
check_release_helper_artifact_from_package_lane() {
  if ! awk '
    ...
    in_job && /BMUX_CI_HELPER_XCODE_APP:/ { saw_helper_xcode_env=1 }
    in_job && /- name: Select helper Xcode/ { saw_helper_select=1; next }
    in_job && /BMUX_CI_REQUIRED_MACOS_SDK_MAJOR=15/ { saw_helper_sdk_pin=1 }
    ...
    in_job && /name:[[:space:]]*bmux-ghostty-cli-helper/ { saw_artifact_name=1 }
...
check_runtime_regressions_collapsed() {
  ...
  if ! awk '
    ...
    in_func && /BMUX_UI_TEST_TARGET_DISPLAY_ID="\$persistent_display_id"/ { saw_display_env=1 }
...
case "${1:-}" in
  add-certificates)
    printf '%s\n' "$*" >> "$BMUX_STUB_SECURITY_LOG"
    ;;
  find-certificate)
    added_count="$(grep -c '^add-certificates ' "$BMUX_STUB_SECURITY_LOG" 2>/dev/null || true)"
    if [[ "${BMUX_STUB_CERT_COUNT_OVERRIDE:-}" != "" ]]; then
      added_count="$BMUX_STUB_CERT_COUNT_OVERRIDE"
    fi
...
EOF
  ...
  if ! PATH="$bin_dir:/usr/bin:/bin" BMUX_STUB_CURL_LOG="$curl_log" BMUX_STUB_SECURITY_LOG="$security_log" "$helper" "$keychain" >"$tmp_dir/success.out" 2>"$tmp_dir/success.err"; then
  ...
  if ! PATH="$bin_dir:/usr/bin:/bin" BMUX_STUB_CURL_LOG="$fb_curl_log" BMUX_STUB_SECURITY_LOG="$fb_security_log" "$fb_helper" "$fb_keychain" >"$tmp_dir/fb.out" 2>"$tmp_dir/fb.err"; then
  ...
  if PATH="$bin_dir:/usr/bin:/bin" BMUX_STUB_CURL_LOG="$curl_log" BMUX_STUB_SECURITY_LOG="$security_log" BMUX_STUB_CERT_COUNT_OVERRIDE=1 "$helper" "$keychain" >"$tmp_dir/fail.out" 2>"$tmp_dir/fail.err"; then
...
check_create_dmg_uses_run_local_npm_prefix() {
  for file in "$ROOT_DIR/.github/workflows/nightly.yml" "$ROOT_DIR/.github/workflows/release.yml"; do
    if ! awk '
      ...
      in_step && /^[[:space:]]*- name:/ { in_step=0 }
      in_step && /BMUX_NODE_BIN="\$\(command -v node\)"/ { saw_node=1 }
      ...
      in_step && /exec "\$BMUX_NODE_BIN" "\$npm_config_prefix\/lib\/node_modules\/create-dmg\/cli\.js" "\\\$@"/ { saw_exec=1 }
...
check_gui_smoke_unsupported_launch_handling() {
  ...
  for needle in \
    'ALLOW_UNSUPPORTED_GUI="${BMUX_SMOKE_ALLOW_UNSUPPORTED_GUI:-0}"' \
    'DIRECT_EXEC="${BMUX_SMOKE_DIRECT_EXEC:-0}"' \
    'BMUX_UI_TEST_MODE="${BMUX_UI_TEST_MODE:-1}"' \
  ...
  if ! awk '
    /scripts\/smoke-launch-macos-app\.sh/ && /BMUX_SMOKE_ALLOW_UNSUPPORTED_GUI=1/ { saw_launchservices=1 }
    /scripts\/smoke-launch-macos-app\.sh/ && /BMUX_SMOKE_DIRECT_EXEC=1/ { saw_direct_exec=1 }
...
check_web_db_behavior_tests() {
  ...
  if ! awk '
    ...
    in_step && /^[[:space:]]*- name:/ { in_step=0 }
    in_step && /BMUX_DB_TEST:[[:space:]]*"1"/ { saw_env=1 }
    in_step && /bun run test:db:behavior/ { saw_runner=1 }
    END { exit !(saw_env && saw_runner) }
  ' "$CI_FILE"; then
    echo "FAIL: ci.yml must run the DB behavior test discovery runner with BMUX_DB_TEST=1"
    exit 1
  fi

  if ! grep -Fq 'grep -q "process\\.env\\.BMUX_DB_TEST"' "$db_runner"; then
    echo "FAIL: DB behavior runner must discover BMUX_DB_TEST-gated files instead of hard-coding a subset"
    exit 1
  fi
...
check_tmux_terminal_nightly_isolation() {
  ...
  if ! awk '
    ...
    in_job && /^  [^[:space:]#][^:]*:[[:space:]]*(#.*)?$/ { in_job=0 }
    in_job && /BMUX_DERIVED_DATA_PATH/ { saw_env=1 }
    in_job && /-derivedDataPath "\$BMUX_DERIVED_DATA_PATH"/ { saw_flag=1 }
...
check_no_self_hosted_fleet_runners() {
  ...
  local fleet='macos-26|warp-macos-26-arm64-6x|bmux-aws-macos|bmux-macos|bmux-local-macos|macfleet|(^|[^a-z0-9-])mac4([^a-z0-9]|$)|(^|[^a-z0-9-])mac-mini([^a-z0-9]|$)|slot-[0-9]|xcode-[0-9]+-[0-9]|(^|[^a-z0-9-])bmux([^a-z0-9-]|$)'
  ...
  local probe
  for probe in 'runs-on: macfleet' '- mac4' '- mac-mini' '- slot-3' '- xcode-26-3' '- bmux' \
               "runs-on: \${{ vars.X || 'macos-26' }}" '- warp-macos-26-arm64-6x' \
               '- bmux-aws-macos-15' '- bmux-macos-26' '- self-hosted' '- macOS' '- ARM64' \
  ...
  # before matching the value so the checkout path (which contains "bmux") can
  # never match the bare `bmux` label.
