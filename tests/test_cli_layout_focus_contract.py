...
def run_cli(
    ...
    env = dict(os.environ)
    for key in ["BMUX_WORKSPACE_ID", "BMUX_SURFACE_ID", "BMUX_TAB_ID"]:
        env.pop(key, None)
    if env_overrides:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-layout-focus-") as tmp:
        socket_path = str(Path(tmp) / "bmux.sock")
        state = FakeBmuxState()
        server = ThreadedUnixServer(socket_path, FakeBmuxHandler)
        server.state = state
