...
Run with a tagged instance:
  BMUX_TAG=<tag> python3 tests/test_split_cwd_inheritance.py
"""
...
from bmux import bmux  # noqa: E402
...
def _focus_first_surface(client: bmux) -> str:
...
def main() -> int:
    ...
    client = bmux(socket_path=socket_path)
    client.connect()

    # Use resolved paths to avoid /tmp -> /private/tmp symlink mismatch on macOS
    test_dir_a = str(Path("/tmp/bmux_split_cwd_test_a").resolve())
    test_dir_b = str(Path("/tmp/bmux_split_cwd_test_b").resolve())
