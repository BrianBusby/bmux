...
def _shell_command() -> str:
    return textwrap.dedent(
        ...
        _bmux_github_repo_slug_for_path "$BMUX_TEST_REPO"
        """
    )
...
def _run_case(
    ...
    *,
    ...
    if output != "manaflow-ai/bmux":
        return 1, f"{shell} {config_mode}: expected manaflow-ai/bmux, got {output!r}"
...
def main() -> int:
    ...
    base = Path("/tmp") / f"bmux_shell_git_config_remote_url_{os.getpid()}"
    try:
