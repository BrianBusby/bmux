...
def write_marker(home: str, marker_name: str, socket_path: str) -> None:
    app_support = os.path.join(home, ".local", "state", "bmux")
...
def temporary_socket_home(prefix: str) -> tempfile.TemporaryDirectory:
    ...
    # roots short because stable sockets live under ~/.local/state/bmux.
...
def bundled_cli_for_variant(cli_path: str, root: str, app_name: str, bundle_id: str) -> str:
    ...
    bundled_cli = os.path.join(bin_dir, "bmux")
...
def run_ping(
    ...
    env.pop("BMUX_BUNDLE_ID", None)
    env.pop("BMUX_TAG", None)
...
def expect_ping_uses_socket(cli_path: str, home: str, socket_path: str, label: str) -> bool:
    ...
    try:
        ...
        print(f"FAIL: invoking {label} bmux ping failed: {exc}")
        return False
    finally:
    ...
    if proc.returncode != 0:
        print(f"FAIL: {label} bmux ping returned non-zero status")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False

    if proc.stdout.strip() != "PONG":
        print(f"FAIL: {label} bmux ping did not use the expected socket")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False
...
def expect_ping_ignores_dev_tag(
    ...
    try:
        proc = run_ping(cli_path, home, extra_env={"BMUX_TAG": rogue_tag})
    except Exception as exc:
        print(f"FAIL: invoking {label} bmux ping failed: {exc}")
        return False
    finally:
    ...
    if proc.stdout.strip() != "PONG":
        print(f"FAIL: {label} bmux ping followed BMUX_TAG to the rogue dev socket")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False
...
def expect_ping_does_not_use_socket(
    ...
    try:
        ...
        print(f"FAIL: invoking {label} bmux ping failed unexpectedly: {exc}")
        return False
    finally:
        server.join(timeout=2.0)
        try:
    ...
    if proc.stdout.strip() == "WRONG":
        print(f"FAIL: {label} bmux ping used the stable socket fallback")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False
...
def python_client_default_bundle_id(extra_env: dict[str, str]) -> str:
    ...
    proc = subprocess.run(
        [sys.executable, "-c", "from bmux import bmux; print(bmux.default_bundle_id())"],
        text=True,
        capture_output=True,
        env=env,
        timeout=8,
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"bmux.py bundle resolution failed: {proc.stderr!r}")
...
def python_client_default_socket_path(extra_env: dict[str, str]) -> str:
    ...
    proc = subprocess.run(
        [sys.executable, "-c", "from bmux import bmux; print(bmux.default_socket_path())"],
        text=True,
        capture_output=True,
        env=env,
        timeout=8,
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"bmux.py socket resolution failed: {proc.stderr!r}")
    return proc.stdout.strip()


def test_python_client_ignores_unknown_bundle_env() -> bool:
    expected_tagged_debug = "com.bmuxterm.app.debug.variant.test.tag"
    actual = python_client_default_bundle_id({
        "BMUX_BUNDLE_ID": "com.example.stale.bundle",
        "BMUX_TAG": "variant-test-tag",
    })
    if actual != expected_tagged_debug:
        print("FAIL: python client trusted unknown BMUX_BUNDLE_ID over BMUX_TAG")
    ...
    actual = python_client_default_bundle_id({
        ...
        "BMUX_TAG": "rogue-stable-tag",
    })
    if actual != "com.bmuxterm.app":
        print("FAIL: python client rejected known stable BMUX_BUNDLE_ID")
        print(f"actual={actual!r}")
        return False

    print("PASS: python client ignores unknown BMUX_BUNDLE_ID values")
    return True


def test_python_client_treats_stable_override_as_implicit() -> bool:
    tag = f"python-stale-stable-{os.getpid()}"
    expected_socket = f"/tmp/bmux-debug-{tag}.sock"

    with temporary_socket_home("bmux-py-") as home:
        app_support = os.path.join(home, ".local", "state", "bmux")
        os.makedirs(app_support, exist_ok=True)
        stable_socket = os.path.join(app_support, "bmux.sock")
        ...
        try:
            ...
            actual = python_client_default_socket_path({
                ...
                "BMUX_SOCKET_PATH": stable_socket,
                "BMUX_TAG": tag,
            })
        finally:
            server.close()
    ...
    if actual != expected_socket:
        print("FAIL: python client followed a stale stable BMUX_SOCKET_PATH")
...
def test_variant_last_socket_markers(cli_path: str) -> bool:
    pid = os.getpid()
    stable_socket = f"/tmp/bmux-issue3542-stable-{pid}.sock"
    nightly_socket = f"/tmp/bmux-issue3542-nightly-{pid}.sock"
    dev_agent_socket = f"/tmp/bmux-issue3542-dev-agent-{pid}.sock"
    rogue_stable_socket = f"/tmp/bmux-debug-rogue-stable-{pid}.sock"
    rogue_stable_tag = f"rogue-stable-{pid}"
    rogue_nightly_socket = f"/tmp/bmux-debug-rogue-nightly-{pid}.sock"
    rogue_nightly_tag = f"rogue-nightly-{pid}"
    rogue_dev_agent_socket = f"/tmp/bmux-debug-rogue-dev-agent-{pid}.sock"
    rogue_dev_agent_tag = f"rogue-dev-agent-{pid}"

    with temporary_socket_home("bmux-home-") as home, \
            tempfile.TemporaryDirectory(prefix="bmux-cli-variant-apps-") as apps:
        ...
        isolated_nightly_cli = bundled_cli_for_variant(
            cli_path,
            apps,
            "bmux NIGHTLY issue3542",
            "com.bmuxterm.app.nightly.issue3542",
        )
        dev_agent_cli = bundled_cli_for_variant(
            cli_path,
            apps,
            "bmux DEV agent",
            "com.bmuxterm.app.debug.agent",
        )
...
def test_base_debug_cli_discovers_bmux_tag(cli_path: str) -> bool:
    tag = f"cli-autodiscover-{os.getpid()}"
    socket_path = f"/tmp/bmux-debug-{tag}.sock"
    server = PingServer(socket_path)
    server.start()
    ...
    env = os.environ.copy()
    env["BMUX_SOCKET_PATH"] = "/tmp/bmux.sock"
    env["BMUX_TAG"] = tag
    env["BMUX_CLI_SENTRY_DISABLED"] = "1"
    env["BMUX_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"

    try:
        with tempfile.TemporaryDirectory(prefix="bmux-cli-base-debug-app-") as apps:
        ...
        print(f"FAIL: invoking bmux ping failed: {exc}")
        return False
    finally:
        server.join(timeout=2.0)
        try:
    ...
    if proc.returncode != 0:
        print("FAIL: bmux ping returned non-zero status")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False

    if proc.stdout.strip() != "PONG":
        print("FAIL: bmux ping did not use auto-discovered socket")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False
...
def main() -> int:
    ...
    if not test_base_debug_cli_discovers_bmux_tag(cli_path):
        return 1
    ...
    print("PASS: bmux ping auto-discovers tagged socket from BMUX_TAG")
    return 0
