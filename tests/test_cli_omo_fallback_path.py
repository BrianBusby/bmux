#!/usr/bin/env python3
"""
Regression test: `bmux omo` preserves fallback OpenCode dirs in PATH.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-omo-fallback-path-") as td:
        root = Path(td)
...
""",
        ...
        env["BMUX_SOCKET_PATH"] = str(root / "missing.sock")
        ...
        if proc.returncode != 0:
            print("FAIL: `bmux omo` failed with OpenCode in a fallback dir")
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-omo-fallback-install-path-") as td:
        root = Path(td)
        fallback_bin = root / ".bun" / "bin"
        fallback_bin.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `bmux omo` failed to install OMO with bun in a fallback dir")
            print(f"exit={proc.returncode}")
            print(f"stdout={proc.stdout.strip()}")
            print(f"stderr={proc.stderr.strip()}")
            return 1
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-omo-skip-app-bundle-path-") as td:
        root = Path(td)
        fallback_bin = root / ".bun" / "bin"
        stale_app_bin = root / "Older bmux.app" / "Contents" / "Resources" / "bin"
        fallback_bin.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `bmux omo` failed when stale app-bundled OpenCode was first on PATH")
            print(f"exit={proc.returncode}")
            print(f"stdout={proc.stdout.strip()}")
            print(f"stderr={proc.stderr.strip()}")
            return 1
    ...
    print("PASS: bmux omo preserves fallback OpenCode dirs in PATH")
    return 0
