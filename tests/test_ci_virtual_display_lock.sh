...
TMP_DIR="$(mktemp -d)"
LOCK_DIR="$TMP_DIR/bmux-test-virtual-display.lock"
...
LOCK_ENV="$(
  ...
  BMUX_VDISPLAY_LOCK_TIMEOUT_SECONDS=2 \
  BMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
  "$SCRIPT" acquire
)"
eval "$LOCK_ENV"

if [ ! -d "$BMUX_VDISPLAY_LOCK_DIR" ] || [ ! -f "$BMUX_VDISPLAY_LOCK_DIR/token" ]; then
  echo "FAIL: acquire did not create tokenized lock" >&2
  exit 1
fi

RUNNER_TEMP="$TMP_DIR" \
BMUX_VDISPLAY_LOCK_DIR="$BMUX_VDISPLAY_LOCK_DIR" \
BMUX_VDISPLAY_LOCK_TOKEN="$BMUX_VDISPLAY_LOCK_TOKEN" \
  "$SCRIPT" set-owner "$$"

if [ "$(cat "$BMUX_VDISPLAY_LOCK_DIR/owner_pid")" != "$$" ]; then
...
if RUNNER_TEMP="$TMP_DIR" \
  BMUX_VDISPLAY_LOCK_DIR="$LOCK_DIR" \
  BMUX_VDISPLAY_LOCK_TIMEOUT_SECONDS=1 \
  BMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
  "$SCRIPT" acquire >/tmp/bmux-vdisplay-second-acquire.out 2>/tmp/bmux-vdisplay-second-acquire.err; then
  cat /tmp/bmux-vdisplay-second-acquire.out
  cat /tmp/bmux-vdisplay-second-acquire.err >&2
...
{
  printf 'created_at=1\n'
  printf 'token=%s\n' "$BMUX_VDISPLAY_LOCK_TOKEN"
} > "$BMUX_VDISPLAY_LOCK_DIR/metadata"

if RUNNER_TEMP="$TMP_DIR" \
  BMUX_VDISPLAY_LOCK_DIR="$LOCK_DIR" \
  BMUX_VDISPLAY_LOCK_TIMEOUT_SECONDS=1 \
  BMUX_VDISPLAY_LOCK_STALE_SECONDS=1 \
  BMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
  "$SCRIPT" acquire >/tmp/bmux-vdisplay-live-owner-acquire.out 2>/tmp/bmux-vdisplay-live-owner-acquire.err; then
  cat /tmp/bmux-vdisplay-live-owner-acquire.out
  cat /tmp/bmux-vdisplay-live-owner-acquire.err >&2
  echo "FAIL: stale cleanup removed a lock whose owner PID was alive" >&2
  exit 1
fi

if ps -p 1 >/dev/null 2>&1; then
  printf '1\n' > "$BMUX_VDISPLAY_LOCK_DIR/owner_pid"
  {
    printf 'created_at=1\n'
    printf 'token=%s\n' "$BMUX_VDISPLAY_LOCK_TOKEN"
  } > "$BMUX_VDISPLAY_LOCK_DIR/metadata"
  ...
  if RUNNER_TEMP="$TMP_DIR" \
    ...
    BMUX_VDISPLAY_LOCK_STALE_SECONDS=1 \
    BMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
    "$SCRIPT" acquire >/tmp/bmux-vdisplay-foreign-owner-acquire.out 2>/tmp/bmux-vdisplay-foreign-owner-acquire.err; then
    cat /tmp/bmux-vdisplay-foreign-owner-acquire.out
    cat /tmp/bmux-vdisplay-foreign-owner-acquire.err >&2
...
RUNNER_TEMP="$TMP_DIR" \
BMUX_VDISPLAY_LOCK_DIR="$BMUX_VDISPLAY_LOCK_DIR" \
BMUX_VDISPLAY_LOCK_TOKEN="wrong-token" \
  "$SCRIPT" release 2>/tmp/bmux-vdisplay-wrong-token-release.err

if [ ! -d "$BMUX_VDISPLAY_LOCK_DIR" ]; then
  echo "FAIL: release removed a lock with the wrong token" >&2
  exit 1
fi

rm -f "$BMUX_VDISPLAY_LOCK_DIR/owner_pid"
{
  printf 'created_at=1\n'
  printf 'token=%s\n' "$BMUX_VDISPLAY_LOCK_TOKEN"
} > "$BMUX_VDISPLAY_LOCK_DIR/metadata"
...
OWNERLESS_LOCK_ENV="$(
  ...
  BMUX_VDISPLAY_LOCK_DIR="$LOCK_DIR" \
  BMUX_VDISPLAY_LOCK_TIMEOUT_SECONDS=2 \
  BMUX_VDISPLAY_LOCK_STALE_SECONDS=1800 \
  BMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
  "$SCRIPT" acquire 2>/tmp/bmux-vdisplay-ownerless-acquire.err
)"
...
DEAD_OWNER_LOCK_ENV="$(
  ...
  BMUX_VDISPLAY_LOCK_STALE_SECONDS=1800 \
  BMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
  "$SCRIPT" acquire 2>/tmp/bmux-vdisplay-dead-owner-acquire.err
)"
...
REAP_LOCK_DIR="$TMP_DIR/bmux-test-reap.lock"
REAP_ENV="$(
  RUNNER_TEMP="$TMP_DIR" \
  BMUX_VDISPLAY_LOCK_DIR="$REAP_LOCK_DIR" \
  "$SCRIPT" acquire
)"
eval "$REAP_ENV"
...
if RUNNER_TEMP="$TMP_DIR" BMUX_VDISPLAY_LOCK_DIR="$BMUX_VDISPLAY_LOCK_DIR" \
