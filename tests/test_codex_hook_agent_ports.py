...
Validates:
1) `bmux hooks codex session-start` records the inferred agent root PID.
...
def _wait_for_port(client: bmux, workspace_id: str, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
...
def _wait_for_port_absent(client: bmux, workspace_id: str, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
        state = _parse_sidebar_state(client.sidebar_state(tab=workspace_id))
        raw = state.get("ports", "")
        if raw == "none" or not raw:
...
host = os.environ["BMUX_TEST_BIND_HOST"]
port = int(os.environ["BMUX_TEST_PORT"])
cli_path = os.environ["BMUX_TEST_CLI_PATH"]
socket_path = os.environ["BMUX_TEST_SOCKET_PATH"]
workspace_id = os.environ["BMUX_WORKSPACE_ID"]
surface_id = os.environ["BMUX_SURFACE_ID"]
state_dir = os.environ["BMUX_AGENT_HOOK_STATE_DIR"]
session_id = os.environ["BMUX_TEST_SESSION_ID"]
cwd = os.environ["BMUX_TEST_CWD"]
ready_file = Path(os.environ["BMUX_TEST_READY_FILE"])
start_file = Path(os.environ["BMUX_TEST_START_FILE"])
server_pid_file = Path(os.environ["BMUX_TEST_SERVER_PID_FILE"])
server_log_file = Path(os.environ["BMUX_TEST_SERVER_LOG_FILE"])

hook_env = os.environ.copy()
hook_env["BMUX_SOCKET_PATH"] = socket_path
hook_env["BMUX_WORKSPACE_ID"] = workspace_id
hook_env["BMUX_SURFACE_ID"] = surface_id
hook_env["BMUX_AGENT_HOOK_STATE_DIR"] = state_dir
...
""",
    ...
    env = os.environ.copy()
    env["BMUX_TEST_BIND_HOST"] = _PREFERRED_BIND_HOST
    env["BMUX_TEST_PORT"] = str(port)
    env["BMUX_TEST_CLI_PATH"] = cli_path
    env["BMUX_TEST_SOCKET_PATH"] = socket_path
    env["BMUX_WORKSPACE_ID"] = workspace_id
    env["BMUX_SURFACE_ID"] = surface_id
    env["BMUX_AGENT_HOOK_STATE_DIR"] = str(state_path.parent)
    env["BMUX_TEST_SESSION_ID"] = session_id
    env["BMUX_TEST_CWD"] = str(cwd)
    env["BMUX_TEST_READY_FILE"] = str(ready_file)
    env["BMUX_TEST_START_FILE"] = str(start_file)
    env["BMUX_TEST_SERVER_PID_FILE"] = str(server_pid_file)
    env["BMUX_TEST_SERVER_LOG_FILE"] = str(server_log_file)
...
def main() -> int:
    ...
    state_dir = Path(tempfile.gettempdir()) / f"bmux_codex_hook_state_{os.getpid()}"
    state_path = state_dir / "codex-hook-sessions.json"
    lock_path = Path(str(state_path) + ".lock")
    base = Path(tempfile.gettempdir()) / f"bmux_codex_ports_{os.getpid()}"
    ...
    except (bmuxError, RuntimeError, AssertionError, ValueError) as exc:
        return fail(str(exc))
    finally:
