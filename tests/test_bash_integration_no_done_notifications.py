#!/usr/bin/env python3
"""
Regression coverage for bash job-control noise from bmux shell integration.
...
def test_bash_integration_does_not_emit_done_notifications() -> None:
    ...
    integration_script = repo_root / "Resources/shell-integration/bmux-bash-integration.bash"

    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        socket_path = tmp_path / "bmux.sock"
        ...
        try:
            ...
            with InteractiveBash(env) as bash:
                ...
                bash.run(f"export BMUX_SOCKET_PATH={shlex.quote(str(socket_path))}")
                bash.run("export BMUX_TAB_ID=tab-test")
                bash.run("export BMUX_PANEL_ID=panel-test")
                bash.run("_BMUX_TTY_NAME=ttys-test")
                ...
                bash.run("true")
                bash.run("sleep 0.2")
                bash.run("echo __BMUX_DONE_CHECK__")
