#!/usr/bin/env python3
"""
Regression coverage for https://github.com/manaflow-ai/bmux/issues/6714.

When a user enables ``setopt noclobber`` in their interactive zsh, bmux's shell
integration prints a spurious error on startup:

    _bmux_install_cli_command_shim:13: file exists: \
        /var/folders/.../T//bmux-cli-shims/<surface-id>/claude

Root cause: ``Resources/shell-integration/bmux-zsh-integration.zsh`` writes a
per-surface CLI shim with a plain ``>`` redirection::

    } >"$shim_path" 2>/dev/null || return 0

``_bmux_install_cli_command_shim`` runs more than once per shell (once at source
time via the top-level ``_bmux_install_cli_wrapper claude`` call, and again on
the first prompt via the ``_bmux_fix_path`` precmd hook). The second write
...
The fix is zsh's explicit clobber redirection (``>|``) for this bmux-owned
...
INTEGRATION = REPO_ROOT / "Resources/shell-integration/bmux-zsh-integration.zsh"

# Drive the real shim writer twice under noclobber. The first call creates the
# shim; the second must overwrite the existing file (bmux owns it) without
...
#
# Sourcing the integration with no BMUX_SHELL_INTEGRATION_DIR makes the
# top-level `_bmux_install_cli_wrapper claude` early-return (no shim written at
...
DRIVER = r"""
setopt noclobber
source "$BMUX_ZSH_INTEGRATION" 2>/dev/null
export TMPDIR="$BMUX_TEST_TMPDIR"
export BMUX_SURFACE_ID="$BMUX_TEST_SURFACE_ID"
_bmux_install_cli_command_shim claude "$BMUX_TEST_WRAPPER_A"
_bmux_install_cli_command_shim claude "$BMUX_TEST_WRAPPER_B"
"""
...
def _run_driver(tmp: Path) -> subprocess.CompletedProcess[str]:
    ...
    # Clean env: drop ambient BMUX_* so nothing the integration reads leaks in,
    # and explicitly disable the socket/ghostty paths so sourcing stays quiet.
    env = {key: value for key, value in os.environ.items() if not key.startswith("BMUX")}
    env.update(
        {
            "LC_ALL": "C",
            "LANG": "C",
            "BMUX_ZSH_INTEGRATION": str(INTEGRATION),
            "BMUX_TEST_TMPDIR": str(tmp),
            "BMUX_TEST_SURFACE_ID": "issue-6714-shim",
            "BMUX_TEST_WRAPPER_A": str(wrapper_a),
            "BMUX_TEST_WRAPPER_B": str(wrapper_b),
            # Keep sourcing side-effect-free: no socket sends, no nested ghostty
            # integration.
            "BMUX_SOCKET_PATH": "",
            "BMUX_LOAD_GHOSTTY_ZSH_INTEGRATION": "0",
...
def test_zsh_shim_refresh_is_silent_and_refreshes_under_noclobber() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="bmux-6714-") as td:
        tmp = Path(td)
        ...
        assert "file exists" not in proc.stderr.lower(), (
            "bmux printed a noclobber 'file exists' error while refreshing its own "
        ...
        assert "_bmux_install_cli_command_shim" not in proc.stderr, (
            "the shim writer reported an error to stderr while refreshing the shim"
            + debug
        )

        shim_path = tmp / "bmux-cli-shims" / "issue-6714-shim" / "claude"
        ...
        assert f'bmux_wrapper="{wrapper_b}"' in contents, (
            "bmux did not refresh its generated shim on the second write under "
            f"noclobber (expected wrapper {wrapper_b!r}).\n--- shim ---\n{contents}"
            + debug
        )
        assert f'bmux_wrapper="{wrapper_a}"' not in contents, (
...
if __name__ == "__main__":
    ...
    print("PASS: bmux refreshes its zsh CLI shim silently under noclobber")
