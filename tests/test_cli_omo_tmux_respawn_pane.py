#!/usr/bin/env python3
"""
Regression tests for OMO subagent panes through bmux's tmux compatibility shim.
"""
...
def shell_wrapped(command: str) -> str:
    """Mirror BMUXCLI.tmuxShellInvokedStartCommand: respawn shell-commands are run
...
def run_cli(
    ...
    env["BMUX_WORKSPACE_ID"] = "workspace:1"
    env["BMUX_SURFACE_ID"] = "surface:1"
    env["TMUX_PANE"] = f"%{PANE_ID}"
    env["HOME"] = str(fake_home)
    env["BMUX_OMO_BMUX_BIN"] = cli_path
...
def assert_caller_pane_respawn_uses_caller_surface(
    ...
    if respawn_params.get("surface_id") != SURFACE_ID:
        raise AssertionError(
            "caller pane respawn should prefer BMUX_SURFACE_ID over the pane's selected tab: "
            f"{respawn_params!r}"
        )
    state.respawn_params.clear()
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="bmux-omo-respawn-") as td:
            tmp = Path(td)
