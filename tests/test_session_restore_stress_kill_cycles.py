...
5) Clean quit, then corrupt the primary session snapshot -> relaunch: bmux
   ...
   `bmux restore-session` keeps working.
"""
...
# because the bmux claude wrapper inserts its own arguments around --resume.
SESSION_SPECS = [
    ("claude", "claude-stress-0", ("BMUX_FAKE_CLAUDE_RESUME:", "--resume claude-stress-0")),
    ("claude", "claude-stress-1", ("BMUX_FAKE_CLAUDE_RESUME:", "--resume claude-stress-1")),
    ("codex", "codex-stress-0", ("BMUX_FAKE_CODEX_RESUME:", "resume codex-stress-0")),
    ("codex", "codex-stress-1", ("BMUX_FAKE_CODEX_RESUME:", "resume codex-stress-1")),
    ("opencode", "opencode-stress-0", ("BMUX_FAKE_OPENCODE_RESUME:", "--session opencode-stress-0")),
    ("opencode", "opencode-stress-1", ("BMUX_FAKE_OPENCODE_RESUME:", "--session opencode-stress-1")),
]
...
def _snapshot_path(bundle_id: str, suffix: str = "") -> Path:
    ...
    return Path.home() / "Library/Application Support/bmux" / f"session-{safe_bundle}{suffix}.json"
...
def _app_pids(app_path: Path) -> list[int]:
    exe = app_path / "Contents" / "MacOS" / "bmux DEV"
...
def _launch(app_path: Path, socket_path: Path, env_overrides: dict[str, str] | None = None) -> None:
    ...
    full_env["BMUX_SOCKET_PATH"] = str(socket_path)
    full_env["BMUX_ALLOW_SOCKET_OVERRIDE"] = "1"
...
def _connect(socket_path: Path) -> bmux:
    client = bmux(socket_path=str(socket_path))
    client.connect()
    if not client.ping():
...
def _read_scrollback(client: bmux) -> str:
...
def _collect_all_scrollbacks(client: bmux) -> str:
    chunks: list[str] = []
...
def main() -> int:
    app_path_str = os.environ.get("BMUX_APP_PATH", "").strip()
    if not app_path_str:
        print("SKIP: set BMUX_APP_PATH to a built bmux DEV .app path")
        return 0
    app_path = Path(app_path_str)
    if not app_path.exists():
        print(f"SKIP: BMUX_APP_PATH does not exist: {app_path}")
        return 0

    bundle_id = _bundle_id(app_path)
    socket_path = Path(f"/tmp/bmux-restore-stress-{bundle_id.replace('.', '-')}.sock")
    snapshot = _snapshot_path(bundle_id)
    previous_snapshot = _snapshot_path(bundle_id, suffix="-previous")

    failures: list[str] = []

    with tempfile.TemporaryDirectory(prefix="bmux-restore-stress-") as td:
        ...
        _write_fake_agent(fake_bin_dir, "claude", "BMUX_FAKE_CLAUDE_RESUME")
        _write_fake_agent(fake_bin_dir, "codex", "BMUX_FAKE_CODEX_RESUME")
        _write_fake_agent(fake_bin_dir, "opencode", "BMUX_FAKE_OPENCODE_RESUME")
        launch_path = f"{fake_bin_dir}:{os.environ.get('PATH', '')}"
        app_env = {
            "PATH": launch_path,
            "BMUX_AGENT_HOOK_STATE_DIR": str(hook_state_dir),
            # Claude resume routes through the bmux claude wrapper, which
            # resolves the real binary; point it at the fake one instead.
            "BMUX_CUSTOM_CLAUDE_PATH": str(fake_bin_dir / "claude"),
        }
        ...
        try:
            ...
            try:
                ...
                if not previous_snapshot.exists():
                    ...
                    # The manual `bmux restore-session` recovery entrypoint must
                    ...
                    restore_env["BMUX_SOCKET_PATH"] = str(socket_path)
