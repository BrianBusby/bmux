...
it, bmux's injected Ghostty zsh integration must not keep surfacing hook errors on
every prompt redraw.
"""
...
def _capture_saturated_session(env: dict[str, str], zsh_path: str) -> bytes:
    def interact(master: int, output: bytearray) -> None:
        ...
        _send(master, "print __BMUX_READY__\n")
        ...
        _send(master, "print __BMUX_AFTER_FILL__\n")
        _read_available(master, output, time.time() + 1.0)
        for _ in range(3):
            _send(master, "\n")
...
def main() -> int:
    ...
    bmux_wrapper_dir = root / "Resources" / "shell-integration"
    ghostty_resources_dir = root / "ghostty" / "src"
    if not (bmux_wrapper_dir / ".zshenv").exists():
        print(f"SKIP: missing bmux zsh wrapper at {bmux_wrapper_dir}")
        return 0
    ...
    base = Path(tempfile.mkdtemp(prefix="bmux_ghostty_zsh_jobs_"))
    try:
        home = base / "home"
        _prepare_home(home)
        ...
        env = dict(os.environ)
        env["HOME"] = str(home)
        env["ZDOTDIR"] = str(bmux_wrapper_dir)
        env["BMUX_ZSH_ZDOTDIR"] = str(home)
        env["BMUX_SHELL_INTEGRATION"] = "1"
        env["BMUX_SHELL_INTEGRATION_DIR"] = str(bmux_wrapper_dir)
        env["BMUX_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        ...
        initial_env["BMUX_ZSH_ZDOTDIR"] = str(initial_home)
        initial_env["BMUX_ZSH_JOB_TABLE_SOFT_LIMIT"] = "1"
        initial_output = _capture_initial_prompt(initial_env, zsh_path)
        if b"__BMUX_READY__" not in initial_output:
        ...
        if b"__BMUX_AFTER_FILL__" not in output:
